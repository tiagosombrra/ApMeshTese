...
void deadline_timer_cancel_test() {
  ...
  for (int i = 10; i < 20; ++i) timers[i].t.async_wait(&timer_handler);

  BOOST_ASIO_CHECK(timers[2].t.cancel() == 1);
  BOOST_ASIO_CHECK(timers[41].t.cancel() == 1);
  for (int i = 10; i < 20; ++i) BOOST_ASIO_CHECK(timers[i].t.cancel() == 1);
}
...
struct custom_allocation_timer_handler {
  ...
  struct allocator {
    ...
    explicit allocator(int* count) BOOST_ASIO_NOEXCEPT : count_(count) {}
...
void deadline_timer_custom_allocation_test() {
  ...
  for (int i = 0; i < 50; ++i) timers[i].t.cancel();
...
void io_context_run(boost::asio::io_context* ioc) { ioc->run(); }
...
boost::asio::deadline_timer make_timer(boost::asio::io_context& ioc,
                                       int* count) {
...
BOOST_ASIO_TEST_SUITE("deadline_timer", BOOST_ASIO_TEST_CASE(null_test))
