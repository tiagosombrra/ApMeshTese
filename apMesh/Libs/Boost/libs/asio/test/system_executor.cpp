...
void increment(boost::asio::detail::atomic_count* count) { ++(*count); }

void system_executor_query_test() {
  BOOST_ASIO_CHECK(
      &boost::asio::query(system_executor(), boost::asio::execution::context) !=
      static_cast<const system_context*>(0));

  BOOST_ASIO_CHECK(
      boost::asio::query(system_executor(), boost::asio::execution::blocking) ==
  ...
  BOOST_ASIO_CHECK(boost::asio::query(system_executor(),
  ...
  BOOST_ASIO_CHECK(
      boost::asio::query(system_executor(), boost::asio::execution::mapping) ==
...
void system_executor_execute_test() {
  ...
  boost::asio::execution::execute(
      boost::asio::prefer(
          boost::asio::require(
              system_executor(), boost::asio::execution::blocking.never,
              boost::asio::execution::outstanding_work.untracked,
              boost::asio::execution::relationship.continuation),
...
BOOST_ASIO_TEST_SUITE("system_executor",
