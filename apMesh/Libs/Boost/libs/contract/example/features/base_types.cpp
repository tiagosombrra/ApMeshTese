...
class pushable {
 public:
  void invariant() const { BOOST_CONTRACT_ASSERT(capacity() <= max_size()); }
...
void pushable<T>::push_back(T x, boost::contract::virtual_* v) {
  ...
  boost::contract::check c =
      boost::contract::public_function(v, this)
          .precondition([&] { BOOST_CONTRACT_ASSERT(capacity() < max_size()); })
          .postcondition(
...
class unique_chars
 ...
 public:
  ...
  virtual void push_back(char x, boost::contract::virtual_* v = 0) {
    boost::contract::old_ptr<bool> old_find = BOOST_CONTRACT_OLDOF(v, find(x));
    boost::contract::old_ptr<unsigned> old_size =
        BOOST_CONTRACT_OLDOF(v, size());
    boost::contract::check c =
        boost::contract::public_function(v, this)
            .precondition([&] { BOOST_CONTRACT_ASSERT(!find(x)); })
            .postcondition([&] {
              if (!*old_find) {
...
#define BASES /* local macro (for convenience) */                        \
 private                                                                 \
  boost::contract::constructor_precondition<chars>, public unique_chars, \
      public virtual pushable<char>, virtual protected has_size,         \
      private has_empty
...
#undef BASES                                            // Undefine local macro.
  ...
  void invariant() const { BOOST_CONTRACT_ASSERT(empty() == (size() == 0)); }
  ...
  void push_back(char x, boost::contract::virtual_* v = 0) /* override */ {
    ...
    boost::contract::old_ptr<unsigned> old_size =
        BOOST_CONTRACT_OLDOF(v, size());
    boost::contract::check c =
        boost::contract::public_function<override_push_back>(
            v, &chars::push_back, this, x)
            .precondition([&] { BOOST_CONTRACT_ASSERT(find(x)); })
            .postcondition([&] {
