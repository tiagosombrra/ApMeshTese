 ...
 public:
  ...
  explicit iarray(unsigned max, unsigned count = 0)
      : boost::contract::constructor_precondition<iarray>(
            boost::bind(&iarray::constructor_pre, max, count)),
        values_(new int[max]),
        capacity_(max) {
    ...
    boost::contract::check c =
        boost::contract::constructor(this).old(old).postcondition(post);
  ...
  virtual ~iarray() {
    ...
    BOOST_LOCAL_FUNCTION_NAME(post)
    boost::contract::check c =
        boost::contract::destructor(this).old(old).postcondition(post);
  ...
  virtual void push_back(int value, boost::contract::virtual_* v = 0) {
    ...
    void BOOST_LOCAL_FUNCTION(const bind v, const bind this_, bind& old_size) {
