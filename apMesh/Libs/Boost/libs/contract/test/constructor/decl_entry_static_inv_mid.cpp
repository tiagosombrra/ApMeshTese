...
#define BOOST_CONTRACT_TEST_entry_inv 1
      ...
      BOOST_PP_IIF(BOOST_CONTRACT_TEST_entry_inv, true, false);
  {
    ...
    a aa;
    ok.str("");
    ok << ok_c() << ok_b() << ok_a()  // Test no a::static_inv so no failure.
       << ok_end();
    BOOST_TEST(out.eq(ok.str()));
  }
...
#endif
    ok.str("");
    ok << ok_c() << ok_b()  // Test b::static_inv failed.
#ifdef BOOST_CONTRACT_NO_ENTRY_INVARIANTS
       << ok_a() << ok_end()
#endif
        ;
      ...
      BOOST_PP_IIF(BOOST_CONTRACT_TEST_entry_inv, true, false);
  {
    ...
    a aa;
    ok.str("");
    ok << ok_c()  // Test no c::static_inv so no failure.
       << ok_b() << ok_a() << ok_end();
    BOOST_TEST(out.eq(ok.str()));
  }
...
#endif
    ok.str("");
    ok << ok_c() << ok_b()  // Test b::static_inv failed (as all did).
#ifdef BOOST_CONTRACT_NO_ENTRY_INVARIANTS
       << ok_a() << ok_end()
#endif
        ;
