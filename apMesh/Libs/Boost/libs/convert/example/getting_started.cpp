...
static void getting_started_example1() {
  ...
  try {
    ...
    int i1 =
        lexical_cast<int>("123");  // boost::lexical_cast standard deployment
    int i2 = convert<int>("123")
                 .value();  // boost::convert with the default converter
    int i3 = convert<int>("123", cnv)
                 .value();  // boost::convert with an explicit converter
    string s1 =
        lexical_cast<string>(123);  // boost::lexical_cast standard deployment
    string s2 = convert<string>(123)
                    .value();  // boost::convert with the default converter
    string s3 = convert<string>(123, cnv)
                    .value();  // boost::convert with an explicit converter
...
static void getting_started_example3() {
  ...
  int i2 =
      convert<int>("123", cnv2).value();  // Two times faster than lexical_cast.
  int i3 = convert<int>("123", cnv3)
               .value();  // Four times faster than lexical_cast.
                          //]
}
...
static void getting_started_example4() {
  ...
  string s2 =
      convert<string>(12.34567, cnv(std::fixed)(std::setprecision(3))).value();
  string s3 =
      convert<string>(12.34567, cnv(std::scientific)(std::setprecision(3)))
          .value();
  string expected = local::is_msc ? "1.235e+001" : "1.235e+01";

  BOOST_TEST(i2 ==
             123);  // boost::cnv::cstream. Successful conversion of "   123".
  BOOST_TEST(
      s1 == "12.34567");  // boost::lexical_cast. Precision is not configurable.
  BOOST_TEST(s2 ==
             "12.346");  // boost::cnv::cstream. Precision was set to 3. Fixed.
  BOOST_TEST(
      s3 ==
      expected);  // boost::cnv::cstream. Precision was set to 3. Scientific.
                  //]
}
...
static void getting_started_example5() {
  ...
  int i3 = convert<int>("uhm", cnv)
               .value_or(-1);  // Returns -1 when conversion fails.
...
static void getting_started_example6() {
  ...
  if (i1 == -1)
    log("bad i1"), i1 = default_i1;  // Log failure. Proceed with the default
  if (i2 == -1)
    log("bad i2"), i2 = default_i2;  // Log failure. Proceed with the default
...
static void getting_started_example7() {
  ...
  int i1 = convert<int>(s1, cnv(std::hex))
               .value_or(default_i1);  // If failed, proceed with the default
  int i2 = convert<int>(s2, cnv(std::dec))
               .value_or(default_i2);  // If failed, proceed with the default
  // ... proceed
  //]
  BOOST_TEST(i1 == 291);
  BOOST_TEST(i2 == 456);
...
static void getting_started_example9() {
  ...
  int i1 = convert<int>(s1).value_or_eval(
      std::bind(fallback_fun, "bad i1", default_i1));
  int i2 = convert<int>(s2).value_or_eval(
      std::bind(fallback_fun, "bad i2", default_i2));
  // ... proceed
  //]
  BOOST_TEST(i1 == 123);
  BOOST_TEST(i2 == 456);
