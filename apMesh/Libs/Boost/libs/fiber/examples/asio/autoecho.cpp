 ...
 *   message printing
 *****************************************************************************/
void print_(std::ostream& out) { out << '\n'; }
  ...
  try {
    for (;;) {
      ...
      std::size_t length = sock->async_read_some(
          boost::asio::buffer(data), boost::fibers::asio::yield[ec]);
      ...
      boost::asio::async_write(*sock, boost::asio::buffer(data, length),
 ...
 *   listening server
 *****************************************************************************/
void server(std::shared_ptr<boost::asio::io_context> const& io_ctx,
            tcp::acceptor& a) {
 ...
 *****************************************************************************/
void client(std::shared_ptr<boost::asio::io_context> const& io_ctx,
            tcp::acceptor& a, boost::fibers::barrier& barrier,
            unsigned iterations) {
  ...
  for (unsigned count = 0; count < iterations; ++count) {
    ...
    for (unsigned msg = 0; msg < 1; ++msg) {
      ...
      boost::system::error_code ec;
      boost::asio::async_write(s, boost::asio::buffer(message),
                               boost::fibers::asio::yield[ec]);
      if (ec == boost::asio::error::eof) {
      ...
      size_t reply_length =
          s.async_read_some(boost::asio::buffer(reply, max_length),
                            boost::fibers::asio::yield[ec]);
      if (ec == boost::asio::error::eof) {
        return;  // connection closed cleanly by peer
      } else if (ec) {
...
int main(int argc, char* argv[]) {
  try {
    ...
    for (unsigned i = 0; i < clients; ++i) {
      boost::fibers::fiber(client, io_ctx, std::ref(a), std::ref(b), iterations)
          .detach();
    }
    //]
    //[asio_rr_run
