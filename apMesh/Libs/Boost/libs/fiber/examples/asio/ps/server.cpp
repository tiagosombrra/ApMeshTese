...
class subscriptions {
 public:
  ...
  void subscribe(subscriber_session_ptr const& s) { subscribers_.insert(s); }

  // unsubscribe from this queue
  void unsubscribe(subscriber_session_ptr const& s) { subscribers_.erase(s); }
...
class registry : private boost::noncopyable {
 private:
  ...
  void publish_(std::string const& queue, std::string const& msg) {
    ...
    std::cout << "message '" << msg << "' to publish on queue '" << queue << "'"
              << std::endl;
  }
...
// a subscriber subscribes to a given queue in order to receive messages
// published on this queue
class subscriber_session
    : public std::enable_shared_from_this<subscriber_session> {
 public:
  explicit subscriber_session(
      std::shared_ptr<boost::asio::io_context> const& io_context, registry& reg)
      : socket_(*io_context), reg_(reg) {}
  ...
  void run() {
    ...
    try {
      ...
      std::cerr << "subscriber [" << queue << "] failed: " << e.what()
                << std::endl;
    }
    // close socket
    socket_.close();
...
void subscriptions::publish(std::string const& msg) {
  for (subscriber_session_ptr s : subscribers_) {
    s->publish(msg);
  }
}
...
class publisher_session
    : public std::enable_shared_from_this<publisher_session> {
 public:
  explicit publisher_session(
      std::shared_ptr<boost::asio::io_context> const& io_context, registry& reg)
      : socket_(*io_context), reg_(reg) {}
  ...
  void run() {
    ...
    try {
      ...
      std::cerr << "publisher [" << queue << "] failed: " << e.what()
                << std::endl;
    }
    // close socket
    socket_.close();
...
void accept_publisher(
    std::shared_ptr<boost::asio::io_context> const& io_context,
    unsigned short port, registry& reg) {
  // create TCP-acceptor
  ...
  for (;;) {
    ...
    // connection was established, after returning from this function a new
    // client (publisher) is connected
    acceptor.async_accept(new_publisher_session->socket(),
                          boost::fibers::asio::yield[ec]);
    if (!ec) {
      // run the new publisher in its own fiber (one fiber for one client)
      boost::fibers::fiber(
          std::bind(&publisher_session::run, new_publisher_session))
          .detach();
    }
  }
}
...
void accept_subscriber(
  ...
  for (;;) {
    ...
    // this function will suspend this execution context (fiber) until a
    // connection was established, after returning from this function a new
    // client (subscriber) is connected
    acceptor.async_accept(new_subscriber_session->socket(),
                          boost::fibers::asio::yield[ec]);
    if (!ec) {
      // run the new subscriber in its own fiber (one fiber for one client)
      boost::fibers::fiber(
          std::bind(&subscriber_session::run, new_subscriber_session))
          .detach();
    }
  }
}
...
int main(int argc, char* argv[]) {
  try {
    ...
    boost::fibers::fiber(accept_publisher, std::ref(io_context), 9997,
                         std::ref(reg))
        .detach();
    // create an acceptor for subscribers, run it as fiber
    boost::fibers::fiber(accept_subscriber, std::ref(io_context), 9998,
                         std::ref(reg))
        .detach();
    // dispatch
    io_context->run();
