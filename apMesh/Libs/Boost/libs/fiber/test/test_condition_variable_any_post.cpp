...
inline std::chrono::system_clock::time_point delay(int secs, int msecs = 0,
                                                   int /*nsecs*/ = 0) {
...
void condition_test_fiber(condition_test_data* data) {
  try {
    data->mutex.lock();
    while (!(data->notified > 0)) data->cond.wait(data->mutex);
    data->awoken++;
  } catch (...) {
  }
  data->mutex.unlock();
...
void wait_fn(boost::fibers::mutex& mtx,
             boost::fibers::condition_variable_any& cond) {
  mtx.lock();
  cond.wait(mtx);
  ++value1;
...
void test_one_waiter_notify_one() {
  ...
  boost::fibers::fiber f1(boost::fibers::launch::post, wait_fn, std::ref(mtx),
                          std::ref(cond));
  BOOST_CHECK_EQUAL(0, value1);

  boost::fibers::fiber f2(boost::fibers::launch::post, notify_one_fn,
                          std::ref(cond));
...
void test_two_waiter_notify_one() {
  ...
  boost::fibers::fiber f2(boost::fibers::launch::post, wait_fn, std::ref(mtx),
                          std::ref(cond));
  BOOST_CHECK_EQUAL(0, value1);

  boost::fibers::fiber f3(boost::fibers::launch::post, notify_one_fn,
                          std::ref(cond));
  BOOST_CHECK_EQUAL(0, value1);

  boost::fibers::fiber f4(boost::fibers::launch::post, notify_one_fn,
                          std::ref(cond));
  BOOST_CHECK_EQUAL(0, value1);
...
void test_two_waiter_notify_all() {
  ...
  boost::fibers::fiber f3(boost::fibers::launch::post, notify_all_fn,
                          std::ref(cond));
  BOOST_CHECK_EQUAL(0, value1);

  boost::fibers::fiber f4(boost::fibers::launch::post, wait_fn, std::ref(mtx),
                          std::ref(cond));
  BOOST_CHECK_EQUAL(0, value1);

  boost::fibers::fiber f5(boost::fibers::launch::post, notify_all_fn,
                          std::ref(cond));
  BOOST_CHECK_EQUAL(0, value1);
...
void fn2(boost::fibers::mutex& m, boost::fibers::condition_variable_any& cv) {
  ...
  int count = 0;
  while (test2 == 0 &&
         cv.wait_until(m, t) == boost::fibers::cv_status::no_timeout)
    count++;
...
void fn4(boost::fibers::mutex& m, boost::fibers::condition_variable_any& cv) {
  ...
  int count = 0;
  while (test2 == 0 &&
         cv.wait_for(m, ms(250)) == boost::fibers::cv_status::no_timeout)
    count++;
  std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
  if (runs == 0) {
    BOOST_CHECK(t1 - t0 < ms(250));
...
void do_test_condition_wait() {
  ...
  m.lock();
  boost::fibers::fiber f(boost::fibers::launch::post, &fn1, std::ref(m),
                         std::ref(cv));
  BOOST_CHECK(test1 == 0);
  while (test1 == 0) cv.wait(m);
  BOOST_CHECK(test1 != 0);
  test2 = 1;
  m.unlock();
  cv.notify_one();
...
void test_condition_wait() {
  boost::fibers::fiber(boost::fibers::launch::post, &do_test_condition_wait)
      .join();
...
void do_test_condition_wait_until() {
  ...
  {
    m.lock();
    boost::fibers::fiber f(boost::fibers::launch::post, &fn2, std::ref(m),
                           std::ref(cv));
    BOOST_CHECK(test1 == 0);
    while (test1 == 0) cv.wait(m);
    BOOST_CHECK(test1 != 0);
...
void test_condition_wait_until() {
  boost::fibers::fiber(boost::fibers::launch::post,
...
void do_test_condition_wait_until_pred() {
  ...
  {
    m.lock();
    boost::fibers::fiber f(boost::fibers::launch::post, &fn3, std::ref(m),
                           std::ref(cv));
    BOOST_CHECK(test1 == 0);
    while (test1 == 0) cv.wait(m);
    BOOST_CHECK(test1 != 0);
...
void do_test_condition_wait_for() {
  ...
  {
    m.lock();
    boost::fibers::fiber f(boost::fibers::launch::post, &fn4, std::ref(m),
                           std::ref(cv));
    BOOST_CHECK(test1 == 0);
    while (test1 == 0) cv.wait(m);
    BOOST_CHECK(test1 != 0);
...
void test_condition_wait_for() {
  boost::fibers::fiber(boost::fibers::launch::post, &do_test_condition_wait_for)
      .join();
...
void do_test_condition_wait_for_pred() {
  ...
  {
    m.lock();
    boost::fibers::fiber f(boost::fibers::launch::post, &fn5, std::ref(m),
                           std::ref(cv));
    BOOST_CHECK(test1 == 0);
    while (test1 == 0) cv.wait(m);
    BOOST_CHECK(test1 != 0);
