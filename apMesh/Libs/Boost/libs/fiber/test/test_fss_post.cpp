...
void fss() {
  ...
  std::cout << "fss_instances = " << fss_instances
            << "; fss_total = " << fss_total << "\n";
  std::cout.flush();
...
void fss_fiber_with_custom_cleanup() { fss_with_cleanup.reset(new Dummy); }
...
void test_fss_with_custom_cleanup() {
  boost::fibers::fiber(boost::fibers::launch::post, fss_with_custom_cleanup)
      .join();
}
...
boost::fibers::fiber_specific_ptr<dummy_class_tracks_deletions>
...
void fss_does_not_call_cleanup_after_ptr_destroyed() {
  boost::fibers::fiber(boost::fibers::launch::post, fiber_with_local_fss_ptr)
      .join();
  BOOST_CHECK(!fss_cleanup_called);
}
...
void test_fss_at_the_same_adress() {
  boost::fibers::fiber(boost::fibers::launch::post, fss_at_the_same_adress)
      .join();
}
...
boost::unit_test::test_suite* init_unit_test_suite(int, char*[]) {
  ...
  test->add(
      BOOST_TEST_CASE(test_fss_does_no_cleanup_with_null_cleanup_function));
  test->add(
      BOOST_TEST_CASE(test_fss_does_not_call_cleanup_after_ptr_destroyed));
