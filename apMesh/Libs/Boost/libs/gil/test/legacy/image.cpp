...
void gray_image_hist(GrayView const& img_view, R& hist) {
  for (auto it = img_view.begin(); it != img_view.end(); ++it) ++hist[*it];
...
struct mandelbrot_fn {
  ...
  result_type operator()(const point_t& p) const {
    ...
    for (std::size_t k = 0; k < num_channels<P>::value; ++k)
      ret[k] = (typename channel_type<value_type>::type)(
 ...
 private:
  double get_num_iter(const point<double>& p) const {
    ...
    for (int i = 0; i < MAX_ITER; ++i) {
      ...
      if (Z.x * Z.x + Z.y * Z.y > 4) return i / (double)MAX_ITER;
    }
    return 0;
  }
};
...
const string out_dir =
    in_dir + "image-out/";  // directory where to write output
