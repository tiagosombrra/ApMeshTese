...
int main() {
  ...
  using namespace boost::histogram::literals;  // _c suffix creates compile-time
                                               // numbers

  // make 2D weighted profile
  auto h = make_weighted_profile(axis::integer<>(0, 2), axis::integer<>(0, 2));

  // The mean is computed from the values marked with the sample() helper
  // function. Weights can be passed as well. The `sample` and `weight`
  // arguments can appear in any order, but they must be the first or last
  // arguments.
  ...
  assert(os.str() ==
         "index 0,0 mean 1.8 variance 0.5\n"
