...
class backward_bool_names : public std::numpunct<char> {
 ...
 protected:
  ...
  base_type::string_type do_truename() const { return "eurt"; }

  base_type::string_type do_falsename() const { return "eslaf"; }
};

void ios_flags_saver_unit_test() {
  std::stringstream ss;
  ...
  BOOST_TEST_EQ(std::ios_base::skipws | std::ios_base::dec, ss.flags());
  {
    boost::io::ios_flags_saver ifs(
        ss, std::ios_base::showbase | std::ios_base::internal);
    BOOST_TEST_EQ(std::ios_base::showbase | std::ios_base::internal,
                  ss.flags());
...
void ios_precision_saver_unit_test() {
  std::stringstream ss;
...
void ios_width_saver_unit_test() {
  std::stringstream ss;
...
void ios_iostate_saver_unit_test() {
  std::stringstream ss;
  ...
  BOOST_TEST(ss.good());
  {
    ...
    BOOST_TEST(ss.good());
    ss >> c;
    BOOST_TEST_EQ(std::ios_base::eofbit | std::ios_base::failbit, ss.rdstate());
    BOOST_TEST(ss.eof());
    BOOST_TEST(ss.fail());
    BOOST_TEST(!ss.bad());
  }
  ...
  BOOST_TEST(ss.good());
  {
    ...
    BOOST_TEST_EQ(std::ios_base::eofbit | std::ios_base::badbit, ss.rdstate());
    BOOST_TEST(ss.eof());
    BOOST_TEST(ss.fail());
    BOOST_TEST(ss.bad());
  }
...
void ios_exception_saver_unit_test() {
  std::stringstream ss;
...
void ios_rdbuf_saver_unit_test() {
  std::iostream s(NULL);
...
void ios_locale_saver_unit_test() {
...
void ios_iword_saver_unit_test(int index) {
  std::stringstream ss;
...
void ios_pword_saver_unit_test(int index) {
  std::stringstream ss;
...
void ios_base_all_saver_unit_test() {
  std::stringstream ss;
  BOOST_TEST_EQ(std::ios_base::skipws | std::ios_base::dec, ss.flags());
  BOOST_TEST_EQ(6, ss.precision());
  BOOST_TEST_EQ(0, ss.width());
  {
    ...
    BOOST_TEST_EQ(
        std::ios_base::unitbuf | std::ios_base::hex | std::ios_base::skipws,
        ss.flags());
...
void ios_word_saver_unit_test(int index) {
  std::stringstream ss;
  BOOST_TEST_EQ(0, ss.iword(index));
  BOOST_TEST(NULL == ss.pword(index));
  {
