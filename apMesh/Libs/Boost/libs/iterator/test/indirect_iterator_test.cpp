...
#if !defined(__SGI_STL_PORT) && (defined(BOOST_MSVC_STD_ITERATOR) ||      \
...
struct indirect_iterator_pair_generator {
  ...
  typedef boost::indirect_iterator<typename Container::iterator,
...
void more_indirect_iterator_tests() {
  ...
  typedef indirect_iterator_pair_generator<pointer_ra_container>
  ...
  boost::random_access_iterator_test(db + 1, store.size() - 1,
                                     boost::next(store.begin()));

  *db = 999;
  BOOST_TEST(store.front() == 999);

  // Borland C++ is getting very confused about the typedefs here
  typedef boost::indirect_iterator<iterator_set::iterator>
...
int main() {
  ...
  boost::shared_ptr<dummyT> zz(
      (dummyT*)0);  // Why? I don't know, but it suppresses a bad instantiation.
#endif
  ...
  // Concept checks
  {
    ...
    BOOST_STATIC_ASSERT(
        has_element_type<
            std::iterator_traits<shared_t::iterator>::value_type>::value);
  ...
  // Test indirect_iterator_generator
  {
    ...
    dummyT* ptr[N];
    for (int k = 0; k < N; ++k) ptr[k] = array + k;
    ...
    boost::random_access_iterator_test(
        boost::indirect_iterator<shared_t::iterator>(shared.begin()), N, array);

    boost::random_access_iterator_test(boost::make_indirect_iterator(ptr), N,
                                       array);
    ...
    boost::random_access_iterator_test(boost::make_indirect_iterator(const_ptr),
                                       N, array);
