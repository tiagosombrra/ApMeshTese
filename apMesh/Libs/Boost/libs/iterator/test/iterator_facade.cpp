...
class counter_iterator
    : public boost::iterator_facade<counter_iterator<Ref>, int const,
                                    boost::single_pass_traversal_tag, Ref> {
 public:
  counter_iterator() {}
...
struct input_iter
    : boost::iterator_facade<input_iter, value,
...
struct wrapper {
  T m_x;
  explicit wrapper(typename boost::call_traits<T>::param_type x) : m_x(x) {}
  template <class U>
...
struct iterator_with_proxy_reference
    : boost::iterator_facade<iterator_with_proxy_reference, wrapper<int>,
                             boost::incrementable_traversal_tag,
                             wrapper<int&> > {
  int& m_x;
  explicit iterator_with_proxy_reference(int& x) : m_x(x) {}

  void increment() {}
  wrapper<int&> dereference() const { return wrapper<int&>(m_x); }
};
...
struct abstract_iterator
    : boost::iterator_facade<abstract_iterator<I, A>,
                             A&
                             // In order to be value type as a reference,
                             // traversal category has to satisfy least forward
                             // traversal.
                             ,
                             boost::forward_traversal_tag, A&> {
  abstract_iterator(I iter) : iter(iter) {}

  void increment() { ++iter; }

  A& dereference() const { return *iter; }

  bool equal(abstract_iterator const& y) const { return iter == y.iter; }
...
int main() {
  ...
  {
    derived d(1);
    boost::readable_iterator_test(abstract_iterator<derived*, base>(&d),
                                  derived(1));
  }
