...
// we just declare a state type but do not create any instance as we want to
...
BOOST_MSM_EUML_ACTION(playing_internal_action){
    ...
    operator()(EVT const&, FSM&, SourceState&,
               TargetState&){cout << "Playing::internal action" << endl;
}  // namespace
}
;
// derive to be able to add an internal transition table
struct Playing_ : public Playing_def {
  BOOST_MSM_EUML_DECLARE_INTERNAL_TRANSITION_TABLE((internal_event /
                                                    playing_internal_action))
};
// choice of back-end
...
void test() {
  ...
  std::cout << "sending open_close event. Conflict with internal transitions "
               "(rejecting event)"
            << std::endl;
  p.process_event(open_close);
  pstate(p);
