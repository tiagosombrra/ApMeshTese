...
BOOST_MSM_EUML_STATE((Open_Entry, Open_Exit, attributes_ << no_attributes_,
                      configure_ << CDLoaded  // flag state with CDLoaded
                      ),
                     Open)

BOOST_MSM_EUML_STATE((Stopped_Entry, Stopped_Exit,
...
BOOST_MSM_EUML_STATE(
    (no_action, no_action, attributes_ << no_attributes_,
     configure_ << PlayingPaused
                << CDLoaded  // flag state with CDLoaded and PlayingPaused
     ),
    Paused)
...
BOOST_MSM_EUML_INTERRUPT_STATE((end_error, ErrorMode_Entry, ErrorMode_Exit),
                               ErrorMode)

// Playing is now a state machine itself.

// It has 3 substates
BOOST_MSM_EUML_STATE((Song1_Entry, Song1_Exit, attributes_ << no_attributes_,
...
BOOST_MSM_EUML_TRANSITION_TABLE(
    (Playing == Stopped + play / start_playback,
     ...
     // we now defer using the defer_ function. This will need deferred_events
     // as config (see below)
     Empty + play / defer_,
     //  +------------------------------------------------------------------------------+
     ...
     Stopped == Stopped + stop, ErrorMode == AllOk + error_found / report_error,
...
static char const* const state_names[] = {
    "Stopped", "Paused", "Open", "Empty", "Playing", "AllOk", "ErrorMode"};
...
void test() {
  ...
  std::cout << "CDLoaded active:" << std::boolalpha
            << p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(CDLoaded)>()
  ...
  std::cout << "PlayingPaused active:" << std::boolalpha
            << p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(PlayingPaused)>()
            << std::endl;  //=> true
  ...
  std::cout << "CDLoaded active:" << std::boolalpha
            << p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(CDLoaded)>()
            << std::endl;  //=> true
  // by default, the flags are OR'ed but you can also use AND. Then the flag
  // must be present in all of the active states
