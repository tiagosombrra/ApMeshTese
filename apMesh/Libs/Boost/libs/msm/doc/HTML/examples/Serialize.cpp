...
void test() {
  ...
  std::cout << "Empty's data should be 3:"
            << p2.get_state<player_::Empty&>().some_dummy_data << std::endl;
  std::cout << "front-end data should be 4:" << p2.front_end_data << std::endl;

  p2.process_event(open_close());
  pstate(p2);
  // will be rejected, wrong disk type
  p2.process_event(cd_detected("louie, louie", DISK_DVD));
  pstate(p2);
  p2.process_event(cd_detected("louie, louie", DISK_CD));
  pstate(p2);
