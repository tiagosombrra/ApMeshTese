...
BOOST_MSM_EUML_STATE((Started_Entry, no_action, attributes_ << m_counter),
                     Started)
...
BOOST_MSM_EUML_STATE((Ringing_Entry, no_action, attributes_ << m_ringing_cpt),
                     Ringing)

// external function
void do_ring(int ringing_time) {
  std::cout << "ringing " << ringing_time << " s" << std::endl;
}
...
BOOST_MSM_EUML_TRANSITION_TABLE(
    (
        //  +------------------------------------------------------------------------------+
        // When we start the countdown, the countdown value is not hardcoded but
        // contained in the start_timer event. We copy this value into Started
        Started ==
            Stopped + start_timer / (target_(m_counter) = event_(m_timer)),
        Stopped == Started + stop_timer,
        // internal transition
        Started +
            tick
                // we here use the message queue to move to Started when the
                // countdown is finished to do this we put start_ringing into
                // the message queue
                / if_then_((source_(m_counter) -= event_(m_tick)) <= Int_<0>(),
                           process_(start_ringing)),
        // when we start ringing, we give to the state its hard-coded ringing
        // time.
        ...
        // to change a bit, we now do not use the message queue but a transition
        // conflict to solve the same problem. When tick is fired, we have an
        // internal transition Ringing -> Ringing, as long as Counter > 0
        ...
        Stopped ==
            Ringing +
                tick[source_(m_ringing_cpt) - event_(m_tick) <= Int_<0>()],
        // we let the user manually stop the ringing by pressing any button
        Stopped == Ringing + stop_timer, Stopped == Ringing + start_timer
        //  +------------------------------------------------------------------------------+
        ),
    transition_table)
...
void test() {
  ...
  p.process_event(start_timer(5));
  pstate(p);  // timer set to 5 ticks
  p.process_event(tick(2));
  pstate(p);
  p.process_event(tick(1));
  pstate(p);
  p.process_event(tick(1));
  pstate(p);
  p.process_event(tick(1));
