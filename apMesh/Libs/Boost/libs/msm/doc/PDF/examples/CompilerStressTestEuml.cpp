...
struct RemoveDummy {
  bool operator()(const OneSong_impl& lhs) const {
    return (lhs.get_attribute(m_song).compare(
                std::string("She-Dummy. Remove this one")) == 0);
  }
};
template <int val>
struct LookFor {
...
BOOST_MSM_EUML_ACTION(SongDeleter){
    bool operator()(const OneSong_impl& lhs)
        const {return lhs.get_data() == "Twist and Shout";
}  // namespace
}
;
struct Generator {
...
struct Found_impl : euml_event<Found_impl>, FoundDef {
  Found_impl() {}
  Found_impl(const string& data) { get_attribute(m_song) = data; }
  int foo() const {
...
// this macro creates a functor and an eUML function wrapper. Now, foo_ can be
// used anywhere
...
struct some_base {
  ...
  int foobar(int i) const {
    std::cout << "foobar(int):" << i << std::endl;
    return 1;
  }
  int foobar(int i, int j) const {
    std::cout << "foobar(int,int):" << i << "," << j << std::endl;
    return 2;
  }
};
// some functions to call
...
BOOST_MSM_EUML_STATE(
    ((insert_(fsm_(m_tgt_container), end_(fsm_(m_tgt_container)),
              append_(event_(m_song),
                      fsm_(m_var7))),  // foo_(event_,Int_<0>()) ,
...
BOOST_MSM_EUML_STATE(
    (if_then_(
         (state_(m_src_it) != end_(fsm_(m_src_container)) &&
          ...
          // second_(associative_equal_range_(fsm_(m_var4),Int_<8>())) ==
          // associative_upper_bound_(fsm_(m_var4),Int_<8>()) &&
          // first_(associative_equal_range_(fsm_(m_var4),Int_<8>())) ==
          // associative_lower_bound_(fsm_(m_var4),Int_<8>())&&
          // second_(*associative_lower_bound_(fsm_(m_var5),Int_<0>())) ==
          // Int_<0>() && //map => pair as return
          // find_if_(begin_(fsm_(m_var4)),end_(fsm_(m_var4)),Predicate_<LookFor<8>
          // >()) != end_(fsm_(m_var4))&&
          //*lower_bound_(begin_(fsm_(m_var4)),end_(fsm_(m_var4)),Int_<9>(),Predicate_<std::less<int>
          //>()) == Int_<9>()&&
          //*upper_bound_(begin_(fsm_(m_var4)),end_(fsm_(m_var4)),Int_<8>(),Predicate_<std::less<int>
          //>()) == Int_<9>() &&
          ...
          // count_(begin_(fsm_(m_var4)),end_(fsm_(m_var4)),Int_<9>()) ==
          // Int_<1>()&&
          // count_if_(begin_(fsm_(m_var4)),end_(fsm_(m_var4)),Predicate_<LookFor<9>
          // >()) == Int_<1>()&&
          // distance_(begin_(fsm_(m_var4)),end_(fsm_(m_var4))) == Int_<2>()&&
          //*min_element_(begin_(fsm_(m_var4)),end_(fsm_(m_var4)),Predicate_<std::less<int>
          //>()) == Int_<8>()&&
          //*max_element_(begin_(fsm_(m_var4)),end_(fsm_(m_var4)),Predicate_<std::less<int>
          //>()) == Int_<9>()&&
          // adjacent_find_(begin_(fsm_(m_var4)),end_(fsm_(m_var4))) ==
          // end_(fsm_(m_var4))&&
         ...
         (process2_(OneSong,
                    *(state_(m_src_it)++)) /*,foobar_(fsm_,Int_<0>())*/)),
     no_action, attributes_ << m_src_it, configure_ << SomeFlag),
    Foreach)
...
BOOST_MSM_EUML_TRANSITION_TABLE(
    (StringFind == Foreach + OneSong[if_then_else_(true_, true_, true_)],
     ...
     Foreach == StringFind + NotFound, Foreach == Insert + Done
     //  +------------------------------------------------------------------------------+
     ),
    transition_table)
...
BOOST_MSM_EUML_DECLARE_STATE_MACHINE(
    (transition_table,  // STT
     ...
     (insert_(state_(m_var4), Int_<5>()), insert_(state_(m_var4), Int_<6>()),
      insert_(state_(m_var4), Int_<7>()), insert_(state_(m_var4), Int_<8>()),
      insert_(state_(m_var4), Int_<9>()),
      associative_erase_(state_(m_var4), Int_<6>()),
      associative_erase_(state_(m_var4), begin_(state_(m_var4))),
      associative_erase_(state_(m_var4), begin_(state_(m_var4)),
                         ++begin_(state_(m_var4))),
      insert_(state_(m_var2), begin_(state_(m_var2)), begin_(state_(m_var3)),
              end_(state_(m_var3))),
      state_(m_var5)[Int_<0>()] = Int_<0>(),
      state_(m_var5)[Int_<1>()] = Int_<1>(),
      attribute_(substate_(Foreach, fsm_),
                 m_src_it) = begin_(fsm_(m_src_container))
      ...
      //,replace_copy_if_(begin_(state_(m_var4)),end_(state_(m_var4)),begin_(state_(m_var4)),Predicate_<LookFor<9>
      //>(),Int_<8>())
      //,replace_(begin_(state_(m_var4)),end_(state_(m_var4)),Int_<8>(),Int_<7>())
      //,replace_if_(begin_(state_(m_var4)),end_(state_(m_var4)),Predicate_<LookFor<9>
      //>(),Int_<8>())
     ...
     //(list_remove_(state_(m_var2),Int_<3>()),list_merge_(state_(m_var2),state_(m_var3),Comparator())),//no_action,
     //// Entry
     no_action,                      // Exit
     attributes_ << m_src_container  // song list
                 << m_tgt_container  // result
                 << m_var2 << m_var3 << m_var4 << m_var5 << m_var6 << m_var7
                 << m_var8 << m_var9 << m_var10,
     configure_ << no_configure_, Log_No_Transition),
    iPodSearch_helper)
...
void test() {
  iPodSearch search;
  // fill our song list
  // search.get_attribute<m_src_container>().push_back(OneSong("She-Dummy.
  // Remove this one"));
  search.get_attribute(m_src_container).push_back(OneSong_impl("Let it be"));
  search.get_attribute(m_src_container)
  ...
  // BOOST_MSM_EUML_STATE_NAME returns the name of the event type of which
  // StringFind is an instance
  search.get_state<BOOST_MSM_EUML_STATE_NAME(StringFind)&>().get_attribute(
      m_letters) = "Sh";
  // needed to start the highest-level SM. This will call on_entry and mark the
  // start of the SM
  search.start();
  // display all the songs
  for (list<OneSong_impl>::const_iterator it =
           search.get_attribute(m_tgt_container).begin();
