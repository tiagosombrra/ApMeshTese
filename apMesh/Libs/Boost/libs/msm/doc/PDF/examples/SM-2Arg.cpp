...
struct player_
  ...
  struct Empty : public msm::front::state<my_visitable_state> {
    ...
    // every (optional) entry/exit methods get the event packed as boost::any.
    // Not useful very often.
    template <class Event, class FSM>
    void on_entry(Event const&, FSM&) {
    ...
    void accept(SomeVisitor& vis, int i) const { vis.visit_state(this, i); }
  };
  ...
  // also possible: AlwaysHistory, the last active state will always be
  // reactivated or NoHistory, always restart from the initial state
  struct Playing_
      : public msm::front::state_machine_def<Playing_, my_visitable_state> {
    ...
    // The list of FSM states
    // the Playing state machine contains a state which is himself a state
    // machine so we have a SM containing a SM containing a SM
    struct Song1_
        : public msm::front::state_machine_def<Song1_, my_visitable_state> {
      ...
      struct LightOn : public msm::front::state<my_visitable_state> {
        template <class Event, class FSM>
        void on_entry(Event const&, FSM&) {
          std::cout << "starting: LightOn" << std::endl;
        }
        template <class Event, class FSM>
        void on_exit(Event const&, FSM&) {
          std::cout << "finishing: LightOn" << std::endl;
        }
      };
      struct LightOff : public msm::front::state<my_visitable_state> {
        template <class Event, class FSM>
        void on_entry(Event const&, FSM&) {
          std::cout << "starting: LightOff" << std::endl;
        }
        template <class Event, class FSM>
        void on_exit(Event const&, FSM&) {
          std::cout << "finishing: LightOff" << std::endl;
        }
      };
      // the initial state. Must be defined
      typedef LightOn initial_state;
      // transition actions
      void turn_light_off(ThreeSec const&) {
        std::cout << "3s off::turn light off\n";
      }
      // guard conditions
  ...
  // the player state machine contains a state which is himself a state machine
  // (2 of them, Playing and Paused)
  struct Paused_
      : public msm::front::state_machine_def<Paused_, my_visitable_state> {
    typedef mpl::vector2<PlayingPaused, CDLoaded> flag_list;
    template <class Event, class FSM>
    void on_entry(Event const&, FSM&) {
    ...
    struct StartBlinking : public msm::front::state<my_visitable_state> {
      template <class Event, class FSM>
      void on_entry(Event const&, FSM&) {
        std::cout << "starting: StartBlinking" << std::endl;
      }
      template <class Event, class FSM>
      void on_exit(Event const&, FSM&) {
        std::cout << "finishing: StartBlinking" << std::endl;
      }
    };
    struct StopBlinking : public msm::front::state<my_visitable_state> {
      template <class Event, class FSM>
      void on_entry(Event const&, FSM&) {
        std::cout << "starting: StopBlinking" << std::endl;
      }
      template <class Event, class FSM>
      void on_exit(Event const&, FSM&) {
        std::cout << "finishing: StopBlinking" << std::endl;
      }
    };
    // the initial state. Must be defined
    typedef StartBlinking initial_state;
    // transition actions
    void start_blinking(TenSec const&) {
      std::cout << "Paused::start_blinking\n";
    }
    void stop_blinking(TenSec const&) {
      std::cout << "Paused::stop_blinking\n";
    }
    // guard conditions
  ...
  void report_error(error_found const&) {
    std::cout << "player::report_error\n";
  }
  void report_end_error(end_error const&) {
    std::cout << "player::report_end_error\n";
  }
  // guard conditions
...
void test() {
  ...
  p.process_event(end_pause());
  pstate(p);
  // test of an event from a state to itself. According to UML spec, call again
  // exit/entry from Stopped
  ...
  std::cout << "CDLoaded active with AND:" << std::boolalpha
            ...
            << std::endl;  //=> false
  std::cout << "CDLoaded active with OR:" << std::boolalpha
            << p.is_flag_active<CDLoaded, player::Flag_OR>()
            << std::endl;  //=> true

  // go back to Playing
  // but play is not leading to Shallow History => do not remember the last
  // active State (SecondSong) and activate again FirstSong and LightOn
  p.process_event(play());
  pstate(p);
