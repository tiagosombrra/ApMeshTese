...
BOOST_AUTO_TEST_CASE(my_test) {
  ...
  p2.start();
  BOOST_CHECK_MESSAGE(
      p2.get_state<BOOST_MSM_EUML_STATE_NAME(Empty)&>().get_attribute(
          entry_counter) == 1,
      "Empty entry not called correctly");

  p2.process_event(open_close());
  BOOST_CHECK_MESSAGE(p2.current_state()[0] == 2,
  ...
  BOOST_CHECK_MESSAGE(
      p2.get_state<BOOST_MSM_EUML_STATE_NAME(Open)&>().get_attribute(
          entry_counter) == 1,
