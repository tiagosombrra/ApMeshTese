...
struct player_ : public msm::front::state_machine_def<player_> {
  ...
  struct transition_table
      : mpl::vector<
            ...
            //    +---------+-------------+---------+---------------------+----------------------+
            Row<State11, event1, State12>, Row<State12, event2, State13>,

            Row<State21, event3, State22>, Row<State22, eventd, State21>
            //    +---------+-------------+---------+---------------------+----------------------+
            > {};
...
BOOST_AUTO_TEST_CASE(TestDeferIn2Regions) {
  ...
  BOOST_CHECK_MESSAGE(p.get_state<player_::State11&>().exit_counter == 1,
                      "State11 exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State11&>().entry_counter == 1,
                      "State11 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State12&>().entry_counter == 1,
                      "State12 entry not called correctly");
  ...
  BOOST_CHECK_MESSAGE(p.get_state<player_::State12&>().entry_counter == 1,
                      "State12 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State12&>().exit_counter == 0,
                      "State12 exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State21&>().exit_counter == 0,
                      "State21 exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State21&>().entry_counter == 1,
                      "State21 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State22&>().exit_counter == 0,
                      "State22 exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State22&>().entry_counter == 0,
                      "State22 entry not called correctly");

  p.process_event(event3());
  BOOST_CHECK_MESSAGE(p.current_state()[0] == 1, "State12 should be active");
  BOOST_CHECK_MESSAGE(p.current_state()[1] == 2, "State21 should be active");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State21&>().exit_counter == 1,
                      "State21 exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State21&>().entry_counter == 2,
                      "State21 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State22&>().exit_counter == 1,
                      "State22 exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State22&>().entry_counter == 1,
                      "State22 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_deferred_queue().size() == 1,
                      "Deferred queue should have one element");
  ...
  BOOST_CHECK_MESSAGE(p.get_state<player_::State22&>().entry_counter == 1,
                      "State22 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_deferred_queue().size() == 0,
                      "Deferred queue should have no element");
}
}  // namespace
