...
//
// Let's start with the simpliest case of an UDT which supports standard
// conversions
//
struct Double {
...
void simplest_case() {
  //
  // conversion_traits<>::udt_builtin_mixture works out of the box as long as
  // boost::is_arithmetic<UDT> yields false
  //
  BOOST_CHECK((conversion_traits<double, Double>::udt_builtin_mixture::value ==
               udt_to_builtin));
  BOOST_CHECK((conversion_traits<Double, double>::udt_builtin_mixture::value ==
               builtin_to_udt));
  BOOST_CHECK((conversion_traits<Double, Double>::udt_builtin_mixture::value ==
  ...
  // BY DEFINITION, a conversion from UDT to Builtin is subranged. No attempt is
  // made to actually compare ranges.
  ...
  // are also supported out-of-the-box by converter<> in its default
  // configuration.
  //
  ...
  //
  // Range checking is disabled by default if an UDT is either the source or
...
// numeric_limits<UDT>::is_integer/is_signed to generate the proper float_in and
// sign mixtures. In most implementations, is_integer/is_signed are both false
// for UDTs if there is no explicit specialization for it. Therefore, the
// converter<> will see any UDT for which numeric_limits<> is not specialized as
// Float AND unsigned. Signess is used in the converter<> for range checking,
// but range checking is disabled by default for UDTs, so, normally, signess is
// mostly irrelevant as far as the library is concerned, except for the
// numeric_traits<>::sign_mixture entry. is_integer, however, is relevant in
// that if the conversion is from a float type to an integer type, the
// conversion is "rounding" and the rounder policies will participate. ALL
// implemented rounder policies require proper definitions for floor(udt) and
// ceil(udt). These names will be searched for using ADL, so, if you need to
// convert TO integral types from a UDT, you need to supply those functions
// along with the UDT in right namespace (that is, any namespace that allows ADL
// to find them)
...
void rounding() { BOOST_CHECK(numeric_cast<int>(Dv) == static_cast<int>(Dv)); }

//
// If your UDT can't or won't provide floor/ceil you can set-up and use your own
// Float2IntRounder policy (though doing this is not always required as shown so
// far)
//
struct DoubleToInt {
  static Double nearbyint(Double const& s) {
...
void custom_rounding() {
  typedef converter<int, Double, conversion_traits<int, Double>,
                    void  // By default UDT disable range checking so this won't
                          // be used
                    ,
                    DoubleToInt>
...
//
// In the next Level of complexity, your UDTs might not support conversion
// operators
//
struct Float {
  Float(float v) : mV(v) {}
...
namespace numeric {
//
// Though static_cast<> won't work with them you can still use numeric_cast<> by
// specializing raw_converter as follows:
//
template <>
struct raw_converter<Float2IntTraits> {
...
template <>
struct raw_converter<Int2FloatTraits> {
...
//
// Alterntively, the custom raw_converter classes can be defined
// non-instrusively (not as specializations) and passed along as policies
//
...
void custom_raw_converter2() {
  ...
  typedef converter<Int, Float, Float2IntTraits,
                    void  // By default UDT disable range checking so this won't
                          // be used
                    ,
                    void  // Float2Int Rounder won't be used if Int isn't marked
                          // as integer via numeric_limits<>
                    ,
