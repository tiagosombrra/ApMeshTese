...
void det(I (&mat)[size][size]) {
  for (int i = 0; i < size; i++)
    for (int j = 0; j < size; j++) mat[i][j] = I(1) / I(i + j + 1);
...
struct variants {
  ...
  typedef
      typename change_rounding<I_op, save_state<rounded_arith_std<T> > >::type
          I_sp;
...
bool test() {
  ...
  det(mat_sp);
  {
    types::I_op::traits_type::rounding rnd;
    det(mat_ou);
  }
  {
    types::I_sp::traits_type::rounding rnd;
    det(mat_su);
  }
  for (int i = 0; i < size; i++)
