...
void rhs_m(const mp_50 x, mp_50 &dxdt,
           const mp_50 t) {  // version for multiprecision
  dxdt = mp_50(3) / (mp_50(2) * t * t) + x / (mp_50(2) * t);
}

void rhs_d(const double x, double &dxdt,
           const double t) {  // version for double precision
  dxdt = 3.0 / (2.0 * t * t) + x / (2.0 * t);
}

// state_type = mp_50 = deriv_type = time_type = mp_50
typedef runge_kutta4<mp_50, mp_50, mp_50, mp_50, vector_space_algebra,
                     default_operations, never_resizer>
    stepper_type_m;

typedef runge_kutta4<double, double, double, double, vector_space_algebra,
                     default_operations, never_resizer>
    stepper_type_d;
