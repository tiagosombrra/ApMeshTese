...

{
  ...
  nt2::tie(cos_sum, sin_sum) =
      nt2::tie(nt2::mean(nt2::cos(x)), nt2::mean(nt2::sin(x)));
...
struct phase_ensemble {
  typedef
      typename boost::dispatch::meta::as_integer<T, unsigned>::type int_type;
...
struct statistics_observer {
  ...
  T get_K_mean(void) const {
    return (m_count != 0) ? m_K_mean / T(m_count) : 0.0;
  }
...
struct test_ode_table {
  ...
  void operator()() {
    for (T epsilon = 0.0; epsilon < 5.0; epsilon += 0.1) {
      ...
      integrate_const(runge_kutta4<array_type, T>(), boost::ref(ensemble), x,
                      T(0.0), T(10.0), dt);

      // integrate and compute the statistics
      integrate_const(runge_kutta4<array_type, T>(), boost::ref(ensemble), x,
                      T(0.0), T(100.0), dt, boost::ref(obs));
      cout << epsilon << "\t" << obs.get_K_mean() << endl;
    }
  }

  friend std::ostream &operator<<(std::ostream &os,
                                  test_ode_table<T> const &p) {
    return os << "(" << p.size() << ")";
  }
