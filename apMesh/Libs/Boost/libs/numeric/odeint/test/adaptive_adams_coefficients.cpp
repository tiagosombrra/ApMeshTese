...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_step, step_type, vector_of_steps) {
  ...
  typedef detail::adaptive_adams_coefficients<steps, deriv_type, time_type>
      aac_type;
  ...
  for (size_t i = 0; i < steps; ++i) {
    ...
    if (coeff.m_eo < steps) coeff.m_eo++;
  }
...
BOOST_AUTO_TEST_CASE(test_copy) {
  ...
  typedef detail::adaptive_adams_coefficients<3, deriv_type, time_type>
      aac_type;
  aac_type c1;
