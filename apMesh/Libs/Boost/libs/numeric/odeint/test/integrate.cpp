...
struct perform_integrate_const_test {
  void operator()(const value_type t_end, const value_type dt) {
    ...
    size_t steps = integrate_const(Stepper(), lorenz, x, 0.0, t_end, dt,
    ...
    std::cout << static_cast<int>(floor(t_end / dt)) << " , " << t_end / dt
              << std::endl;
...
struct perform_integrate_adaptive_test {
  void operator()(const value_type t_end = 10.0, const value_type dt = 0.03) {
    // std::cout << "Testing integrate_adaptive with " << typeid( Stepper
    // ).name() << std::endl;
    ...
    // dt << " , " << 10.0+t_end << "=" << x_end[0] << std::endl;
...
struct perform_integrate_times_test {
  void operator()(const int n = 10, const int dn = 1,
    ...
    for (int i = 0; boost::numeric::odeint::detail::less_with_sign(
             static_cast<double>(i), static_cast<double>(obs_times.size()), dt);
         i += dn) {
      obs_times[i] = i;
    }
    // simple stepper
...
struct perform_integrate_n_steps_test {
  void operator()(const int n = 200, const value_type dt = 0.01) {
    // std::cout << "Testing integrate_n_steps with " << typeid( Stepper
    // ).name() << ". "; std::cout << "dt=" << dt << std::endl;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(integrate_n_steps_test_case, Stepper,
  ...
  if (!boost::is_same<
          Stepper,
          controlled_adams_bashforth_moulton<
              adaptive_adams_bashforth_moulton<3, state_type> > >::value &&
      !boost::is_same<
          Stepper,
          controlled_adams_bashforth_moulton<
              adaptive_adams_bashforth_moulton<5, state_type> > >::value) {
