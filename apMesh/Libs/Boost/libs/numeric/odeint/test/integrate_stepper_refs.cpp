...
class controlled_stepper_nc : boost::noncopyable {
 public:
  ...
  template <class System>
  controlled_step_result try_step(System system, state_type &in, time_type &t,
                                  time_type &dt) {
...
class dense_out_stepper_nc : boost::noncopyable {
 public:
  ...
  void initialize(const state_type &x0, const time_type t0,
                  const time_type dt0) {
    m_x = x0;
    m_t = t0;
    m_dt = dt0;
  ...
  void calc_state(const time_type t_inter, state_type &x) { x = m_x; }

  const state_type &current_state() const { return m_x; }
...
struct perform_integrate_const_test {
  void operator()() {
    ...
    integrate_const(boost::ref(stepper), lorenz, x, 0.0, 1.0, 0.1,
...
struct perform_integrate_adaptive_test {
  void operator()() {
    ...
    integrate_adaptive(boost::ref(stepper), lorenz, x, 0.0, 1.0, 0.1,
                       push_back_time(times, x_end));
  }
};
...
struct perform_integrate_n_steps_test {
  void operator()() {
    ...
    integrate_n_steps(boost::ref(stepper), lorenz, x, 0.0, 0.1, 10,
                      push_back_time(times, x_end));
  }
};
...
struct perform_integrate_times_test {
  void operator()() {
    ...
    integrate_times(
        boost::ref(stepper), lorenz, x, boost::counting_iterator<int>(0),
        boost::counting_iterator<int>(10), 0.1, push_back_time(times, x_end));
  }
};

class stepper_methods : public mpl::vector<simple_stepper_nc<state_type>,
