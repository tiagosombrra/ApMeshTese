...
class complete_steppers
    : public mpl::vector<symplectic_euler<container_type>,
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_integrate_const, Stepper,
                              complete_steppers) {
  ...
  BOOST_CHECK_CLOSE(q[0], std::cos(1.0),
                    100 * std::pow(0.1, Stepper::order_value));
}
