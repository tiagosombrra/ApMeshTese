...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_iterator_factory, Stepper,
  ...
  std::for_each(make_n_step_time_iterator_begin(stepper, boost::ref(system), x,
                                                0.0, 0.1, 10),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_range, Stepper, dummy_steppers) {
  ...
  boost::for_each(
      make_n_step_time_range(stepper, boost::ref(system), x, 0.0, 0.1, 10),
      dummy_observer());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_iterator_with_reference_wrapper_factory,
  ...
  std::for_each(
      make_n_step_time_iterator_begin(boost::ref(stepper), boost::ref(system),
                                      x, 0.0, 0.1, 10),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_range_with_reference_wrapper, Stepper,
  ...
  boost::for_each(make_n_step_time_range(boost::ref(stepper),
                                         boost::ref(system), x, 0.0, 0.1, 10),
                  dummy_observer());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(transitivity1, Stepper, dummy_steppers) {
  typedef n_step_time_iterator<Stepper, empty_system, state_type>
      stepper_iterator;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_factory, Stepper,
  ...
  state_type x = {{1.0}};
  result_vector res;
  std::copy(make_n_step_time_iterator_begin(Stepper(), empty_system(), x, 0.0,
                                            0.1, 3),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_range_factory, Stepper,
  ...
  result_vector res;
  boost::range::copy(
      make_n_step_time_range(Stepper(), empty_system(), x, 0.0, 0.1, 3),
      std::back_insert_iterator<result_vector>(res));
