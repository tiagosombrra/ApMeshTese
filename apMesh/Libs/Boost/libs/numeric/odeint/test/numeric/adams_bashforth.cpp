...
struct perform_adams_bashforth_test {
  void operator()(void) {
    ...
    double t = 0.0;
    double dt = 0.2;
    // initialization, does a number of steps already to fill internal buffer, t
    // is increased
    ...
    stepper.do_step(osc(), x1, t, dt);
    // only examine the error of the adams-bashforth step, not the
    // initialization
    const double f = 2.0 * std::abs(A * sin(t + dt + phi) - x1[0]) /
    ...
    while (f * std::pow(dt, o) > 1E-16) {
      ...
      std::cout << "Testing dt=" << dt << " , "
                << std::abs(A * sin(t + dt + phi) - x1[0]) << std::endl;
      BOOST_CHECK_LT(std::abs(A * sin(t + dt + phi) - x1[0]),
                     f * std::pow(dt, o));
      dt *= 0.5;
    }
  }
...
typedef mpl::vector<
    adams_bashforth<2, state_type>, adams_bashforth<3, state_type>,
    adams_bashforth<4, state_type>, adams_bashforth<5, state_type>,
    adams_bashforth<6, state_type>, adams_bashforth<7, state_type>,
    adams_bashforth<8, state_type> >
    adams_bashforth_steppers;

BOOST_AUTO_TEST_CASE_TEMPLATE(adams_bashforth_test, Stepper,
                              adams_bashforth_steppers) {
