/*
 ...
 It's one of the main tests of odeint. [end_description]
...
void check_stepper_concept(Stepper &stepper, System system,
  ...
  stepper.do_step(system, x, static_cast<time_type>(0.0),
...
struct perform_stepper_test<Stepper, std::vector<T> > {
  ...
  void operator()(void) {
    ...
    check_stepper_concept(stepper,
                          constant_system_standard<vector_type, vector_type,
                                                   typename Stepper::time_type>,
                          x);
#else
    check_stepper_concept(stepper, boost::cref(sys), x);
...
struct perform_stepper_test<Stepper, boost::array<T, 1> > {
  ...
  void operator()(void) {
    ...
    // dont run this for MSVC due to compiler bug 697006
    check_stepper_concept(stepper,
                          constant_system_standard<array_type, array_type,
                                                   typename Stepper::time_type>,
                          x);
#else
    check_stepper_concept(stepper, boost::cref(sys), x);
...
typedef mpl::copy<
    ...
    mpl::inserter<mpl::vector0<>,
                  mpl::insert_range<mpl::_1, mpl::end<mpl::_1>,
                                    stepper_methods1<mpl::_2> > > >::type
    stepper_combinations1;

typedef mpl::copy<
    container_types,
    mpl::inserter<mpl::vector0<>,
                  mpl::insert_range<mpl::_1, mpl::end<mpl::_1>,
                                    stepper_methods2<mpl::_2> > > >::type
    stepper_combinations2;
