...
BOOST_AUTO_TEST_CASE(test_step_adjuster) {
  ...
  default_step_adjuster<double, double> limited_adjuster(dt);
...
void test_explicit_stepper(Stepper stepper, const double max_dt) {
  ...
  integrate_adaptive(stepper, damped_osc, x, 0.0, steps * max_dt, max_dt,
  ...
  integrate_adaptive(stepper, damped_osc, x, 0.0, steps * max_dt, 10 * max_dt,
                     push_back_time(times));
...
BOOST_AUTO_TEST_CASE(test_controlled) {
  ...
  test_explicit_stepper(
      make_controlled(1E-2, 1E-2, max_dt, runge_kutta_dopri5<state_type>()),
      max_dt);
  test_explicit_stepper(
      make_controlled(1E-2, 1E-2, -max_dt, runge_kutta_dopri5<state_type>()),
      -max_dt);
  ...
  test_explicit_stepper(
      bulirsch_stoer<state_type>(1E-2, 1E-2, 1.0, 1.0, max_dt), max_dt);
  test_explicit_stepper(
      bulirsch_stoer<state_type>(1E-2, 1E-2, 1.0, 1.0, -max_dt), -max_dt);
}
...
BOOST_AUTO_TEST_CASE(test_dense_out) {
  const double max_dt = 0.01;
  test_explicit_stepper(
      make_dense_output(1E-2, 1E-2, max_dt, runge_kutta_dopri5<state_type>()),
      max_dt);
  test_explicit_stepper(
      make_dense_output(1E-2, 1E-2, -max_dt, runge_kutta_dopri5<state_type>()),
      -max_dt);
  ...
  test_explicit_stepper(
      bulirsch_stoer_dense_out<state_type>(1E-2, 1E-2, 1, 1, max_dt), max_dt);
...
struct osc_jacobi {
  void operator()(const vector_type &x, matrix_type &jacobi,
                  const value_type &t, vector_type &dfdt) const {
    jacobi(0, 0) = 0;
    jacobi(0, 1) = 1;
    jacobi(1, 0) = -1;
    jacobi(1, 1) = 0;
...
void test_rosenbrock_stepper(Stepper stepper, const double max_dt) {
  ...
  integrate_adaptive(stepper, std::make_pair(osc_rhs(), osc_jacobi()), x, 0.0,
                     steps * max_dt, max_dt, push_back_time(times));
  ...
  // this should also work when we provide some bigger initial dt
  integrate_adaptive(stepper, std::make_pair(osc_rhs(), osc_jacobi()), x, 0.0,
                     steps * max_dt, 10 * max_dt, push_back_time(times));
...
BOOST_AUTO_TEST_CASE(test_controlled_rosenbrock) {
  ...
  test_rosenbrock_stepper(
      make_controlled(1E-2, 1E-2, max_dt, rosenbrock4<value_type>()), max_dt);
  test_rosenbrock_stepper(
      make_controlled(1E-2, 1E-2, -max_dt, rosenbrock4<value_type>()), -max_dt);
}
...
BOOST_AUTO_TEST_CASE(test_dense_out_rosenbrock) {
  ...
  test_rosenbrock_stepper(
      make_dense_output(1E-2, 1E-2, max_dt, rosenbrock4<value_type>()), max_dt);
  test_rosenbrock_stepper(
      make_dense_output(1E-2, 1E-2, -max_dt, rosenbrock4<value_type>()),
      -max_dt);
}
