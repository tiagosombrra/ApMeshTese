...
typedef mpl::vector<euler<double>, runge_kutta4<double>,
                    // with floats all four parameters have to be given
                    // explicitly to override default double
                    euler<float, float, float, float>,
                    runge_kutta4<float, float, float, float> >::type
    stepper_types;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_do_step, T, stepper_types) {
  ...
  BOOST_CHECK_CLOSE(
      x, 0.1,
      100 * std::numeric_limits<typename stepper_type::state_type>::epsilon());
...
typedef mpl::vector<runge_kutta_cash_karp54<double>, runge_kutta_dopri5<double>,
                    // with floats all four parameters have to be given
                    // explicitly to override default double
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_integrate, T, error_stepper_types) {
  ...
  typename stepper_type::time_type dt = 0.1;
  integrate_adaptive(make_controlled<stepper_type>(1e-6, 1e-6),
                     constant_system(), x, t0, t1, dt);
  BOOST_CHECK_CLOSE(
      x, 1.0,
      100 * std::numeric_limits<typename stepper_type::state_type>::epsilon());
}
