...
struct stepper {
  typedef runge_kutta_dopri5<State, double, State, double, vector_space_algebra>
      type;
};
...
struct dense_output_stepper {
  typedef dense_output_runge_kutta<typename controlled_stepper<State>::type>
      type;
};
...
BOOST_AUTO_TEST_CASE(compile_time_matrix) {
  ...
  // vector_space_algebra > rk4; rk4.do_step( sys() , x , 0.0 , 0.1 );
