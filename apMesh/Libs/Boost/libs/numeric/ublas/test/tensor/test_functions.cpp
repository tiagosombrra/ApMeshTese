...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_prod_vector, value, test_types,
                                 fixture) {
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_prod_matrix, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_prod_tensor_1, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  for (auto const& na : extents) {
    ...
    for (auto q = 0ul; q <= pa; ++q) {  // pa
      ...
      for (auto i = 0ul; i < q; ++i) acc *= a.extents().at(phi.at(i) - 1);
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_prod_tensor_2, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  auto permute_extents = [](auto const& pi, auto const& na) {
    ...
    for (auto j = 0u; j < pi.size(); ++j) nb[pi[j] - 1] = na[j];
    return nb;
  };
  ...
  for (auto const& na : extents) {
    ...
    for (auto f = 0ul; f < fac; ++f) {
      ...
      for (auto q = 0ul; q <= pa; ++q) {  // pa
        ...
        auto acc = value_type(1);
        for (auto i = 0ul; i < q; ++i) acc *= a.extents().at(phia.at(i) - 1);
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_inner_prod, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
...
BOOST_FIXTURE_TEST_CASE(test_tensor_real_imag_conj, fixture) {
  ...
  for (auto const& n : extents) {
    ...
    std::transform(b.begin(), b.end(), r0.begin(),
    ...
    std::transform(b.begin(), b.end(), r00.begin(),
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_outer_prod, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_trans, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  for (auto const& n : extents) {
    ...
    auto v = value_type{};
    for (auto i = 0u; i < s; ++i, v += 1) aref[i] = v;
    auto a = aref;
