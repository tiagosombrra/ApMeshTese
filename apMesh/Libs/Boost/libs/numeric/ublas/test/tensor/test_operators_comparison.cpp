...
BOOST_AUTO_TEST_SUITE(test_tensor_comparison,
                      *boost::unit_test::depends_on("test_tensor"))

BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_comparison, value, test_types,
                                 fixture) {
  ...
  BOOST_CHECK_THROW(b = (tensor_type(e1) >= tensor_type(e2)),
                    std::runtime_error);
  BOOST_CHECK_THROW(b = (tensor_type(e1) <= tensor_type(e2)),
                    std::runtime_error);
  BOOST_CHECK_THROW(b = (tensor_type(e1) < tensor_type(e2)),
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_comparison_with_tensor_expressions,
  ...
  auto b = false;
  BOOST_CHECK_NO_THROW(b = tensor_type(e0) ==
                           (tensor_type(e0) + tensor_type(e0)));
  BOOST_CHECK_NO_THROW(b = tensor_type(e1) ==
                           (tensor_type(e2) + tensor_type(e2)));
  BOOST_CHECK_NO_THROW(b = tensor_type(e0) == (tensor_type(e2) + 2));
  BOOST_CHECK_NO_THROW(b = tensor_type(e1) != (2 + tensor_type(e2)));

  BOOST_CHECK_NO_THROW(b = (tensor_type(e0) + tensor_type(e0)) ==
                           tensor_type(e0));
  BOOST_CHECK_NO_THROW(b = (tensor_type(e2) + tensor_type(e2)) ==
                           tensor_type(e1));
  BOOST_CHECK_NO_THROW(b = (tensor_type(e2) + 2) == tensor_type(e0));
  BOOST_CHECK_NO_THROW(b = (2 + tensor_type(e2)) != tensor_type(e1));

  BOOST_CHECK_THROW(b = tensor_type(e1) >= (tensor_type(e2) + tensor_type(e2)),
                    std::runtime_error);
  BOOST_CHECK_THROW(b = tensor_type(e1) <= (tensor_type(e2) + tensor_type(e2)),
                    std::runtime_error);
  BOOST_CHECK_THROW(b = tensor_type(e1) < (tensor_type(e2) + tensor_type(e2)),
  ...
  BOOST_CHECK_THROW(b = tensor_type(e1) >= (tensor_type(e2) + 2),
                    std::runtime_error);
  BOOST_CHECK_THROW(b = tensor_type(e1) <= (2 + tensor_type(e2)),
                    std::runtime_error);
  BOOST_CHECK_THROW(b = tensor_type(e1) < (tensor_type(e2) + 3),
                    std::runtime_error);
  BOOST_CHECK_THROW(b = tensor_type(e1) > (4 + tensor_type(e2)),
                    std::runtime_error);
}

BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_comparison_with_scalar, value,
                                 test_types, fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
