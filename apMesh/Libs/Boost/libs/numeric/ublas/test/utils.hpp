...
template <typename T>
BOOST_UBLAS_INLINE bool(isnan)(::std::complex<T> const& z) {
  // According to IEEE, NaN is different even by itself
  return (z != z) || (boost::math::isnan)(z.real()) ||
         (boost::math::isnan)(z.imag());
}

/// Check if two (real) numbers are close each other (wrt a given tolerance).
template <typename T1, typename T2, typename T3>
BOOST_UBLAS_INLINE bool close_to(T1 x, T2 y, T3 tol) {
  ...
  return abs(x - y) <=
         (max(static_cast<real_type>(abs(x)), static_cast<real_type>(abs(y))) *
          tol);
}

/// Check if two complex numbers are close each other (wrt a given tolerance).
template <typename T1, typename T2, typename T3>
BOOST_UBLAS_INLINE bool close_to(::std::complex<T1> const& x,
                                 ::std::complex<T2> const& y, T3 tol) {
  typedef typename promote_traits<typename promote_traits<T1, T2>::promote_type,
                                  T3>::promote_type real_type;
...
/// Check if two (real) numbers are close each other (wrt a given tolerance).
template <typename T1, typename T2, typename T3>
BOOST_UBLAS_INLINE bool rel_close_to(T1 x, T2 y, T3 tol) {
  // typedef typename promote_traits<typename
  // promote_traits<T1,T2>::promote_type, 								T3>::promote_type real_type;
...
/// Check if two complex numbers are close each other (wrt a given tolerance).
template <typename T1, typename T2, typename T3>
BOOST_UBLAS_INLINE bool rel_close_to(::std::complex<T1> const& x,
                                     ::std::complex<T2> const& y, T3 tol) {
  typedef typename promote_traits<typename promote_traits<T1, T2>::promote_type,
                                  T3>::promote_type real_type;
...
#define BOOST_UBLAS_DEBUG_TRACE(x) \
  ::std::cerr << "[Debug>> " << BOOST_UBLAS_TEST_EXPAND_(x) << ::std::endl
#else
#define BOOST_UBLAS_DEBUG_TRACE(x) /**/
#endif                             // NDEBUG

/// Define the name \a m of the entire test suite.
#define BOOST_UBLAS_TEST_SUITE(m)                                            \
  ::std::cerr << "--- Test Suite: " << BOOST_UBLAS_TEST_EXPAND_(m) << " ---" \
              << ::std::endl;
...
#define BOOST_UBLAS_TEST_DEF(x) \
  static void BOOST_UBLAS_TEST_EXPAND_(x)(::std::size_t & test_fails__)
...
/// Note: we don't use macro expansion inside parenthesis to let \a m be an
///  expression of the form <code>a &lt;&lt; b</code>.
#define BOOST_UBLAS_TEST_TRACE(m) \
  ::std::cerr << "[Info>> " << BOOST_UBLAS_TEST_EXPAND_(m) << ::std::endl

/// Check the truth of assertion \a x.
#define BOOST_UBLAS_TEST_CHECK(x) /* [BOOST_UBLAS_TEST_CHECK] */ \
  if (!BOOST_UBLAS_TEST_EXPANDP_(x)) {                           \
    BOOST_UBLAS_TEST_ERROR(                                      \
        "Failed assertion: " << BOOST_UBLAS_TEST_STRINGIFY_(x)); \
    ++test_fails__;                                              \
  }                                                              \
...
#define BOOST_UBLAS_TEST_CHECK_EQ(x, y) /* [BOOST_UBLAS_TEST_CHECK_EQUAL] */ \
  if (!(BOOST_UBLAS_TEST_EXPANDP_(x) == BOOST_UBLAS_TEST_EXPANDP_(y))) {     \
    BOOST_UBLAS_TEST_ERROR("Failed assertion: ("                             \
                           << BOOST_UBLAS_TEST_STRINGIFY_(x) << " == "       \
                           << BOOST_UBLAS_TEST_STRINGIFY_(y) << ")");        \
    ++test_fails__;                                                          \
  }                                                                          \
...
#define BOOST_UBLAS_TEST_CHECK_CLOSE(x, y,                                    \
                                     e) /* [BOOST_UBLAS_TEST_CHECK_CLOSE] */  \
  if (!::boost::numeric::ublas::test::detail::close_to(                       \
          BOOST_UBLAS_TEST_EXPAND_(x), BOOST_UBLAS_TEST_EXPAND_(y),           \
          BOOST_UBLAS_TEST_EXPAND_(e))) {                                     \
    BOOST_UBLAS_TEST_ERROR("Failed assertion: abs("                           \
                           << BOOST_UBLAS_TEST_STRINGIFY_(x) << "-"           \
                           << BOOST_UBLAS_TEST_STRINGIFY_(y)                  \
                           << ") <= " << BOOST_UBLAS_TEST_STRINGIFY_(e)       \
                           << " [with " << BOOST_UBLAS_TEST_STRINGIFY_(x)     \
                           << " == " << BOOST_UBLAS_TEST_EXPANDP_(x) << ", "  \
                           << BOOST_UBLAS_TEST_STRINGIFY_(y)                  \
                           << " == " << BOOST_UBLAS_TEST_EXPANDP_(y)          \
                           << " and " << BOOST_UBLAS_TEST_STRINGIFY_(e)       \
                           << " == " << BOOST_UBLAS_TEST_EXPANDP_(e) << "]"); \
    ++test_fails__;                                                           \
  }                                                                           \
  /* [/BOOST_UBLAS_TEST_CHECK_CLOSE] */

/// Alias for macro \c BOOST_UBLAS_TEST_CHECK_CLOSE (for backward
/// compatibility),
#define BOOST_UBLAS_TEST_CHECK_PRECISION(x, y, e) \
  BOOST_UBLAS_TEST_CHECK_CLOSE(x, y, e)

/// Check that \a x is close to \a y with respect to a given relative precision
/// \a e.
#define BOOST_UBLAS_TEST_CHECK_REL_CLOSE(                                     \
    x, y, e) /* [BOOST_UBLAS_TEST_CHECK_REL_CLOSE] */                         \
  if (!::boost::numeric::ublas::test::detail::rel_close_to(                   \
          BOOST_UBLAS_TEST_EXPAND_(x), BOOST_UBLAS_TEST_EXPAND_(y),           \
          BOOST_UBLAS_TEST_EXPAND_(e))) {                                     \
    BOOST_UBLAS_TEST_ERROR("Failed assertion: abs(("                          \
                           << BOOST_UBLAS_TEST_STRINGIFY_(x) << "-"           \
...
/// Alias for macro \c BOOST_UBLAS_TEST_CHECK_REL_CLOSE (for backward
/// compatibility),
#define BOOST_UBLAS_TEST_CHECK_REL_PRECISION(x, y, e) \
  BOOST_UBLAS_TEST_CHECK_REL_CLOSE(x, y, e)

/// Check that elements of \a x and \a y are equal.
#define BOOST_UBLAS_TEST_CHECK_VECTOR_EQ(                                   \
    x, y, n) /* [BOOST_UBLAS_TEST_CHECK_VECTOR_EQ] */                       \
  if (BOOST_UBLAS_TEST_EXPANDP_(n) > 0) {                                   \
    ::std::size_t n__ = BOOST_UBLAS_TEST_EXPAND_(n);                        \
    for (::std::size_t i__ = n__; i__ > 0; --i__) {                         \
      if (!(BOOST_UBLAS_TEST_EXPANDP_(x)[n__ - i__] ==                      \
            BOOST_UBLAS_TEST_EXPANDP_(y)[n__ - i__])) {                     \
        BOOST_UBLAS_TEST_ERROR(                                             \
            "Failed assertion: ("                                           \
            << BOOST_UBLAS_TEST_STRINGIFY_(x[i__])                          \
            << "==" << BOOST_UBLAS_TEST_STRINGIFY_(y[i__]) << ")"           \
            << " [with " << BOOST_UBLAS_TEST_STRINGIFY_(x[i__])             \
            << " == " << BOOST_UBLAS_TEST_EXPANDP_(x)[n__ - i__] << ", "    \
            << BOOST_UBLAS_TEST_STRINGIFY_(y[i__])                          \
            << " == " << BOOST_UBLAS_TEST_EXPANDP_(y)[n__ - i__] << ", "    \
            << BOOST_UBLAS_TEST_STRINGIFY_(i__) << " == " << i__ << " and " \
            << BOOST_UBLAS_TEST_STRINGIFY_(n) << " == " << n__ << "]");     \
        ++test_fails__;                                                     \
      }                                                                     \
    }                                                                       \
  }                                                                         \
...
/// precision \a e.
#define BOOST_UBLAS_TEST_CHECK_VECTOR_CLOSE(                                \
    x, y, n, e) /* [BOOST_UBLAS_TEST_CHECK_VECTOR_CLOSE] */                 \
  if (BOOST_UBLAS_TEST_EXPANDP_(n) > 0) {                                   \
    ::std::size_t n__ = BOOST_UBLAS_TEST_EXPAND_(n);                        \
    for (::std::size_t i__ = n__; i__ > 0; --i__) {                         \
      if (!::boost::numeric::ublas::test::detail::close_to(                 \
              ...
              BOOST_UBLAS_TEST_EXPANDP_(e))) {                              \
        BOOST_UBLAS_TEST_ERROR(                                             \
            "Failed assertion: abs(("                                       \
            << BOOST_UBLAS_TEST_STRINGIFY_(x[i__]) << "-"                   \
            << BOOST_UBLAS_TEST_STRINGIFY_(y[i__])                          \
            << ") <= " << BOOST_UBLAS_TEST_STRINGIFY_(e) << " [with "       \
            << BOOST_UBLAS_TEST_STRINGIFY_(x[i__])                          \
            << " == " << BOOST_UBLAS_TEST_EXPANDP_(x)[n__ - i__] << ", "    \
            << BOOST_UBLAS_TEST_STRINGIFY_(y[i__])                          \
            << " == " << BOOST_UBLAS_TEST_EXPANDP_(y)[n__ - i__] << ", "    \
...
/// Check that elements of \a x and \a y are close with respect to a given
/// relative precision \a e.
#define BOOST_UBLAS_TEST_CHECK_VECTOR_REL_CLOSE(                            \
    x, y, n, e) /* [BOOST_UBLAS_TEST_CHECK_VECTOR_REL_CLOSE] */             \
  if (BOOST_UBLAS_TEST_EXPANDP_(n) > 0) {                                   \
    ::std::size_t n__ = BOOST_UBLAS_TEST_EXPAND_(n);                        \
    for (::std::size_t i__ = n__; i__ > 0; --i__) {                         \
      if (!::boost::numeric::ublas::test::detail::rel_close_to(             \
...
#define BOOST_UBLAS_TEST_CHECK_MATRIX_EQ(                                     \
    x, y, nr, nc) /* [BOOST_UBLAS_TEST_CHECK_MATRIX_EQ] */                    \
  for (::std::size_t i__ = 0; i__ < BOOST_UBLAS_TEST_EXPANDP_(nr); ++i__) {   \
    for (::std::size_t j__ = 0; j__ < BOOST_UBLAS_TEST_EXPANDP_(nc); ++j__) { \
      if (!(BOOST_UBLAS_TEST_EXPANDP_(x)(i__, j__) ==                         \
            BOOST_UBLAS_TEST_EXPANDP_(y)(i__, j__))) {                        \
        BOOST_UBLAS_TEST_ERROR(                                               \
            "Failed assertion: ("                                             \
            << BOOST_UBLAS_TEST_STRINGIFY_(x(i__, j__))                       \
            << " == " << BOOST_UBLAS_TEST_STRINGIFY_(y(i__, j__))             \
            << ") [with " << BOOST_UBLAS_TEST_STRINGIFY_(x(i__, j__))         \
            << " == " << BOOST_UBLAS_TEST_EXPANDP_(x)(i__, j__) << ", "       \
            << BOOST_UBLAS_TEST_STRINGIFY_(y(i__, j__))                       \
            << " == " << BOOST_UBLAS_TEST_EXPANDP_(y)(i__, j__) << ", "       \
            << BOOST_UBLAS_TEST_STRINGIFY_(i__) << " == " << i__ << ", "      \
            << BOOST_UBLAS_TEST_STRINGIFY_(j__)                               \
            << " == " << BOOST_UBLAS_TEST_EXPANDP_(j__) << ", "               \
            << BOOST_UBLAS_TEST_STRINGIFY_(nr)                                \
            << " == " << BOOST_UBLAS_TEST_EXPANDP_(nr) << " and "             \
            << BOOST_UBLAS_TEST_STRINGIFY_(nc)                                \
            << " == " << BOOST_UBLAS_TEST_EXPANDP_(nc) << "]");               \
        ++test_fails__;                                                       \
      }                                                                       \
    }                                                                         \
  }                                                                           \
...
/// given precision \a e.
#define BOOST_UBLAS_TEST_CHECK_MATRIX_CLOSE(                                  \
    x, y, nr, nc, e) /* [BOOST_UBLAS_TEST_CHECK_MATRIX_CLOSE] */              \
  for (::std::size_t i__ = 0; i__ < BOOST_UBLAS_TEST_EXPANDP_(nr); ++i__) {   \
    for (::std::size_t j__ = 0; j__ < BOOST_UBLAS_TEST_EXPANDP_(nc); ++j__) { \
      if (!::boost::numeric::ublas::test::detail::close_to(                   \
              BOOST_UBLAS_TEST_EXPANDP_(x)(i__, j__),                         \
...
/// Check that elements of matrices \a x and \a y are close with respect to a
/// given relative precision \a e.
#define BOOST_UBLAS_TEST_CHECK_MATRIX_REL_CLOSE(                              \
    x, y, nr, nc, e) /* [BOOST_UBLAS_TEST_CHECK_MATRIX_REL_CLOSE] */          \
  for (::std::size_t i__ = 0; i__ < BOOST_UBLAS_TEST_EXPANDP_(nr); ++i__) {   \
    for (::std::size_t j__ = 0; j__ < BOOST_UBLAS_TEST_EXPANDP_(nc); ++j__) { \
      if (!::boost::numeric::ublas::test::detail::rel_close_to(               \
              BOOST_UBLAS_TEST_EXPANDP_(x)(i__, j__),                         \
...
#ifdef _MSC_VER
#define BOOST_UBLAS_TEST_ERROR(x)                                     \
  ::std::cerr << "[Error (" << __FILE__ << ":" << __FUNCTION__ << ":" \
              << __LINE__ << ")>> " << BOOST_UBLAS_TEST_EXPAND_(x)    \
              << ::std::endl
#else
#define BOOST_UBLAS_TEST_ERROR(x)                                             \
  ::std::cerr << "[Error (" << __FILE__ << ":" << __func__ << ":" << __LINE__ \
              << ")>> " << BOOST_UBLAS_TEST_EXPAND_(x) << ::std::endl
#endif
