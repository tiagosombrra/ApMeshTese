...
// boost::pfr::for_each_field crashes when sizeof(MyConfig) > 248 (probably >=
// 256)
...
// all configuration data apart from audio and midi devices, which is handled by
// special juce support the class is supposed to be iterated with boost::pfr
// library. Thus its members must met the requirements (aggregate
// initializeable)
class MyConfig {
 public:
...
void printer(const T& value, std::true_type) {
  std::cout << "- " << value.getAttrName() << ": " << value.getValue()
            << std::ends;
}
...
int main() {
  ...
  boost::pfr::for_each_field(
      aCfg, [](auto& value) { printer(value, value.is_config_field); });
