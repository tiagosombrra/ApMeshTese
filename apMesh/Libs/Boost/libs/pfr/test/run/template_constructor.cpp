...
struct constrained_template {
  ...
  template <class U = T,
            std::enable_if_t<std::is_constructible<T, U&&>::value ||
                             ...
                             bool> = false>
  constexpr constrained_template(U&& val) : value_{std::forward<U>(val)} {}
...
int main() {
  static_assert(
      std::is_same<boost::pfr::tuple_element_t<0, aggregate_constrained>,
                   constrained_template<short> >::value,
      "Precise reflection with template constructors fails to work");

  static_assert(
      std::is_same<boost::pfr::tuple_element_t<1, aggregate_constrained>,
                   constrained_template<int_element> >::value,
      "Precise reflection with template constructors fails to work");
