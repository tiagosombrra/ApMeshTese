...
int main() {
  ...
  auto make_sprite = []() -> std::unique_ptr<sprite> {
    ...
    switch (rnd(gen)) {
      ...
      default:
        //->
      case 0:
        return std::make_unique<warrior>(id++);
        break;
      case 1:
        return std::make_unique<juggernaut>(id++);
        break;
      case 2:
        return std::make_unique<goblin>(id++);
        break;
    }
    //<-
  };
  ...
  try {
    //->
    c1.insert(
        *c2.begin());  // throws: actual type of *c2.begin() not known by c1
                       //<-
  } catch (boost::poly_collection::unregistered_type&) {
  }
  //->
  ...
  /*=const char**/ comma = "";
  for (auto first = c.begin<warrior>(), last = c.end<warrior>(); first != last;
       ++first) {
  ...
  std::cout << c.capacity<warrior>() << ", " << c.capacity<juggernaut>() << ", "
