...
bool compare_connectivity(
    std::string& ref_net, std::string& net,
    schematic_database& reference_schematic, schematic_database& schematic,
...
bool compare_schematics_recursive(
  ...
  for (std::size_t i = 0; i < node_id; ++i) {
    for (std::size_t j = 0; j < reference_schematic.devices[i].terminals.size();
         ++j) {
      device& rd = reference_schematic.devices[i];
      device& xd = schematic.devices[reference_to_internal_device_map[i]];
      if (rd.type == "PIN") {
        if (rd.terminals[j] != xd.terminals[j]) return false;
      } else {
        // connectivity must be the same
        if (j == 1) {
          // gate has to be the same net
          if (!compare_connectivity(rd.terminals[1], xd.terminals[1],
                                    reference_schematic, schematic,
                                    reference_to_internal_device_map, node_id))
            return false;
        } else {
          // order of nets in source and drain is not important so check both
          // ways and accept either
          if (!compare_connectivity(
                  rd.terminals[j], xd.terminals[0], reference_schematic,
                  schematic, reference_to_internal_device_map, node_id) &&
              !compare_connectivity(rd.terminals[j], xd.terminals[2],
                                    reference_schematic, schematic,
                                    reference_to_internal_device_map, node_id))
            return false;
        }
  ...
  for (std::size_t i = 0; i < schematic.devices.size(); ++i) {
    if (reference_schematic.devices[node_id].type != schematic.devices[i].type)
    ...
    if (assigned_devices.find(i) == assigned_devices.end()) {
      ...
      std::set<std::size_t>::iterator itr =
          assigned_devices.insert(assigned_devices.end(), i);
...
inline bool compare_schematics(schematic_database& reference_schematic,
  ...
  std::vector<std::size_t> reference_to_internal_device_map(
      reference_schematic.devices.size(), 0);
