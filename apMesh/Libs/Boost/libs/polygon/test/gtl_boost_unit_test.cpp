...
namespace boost {
namespace polygon {
void addpoly(polygon_45_set_data<int>& pset, int* pts, unsigned int numpts) {
...
inline std::ostream& operator<<(std::ostream& o,
                                const polygon_45_set_data<Unit>& p) {
  o << "Polygon45Set ";
  o << " " << !p.sorted() << " " << p.dirty() << " { ";
...
std::ostream& operator<<(std::ostream& o, const polygon_90_data<T>& r) {
  o << "Polygon { ";
  for (typename polygon_90_data<T>::iterator_type itr = r.begin();
       itr != r.end(); ++itr) {
    o << *itr << ", ";
  }
  return o << "} ";
...
std::ostream& operator<<(std::ostream& o,
                         const std::vector<polygon_90_data<T> >& r) {
  o << r.size() << ' ';
...
template <typename T>
std::ostream& operator<<(std::ostream& o,
                         const polygon_90_with_holes_data<T>& poly) {
  ...
  o << " { ";
  for (typename polygon_90_with_holes_data<T>::iterator_holes_type itr =
           poly.begin_holes();
...
template <typename T>
std::ostream& operator<<(std::ostream& o,
                         const polygon_45_with_holes_data<T>& poly) {
  o << "Polygon With Holes { ";
  ...
  o << " { ";
  for (typename polygon_45_with_holes_data<T>::iterator_holes_type itr =
           poly.begin_holes();
       itr != poly.end_holes(); ++itr) {
    o << (*itr);
  }
...
template <typename T>
std::ostream& operator<<(std::ostream& o,
                         const polygon_with_holes_data<T>& poly) {
  o << "Polygon With Holes { ";
  ...
  o << " { ";
  for (typename polygon_with_holes_data<T>::iterator_holes_type itr =
           poly.begin_holes();
       itr != poly.end_holes(); ++itr) {
    o << (*itr);
  }
...
print_is_polygon_90_set_concept(const T&) {
  std::cout << "is polygon 90 set concept\n";
}
template <typename T>
typename enable_if<typename is_mutable_polygon_90_set_type<T>::type, void>::type
print_is_mutable_polygon_90_set_concept(const T&) {
  std::cout << "is mutable polygon 90 set concept\n";
}
...
inline bool testBooleanOr() {
  ...
  if (container[0] !=
      std::pair<interval_data<Unit>, int>(interval_data<Unit>(0, 10), 1)) {
    std::cout << "Test one rectangle, first output wrong: Interval("
              << container[0].first << "), " << container[0].second
              << std::endl;
  }
  if (container[1] !=
      std::pair<interval_data<Unit>, int>(interval_data<Unit>(0, 10), -1)) {
    std::cout << "Test one rectangle, second output wrong: Interval("
              << container[1].first << "), " << container[1].second
              << std::endl;
  }
  ...
  if (container[1] !=
      std::pair<interval_data<Unit>, int>(interval_data<Unit>(10, 15), 1)) {
    std::cout << "Test two rectangles, second output wrong: Interval("
              << container[1].first << "), " << container[1].second
              << std::endl;
  }
  if (container[2] !=
      std::pair<interval_data<Unit>, int>(interval_data<Unit>(0, 5), -1)) {
    std::cout << "Test two rectangles, third output wrong: Interval("
              << container[2].first << "), " << container[2].second
              << std::endl;
  }
  if (container[3] !=
      std::pair<interval_data<Unit>, int>(interval_data<Unit>(5, 15), -1)) {
    std::cout << "Test two rectangles, fourth output wrong: Interval("
              << container[3].first << "), " << container[3].second
              << std::endl;
  }
  ...
  if (container[0] !=
      std::pair<interval_data<Unit>, int>(interval_data<Unit>(0, 10), 1)) {
    std::cout
        << "Test two nonoverlapping rectangles, first output wrong: Interval("
        << container[0].first << "), " << container[0].second << std::endl;
  }
  if (container[1] !=
      std::pair<interval_data<Unit>, int>(interval_data<Unit>(15, 25), 1)) {
    std::cout
        << "Test two nonoverlapping rectangles, second output wrong: Interval("
        << container[1].first << "), " << container[1].second << std::endl;
  }
  if (container[2] !=
      std::pair<interval_data<Unit>, int>(interval_data<Unit>(0, 10), -1)) {
    std::cout
        << "Test two nonoverlapping rectangles, third output wrong: Interval("
        << container[2].first << "), " << container[2].second << std::endl;
  }
  if (container[3] !=
      std::pair<interval_data<Unit>, int>(interval_data<Unit>(15, 25), -1)) {
    std::cout
        << "Test two nonoverlapping rectangles, fourth output wrong: Interval("
...
void test_view_as() {
  ...
  if (!equivalence(p90, rect)) std::cout << "fail 1\n";
  assign(p45, view_as<polygon_45_concept>(p));
  if (!equivalence(p45, rect)) std::cout << "fail 2\n";
  assign(p90, view_as<polygon_90_concept>(p45));
  if (!equivalence(p90, rect)) std::cout << "fail 3\n";
...
bool testRectangle() {
  ...
  if (!equivalence(horizontal(rect),
                   interval_data<polygon_long_long_type>(0, 10)))
    return false;
  if (!equivalence(vertical(rect2),
                   interval_data<polygon_long_long_type>(20, 30)))
    return false;
#endif
...
#else
  set_points(rect, point_data<int>(0, 20),
             point_data<polygon_long_long_type>(10, 30));
#endif
  if (xl(rect) != 0) return false;
  convolve(rect, rect2);
...
bool testPolygon() {
  ...
  polygon_90_data<int> p90;
  assign(p90, rectangle_data<int>(interval_data<int>(0, 10),
                                  interval_data<int>(20, 30)));
  if (winding(p90) != COUNTERCLOCKWISE) return false;
  polygon_45_data<int> p45;
  assign(p45, rectangle_data<int>(interval_data<int>(0, 10),
                                  interval_data<int>(20, 30)));
  if (winding(p45) != COUNTERCLOCKWISE) return false;
  polygon_data<int> p;
  assign(p, rectangle_data<int>(interval_data<int>(0, 10),
                                interval_data<int>(20, 30)));
  ...
  if (bounding_box != rectangle_data<int>(interval_data<int>(-7, 13),
                                          interval_data<int>(0, 20)))
    return false;
...
bool testPolygonWithHoles() {
  ...
  points.insert(points.end(), itr, itr_end);
  polygon_45_with_holes_data<int> p45wh;
  assign(p45wh, rectangle_data<int>(interval_data<int>(0, 10),
                                    interval_data<int>(20, 30)));
...
int getRandomInt() { return rand() % 6 - 2; }
...
bool validate_polygon_set_op(Polygon45Set& ps45_o, const Polygon45Set& ps45_1,
                             const Polygon45Set& ps45_2, int op_type) {
...
bool test_two_polygons(const Polygon45& p45_1, const Polygon45& p45_2) {
...
bool test_45_touch_boundaries() {
  ...
  for (unsigned int i = 0; i < 9; ++i) {
    std::cout << i << ": ";
    for (std::set<int>::iterator itr = graph[i].begin(); itr != graph[i].end();
         ++itr) {
      std::cout << *itr << " ";
    }
    std::cout << std::endl;
...
bool test_aa_touch_ur2() {
  ...
  connectivity_extraction<int> ce;
  rectangle_data<int> rect2(5, 5, 10, 10);
  point_data<int> pts[3] = {point_data<int>(0, 0), point_data<int>(5, 5),
                            point_data<int>(0, 5)};
...
bool test_SQRT1OVER2() {
  Point pts[] = {
      Point(100, 100), Point(0, 100),   Point(100, 200), Point(0, 300),
      Point(100, 400), Point(0, 500),   Point(100, 500), Point(100, 600),
      Point(200, 500), Point(300, 600), Point(400, 500), Point(500, 600),
      Point(500, 500), Point(600, 500), Point(500, 400), Point(600, 300),
      Point(500, 200), Point(600, 100), Point(500, 100), Point(500, 0),
      Point(400, 100), Point(300, 0),   Point(200, 100), Point(100, 0),
      Point(100, 100)};
  ...
  if (polys.size() != 1) return false;
  Point pts2[] = {
      Point(90, 90),   Point(-10, 90),  Point(-10, 100), Point(90, 200),
      Point(-10, 300), Point(90, 400),  Point(-10, 500), Point(-10, 510),
      Point(90, 510),  Point(90, 610),  Point(100, 610), Point(200, 510),
      Point(300, 610), Point(400, 510), Point(500, 610), Point(510, 610),
      Point(510, 510), Point(610, 510), Point(610, 500), Point(510, 400),
      Point(610, 300), Point(510, 200), Point(610, 100), Point(610, 90),
      Point(510, 90),  Point(510, -10), Point(500, -10), Point(400, 90),
      Point(300, -10), Point(200, 90),  Point(100, -10), Point(90, -10),
      Point(90, 90)};
...
bool test_scaling_by_floating() {
  Point pts[] = {Point(1, 1), Point(10, 1), Point(1, 10)};
...
void testHand() {
  using namespace gtl;
  int handcoords[] = {
      12375, 11050, 13175, 10200, 15825, 9275,  18750, 8525,  24150, 8300,
      27575, 8400,  31775, 7800,  35975, 7200,  41375, 4800,  42575, 4200,
      43175, 4200,  47375, 2400,  49175, 1800,  51150, 2200,  52275, 2825,
      52625, 4150,  52375, 4975,  51575, 6000,  49275, 6850,  45700, 7950,
      43175, 9600,  39575, 10800, 37775, 12000, 37775, 12600, 37775, 13800,
      38975, 14400, 41375, 14400, 45575, 13200, 48600, 13000, 51575, 13200,
      55175, 12600, 58775, 12600, 61175, 13200, 62375, 14400, 62550, 15700,
      61975, 16875, 60775, 17600, 60100, 17675, 58525, 17675, 56150, 17575,
      52175, 18000, 47975, 18600, 45575, 19200, 44375, 19200, 42675, 19325,
      41600, 19775, 41600, 20500, 42100, 20825, 44975, 20400, 48575, 20400,
      52775, 21000, 53975, 21000, 57575, 21000, 62375, 21000, 65450, 22000,
      66300, 23100, 66100, 24550, 64750, 25925, 62975, 26400, 61175, 26400,
      58775, 26400, 56025, 26050, 53450, 26025, 50975, 26400, 48575, 26400,
      46775, 26400, 43650, 26075, 41375, 26400, 40775, 27000, 40775, 27600,
      42225, 28650, 44375, 29400, 48575, 30000, 50975, 31200, 53975, 31800,
      58775, 33000, 61200, 34300, 62375, 35400, 62375, 37200, 61175, 38400,
      60000, 38700, 57575, 38400, 54550, 37575, 50975, 36600, 49075, 36125,
      47750, 36125, 45700, 35425, 42350, 34350, 38900, 33775, 30575, 33000,
      26975, 33600, 25975, 34900, 26375, 36600, 28175, 38400, 30575, 40800,
      32375, 43800, 33200, 46200, 33200, 48000, 32650, 49300, 31425, 50000,
// 12600, 19775, 13225, 16775, 13800, 14975, 14400, 13050, 14000, 11975, 12600,
//     0, 0 };
...
//   handpoly.set(handpoints.begin(), handpoints.end());
//   double spiralcoords [] = {
// 37200, 3600, 42075, 4025, 47475, 5875, 51000, 7800, 55800, 12300, 59000,
// 17075, 60000, 20400, 61200, 25800, 61200, 29400, 60600, 33600, 58800, 38400,
// 55800, 42600, 53200, 45625, 49200, 48600, 43200, 51000, 35400, 51600, 29400,
// 50400, 23400, 47400, 19200, 43800, 16200, 39600, 14400, 35400, 13200, 29400,
// 13200, 24000, 15000, 18600, 17400, 13800, 20525, 10300, 24600, 7200, 29400,
// 4800, 32450, 4000, 34825, 3675, 35625, 3625, 35825, 7275, 39600, 7200, 43800,
// 8400, 46800, 9600, 50400, 12000, 53400, 15000, 55800, 18600, 57000, 23400,
// 57600, 27000, 57000, 32400, 55200, 37200, 52200, 41400, 48000, 45000, 42000,
// 47400, 35400, 48000, 30000, 46800, 24600, 43800, 20325, 39100, 17850, 34275,
// 16800, 27600, 17400, 22200, 20400, 16200, 24600, 11400, 28800, 9000, 32400,
// 7800, 33200, 7575, 33925, 11050, 35400, 10800, 37200, 10800, 41400, 11400,
// 46200, 13200, 49800, 16200, 51600, 19200, 53400, 23400, 54000, 29400, 52800,
// 33600, 49800, 39000, 45000, 42600, 39000, 44400, 33600, 43800, 28200, 42000,
// 24000, 37800, 21000, 33000, 20400, 26400, 21600, 21000, 24600, 16200, 28200,
// 13200, 31875, 11625, 33200, 15625, 36000, 15000, 39000, 15000, 43800, 16800,
// 46800, 19200, 49200, 23400, 49800, 27600, 48750, 32700, 46350, 36275, 42600,
// 39000, 38400, 40200, 31800, 39000, 28200, 36600, 25200, 31200, 24600, 26400,
// 26025, 21800, 28200, 18600, 30600, 16800, 32575, 19875, 34200, 19200, 36000,
// 18600, 37200, 18600, 40375, 19125, 43200, 21000, 45600, 24000, 46200, 27600,
// 45600, 30600, 43800, 33600, 41475, 35625, 37800, 36600, 33600, 36000, 30000,
// 33600, 28200, 28800, 28800, 24600, 30000, 22200, 31200, 23400, 30600, 25200,
// 30000, 27000, 30600, 30000, 31800, 32400, 34200, 34200, 38400, 34800, 41400,
// 33000, 44025, 30225, 44400, 26400, 43200, 23400, 40900, 21200, 37800, 20400,
// 34950, 20675, 32400, 22200, 30175, 19475, 28425, 21300, 27000, 24000, 26400,
// 27600, 27000, 31800, 31200, 36600, 36600, 38400, 42600, 37200, 46200, 33600,
// 48000, 30000, 47650, 24425, 45600, 20400, 42650, 18200, 39000, 16800, 35400,
// 16800, 33600, 17400, 32875, 17675, 31100, 13850, 28200, 15600, 25200, 18600,
// 22800, 22800, 22200, 27000, 23400, 33600, 26400, 38400, 31675, 41575, 37800,
// 42600, 40850, 42150, 42800, 41550, 47050, 39025, 50100, 35375, 52200, 29400,
// 51675, 23950, 49800, 19200, 46200, 15600, 41400, 13200, 37800, 12600, 35025,
// 12750, 33350, 13050, 32400, 9600, 30025, 10325, 25925, 12725, 22200, 16800,
// 19800, 21000, 18600, 25800, 18600, 30000, 20400, 35400, 22575, 39250, 25225,
// 41825, 28200, 43800, 33600, 46200, 39000, 46200, 44400, 45000, 48650, 42350,
// 52800, 37800, 55200, 32400, 55800, 26400, 54600, 21000, 53400, 18000, 50400,
// 14400, 47400, 12000, 42600, 9600, 39000, 9000, 36000, 9000, 34775, 9125,
// 34300, 5600, 30000, 6600, 25800, 8400, 22025, 11350, 18725, 15125, 16200,
// 20400, 15000, 24600, 15000, 30600, 16800, 36600, 20400, 42600, 25800, 46800,
// 31200, 49200, 38400, 49800, 45000, 48600, 51000, 45000, 55475, 40225, 58200,
// 34800, 59400, 30000, 59400, 25200, 58200, 19800, 55200, 14400, 52225, 11150,
// 47400, 7800, 44175, 6500, 40200, 5400, 38400, 5400, 37200, 5400, 0, 0 };
...
bool testMaxCover() {
  ...
  std::cout << "expected result XOR clean: "
            << equivalence(expected_result, rects) << std::endl;
  std::vector<Rectangle>& output = result;
  std::vector<Rectangle>& voutput = expected_result;
  std::sort(output.begin(), output.end(),
            less_rectangle_concept<Rectangle, Rectangle>());
...
//     typedef typename polygon_traits<T>::iterator_type iterator_type;
//     typedef typename polygon_traits<T>::point_type point_type;
//     typedef iterator_points_to_compact<iterator_type, point_type>
...
bool test_colinear_duplicate_points() {
  Point pts[6] = {Point(0, 10),    Point(0, 0),   Point(100, 0),
                  Point(100, 100), Point(0, 100), Point(0, 10)};
  Polygon45 p1;
  p1.set(pts, pts + 5);
  Polygon45 pg;
  pg.set(pts, pts + 6);
  ...
  if (!equivalence(view_as<polygon_90_concept>(p2),
                   view_as<polygon_90_concept>(pg)))
    return false;
  std::cout << p1 << std::endl;
  if (!equivalence(view_as<polygon_90_concept>(p1),
                   view_as<polygon_90_concept>(pg)))
    return false;
  return true;
}
...
bool test_extents() {
  ...
  if (rect != rect2) {
    std::cout << "gtl::Rectangles differ: " << gtl::xl(rect) << " "
              << gtl::xh(rect) << " " << gtl::yl(rect) << " " << gtl::yh(rect)
              << std::endl;
    std::cout << "                        " << gtl::xl(rect2) << " "
              << gtl::xh(rect2) << " " << gtl::yl(rect2) << " "
              << gtl::yh(rect2) << std::endl;
    return false;
  }
  return true;
}
...
bool test_extents2() {
  Polygon45Set psT;
  Point xy[] = {Point(130, 50),  Point(50, 50),  Point(50, 100),
                Point(119, 100), Point(119, 59), Point(89, 89),
                Point(59, 59),   Point(119, 59), Point(119, 100),
                Point(130, 100)};
  ...
  psT.extents(rect2);
  gtl::extents(rect, psT);
  std::cout << "Extents: " << gtl::xl(rect) << " " << gtl::xh(rect) << " "
            << gtl::yl(rect) << " " << gtl::yh(rect) << std::endl;
  std::cout << "Extents: " << gtl::xl(rect2) << " " << gtl::xh(rect2) << " "
            << gtl::yl(rect2) << " " << gtl::yh(rect2) << std::endl;
...
int test_new_polygon_formation(int argc, char** argv) {
  ...
  std::cout << "Found " << result.size() << "polygons after union" << std::endl;
  for (size_t i = 0; i < result.size(); i++) {
    PrintPolygon(result[i]);
  }

  intDC shell_coords[] = {0,  0, 10, 0, 10, 21, 0,  21, 0,  15, 3,
                          15, 3, 13, 0, 13, 0,  10, 5,  10, 5,  8,
                          0,  8, 0,  5, 5,  5,  5,  3,  0,  3};
  ...
  std::cout << "platinum_area = " << platinum_area
            << " , gold_area=" << gold_area << std::endl;
  ...
  std::cout << "platinum_area=" << platinum_area << ", gold_area=" << gold_area
            << std::endl;
  ...
  for (size_t i = 0; i < 1; i++) {
    ...
    for (size_t i = 0; i < result.size(); i++) {
      if (result[i].size() > (i ? 4 : 6)) {
        ...
        std::cerr << "FAILED: Expecting no more than " << expected
                  << " vertices" << std::endl;
        return 1;
      }
      PrintPolygon(result[i]);
...
inline void PrintPolygon(const GTLPolygon& p) {
  ...
  for (itr = p.begin(); itr != p.end(); ++itr) {
    pt = *itr;
    std::cout << "Vertex-" << ++vertex_id << "(" << pt.x() << "," << pt.y()
              << ")" << std::endl;
  }
}
...
int main() {
  ...
  }
  {
    ...
    polygon_data<int> p, p1;
    p.set(pts3.begin(), pts3.end());
    polygon_45_data<int> p45, p451;
    p45.set(pts2.begin(), pts2.end());
    polygon_90_data<int> p90, p901;
    p90.set(pts.begin(), pts.end());
    polygon_with_holes_data<int> pwh, pwh1;
    pwh.set(pts3.begin(), pts3.end());
    polygon_45_with_holes_data<int> p45wh, p45wh1;
    p45wh.set(pts2.begin(), pts2.end());
    polygon_90_with_holes_data<int> p90wh, p90wh1;
    p90wh.set(pts.begin(), pts.end());
    assign(p, p90);
    assign(p, p45);
    assign(p1, p);
    ...
    std::cout
        << (polygon_arbitrary_formation<int>::testPolygonArbitraryFormationRect(
               std::cout))
        << std::endl;
    std::cout
        << (polygon_arbitrary_formation<int>::testPolygonArbitraryFormationP1(
               std::cout))
        << std::endl;
    std::cout
        << (polygon_arbitrary_formation<int>::testPolygonArbitraryFormationP2(
               std::cout))
        << std::endl;
    std::cout << (polygon_arbitrary_formation<
                     int>::testPolygonArbitraryFormationPolys(std::cout))
              << std::endl;
    std::cout << (polygon_arbitrary_formation<
                     int>::testPolygonArbitraryFormationSelfTouch1(std::cout))
              << std::endl;
    std::cout << (polygon_arbitrary_formation<
                     int>::testPolygonArbitraryFormationSelfTouch2(std::cout))
              << std::endl;
    std::cout << (polygon_arbitrary_formation<
                     int>::testPolygonArbitraryFormationSelfTouch3(std::cout))
              << std::endl;
    std::cout << (polygon_arbitrary_formation<int>::testSegmentIntersection(
                     std::cout))
              << std::endl;
    std::cout << (property_merge<int, int>::test_insertion(std::cout))
              << std::endl;
    std::cout << (line_intersection<int>::test_verify_scan(std::cout))
              << std::endl;
    std::cout << (line_intersection<int>::test_validate_scan(std::cout))
              << std::endl;
    std::cout << (scanline<int, int>::test_scanline(std::cout)) << std::endl;
    std::cout << (property_merge<int, int>::test_merge(std::cout)) << std::endl;
    std::cout << (property_merge<int, int>::test_intersection(std::cout))
              << std::endl;
    std::cout << (polygon_arbitrary_formation<
                     int>::testPolygonArbitraryFormationColinear(std::cout))
              << std::endl;
    std::cout << (property_merge<int, int>::test_manhattan_intersection(
                     std::cout))
              << std::endl;
  ...
  }
  {
    ...
    std::cout << (polygon_45_formation<Unit>::testPolygon45Formation(std::cout))
              << std::endl;
    std::cout << (polygon_45_formation<Unit>::testPolygon45Tiling(std::cout))
              << std::endl;
  ...
  }
  {
    ...
    ps45_3.get(p45s);
    if (p45s.size() == 2)
      std::cout << p45s[0] << " " << p45s[1] << std::endl;
    else {
      std::cout << "test failed\n";
      return 1;
    }
  ...
  if (!nonInteger45StessTest()) return 1;
  {
    ...
    int i = 0;
    for (std::map<std::set<int>, polygon_45_set_data<int> >::iterator itr =
             result.begin();
         itr != result.end(); ++itr) {
    ...
    pm.merge(mp2);
    i = 0;
    for (std::map<std::vector<int>, polygon_45_set_data<int> >::iterator itr =
             mp2.begin();
         itr != mp2.end(); ++itr) {
  ...
  {
    std::cout << trapezoid_arbitrary_formation<
                     int>::testTrapezoidArbitraryFormationRect(std::cout)
              << std::endl;
    std::cout << trapezoid_arbitrary_formation<
                     int>::testTrapezoidArbitraryFormationP1(std::cout)
              << std::endl;
    std::cout << trapezoid_arbitrary_formation<
                     int>::testTrapezoidArbitraryFormationP2(std::cout)
              << std::endl;
    std::cout << trapezoid_arbitrary_formation<
                     int>::testTrapezoidArbitraryFormationPolys(std::cout)
              << std::endl;
    std::cout << polygon_arbitrary_formation<
                     int>::testPolygonArbitraryFormationSelfTouch1(std::cout)
              << std::endl;
    std::cout << trapezoid_arbitrary_formation<
                     int>::testTrapezoidArbitraryFormationSelfTouch1(std::cout)
              << std::endl;
    typedef rectangle_data<int> Rectangle;
    polygon_set_data<int> ps;
...

  {
    ...
    // this test is waived due to rounding differences between 45 and general
    // resizing general resizing is computing floating point coordinates for the
    // intersection and rounding those to closest while 45 is computing the
    // normal point and rounding that to closest, it turns out to result in
    // different intersection point we want the general to be more accurate to
    // avoid artifacts
  ...
  {
    Point pts[] = {construct<Point>(1565, 5735), construct<Point>(915, 5735),
                   construct<Point>(915, 7085), construct<Point>(1565, 7085)};
    Polygon poly;
    ...
    polygon_with_holes_data<int> pwh;
    polygon_90_with_holes_data<int> p90wh;
    Point pts2[] = {construct<Point>(565, 15735), construct<Point>(15, 15735),
                    construct<Point>(15, 17085), construct<Point>(565, 17085)};
  ...
  }
  {
    ...
    int mp1[7][2 * 4] = {{200, 400, 100, 400, 100, 300, 200, 400},
                         {600, 700, 500, 700, 500, 600, 600, 700},
                         {700, 300, 600, 300, 600, 200, 700, 300},
                         {800, 500, 700, 600, 700, 500, 800, 500},
                         {900, 800, 800, 700, 900, 700, 900, 800},
...

  {
    ...
    sarray[0] =
        segment_data<int>(point_data<int>(0, 0), point_data<int>(10, 10));
    sarray[1] =
        segment_data<int>(point_data<int>(10, 0), point_data<int>(0, 10));
