...
enum kOrientation { RIGHT = -1, COLLINEAR = 0, LEFT = 1 };

template <typename VERTEX>
kOrientation get_orientation(const VERTEX& v1, const VERTEX& v2,
                             const VERTEX& v3) {
...
bool verify_cell_convexity(const OUTPUT& output) {
  ...
  for (cell_it = output.cells().begin(); cell_it != output.cells().end();
       cell_it++) {
    ...
    if (edge) do {
        ...
        if (edge->vertex0() != NULL && edge->vertex1() != NULL &&
          ...
          if (get_orientation(*edge->vertex0(), *edge->vertex1(),
...
bool verify_incident_edges_ccw_order(const OUTPUT& output) {
  ...
  for (vertex_it = output.vertices().begin();
    ...
    if (vertex_it->is_degenerate()) continue;
    ...
    do {
      ...
      if (edge->vertex1() != NULL && next_edge->vertex1() != NULL &&
          get_orientation(*edge->vertex1(), *edge->vertex0(),
...
bool verfiy_no_line_edge_intersections(const Output& output) {
  ...
  for (edge_it = output.edges().begin(); edge_it != output.edges().end();
       edge_it++) {
...
bool intersection_check(
  ...
  typedef typename std::map<point_type, std::vector<point_type>,
                            cmp<Point2D> >::const_iterator edge_map_iterator;
  typedef typename std::vector<point_type>::size_type size_type;
  edge_map_iterator edge_map_it1, edge_map_it2, edge_map_it_bound;
  for (edge_map_it1 = edge_map.begin(); edge_map_it1 != edge_map.end();
       edge_map_it1++) {
    ...
    for (size_type i = 0; i < edge_map_it1->second.size(); i++) {
      ...
      for (; edge_map_it2 != edge_map_it_bound; edge_map_it2++) {
        ...
        for (size_type j = 0; j < edge_map_it2->second.size(); j++) {
          ...
          if (!(max_y1 > min_y2 && max_y2 > min_y1)) continue;

          // Intersection check.
          if (get_orientation(point1, point2, point3) *
                      get_orientation(point1, point2, point4) ==
                  RIGHT &&
              get_orientation(point3, point4, point1) *
                      get_orientation(point3, point4, point2) ==
                  RIGHT)
            return true;
        }
      }
...
bool verify_output(const Output& output, kVerification mask) {
  bool result = true;
  if (mask & CELL_CONVEXITY) result &= verify_cell_convexity(output);
...
void save_points(PointIterator first, PointIterator last,
...
void save_segments(SegmentIterator first, SegmentIterator last,
                   const char* file_name) {
  std::ofstream ofs(file_name);
  ofs << std::distance(first, last) << std::endl;
...
void clean_segment_set(std::vector<segment_data<T> >& data) {
  ...
  line_intersection<Unit>::validate_scan(half_edges_out, half_edges.begin(),
                                         half_edges.end());
