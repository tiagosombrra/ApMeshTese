...
string invalid_syntax::get_template(kind_t kind) {
  ...
  switch (kind) {
    case empty_adjacent_parameter:
      msg =
          "the argument for option '%canonical_option%' should follow "
    ...
    case unrecognized_line:
      msg =
          "the options configuration file contains an invalid line "
          "'%invalid_line%'";
      break;
    ...
    case long_adjacent_not_allowed:
      msg =
          "the unabbreviated option '%canonical_option%' does not take any "
          "arguments";
      break;
    case short_adjacent_not_allowed:
      msg =
          "the abbreviated option '%canonical_option%' does not take any "
          "arguments";
      break;
...
void cmdline::check_style(int style) const {
  ...
  if (allow_some_long && !(style & long_allow_adjacent) &&
    ...
    error =
        ...
        "'command_line_style::long_allow_adjacent' ('=' separated arguments) "
        "for "
        "long options.";
  ...
  if (!error && (style & allow_short) && !(style & short_allow_adjacent) &&
    ...
    error =
        ...
        "(whitespace separated arguments) or "
        "'command_line_style::short_allow_adjacent' ('=' separated arguments) "
        "for "
        "short options.";
  ...
  if (error) boost::throw_exception(invalid_command_line_style(error));
...
vector<option> cmdline::run() {
  ...
  while (!args.empty()) {
    ...
    for (unsigned i = 0; i < style_parsers.size(); ++i) {
      ...
      if (!next.empty()) {
        ...
        for (unsigned j = 0; j < next.size(); ++j) result.push_back(next[j]);
      }
  ...
  int position_key = 0;
  for (unsigned i = 0; i < result.size(); ++i) {
    if (result[i].string_key.empty()) result[i].position_key = position_key++;
  }
...
void cmdline::finish_option(option& opt, vector<string>& other_tokens,
  ...
  try {
    ...
    const option_description* xd =
        m_desc->find_nothrow(opt.string_key, is_style_active(allow_guessing),
                             is_style_active(long_case_insensitive),
                             is_style_active(short_case_insensitive));
