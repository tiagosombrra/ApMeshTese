...
const option_description& options_description::find(
  ...
  if (!d) boost::throw_exception(unknown_option());
  return *d;
}
...
const option_description* options_description::find_nothrow(
  ...
  for (unsigned i = 0; i < m_options.size(); ++i) {
    ...
    if (r == option_description::full_match) {
      ...
      if (!had_full_match) found = m_options[i];
    }
  }
  ...
  if (par.size() < line_length) {
    ...
    while (line_begin < par_end)  // paragraph lines
    {
      ...
      string::const_iterator line_end =
          line_begin + ((remaining < line_length) ? remaining : line_length);
...
void format_one(std::ostream& os, const option_description& opt,
  ...
  if (!opt.description().empty()) {
    if (ss.str().size() >= first_column_width) {
      ...
      for (unsigned pad =
               first_column_width - static_cast<unsigned>(ss.str().size());
           pad > 0; --pad) {
        os.put(' ');
      }
    }
