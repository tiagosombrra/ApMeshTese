...
void untyped_value::xparse(boost::any& value_store,
  ...
  if (!value_store.empty()) boost::throw_exception(multiple_occurrences());
  if (new_tokens.size() > 1) boost::throw_exception(multiple_values());
...
BOOST_PROGRAM_OPTIONS_DECL void validate(any& v, const vector<string>& xs,
  ...
  for (size_t i = 0; i < s.size(); ++i) s[i] = char(tolower(s[i]));
...
void validate(any& v, const vector<wstring>& xs, bool*, int) {
  ...
  for (size_t i = 0; i < s.size(); ++i) s[i] = wchar_t(tolower(s[i]));
...
void check_first_occurrence(const boost::any& value) {
  if (!value.empty()) boost::throw_exception(multiple_occurrences());
}
}  // namespace validators
...
string validation_error::get_template(kind_t kind) {
  ...
  switch (kind) {
    case invalid_bool_value:
      msg =
          "the argument ('%value%') for option '%canonical_option%' is "
