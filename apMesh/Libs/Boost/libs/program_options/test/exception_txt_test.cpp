...
//
//   Uncomment to print out the complete set of diagnostic messages for the
...
//
//  test exception for each specified command line style, e.g. short dash or
//  config file
//
template <typename EXCEPTION>
void test_each_exception_message(const string& test_description,
                                 const vector<const char*>& argv,
                                 options_description& desc, int style,
                                 string exception_msg, istream& is = cin) {
  if (exception_msg.length() == 0) return;
  variables_map vm;
...
//
//  test exception messages for all command line styles
//  (unix/long/short/slash/config file)
//
//      try each command line style in turn
const int unix_style = command_line_style::unix_style;
const int short_dash =
    command_line_style::allow_dash_for_short | command_line_style::allow_short |
    command_line_style::short_allow_adjacent | command_line_style::allow_sticky;
const int short_slash = command_line_style::allow_slash_for_short |
                        command_line_style::allow_short |
                        command_line_style::short_allow_adjacent;
const int long_dash = command_line_style::allow_long |
...
void test_exception_message(const vector<vector<const char*> >& argv,
  ...
  if (expected_message.length()) {
    ...
    test_each_exception_message<EXCEPTION>(
        error_description + " -- config_file", argv[4], desc, -1,
        expected_message, istrm);
  }
}

#define VEC_STR_PUSH_BACK(vec, c_array) \
  vec.push_back(vector<const char*>(    \
      c_array, c_array + sizeof(c_array) / sizeof(char*)));
...
void test_invalid_option_value_exception_msg() {
  options_description desc;
  desc.add_options()("int-option,d", value<int>(),
                     "An option taking an integer");
  ...
  test_exception_message<invalid_option_value>(
      argv, desc, "invalid_option_value", expected_msg);
}
...
void test_missing_value_exception_msg() {
  options_description desc;
  desc.add_options()("cfgfile,e", value<string>(), "the config file")(
  ...
  test_exception_message<invalid_command_line_syntax>(
      argv, desc, "invalid_syntax::missing_parameter", expected_msg);
}
...
void test_ambiguous_option_exception_msg() {
  options_description desc;
  desc.add_options()("cfgfile1,c", value<string>(), "the config file")(
      "cfgfile2,o", value<string>(), "the config file")(
      "good,g", "good option")("output,c", value<string>(), "the output file")(
      "output", value<string>(), "the output file");

  vector<vector<const char*> > argv;
  const char* argv0[] = {"program", "-ggc", "file", "-o", "anotherfile"};
  VEC_STR_PUSH_BACK(argv, argv0);
  const char* argv1[] = {"program", "--cfgfile", "file", "--cfgfile",
                         "anotherfile"};
  VEC_STR_PUSH_BACK(argv, argv1);
  ...
  const char* expected_msg[5] = {
      ...
      "option '--cfgfile' is ambiguous and matches '--cfgfile1', and "
      "'--cfgfile2'",
...
void test_multiple_occurrences_exception_msg() {
  ...
  VEC_STR_PUSH_BACK(argv, argv0);
  const char* argv1[] = {"program", "--cfgfi", "file", "--cfgfi",
                         "anotherfile"};
  VEC_STR_PUSH_BACK(argv, argv1);
  ...
  test_exception_message<multiple_occurrences>(
      argv, desc, "multiple_occurrences", expected_msg);
}
...
void test_unknown_option_exception_msg() {
  ...
  VEC_STR_PUSH_BACK(argv, argv4);
  const char* expected_msg[5] = {
      "unrecognised option '-ggc'",    "unrecognised option '--cfgfile'",
      "unrecognised option '-ggc'",    "unrecognised option '/c'",
      "unrecognised option 'cfgfile'",
  };
  test_exception_message<unknown_option>(argv, desc, "unknown_option",
                                         expected_msg);
}
...
void test_invalid_bool_value_exception_msg() {
  ...
  VEC_STR_PUSH_BACK(argv, argv4);
  const char* expected_msg[5] = {
      "the argument ('file') for option '--bool_option' is invalid. Valid "
      "choices are 'on|off', 'yes|no', '1|0' and 'true|false'",
      "the argument ('file') for option '--bool_option' is invalid. Valid "
      "choices are 'on|off', 'yes|no', '1|0' and 'true|false'",
      "the argument ('file') for option '-b' is invalid. Valid choices are "
      "'on|off', 'yes|no', '1|0' and 'true|false'",
      "the argument ('file') for option '/b' is invalid. Valid choices are "
      "'on|off', 'yes|no', '1|0' and 'true|false'",
      "the argument ('output.txt') for option 'bool_option' is invalid. Valid "
      "choices are 'on|off', 'yes|no', '1|0' and 'true|false'",
  };
  test_exception_message<validation_error>(
      argv, desc, "validation_error::invalid_bool_value", expected_msg);
}
...
//________________________________________________________________________________________
//
//  Strange exception: sole purpose seems to be catching multitoken() associated
//  with a scalar validation_error::multiple_values_not_allowed seems thus to be
//  a programmer error
//
//
void test_multiple_values_not_allowed_exception_msg() {
  ...
  VEC_STR_PUSH_BACK(argv, argv0);
  const char* argv1[] = {"program", "--cfgfil", "file", "c",
                         "--outpu", "fritz",    "hugo"};
  VEC_STR_PUSH_BACK(argv, argv1);
  ...
  test_exception_message<validation_error>(
      argv, desc, "validation_error::multiple_values_not_allowed",
      expected_msg);
}
...
//________________________________________________________________________________________
//
//  Strange exception: sole purpose seems to be catching zero_tokens()
//  associated with a scalar validation_error::multiple_values_not_allowed seems
//  thus to be a programmer error
//
//
void test_at_least_one_value_required_exception_msg() {
  options_description desc;
  desc.add_options()("cfgfile,c", value<int>()->zero_tokens(),
                     "the config file")("other,o", value<string>(), "other");
  ...
  test_exception_message<validation_error>(
      argv, desc, "validation_error::at_least_one_value_required",
      expected_msg);
}
...
void test_required_option_exception_msg() {
  ...
  test_exception_message<required_option>(argv, desc, "required_option",
                                          expected_msg);
}

/**
 * Check if this is the expected exception with the right message is being
 * thrown inside func
 */
template <typename EXCEPTION, typename FUNC>
void test_exception(const string& test_name, const string& exception_txt,
                    FUNC func) {
  try {
    options_description desc;
    variables_map vm;
...
void config_file_wildcard(options_description& desc, variables_map& vm) {
  desc.add_options()("outpu*", value<string>(), "the output file1")(
...
//
//________________________________________________________________________________________
void abbreviated_options_in_config_file(options_description& desc,
                                        variables_map& vm) {
...
void too_many_positional_options(options_description& desc, variables_map& vm) {
  ...
  store(command_line_parser(4, argv)
            .options(desc)
...
void test_invalid_command_line_style_exception_msg() {
  ...
  invalid_diagnostics.push_back(
      "boost::program_options misconfiguration: choose one "
  ...
  for (unsigned ii = 0; ii < 3; ++ii) {
    ...
    try {
      ...
      string error_msg(
          "arguments are not allowed for unabbreviated option names");
...
void test_empty_value_inner(options_description& opts, variables_map& vm) {
  ...
  opts.add_options()("foo",
                     value<uint32_t>()->value_name("<time>")->required());
  popts.add("foo", 1);
...
void test_empty_value() {
  // Test that passing empty token for an option that requires integer does not
  // result in out-of-range error in error reporting code.
  test_exception<invalid_option_value>(
      "test_empty_value", "the argument for option '--foo' is invalid",
...
int main(int /*ac*/, char** /*av*/) {
  ...
  expected_message =
      "options 'outpu*' and 'outp*' will both match the same arguments from "
  ...
  expected_message =
      "the options configuration file contains an invalid line 'funny wierd "
      "line'";
  test_exception<invalid_syntax>(test_name, expected_message,
                                 unrecognized_line);

  // abbreviated_options_in_config_file
  test_name = "abbreviated_options_in_config_file";
  expected_message =
      "abbreviated option names are not permitted in options configuration "
      "files";
  test_exception<error>(test_name, expected_message,
                        abbreviated_options_in_config_file);
