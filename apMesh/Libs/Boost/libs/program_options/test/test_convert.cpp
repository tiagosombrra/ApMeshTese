...
std::wstring from_8_bit_2(const std::string& s,
  ...
  while (from != from_end) {
    ...
    // error.
    if (to_next == buffer) throw logic_error("character conversion failed");
...
void test_convert(const std::string& input,
  ...
  std::wstring output;
  {
    ...
    for (int i = 0; i < 10000; ++i) output = boost::from_8_bit(input, facet);
  }

  {
    boost::progress_timer t;
    for (int i = 0; i < 10000; ++i) output = from_8_bit_2(input, facet);
  }
