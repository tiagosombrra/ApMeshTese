...
void check_value(const woption& option, const char* name,
                 const wchar_t* value) {
...
void test_command_line() {
  ...
  desc.add_options()("foo,f", new untyped_value(), "")
      ...
      ("bar,b", po::value<std::string>(), "")("baz", new untyped_value())(
          "qux,plug*", new untyped_value());

  const wchar_t* cmdline4_[] = {L"--foo=1\u0FF52", L"-f4", L"--bar=11", L"-b4",
                                L"--plug3=10"};
