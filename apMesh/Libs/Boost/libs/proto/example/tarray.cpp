...
struct TArrayGrammar
    : proto::or_<proto::terminal<int>, proto::terminal<int[3]>,
...
struct TArraySubscriptCtx : proto::callable_context<TArraySubscriptCtx const> {
...
struct TArrayPrintCtx : proto::callable_context<TArrayPrintCtx const> {
  ...
  template <typename L, typename R>
  std::ostream &operator()(proto::tag::multiplies, L const &l,
                           R const &r) const {
    return std::cout << l << " * " << r;
  }
...
template <typename Expr>
struct TArrayExpr : proto::extends<Expr, TArrayExpr<Expr>, TArrayDomain> {
...
struct TArray : TArrayExpr<proto::terminal<int[3]>::type> {
  ...
  int &operator[](std::ptrdiff_t i) { return proto::value(*this)[i]; }
