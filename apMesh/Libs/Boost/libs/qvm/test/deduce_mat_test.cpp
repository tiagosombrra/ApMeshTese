...
struct deduce_mat<m2<M2S, M2R, M2C>, R, C, S> {
  typedef m2<S, R, C> type;
};

template <int R, int C, class S, class AS, int AR, int AC, class BS, int BR,
          int BC>
struct deduce_mat2<m2<AS, AR, AC>, m2<BS, BR, BC>, R, C, S> {
  typedef m2<S, R, C> type;
};
}  // namespace qvm
}  // namespace boost
...
int main() {
  ...
  same_type<boost::qvm::deduce_mat<m1<int, 4, 2>, 4, 4>::type,
            boost::qvm::mat<int, 4, 4> >();
  ...
  same_type<boost::qvm::deduce_mat<m2<int, 4, 2>, 4, 4>::type,
            m2<int, 4, 4> >();
