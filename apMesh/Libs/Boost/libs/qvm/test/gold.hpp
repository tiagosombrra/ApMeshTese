...
template <class T, int D>
void inverse(T (&out)[D][D], T (&in)[D][D]) {
  T **m = static_cast<T **>(malloc(D * sizeof(T *)));
  for (int i = 0; i != D; ++i) {
    m[i] = static_cast<T *>(malloc(D * sizeof(T)));
    for (int j = 0; j != D; ++j) m[i][j] = in[i][j];
  ...
  free(m);
  for (int i = 0; i != D; ++i)
    for (int j = 0; j != D; ++j) out[j][i] = b[i][j] * f;
}

template <class T, int M, int N>
void init_m(T (&r)[M][N], T start = T(0), T step = T(0)) {
  for (int i = 0; i < M; ++i)
    for (int j = 0; j < N; ++j, start += step) r[i][j] = start;
}

template <class T, int D>
void init_v(T (&r)[D], T start = T(0), T step = T(0)) {
  for (int i = 0; i < D; ++i, start += step) r[i] = start;
}

template <class T, int M, int N>
void zero_mat(T (&r)[M][N]) {
  for (int i = 0; i < M; ++i)
    for (int j = 0; j < N; ++j) r[i][j] = T(0);
}

template <class T, int D>
void zero_vec(T (&r)[D]) {
  for (int i = 0; i < D; ++i) r[i] = T(0);
}

template <class T, int D>
void identity(T (&r)[D][D]) {
  for (int i = 0; i < D; ++i)
    for (int j = 0; j < D; ++j) r[i][j] = (i == j) ? T(1) : T(0);
}

template <class T, class U, class V, int M, int N>
void add_m(T (&r)[M][N], U (&a)[M][N], V (&b)[M][N]) {
  for (int i = 0; i < M; ++i)
    for (int j = 0; j < N; ++j) r[i][j] = a[i][j] + b[i][j];
}

template <class T, class U, class V, int D>
void add_v(T (&r)[D], U (&a)[D], V (&b)[D]) {
  for (int i = 0; i < D; ++i) r[i] = a[i] + b[i];
}

template <class T, class U, class V, int M, int N>
void subtract_m(T (&r)[M][N], U (&a)[M][N], V (&b)[M][N]) {
  for (int i = 0; i < M; ++i)
    for (int j = 0; j < N; ++j) r[i][j] = a[i][j] - b[i][j];
}

template <class T, class U, class V, int D>
void subtract_v(T (&r)[D], U (&a)[D], V (&b)[D]) {
  for (int i = 0; i < D; ++i) r[i] = a[i] - b[i];
}

template <class T, int D, class U>
void rotation_x(T (&r)[D][D], U angle) {
  identity(r);
...
template <class T, int D, class U>
void rotation_y(T (&r)[D][D], U angle) {
  identity(r);
  T c = ::test_qvm::detail::cos(angle);
  T s = ::test_qvm::detail::sin(angle);
  r[0][0] = c;
...
template <class T, int D, class U>
void rotation_z(T (&r)[D][D], U angle) {
  identity(r);
  T c = ::test_qvm::detail::cos(angle);
  T s = ::test_qvm::detail::sin(angle);
  r[0][0] = c;
...
template <class T, int D>
void translation(T (&r)[D][D], T (&t)[D - 1]) {
  identity(r);
  for (int i = 0; i != D - 1; ++i) r[i][D - 1] = t[i];
}

template <class R, class T, class U, int M, int N, int P>
void multiply_m(R (&r)[M][P], T (&a)[M][N], U (&b)[N][P]) {
  for (int i = 0; i < M; ++i)
    for (int j = 0; j < P; ++j) {
      R x = 0;
      for (int k = 0; k < N; ++k) x += R(a[i][k]) * R(b[k][j]);
      r[i][j] = x;
    }
}

template <class R, class T, class U, int M, int N>
void multiply_mv(R (&r)[M], T (&a)[M][N], U (&b)[N]) {
  for (int i = 0; i < M; ++i) {
    R x = 0;
    for (int k = 0; k < N; ++k) x += R(a[i][k]) * R(b[k]);
    r[i] = x;
  }
}

template <class R, class T, class U, int N, int P>
void multiply_vm(R (&r)[P], T (&a)[N], U (&b)[N][P]) {
  for (int j = 0; j < P; ++j) {
    R x = 0;
    for (int k = 0; k < N; ++k) x += R(a[k]) * R(b[k][j]);
    r[j] = x;
  }
}

template <class T, class U, int M, int N, class S>
void scalar_multiply_m(T (&r)[M][N], U (&a)[M][N], S scalar) {
  for (int i = 0; i < M; ++i)
    for (int j = 0; j < N; ++j) r[i][j] = a[i][j] * scalar;
}

template <class T, class U, int D, class S>
void scalar_multiply_v(T (&r)[D], U (&a)[D], S scalar) {
  for (int i = 0; i < D; ++i) r[i] = a[i] * scalar;
}

template <class T, int M, int N>
void transpose(T (&r)[M][N], T (&a)[N][M]) {
  for (int i = 0; i < M; ++i)
...
template <class T>
inline void matrix_perspective_lh(T (&r)[4][4], T fov_y, T aspect_ratio, T zn,
                                  T zf) {
...
template <class T>
inline void matrix_perspective_rh(T (&r)[4][4], T fov_y, T aspect_ratio, T zn,
                                  T zf) {
