...
struct matrix {
  ...
  explicit matrix(T start = T(0), T step = T(0)) {
    for (int i = 0; i != Rows; ++i)
      for (int j = 0; j != Cols; ++j, start += step) a[i][j] = b[i][j] = start;
  }
};

template <int Rows, int Cols, class Tag1, class T1, class Tag2, class T2>
void dump_ab(matrix<Tag1, Rows, Cols, T1> const& a,
             matrix<Tag2, Rows, Cols, T2> const& b) {
...
struct mat_traits<test_qvm::matrix<Tag, Rows, Cols, T> >
    : mat_traits_defaults<test_qvm::matrix<Tag, Rows, Cols, T>, T, Rows, Cols> {
  typedef mat_traits_defaults<test_qvm::matrix<Tag, Rows, Cols, T>, T, Rows,
                              Cols>
      base;

  template <int R, int C>
  static typename base::scalar_type& write_element(typename base::mat_type& m) {
...
template <class Tag, class T, int R1, int C1, int R2, int C2, int Rows,
          int Cols>
struct deduce_mat2<test_qvm::matrix<Tag, R1, C1, T>,
                   test_qvm::matrix<Tag, R2, C2, T>, Rows, Cols> {
