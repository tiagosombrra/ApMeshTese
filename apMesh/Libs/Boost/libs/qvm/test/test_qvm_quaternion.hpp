...
struct quaternion {
  ...
  explicit quaternion(T start = T(0), T step = T(0)) {
    for (int i = 0; i != 4; ++i, start += step) a[i] = b[i] = start;
  }
};

template <class Tag1, class T1, class Tag2, class T2>
void dump_ab(quaternion<Tag1, T1> const& a, quaternion<Tag2, T2> const& b) {
...
struct quat_traits<test_qvm::quaternion<Tag, T> >
    : quat_traits_defaults<test_qvm::quaternion<Tag, T>, T> {
  typedef quat_traits_defaults<test_qvm::quaternion<Tag, T>, T> base;

  template <int I>
  static typename base::scalar_type& write_element(
      typename base::quat_type& m) {
...
template <class Tag, class T>
struct deduce_quat2<test_qvm::quaternion<Tag, T>,
                    test_qvm::quaternion<Tag, T> > {
