...
struct seed_seq_archetype : Base {
  ...
  BOOST_CONCEPT_REQUIRES(
      ((Mutable_RandomAccessIterator<Iter>))(
          (UnsignedInteger<
              typename Mutable_RandomAccessIterator<Iter>::value_type>)),
      (void))
  generate(Iter, Iter) {}
};
...
struct SeedSeq {
 public:
  BOOST_CONCEPT_USAGE(SeedSeq) { q.generate(rb, re); }
...
template <class E>
struct RandomNumberEngine : DefaultConstructible<E>,
                            CopyConstructible<E>,
                            Assignable<E>,
...
template <class D>
struct RandomNumberDistribution : DefaultConstructible<D>,
                                  CopyConstructible<D>,
                                  Assignable<D>,
