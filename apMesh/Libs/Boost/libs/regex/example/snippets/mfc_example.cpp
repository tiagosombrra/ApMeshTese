...
void enumerate_links2(const CString& html) {
  ...
  boost::tregex r(
      __T("href=[\"\']((http:\\/\\/|\\.\\/|\\/)?\\w+(\\.\\w+)*(\\/"
          "\\w+(\\.\\w+)?)*(\\/|\\?\\w*=\\w*(&\\w*=\\w*)*)?)[\"\']"));
  boost::tregex_token_iterator i(boost::make_regex_token_iterator(html, r, 1)),
      j;
  while (i != j) {
    std::cout << *i << std::endl;
    ++i;
...
//
const boost::tregex e(
    __T("\\A(\\d{3,4})[- ]?(\\d{4})[- ]?(\\d{4})[- ]?(\\d{4})\\z"));
...
int main() {
  ...
  CString text =
      "<dt><a href=\"syntax_perl.html\">Perl Regular "
      "Expressions</a></dt><dt><a href=\"syntax_extended.html\">POSIX-Extended "
      "Regular Expressions</a></dt><dt><a "
