 ...
 *   DESCRIPTION: regex_grep example 3: searches a cpp file for class
 * definitions, using a bound member function callback.
 */
...
class class_index {
 ...
 public:
  ...
  class_index() : expression(re) {}
};

bool class_index::grep_callback(
  ...
  index[std::string(what[5].first, what[5].second) +
        std::string(what[6].first, what[6].second)] = what[5].first - base;
  return true;
}
...
void class_index::IndexClasses(const std::string& file) {
  ...
  boost::regex_grep(
      std::bind1st(std::mem_fun1(&class_index::grep_callback), this), start,
      end, expression);
#elif defined(BOOST_NO_CXX98_BINDERS)
  boost::regex_grep(
      std::bind(&class_index::grep_callback, this, std::placeholders::_1),
      start, end, expression);
#else
  boost::regex_grep(
      std::bind1st(std::mem_fun(&class_index::grep_callback), this), start, end,
      expression);
#endif
}
...
int main(int argc, const char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    d = idx.get_map().end();
    while (c != d) {
      cout << "class \"" << (*c).first << "\" found at index: " << (*c).second
           << endl;
      ++c;
    }
  }
