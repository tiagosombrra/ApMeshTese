 ...
 *   DESCRIPTION: regex_grep example 4: searches a cpp file for class
 * definitions, using a C++ Builder closure as a callback.
 */
...
class class_index {
 ...
 public:
  ...
  class_index() : index(), expression(re) {}
};
...
void class_index::IndexClasses(const std::string& file) {
  ...
  boost::regex_grep(cl, start, end, expression);
}
