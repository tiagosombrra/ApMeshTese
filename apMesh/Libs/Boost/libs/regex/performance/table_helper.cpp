...
inline std::string sanitize_string(const std::string& s) {
  ...
  while (result[0] == '_') result.erase(0);
  return result;
}
...
void load_table(std::vector<std::vector<std::string> >& table,
  ...
  boost::regex_token_iterator<std::string::const_iterator> i(begin, end,
                                                             item_e),
      j;
  ...
  while (i != j) {
    ...
    boost::regex_token_iterator<std::string::const_iterator> k(
        i->first + 1, i->second - 1, item_e);
    while (k != j) {
      // Add a cell:
...
std::string save_table(std::vector<std::vector<std::string> >& table) {
  ...
  for (std::vector<std::vector<std::string> >::const_iterator i = table.begin(),
                                                              j = table.end();
       i != j; ++i) {
    result += "[";
    for (std::vector<std::string>::const_iterator k = i->begin(), l = i->end();
         k != l; ++k) {
      result += "[";
      result += *k;
      result += "]";
...
void add_to_all_sections(const std::string& id,
                         std::string list_name = "performance_all_sections") {
  std::string::size_type pos = content.find("[template " + list_name + "[]"),
                         end_pos;
  if (pos == std::string::npos) {
    ...
    //
    // Read in the all list of sections, add our new one (in alphabetical
    // order), and then rewrite the whole thing:
    //
    static const boost::regex item_e(
        "\\["
        "((?=[^\\]])[^\\[\\]]*+(?0)?+)*+"
        "\\]|\\]");
    boost::regex_token_iterator<std::string::const_iterator> i(
        content.begin() + pos + 12 + list_name.size(), content.end(), item_e),
        j;
    ...
    sections.insert(id);
    std::string new_list = "\n";
    for (std::set<std::string>::const_iterator sec = sections.begin();
         sec != sections.end(); ++sec) {
      new_list += "[" + *sec + "]\n";
    }
    content.replace(pos + 12 + list_name.size(),
                    end_pos - pos - 12 - list_name.size(), new_list);
  }
}

std::string get_colour(boost::uintmax_t val, boost::uintmax_t best) {
  if (val <= best * 1.2) return "green";
  if (val > best * 4) return "red";
  return "blue";
}
...
void add_cell(boost::intmax_t val, const std::string& table_name,
              const std::string& row_name, const std::string& column_heading) {
  //
  ...
  if (regex_search(content, table_location, table_e)) {
    ...
    std::sort(table_data.begin() + 1, table_data.end(),
              [](std::vector<std::string> const& a,
                 std::vector<std::string> const& b) { return a[0] < b[0]; });
...
void report_execution_time(double t, std::string table, std::string row,
                           std::string heading) {
  try {
...
std::string boost_name() {
  return "boost " + boost::lexical_cast<std::string>(BOOST_VERSION / 100000) +
