...
test_locale::test_locale(const char* c_name, boost::uint32_t lcid) {
  ...
  if (std::setlocale(LC_ALL, c_name)) {
    ...
    s_c_locale = no_test;
    std::cout << "The global C locale: " << c_name
              << " is not available and will not be tested." << std::endl;
  }
#else
  s_c_locale = no_test;
#endif
  // Disabled for VC15.7 (and later?) as the C runtime asserts if you pass an
  // invalid locale name to std::locale, rather than throwing the expected
  // exception.
#if !defined(BOOST_NO_STD_LOCALE) && !defined(BOOST_NO_EXCEPTIONS) && \
    !BOOST_WORKAROUND(BOOST_MSVC, > 1913)
  ...
  int r = ::GetLocaleInfoA(lcid,             // locale identifier
                           LOCALE_SCOUNTRY,  // information type
                           0,                // information buffer
                           0                 // size of buffer
  );
  p.reset(new char[r + 1]);
  r = ::GetLocaleInfoA(lcid,             // locale identifier
                       LOCALE_SCOUNTRY,  // information type
  ...
  int r =
      ::GetLocaleInfoW(lcid, LOCALE_IDEFAULTANSICODEPAGE, code_page_string, 7);
  BOOST_ASSERT(r != 0);

  UINT code_page = static_cast<UINT>(_wtol(code_page_string));

  boost::scoped_array<wchar_t> wp;
  r = ::GetLocaleInfoW(lcid,             // locale identifier
                       LOCALE_SCOUNTRY,  // information type
                       0,                // information buffer
                       0                 // size of buffer
  );
  ...
  int conv_r = ::WideCharToMultiByte(code_page, 0, wp.get(), r, p.get(),
                                     name_size, NULL, NULL);
  ...
  if (::IsValidLocale(lcid, LCID_INSTALLED)) {
    ...
    std::cout << "Testing the Win32 locale: \"" << p.get() << "\" (0x"
              << std::hex << lcid << ")" << std::endl;
  } else {
    s_win_locale = no_test;
    std::cout << "The Win32 locale: \"" << p.get() << "\" (0x" << std::hex
              << lcid << ") is not available and will not be tested."
              << std::endl;
  }
#else
  s_win_locale = no_test;
...
void test_en_locale(const char* name, boost::uint32_t lcid) {
  ...
  TEST_REGEX_SEARCH_L(
      ...
      "\xf0\xf1\xf2\xf3\xf4\xf5\xf6\xf8\xf9\xfa\xfb\xfc\xfd\xfe\xf7",
      match_default, make_array(1, 32, -2, -2));
  TEST_REGEX_SEARCH_L(
      "[[:upper:]]+", perl,
      "\xbf\xc0\xc1\xc2\xc3\xc4\xc5\xc6\xc7\xc8\xc9\xca\xcb\xcc\xcd\xce\xcf\xd0"
      "\xd1\xd2\xd3\xd4\xd5\xd6\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf",
      match_default, make_array(1, 31, -2, -2));
  //   TEST_REGEX_SEARCH_L("[[:punct:]]+", perl,
  //   "\xa1\xa2\xa3\xa4\xa5\xa6\xa7\xa8\xa9\xaa\xab\xac\xad\xae\xaf\xb0\xb1\xb2\xb3\xb4\xb5\xb6\xb7\xb8\xb9\xba\xbb\xbc\xbd\xbe\xbf\xc0",
  //   match_default, make_array(0, 31, -2, -2));
  TEST_REGEX_SEARCH_L(
      "[[:print:]]+", perl,
      "\xa1\xa2\xa3\xa4\xa5\xa6\xa7\xa8\xa9\xaa\xab\xac\xae\xaf\xb0\xb1\xb2\xb3"
      ...
      "\xfc\xfd\xfe",
      match_default, make_array(0, 93, -2, -2));
  TEST_REGEX_SEARCH_L(
      "[[:graph:]]+", perl,
      "\xa1\xa2\xa3\xa4\xa5\xa6\xa7\xa8\xa9\xaa\xab\xac\xae\xaf\xb0\xb1\xb2\xb3"
      "\xb4\xb5\xb6\xb7\xb8\xb9\xba\xbb\xbc\xbd\xbe\xbf\xc0\xc1\xc2\xc3\xc4\xc5"
  ...
  TEST_REGEX_SEARCH_L(
      ...
      "\xd2\xd3\xd4\xd5\xd6\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xe0\xe1\xe2\xe3\xe4"
  ...
  // causes these tests to crash (pointer overrun in
  // std::collate<wchar_t>::do_transform).
  TEST_REGEX_SEARCH_L(
      "[a-z]+", perl | ::boost::regex_constants::collate,
      "\xdf\xe0\xe1\xe2\xe3\xe4\xe5\xe7\xe8\xe9\xea\xeb\xec\xed\xee\xef\xf0\xf1"
      "\xf2\xf3\xf4\xf5\xf6\xf8\xf9\xfa\xfb\xfc",
      match_default, make_array(0, 28, -2, -2));
  TEST_REGEX_SEARCH_L(
      "[a-z]+", perl | ::boost::regex_constants::collate,
      "\xbf\xc0\xc1\xc2\xc3\xc4\xc5\xc7\xc8\xc9\xca\xcb\xcc\xcd\xce\xcf\xd0\xd1"
      "\xd2\xd3\xd4\xd5\xd6\xd8\xd9\xda\xdb\xdc",
      match_default, make_array(1, 28, -2, -2));
  if (lcid != 0x09) {
    // and equivalence classes, these fail for locale "en" but pass for "en_UK"
    // and "en_US":
    TEST_REGEX_SEARCH_L("[[=a=]]+", perl,
                        "aA\xe0\xe1\xe2\xe3\xe4\xe5\xc0\xc1\xc2\xc3\xc4\xc5",
                        match_default, make_array(0, 14, -2, -2));
  }
  // case mapping:
  TEST_REGEX_SEARCH_L(
      "[A-Z]+", perl | icase | ::boost::regex_constants::collate,
      "\xdf\xe0\xe1\xe2\xe3\xe4\xe5\xe7\xe8\xe9\xea\xeb\xec\xed\xee\xef\xf0\xf1"
      "\xf2\xf3\xf4\xf5\xf6\xf8\xf9\xfa\xfb\xfc",
      match_default, make_array(0, 28, -2, -2));
  TEST_REGEX_SEARCH_L(
  ...
  TEST_REGEX_SEARCH_L(
      ...
      perl | icase,
      "\xdf\xe0\xe1\xe2\xe3\xe4\xe5\xe6\xe7\xe8\xe9\xea\xeb\xec\xed\xee\xef\xf0"
      "\xf1\xf2\xf3\xf4\xf5\xf6\xf8\xf9\xfa\xfb\xfc\xfd\xfe",
      match_default, make_array(1, 30, -2, -2));
#endif
}
...
void test_en_locale() {
  ...
  // debugger, but hangs when run from bjam:
    ...
    !(defined(__ICL) && defined(_MSC_VER) && (_MSC_VER == 1200))
