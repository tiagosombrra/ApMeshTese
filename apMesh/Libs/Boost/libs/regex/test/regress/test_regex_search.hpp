...
void test_sub_match(const boost::sub_match<BidirectionalIterator>& sub,
                    BidirectionalIterator base, const int* answer_table, int i,
                    bool recurse = true) {
#ifdef BOOST_MSVC
#pragma warning(push)
#pragma warning(disable : 4244)
  ...
  if ((sub.matched == 0) && !((i == 0) && (test_info<charT>::match_options() &
                                           boost::match_partial))) {
    if (answer_table[2 * i] >= 0) {
      BOOST_REGEX_TEST_ERROR(
          "Sub-expression " << i
                            << " was not matched when it should have been.",
          charT);
    }
  } else {
    if (std::distance(base, sub.first) != answer_table[2 * i]) {
      BOOST_REGEX_TEST_ERROR("Error in start location of sub-expression "
                                 << i << ", found "
                                 << std::distance(base, sub.first)
                                 << ", expected " << answer_table[2 * i] << ".",
                             charT);
    }
...
void test_result(
    const boost::match_results<BidirectionalIterator, Allocator>& what,
    BidirectionalIterator base, const int* answer_table, bool recurse = true) {
  if (recurse) {
...
void test_simple_search(boost::basic_regex<charT, traits>& r) {
  ...
  if (boost::regex_search(search_text.begin(), search_text.end(), what, r,
    ...
    // setting match_any should have no effect on the result returned:
    else if (boost::regex_search(search_text.begin(), search_text.end(), r,
                                 opts | boost::regex_constants::match_any)) {
      BOOST_REGEX_TEST_ERROR(
  ...
  if (boost::regex_search(rsearch_text.begin(), rsearch_text.end(), rwhat, r,
                          opts)) {
...
void test_regex_token_iterator(boost::basic_regex<charT, traits>& r) {
  typedef typename std::basic_string<charT>::const_iterator const_iterator;
  typedef boost::regex_token_iterator<const_iterator, charT, traits>
      test_iterator;
  const std::basic_string<charT>& search_text = test_info<charT>::search_text();
  boost::regex_constants::match_flag_type opts =
      test_info<charT>::match_options();
  ...
  //
  test_iterator start2(search_text.begin(), search_text.end(), r, -1, opts),
      end2;
...
#pragma warning(disable : 4244)
    ...
    int expected_end = static_cast<int>(answer_table[0] < 0 ? search_text.size()
                                                            : answer_table[0]);
...
#pragma warning(pop)
    ...
    if ((start2 == end2) && (answer_table[0] >= 0)) {
      BOOST_REGEX_TEST_ERROR("Expected $0 match not found", charT);
    }
    if (start2 != end2) {
...
struct grep_test_predicate {
  ...
  grep_test_predicate(test_iter b, const int* a) : m_base(b), m_table(a) {}
    ...
    if ((answer_table[0] > 0) && boost::regex_match(search_text, r, opts)) {
      BOOST_REGEX_TEST_ERROR(
          ...
          charT);
               ...
               (answer_table[1] == static_cast<int>(search_text.size()))) {
      if (boost::regex_match(search_text.begin(), search_text.end(), what, r,
                             opts)) {
        test_result(what, search_text.begin(), answer_table);
      } else if (answer_table[0] >= 0) {
        // we should have had a match but didn't:
...
void test(boost::basic_regex<charT, traits>& r, const test_regex_search_tag&) {
  ...
  boost::regex_constants::syntax_option_type syntax_options =
      test_info<charT>::syntax_options();
...
#endif
  {
    ...
    if ((syntax_options & boost::regbase::save_subexpression_location) == 0) {
      ...
      if (!have_except) {
        BOOST_REGEX_TEST_ERROR(
            "Expected std::out_of_range error was not found.", charT);
      }
    }
#endif
    r.assign(expression,
             syntax_options | boost::regbase::save_subexpression_location);
    for (std::size_t i = 0; i < r.mark_count(); ++i) {
      std::pair<const charT*, const charT*> p = r.subexpression(i);
      if (*p.first != '(') {
        BOOST_REGEX_TEST_ERROR("Starting location of sub-expression "
                                   << i << " iterator was invalid.",
                               charT);
      }
      if (*p.second != ')') {
        BOOST_REGEX_TEST_ERROR("Ending location of sub-expression "
                                   << i << " iterator was invalid.",
                               charT);
      }
    }
    ...
    BOOST_REGEX_TEST_ERROR(
        "Received an unexpected std::runtime_error: " << e.what(), charT);
  } catch (const std::exception& e) {
    BOOST_REGEX_TEST_ERROR(
        "Received an unexpected std::exception: " << e.what(), charT);
  } catch (...) {
