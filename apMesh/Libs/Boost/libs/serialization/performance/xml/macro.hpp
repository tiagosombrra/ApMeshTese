...
#define BSL_EXP_OP(B, D)                                                     \
  (BOOST_PP_DEC(BOOST_PP_TUPLE_ELEM(3, 0, D)), BOOST_PP_TUPLE_ELEM(3, 1, D), \
   BOOST_PP_MUL_D(B, BOOST_PP_TUPLE_ELEM(3, 2, D),                           \
                  BOOST_PP_TUPLE_ELEM(3, 1, D))) /**/

#define BSL_EXP(X, N)                                                      \
  BOOST_PP_TUPLE_ELEM(3, 2,                                                \
                      BOOST_PP_WHILE(BSL_EXP_PRED, BSL_EXP_OP, (N, X, 1))) \
  /**/
...
#define BSL_NODE_xDECL_CTOR() \
  node(void) {}

#define BSL_NODE_DECL_CTOR(P) \
  BOOST_PP_IF(P, BSL_NODE_xDECL_CTOR, BOOST_PP_EMPTY)() /**/
...
#define BSL_NODE_INIT_LIST(Z, N, _)                             \
  BOOST_PP_COMMA_IF(N)                                          \
  BOOST_PP_CAT(element, N) BOOST_PP_LPAREN() BOOST_PP_CAT(p, N) \
      BOOST_PP_RPAREN() /**/

#define BSL_NODE_DECL(Z, N, _)                                                \
  template <BOOST_PP_ENUM_PARAMS_Z(Z, N, typename T)>                         \
  struct node<BOOST_PP_ENUM_PARAMS_Z(Z, N, T) BOOST_PP_COMMA_IF(N)            \
                  BOOST_PP_ENUM_##Z(BOOST_PP_SUB(BSL_NODE_MAX, N), BSL_PRINT, \
                                    unused_type)> {                           \
...
#define BSL_INST_BASE(Z, N, L)                                \
  T0 T0##_##N(BOOST_PP_ENUM_##Z(                              \
      ...
      boost::archive::xml::random<BSL_TYPE> BOOST_PP_LPAREN() \
          BOOST_PP_RPAREN()));                                \
  /**/
...
#define BSL_INST_yNODES(Z, N, L)                                              \
  ...
  BOOST_PP_CAT(                                                               \
      ...
      BOOST_PP_CAT(BOOST_PP_LIST_AT(L, 1),                                    \
                   BOOST_PP_CAT(_, BOOST_PP_ADD(N, BOOST_PP_LIST_AT(L, 0))))) \
  /**/

#define BSL_INST_xNODES(Z, N, L)                                             \
  T##L T##L##_##N(BOOST_PP_REPEAT_##Z(                                       \
      BSL_NODE_MAX, BSL_INST_yNODES,                                         \
      (BOOST_PP_MUL(N, BSL_NODE_MAX), (BOOST_PP_SUB(L, 1), BOOST_PP_NIL)))); \
  /**/

#define BSL_INST_NODES(Z, N, L)                                          \
  BOOST_PP_REPEAT_##Z(BSL_EXP(BSL_NODE_MAX, BOOST_PP_SUB(BSL_DEPTH, N)), \
                      BSL_INST_xNODES, N) /**/

#define BSL_TYPEDEF_NODES(Z, N, L)                                   \
  typedef boost::archive::xml::node<BOOST_PP_ENUM_##Z(               \
...
#define BSL_MAIN                                                             \
  int main(void) {                                                           \
    ...
    BOOST_PP_REPEAT_FROM_TO(1, BSL_DEPTH, BSL_TYPEDEF_NODES, _)              \
                                                                             \
    typedef node<BOOST_PP_ENUM(BSL_NODE_MAX, BSL_PRINT,                      \
    ...
    while (rounds-- > 0) {                                                   \
      ...
      std::pair<double, HEAD> r = restore_archive<HEAD>(fn);                 \
                                                                             \
      std::cout << "round " << ((BSL_ROUNDS - 1) - rounds) << " -> " << fn   \
                << "\n";                                                     \
                                                                             \
      BOOST_PP_IF(BSL_SAVE_TMPFILE, BOOST_PP_EMPTY(),                        \
                  std::remove(fn.c_str());)                                  \
                                                                             \
      results.entries.push_back(entry(BOOST_PP_STRINGIZE(BSL_TYPE), BSL_EXP(BSL_NODE_MAX, BSL_DEPTH), r.first));            \
    }                                                                        \
                                                                             \
