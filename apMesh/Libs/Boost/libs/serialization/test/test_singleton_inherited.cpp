...
// Test the singleton class for a "inherited" singleton (used as Foo:public
// singleton<Foo>) This can be uses as singleton<Foo>::get_const_instance() OR
// Foo::get_const_instance()
//
...
// b) This is intended to be used by shared libraries too which would then need
// their own report_errors call We halso have to disable the Wterminate warning
// as we call this from dtors C++ will terminate the program in such cases which
// is OK here
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wterminate"
#define THROW_ON_FALSE(cond) \
  if (!(cond))               \
  throw std::runtime_error(  \
      __FILE__ "(" BOOST_PP_STRINGIZE(__LINE__) ") Assertion failed: " #cond)

// Enum to designate the state of the singletonized instances
enum ConstructionState { CS_UNINIT, CS_INIT, CS_DESTROYED };

// We need another singleton to check for the destruction of the singletons at
// program exit We don't need all the magic for shared library anti-optimization
// and can keep it very simple
struct controller {
...
struct Foo : boost::serialization::singleton<Foo> {
  Foo() : i(42) {
    // access controller singleton. Therefore controller will be constructed
    // before this
    THROW_ON_FALSE(controller::getInstance().state == CS_UNINIT);
    controller::getInstance().state = CS_INIT;
  }
  ~Foo() {
    // Because controller is constructed before this, it will be destructed
    // AFTER this. Hence controller is still valid
...
controller::~controller() {
  ...
  // If this fails, then the destroyed flag is not set and one may use a deleted
  // instance if relying on this flag
...
int test_main(int /* argc */, char* /* argv */[]) {
  ...
  BOOST_CHECK(boost::serialization::singleton<Foo>::get_const_instance().i ==
              42);
