...
void save_unregistered1(const char *testfile) {
  ...
  bool except = false;
  BOOST_TRY { oa << BOOST_SERIALIZATION_NVP(rb1); }
  BOOST_CATCH(boost::archive::archive_exception const &aex) { except = true; }
  BOOST_CATCH_END
...
void save_unregistered2(const char *testfile) {
  ...
  bool except = false;
  BOOST_TRY { oa << BOOST_SERIALIZATION_NVP(rd1); }
  BOOST_CATCH(boost::archive::archive_exception const &aex) { except = true; }
  BOOST_CATCH_END
...
void load_unregistered2(const char *testfile) {
  ...
  bool except = false;
  BOOST_TRY { ia >> BOOST_SERIALIZATION_NVP(rd1); }
  BOOST_CATCH(boost::archive::archive_exception const &aex) {
    except = true;
    BOOST_CHECK_MESSAGE(NULL == rd1,
                        "failed load resulted in a non-null pointer");
  }
  BOOST_CATCH_END
  BOOST_CHECK_MESSAGE(!except, "registration not detected !");
...
void save_unregistered_pointer(const char *testfile) {
  ...
  BOOST_CATCH(...) { BOOST_CHECK_MESSAGE(false, "unexpected exception"); }
  BOOST_CATCH_END
...
void load_unregistered_pointer(const char *testfile) {
  ...
  BOOST_CATCH(boost::archive::archive_exception const &aex) {
    ...
    BOOST_CHECK_MESSAGE(std::strcmp(aex.what(), "unregistered class") == 0,
                        "incorrect exception");
  }
  BOOST_CATCH_END
  BOOST_CHECK_MESSAGE(except, "lack of registration not detected !");
