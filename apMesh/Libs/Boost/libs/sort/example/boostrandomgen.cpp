...
int main(int argc, const char **argv) {
  ...
  if (argc > 1) mod_shift = atoi(argv[1]) - 1;
  std::ofstream ofile;
  ...
  if (mod_shift < 31 && mod_shift >= 0) {
    max_int %= 1 << mod_shift;
    if (-max_int > min_int) min_int = -max_int;
  }
  ...
  if (argc > 3 && (string(argv[3]) == "-normal")) {
    ...
    variate_generator<mt19937 &, lognormal_distribution<> > gen(rng,
                                                                everything);
    generate(result.begin(), result.end(), gen);
  } else {
  ...
  ofile.write(reinterpret_cast<char *>(&(result[0])),
