...
void execute_test(DistrF distr, std::string distr_name, int repeats) {
  ...
  for (size_t sz = 1; sz <= 1000; sz *= 10) {
    ...
    for (int i = 0; i < repeats; ++i) {
      ...
      BOOST_CHECK_MESSAGE(std::is_sorted(u32l.begin(), u32l.end(), u32less),
                          "pdqsort<uint32_t, std::less> " + distr_name +
                              " failed with size " + std::to_string(sz));

      BOOST_CHECK_MESSAGE(std::is_sorted(u32g.begin(), u32g.end(), u32greater),
                          "pdqsort<uint32_t, std::greater> " + distr_name +
                              " failed with size " + std::to_string(sz));

      BOOST_CHECK_MESSAGE(std::is_sorted(sl.begin(), sl.end(), sless),
                          "pdqsort<std::string, std::less> " + distr_name +
                              " failed with size " + std::to_string(sz));

      BOOST_CHECK_MESSAGE(std::is_sorted(sg.begin(), sg.end(), sgreater),
                          "pdqsort<std::string, std::greater> " + distr_name +
                              " failed with size " + std::to_string(sz));
    }
  }
}
