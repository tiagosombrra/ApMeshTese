...
class actor_string {
 public:
  actor_string(std::string &rstr) : matched(rstr) {}
...
int main() {
  ...
  cpp_comment = comment_p("/*", "*/")  // rule for C-comments
  ...
  if (result.hit) {
    cout << "Parsed HTML snippet \"<b>Body text</b>\" successfully "
            "(with re-attached actor)!"
         << endl;
    cout << "Found body (" << (int)body.size() << " characters): ";
    cout << "\"" << body << "\"" << endl;
  } else {
    cout << "Failed to parse HTML snippet (with re-attached actor)!" << endl;
  }
  cout << endl;
  ...
  //      confix parser by hand. To see, how it fails, you can try the
  //      following:
  //
  //          html_tag_direct =
  //              confix_p.direct(
  ...
  html_tag_direct = confix_p.direct(
      str_p("<b>"), (*(body_text - str_p("</b>")))[actor_string(bodydirect)],
      str_p("</b>"));
