...
class declaration_type {
 public:
  ...
  declaration_type(string const &type_) : type(vartype_unknown) { init(type_); }
...
//
///////////////////////////////////////////////////////////////////////////////
struct var_decl_closure
    : BOOST_SPIRIT_CLASSIC_NS::closure<var_decl_closure, declaration_type> {
  member1 val;
};
...
class symbols_with_data {
 public:
  typedef symbol_inserter<T, BOOST_SPIRIT_CLASSIC_NS::impl::tst<T, char> >
      symbol_inserter_t;
  ...
  symbol_inserter_t const &operator()(IteratorT const &first_,
...
inline symbols_with_data<T, InitT> symbols_gen(
    symbol_inserter<T, BOOST_SPIRIT_CLASSIC_NS::impl::tst<T, CharT> > const
        &add_,
    InitT const &data_) {
...
struct var_decl_list
  ...
  struct definition {
    definition(var_decl_list const &self) {
      ...
      type = str_p("int")[type.val = construct_<string>(arg1, arg2)] |
    ...
    rule<ScannerT> const &start() const { return decl; }
...
int main() {
  ...
  if (parse(pbegin, decl[assign(type)]).full) {
    ...
    cout << endl << "Parsing the input stream failed!" << endl;
  }
  return 0;
}
