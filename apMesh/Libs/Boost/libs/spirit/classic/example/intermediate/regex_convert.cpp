...
//  the Boost.Regex library from one translation unit only. Otherwise you have
//  to link with the Boost.Regex library as defined in the related documentation
...
namespace {
long triple(long val) { return 3 * val; }
///////////////////////////////////////////////////////////////////////////
//
// actions
//
struct emit_constant {
  emit_constant(string const &text) : msg(text) {}
...
void copy_unmodified(char letter) { cout.rdbuf()->sputc(letter); }

struct emit_modified_subscript {
  emit_modified_subscript(boost::function<long(long)> const &f) : modifier(f) {}
...
struct conversion_grammar : grammar<conversion_grammar> {
