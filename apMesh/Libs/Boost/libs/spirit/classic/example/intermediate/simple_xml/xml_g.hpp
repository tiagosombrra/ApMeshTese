...
#define BOOST_SPIRIT_DEBUG_FLAGS \
  (BOOST_SPIRIT_DEBUG_FLAGS_MAX - BOOST_SPIRIT_DEBUG_FLAGS_CLOSURES)
#endif
...
struct attrib_cls : SP::closure<attrib_cls, std::pair<std::string, std::string>,
...
struct tagged_cls : SP::closure<tagged_cls, tag, std::string,
...
struct xml_g : SP::grammar<xml_g> {
  ...
  struct definition {
    definition(const xml_g &s) {
      ...
      tagged = (start_tag >> *inner >> end_tag | simple_start_tag)[store_tag(
          tagged.value, tagged.ID, tagged.attribs, tagged.children)];

      end_tag = SP::str_p("</") >> SP::f_str_p(tagged.ID) >> '>';

      inner = (tagged | str)[push_child(tagged.children, arg1)];

      str = SP::lexeme_d[+(SP::anychar_p - '<')]
                        [str.value = construct_<std::string>(arg1, arg2)];

      top = +tagged[push_back(var(s.tags), arg1)];

      starter = SP::ch_p('<') >>
                SP::lexeme_d[+SP::alpha_p]
                            [tagged.ID = construct_<std::string>(arg1, arg2)] >>
                *attrib[store_in_map(tagged.attribs, arg1)] >> !white;
      start_tag = starter >> '>';
    ...
    SP::rule<Scan> const &start() const { return top; }
  };
};
