...
static void parse(char const *s, ParserT const &p, bool match) {
...
// primitive parsers
static void epsilon_as_primitive() {
...
// functors
static void epsilon_as_parser_generator_for_functors() {
  bool flag = false;
...
// a parser generator for creating parsers from
// other parsers
static void epsilon_as_parser_generator_for_parsers() {
...
static void negation_operator_for_epsilon() {
  bool flag = false;
  parse(empty, ~BOOST_SPIRIT_CLASSIC_NS::epsilon_p(var(flag)), !flag);
  BOOST_TEST(full_match);
  parse(empty, ~~BOOST_SPIRIT_CLASSIC_NS::epsilon_p(var(flag)), flag);
