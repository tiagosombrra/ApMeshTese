...
void test_for(char const* s, bool succeed, rule_t const& r,
  ...
  cout << "     " << iterations_performed << " of " << iterations_desired
       << " iterations\n";
}
...
int main() {
  ...
  for_rule = uint_p[assign_a(iterations_desired)] >> ':' >>
             for_p(&zero, cmp(), inc())["xy"][assign_a(input_matched)];

  for_rule2 = for_p(&zero, '.', inc())["xy"][assign_a(input_matched)];
