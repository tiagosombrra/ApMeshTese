...
//
///////////////////////////////////////////////////////////////////////////////
void traverse_identity_tests() {
  //  test type equality
  typedef sequence<chlit<char>, chlit<char> > test_sequence1_t;
  BOOST_STATIC_ASSERT(
      (::boost::is_same<test_sequence1_t,
                        post_order::result<identity_transform,
                                           test_sequence1_t>::type>::value));

  //  test (rough) runtime equality
  BOOST_TEST(
      parse("ab", post_order::traverse(identity_transform(), ch_p('a') >> 'b'))
          .full);
  BOOST_TEST(
      !parse("ba", post_order::traverse(identity_transform(), ch_p('a') >> 'b'))
           .hit);

  ///////////////////////////////////////////////////////////////////////////
  BOOST_TEST(!parse("cba", post_order::traverse(identity_transform(),
                                                ch_p('a') >> 'b' >> 'c'))
                  .hit);
  ...
  //  test: ((a >> b) >> c) >> d
  typedef sequence<
      sequence<sequence<kleene_star<chlit<> >, action<chlit<>, assign_actor> >,
               chlit<> >,
      optional<chlit<> > >
      test_sequence2_t;

  BOOST_STATIC_ASSERT(
      (::boost::is_same<test_sequence2_t,
                        post_order::result<identity_transform,
                                           test_sequence2_t>::type>::value));
  ...
  ///////////////////////////////////////////////////////////////////////////////
  //  test: (a >> (b >> c)) >> d
  typedef sequence<sequence<kleene_star<chlit<> >,
                            sequence<action<chlit<>, assign_actor>, chlit<> > >,
                   optional<chlit<char> > >
      test_sequence3_t;

  BOOST_STATIC_ASSERT(
      (::boost::is_same<test_sequence3_t,
                        post_order::result<identity_transform,
                                           test_sequence3_t>::type>::value));
  ...
  ///////////////////////////////////////////////////////////////////////////////
  //  test: a >> (b >> (c >> d))
  typedef sequence<kleene_star<chlit<> >,
                   sequence<action<chlit<>, assign_actor>,
                            sequence<chlit<>, optional<chlit<> > > > >
      test_sequence4_t;

  BOOST_STATIC_ASSERT(
      (::boost::is_same<test_sequence4_t,
                        post_order::result<identity_transform,
                                           test_sequence4_t>::type>::value));

  c = 0;
  BOOST_TEST(parse("aabcd",
                   post_order::traverse(identity_transform(),
                                        *ch_p('a') >> (ch_p('b')[assign_a(c)] >>
                                                       ('c' >> !ch_p('d')))))
                 .full);
  BOOST_TEST(c == 'b');
  ...
  //  test: a >> ((b >> c) >> d)
  typedef sequence<kleene_star<chlit<> >,
                   sequence<sequence<action<chlit<>, assign_actor>, chlit<> >,
                            optional<chlit<> > > >
      test_sequence5_t;

  BOOST_STATIC_ASSERT(
      (::boost::is_same<test_sequence5_t,
                        post_order::result<identity_transform,
                                           test_sequence5_t>::type>::value));
  ...
  //  test: (a >> b) >> (c >> d)
  typedef sequence<
      sequence<kleene_star<chlit<> >, action<chlit<>, assign_actor> >,
      sequence<chlit<>, optional<chlit<> > > >
      test_sequence6_t;

  BOOST_STATIC_ASSERT(
      (::boost::is_same<test_sequence6_t,
                        post_order::result<identity_transform,
                                           test_sequence6_t>::type>::value));

  c = 0;
  BOOST_TEST(parse("aabcd", post_order::traverse(
                                identity_transform(),
                                (*ch_p('a') >> ch_p('b')[assign_a(c)]) >>
                                    ('c' >> !ch_p('d'))))
                 .full);
...
class trace_identity_transform
 ...
 public:
  ...
  generate_plain(ParserT const &parser_, EnvT const &env) const {
    OSSTREAM strout;
    strout << EnvT::node << ": plain (" << EnvT::level << ", " << EnvT::index
           << "): " << parser_name(parser_);
  ...
  generate_unary(UnaryT const &unary_, SubjectT const &subject_,
    ...
    OSSTREAM strout;
    strout << EnvT::node << ": unary (" << EnvT::level
           << "): " << parser_name(unary_);
    traces.push_back(GETSTRING(strout));
  ...
  generate_action(ActionT const &action_, SubjectT const &subject_,
                  EnvT const &env) const {
    OSSTREAM strout;
    strout << EnvT::node << ": action(" << EnvT::level
           << "): " << parser_name(action_);
    traces.push_back(GETSTRING(strout));
  ...
  typename parser_traversal_binary_result<self_t, BinaryT, LeftT, RightT,
                                          EnvT>::type
  generate_binary(BinaryT const &binary_, LeftT const &left_,
                  RightT const &right_, EnvT const &env) const {
    OSSTREAM strout;
    strout << EnvT::node << ": binary(" << EnvT::level
           << "): " << parser_name(binary_);
    traces.push_back(GETSTRING(strout));
...
void post_order_trace_test(ParserT const &parser_, char const *first[],
                           size_t cnt) {
  // traverse
  ...
  std::vector<std::string>::const_iterator it =
      trace_vector.get_output().begin();
  std::vector<std::string>::const_iterator end =
      trace_vector.get_output().end();
...
void traverse_trace_tests() {
  ...
  post_order_trace_test(ch_p('a') >> 'b', test_result1, _countof(test_result1));
  ...
  const char *test_result2[] = {
      ...
      "5: plain (2, 2): chlit('c')",
      "6: binary(1): sequence[sequence[kleene_star[chlit('a')], "
      "action[chlit('b')]], chlit('c')]",
      "7: plain (2, 3): chlit('d')",
      "8: unary (1): optional[chlit('d')]",
      "9: binary(0): sequence[sequence[sequence[kleene_star[chlit('a')], "
      "action[chlit('b')]], chlit('c')], optional[chlit('d')]]",
  };
  ...
  const char *test_result3[] = {
      ...
      "6: binary(1): sequence[kleene_star[chlit('a')], "
      "sequence[action[chlit('b')], chlit('c')]]",
      "7: plain (2, 3): chlit('d')",
      "8: unary (1): optional[chlit('d')]",
      "9: binary(0): sequence[sequence[kleene_star[chlit('a')], "
      "sequence[action[chlit('b')], chlit('c')]], optional[chlit('d')]]",
  };
  ...
  const char *test_result4[] = {
      ...
      "8: binary(1): sequence[action[chlit('b')], sequence[chlit('c'), "
      "optional[chlit('d')]]]",
      "9: binary(0): sequence[kleene_star[chlit('a')], "
      "sequence[action[chlit('b')], sequence[chlit('c'), "
      "optional[chlit('d')]]]]",
  };
  ...
  const char *test_result5[] = {
      ...
      "8: binary(1): sequence[sequence[action[chlit('b')], chlit('c')], "
      "optional[chlit('d')]]",
      "9: binary(0): sequence[kleene_star[chlit('a')], "
      "sequence[sequence[action[chlit('b')], chlit('c')], "
      "optional[chlit('d')]]]",
  };
  ...
  const char *test_result6[] = {
      ...
      "9: binary(0): sequence[sequence[kleene_star[chlit('a')], "
      "action[chlit('b')]], sequence[chlit('c'), optional[chlit('d')]]]",
  };
