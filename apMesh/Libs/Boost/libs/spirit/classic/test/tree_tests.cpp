...
template <typename ScannerT, unsigned long ID = 0,
          typename ContextT = parser_context<> >
class rule_id : public rule<ScannerT, ContextT, parser_tag<ID> > {
...
enum RULE_ID { ID_A = 1, ID_B, ID_C, ID_ROOT };
...
bool equal(TreeT& a, TreeT& b) {
  ...
  if (num_children(a) != num_children(b)) return false;

  unsigned n = num_children(a);
  for (unsigned i = 0; i < n; i++)
    if (!equal(child(a, i), child(b, i))) return false;
...
void dump(ostream& o, TreeT& t, int level = 0) {
  string name;
  string value;
  map<parser_id, string>::iterator iter = rule_names.find(id(t));
  ...
  for (unsigned c = 0; c < n; c++) dump(o, child(t, c), level + 1);
}
...
struct fold_node {
  // assign a subtree
  void operator()(ParmT& t, ParmT ch) const { t = ch; }
...
template <typename TreeT, typename T, typename T1, typename T2, typename T3,
...
TreeT fold(T p, T1 p1, T2 p2, T3 p3, T4 p4, T5 p5, T6 p6, T7 p7, T8 p8) {
  ...
  if (NUM_CHILDREN > 0) t.children.resize(NUM_CHILDREN);
...
#define DEFINE_FOLD(Z, N, _)                                                  \
  ...
  TreeT fold(T p BOOST_PP_COMMA_IF(N) BOOST_PP_ENUM_BINARY_PARAMS(N, T, p)) { \
    return fold<TreeT>(p BOOST_PP_COMMA_IF(N) BOOST_PP_ENUM_PARAMS(           \
        N, p) BOOST_PP_COMMA_IF(BOOST_PP_SUB(8, N))                           \
...
struct test_banal : public grammar<test_banal> {
  ...
  struct definition {
    ...
    const rule_id<T, ID_ROOT>& start() { return root; }
  };

  const char* pattern(void) { return "bc"; }

  template <typename TreeT>
  TreeT expected_tree(void) {
    return fold<TreeT>(ID_ROOT, fold<TreeT>(ID_A, ID_B), ID_C);
  }
};
...
struct run_test {
  ...
  void operator()(TestT gram) {
    ...
    typedef typename factory_t ::BOOST_NESTED_TEMPLATE
        factory<iterator_t>::node_t node_t;
