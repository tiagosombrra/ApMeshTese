...
//
//  We use explicit token attribute types, making the corresponding token
//  instances carry convert the matched input into an instance of that type. The
//  token attribute is exposed as the parser attribute if this token is used as
//  a parser component somewhere in a grammar.
//
...
struct example4_grammar : qi::grammar<Iterator, qi::in_state_skipper<Lexer> > {
  template <typename TokenDef>
  example4_grammar(TokenDef const& tok) : example4_grammar::base_type(program) {
    ...
    if_stmt = (tok.if_ >> '(' >> expression >> ')' >> block >>
               -(tok.else_ >>
                 block))[std::cout << val("if expression: ") << _2 << "\n"];

    while_stmt = (tok.while_ >> '(' >> expression >> ')' >>
                  block)[std::cout << val("while expression: ") << _2 << "\n"];
  ...
  qi::rule<Iterator, expression_type(), qi::in_state_skipper<Lexer> >
      expression;
};
...
int main() {
  ...
  typedef example4_grammar<iterator_type, example4_tokens::lexer_def>
      example4_grammar;
