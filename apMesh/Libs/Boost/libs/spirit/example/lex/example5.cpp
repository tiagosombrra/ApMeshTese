...
struct example5_grammar : example5_base_grammar<Iterator, Lexer> {
  ...
  example5_grammar(TokenDef const& tok)
    ...
    this->if_stmt = this->if_stmt.copy() >> -(tok.else_ >> this->block);
  }
};
...
int main() {
  ...
  typedef example5_grammar<iterator_type, example5_tokens::lexer_def>
      example5_grammar;
