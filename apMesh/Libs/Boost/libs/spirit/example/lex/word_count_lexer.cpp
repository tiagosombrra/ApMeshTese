...
struct distance_func {
  ...
  template <typename Iterator1, typename Iterator2>
  typename result<Iterator1, Iterator2>::type operator()(
      Iterator1 const& begin, Iterator2 const& end) const {
...
int main(int argc, char* argv[]) {
  /*<  Specifying `omit` as the token attribute type generates a token class
       not holding any token attribute at all (not even the iterator range of
  the matched input sequence), therefore optimizing the token, the lexer, and
       ...
       aggressive optimizations. As a result the token instances contain the
  token ids as the only data member.
  >*/
