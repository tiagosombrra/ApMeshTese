...
#if !defined( \
    BOOST_SPIRIT_LEXER_NEXT_TOKEN_CONJURE_STATIC_SWITCH_JUL_25_2011_07_03_08)
...
char const* const lexer_state_names_conjure_static_switch[1] = {"INITIAL"};
...
std::size_t next_token_conjure_static_switch(std::size_t& /*start_state_*/,
                                             bool& /*bol_*/,
  ...
  if ((ch_ >= 'A' && ch_ <= 'Z') || ch_ == '_' || (ch_ >= 'a' && ch_ <= 'd') ||
      (ch_ >= 'g' && ch_ <= 'h') || (ch_ >= 'j' && ch_ <= 'q') || ch_ == 's' ||
      ch_ == 'u' || (ch_ >= 'x' && ch_ <= 'z'))
    goto state0_25;
...
state0_2:
  ...
  if ((ch_ >= '0' && ch_ <= '9') || (ch_ >= 'A' && ch_ <= 'Z') || ch_ == '_' ||
      (ch_ >= 'a' && ch_ <= 'q') || (ch_ >= 's' && ch_ <= 'z'))
    goto state0_25;
...
struct lexer_conjure_static_switch {
  ...
  static std::size_t next(std::size_t& start_state_, bool& bol_,
                          ...
                          std::size_t& unique_id_) {
    return next_token_conjure_static_switch(start_state_, bol_, start_token_,
                                            end_, unique_id_);
  }
};
