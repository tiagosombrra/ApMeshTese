...
struct value {
 ...
 private:
  ...
  value(llvm::Value* v, bool is_lvalue_, llvm::IRBuilder<>* builder);
...
struct basic_block {
 ...
 private:
  basic_block(llvm::BasicBlock* b) : b(b) {}

  operator llvm::BasicBlock*() const { return b; }
...
struct function {
 private:
  ...
  typedef boost::transform_iterator<to_value, arg_iterator> arg_val_iterator;
 ...
 private:
  function(llvm::Function* f, llvm_compiler* c) : f(f), c(c) {}
...
struct llvm_compiler {
  llvm_compiler(vmachine& vm)
      : llvm_builder(context()), vm(vm), fpm(vm.module()) {
    init_fpm();
  }
  ...
  function declare_function(bool void_return, std::string const& name,
                            std::size_t nargs);

  basic_block make_basic_block(char const* name, function parent = function(),
                               basic_block before = basic_block());

  basic_block get_insert_block();
  void set_insert_point(basic_block b);

  void conditional_branch(value cond, basic_block true_br,
 ...
 protected:
  ...
  llvm::IRBuilder<>& builder() { return llvm_builder; }
 ...
 private:
  ...
  template <typename C>
  llvm::Value* call_impl(function callee, C const& args);
...
struct compiler : llvm_compiler {
  ...
  compiler(vmachine& vm, ErrorHandler& error_handler_) : llvm_compiler(vm) {
 ...
 private:
  value compile_binary_expression(value lhs, value rhs, token_ids::type op);
