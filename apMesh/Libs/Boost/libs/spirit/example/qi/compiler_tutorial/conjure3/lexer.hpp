...
struct get_lexer_type {
  ...
  typedef boost::mpl::vector<std::string, unsigned int, bool> token_value_types;
  ...
  typedef lex::lexertl::position_token<BaseIterator, token_value_types,
                                       boost::mpl::false_>
      token_type;
  ...
  typedef lex::lexertl::static_actor_lexer<
      token_type, boost::spirit::lex::lexertl::static_::lexer_conjure_static>
      type;
#elif CONJURE_LEXER_STATIC_SWITCH != 0
...
struct conjure_tokens
 ...
 private:
  ...
  typedef
      typename boost::spirit::result_of::terminal<boost::spirit::tag::raw_token(
          token_ids::type)>::type raw_token_spec;

  typedef typename boost::spirit::result_of::terminal<boost::spirit::tag::token(
      token_ids::type)>::type token_spec;
 ...
 protected:
  ...
  struct keyword_adder {
    ...
    keyword_adder& operator()(std::string const& keyword,
                              int id = token_ids::invalid) {
      l.add_(keyword, id);
      return *this;
    }
  };
 ...
 public:
  ...
  raw_token_spec operator()(std::string const& kwd) const {
    ...
    return raw_token((it != keywords_.end()) ? (*it).second
                                             : token_ids::invalid);
  }
