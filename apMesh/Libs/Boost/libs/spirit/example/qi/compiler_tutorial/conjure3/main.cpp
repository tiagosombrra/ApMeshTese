...
int main(int argc, char** argv) {
  ...
  client::error_handler<base_iterator_type, iterator_type> error_handler(
      first, last);  // Our error handler
  client::parser::function<iterator_type, lexer_type> function(
      error_handler, lexer);                               // Our parser
  client::code_gen::compiler compiler(vm, error_handler);  // Our compiler
  ...
  if (success && iter == end) {
    if (compiler(ast)) {
      ...
      if (main_function.arity() != nargs) {
        std::cerr << "Error: main function requires " << main_function.arity()
                  << " arguments." << std::endl;
      ...
      switch (nargs) {
        ...
        case 1:
          r = main_function(boost::lexical_cast<int>(args[0]));
          break;
