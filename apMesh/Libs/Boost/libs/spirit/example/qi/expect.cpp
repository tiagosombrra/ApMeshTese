...
void parse(const std::string message, const std::string& input,
           const std::string& rule, const Parser& parser) {
...
struct ident : qi::grammar<Iterator, std::string(), qi::space_type> {
  ...
  qi::rule<iterator_type, std::string(), qi::space_type> id, id_list,
      qualified_id;
};
...
int main() {
  ...
  parse("expect directive, fail on first", "1234; id2;",
        "qi::expect[ id  >> qi::lit(';') ]", id);

  parse("expect directive, fail on second", "id1, id2",
        "qi::expect[ id  >> qi::lit(';') ]", id);

  parse("expect directive, success", "id1;",
        "qi::expect[ id  >> qi::lit(';') ]", id);
