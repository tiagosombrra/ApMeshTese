...
struct compare_result {
  compare_result(Iterator pos, bool full_match)
...
compare_result<typename Range::const_iterator> compare(
    Range const& in, Range const& tem, char const* re_prefix = "<%",
...
////////////////////////////////////////////////////////////////////////////
template <typename F>
bool compare(fs::path input_path, fs::path expect_path, F f,
             char const* re_prefix = "<%", char const* re_suffix = "%>");
...
template <typename Iterator>
inline bool is_regex(Iterator& first, Iterator last, std::string& re,
                     char const* re_prefix, char const* re_suffix) {
  boost::regex e(re_prefix + std::string("(.*?)") + re_suffix);
  boost::match_results<Iterator> what;
  if (boost::regex_search(first, last, what, e,
...
template <typename Range>
inline compare_result<typename Range::const_iterator> compare(
    Range const& in, Range const& tem, char const* re_prefix,
    char const* re_suffix) {
  ...
  while (in_first != in_last && tem_first != tem_last) {
    if (is_regex(tem_first, tem_last, re, re_prefix, re_suffix)) {
      ...
      if (*in_first++ != *tem_first++) return compare_result_t(in_first, false);
    }
  }
...
inline int for_each_file(fs::path p, F f) {
  try {
    if (fs::exists(p) && fs::is_directory(p)) {
      for (auto i = fs::directory_iterator(p); i != fs::directory_iterator();
           ++i) {
      ...
      std::cerr << "Directory: " << fs::absolute(p) << " does not exist."
                << std::endl;
      return 1;
    }
  }
...
template <typename F>
inline bool compare(fs::path input_path, fs::path expect_path, F f,
                    char const* re_prefix, char const* re_suffix) {
  ...
  if (!result.full_match) {
    ...
    std::cerr << "==== File: " << expect_path << ", Line: " << line
              << ", Column: " << col << std::endl;
    std::cerr << "=============================================" << std::endl;
