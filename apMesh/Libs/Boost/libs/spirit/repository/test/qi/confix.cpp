...
struct confix_spec_traits {
  typedef typename spirit::result_of::terminal<repo::tag::confix(
      Prefix, Suffix)>::type type;
};

template <typename Prefix, typename Suffix>
inline typename confix_spec_traits<Prefix, Suffix>::type confix_spec(
    Prefix const& prefix, Suffix const& suffix) {
  return repo::confix(prefix, suffix);
}

inline confix_spec_traits<std::string>::type confix_spec(const char* prefix,
                                                         const char* suffix) {
  return repo::confix(std::string(prefix), std::string(suffix));
}
confix_spec_traits<std::string>::type const c_comment = confix_spec("/*", "*/");
confix_spec_traits<std::string>::type const cpp_comment =
    confix_spec("//", "\n");
}  // namespace comment
