...
int main() {
  ...
  {  // test optional attributes
    ...
    BOOST_TEST(
        binary_test("\x00\x00\x00\x00\x00\x00\xf0\x3f", 8, bin_double, vd));
    ...
    boost::optional<double> vd(1.0);
    BOOST_TEST(
        binary_test("\x3f\xf0\x00\x00\x00\x00\x00\x00", 8, bin_double, vd));
...
// we support Phoenix attributes only starting with V2.2
  ...
  {  // test Phoenix expression attributes, only supported if
    ...
    BOOST_TEST(
        binary_test("\x01\x02\x03\x04", 4, dword, phoenix::val(0x04030201)));
...
#endif
    BOOST_TEST(
        binary_test("\x00\x00\x80\x3f", 4, bin_float, phoenix::val(1.0f)));
    float vf(1.0f);
    BOOST_TEST(binary_test("\x00\x00\x80\x3f", 4, bin_float, phoenix::ref(vf)));
    BOOST_TEST(
        binary_test("\x00\x00\x00\x40", 4, bin_float, ++phoenix::ref(vf)));

    BOOST_TEST(binary_test("\x00\x00\x00\x00\x00\x00\xf0\x3f", 8, bin_double,
                           phoenix::val(1.0)));
    double vd(1.0);
    BOOST_TEST(binary_test("\x00\x00\x00\x00\x00\x00\xf0\x3f", 8, bin_double,
                           phoenix::ref(vd)));
    BOOST_TEST(binary_test("\x00\x00\x00\x00\x00\x00\x00\x40", 8, bin_double,
                           ++phoenix::ref(vd)));
    ...
    BOOST_TEST(
        binary_test("\x01\x02\x03\x04", 4, dword, phoenix::val(0x01020304)));
...
#endif
    BOOST_TEST(
        binary_test("\x3f\x80\x00\x00", 4, bin_float, phoenix::val(1.0f)));
    float vf(1.0f);
    BOOST_TEST(binary_test("\x3f\x80\x00\x00", 4, bin_float, phoenix::ref(vf)));
    BOOST_TEST(
        binary_test("\x40\x00\x00\x00", 4, bin_float, ++phoenix::ref(vf)));

    BOOST_TEST(binary_test("\x3f\xf0\x00\x00\x00\x00\x00\x00", 8, bin_double,
                           phoenix::val(1.0)));
    double vd(1.0);
    BOOST_TEST(binary_test("\x3f\xf0\x00\x00\x00\x00\x00\x00", 8, bin_double,
                           phoenix::ref(vd)));
    BOOST_TEST(binary_test("\x40\x00\x00\x00\x00\x00\x00\x00", 8, bin_double,
                           ++phoenix::ref(vd)));
