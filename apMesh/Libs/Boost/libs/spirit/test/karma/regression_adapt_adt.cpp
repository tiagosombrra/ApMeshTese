...
class data1 {
 ...
 public:
  data1() : width_(400), height_(400) {}

  data1(int width, int height) : width_(width), height_(height) {}
...
BOOST_FUSION_ADAPT_ADT(data1, (int, int, obj.width(),
                               obj.set_width(val))(int, int, obj.height(),
                                                   obj.set_height(val)))
...
class data2 {
 ...
 public:
  ...
  data2(std::string const& data) : data_(data) {}

  std::string const& data() const { return data_; }
  void set_data(std::string const& data) { data_ = data; }
};

BOOST_FUSION_ADAPT_ADT(data2, (std::string, std::string const&, obj.data(),
                               obj.set_data(val)))
...
class data3 {
 ...
 public:
  data3(double data = 0.0) : data_(data) {}

  double data() const { return data_; }
  void set_data(double data) { data_ = data; }
};

BOOST_FUSION_ADAPT_ADT(data3, (double, double, obj.data(), obj.set_data(val)))
...
#define NO_SETTER                                   \
  (BOOST_ASSERT_MSG(false, "unused setter called"), \
   boost::ignore_unused(obj, val))

BOOST_FUSION_ADAPT_ADT(
    data4, (boost::optional<int>, boost::optional<int> const&, obj.a(),
            NO_SETTER)(boost::optional<double>, boost::optional<double> const&,
                       obj.b(), NO_SETTER)(boost::optional<std::string>,
...
#pragma warning(pop)
...

  {
    ...
    BOOST_TEST(test("Testing: b: 10.0\n",
                    "Testing: " << -("a: " << int_ << "\n")
                                << -("b: " << double_ << "\n")
                                << -("c: " << string << "\n"),
                    d));
