...
struct seqsize_impl {
  ...
  struct result : boost::fusion::result_of::size<Sequence> {};
  ...
  template <typename Sequence>
  typename result<Sequence>::type operator()(Sequence const& seq) const {
...
int main() {
...

  {
    ...
    BOOST_TEST(test_delimited("1 , 2 , 3 ", int_ << ',' << int_ << ',' << int_,
                              v, ' '));
  }

  {
    ...
    BOOST_TEST(
        test_delimited("BEGIN END ", upper[lit("begin") << "end"], char(' ')));
    BOOST_TEST(!test_delimited("BEGIN END ", upper[lit("begin") << "nend"],
                               char(' ')));
  ...
  // action tests
  {
    ...
    BOOST_TEST(test_delimited("a 12 c ",
                              (char_ << lit(12) << char_)[(_1 = 'a', _2 = 'c')],
                              char(' ')));

    char c = 'c';
    BOOST_TEST(test(
        "abc", (char_[_1 = 'a'] << 'b' << char_)[(_1 = 'x', _2 = ref(c))]));
    BOOST_TEST(test_delimited(
        "a b c ", (char_[_1 = 'a'] << 'b' << char_)[_2 = ref(c)], char(' ')));
...

  {
    ...
    BOOST_TEST(
        test_delimited("1 2 3 4 ", repeat(2)[int_] << *int_, v, char(' ')));
  }
