...
enum token_id { ID_WORD = lex::min_token_id + 1, ID_EOL };

typedef lex::lexertl::token<file_iterator, boost::mpl::vector<>,
                            boost::mpl::true_, token_id>
    token_type;
...
struct lexer : lex::lexer<lex::lexertl::actor_lexer<token_type> > {
  lexer() : word("^[a-zA-Z0-9]+$", ID_WORD) {
    ...
    self("INITIAL", "O") =
        word | toked_def("!.*$")[lex::_pass = lex::pass_flags::pass_ignore] |
        toked_def('\n', ID_EOL);

    self("O", "INITIAL") =
        toked_def(".")[lex::_pass = lex::pass_flags::pass_fail];
  }
