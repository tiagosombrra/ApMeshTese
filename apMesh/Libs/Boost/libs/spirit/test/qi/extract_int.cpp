...
struct custom_int {
  ...
  custom_int& operator+=(custom_int x) {
    value_ += x.value_;
    return *this;
  }
  custom_int& operator-=(custom_int x) {
  ...
  friend std::basic_ostream<Char, Traits>& operator<<(
      std::basic_ostream<Char, Traits>& os, custom_int x) {
    return os << x.value_;
  }
...
template <>
struct digits<-9, 9> {
  BOOST_STATIC_CONSTEXPR int r2 = 3, r10 = 1;
};
template <>
struct digits<-10, 10> {
  BOOST_STATIC_CONSTEXPR int r2 = 3, r10 = 1;
...
class numeric_limits<custom_int<Min, Max> > : public numeric_limits<int> {
 public:
  static BOOST_CONSTEXPR custom_int<Min, Max> max() BOOST_NOEXCEPT_OR_NOTHROW {
    return Max;
  }
  static BOOST_CONSTEXPR custom_int<Min, Max> min() BOOST_NOEXCEPT_OR_NOTHROW {
    return Min;
  }
  static BOOST_CONSTEXPR custom_int<Min, Max> lowest()
      BOOST_NOEXCEPT_OR_NOTHROW {
    return min();
  }
  BOOST_STATIC_ASSERT_MSG(numeric_limits<int>::radix == 2,
                          "hardcoded for digits of radix 2");
...
void test_overflow_handling(char const* begin, char const* end, int i) {
  ...
  BOOST_STATIC_ASSERT_MSG(std::numeric_limits<T>::is_bounded,
                          "tests prerequest");
  BOOST_ASSERT_MSG(MaxDigits == -1 || static_cast<int>(std::pow(
                                          float(Base), MaxDigits)) > T::max,
                   "test prerequest");
...
void test_unparsed_digits_are_not_consumed(char const* it, char const* end,
                                           int i) {
...
void test_ignore_overflow_digits(char const* it, char const* end, int i) {
  ...
  if (T::min <= i && i <= T::max) {
    ...
    BOOST_TEST_EQ(it - begin,
                  (qi::detail::digits_traits<T, Base>::value) + has_sign);
