...
int main() {
...

  {
    ...
    vector<> attr;
    BOOST_TEST(
        (test_attr("abc", omit[char_] >> omit['b'] >> omit[char_], attr)));
  }
...

  {
    ...
    BOOST_TEST((test_attr(
        "abcde", char_ >> (omit[char_] >> omit['c'] >> omit[char_]) >> char_,
        attr)));
  ...
  {
    // "hello" has an unused_type. unused attrubutes are not part of the
    // sequence
    vector<char, char> attr;
    BOOST_TEST(
  ...
  {  // test action with omitted attribute
    ...
    BOOST_TEST(test("x123\"a string\"",
                    (char_ >> omit[int_] >> "\"a string\"")[ref(c) = _1]));
    BOOST_TEST(c == 'x');
  }
  ...
  {  // test action with omitted attribute
    ...
    BOOST_TEST(test("x 123 \"a string\"",
                    (omit[char_] >> int_ >> "\"a string\"")[ref(n) = _1],
                    space));
    BOOST_TEST(n == 123);
  }
