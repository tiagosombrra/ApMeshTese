...
BOOST_FUSION_ADAPT_ADT(test1,
                       (unsigned&, unsigned const&, obj.getvar(),
                        obj.setvar(val))(boost::optional<unsigned>&,
                                         boost::optional<unsigned> const&,
...
BOOST_FUSION_ADAPT_ADT(test2,
                       (std::string&, std::string const&, obj.getstring(),
                        obj.setstring(val))(boost::optional<std::string>&,
                                            boost::optional<std::string> const&,
                                            obj.getoptstring(),
                                            obj.setoptstring(val)))
...
int main() {
...

  {
    ...
    BOOST_TEST(test_attr("Hello:OptionalHello",
               ...
               data.optstr.get() == "OptionalHello");
  }

  {
    test2 data;
    BOOST_TEST(test_attr("Hello", +qi::alnum >> -(':' >> +qi::alnum), data) &&
               data.str == "Hello" && !data.optstr);
  }
