...
struct ops_1_parser : boost::spirit::qi::primitive_parser<ops_1_parser<T1> > {
  ops_1_parser(T1 t1) : t1(t1) {}
  ...
  bool parse(Iterator& first, Iterator const& last, Context& /*context*/,
    ...
    for (T1 t = 0; t < t1; t++, count++)
      if (it == last || *it++ != Char('+')) return false;
...
struct ops_2_parser
  ...
  ops_2_parser(T1 t1, T2 t2) : t1(t1), t2(t2) {}
...
struct ops_3_parser
  ...
  ops_3_parser(T1 t1, T2 t2, T3 t3) : t1(t1), t2(t2), t3(t3) {}
...
template <typename T1, typename T2, typename T3>
struct use_terminal<
    qi::domain, terminal_ex<testns::tag::ops, fusion::vector3<T1, T2, T3> > >
    : mpl::true_ {};

template <>
struct use_lazy_terminal<qi::domain, testns::tag::ops, 1> : mpl::true_ {};

template <>
struct use_lazy_terminal<qi::domain, testns::tag::ops, 2> : mpl::true_ {};

template <>
struct use_lazy_terminal<qi::domain, testns::tag::ops, 3> : mpl::true_ {};
...
struct make_primitive<terminal_ex<testns::tag::ops, fusion::vector1<T1> >,
                      Modifiers> {
...
struct make_primitive<terminal_ex<testns::tag::ops, fusion::vector2<T1, T2> >,
                      Modifiers> {
...
struct make_primitive<
    terminal_ex<testns::tag::ops, fusion::vector3<T1, T2, T3> >, Modifiers> {
...
void check_type_1(const T& /*t*/) {
  BOOST_STATIC_ASSERT(
      (boost::is_same<T, typename boost::spirit::terminal<
                             testns::tag::ops>::result<T1>::type>::value));
}
...
void check_type_2(const T& /*t*/) {
  BOOST_STATIC_ASSERT(
      (boost::is_same<T, typename boost::spirit::terminal<
                             testns::tag::ops>::result<T1, T2>::type>::value));
}

template <typename T1, typename T2, typename T3, typename T>
void check_type_3(const T& /*t*/) {
  BOOST_STATIC_ASSERT(
      (boost::is_same<T, typename boost::spirit::terminal<testns::tag::ops>::
                             result<T1, T2, T3>::type>::value));
}
}  // namespace testns
...
#define MP5 ops(val(5) - val(3), 2, val(2))
    check_type_3<
        phx::expression::minus<value<int>::type, value<int>::type>::type, int,
        value<int>::type>(MP5);
