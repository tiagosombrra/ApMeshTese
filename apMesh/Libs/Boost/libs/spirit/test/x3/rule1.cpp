...
int main() {
  ...
  static_assert(
      has_attribute<rule<class r, int>, /*Context=*/unused_type>::value, "");
  static_assert(!has_attribute<rule<class r>, /*Context=*/unused_type>::value,
                "");
  static_assert(has_attribute<decltype(rule<class r, int>{} = int_),
                              /*Context=*/unused_type>::value,
                "");
  static_assert(!has_attribute<decltype(rule<class r>{} = int_),
                               /*Context=*/unused_type>::value,
                "");
  ...
  {  // basic tests w/ skipper but no final post-skip
    ...
    auto a = rule<class a>() = lit('a');

    auto b = rule<class b>() = lit('b');

    auto c = rule<class c>() = lit('c');

    {
      auto start = rule<class start>() = *(a | b) >> c;

      char const *s1 = " a b a a b b a c ... ", *const e1 =
                                                    s1 + std::strlen(s1);
      BOOST_TEST(
          phrase_parse(s1, e1, start, space, skip_flag::dont_post_skip) &&
          s1 == e1 - 5);
    }

    {
      ...
      {
        char const *s1 = " a a a a b a b a b a a a b b b c ", *const e1 =
                                                                  s1 +
                                                                  std::strlen(
                                                                      s1);
