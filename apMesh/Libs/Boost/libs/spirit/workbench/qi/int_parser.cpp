...
struct atoi_test : test::base {
  void benchmark() {
    for (int i = 0; i < 9; ++i) this->val += atoi(first[i]);
  }
};
...
struct spirit_int_test : test::base {
  ...
  void benchmark() {
    for (int i = 0; i < 9; ++i) this->val += parse(first[i], last[i]);
  }
};
}  // namespace
...
int main() {
  ...
  BOOST_SPIRIT_TEST_BENCHMARK(
      ...
      (atoi_test)(strtol_test)(spirit_int_test))
