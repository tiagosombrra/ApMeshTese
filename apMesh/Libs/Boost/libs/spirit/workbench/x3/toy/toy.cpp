...
struct sequence_parser : parser<sequence_parser<Left, Right>> {
  ...
  template <typename Iterator, typename Context>
  bool parse(Iterator& first, Iterator last, Context const& ctx) const {
    return left.parse(first, last, ctx) && right.parse(first, last, ctx);
  }
...
template <typename Left, typename Right>
inline sequence_parser<Left, Right> operator>>(parser<Left> const& left,
                                               parser<Right> const& right) {
...
struct alternative_parser : parser<alternative_parser<Left, Right>> {
  ...
  template <typename Iterator, typename Context>
  bool parse(Iterator& first, Iterator last, Context const& ctx) const {
    if (left.parse(first, last, ctx)) return true;
...
template <typename Left, typename Right>
inline alternative_parser<Left, Right> operator|(parser<Left> const& left,
                                                 parser<Right> const& right) {
...
struct context {
  ...
  T const& get(mpl::identity<ID>) const { return val; }

  template <typename Identity>
  decltype(std::declval<NextContext>().get(Identity())) get(Identity id) const {
...
struct rule : parser<rule<ID>> {
  template <typename Derived>
  rule_definition<ID, Derived> operator=(
      parser<Derived> const& definition) const {
...
int main() {
  ...
  {  // a grammar ( gcc and clang only: see
  ...
  {  // another grammar using namespaces (standard c++, see grammar g definition
     // above in namespace parser.)
    using parser::g;
