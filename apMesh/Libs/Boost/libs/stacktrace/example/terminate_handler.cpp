...
BOOST_NOINLINE void foo(int i) { bar(--i); }
...
void my_signal_handler2(int signum) {
  ...
  *f = reinterpret_cast<void*>(
      1);  // Setting flag that shared memory now constains stacktrace.
...
inline void copy_and_run(const char* exec_name, char param, bool not_null) {
  ...
  command = command.parent_path() /
            (command.stem().string() + param + command.extension().string());
  boost::filesystem::copy_file(
      exec_name, command, boost::filesystem::copy_option::overwrite_if_exists);
  ...
  std::cout << "End Running with param " << param << "; ret code is " << ret
            << std::endl;
...
int run_4(const char* argv[]) {
  ...
  if (*f) {  // Checking if memory constains stacktrace.
    boost::stacktrace::stacktrace st = boost::stacktrace::stacktrace::from_dump(
        f + 1, g_region.get_size() - sizeof(bool));
...
int test_inplace() {
...

  {
    ...
    // work, because:
    // * BOOST_NOINLINE could be ignored by MSVC compiler if link-time
    // optimization is enabled.
    // * BOOST_FORCEINLINE could be ignored by GCC depending on the std::vector
    // default constructor length.
    const std::size_t frames_ss1 =
        boost::stacktrace::safe_dump_to("./backtrace2.dump");
    boost::stacktrace::stacktrace ss2;
    std::ifstream ifs("./backtrace2.dump");
    boost::stacktrace::stacktrace ss1 =
        boost::stacktrace::stacktrace::from_dump(ifs);
    ifs.close();
...

  {
    ...
    boost::stacktrace::stacktrace ss2;
    boost::stacktrace::stacktrace ss1 =
        boost::stacktrace::stacktrace::from_dump(data, sizeof(data));
...

  {
    ...
    const std::size_t size_1_skipped =
        boost::stacktrace::stacktrace::from_dump(data, sizeof(data)).size();
    boost::stacktrace::safe_dump_to(0, data, sizeof(data));
    const std::size_t size_0_skipped =
        boost::stacktrace::stacktrace::from_dump(data, sizeof(data)).size();
...
int main(int argc, const char* argv[]) {
  if (argc < 2) {
    // On Windows the debugger could be active. In that case tests hang and the
    // CI run fails.
#ifndef BOOST_WINDOWS
    copy_and_run(argv[0], '0', true);

    // We are copying files to make sure that stacktrace printing works
    // independently from executable name
    copy_and_run(argv[0], '1', true);
    copy_and_run(argv[0], '2', false);

    // There are some issues with async-safety of shared memory writes on
    // Windows.
