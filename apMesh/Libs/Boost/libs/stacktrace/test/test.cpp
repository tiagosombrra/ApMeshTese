...
using boost::stacktrace::stacktrace;
     ...
     !defined(BOOST_STACKTRACE_USE_BACKTRACE) &&     \
...
void test_deeply_nested_namespaces() {
  ...
  BOOST_TEST(
      ss.str().find("get_backtrace_from_nested_namespaces") !=
          std::string::npos ||
      ss.str().find("1# return_from_nested_namespaces") !=
          std::string::npos);  // GCC with -O1 has strange inlining, so this
                               // line is true while the prev one is false.

  BOOST_TEST(ss.str().find("return_from_nested_namespaces") !=
             std::string::npos);
#endif

  stacktrace ns1 = return_from_nested_namespaces();
  BOOST_TEST(ns1 != return_from_nested_namespaces());  // Different addresses in
...
// Template parameter Depth is to produce different functions on each Depth.
// This simplifies debugging when one of the tests catches error
template <std::size_t Depth>
void test_nested(bool print = true) {
  std::pair<stacktrace, stacktrace> res =
      function_from_library(Depth, function_from_main_translation_unit);
  ...
  BOOST_TEST(ss1.str().find("function_from_main_translation_unit") !=
             std::string::npos);
  BOOST_TEST(ss2.str().find("function_from_main_translation_unit") !=
             std::string::npos);
#endif
}
...
void test_comparisons_base(Bt nst, Bt st) {
  ...
  // This is very dependent on compiler and link flags. No sane way to make it
  // work, because BOOST_NOINLINE could be ignored by MSVC compiler if link-time
...
void test_iterators() {
  ...
  BOOST_TEST(st.size() ==
             static_cast<std::size_t>(std::distance(st.rbegin(), st.rend())));
  BOOST_TEST(st.size() ==
             static_cast<std::size_t>(std::distance(st.crbegin(), st.rend())));
  BOOST_TEST(st.size() ==
             static_cast<std::size_t>(std::distance(st.crbegin(), st.crend())));
  BOOST_TEST(st.size() ==
             static_cast<std::size_t>(std::distance(st.rbegin(), st.crend())));
...
void test_frame() {
  ...
  const std::size_t min_size =
      (nst.size() < st.size() ? nst.size() : st.size());
  ...
  for (std::size_t i = 0; i < min_size; ++i) {
    ...
    if (i > 1 &&
      ...
      // MSVC can not get function name withhout debug symbols even if it is
      // exported
      ...
      if (st[i].source_line()) {
        BOOST_TEST(st[i].source_file() != fv.source_file() ||
                   st[i].source_line() != fv.source_line());
      }
      BOOST_TEST(st[i]);
    }
...
// Template parameter bool BySkip is to produce different functions on each
// BySkip. This simplifies debugging when one of the tests catches error
