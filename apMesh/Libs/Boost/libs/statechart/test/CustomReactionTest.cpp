...
struct CustomReactionTest : sc::state_machine<CustomReactionTest, A> {
 public:
  ...
  void ClearVisited() { visitedStates_.clear(); }
  ...
  void AssertVisitedAll(const std::string& stateNames) const {
    for (std::string::const_iterator expectedName = stateNames.begin();
      ...
      BOOST_REQUIRE(visitedStates_.find(std::string(1, *expectedName)) !=
                    visitedStates_.end());
    }
  }
  ...
  void AssertVisitedOne(const std::string& stateNames) const {
    ...
    for (std::string::const_iterator actualName = stateNames.begin();
      ...
      found = found + (visitedStates_.find(std::string(1, *actualName)) !=
                       visitedStates_.end())
                  ? 1
                  : 0;
    }
...
struct A : sc::simple_state<A, CustomReactionTest, B> {
  typedef mpl::list<
      sc::custom_reaction<EvDiscardNever>, sc::custom_reaction<EvDiscardInB>,
      sc::custom_reaction<EvDiscardInD>, sc::custom_reaction<EvDefer>,
      sc::custom_reaction<EvTerminate>,
      sc::custom_reaction<EvTransitWithAction>, sc::custom_reaction<EvTransit> >
      reactions;
  ...
  // reactions call.
  sc::result react(const EvDefer&) { return defer_event(); }

  sc::result react(const EvTerminate&) { return terminate(); }

  sc::result react(const EvTransit&) { return transit<A>(); }
...
struct B : sc::simple_state<B, A, C> {
  typedef mpl::list<sc::custom_reaction<EvDiscardNever>,
...
struct D : sc::simple_state<D, B, mpl::list<E, F> > {
  typedef mpl::list<
      sc::transition<EvToC, C>, sc::custom_reaction<EvDiscardNever>,
      sc::custom_reaction<EvDiscardInB>, sc::custom_reaction<EvDiscardInD> >
      reactions;
...
struct C : sc::simple_state<C, B> {
  typedef mpl::list<
      sc::transition<EvToD, D>, sc::custom_reaction<EvDiscardNever>,
      sc::custom_reaction<EvDiscardInB>, sc::custom_reaction<EvDiscardInD> >
      reactions;
...
struct X1;
struct CustomReactionEventBaseTest
    : sc::state_machine<CustomReactionEventBaseTest, X1> {
 public:
  CustomReactionEventBaseTest() : reactionCount_(0) {}

  void IncrementReactionCount() { ++reactionCount_; }

  unsigned int GetReactionCount() const { return reactionCount_; }
