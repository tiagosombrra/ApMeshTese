...
struct http_category_impl : public error_category {
  ...
  char const* name() const BOOST_NOEXCEPT { return "http"; }
  ...
  bool failed(int ev) const BOOST_NOEXCEPT { return !(ev >= 200 && ev < 300); }
};
...
#define TEST_NOT_FAILED(ec)      \
  BOOST_TEST(!ec.failed());      \
  BOOST_TEST(ec ? false : true); \
  BOOST_TEST(!ec);
#define TEST_FAILED(ec)    \
  BOOST_TEST(ec.failed()); \
  BOOST_TEST(ec);          \
  BOOST_TEST(!!ec);
