...
std::pair<double, double> estimate_polynomial_roots(
    random_generator_t& gen, std::function<double(double)> polynomial) {
  using namespace std;
  ...
  return std::make_pair((minus_b - sqrt(minus_b * minus_b - 4 * c)) / 2,
...
BOOST_AUTO_TEST_CASE(quadratic_estimation) {
  ...
  for (int i = 0; i < 50; i++) {
    ...
    std::pair<double, double> estimated =
        estimate_polynomial_roots(gen, [root1, root2](double x) -> double {
          return (x - root1) * (x - root2);
        });

    BOOST_TEST(estimated.first == double(root1),
               10. % boost::test_tools::tolerance());
    BOOST_TEST(estimated.second == double(root2),
               10. % boost::test_tools::tolerance());
  }
}
//]
