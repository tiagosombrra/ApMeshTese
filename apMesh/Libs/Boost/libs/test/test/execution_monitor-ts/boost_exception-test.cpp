...
void exception_raised() {
  BOOST_THROW_EXCEPTION(myexception()
                        << error_code(123) << error_string("error%%string"));
}

struct output_test_stream2 : public output_test_stream {
  std::string get_stream_string_representation() const {
    (void)const_cast<output_test_stream2*>(this)
        ->output_test_stream::check_length(0, false);  // to sync only!!
...
BOOST_AUTO_TEST_CASE(test_logs) {
  ...
  // the message is "Dynamic exception type:
  // boost::exception_detail::clone_impl<myexception>" on Unix and "Dynamic
  // exception type: boost::exception_detail::clone_impl<struct myexception>" on
  // Windows. Also contains "[tag_error_code*] = 123" on Unix and "[struct
  // tag_error_code * __ptr64] = 123" On Windows Also contains
  // "[tag_error_string*] = error%%string" on Unix and "[struct tag_error_string
  // * __ptr64] = error%%string" On Windows
