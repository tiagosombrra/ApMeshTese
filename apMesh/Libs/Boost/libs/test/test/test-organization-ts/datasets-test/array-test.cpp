...
BOOST_AUTO_TEST_CASE(test_array) {
  ...
  int c = 0;
  int* ptr4 = arr4;
  data::for_each_sample(data::make(arr4),
                        [&c, ptr4](int i) { BOOST_TEST(i == ptr4[c++]); });
...
BOOST_AUTO_TEST_CASE(test_array_make_type) {
  ...
  typedef int(&arr_t)[3];
  BOOST_STATIC_ASSERT(
      (boost::is_array<boost::remove_reference<arr_t>::type>::value));
  ...
  double const arr2[] = {7.4, 3.2};
  typedef data::result_of::make<double const(&)[2]>::type
      dataset_array_double_type;
