...
template <typename T>
struct forwarded_to_collection<data::monomorphic::collection<T> >
    : std::true_type {};

BOOST_AUTO_TEST_CASE(test_forwarded_to_collection) {
  {
    std::vector<int> samples1;
    BOOST_TEST(boost::unit_test::is_container_forward_iterable<
                   decltype(samples1)>::value,
               "forward iterable");
  ...
  }
  {
    ...
    BOOST_TEST(!boost::unit_test::is_container_forward_iterable<
               ...
               "forward iterable");
    BOOST_TEST(
        !(forwarded_to_collection<decltype(data::make(samples1))>::value),
        "not properly forwarded to a collection");
  }
}
...
BOOST_AUTO_TEST_CASE(test_collection_sizes) {
  ...
  data::for_each_sample(data::make(std::list<double>(2)),
                        check_arg_type<double>());
...
BOOST_AUTO_TEST_CASE(test_collection) {
  ...
  data::for_each_sample(data::make(samples2),
                        [&it](char const* str) { BOOST_TEST(str == *it++); });
}
...
BOOST_AUTO_TEST_CASE(test_collection_copies) {
  ...
  data::for_each_sample(data::make(std::vector<copy_count>(2)),
                        check_arg_type<copy_count>());
  BOOST_TEST(copy_count::value() ==
             (exp_copy_count + std_vector_constructor_count) * 2);
  ...
  BOOST_TEST(copy_count::value() ==
             (exp_copy_count + std_vector_constructor_count) * 3);
  ...
  BOOST_TEST(copy_count::value() ==
             (exp_copy_count + std_list_constructor_count + 1) *
                 3);  // no rvalue constructor for the dataset
}
