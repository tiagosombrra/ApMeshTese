...
BOOST_AUTO_TEST_CASE(test_initializer_list) {
  ...
  int c = 0;
  int exp[] = {7, 11, 13, 17};
  data::for_each_sample(data::make({7, 11, 13, 17}),
  ...
  copy_count::value() = 0;
  data::for_each_sample(data::make({copy_count(), copy_count()}),
                        check_arg_type<copy_count>());
  BOOST_TEST(
      copy_count::value() ==
      2);  // this is copied in this case, because of the initializer_list
}
