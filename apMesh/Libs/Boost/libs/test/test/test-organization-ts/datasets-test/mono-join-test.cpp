...
BOOST_AUTO_TEST_CASE(test_mono_join) {
  ...
  BOOST_TEST((data::make(std::vector<int>(2)) + data::make(std::list<int>(3)))
                 .size() == 5);
  ...
  BOOST_TEST((data::make(std::vector<int>(2)) + data::make(1) +
              data::make(arr2) + data::make(17))
                 .size() == 6);
  ...
  auto samples1 = data::make(std::vector<int>(arr1, arr1 + 2)) + data::make(5) +
                  data::make(arr2) + data::make(37);
  ...
  data::for_each_sample(samples1,
                        [&c, exp](int i) { BOOST_TEST(i == exp[c++]); });

  auto samples2 = data::make(std::vector<int>(arr1, arr1 + 2)) +
                  (data::make(5) + (data::make(arr2) + data::make(37)));
  ...
  auto samples3 =
      (data::make(std::vector<int>(arr1, arr1 + 2)) + data::make(5)) +
      (data::make(arr2) + data::make(37));
  ...
  copy_count::value() = 0;
  data::for_each_sample((ds1 + ds1) + (ds2 + ds2),
                        check_arg_type<copy_count>());
  BOOST_TEST(copy_count::value() == 0);
...
// ticket 13380
// note that the test relies on the fact that the references to temporaries are
// properly escalated as errors. Currently only on clang.
BOOST_DATA_TEST_CASE(sampleTest,
                     (data::make(1) ^ data::make(2)) +
                         (data::make(3) ^ data::make(4)),
                     var1, var2) {
