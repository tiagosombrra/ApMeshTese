...
BOOST_DATA_TEST_CASE(test_case_interface_03, data::make({1, 2, 3}) + samples3,
                     val) {
  if (index3 < 3)
...
BOOST_DATA_TEST_CASE(test_case_interface_04, samples2 ^ data::make({7, 9}), str,
                     intval) {
...
BOOST_DATA_TEST_CASE(test_case_interface_05, samples1* samples2, sample0,
                     sample1) {
...
BOOST_DATA_TEST_CASE(test_case_interface_06, samples1* samples2* samples3,
                     intval, str, val2) {
...
BOOST_DATA_TEST_CASE(test_case_interface_07,
                     samples1* samples2* samples3* samples4, intval, str, val2,
                     floatval) {
  BOOST_TEST_CONTEXT("index7 " << index7) {
    BOOST_TEST(intval ==
               samples1[index7 / (sizeoftable(samples4) *
                                  sizeoftable(samples3) * samples2.size())]);
    BOOST_TEST(
        str ==
        samples2[(index7 / (sizeoftable(samples4) * sizeoftable(samples3))) %
                 samples2.size()]);
    BOOST_TEST(
        val2 ==
        samples3[(index7 / sizeoftable(samples4)) % sizeoftable(samples3)]);
...
struct SharedFixture {
  SharedFixture() : m_expected(index8++) {}

  int m_expected;
};

BOOST_DATA_TEST_CASE_F(SharedFixture, test_case_interface_08,
                       data::make({1, 2, 3})) {
...
BOOST_DATA_TEST_CASE(test_case_interface_correct_file_line_declaration,
                     samples2) {
  boost::unit_test::test_case const& current_test_case =
...
BOOST_DATA_TEST_CASE(test_arity_above_9,
                     data::make({1, 2, 3, 5}) ^ data::make({1, 2, 3, 5}) ^
                         data::make({1, 2, 3, 5}) ^ data::make({1, 2, 3, 5}) ^
                         data::make({1, 2, 3, 5}) ^ data::make({1, 2, 3, 5}) ^
                         data::make({1, 2, 3, 5}) ^ data::make({1, 2, 3, 5}) ^
                         data::make({1, 2, 3, 5}) ^ data::make({1, 2, 3, 5}),
...
struct SharedFixtureSetupTeardown {
  SharedFixtureSetupTeardown() : m_expected(1 + index_fixture_setup_teardown) {}

  void setup() { m_expected *= m_expected; }

  void teardown() { index_fixture_setup_teardown++; }

  int m_expected;
};

BOOST_DATA_TEST_CASE_F(SharedFixtureSetupTeardown,
                       test_case_interface_setup_teardown,
                       data::make({0, 1, 2, 3})) {
...
BOOST_DATA_TEST_CASE(BoostDataTest,
                     boost::unit_test::data::make({false, true, true, true,
                                                   false, false}) ^
