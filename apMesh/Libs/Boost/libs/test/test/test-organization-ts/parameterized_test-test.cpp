...
//  Note: this file should be compatible with C++03 compilers (features in
//  boost.test v2)
// ***************************************************************************
...
void test0(int i) {
  BOOST_TEST((i % 2 == 0));  // amounts to BOOST_CHECK, for backward
                             // compatibility wrt. boost.test v2
}
...
static void setup_tree(ut::test_suite* master_tu) {
...
struct logger_guard {
  logger_guard(std::ostream& s_out) { ut::unit_test_log.set_stream(s_out); }
  ~logger_guard() { ut::unit_test_log.set_stream(std::cout); }
};

BOOST_AUTO_TEST_CASE(test_case1) {
  // if an exception is thrown in the test, this object is destructed when we
  // reach the logger for logging the exception. This happens for instance if
  // the test->add throws:
  // - test case aborted, null_output destructed but still refered from the
  // logger
  ...
  int test_data[] = {2, 2, 2};
  test->add(
      BOOST_PARAM_TEST_CASE(&test0, (int*)test_data,
                            (int*)test_data + sizeof(test_data) / sizeof(int)));
...
BOOST_AUTO_TEST_CASE(test_case5) {
  ...
  int test_data[] = {6, 6, 6};
  test->add(
      BOOST_PARAM_TEST_CASE(&test1, (int*)test_data,
                            (int*)test_data + sizeof(test_data) / sizeof(int)));
