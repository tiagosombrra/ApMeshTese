...
void consumer(the_ostream & /*mos*/, boost::sync_queue<int> &sbq) {
  using namespace boost;
  try {
    for (int i = 0;; ++i) {
      int r;
...
int main() {
...

  {
    ...
    scoped_thread<> t11(
        boost::thread(producer, boost::ref(mcerr), boost::ref(sbq)));
    scoped_thread<> t12(
        boost::thread(producer, boost::ref(mcerr), boost::ref(sbq)));
    scoped_thread<> t2(
        boost::thread(consumer, boost::ref(mcout), boost::ref(sbq)));
