    ...
    while (!current_thread_data->tss_data.empty() ||
      ...
      while (!current_thread_data->tss_data.empty()) {
        ...
        if (current->second.func && (current->second.value != 0)) {
          (*current->second.caller)(current->second.func,
...
bool thread::start_thread_noexcept(const attributes& attr) {
  ...
  uintptr_t const new_thread = _beginthreadex(
      0, static_cast<unsigned int>(attr.get_stack_size()),
...
bool thread::join_noexcept() {
  detail::thread_data_ptr local_thread_info = (get_thread_info)();
  if (local_thread_info) {
    this_thread::interruptible_wait(
        this->native_handle(), detail::internal_platform_timepoint::getMax());
    release_handle();
    return true;
  } else {
    return false;
...
bool thread::interruption_requested() const BOOST_NOEXCEPT {
  ...
  return local_thread_info.get() &&
         (winapi::WaitForSingleObjectEx(local_thread_info->interruption_handle,
                                        0, 0) == 0);
}
...
unsigned thread::physical_concurrency() BOOST_NOEXCEPT {
  ...
  if (GetLogicalProcessorInformation(&buffer.front(), &size) == FALSE) return 0;
...
bool interruptible_wait(detail::win32::handle handle_to_wait_for,
  ...
  if (timeout != detail::internal_platform_timepoint::getMax()) {
    boost::intmax_t const time_left_msec =
        (timeout - detail::internal_platform_clock::now()).getMs();
    ...
    if (timer_handle != 0) {
      ...
      if (time_left_msec > 0) {
        due_time.QuadPart =
            -(time_left_msec * 10000);  // negative indicates relative time
      }
      bool const set_time_succeeded =
          detail_::SetWaitableTimerEx()(timer_handle, &due_time, 0, 0, 0, NULL,
                                        tolerable) != 0;
  ...
  if (!using_timer &&
      timeout != detail::internal_platform_timepoint::getMax()) {
  ...
  do {
    if (handle_count) {
      unsigned long const notified_index = winapi::WaitForMultipleObjectsEx(
          handle_count, handles, false, static_cast<DWORD>(time_left_msec), 0);
...
void add_new_tss_node(void const* key,
  ...
  current_thread_data->tss_data.insert(
      std::make_pair(key, tss_data_node(caller, func, tss_data)));
}
...
void set_tss_data(void const* key,
                  detail::tss_data_node::cleanup_caller_t caller,
                  detail::tss_data_node::cleanup_func_t func, void* tss_data,
