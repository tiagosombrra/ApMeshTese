...
extern "C" {
PIMAGE_TLS_CALLBACK __crt_xl_tls_callback__
...
extern "C" const IMAGE_TLS_DIRECTORY32 _tls_used
    __attribute__((section(".rdata$T"))) = {(DWORD)&__tls_start__,
                                            (DWORD)&__tls_end__,
                                            (DWORD)&__tls_index__,
...
extern BOOL(WINAPI* const _pDefaultRawDllMainOrig)(HINSTANCE, DWORD,
                                                   LPVOID) = NULL;
#if defined(_M_IX86)
...
// Definitions required by implementation
    ...
    ((_MSC_VER > 1900) && \
     (_MSC_VER <          \
      1910))  // 1300 == VC++ 7.0, 1900 == VC++ 14.0, 1910 == VC++ 2017
...
extern const __declspec(allocate(".CRT$XLC")) _TLSCB p_tls_callback =
    on_tls_callback;
extern const __declspec(allocate(".CRT$XIU")) _PIFV_ p_tls_prepare =
    on_tls_prepare;
extern const __declspec(allocate(".CRT$XCU")) _PVFV_ p_process_init =
    on_process_init;
extern const __declspec(allocate(".CRT$XTU")) _PVFV_ p_process_term =
    on_process_term;
#else
...
extern "C" {
extern BOOL(WINAPI* const _pRawDllMain)(HINSTANCE, DWORD,
                                        LPVOID) = &dll_callback;
}
namespace boost {
void tss_cleanup_implemented() {
