...
class non_copyable {
 ...
 public:
  ...
  non_copyable& operator=(BOOST_RV_REF(non_copyable) x) {
    val = x.val;
    return *this;
  }
...
int main() {
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  {
    ...
    boost::sync_deque<non_copyable> q;
    BOOST_TEST(boost::queue_op_status::success ==
               q.try_push_back(non_copyable(1)));
    BOOST_TEST(!q.empty());
    BOOST_TEST(!q.full());
    BOOST_TEST_EQ(q.size(), 1u);
    BOOST_TEST(!q.closed());
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  {
    ...
    boost::sync_deque<non_copyable> q;
    BOOST_TEST(boost::queue_op_status::success ==
               q.nonblocking_push_back(non_copyable(1)));
    BOOST_TEST(!q.empty());
    BOOST_TEST(!q.full());
    BOOST_TEST_EQ(q.size(), 1u);
    BOOST_TEST(!q.closed());
...
#endif
  {
    ...
    non_copyable nc(1);
    BOOST_TEST(boost::queue_op_status::success ==
               q.nonblocking_push_back(boost::move(nc)));
    BOOST_TEST(!q.empty());
    BOOST_TEST(!q.full());
    BOOST_TEST_EQ(q.size(), 1u);
    BOOST_TEST(!q.closed());
