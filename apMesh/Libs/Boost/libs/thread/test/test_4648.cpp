...
int boostThreadLocksTest::firstFunction(
    boostThreadLocksTest *pBoostThreadLocksTest) {
  std::cout << "Entering " << boost::this_thread::get_id() << " "
            << "firstFunction" << std::endl;
  boost::upgrade_lock<boost::shared_mutex> myLock(
      pBoostThreadLocksTest->myMutex);
...
int boostThreadLocksTest::secondFunction(
    boostThreadLocksTest *, boost::upgrade_lock<boost::shared_mutex> &upgr) {
...
int main() {
  ...
  myThreadGroup.create_thread(
      boost::bind(boostThreadLocksTest::firstFunction, &myObject));
  myThreadGroup.create_thread(
      boost::bind(boostThreadLocksTest::firstFunction, &myObject));
  myThreadGroup.create_thread(
      boost::bind(boostThreadLocksTest::firstFunction, &myObject));
