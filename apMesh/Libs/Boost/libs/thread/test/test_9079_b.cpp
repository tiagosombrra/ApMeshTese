...
int main() {
  ...
  while (i--) {
    ...
    const TimePoint now = real_time_now();
    const TimePoint next_minimum_spawn_time =
        now + MIN_TIME_TASKS_SPAWN_FREQUENCY;
    const TimePoint next_spawn_time =
        next_task_spawn_time > TimePoint() &&
    ...
    if (wait_time > wait_time.zero()) {
      // BOOST_LOG_TRIVIAL(trace) << "WAIT TIME: " << wait_time; // UNCOMMENT
      // THIS: MAKES IT WORKS. WAT??????
