...
#define BOOST_TEST_MODULE \
  Boost.Threads : condition_variable notify_all test suite
...
void do_test_condition_notify_all_wakes_from_timed_wait_with_predicate() {
  ...
  try {
    for (unsigned i = 0; i < number_of_test_threads; ++i) {
      group.create_thread(
          bind(&wait_for_flag::timed_wait_with_predicate, data));
    }
...
BOOST_AUTO_TEST_CASE(test_condition_notify_all) {
  ...
  timed_test(&do_test_condition_notify_all_wakes_from_wait_with_predicate,
             timeout_seconds);
  timed_test(&do_test_condition_notify_all_wakes_from_timed_wait,
             timeout_seconds);
  timed_test(&do_test_condition_notify_all_wakes_from_timed_wait_with_predicate,
             timeout_seconds);
  timed_test(
      &do_test_condition_notify_all_wakes_from_relative_timed_wait_with_predicate,
      timeout_seconds);
  timed_test(&do_test_notify_all_following_notify_one_wakes_all_threads,
             timeout_seconds);
}
