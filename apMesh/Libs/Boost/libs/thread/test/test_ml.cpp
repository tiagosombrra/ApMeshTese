...
template <typename F>
boost::unique_future<typename boost::result_of<F()>::type> async(F&& f) {
  std::cout << __FILE__ << ":" << __LINE__ << std::endl;
  typedef typename boost::result_of<F()>::type RetType;
  std::cout << __FILE__ << ":" << __LINE__ << std::endl;
  async_func_pt<RetType>* p = new async_func_pt<RetType>(
      boost::packaged_task<RetType>(boost::forward<F>(f)));
  std::cout << __FILE__ << ":" << __LINE__ << std::endl;
...
boost::unique_future<typename boost::result_of<F(A1)>::type> async(F&& f,
                                                                   A1&& a1) {
  std::cout << __FILE__ << ":" << __LINE__ << std::endl;
 ...
 *
 ...
 -o
 "../../../bin.v2/libs/thread/test/test_ml.test/clang-darwin-3.0x/debug/threading-multi/test_ml"
 ...
 "../../../bin.v2/libs/system/build/clang-darwin-3.0x/debug/threading-multi/libboost_system.dylib"
 -g
*/
...
async (F&& f)
 ...
 typedef typename boost::result_of< F() >::type RetType;
 async_func_pt<RetType>* p= new  async_func_pt<RetType>
(boost::packaged_task<RetType>(f)); boost::unique_future<RetType> future_result=
p->get_future(); async_core (p); return std::move(future_result);
 }
...
(&calculate_the_answer_to_life_the_universe_and_everything); int i= fi.get();
