...
struct test_lock_times_out_if_other_thread_has_lock {
  ...
  test_lock_times_out_if_other_thread_has_lock() : done(false), locked(false) {}
...
void do_test_mutex() { test_lock<boost::mutex>()(); }

BOOST_AUTO_TEST_CASE(test_mutex) { timed_test(&do_test_mutex, 3); }
...
BOOST_AUTO_TEST_CASE(test_try_mutex) { timed_test(&do_test_try_mutex, 3); }
...
BOOST_AUTO_TEST_CASE(test_timed_mutex) { timed_test(&do_test_timed_mutex, 3); }
