...
/*`
   ...
   usable at compile time on a C++14 compatible compilers.

    In this example we'll create and use a constexpr function that checks
   namespace of the provided type.
*/
...
struct serializer {
  ...
  void serialize(const T& value) {
    static_assert(in_namespace<T>("my_project::types") ||
                      in_namespace<T>("my_project::types_ext"),
                  "Only types from namespaces `my_project::types` and "
                  "`my_project::types_ext` are allowed to be serialized using "
                  "`my_project::serializer`");
