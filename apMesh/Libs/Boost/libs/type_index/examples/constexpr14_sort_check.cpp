...
/*`
    The following example shows that `boost::typeindex::ctti_type_index` is
   usable at compile time on a C++14 compatible compilers to check order of
   template parameters.

    Consider the situation when we have a function that accepts std::tuple,
   boost::variant or some other class that uses variadic templates:
*/
...
/*`
   ...
   the size of the resulting program. Each instantionation of such function with
   different templates order consumes space in the resulting program:

        // Types are same, but different order leads to new instantionation of
   do_something function. types<bool, double, int> types<bool, int, double>
...
// Implementing type trait that returns true if the types are sorted
// lexographicaly
template <class... T>
...
//] [/type_index_constexpr14_sort_check_example]
       ...
       // !defined(BOOST_NO_CXX11_CONSTEXPR) &&
       // !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES) && (!defined(_MSC_VER) ||
       // (_MSC_VER > 1916))
