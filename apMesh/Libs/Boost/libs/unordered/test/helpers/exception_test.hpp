...
#define UNORDERED_EXCEPTION_TEST_CASE(name, test_func, type)                 \
  UNORDERED_AUTO_TEST(name) {                                                \
    ...
    ::test::lightweight::exception_safety(fixture,                           \
                                          BOOST_STRINGIZE(test_func<type>)); \
  }
...
template <class T, class P1, class P2, class T2>
inline void call_ignore_extra_parameters(void (T::*fn)(P1&) const,
                                         T2 const& obj, P1& p1, P2&) {
  (obj.*fn)(p1);
}

template <class T, class P1, class P2, class T2>
inline void call_ignore_extra_parameters(void (T::*fn)(P1&, P2&) const,
                                         T2 const& obj, P1& p1, P2& p2) {
  (obj.*fn)(p1, p2);
}
  ...
  do {
    ...
    try {
      ...
      if (error_count != boost::detail::test_errors()) {
        BOOST_LIGHTWEIGHT_TEST_OSTREAM << "Iteration: " << iteration
                                       << " Error found for epoint: " << e.name
                                       << std::endl;
      }
    } catch (...) {
