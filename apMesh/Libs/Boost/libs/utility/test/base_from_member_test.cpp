...
class object_registrar : private boost::noncopyable {
 public:
...
class bad_class : public base_class {
 public:
  bad_class();
  ~bad_class();
...
class good_class_1 : private boost::base_from_member<base_or_member>,
                     public base_class {
...
// of the "same" type.
class good_class_2 : private boost::base_from_member<base_or_member, 0>,
...
std::ostream &operator<<(std::ostream &os, object_id const &oi) {
  ...
  return os << '[' << (oi.second ? oi.second->name() : "NOTHING") << " at "
            << oi.first << ']';
}

// Get an object ID given an object
template <typename T>
inline object_id identify(T &obj) {
  return std::make_pair(static_cast<void *>(&obj), &(typeid(obj)));
}

// Compare two object IDs
bool object_id_compare::operator()(object_id const &a,
                                   object_id const &b) const {
...
void object_registrar::register_object_imp(object_id obj) {
  if (db_.count(obj) <= 0) {
    db_.insert(obj);
...
void object_registrar::register_use_imp(object_id owner, object_id owned) {
...
#if CONTROL_EXTRA_PRINTING
    std::cout << "Attempted to own a non-existant " << owned << " by " << owner
              << '.' << std::endl;
#endif
  }
}
...
void object_registrar::unregister_use_imp(object_id owner, object_id owned) {
  if (db_.count(owned) > 0) {
...
#if CONTROL_EXTRA_PRINTING
    std::cout << "Attempted to disown a non-existant " << owned << " by "
              << owner << '.' << std::endl;
#endif
  }
}
...
void object_registrar::unregister_object_imp(object_id obj) {
...
#else
#define PRIVATE_REGISTER_BIRTH(o) obj_reg.register_object_imp(identify((o)))
#define PRIVATE_REGISTER_DEATH(o) obj_reg.unregister_object_imp(identify((o)))
#define PRIVATE_REGISTER_USE(o, w) \
  obj_reg.register_use_imp(identify((o)), identify((w)))
#define PRIVATE_UNREGISTER_USE(o, w) \
  obj_reg.unregister_use_imp(identify((o)), identify((w)))
#endif

// Create a base_or_member, with arguments to simulate member initializations
base_or_member::base_or_member(int x,    // = 1
                               double y  // = -0.25
) {
...
inline base_or_member::~base_or_member() { PRIVATE_REGISTER_DEATH(*this); }

// Create a base_class, registering any objects used
base_class::base_class(base_or_member &x,
                       base_or_member *y,  // = 0
                       base_or_member *z   // = 0
                       )
    : x_(&x), y_(y), z_(z) {
...
bad_class::bad_class()
    : x_(-7, 16.75),
      base_class(x_)  // this order doesn't matter
{
  PRIVATE_REGISTER_BIRTH(*this);

#if CONTROL_EXTRA_PRINTING
  std::cout << "\tMy factor is at " << &x_ << " and my base is at "
...
#if CONTROL_EXTRA_PRINTING
  std::cout << "\tMy factor was at " << &x_ << " and my base was at "
            << static_cast<base_class *>(this) << '.' << std::endl;
#endif
}

// Create a good_class_1, noting the proper construction order
good_class_1::good_class_1() : pbase_type(8), base_type(member) {
  PRIVATE_REGISTER_BIRTH(*this);

#if CONTROL_EXTRA_PRINTING
  std::cout << "\tMy factor is at " << &member << " and my base is at "
            << static_cast<base_class *>(this) << '.' << std::endl;
#endif
}
...
#if CONTROL_EXTRA_PRINTING
  std::cout << "\tMy factor was at " << &member << " and my base was at "
            << static_cast<base_class *>(this) << '.' << std::endl;
#endif
}
...
good_class_2::good_class_2()
  ...
  std::cout << "\tMy factors are at " << &this->pbase_type0::member << ", "
            << &this->pbase_type1::member << ", " << &this->pbase_type2::member
