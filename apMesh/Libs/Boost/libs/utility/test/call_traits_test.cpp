...
// 18 Mar 2002:
//    Changed some names to prevent conflicts with some new type_traits
//    additions.
// 03 Oct 2000:
...
#ifdef BOOST_MSVC
#pragma warning(disable : 4181)  // : warning C4181: qualifier applied to
...
struct contained<T[N]> {
  ...
  contained(param_type p) { std::copy(p, p + N, v_); }
  // return byval:
  result_type value() const { return v_; }
  // return by_ref:
  reference get() { return v_; }
...
template <class T>
contained<typename boost::call_traits<T>::value_type> test_wrap_type(
    const T& t) {
...
make_pair(const T1& t1, const T2& t2) {
  return std::pair<typename boost::call_traits<T1>::value_type,
...
#ifndef __ICL
  // cout << "typeof contained<" << typeid(T).name() << ">::v_ is:           "
  // << typeid(&contained<T>::v_).name() << endl;
  cout << "typeof contained<" << typeid(T).name()
       << ">::value() is:      " << typeid(&contained<T>::value).name() << endl;
  cout << "typeof contained<" << typeid(T).name()
       << ">::get() is:        " << typeid(&contained<T>::get).name() << endl;
  cout << "typeof contained<" << typeid(T).name()
       << ">::const_get() is:  " << typeid(&contained<T>::const_get).name()
       << endl;
  cout << "typeof contained<" << typeid(T).name()
       << ">::call() is:       " << typeid(&contained<T>::call).name() << endl;
  cout << endl;
#endif
}
...
struct call_traits_checker<T[N]> {
  ...
  void operator()(param_type t) {
    ...
    unsigned int i = 0;
    for (i = 0; i < N; ++i) BOOST_CHECK(t[i] == c.value()[i]);
    for (i = 0; i < N; ++i) BOOST_CHECK(t[i] == c.get()[i]);
    for (i = 0; i < N; ++i) BOOST_CHECK(t[i] == c.const_get()[i]);

    cout << "typeof contained<" << typeid(T[N]).name()
         << ">::v_ is:         " << typeid(&contained<T[N]>::v_).name() << endl;
    cout << "typeof contained<" << typeid(T[N]).name()
         << ">::value is:      " << typeid(&contained<T[N]>::value).name()
         << endl;
    cout << "typeof contained<" << typeid(T[N]).name()
         << ">::get is:        " << typeid(&contained<T[N]>::get).name()
         << endl;
    cout << "typeof contained<" << typeid(T[N]).name()
         << ">::const_get is:  " << typeid(&contained<T[N]>::const_get).name()
         << endl;
    cout << "typeof contained<" << typeid(T[N]).name()
         << ">::call is:       " << typeid(&contained<T[N]>::call).name()
         << endl;
    cout << endl;
  }
};
...
void check_make_pair(T c, U u, V v) {
  cout << "checking std::pair<" << typeid(c.first).name() << ", "
       << typeid(c.second).name() << ">..." << endl;
...
int main() {
  ...
  c5(i);
    ...
    !defined(__MWERKS__) && !defined(__SUNPRO_CC)
  ...
  BOOST_CHECK_TYPE(comparible_UDT,
                   boost::call_traits<comparible_UDT>::value_type);
  BOOST_CHECK_TYPE(comparible_UDT&,
                   boost::call_traits<comparible_UDT>::reference);
  BOOST_CHECK_TYPE(const comparible_UDT&,
                   boost::call_traits<comparible_UDT>::const_reference);
  BOOST_CHECK_TYPE(const comparible_UDT&,
                   boost::call_traits<comparible_UDT>::param_type);
  ...
  BOOST_CHECK_TYPE(const int(&)[3],
                   boost::call_traits<int[3]>::const_reference);
  ...
  BOOST_CHECK_TYPE(const int(&)[3],
                   boost::call_traits<const int[3]>::reference);
  BOOST_CHECK_TYPE(const int(&)[3],
                   boost::call_traits<const int[3]>::const_reference);
  BOOST_CHECK_TYPE(const int* const,
                   boost::call_traits<const int[3]>::param_type);
  ...
  BOOST_CHECK_TYPE(const test_abc1&,
                   boost::call_traits<test_abc1>::const_reference);
...
#else
  std::cout << "You're compiler does not support partial template "
               "specialiation, skipping 20 tests (20 errors)"
            << std::endl;
#endif
  // test with an incomplete type:
  BOOST_CHECK_TYPE(incomplete_type,
                   boost::call_traits<incomplete_type>::value_type);
  BOOST_CHECK_TYPE(incomplete_type&,
                   boost::call_traits<incomplete_type>::reference);
  BOOST_CHECK_TYPE(const incomplete_type&,
                   boost::call_traits<incomplete_type>::const_reference);
  BOOST_CHECK_TYPE(const incomplete_type&,
                   boost::call_traits<incomplete_type>::param_type);
  // test enum:
  ...
  BOOST_CHECK_TYPE(const enum_UDT&,
                   boost::call_traits<enum_UDT>::const_reference);
  BOOST_CHECK_TYPE(const enum_UDT, boost::call_traits<enum_UDT>::param_type);
  return 0;
}

//
// define call_traits tests to check that the assertions in the docs do actually
// work this is an compile-time only set of tests:
//
...
void call_traits_test<T, isarray>::assert_construct(
    typename call_traits_test<T, isarray>::param_type val) {
  //
...
template <typename T>
void call_traits_test<T, true>::assert_construct(
