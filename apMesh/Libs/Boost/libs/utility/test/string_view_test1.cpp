...
void test_substr(const std::string &str) {
  ...
  for (size_t i = 0; i <= sz; ++i) interop(str.substr(i), ref.substr(i));

  //  Substrings at the beginning
  for (size_t i = 0; i <= sz; ++i) interop(str.substr(0, i), ref.substr(0, i));

  //  All possible substrings
  for (size_t i = 0; i < sz; ++i)
    for (size_t j = i; j < sz; ++j) interop(str.substr(i, j), ref.substr(i, j));
}
...
const char *test_strings[] = {"", "1", "ABCDEFGHIJKLMNOPQRSTUVWXYZ",
                              "0123456789", NULL};
