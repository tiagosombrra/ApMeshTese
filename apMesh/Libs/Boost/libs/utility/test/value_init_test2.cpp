...
bool is_value_initialized(const derived_struct& arg) { return arg.data == 0; }
...
bool is_value_initialized(const T (&arg)[2]) {
  return is_value_initialized(arg[0]) && is_value_initialized(arg[1]);
}
...
// Returns zero when the specified object is value-initializated, and one
// otherwise. Prints a message to standard output if the value-initialization
// has failed.
template <class T>
unsigned failed_to_value_initialized(const T& object,
...
int main() {
  ...
  BOOST_TEST(is_value_initialized(
      boost::value_initialized<virtual_destructor_holder[2]>()));
  BOOST_TEST(
      is_value_initialized(boost::value_initialized<private_int_array_pair>()));
  ...
  BOOST_TEST(
      is_value_initialized(boost::value_initialized<ptr_to_member_struct>()));
