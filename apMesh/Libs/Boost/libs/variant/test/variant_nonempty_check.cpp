...
// In this file we are making tests to ensure that variant guarantees
// nonemptiness.
//
// For that purpose we create a `throwing_class`, that throws exception at a
// specified assignment attempt. If exception was thrown during move/assignemnt
// operation we make sure that data in variant is same as before move/assignemnt
// operation or that a fallback type is stored in variant.
//
// Different nonthrowing_class'es are used to tests different variant internal
// policies: with/without fallback type + throw/nothrow copyable + throw/nothrow
// movable
...
struct throwing_class {
  ...
  throwing_class(const throwing_class& b) BOOST_NOEXCEPT_IF(false)
      : trash(b.trash) {
    if (is_throw()) {
...
struct nonthrowing_class {
  ...
  const nonthrowing_class& operator=(const nonthrowing_class&)
      BOOST_NOEXCEPT_IF(false) {
    prevent_compiler_noexcept_detection();
    return *this;
  }
  ...
  const nonthrowing_class& operator=(nonthrowing_class&&)
      BOOST_NOEXCEPT_IF(false) {
    prevent_compiler_noexcept_detection();
    return *this;
  }
...
struct nonthrowing_class3 {
  ...
  const nonthrowing_class3& operator=(const nonthrowing_class3&)
      BOOST_NOEXCEPT_IF(false) {
    prevent_compiler_noexcept_detection();
    return *this;
  }
  ...
  const nonthrowing_class3& operator=(nonthrowing_class3&&)
      BOOST_NOEXCEPT_IF(false) {
    prevent_compiler_noexcept_detection();
    return *this;
  }
...
struct nonthrowing_class4 {
  ...
  const nonthrowing_class4& operator=(const nonthrowing_class4&)
      BOOST_NOEXCEPT_IF(false) {
    prevent_compiler_noexcept_detection();
    return *this;
  }
  ...
  const nonthrowing_class4& operator=(nonthrowing_class4&&)
...
int main() {
  // throwing_class::throw_after_1 + 1  => throw on first
  // assignment/construction throwing_class::throw_after_1  => throw on second
  // assignment/construction throwing_class::throw_after_2  => throw on third
  // assignment/construction
  // ...
  for (int i = throwing_class::throw_after_1 + 1;
       i != throwing_class::do_not_throw; --i) {
    check_1(i);
    check_2(i);
    check_3(i);
    check_4(i);
