...
inline int get_value(prefix const& v) { return v.v_; }

#if !defined(NO_V2)

template <class... T>
int get_value(boost::variant2::variant<T...> const& v) {
...
template <class... T>
int get_value(boost::variant<T...> const& v) {
...
template <class... T>
int get_value(std::variant<T...> const& v) {
  return visit([](prefix const& x) { return x.v_; }, v);
}
