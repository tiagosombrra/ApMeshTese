...
#include "../Headers/Method/Method_Sequential.h"
...
// argv[1] = "seq ou par",
// argv[2] = "entrada",   OBS: Projects-> Comands line arguments -> ../../apMesh/Entrada/mountain_289_patches.bp
// argv[3] = "nome do modelo",
// argv[4] = "n° de process"
// argv[5] = "salva a malha gerada em cada passo e processo"
...
int main(int argc, char **argv)
{
    ...
    //Verifica a quantidade parametros de entreda
    if (argc > 1) {
        entrada = argv[2];
        nameModel = argv[3];
        numberProcess = argv[4];
        WRITE_MESH = argv[5];

     //Teste sequencial
    } else{
        timer->SetTypeRun("seq");
        Method_Sequential* ms = new Method_Sequential(argc, argv);
        if (ms->execute(timer) == 0) {
            cout << "Método Sequencial Finalizado com Sucesso!" << endl;
            finish = 0;
        } else {
            cout << "Erro de Finalização do Método Sequencial!" << endl;
            finish = 1;
            return -1;
        };
        delete ms;
    ...
    if (argv[1] == std::string("seq")) {
        ...
        if (mp->execute(argc, argv, timer) == 0) {
            ...
            cout << "Erro de Finalização do Método Paralelo!" << endl;
            finish = 1;
        }
        delete mp;
#endif //#if USE_OPENMPI
