...
class SubMesh {
 public:
  ...
  vector<std::shared_ptr<NodeAdaptive>> GetNos();
  vector<std::shared_ptr<ElementAdaptive>> GetElements();

 protected:
  std::shared_ptr<Patch> patch_;
  vector<std::shared_ptr<NodeAdaptive>> nos_;
  vector<std::shared_ptr<ElementAdaptive>> elements_;
};
