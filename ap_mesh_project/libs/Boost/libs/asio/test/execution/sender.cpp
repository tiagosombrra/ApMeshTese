...
struct sender_using_base : boost::asio::execution::sender_base {
  sender_using_base() {}
};
...
template <typename T>
bool is_unspecialised(
    T*, typename boost::asio::void_type<typename exec::sender_traits<
            T>::asio_execution_sender_traits_base_is_unspecialised>::type*) {
  return true;
}
