...
void test_dynamic_string_read_until_char() {
  ...
  boost::asio::dynamic_string_buffer<char, std::string::traits_type,
      ...
      sb1 = boost::asio::dynamic_buffer(data1);
  boost::asio::dynamic_string_buffer<char, std::string::traits_type,
                                     std::string::allocator_type>
      sb2 = boost::asio::dynamic_buffer(data2, 25);
  boost::system::error_code ec;
...
void async_read_handler(const boost::system::error_code& err,
                        ...
                        std::size_t bytes_transferred, std::size_t* bytes_out,
                        bool* called) {
  *err_out = err;
...
void test_dynamic_string_async_read_until_char() {
  ...
  boost::asio::async_read_until(
      s, sb1, 'Z',
      bindns::bind(async_read_handler, _1, &ec, _2, &length, &called));
  ioc.restart();
  ioc.run();
  ...
  sb2.consume(sb2.size());
  int i =
      boost::asio::async_read_until(s, sb2, 'Y', archetypes::lazy_handler());
...
void test_dynamic_string_async_read_until_string() {
  ...
  sb2.consume(sb2.size());
  int i =
      boost::asio::async_read_until(s, sb2, "WXY", archetypes::lazy_handler());
  BOOST_ASIO_CHECK(i == 42);
  ioc.restart();
  ioc.run();
}
...
BOOST_ASIO_TEST_SUITE(
    ...
    BOOST_ASIO_TEST_CASE(test_dynamic_string_read_until_char)
        BOOST_ASIO_TEST_CASE(test_streambuf_read_until_char) BOOST_ASIO_TEST_CASE(
            test_dynamic_string_read_until_string)
            BOOST_ASIO_TEST_CASE(test_streambuf_read_until_string) BOOST_ASIO_TEST_CASE(
                test_dynamic_string_read_until_match_condition)
                BOOST_ASIO_TEST_CASE(test_streambuf_read_until_match_condition)
                    BOOST_ASIO_TEST_CASE(
                        test_dynamic_string_async_read_until_char)
                        BOOST_ASIO_TEST_CASE(
                            test_streambuf_async_read_until_char)
                            BOOST_ASIO_TEST_CASE(
                                test_dynamic_string_async_read_until_string)
                                BOOST_ASIO_TEST_CASE(
                                    test_streambuf_async_read_until_string)
                                    BOOST_ASIO_TEST_CASE(
                                        test_dynamic_string_async_read_until_match_condition)
                                        BOOST_ASIO_TEST_CASE(
                                            test_streambuf_async_read_until_match_condition))
