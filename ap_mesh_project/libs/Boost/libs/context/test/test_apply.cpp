...
struct callable {
  ...
  callable(int k_) : k{k_} {}

  int foo(int i, int j) const { return i + j + k; }

  int operator()(int i, int j) const { return foo(i, j); }
};
...
struct movable {
  ...
  movable(movable&& other) : k{other.k} { other.k = -1; }
...
int* fn2(int* ip) { return ip; }

int* fn3(int& ir) { return &ir; }

int& fn4(int& ir) { return ir; }

int fn5(int i, callable&& c) { return i + c.k; }

int fn6(int i, movable&& m) { return i + m.k; }
...
void test7() {
  ...
  }
  {
    movable m;
    int result =
        ctx::detail::apply(&movable::foo, std::make_tuple(std::move(m), 1, 2));
    BOOST_CHECK_EQUAL(result, 3);
  }
}
