...
void test_prealloc() {
  ...
  int i = 7;
  ctx::fiber f{std::allocator_arg, ctx::preallocated(sp, size, sctx), alloc,
               [&i](ctx::fiber&& f) {
                 value1 = i;
                 return std::move(f);
               }};
...
void test_ontop() {
  ...
  }
  {
    ...
    f = std::move(f).resume();
    f = std::move(f).resume_with([&f1](ctx::fiber&& f) {
      f1 = std::move(f);
      return std::move(f);
    });
  }
...
void test_ontop_exception() {
  ...
  BOOST_CHECK_EQUAL(3, value1);
  const char* what = "hello world";
  f = std::move(f).resume_with([what](ctx::fiber&& f) {
