...
T const& abstract_stack<T>::item(boost::contract::virtual_* v) const {
  ...
  boost::contract::check c =
      boost::contract::public_function(v, result, this).precondition([&] {
...
void abstract_stack<T>::push(T const& value, boost::contract::virtual_* v) {
  boost::contract::check c =
      boost::contract::public_function(v, this)
          .precondition([&] { BOOST_CONTRACT_ASSERT(!full()); })
...
void abstract_stack<T>::clear(boost::contract::virtual_* v) {
  boost::contract::check c =
      boost::contract::public_function(v, this).postcondition(
          [&] { BOOST_CONTRACT_ASSERT(empty()); });
  assert(false);
}
...
#define BASES                                            \
 private                                                 \
  boost::contract::constructor_precondition<vstack<T> >, \
...
#undef BASES
 ...
 public:
  ...
  virtual int length(boost::contract::virtual_* v = 0) const /* override */ {
    int result;
    boost::contract::check c =
        boost::contract::public_function<override_length>(
            v, result, &vstack::length, this);
    return result = len_;
  }

  virtual int capacity(boost::contract::virtual_* v = 0) const /* override */ {
    int result;
    boost::contract::check c =
        boost::contract::public_function<override_capacity>(
            v, result, &vstack::capacity, this);
    return result = vect_.size();
  }

  virtual T const& item(boost::contract::virtual_* v = 0) const /* override */ {
    boost::optional<T const&> result;
    boost::contract::check c = boost::contract::public_function<override_item>(
  ...
  /* override */ {
    boost::contract::check c = boost::contract::public_function<override_push>(
  ...
  virtual T const& pop(boost::contract::virtual_* v = 0) /* override */ {
    boost::optional<T const&> result;
    boost::contract::check c = boost::contract::public_function<override_pop>(
        v, result, &vstack::pop, this);
    return *(result = vect_[--len_]);
  }

  virtual void clear(boost::contract::virtual_* v = 0) /* override */ {
    boost::contract::check c = boost::contract::public_function<override_clear>(
        v, &vstack::clear, this);
    len_ = 0;
  }
