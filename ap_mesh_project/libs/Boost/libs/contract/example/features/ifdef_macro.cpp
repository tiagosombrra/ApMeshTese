...
class pushable {
  ...
  BOOST_CONTRACT_INVARIANT({ BOOST_CONTRACT_ASSERT(capacity() <= max_size()); })
...
#undef BASES
  ...
  BOOST_CONTRACT_INVARIANT({ BOOST_CONTRACT_ASSERT(size() <= capacity()); })
 ...
 public:
  ...
  virtual void push_back(
    ...
    (v, &integers::push_back, this, x) BOOST_CONTRACT_PRECONDITION([&] {
      BOOST_CONTRACT_ASSERT(size() < max_size());
    }) BOOST_CONTRACT_OLD([&] { old_size = BOOST_CONTRACT_OLDOF(v, size()); })
        BOOST_CONTRACT_POSTCONDITION([&] {
