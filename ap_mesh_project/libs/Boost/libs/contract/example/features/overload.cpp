...
class lines {
 public:
  ...
  virtual void put(std::string const& x, boost::contract::virtual_* v = 0) = 0;
...
std::string& lines::str(boost::contract::virtual_* v) {
  ...
  boost::contract::check c =
      boost::contract::public_function(v, result, this)
          .postcondition(
              [&](boost::optional<std::string const&> const& result) {
                if (*result != "")
                  BOOST_CONTRACT_ASSERT(*result->rbegin() == '\n');
              });
  assert(false);
  return *result;
}
...
void lines::put(int x, bool tab, boost::contract::virtual_* v) {
  boost::contract::check c =
      boost::contract::public_function(v, this).precondition(
...
#undef BASES
  ...
  std::string str(boost::contract::virtual_* v = 0) const /* override */ {
    std::string result;
    boost::contract::check c = boost::contract::public_function<override_str>(
        v, result,
        // `static_cast` resolves overloaded function pointer ambiguities.
        static_cast<std::string (string_lines::*)(boost::contract::virtual_*)
                        const>(&string_lines::str),
        this);
  ...
  void put(char x, boost::contract::virtual_* v = 0) /* override */ {
    ...
    boost::contract::check c =
        boost::contract::public_function<override_put>(
            v,
            // `static_cast` resolves overloaded function pointer ambiguities.
            static_cast<void (string_lines::*)(
                char, boost::contract::virtual_*)>(&string_lines::put),
            this, x)
            .postcondition(
                [&] { BOOST_CONTRACT_ASSERT(str() == *old_str + x + '\n'); });
  ...
  void put(int x, bool tab = false,
    ...
    boost::contract::check c =
        boost::contract::public_function<override_put>(
            ...
            // `static_cast` resolves overloaded function pointer ambiguities.
            static_cast<void (string_lines::*)(
                int, bool, boost::contract::virtual_*)>(&string_lines::put),
            this, x, tab)
            .postcondition([&] {
              std::ostringstream s;
              s << x;
              BOOST_CONTRACT_ASSERT(str() == *old_str + (tab ? "\t" : "") +
                                                 s.str() + '\n');
            });
