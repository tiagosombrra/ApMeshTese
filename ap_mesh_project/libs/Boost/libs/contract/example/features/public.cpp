...
class unique_identifiers
  ...
  bool find(int id) const {
    ...
    boost::contract::check c =
        boost::contract::public_function(this)
            .precondition([&] { BOOST_CONTRACT_ASSERT(id >= 0); })
            .postcondition([&] {
 ...
 public:
  ...
  virtual int push_back(int id,
                        boost::contract::virtual_* v = 0) {  // Extra `v`.
    int result;
    ...
    boost::contract::check c =
        boost::contract::public_function(v, result,
                                         this)  // Pass `v` and `result`.
            .precondition([&] {
              BOOST_CONTRACT_ASSERT(id >= 0);
              BOOST_CONTRACT_ASSERT(
                  !find(id));  // ID cannot be already present.
            })
...
#undef BASES
 ...
 public:
  ...
  int push_back(int id, boost::contract::virtual_* v = 0) /* override */ {
    int result;
    boost::contract::old_ptr<bool> old_find = BOOST_CONTRACT_OLDOF(v, find(id));
    boost::contract::old_ptr<int> old_size = BOOST_CONTRACT_OLDOF(v, size());
    boost::contract::check c =
        boost::contract::public_function<
            override_push_back  // Pass override type plus below function
                                // pointer...
            >(v, result, &identifiers::push_back, this,
              id)                // ...and arguments.
