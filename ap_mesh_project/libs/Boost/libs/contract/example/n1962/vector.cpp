...
class vector {
 ...
 public:
  ...
  vector(InputIter first, InputIter last) : vect_(first, last) {
    boost::contract::check c =
        boost::contract::constructor(this).postcondition([&] {
          BOOST_CONTRACT_ASSERT(std::distance(first, last) == int(size()));
        });
  }
  ...
  /* implicit */ vector(vector const& other) : vect_(other.vect_) {
    boost::contract::check c =
        boost::contract::constructor(this).postcondition([&] {
          BOOST_CONTRACT_ASSERT(
              boost::contract::condition_if<boost::has_equal_to<T> >(
                  boost::bind(std::equal_to<vector<T> >(), boost::cref(*this),
                              boost::cref(other))));
        });
  }
  ...
  vector& operator=(vector const& other) {
    ...
    boost::contract::check c =
        boost::contract::public_function(this).postcondition([&] {
          ...
          BOOST_CONTRACT_ASSERT(
              boost::contract::condition_if<boost::has_equal_to<T> >(
                  boost::bind(std::equal_to<vector<T> >(), boost::cref(*result),
                              boost::cref(*this))));
        });
  ...
  void reserve(size_type count) {
    boost::contract::check c =
        boost::contract::public_function(this)
            ...
            .postcondition([&] { BOOST_CONTRACT_ASSERT(capacity() >= count); });
  ...
  void resize(size_type count, T const& value = T()) {
    boost::contract::old_ptr<size_type> old_size = BOOST_CONTRACT_OLDOF(size());
    boost::contract::check c =
        boost::contract::public_function(this).postcondition([&] {
          BOOST_CONTRACT_ASSERT(size() == count);
          if (count > *old_size) {
            BOOST_CONTRACT_ASSERT(
                boost::contract::condition_if<boost::has_equal_to<T> >(
                    boost::bind(all_of_equal_to(), begin() + *old_size, end(),
                                boost::cref(value))));
          }
        });
  ...
  void push_back(T const& value) {
    ...
    boost::contract::check c =
        boost::contract::public_function(this)
            .precondition([&] { BOOST_CONTRACT_ASSERT(size() < max_size()); })
            .postcondition([&] {
  ...
  void insert(iterator where, size_type count, T const& value) {
    ...
    boost::contract::old_ptr<size_type> old_capacity =
        BOOST_CONTRACT_OLDOF(capacity());
    boost::contract::old_ptr<iterator> old_where = BOOST_CONTRACT_OLDOF(where);
    boost::contract::check c =
        boost::contract::public_function(this)
            .precondition(
            ...
            .postcondition([&] {
              ...
              if (capacity() == *old_capacity) {
                BOOST_CONTRACT_ASSERT(
                    boost::contract::condition_if<boost::has_equal_to<T> >(
                        boost::bind(all_of_equal_to(), boost::prior(*old_where),
  ...
  void insert(iterator where, InputIter first, InputIter last) {
    ...
    boost::contract::check c =
        boost::contract::public_function(this)
            ...
            .postcondition([&] {
              BOOST_CONTRACT_ASSERT(size() ==
                                    *old_size() + std::distance(first, last));
              BOOST_CONTRACT_ASSERT(capacity() >= *old_capacity);
              if (capacity() == *old_capacity) {
                BOOST_CONTRACT_ASSERT(
                    boost::contract::condition_if<boost::has_equal_to<T> >(
  ...
  void swap(vector& other) {
    ...
    boost::contract::check c =
        boost::contract::public_function(this)
            ...
            .postcondition([&] {
              BOOST_CONTRACT_ASSERT_AUDIT(boost::contract::condition_if<
                                          boost::has_equal_to<vector<T> > >(
                  boost::bind(std::equal_to<vector<T> >(), boost::cref(*this),
                              boost::cref(*old_other))));
              BOOST_CONTRACT_ASSERT_AUDIT(boost::contract::condition_if<
                                          boost::has_equal_to<vector<T> > >(
                  boost::bind(std::equal_to<vector<T> >(), boost::cref(other),
                              boost::cref(*old_me))));
            });
  ...
  friend bool operator==(vector const& left, vector const& right) {
    ...
    boost::contract::check left_inv = boost::contract::public_function(&left);
    boost::contract::check right_inv = boost::contract::public_function(&right);
