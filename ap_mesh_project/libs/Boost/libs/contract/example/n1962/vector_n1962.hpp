...
class vector {
 ...
 public:
  ...
  vector() postcondition { empty(); }
        : vect_()
    {}

        explicit vector(Allocator const& alloc) postcondition {
          empty();
          get_allocator() == alloc;
        }
        : vect_(alloc)
    {}
        ...
        explicit vector(size_type count) postcondition {
          size() == count;
    ...
    {}
        ...
        vector(size_type count, T const& value) postcondition {
          size() == count;
          if constexpr (boost::has_equal_to<T>::value) {
    ...
    {}
        ...
        vector(size_type count, T const& value,
               Allocator const& alloc) postcondition {
          size() == count;
          if constexpr (boost::has_equal_to<T>::value) {
            boost::algorithm::all_of_equal(begin(), end(), value);
          }
    ...
    {}
        ...
        vector(InputIter first, InputIter last) postcondition {
    ...
    {}
        ...
        /* implicit */ vector(vector const& other) postcondition {
          if constexpr (boost::has_equal_to<T>::value) {
            *this == other;
          }
        }
    ...
    {}
        ...
        vector& operator=(vector const& other) postcondition(result) {
          if constexpr (boost::has_equal_to<T>::value) {
            *this == other;
            result == *this;
          }
        ...
        void reserve(size_type count) precondition { count < max_size(); }
        postcondition { capacity() >= count; }
        { vect_.reserve(count); }

        size_type capacity() const postcondition(result) { result >= size(); }
        { return vect_.capacity(); }

        iterator begin() postcondition {
          if (empty()) result == end();
        }
        { return vect_.begin(); }

        const_iterator begin() const postcondition(result) {
          if (empty()) result == end();
        }
        { return vect_.begin(); }
        ...
        reverse_iterator rbegin() postcondition(result) {
          if (empty()) result == rend();
        }
        { return vect_.rbegin(); }

        const_reverse_iterator rbegin() const postcondition(result) {
          if (empty()) result == rend();
        }
        { return vect_.rbegin(); }

        reverse_iterator rend() { return vect_.rend(); }

        const_reverse_iterator rend() const { return vect_.rend(); }

        void resize(size_type count, T const& value = T()) postcondition {
          size() == count;
          if constexpr (boost::has_equal_to<T>::value) {
            if (count > oldof(size())) {
        ...
        size_type size() const postcondition(result) { result <= capacity(); }
        { return vect_.size(); }

        size_type max_size() const postcondition(result) {
          result >= capacity();
        }
        { return vect_.max_size(); }

        bool empty() const postcondition(result) { result == (size() == 0); }
        { return vect_.empty(); }

        Alloctor get_allocator() const { return vect_.get_allocator(); }
        ...
        reference operator[](size_type index) precondition { index < size(); }
        { return vect_[index]; }

        const_reference operator[](size_type index) const precondition {
          index < size();
        }
        { return vect_[index]; }

        reference front() precondition { !empty(); }
        { return vect_.front(); }

        const_reference front() const precondition { !empty(); }
        { return vect_.front(); }

        reference back() precondition { !empty(); }
        { return vect_.back(); }

        const_reference back() const precondition { !empty(); }
        { return vect_.back(); }

        void push_back(T const& value) precondition { size() < max_size(); }
        postcondition {
          size() == oldof(size()) + 1;
          capacity() >=
              oldof(capacity()) if constexpr (boost::has_equal_to<T>::value) {
            back() == value;
          }
        }
        ...
        void assign(size_type count, T const& vallue) precondition {
          count <= max_size();
        }
        postcondition {
          if constexpr (boost::has_equal_to<T>::value) {
        ...
        iterator insert(iterator where, T const& value) precondition {
          size() < max_size();
        }
        ...
        template <typename InputIter>
        void insert(iterator where, Iterator first,
                    Iterator last) precondition {
        ...
        iterator erase(iterator where) precondition {
          !empty();
          where != end();
        }
        postcondition(result) {
        ...
        iterator erase(iterator first, iterator last) precondition {
        ...
        void clear() postcondition { empty(); }
        { vect_.clear(); }

        void swap(vector& other) precondition {
