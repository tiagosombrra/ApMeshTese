...
struct counter {
  ...
  static counter const& eval(counter const& me) {
    ++me.evals_;
    return me;
  }
...
template <class Tag, typename T>
unsigned counter<Tag, T>::ctors_ = 0;
template <class Tag, typename T>
unsigned counter<Tag, T>::dtors_ = 0;
template <class Tag, typename T>
unsigned counter<Tag, T>::ctor_copies_ = 0;
template <class Tag, typename T>
unsigned counter<Tag, T>::op_copies_ = 0;
template <class Tag, typename T>
unsigned counter<Tag, T>::evals_ = 0;
