...
#define BASES \
 private      \
  boost::contract::constructor_precondition<a>, public b
    : BASES {
...
#undef BASES
  ...
  virtual void f(char x,
                 boost::contract::virtual_* v = 0) volatile /* override */ {
    boost::contract::check c =
        boost::contract::public_function<override_f>(
            v,
            static_cast<void (a::*)(
                char x, boost::contract::virtual_*) volatile>(&a::f),
            this, x)
            .precondition([&] {
...
int main() {
  ...
  {  // Test volatile call with bases.
    ...
    a volatile av;
    ok.str("");
    ok  // Ctors always check const_inv (even if volatile).
    ...
    av.f('a');
    ok.str("");
    ok  // Volatile checks static and cv (but not const) inv.
#ifndef BOOST_CONTRACT_NO_ENTRY_INVARIANTS
#ifdef BOOST_CONTRACT_TEST_STATIC_INV
        << "b::static_inv" << std::endl
#endif
  ...
  ok.str("");
  ok  // Dtors always check const_inv (even if volatile).
#ifndef BOOST_CONTRACT_NO_ENTRY_INVARIANTS
#ifdef BOOST_CONTRACT_TEST_STATIC_INV
      << "a::static_inv" << std::endl
#endif
  ...
  {  // Test non-volatile call with bases.
    ...
    a aa;
    ok.str("");
    ok  // Ctors always check cv_inv (even if not volatile).
#ifndef BOOST_CONTRACT_NO_PRECONDITIONS
        << "a::ctor::pre" << std::endl
        << "b::ctor::pre" << std::endl
#endif
    ...
    aa.f('a');
    ok.str("");
    ok  // Non-cv checks static and const (but not cv) inv.
#ifndef BOOST_CONTRACT_NO_ENTRY_INVARIANTS
#ifdef BOOST_CONTRACT_TEST_STATIC_INV
        << "b::static_inv" << std::endl
#endif
  ...
  ok.str("");
  ok  // Dtors always check cv_inv (even if not volatile).
#ifndef BOOST_CONTRACT_NO_ENTRY_INVARIANTS
#ifdef BOOST_CONTRACT_TEST_STATIC_INV
      << "a::static_inv" << std::endl
#endif
