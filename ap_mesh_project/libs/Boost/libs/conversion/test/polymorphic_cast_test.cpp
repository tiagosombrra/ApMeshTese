...
// assertion handler throws it to exit like assert, but to be able to catch it
// and stop usage: BOOST_TEST_THROWS( function_with_assert(), expected_assertion
// );
...
void boost::assertion_failed(char const* expr, char const* function,
  ...
  if (expect_assertion) {
    ...
    BOOST_LIGHTWEIGHT_TEST_OSTREAM << file << "(" << line << "): assertion '"
...
static void test_polymorphic_cast() {
  ...
  try {
    ...
    BOOST_ERROR(
        "boost::polymorphic_cast<Derived*>( base ) threw std::bad_cast");
  }
...
static void test_polymorphic_pointer_cast() {
  ...
  try {
    ...
    BOOST_ERROR(
        "boost::polymorphic_pointer_cast<Derived>( base ) threw std::bad_cast");
  }
  ...
  try {
    ...
    BOOST_ERROR(
        "boost::polymorphic_pointer_cast<Base2>( base ) threw std::bad_cast");
  }
  ...
  try {
    ...
    BOOST_ERROR(
        "boost::polymorphic_pointer_cast<Base2>( sp_base ) threw "
        "std::bad_cast");
  }
...
static void test_polymorphic_downcast() {
  ...
  Derived* derived_pointer =
      boost::polymorphic_downcast<Derived*>(base_pointer);
...
static void test_polymorphic_pointer_downcast_boost_shared() {
  ...
  boost::shared_ptr<Derived> derived =
      boost::polymorphic_pointer_downcast<Derived>(base);
...
static void test_polymorphic_pointer_downcast_intrusive() {
  ...
  boost::intrusive_ptr<Derived> derived =
      boost::polymorphic_pointer_downcast<Derived>(base);
...
static void test_polymorphic_pointer_cast_fail() {
  Base* base = new Base;
  BOOST_TEST_THROWS(boost::polymorphic_pointer_cast<Derived>(base),
                    std::bad_cast);
  delete base;

  BOOST_TEST_THROWS(boost::polymorphic_pointer_cast<Derived>(
                        boost::shared_ptr<Base>(new Base)),
                    std::bad_cast);

#ifndef BOOST_NO_CXX11_SMART_PTR
  BOOST_TEST_THROWS(
      boost::polymorphic_pointer_cast<Derived>(std::shared_ptr<Base>(new Base)),
      std::bad_cast);
#endif

  BOOST_TEST_THROWS(boost::polymorphic_pointer_cast<Derived>(
                        boost::intrusive_ptr<Base>(new Base)),
                    std::bad_cast);
}
...
static void test_polymorphic_downcast_fail() {
...

  {
    ...
    BOOST_TEST_THROWS(boost::polymorphic_downcast<Derived*>(base_pointer),
                      expected_assertion);  // should assert
...
static void test_polymorphic_pointer_downcast_builtin_fail() {
  ...
  BOOST_TEST_THROWS(boost::polymorphic_pointer_downcast<Derived>(base),
                    expected_assertion);  // should assert
