...
static void introduction() {
  ...
  /*`The following code demonstrates conversion of an array of integers from
     their textual ['hexadecimal] representation. It assigns -1 to those which
     fail to convert:
  */
...
static void example1() {
  //[algorithm_example1
  /*`The following code demonstrates a failed attempt (and one of the reasons
     ['Boost.Convert] has been developed) to convert a few `string`s to `int`s
  ...
  */
  ...
  try {
    std::transform(
        strs.begin(), strs.end(), std::back_inserter(ints),
...
static void example2() {
  //[algorithm_example2
  /*`If the exception-throwing behavior is the desired behavior, then
    ['Boost.Convert] supports that. In addition, it also supports a non-throwing
    process-flow:
  */
  std::array<char const*, 3> strs = {{" 5", "0XF", "not an int"}};
  std::vector<int> ints;
...
static void example3() {
  ...
  /*`Deploying `boost::cnv::cstream` with better formatting capabilities yields
     better results with exception-throwing and non-throwing process-flows still
     supported:
  */
...
static void example4() {
  ...
  /*`[important One notable difference in the deployment of
     `boost::cnv::cstream` with algorithms is the use of `std::cref`.

     It needs to be remembered that with standard algorithms the deployed
     converter needs to be
     [@http://en.cppreference.com/w/cpp/named_req/TriviallyCopyable copyable] or
     [@http://en.cppreference.com/w/cpp/named_req/MoveAssignable movable
     (C++11)] and is, in fact, copied or moved by the respective algorithm
     before being used. Given that `std::cstringstream` is not copyable,
     `boost::cnv::cstream` is not copyable either. That limitation is routinely
     worked-around using `std::ref` or `std::cref`.]
  */
  //]
}
...
static void example5() {
  //[algorithm_example5
  /*`And now an example of algorithm-based integer-to-string formatted
     conversion with `std::hex`, `std::uppercase` and `std::showbase` formatting
     applied:
  */
