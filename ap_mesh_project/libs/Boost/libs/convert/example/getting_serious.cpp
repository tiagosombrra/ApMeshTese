...
static void example1() {
...

  {
    ...
    int i2 =
        convert<int>("not an int", cnv).value_or(-1);  // after the call i2==-1
  ...
  }
  {
    ...
    try {
      ...
      int i2 =
          convert<int>(str, cnv).value();  // Throws if the conversion fails.
    } catch (...) {
      process_failure();
    }
    //]
  }
  {
    ...
    optional<int> r1 =
        convert<int>(str1, cnv);  // Does not throw on conversion failure.
    optional<int> r2 =
        convert<int>(str2, cnv);  // Does not throw on conversion failure.
    // ...
