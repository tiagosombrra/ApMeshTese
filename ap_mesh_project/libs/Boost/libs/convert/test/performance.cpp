...
template <typename T>
static typename array<T>::type const& get();
...
struct timer : public boost::timer::cpu_timer {
  ...
  double value() const {
    ...
    int const use_sum = (sum % 2) ? 0 : (sum % 2);
    BOOST_TEST(use_sum == 0);

    return double(times.user + times.system) / 1000000000 + use_sum;
  }
};
template <typename Type, typename Cnv>
static double str_to(Cnv const&);
template <typename S, typename Type, typename Cnv>
static double to_str(Cnv const&);

template <>
local::array<int>::type const& get<int>() {
  ...
  if (!filled) {
    ...
    boost::random::uniform_int_distribution<> dist(
        INT_MIN, INT_MAX);  // INT_MAX(32) = 2,147,483,647

    for (size_t k = 0; k < ints.size(); ++k) ints[k] = dist(gen);
...
template <>
array<long int>::type const& get<long int>() {
  ...
  if (!filled) {
    boost::random::mt19937 gen(::time(0));
    boost::random::uniform_int_distribution<> dist(
        INT_MIN, INT_MAX);  // INT_MAX(32) = 2147483647
...
template <>
array<double>::type const& get<double>() {
...
double raw_str_to(Converter const& cnv) {
  auto strings = local::get_strs();  // Create strings on the stack
  int size = strings.size();
  auto timer = local::timer();

  for (int t = 0; t < local::num_cycles; ++t)
    for (int k = 0; k < size; ++k) local::sum += cnv(strings[k].c_str());

  return timer.value();
}

template <typename Type, typename Converter>
double local::str_to(Converter const& try_converter) {
  auto strings = local::get_strs();  // Create strings on the stack
  int size = strings.size();
  auto timer = local::timer();
  ...
  for (int t = 0; t < local::num_cycles; ++t)
    for (int k = 0; k < size; ++k)
      local::sum +=
          boost::convert<Type>(strings[k].c_str(), try_converter).value();

  return timer.value();
}

template <typename string_type, typename Type, typename Converter>
double local::to_str(Converter const& try_converter) {
  ...
  for (int t = 0; t < local::num_cycles; ++t)
    for (int k = 0; k < size; ++k)
      local::sum += *boost::convert<string_type>(Type(values[k]), try_converter)
                         .value()
                         .begin();
...
double performance_str_to_type(Converter const& try_converter) {
...
template <typename Converter>
double performance_type_to_str(Converter const& try_converter) {
...
void performance_comparative(Raw const& raw, Cnv const& cnv, char const* txt) {
...
int main(int, char const*[]) {
  ...
  printf(
      "str-to-int: "
      "spirit/strtol/lcast/scanf/stream=%7.2f/%7.2f/%7.2f/%7.2f/%7.2f "
      "seconds.\n",
  ...
  printf(
      "int-to-str: "
      "spirit/strtol/lcast/prntf/stream=%7.2f/%7.2f/%7.2f/%7.2f/%7.2f "
      "seconds.\n",
  ...
  printf(
      ...
      local::to_str<std::string, double>(
          boost::cnv::strtol()(arg::precision = 6)),
      local::to_str<std::string, double>(boost::cnv::lexical_cast()),
      local::to_str<std::string, double>(
          boost::cnv::printf()(arg::precision = 6)),
      local::to_str<std::string, double>(
          boost::cnv::cstream()(arg::precision = 6)));
  ...
  //]
  performance_comparative(raw_str_to_int_spirit(), boost::cnv::spirit(),
                          "spirit");
  performance_comparative(raw_str_to_int_lxcast(), boost::cnv::lexical_cast(),
                          "lxcast");
