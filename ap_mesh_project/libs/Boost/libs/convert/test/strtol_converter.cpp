...
static void test_uint_to_str() {
  ...
  unsigned long int const ulmax =
      (std::numeric_limits<unsigned long int>::max)();
  unsigned long long int const ullmax =
...
static void test_width() {
  ...
  string s02 =
      convert<string>(12, cnv(arg::width = 5)(arg::fill = '*')).value();
  string s03 = convert<string>(12, cnv(arg::width = 5)(arg::fill = 'x')(
                                       arg::adjust = cnv::adjust::left))
                   .value();
  string s04 = convert<string>(-98, cnv(arg::width = 6)(arg::fill = 'Z')(
                                        arg::adjust = cnv::adjust::right))
                   .value();

  string s05 =
      convert<string>(-12.3451,
                      cnv(arg::precision = 2)(arg::width = 10)(arg::fill = '*')(
                          arg::adjust = cnv::adjust::left))
          .value();
  string s06 =
      convert<string>(-12.3450, cnv(arg::adjust = cnv::adjust::right)).value();
  string s07 =
      convert<string>(-12.3450, cnv(arg::adjust = cnv::adjust::center)).value();
...
static void test_base() {
  ...
  BOOST_TEST("11111110" ==
             convert<string>(254, cnv(arg::base = cnv::base::bin)).value());
  BOOST_TEST("254" ==
             convert<string>(254, cnv(arg::base = cnv::base::dec)).value());
  BOOST_TEST("FE" ==
             convert<string>(254, cnv(arg::base = cnv::base::hex)).value());
  BOOST_TEST("376" ==
             convert<string>(254, cnv(arg::base = cnv::base::oct)).value());
  //]
  //[wide_strtol_numeric_base
  BOOST_TEST(L"11111110" ==
             convert<wstring>(254, cnv(arg::base = cnv::base::bin)).value());
  BOOST_TEST(L"254" ==
             convert<wstring>(254, cnv(arg::base = cnv::base::dec)).value());
  BOOST_TEST(L"FE" ==
             convert<wstring>(254, cnv(arg::base = cnv::base::hex)).value());
  BOOST_TEST(L"376" ==
             convert<wstring>(254, cnv(arg::base = cnv::base::oct)).value());
  //]
}

static void test_upper() {
  //    boost::cnv::strtol cnv;
  ...
  //
  //    BOOST_TEST(c_lcase == convert< string>(c_lcase, cnv(arg::uppercase =
  //    false)).value_or("")); BOOST_TEST(c_ucase == convert< string>(c_lcase,
  //    cnv(arg::uppercase =  true)).value_or("")); BOOST_TEST(w_ucase ==
  //    convert<wstring>(w_lcase, cnv(arg::uppercase = false)).value_or(""));
  //    BOOST_TEST(w_ucase == convert<wstring>(w_lcase, cnv(arg::uppercase =
  //    true)).value_or(""));
}
...
static void dbl_to_str_example() {
  //[strtol_precision
  boost::cnv::strtol cnv;

  BOOST_TEST("12.3" ==
             convert<string>(12.3456, cnv(arg::precision = 1)).value());
  BOOST_TEST("12.35" ==
             convert<string>(12.3456, cnv(arg::precision = 2)).value());
  BOOST_TEST("12.346" ==
             convert<string>(12.3456, cnv(arg::precision = 3)).value());
...
static std::pair<double, int> get_random() {
...
static void compare(std::pair<double, int> pair) {
  boost::cnv::strtol cnv1;
  boost::cnv::printf cnv2;

  string s1 =
      convert<string>(pair.first, cnv1(arg::precision = pair.second)).value();
  string s2 =
      convert<string>(pair.first, cnv2(arg::precision = pair.second)).value();
...
static void test_dbl_to_str() {
  ...
  double const dbls[] = {0.90, 1.0,  1.1,  0.94,  0.96,
                         1.04, 1.05, 1.06, 9.654, 999.888};
  ...
  BOOST_TEST("0" ==
             convert<string>(0.0, cnv::strtol()(arg::precision = 0)).value());
  BOOST_TEST("0.0" ==
             convert<string>(0.0, cnv::strtol()(arg::precision = 1)).value());
  BOOST_TEST("0.00" ==
             convert<string>(0.0, cnv::strtol()(arg::precision = 2)).value());
  BOOST_TEST("1" ==
             convert<string>(0.95, cnv::strtol()(arg::precision = 0)).value());
  BOOST_TEST("1.0" ==
             convert<string>(0.95, cnv::strtol()(arg::precision = 1)).value());
  BOOST_TEST("0.95" ==
             convert<string>(0.95, cnv::strtol()(arg::precision = 2)).value());

  for (int k = 0; k < num_tries; ++k) compare(get_random());
...
static void test_user_string() {
  //[strtol_user_string
  boost::cnv::strtol cnv;

  BOOST_TEST("12" == convert<my_string>(12, cnv).value());
  BOOST_TEST("0.95" ==
             convert<my_string>(0.95, cnv(arg::precision = 2)).value());
  //]
}

static void test_user_type() {
  //[strtol_user_type
  boost::cnv::strtol cnv;
