...
static void test_user_type() {
  ...
  //  string const           up_dir0_str = boost::convert<string,
  //  direction>(direction::up, cnv).value(); string const           dn_dir0_str
  //  = boost::convert<string, direction>(direction::dn, cnv).value();
  ...
  direction const dn_dir4 =
      boost::convert<direction>("junk", cnv).value_or(direction::dn);
...
static void test_algorithms() {
  ...
  std::array<change::value_type, 3> chgs2 = {
      {change::no, change::up, change::dn}};
