...
struct X {
  ...
  X(std::string const& str_) : str(str_) {}
};
...
int main(int argc, char* argv[]) {
  try {
    ...
    desc.add_options()("help", "help message")(
        "bind,b", boost::program_options::value<bool>(&bind),
        "bind thread to CPU")(
        "jobs,j", boost::program_options::value<boost::uint64_t>(&jobs),
        "jobs to run");
    ...
    std::cout << "overhead " << overhead_c.count() << " nano seconds"
              << std::endl;
