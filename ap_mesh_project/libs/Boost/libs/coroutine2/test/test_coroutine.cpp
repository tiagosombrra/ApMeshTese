...
class copyable {
 public:
  ...
  copyable() : state(false) {}

  copyable(int) : state(true) {}

  void operator()(coro::coroutine<int>::push_type&) { value3 = state; }
};
...
class moveable {
 public:
  ...
  moveable() : state(false) {}

  moveable(int) : state(true) {}

  moveable(moveable const&) = delete;
  moveable& operator=(moveable const&) = delete;

  moveable(moveable&& other) : state(false) { std::swap(state, other.state); }
...
void f2(coro::coroutine<void>::push_type&) { ++value1; }
...
void f6(coro::coroutine<int>::pull_type& c) { value1 = c.get(); }

void f7(coro::coroutine<std::string>::pull_type& c) { value2 = c.get(); }
...
void f9(coro::coroutine<int*>::pull_type& c) { value5 = c.get(); }
...
void test_unwind() {
  ...
  int i = 0;
  {
    coro::coroutine<void>::push_type coro(
        [&i](coro::coroutine<void>::pull_type&) mutable { i = 7; });
  }
  {
    BOOST_CHECK_EQUAL((int)0, value1);
...
void test_input_iterator() {
  {
    ...
    for (coro::coroutine<int>::pull_type::iterator i = std::begin(coro); i != e;
         ++i) {
      vec.push_back(*i);
    }
...
void test_output_iterator() {
  ...
  for (coro::coroutine<int>::push_type::iterator i(begin(coro)); i != e; ++i) {
    i = ++counter;
  }
...
void start_child_coroutine() {
  child = new coro::coroutine<void>::pull_type(
...
coro::coroutine<void>::pull_type start_parent_coroutine() {
  return coro::coroutine<void>::pull_type(
      [=](coro::coroutine<void>::push_type& yield) {
        vec.push_back(1);
