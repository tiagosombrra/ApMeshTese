...
class scoped_temp_file {
 public:
  scoped_temp_file() : m_path(boost::filesystem::unique_path()) {}

  ~scoped_temp_file() { boost::filesystem::remove(m_path); }
...
struct bitset_test {
  ...
  static void from_unsigned_long(NumBits num_bits, Value num) {
    ...
    size_type m = sz;
    if (ulong_width < sz) m = ulong_width;

    size_type i = 0;
    for (; i < m; ++i)
      BOOST_TEST(b.test(i) == nth_bit(static_cast<unsigned long>(num), i));
    for (; i < sz; ++i) BOOST_TEST(b.test(i) == 0);
  }
  ...
  static void from_string(const std::basic_string<Ch, Tr, Al>& str,
    ...
    std::size_t j;
    for (j = 0; j < m; ++j) BOOST_TEST(b[j] == (str[pos + m - 1 - j] == '1'));
    // If M < N, remaining bit positions are zero
    for (; j < actual_size; ++j) BOOST_TEST(b[j] == 0);
  }
  ...
  static void max_size(const Bitset& b) { BOOST_TEST(b.max_size() > 0); }
  ...
  static void swap(const Bitset& lhs, const Bitset& rhs) {
    ...
    for (typename Bitset::size_type i = 0; i < lhs.size(); ++i) {
      ...
      bool x = ref;
      if (i < b2.size()) b2[i] = !x;  // make sure b2[i] is different
      b1.swap(b2);
  ...
  static void resize(const Bitset& lhs) {
    ...
    std::size_t i;
    for (i = 0; i < lhs.size(); ++i) BOOST_TEST(b[i] == lhs[i]);
    for (; i < b.size(); ++i) BOOST_TEST(b[i] == true);
  ...
  static void pop_back(const Bitset& lhs) {
    ...
    for (std::size_t i = 0; i < b.size(); ++i) BOOST_TEST(b[i] == lhs[i]);

    b.pop_back();
    BOOST_TEST(b.size() == lhs.size() - 2);
    for (std::size_t j = 0; j < b.size(); ++j) BOOST_TEST(b[j] == lhs[j]);
  }
  ...
  static void append_bit(const Bitset& lhs) {
    ...
    for (std::size_t i = 0; i < lhs.size(); ++i) BOOST_TEST(b[i] == lhs[i]);

    b.push_back(false);
    BOOST_TEST(b.size() == lhs.size() + 2);
    BOOST_TEST(b[b.size() - 1] == false);
    BOOST_TEST(b[b.size() - 2] == true);
    for (std::size_t j = 0; j < lhs.size(); ++j) BOOST_TEST(b[j] == lhs[j]);
  }
  ...
  static void append_block_range(const Bitset& lhs,
  ...
  // PRE: b[i] == bit_vec[i]
  static void operator_bracket(const Bitset& lhs,
  ...
  static void set_all(const Bitset& b) {
    Bitset lhs(b);
    lhs.set();
    for (std::size_t I = 0; I < lhs.size(); ++I) BOOST_TEST(lhs[I] == 1);
  }
  ...
  static void set_one(const Bitset& b, std::size_t pos, bool value) {
    ...
    if (pos < N) {
      ...
      for (std::size_t I = 0; I < N; ++I)
        if (I != pos) BOOST_TEST(lhs[I] == prev[I]);
    } else {
      // Not in range, doesn't satisfy precondition.
    }
  }

  static void set_segment(const Bitset& b, std::size_t pos, std::size_t len,
                          bool value) {
    Bitset lhs(b);
    std::size_t N = lhs.size();
    Bitset prev(lhs);
  ...
  static void reset_all(const Bitset& b) {
    ...
    lhs.reset();
    for (std::size_t I = 0; I < lhs.size(); ++I) BOOST_TEST(lhs[I] == 0);
  }
  ...
  static void reset_segment(const Bitset& b, std::size_t pos, std::size_t len) {
    Bitset lhs(b);
    std::size_t N = lhs.size();
    Bitset prev(lhs);
    lhs.reset(pos, len);
  ...
  static void flip_all(const Bitset& b) {
    ...
    for (std::size_t I = 0; I < N; ++I) BOOST_TEST(lhs[I] == !prev[I]);
  }
  ...
  static void flip_segment(const Bitset& b, std::size_t pos, std::size_t len) {
    Bitset lhs(b);
    std::size_t N = lhs.size();
    Bitset prev(lhs);
    lhs.flip(pos, len);
  ...
  static void intersects(const Bitset& a, const Bitset& b) {
    ...
    for (typename Bitset::size_type i = 0; i < m && !have_intersection; ++i)
      if (a[i] == true && b[i] == true) have_intersection = true;
  ...
  static void operator_equal(const Bitset& a, const Bitset& b) {
    if (a == b) {
      for (std::size_t I = 0; I < a.size(); ++I) BOOST_TEST(a[I] == b[I]);
    } else {
      if (a.size() == b.size()) {
        bool diff = false;
        for (std::size_t I = 0; I < a.size(); ++I)
  ...
  static bool less_than(const Bitset& a, const Bitset& b) {
    ...
    if (!bsize) {
      ...
      size_type leqsize(
          std::min BOOST_PREVENT_MACRO_SUBSTITUTION(asize, bsize));
      size_type I;
  ...
  static typename Bitset::size_type next_bit_on(
      const Bitset& b, typename Bitset::size_type prev) {
    // helper function for find_next()
    //

    if (b.none() == true || prev == Bitset::npos) return Bitset::npos;

    ++prev;

    if (prev >= b.size()) return Bitset::npos;
  ...
  // operator|
  static void operator_or(const Bitset& lhs, const Bitset& rhs) {
    Bitset x(lhs);
    BOOST_TEST((lhs | rhs) == (x |= rhs));
  }

  // operator&
  static void operator_and(const Bitset& lhs, const Bitset& rhs) {
    Bitset x(lhs);
    BOOST_TEST((lhs & rhs) == (x &= rhs));
  }

  // operator^
  static void operator_xor(const Bitset& lhs, const Bitset& rhs) {
    Bitset x(lhs);
    BOOST_TEST((lhs ^ rhs) == (x ^= rhs));
  }

  // operator-
  static void operator_sub(const Bitset& lhs, const Bitset& rhs) {
    Bitset x(lhs);
    BOOST_TEST((lhs - rhs) == (x -= rhs));
  }
  ...
  template <typename Stream>
  static void stream_inserter(const Bitset& b, Stream& s,
  ...
  static void stream_extractor(Bitset& b, Stream& is, String& str) {
    ...
    if (!stream_was_good) {
      ...
      for (; pos < len && (pos - after_spaces) < max_digits; ++pos) {
        if (!is_one_or_zero(is, str[pos])) break;
      }
      after_digits = pos;
