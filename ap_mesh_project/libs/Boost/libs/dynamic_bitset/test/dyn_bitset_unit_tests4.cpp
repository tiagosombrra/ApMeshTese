...
void run_test_cases(BOOST_EXPLICIT_TEMPLATE_TYPE(Block)) {
  ...
  // Test stream operator<<
  {
    ...
    // exception mask, the width, the fill char and the padding side
    // (left/right)
    ...
    for (std::size_t mi = 0; mi < num_masks; ++mi) {
      for (std::size_t si = 0; si < num_strings; ++si) {
        ...
        for (std::size_t ci = 0; ci < num_chars; ++ci) {
          ...
          const std::streamsize widths[] = {-1 - slen / 2, 0, slen / 2,
                                            1 + slen * 2};
  ...
  // Test stream operator>>
  {
    ...
    // used exactly because they are the ones which will most likely give
    // problems on some systems (for instance '\f' could actually be written as
    // a sequence of new-lines, and we could never be able to read it back)
    //
