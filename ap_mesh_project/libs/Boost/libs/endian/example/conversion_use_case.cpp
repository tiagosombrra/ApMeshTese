...
//  This program reads a binary file of fixed length records, with a format
//  defined in a header file supplied by a third-party. The records inserted
//  into a vector, and the vector is sorted. The sorted records are then written
//  to an output file.
...
int main() {
  ...
  if (!in) {
    std::cout << "Could not open data.bin\n";
    return 1;
  }

  std::ofstream out("sorted-data.bin", std::ios::binary);
  if (!out) {
    std::cout << "Could not open sorted-data.bin\n";
    return 1;
  }
  ...
  while (!in.eof())  // read each record
  {
    ...
    rec.balance =
        boost::endian::big_to_native(rec.balance);  // reverse if needed
    recs.push_back(rec);
  }
  ...
  {
    out_rec.balance =
        boost::endian::native_to_big(out_rec.balance);  // reverse if needed
