...
void process_command_line(int argc, char* argv[]) {
  ...
  if (argc < 2) {
    cout << "Usage: benchmark n [Options]\n"
            ...
            "  Options:\n"
            "   -v       Verbose messages\n"
            "   -p#      Decimal places for times; default -p"
         << places << "\n";
    return std::exit(1);
  }
}
...
inline int32_t two_operand(int32_t x, int32_t& y) {
  return y = ((x << 24) & 0xff000000) | ((x << 8) & 0x00ff0000) |
...
inline void non_modify_assign(int32_t x, int32_t& y) { y = x; }

inline void non_modify_two_operand(int32_t x, int32_t& y) { two_operand(x, y); }

inline void non_modify_by_return(int32_t x, int32_t& y) { y = by_return(x); }
...
int main(int argc, char* argv[]) {
  ...
  benchmark(modify_by_return_intrinsic,
            "modify by return_intrinsic" /*, overhead*/);
#else
