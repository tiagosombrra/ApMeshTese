...
using namespace boost::endian;  //   namespaces works. See
...
#define VERIFY_VALUE_AND_OPS(endian_t, expected_t, expected) \
  verify_value_and_ops<endian_t, expected_t>(expected, __LINE__)
#define VERIFY_BIG_REPRESENTATION(t) verify_representation<t>(true, __LINE__)
#define VERIFY_LITTLE_REPRESENTATION(t) \
  verify_representation<t>(false, __LINE__)
#define VERIFY_NATIVE_REPRESENTATION(t) \
  verify_native_representation<t>(__LINE__)
...
void verify_representation(bool is_big, int line) {
  int silence = 0;
  Endian x(static_cast<typename Endian::value_type>(0x123456789abcdef0LL +
                                                    silence));  // will truncate

  if (is_big)
    verify(
        memcmp(&x, reinterpret_cast<const char *>(big_rep) + 8 - sizeof(Endian),
               sizeof(Endian)) == 0,
        line);
  else
...
void detect_order() {
  ...
  if (memcmp(v.c, "\x8\7\6\5\4\3\2\1", 8) == 0) {
    ...
    if (order::native != order::little) {
      cout << "yet boost::endian::order::native does not equal "
              "boost::endian::order::little.\n"
              "The Boost Endian library must be revised to work correctly on "
              "this system.\n"
    ...
    cout << "This machine is neither strict big-endian nor strict "
            "little-endian\n"
            "The Boost Endian library must be revised to work correctly on "
            "this system.\n"
            "Please report this problem to the Boost mailing list.\n";
...
void check_data() {
  ...
  VERIFY(little_16.data() ==
         reinterpret_cast<const unsigned char *>(&little_16));
  VERIFY(little_24.data() ==
         reinterpret_cast<const unsigned char *>(&little_24));
  VERIFY(little_32.data() ==
         reinterpret_cast<const unsigned char *>(&little_32));
  VERIFY(little_40.data() ==
         reinterpret_cast<const unsigned char *>(&little_40));
  VERIFY(little_48.data() ==
         reinterpret_cast<const unsigned char *>(&little_48));
  VERIFY(little_56.data() ==
         reinterpret_cast<const unsigned char *>(&little_56));
  VERIFY(little_64.data() ==
         reinterpret_cast<const unsigned char *>(&little_64));

  VERIFY(little_u8.data() ==
         reinterpret_cast<const unsigned char *>(&little_u8));
  VERIFY(little_u16.data() ==
         reinterpret_cast<const unsigned char *>(&little_u16));
  VERIFY(little_u24.data() ==
         reinterpret_cast<const unsigned char *>(&little_u24));
  VERIFY(little_u32.data() ==
         reinterpret_cast<const unsigned char *>(&little_u32));
  VERIFY(little_u40.data() ==
         reinterpret_cast<const unsigned char *>(&little_u40));
  VERIFY(little_u48.data() ==
         reinterpret_cast<const unsigned char *>(&little_u48));
  VERIFY(little_u56.data() ==
         reinterpret_cast<const unsigned char *>(&little_u56));
  VERIFY(little_u64.data() ==
         reinterpret_cast<const unsigned char *>(&little_u64));

  VERIFY(native_8.data() == reinterpret_cast<const unsigned char *>(&native_8));
  VERIFY(native_16.data() ==
         reinterpret_cast<const unsigned char *>(&native_16));
  VERIFY(native_24.data() ==
         reinterpret_cast<const unsigned char *>(&native_24));
  VERIFY(native_32.data() ==
         reinterpret_cast<const unsigned char *>(&native_32));
  VERIFY(native_40.data() ==
         reinterpret_cast<const unsigned char *>(&native_40));
  VERIFY(native_48.data() ==
         reinterpret_cast<const unsigned char *>(&native_48));
  VERIFY(native_56.data() ==
         reinterpret_cast<const unsigned char *>(&native_56));
  VERIFY(native_64.data() ==
         reinterpret_cast<const unsigned char *>(&native_64));

  VERIFY(native_u8.data() ==
         reinterpret_cast<const unsigned char *>(&native_u8));
  VERIFY(native_u16.data() ==
         reinterpret_cast<const unsigned char *>(&native_u16));
  VERIFY(native_u24.data() ==
         reinterpret_cast<const unsigned char *>(&native_u24));
  VERIFY(native_u32.data() ==
         reinterpret_cast<const unsigned char *>(&native_u32));
  VERIFY(native_u40.data() ==
         reinterpret_cast<const unsigned char *>(&native_u40));
  VERIFY(native_u48.data() ==
         reinterpret_cast<const unsigned char *>(&native_u48));
  VERIFY(native_u56.data() ==
         reinterpret_cast<const unsigned char *>(&native_u56));
  VERIFY(native_u64.data() ==
         reinterpret_cast<const unsigned char *>(&native_u64));

  VERIFY(big_align_int16.data() ==
         reinterpret_cast<const unsigned char *>(&big_align_int16));
  VERIFY(big_align_int32.data() ==
         reinterpret_cast<const unsigned char *>(&big_align_int32));
  VERIFY(big_align_int64.data() ==
         reinterpret_cast<const unsigned char *>(&big_align_int64));

  VERIFY(big_align_uint16.data() ==
         reinterpret_cast<const unsigned char *>(&big_align_uint16));
  VERIFY(big_align_uint32.data() ==
         reinterpret_cast<const unsigned char *>(&big_align_uint32));
  VERIFY(big_align_uint64.data() ==
         reinterpret_cast<const unsigned char *>(&big_align_uint64));

  VERIFY(little_align_int16.data() ==
         reinterpret_cast<const unsigned char *>(&little_align_int16));
  VERIFY(little_align_int32.data() ==
         reinterpret_cast<const unsigned char *>(&little_align_int32));
  VERIFY(little_align_int64.data() ==
         reinterpret_cast<const unsigned char *>(&little_align_int64));

  VERIFY(little_align_uint16.data() ==
         reinterpret_cast<const unsigned char *>(&little_align_uint16));
  VERIFY(little_align_uint32.data() ==
         reinterpret_cast<const unsigned char *>(&little_align_uint32));
  VERIFY(little_align_uint64.data() ==
         reinterpret_cast<const unsigned char *>(&little_align_uint64));
}
...
void check_alignment() {
  ...
  if (saved_err_count == err_count) {
    cout << "Size and alignment for structures of endian types are as "
            "expected.\n";
  }
}  // check_alignment
...
void check_representation_and_range_and_ops() {
  ...
  VERIFY_VALUE_AND_OPS(little_int64_t, int_least64_t,
  ...
  VERIFY_VALUE_AND_OPS(native_int64_t, int_least64_t,
                       -0x7fffffffffffffffLL - 1);
  ...
  VERIFY_VALUE_AND_OPS(little_int64_at, int_least64_t,
                       -0x7fffffffffffffffLL - 1);
...
/*
  ...
  void check_udt()
  {
    typedef boost::endian::endian_arithmetic< order::big, MyInt, 32 >
mybig_int32_ut;
...
//    cout << "v+v is " << +(v+v) << endl;
  ...
  void check_udt_le()
  {
    typedef boost::endian::endian_arithmetic< order::little, MyInt, 32 >
mylittle_int32_ut;
