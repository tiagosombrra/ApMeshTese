 ...
 *   behind the curtain...
 *****************************************************************************/
NonblockingAPI::NonblockingAPI() : injected_(0), tries_(0), chunksize_(9999) {}
...
void NonblockingAPI::inject_error(int ec) { injected_ = ec; }
...
class IncompleteRead : public std::runtime_error {
 public:
  IncompleteRead(std::string const& what, std::string const& partial, int ec)
      : std::runtime_error(what), partial_(partial), ec_(ec) {}

  std::string get_partial() const { return partial_; }

  int get_errorcode() const { return ec_; }
...
std::string read(NonblockingAPI& api, std::size_t desired) {
  ...
  msg << "NonblockingAPI::read() error " << ec << " after " << data.length()
