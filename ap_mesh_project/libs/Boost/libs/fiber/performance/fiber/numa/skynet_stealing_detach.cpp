...
void thread(std::uint32_t cpu_id, std::uint32_t node_id,
            std::vector<boost::fibers::numa::node> const& topo) {
  boost::fibers::use_scheduling_algorithm<
      boost::fibers::numa::algo::work_stealing>(cpu_id, node_id, topo);
  lock_type lk(mtx);
...
int main() {
  try {
    std::vector<boost::fibers::numa::node> topo =
    ...
    boost::fibers::use_scheduling_algorithm<
        boost::fibers::numa::algo::work_stealing>(main_cpu_id, node.id, topo);
    ...
    std::cout << "duration: " << duration.count() / 1000000 << " ms"
              << std::endl;
    return EXIT_SUCCESS;
