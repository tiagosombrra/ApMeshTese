...
class buffered_channel {
 ...
 private:
  ...
  bool is_full_() {
    ...
    return 0 >
           static_cast<std::intptr_t>(slots_[idx & (capacity_ - 1)].cycle.load(
  ...
  channel_op_status try_push_(ValueType&& value) {
    ...
    for (;;) {
      ...
      std::intptr_t diff{static_cast<std::intptr_t>(cycle) -
                         static_cast<std::intptr_t>(idx)};
      if (0 == diff) {
        if (producer_idx_.compare_exchange_weak(idx, idx + 1,
  ...
  channel_op_status try_value_pop_(slot*& s, std::size_t& idx) {
    ...
    for (;;) {
      ...
      std::intptr_t diff{static_cast<std::intptr_t>(cycle) -
                         static_cast<std::intptr_t>(idx + 1)};
      if (0 == diff) {
        if (consumer_idx_.compare_exchange_weak(idx, idx + 1,
                                                std::memory_order_relaxed)) {
          break;
        }
      } else if (0 > diff) {
  ...
  channel_op_status try_pop_(value_type& value) {
    ...
    if (channel_op_status::success == status) {
      value =
          std::move(*reinterpret_cast<value_type*>(std::addressof(s->storage)));
 ...
 public:
  explicit buffered_channel(std::size_t capacity) : capacity_{capacity} {
  ...
  value_type value_pop() {
    for (;;) {
      ...
      if (channel_op_status::success == status) {
        ...
        not_empty_cnd_.wait(lk,
                            [this]() { return is_closed() || !is_empty_(); });
        --waiting_consumer_;
      } else {
        BOOST_ASSERT(channel_op_status::closed == status);
        throw std::runtime_error{"boost fiber: channel is closed"};
