...
class main_context final : public context {
 public:
  main_context() noexcept : context{1, type::main_context, launch::post} {}
};

class dispatcher_context final : public context {
 private:
  boost::context::fiber run_(boost::context::fiber&& c) {
 ...
 public:
  dispatcher_context(boost::context::preallocated const& palloc,
                     default_stack&& salloc)
...
static intrusive_ptr<context> make_dispatcher_context() {
  ...
  void* storage = reinterpret_cast<void*>(
      (reinterpret_cast<uintptr_t>(sctx.sp) -
  ...
  return intrusive_ptr<context>{new (storage) dispatcher_context{
...
void context::suspend() noexcept { get_scheduler()->suspend(); }
...
bool context::wait_until(std::chrono::steady_clock::time_point const& tp,
                         detail::spinlock_lock& lk, waker&& w) noexcept {
  BOOST_ASSERT(nullptr != get_scheduler());
  BOOST_ASSERT(this == active());
...
bool context::wake(const size_t epoch) noexcept {
  ...
  bool is_last_waker = waker_epoch_.compare_exchange_strong(
      expected, epoch + 1, std::memory_order_acq_rel);
  if (!is_last_waker) {
...
void context::set_fss_data(
    void const* vp, detail::fss_cleanup_function::ptr_t const& cleanup_fn,
