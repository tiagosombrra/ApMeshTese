...
class procinfo_iterator {
 ...
 public:
  ...
  procinfo_iterator(LOGICAL_PROCESSOR_RELATIONSHIP relship) {
    ...
    if (BOOST_UNLIKELY(
            !::GetLogicalProcessorInformationEx(relship, buffer_, &length_))) {
      throw std::system_error{
          std::error_code{static_cast<int>(::GetLastError()),
                          std::system_category()},
          "::GetLogicalProcessorInformation() failed"};
  ...
  procinfo_iterator& operator++() noexcept {
    if (nullptr != procinfo_) {
      ...
      if (0 != length_) {
        procinfo_ = reinterpret_cast<SLPI*>(reinterpret_cast<BYTE*>(procinfo_) +
                                            procinfo_->Size);
      } else {
        LocalFree(buffer_);
        buffer_ = nullptr;
  ...
  SLPI* operator->() noexcept { return procinfo_; }

  bool operator==(procinfo_iterator const& other) const noexcept {
    return other.buffer_ == buffer_ && other.procinfo_ == procinfo_ &&
...
std::vector<node> topology() {
  ...
  for (procinfo_iterator i{RelationNumaNode}; i != e; ++i) {
    ...
    auto cpus = compute_cpu_set(i->NumaNode.GroupMask.Group,
                                i->NumaNode.GroupMask.Mask);
