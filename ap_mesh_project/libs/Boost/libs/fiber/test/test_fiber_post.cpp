...
class copyable {
 public:
  ...
  copyable() : state(false), value(-1) {}

  copyable(int v) : state(true), value(v) {}

  void operator()() { value1 = value; }
};
...
class moveable {
 public:
  ...
  moveable() : state(false), value(-1) {}

  moveable(int v) : state(true), value(v) {}

  moveable(moveable&& other) : state(other.state), value(other.value) {
    other.state = false;
    other.value = -1;
  }
...
class detachable {
 ...
 public:
  ...
  detachable() : alive_count_(1) { ++alive_count; }

  detachable(detachable const& g) : alive_count_(g.alive_count_) {
    ++alive_count;
  }
...
void fn4() { boost::this_fiber::yield(); }
...
void test_join_lambda() {
  {
    ...
    int i = 3;
    std::string abc("abc");
    boost::fibers::fiber f(boost::fibers::launch::post, [i, abc]() {
      value1 = i;
      value2 = abc;
    });
    f.join();
...
void test_join_bind() {
  {
    ...
    int i = 3;
    std::string abc("abc");
    boost::fibers::fiber f(boost::fibers::launch::post, std::bind([i, abc]() {
                             value1 = i;
                             value2 = abc;
                           }));
    f.join();
  ...
  }
  {
    ...
    value2 = "";
    std::string abc("abc");
    boost::fibers::fiber f(boost::fibers::launch::post,
                           std::bind(
                               [](std::string& str) {
                                 value1 = 3;
                                 value2 = str;
...
void test_sleep_for() {
  ...
  // This test is spurious as it depends on the time the fiber system switches
  // the fiber
  BOOST_CHECK((std::max)(ns.count(), -ns.count()) <
              (err + std::chrono::milliseconds(1000)).count());
}
...
void do_wait(boost::fibers::barrier* b) { b->wait(); }
