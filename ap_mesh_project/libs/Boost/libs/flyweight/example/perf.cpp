...
class count_allocator {
 public:
  ...
  template <class U>
  count_allocator(const count_allocator<U>&, int = 0) {}
...
typedef std::basic_string<char, std::char_traits<char>, count_allocator<char> >
    count_string;

typedef hashed_factory<boost::hash<boost::mpl::_2>,
                       std::equal_to<boost::mpl::_2>,
                       count_allocator<boost::mpl::_1> >
    count_hashed_factory;

typedef set_factory<std::less<boost::mpl::_2>, count_allocator<boost::mpl::_1> >
    count_set_factory;
...
class timer {
 public:
  ...
  void time(const char* str) {
    std::cout << str << ": " << (double)(std::clock() - t) / CLOCKS_PER_SEC
              << " s\n";
  }
...
template <typename T>
struct is_flyweight : boost::mpl::false_ {};
...
struct test {
  static std::size_t run(const std::string& file) {
    ...
    typedef boost::tokenizer<boost::char_separator<char>, char_iterator>
        tokenizer;
    ...
    tokenizer tok =
        tokenizer(char_iterator(ifs), char_iterator(),
    ...
    if (is_flyweight<String>::value) {
      std::size_t flyweight_mem =
          (txt.capacity() + txt2.capacity()) * sizeof(String);
      std::cout << "= flyweights(" << flyweight_mem << ")+values("
                << count_allocator_mem - flyweight_mem << ")";
    }
    std::cout << "\n";
...
test_case test_table[] = {
    {"simple string", test<count_string>::run},
