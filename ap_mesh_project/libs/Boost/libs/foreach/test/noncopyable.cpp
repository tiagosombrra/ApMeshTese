...
struct noncopy_vector : std::vector<int>, private boost::noncopyable {
  noncopy_vector() {}
};

struct noncopy_range : boost::iterator_range<noncopy_vector::iterator>,
...
inline boost::mpl::true_ *boost_foreach_is_noncopyable(noncopy_vector *&,
                                                       boost::foreach::tag) {
  return 0;
}

inline boost::mpl::true_ *boost_foreach_is_noncopyable(noncopy_range *&,
                                                       boost::foreach::tag) {
  return 0;
}
#endif
...
inline boost::mpl::true_ *boost_foreach_is_lightweight_proxy(
    noncopy_range *&, boost::foreach::tag) {
  return 0;
}
...
int main(int, char *[]) {
  ...
  BOOST_FOREACH (int const &j, v2) {
    (void)j;
  }

  noncopy_range rng1;
  BOOST_FOREACH (int &k, rng1) {
    (void)k;
  }

  noncopy_range const rng2;
  BOOST_FOREACH (int &l, rng2) {
    (void)l;
  }
