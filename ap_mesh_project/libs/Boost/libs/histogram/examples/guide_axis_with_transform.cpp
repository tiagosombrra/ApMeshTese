...
int main() {
  ...
  // make a regular axis with a log transform over [10, 100), [100, 1000),
  // [1000, 10000)
  axis::regular<double, axis::transform::log> r_log{3, 10., 10000.};
  // log transform:
  // - useful when values vary dramatically in magnitude, like brightness of
  // stars
  // - edges are not exactly at 10, 100, 1000, because of finite floating point
  // precision
  // - values >= 0 but smaller than the starting value of the axis are mapped to
  // -1
  // - values < 0 are mapped to `size()`, because the result of std::log(value)
  // is NaN
  ...
  // - values < 0 are mapped to `size()`, because the result of std::sqrt(value)
  // is NaN
  ...
  // make a regular axis with a power transform x^1/3 over [1, 8), [8, 27), [27,
  // 64)
