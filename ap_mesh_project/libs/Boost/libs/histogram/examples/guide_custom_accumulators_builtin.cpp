...
int main() {
  ...
  // The factory function `make_profile` is provided as a shorthand for this, so
  // this is equivalent to the previous line: auto h =
  // make_profile(axis::integer<>(0, 2));
  ...
  for (auto&& x : indexed(h)) {
    ...
    os << boost::format("index %i count %i mean %.1f variance %.1f\n") %
              x.index() % x->count() % x->value() % x->variance();
  }
  std::cout << os.str() << std::flush;
  assert(os.str() ==
         "index 0 count 2 mean 1.5 variance 0.5\n"
