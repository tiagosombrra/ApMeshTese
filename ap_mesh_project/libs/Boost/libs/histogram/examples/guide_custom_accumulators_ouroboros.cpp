...
int main() {
  ...
  // We only need to implement operator(). A matching version of operator() is
  // actually present in base_t, but it is templated and this is not allowed by
  // the accumulator concept. Initialization could also happen here. We don't
  // need to initialize anything here, because the default constructor of base_t
  // is called automatically and sufficient for this example.
