...
int main() {
  ...
  // make a 2d histogram
  auto h = make_histogram(axis::regular<>(4, 0.0, 4.0),
                          axis::regular<>(4, -2.0, 2.0));
  ...
  // reduce does not change the total count if the histogram has
  // underflow/overflow bins
  assert(algorithm::sum(h) == 4 && algorithm::sum(h2) == 4);

  // One can also explicitly specify the index of the axis in the histogram on
  // which the command should act, by using this index as the the first
  // parameter. The position of the command in the argument list of reduce is
  // then ignored. We use this to slice only the second axis (axis has index 1
  // in the histogram) from bin index 2 to 4.
