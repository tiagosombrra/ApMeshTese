...
int main() {
  ...
  // use std::fill to set all counters to 0.25 (except under- and overflow
  // counters)
  ...
  auto b = std::any_of(ind.begin(), ind.end(),
  ...
  // computing cosine similiarity: cos(theta) = A dot B / sqrt((A dot A) * (B
  // dot B))
  auto ind2 = indexed(h2);
  const auto aa = std::inner_product(ind.begin(), ind.end(), ind.begin(), 0.0);
  const auto bb =
      std::inner_product(ind2.begin(), ind2.end(), ind2.begin(), 0.0);
