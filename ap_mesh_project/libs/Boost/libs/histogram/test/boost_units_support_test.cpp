...
int main() {
  ...
  // axis::regular with quantity
  {
    ...
    BOOST_TEST_EQ(b.bin(2).upper() / si::meter,
                  std::numeric_limits<double>::infinity());

    BOOST_TEST_EQ(b.index(-std::numeric_limits<double>::infinity() * si::meter),
                  -1);
  ...
  // axis::regular with quantity and transform
  {
    ...
    BOOST_TEST_EQ(b.value(3) / si::meter,
                  std::numeric_limits<double>::infinity());
  }
  ...
  // histogram with quantity axis
  {
    ...
    for (auto&& x : indexed(h)) {
      BOOST_TEST_THROWS(x.density(),
                        std::runtime_error);  // cannot use density method
