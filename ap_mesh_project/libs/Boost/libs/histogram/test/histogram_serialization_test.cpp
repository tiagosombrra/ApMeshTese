...
void run_tests(const std::string& filename) {
  ...
  auto a = make(
      Tag(), axis::regular<double, def, def, none_t>(1, -1, 1, "reg"),
      axis::circular<float, def, none_t>(1, 0.0, 1.0, "cir"),
      axis::regular<double, tr::log, def, none_t>(1, 1, std::exp(2), "reg-log"),
      axis::regular<double, tr::pow, std::vector<int>,
                    axis::option::overflow_t>(tr::pow(0.5), 1, 1, 100,
                                              {1, 2, 3}),
...
int main(int argc, char** argv) {
  ...
  run_tests<static_tag>(
      join(argv[1], "histogram_serialization_test_static.xml"));
  run_tests<dynamic_tag>(
      join(argv[1], "histogram_serialization_test_dynamic.xml"));
