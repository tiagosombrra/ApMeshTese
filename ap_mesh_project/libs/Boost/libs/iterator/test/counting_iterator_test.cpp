...
#pragma warn - 8091    // template argument ForwardIterator passed to '...' is a
                       // output iterator
#pragma warn - 8071    // Conversion may lose significant digits (due to
                       // counting_iterator<char> += n).
#endif
...
struct assert_nonnegative
    : boost::mpl::if_c<std::numeric_limits<T>::is_signed,
...
void category_test(CountingIterator start, CountingIterator finish, Value,
  ...
  typedef typename std::iterator_traits<CountingIterator>::difference_type
      difference_type;
...
template <class CountingIterator, class Value>
void category_test(CountingIterator start, Value v1,
...
void test_aux(CountingIterator start, CountingIterator finish, Value v1) {
  ...
  // Okay, brute force...
  for (CountingIterator p = start; p != finish && boost::next(p) != finish;
       ++p) {
...
void test(Incrementable start, Incrementable finish) {
  test_aux(boost::make_counting_iterator(start),
           boost::make_counting_iterator(finish), start);
}
...
void test_integer3(Integer* = 0, Category* = 0,
                   Difference* = 0)  // default arg works around MSVC bug
{
  Integer start = 0;
  Integer finish = 120;
...
int main() {
  ...
  // Some tests on container iterators, to prove we handle a few different
  // categories
