...
// Force boost::result_of use decltype, even on compilers that don't support
// N3276. This enables this test to also verify if the iterator works with
// lambdas on such compilers with this config macro. Note that without the macro
// result_of (and consequently the iterator) is guaranteed to _not_ work, so
// this case is not worth testing anyway.
...
int main() {
  ...
  copy(boost::make_function_input_iterator(ones_generator, 0),
       boost::make_function_input_iterator(ones_generator, 10),
       back_inserter(generated));

  BOOST_TEST_ALL_EQ(values.begin(), values.end(), generated.begin(),
                    generated.end());

  // test the iterator with normal functions
  vector<int>().swap(generated);
  copy(boost::make_function_input_iterator(&ones_function, 0),
  ...
  vector<int>().swap(generated);
  copy(boost::make_function_input_iterator(counter_generator, 0),
  ...
  vector<int>().swap(generated);
  copy(boost::make_function_input_iterator(lambda_generator, 0),
