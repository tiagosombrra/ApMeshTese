...
template <class V>
struct ptr_iterator : boost::iterator_adaptor<ptr_iterator<V>, V*, V,
                                              ...
                                              > {
 private:
  typedef boost::iterator_adaptor<ptr_iterator<V>, V*, V,
                                  boost::random_access_traversal_tag
#if BOOST_WORKAROUND(BOOST_BORLANDC, BOOST_TESTED_AT(0x551))
                                  ,
                                  V&
...
struct modify_traversal
    : boost::iterator_adaptor<modify_traversal<Iter, Traversal>, Iter,
                              boost::use_default, Traversal> {};

template <class T>
struct fwd_iterator
    : boost::iterator_adaptor<fwd_iterator<T>,
                              boost::forward_iterator_archetype<T> > {
 private:
  typedef boost::iterator_adaptor<fwd_iterator<T>,
                                  boost::forward_iterator_archetype<T> >
      super_t;
...
template <class T>
struct in_iterator
    : boost::iterator_adaptor<in_iterator<T>,
                              boost::input_iterator_archetype_no_proxy<T> > {
 private:
  typedef boost::iterator_adaptor<in_iterator<T>,
                                  boost::input_iterator_archetype_no_proxy<T> >
      super_t;
...
int static_assert_traversal(Iter* = 0, Trav* = 0) {
  typedef typename boost::iterator_category_to_traversal<
      BOOST_DEDUCED_TYPENAME Iter::iterator_category>::type t2;
...
int main() {
...
#if !BOOST_WORKAROUND(__MWERKS__, <= 0x2407)
    BOOST_STATIC_ASSERT(
        (boost::is_convertible<Iter1::iterator_category,
...
#if !BOOST_WORKAROUND(BOOST_BORLANDC, \
                      BOOST_TESTED_AT(0x564))  // borland drops constness
...

  {
    ...
    static_assert_traversal<IncrementableIter,
  ...
  // check that base_type is correct
  {
    ...
    test = static_assert_same<constant_iterator<BaseIter>::base_type,
                              BaseIter>::value;
