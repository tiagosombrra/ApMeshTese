...
namespace detail {
template <>
struct function_object_result<int (*)(int)> {
  typedef int type;
};
}  // namespace detail
}  // namespace boost
...
int main() {
  ...
  // Concept checks
  {
    ...
    typedef boost::transform_iterator<adaptable_mult_functor, int const*>
        c_iter_t;

    boost::function_requires<
        boost_concepts::InteroperableIteratorConcept<iter_t, c_iter_t> >();
  }
  ...
  // Test transform_iterator
  {
    ...
    boost::input_iterator_test(iter_t(&y[0], adaptable_mult_functor(2)), x[0],
                               x[1]);
  ...
  {{typedef boost::transform_iterator<adaptable_mult_functor, int*, float>
        iter_t;
...
{
  typedef boost::transform_iterator<adaptable_mult_functor, int*,
                                    boost::use_default, float>
      iter_t;
...
{
  typedef boost::transform_iterator<adaptable_mult_functor, int*, float, double>
      iter_t;
  BOOST_STATIC_ASSERT((boost::is_same<iter_t::reference, float>::value));
...
// Test transform_iterator with function pointers
{
  ...
  boost::input_iterator_test(boost::make_transform_iterator(y, mult_2), x[0],
                             x[1]);
...
// Test transform_iterator as projection iterator
{
  ...
  boost::random_access_readable_iterator_test(
      boost::make_transform_iterator((pair_t*)values, value_select_first()), N,
      x);

  boost::random_access_readable_iterator_test(
      boost::make_transform_iterator((pair_t*)values, const_select_first()), N,
      x);
  ...
  boost::non_const_lvalue_iterator_test(
      boost::make_transform_iterator((pair_t*)values, select_first()), x[0],
      17);
...
// Test transform_iterator with polymorphic object function
{
  ...
  boost::input_iterator_test(
      boost::make_transform_iterator(y, polymorphic_mult_functor()), x[0],
      x[1]);

  boost::input_iterator_test(
      boost::make_transform_iterator(&y[0], polymorphic_mult_functor()), x[0],
      x[1]);
