...
static inline void test_ss_noconstr(StringStreamT& ss,
                                    const boost::array<CharT, N>& in_val) {
  OutT out_val;
...
struct structure_sprintf {
  ...
  static inline void test(BufferT* buffer, const InT& in_val,
  ...
  static inline void test(BufferT* buffer, const std::string& in_val,
                          const char* const conv) {
...
struct structure_sscanf {
  ...
  static inline void test(BufferT* /*buffer*/,
                          const boost::array<CharT, N>& in_val,
                          const char* const conv) {
    OutT out_val;
  ...
  template <class OutT, class BufferT>
  static inline void test(BufferT* /*buffer*/,
                          const boost::iterator_range<const char*>& in_val,
                          const char* const conv) {
    OutT out_val;
...
static inline void min_fancy_output(T v1, T v2, T v3, T v4) {
  ...
  if (!v4)
    fout << "<1";
  else if (v4 == fake_test_value)
    fout << "---";
  else
    fout << v4;

  fout << (res == 4 ? end_mark : no_mark) << " ]";
}
...
static inline void perf_test_impl(const FromT& in_val, const char* const conv) {
  ...
  lexical_cast_time =
      boost::chrono::duration_cast<duration_t>(test_clock::now() - start);
  ...
  min_fancy_output(lexical_cast_time.count(), ss_constr_time.count(),
                   ss_noconstr_time.count(),
                   boost::is_same<SprintfT, structure_fake>::value
                       ? fake_test_value
                       : printf_time.count());
}

template <class ToT, class SprintfT, class FromT>
static inline void perf_test(const std::string& test_name, const FromT& in_val,
                             const char* const conv) {
...
void string_like_test_set(const std::string& from) {
  ...
  perf_test<boost::long_long_type, ssc_t>(from + "->long long", conv("100"),
                                          "%lld");

  perf_test<unsigned int, ssc_t>(from + "->unsigned int", conv("100"), "%u");
  perf_test<unsigned short, ssc_t>(from + "->unsigned short", conv("100"),
                                   "%hu");
  perf_test<unsigned long int, ssc_t>(from + "->unsigned long int", conv("100"),
                                      "%lu");
  perf_test<boost::ulong_long_type, ssc_t>(from + "->unsigned long long",
                                           conv("100"), "%llu");
  ...
  perf_test<boost::array<char, 50>, ssc_t>(from + "->array<char, 50>",
                                           conv("1.123"), "%s");

  perf_test<std::string, structure_fake>(from + "->string", conv("string"),
                                         "%Lf");
  perf_test<boost::container::string, structure_fake>(
      from + "->container::string", conv("string"), "%Lf");
}

struct to_string_conv {
  std::string operator()(const char* const c) const { return c; }
};

struct to_char_conv {
  const char* operator()(const char* const c) const { return c; }
};
...
int main(int argc, char** argv) {
  ...
  perf_test<std::string, structure_sprintf>(
  ...
  perf_test<std::string, structure_sprintf>("short->string",
                                            static_cast<short>(100), "%hd");
  ...
  perf_test<std::string, structure_sprintf>("unsigned short->string", 100u,
                                            "%hu");
  perf_test<std::string, structure_sprintf>("unsigned long int->string", 100ul,
                                            "%lu");
  perf_test<std::string, structure_sprintf>(
      "unsigned long long->string", static_cast<boost::ulong_long_type>(100),
      "%llu");
  ...
  perf_test<std::string, structure_sprintf>("long double->string", 1.123L,
                                            "%Lf");
