...
std::basic_string<CharT> to_str_gcc_workaround(std::basic_string<CharT> str) {
  ...
  if (np.grouping().empty()) return str;

  CharT prefix[3] = {ct.widen('-'), np.thousands_sep(), CharT()};

  if (str.find(prefix) != 0) return str;
...
void test_conversion_from_to_float_for_locale() {
  ...
  if (!np.grouping().empty()) {
    BOOST_CHECK_THROW(lexical_cast<T>(std::string("100") + np.thousands_sep() +
                                      np.thousands_sep() + "0"),
                      bad_lexical_cast);
    BOOST_CHECK_THROW(lexical_cast<T>(std::string("100") + np.thousands_sep()),
                      bad_lexical_cast);
    BOOST_CHECK_THROW(lexical_cast<T>(np.thousands_sep() + std::string("100")),
                      bad_lexical_cast);
    BOOST_CHECK_THROW(lexical_cast<T>(std::string("1") + np.thousands_sep() +
                                      np.decimal_point() + "e10"),
                      bad_lexical_cast);
    BOOST_CHECK_THROW(lexical_cast<T>(std::string("1e10") + np.thousands_sep()),
                      bad_lexical_cast);
    BOOST_CHECK_THROW(
        lexical_cast<T>(std::string("1") + np.thousands_sep() + "e10"),
        bad_lexical_cast);

    BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(to_str<char>(100000)), 100000,
                               (boost::math::tools::epsilon<T>()));
    BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(to_str<char>(10000000u)),
                               10000000u, (boost::math::tools::epsilon<T>()));
    BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(to_str<char>(100)), 100,
                               (boost::math::tools::epsilon<T>()));
#if !defined(BOOST_LCAST_NO_WCHAR_T) && !defined(BOOST_NO_INTRINSIC_WCHAR_T)
    BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(to_str<wchar_t>(100000)), 100000,
                               (boost::math::tools::epsilon<T>()));
    BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(to_str<wchar_t>(10000000u)),
                               10000000u, (boost::math::tools::epsilon<T>()));
    BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(to_str<wchar_t>(100)), 100,
                               (boost::math::tools::epsilon<T>()));
#endif
    // Exception must not be thrown, when we are using no separators at all
    BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>("30000"), static_cast<T>(30000),
                               (boost::math::tools::epsilon<T>()));
  }
}
...
void test_converion_to_float_types() {
  ...
  //    BOOST_CHECK_CLOSE_FRACTION(1e-35L,
  //    lexical_cast<test_t>("0.00000000000000000000000000000000001"),
  //    (boost::math::tools::epsilon<test_t>()) );
  BOOST_CHECK_CLOSE_FRACTION(
  ...
  BOOST_CHECK_THROW(lexical_cast<test_t>(".e"), bad_lexical_cast);
  BOOST_CHECK_THROW(lexical_cast<test_t>(".111111111111111111111111111111111111"
                                         "11111111111111111111111111111111ee"),
                    bad_lexical_cast);
  BOOST_CHECK_THROW(lexical_cast<test_t>(".111111111111111111111111111111111111"
...
void test_float_typess_for_overflows() {
  ...
  BOOST_CHECK_CLOSE_FRACTION(minvalue, lexical_cast<test_t>(minvalue),
                             (boost::math::tools::epsilon<test_t>()));
  BOOST_CHECK_CLOSE_FRACTION(minvalue, lexical_cast<test_t>(s_min_value),
  ...
  BOOST_CHECK_CLOSE_FRACTION(maxvalue, lexical_cast<test_t>(maxvalue),
                             (boost::math::tools::epsilon<test_t>()));
  BOOST_CHECK_CLOSE_FRACTION(maxvalue, lexical_cast<test_t>(s_max_value),
                             (boost::math::tools::epsilon<test_t>()));
  ...
  // VC9 can fail the following tests on floats and doubles when using
  // stingstream...
...
#define TEST_TO_FROM_CAST_AROUND_TYPED(VAL, STRING_TYPE)                       \
  ...
  BOOST_CHECK_CLOSE_FRACTION(test_value, converted_val,                        \
                             boost::math::tools::epsilon<test_t>() * 2);

/*
 * For interval [ from_mult*epsilon+VAL, to_mult*epsilon+VAL ], converts float
 * type numbers to string[wstring] and then back to float type, then compares
 * initial values and generated. Step is epsilon
 */
#ifndef BOOST_LCAST_NO_WCHAR_T
...
void test_converion_from_to_float_types() {
  ...
  long double val1;
  for (val1 = 1.0e-10L; val1 < 1e11; val1 *= 10) TEST_TO_FROM_CAST_AROUND(val1);
...
void test_conversion_from_char_to_float(CharT zero) {
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 0)),
                             static_cast<T>(0),
                             (boost::math::tools::epsilon<T>()));
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 1)),
                             static_cast<T>(1),
                             (boost::math::tools::epsilon<T>()));
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 2)),
                             static_cast<T>(2),
                             (boost::math::tools::epsilon<T>()));
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 3)),
                             static_cast<T>(3),
                             (boost::math::tools::epsilon<T>()));
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 4)),
                             static_cast<T>(4),
                             (boost::math::tools::epsilon<T>()));
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 5)),
                             static_cast<T>(5),
                             (boost::math::tools::epsilon<T>()));
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 6)),
                             static_cast<T>(6),
                             (boost::math::tools::epsilon<T>()));
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 7)),
                             static_cast<T>(7),
                             (boost::math::tools::epsilon<T>()));
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 8)),
                             static_cast<T>(8),
                             (boost::math::tools::epsilon<T>()));
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>(static_cast<CharT>(zero + 9)),
                             static_cast<T>(9),
                             (boost::math::tools::epsilon<T>()));

  BOOST_CHECK_THROW(lexical_cast<T>(static_cast<CharT>(zero + 10)),
                    bad_lexical_cast);
  BOOST_CHECK_THROW(lexical_cast<T>(static_cast<CharT>(zero - 1)),
                    bad_lexical_cast);
}
...
void test_conversion_from_to_float() {
  ...
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>("+1"), 1,
                             boost::math::tools::epsilon<T>());
  BOOST_CHECK_CLOSE_FRACTION(lexical_cast<T>("+9"), 9,
                             boost::math::tools::epsilon<T>());
  ...
  if (grouping1 != grouping2) test_conversion_from_to_float_for_locale<T>();

  if (grouping1.empty() && grouping2.empty())
    BOOST_TEST_MESSAGE("Formatting with thousands_sep has not been tested");
}

void test_conversion_from_to_float() { test_conversion_from_to_float<float>(); }
void test_conversion_from_to_double() {
  test_conversion_from_to_float<double>();
}
void test_conversion_from_to_long_double() {
// We do not run tests on compilers and Standard Libraries with poor support of
// long double
