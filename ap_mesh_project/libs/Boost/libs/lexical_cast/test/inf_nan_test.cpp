...
bool is_neg_nan(T value) {
   ...
   * long double. It is a IA64 feature, or it is a boost::math feature, not a
   * lexical_cast bug */
#if defined(__ia64__) || defined(_M_IA64)
  return (boost::math::isnan)(value) &&
         (boost::is_same<T, long double>::value ||
          (boost::math::signbit)(value));
#else
  return (boost::math::isnan)(value) && (boost::math::signbit)(value);
#endif
}
...
void test_inf_nan_templated() {
  ...
  BOOST_CHECK(lexical_cast<std::string>((boost::math::changesign)(
  ...
  BOOST_CHECK(lexical_cast<std::wstring>((boost::math::changesign)(
                  std::numeric_limits<test_t>::infinity())) == L"-inf");
  BOOST_CHECK(lexical_cast<std::wstring>(
                  std::numeric_limits<test_t>::infinity()) == L"inf");
  BOOST_CHECK(lexical_cast<std::wstring>(
                  std::numeric_limits<test_t>::quiet_NaN()) == L"nan");
#if !defined(__ia64__) && !defined(_M_IA64)
  BOOST_CHECK(lexical_cast<std::wstring>((boost::math::changesign)(
                  std::numeric_limits<test_t>::quiet_NaN())) == L"-nan");
#endif
...
void test_inf_nan_float() { test_inf_nan_templated<float>(); }

void test_inf_nan_double() { test_inf_nan_templated<double>(); }
