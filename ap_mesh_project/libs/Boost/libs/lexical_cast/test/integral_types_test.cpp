...
//
// Note: The unit test no longer compile on MSVC 6, but lexical_cast itself
// works for it.
...
void test_conversion_from_char_to_integral(CharT zero) {
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 0)) ==
              static_cast<T>(0));
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 1)) ==
              static_cast<T>(1));
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 2)) ==
              static_cast<T>(2));
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 3)) ==
              static_cast<T>(3));
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 4)) ==
              static_cast<T>(4));
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 5)) ==
              static_cast<T>(5));
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 6)) ==
              static_cast<T>(6));
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 7)) ==
              static_cast<T>(7));
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 8)) ==
              static_cast<T>(8));
  BOOST_CHECK(lexical_cast<T>(static_cast<CharT>(zero + 9)) ==
              static_cast<T>(9));
...
void test_conversion_from_string_to_integral(CharT) {
  ...
  BOOST_CHECK_EQUAL(lexical_cast<T>(s), max_val);
  {
    ...
    if (BOOST_USE_FACET(numpunct, loc).grouping().empty()) {
      ...
      BOOST_CHECK_EQUAL(
          lexical_cast<T>(to_str<CharT>(0) + to_str<CharT>(0) + s), max_val);
      BOOST_CHECK_EQUAL(lexical_cast<T>(to_str<CharT>(0) + to_str<CharT>(0) +
                                        to_str<CharT>(0) + s),
                        max_val);
    }

    for (int i = 1; i <= 256; ++i) {
      BOOST_CHECK_THROW(lexical_cast<T>(to_str<CharT>(i) + s),
                        bad_lexical_cast);
    }
    ...
    if (!(boost::is_same<T, promoted>::value)) {
      ...
      s = to_str<CharT>(max_val);
      for (int i = 1; i <= 256; ++i) {
        BOOST_CHECK_THROW(lexical_cast<T>(to_str<CharT>(prom + i)),
                          bad_lexical_cast);
        BOOST_CHECK_THROW(lexical_cast<T>(to_str<CharT>(i) + s),
                          bad_lexical_cast);
      }
...
void test_conversion_from_to_integral_for_locale() {
  ...
  if (lexical_cast<std::wstring>(t) != to_str<wchar_t>(t)) {
    ...
    BOOST_CHECK_EQUAL(lcast_str.length(),
                      lexical_cast<std::wstring>(t).length());
...
void test_conversion_from_to_integral_minimal() {
  ...
  for (int i = 0; i < 15; ++i) {
    ...
    BOOST_CHECK_THROW(lexical_cast<T>(must_owerflow_negative_str),
                      bad_lexical_cast);
...
void test_conversion_from_to_integral() {
  ...
  if (grouping1 != grouping2) test_conversion_from_to_integral_for_locale<T>();
...
void test_conversion_from_to_int() { test_conversion_from_to_integral<int>(); }
...
struct test_if_specialized<true, T> {
  static void test() { test_conversion_from_to_integral_minimal<T>(); }
};

void test_conversion_from_to_int128() {
  test_if_specialized<std::numeric_limits<boost::int128_type>::is_specialized,
                      boost::int128_type>::test();
}

void test_conversion_from_to_uint128() {
  test_if_specialized<std::numeric_limits<boost::int128_type>::is_specialized,
                      boost::uint128_type>::test();
}
#endif
...
void test_integral_conversions_on_min_max_impl() {
  ...
  typedef BOOST_DEDUCED_TYPENAME boost::make_unsigned<signed_t>::type
      unsigned_t;

  typedef std::numeric_limits<signed_t> s_limits;
  typedef std::numeric_limits<unsigned_t> uns_limits;

  BOOST_CHECK_EQUAL(lexical_cast<unsigned_t>((uns_limits::max)()),
                    (uns_limits::max)());
  BOOST_CHECK_EQUAL(lexical_cast<unsigned_t>((uns_limits::min)()),
                    (uns_limits::min)());

  BOOST_CHECK_EQUAL(lexical_cast<signed_t>((s_limits::max)()),
                    (s_limits::max)());
  BOOST_CHECK_EQUAL(lexical_cast<signed_t>((uns_limits::min)()),
                    static_cast<signed_t>((uns_limits::min)()));

  BOOST_CHECK_EQUAL(lexical_cast<unsigned_t>((s_limits::max)()),
                    static_cast<unsigned_t>((s_limits::max)()));
  BOOST_CHECK_EQUAL(lexical_cast<unsigned_t>((s_limits::min)()),
