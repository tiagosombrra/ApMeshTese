...
index_type map_direct(boundary_type t, icu::BreakIterator *it, int reserve) {
  ...
  while ((pos = it->next()) != icu::BreakIterator::DONE) {
    ...
    if (t != character && rbbi) {
      //
      // There is a collapse for MSVC: int32_t defined by both boost::cstdint
      // and icu... So need to pick one ;(
      //
...
icu::BreakIterator *get_iterator(boundary_type t, icu::Locale const &loc) {
  ...
  check_and_throw_icu_error(err);
  if (!bi.get()) throw std::runtime_error("Failed to create break iterator");
  return bi.release();
}
...
index_type do_map(boundary_type t, CharType const *begin, CharType const *end,
...
#endif
  {
    ...
    for (size_t i = 1; i < indirect.size(); i++) {
      ...
      indx[i].offset = offset_direct + cvt.cut(str, begin, end, diff,
                                               offset_inderect, offset_direct);
    }
  }
  return indx;
}  // do_map
