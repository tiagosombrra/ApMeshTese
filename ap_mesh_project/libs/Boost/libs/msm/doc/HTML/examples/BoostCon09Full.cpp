...
struct player_ : public msm::front::state_machine_def<player_> {
  ...
  struct Playing_ : public msm::front::state_machine_def<Playing_> {
    ...
    struct CDFinished : public msm::front::exit_pseudo_state<AllSongsPlayed> {
      template <class Event, class FSM>
      void on_entry(Event const&, FSM&) {
        std::cout << "entering: Playing::CDFinished" << std::endl;
      }
      template <class Event, class FSM>
      void on_exit(Event const&, FSM&) {
        std::cout << "leaving: Playing::CDFinished" << std::endl;
      }
    };
    ...
    void all_songs_played(NextSong const&) {
      std::cout << "Playing::all_songs_played\n";
    }
    // guard conditions
  ...
  void end_playback(AllSongsPlayed const&) {
    std::cout << "player::end_playback\n";
  }
  ...
  struct transition_table
      : mpl::vector<
            ...
            a_row<Playing::exit_pt<Playing_::CDFinished>, AllSongsPlayed,
                  Stopped, &p::end_playback>,
            //  +-------------+---------------+---------+---------------------+----------------------+
...
void test() {
  ...
  std::cout
      << "play is not handled in the current state but is marked as delayed"
      << std::endl;
  p.process_event(play());
  pstate(p);
  std::cout << "cd_detected will cause play to be handled also" << std::endl;
  // will be rejected, wrong disk type
  p.process_event(cd_detected(DISK_DVD));
  pstate(p);
  // will be accepted, wrong disk type
  p.process_event(cd_detected(DISK_CD));
  pstate(p);
  ...
  mpl::for_each<simple_states, boost::msm::wrap<mpl::placeholders::_1> >(
      msm::back::display_type());

  std::cout << "the event list:" << std::endl;
  typedef msm::back::generate_event_set<Stt>::type event_list;
  mpl::for_each<event_list, boost::msm::wrap<mpl::placeholders::_1> >(
      msm::back::display_type());
  std::cout << std::endl;
  ...
  mpl::for_each<all_events, boost::msm::wrap<mpl::placeholders::_1> >(
      msm::back::display_type());
}
}  // namespace
