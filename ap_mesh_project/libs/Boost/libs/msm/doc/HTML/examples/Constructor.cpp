...
struct player_ : public msm::front::state_machine_def<player_> {
  player_(SomeExternalContext& context, int someint) : context_(context) {
    std::cout << "context value:" << context_.bla << " with value:" << someint
              << std::endl;
    context.bla = 10;
  }
...
void test() {
  ...
  // example 2
  // create a player with a copy of an Empty state, an argument by reference and
  // an int (last 2 go to the constructor front-end)
  player p2(msm::back::states_ << player_::Empty(1), boost::ref(ctx), 5);
  std::cout << "Empty's data should be 1. Is: "
            << p2.get_state<player_::Empty&>().data_ << std::endl;

  std::cout << "Set a new Empty state" << std::endl;
  p2.set_states(msm::back::states_ << player_::Empty(5));
  std::cout << "Empty's data should be 5. Is: "
            << p2.get_state<player_::Empty&>().data_ << std::endl;

  std::cout << "Set new Empty and Open states" << std::endl;
  p2.set_states(msm::back::states_ << player_::Empty(7) << player_::Open(2));
  std::cout << "Empty's data should be 7. Is: "
            << p2.get_state<player_::Empty&>().data_ << std::endl;
  std::cout << "Open's data should be 2. Is: "
            << p2.get_state<player_::Open&>().data_ << std::endl;

  // example 3
  // create a player with a copy of an Empty state, a copy of a Playing
  // submachine (which is itself created by a copy of Song1) an argument by
  // reference and an int (last 2 go to the constructor front-end)
  player p(msm::back::states_
               << player_::Empty(1)
               << player_::Playing(msm::back::states_
                                   << player_::Playing_::Song1(8)),
           boost::ref(ctx), 5);

  std::cout << "Song1's data should be 8. Is: "
            << p.get_state<player_::Playing&>()
                   .get_state<player_::Playing_::Song1&>()
                   .data_
            << std::endl;
}
}  // namespace
