...
struct cd_detected_event
    : boost::msm::front::euml::euml_event<cd_detected_event> {
  cd_detected_event() {}
  cd_detected_event(std::string const& name, DiskTypeEnum disk)
      : cd_name(name), cd_type(disk) {}
  std::string cd_name;
...
struct open_entry_impl {
  ...
  void operator()() { cout << "entering: Open" << endl; }
};
boost::phoenix::function<open_entry_impl> open_entry;
struct open_exit_impl {
  typedef void result_type;
  void operator()() { cout << "leaving: Open" << endl; }
};
...
struct start_playback_impl {
  typedef void result_type;
  void operator()() { cout << "calling: start_playback" << endl; }
};
...
struct store_cd_info_impl {
  typedef void result_type;
  void operator()() { cout << "calling: store_cd_info" << endl; }
};
...
// it is also possible to use BOOST_PHOENIX_ADAPT_CALLABLE to avoid defining a
// global variable
...
void test() {
  ...
  p.process_event(cd_detected_event("louie, louie", DISK_DVD));
  pstate(p);
  p.process_event(cd_detected_event("louie, louie", DISK_CD));
  pstate(p);
