...
BOOST_MSM_EUML_ACTION(State3_Exit){
    template <class Event, class FSM, class STATE> void operator()(
        Event const&, FSM&, STATE&){std::cout << "leaving: State3" << std::endl;
}
}
;
...
BOOST_MSM_EUML_ACTION(State4_Exit){
    template <class Event, class FSM, class STATE> void operator()(
        Event const&, FSM&, STATE&){std::cout << "leaving: State4" << std::endl;
}
}
;
...
BOOST_MSM_EUML_ACTION(always_true){
    ...
    operator()(EVT const& evt, FSM&, SourceState&,
               TargetState&){std::cout << "always_true" << std::endl;
return true;
}
}
;
BOOST_MSM_EUML_ACTION(always_false){
    template <class FSM, class EVT, class SourceState, class TargetState> bool
    operator()(EVT const& evt, FSM&, SourceState&,
               TargetState&){std::cout << "always_false" << std::endl;
return false;
}
}
;
