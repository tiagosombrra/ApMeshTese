...
// if we need something special, like a template constructor, we cannot use the
// helper macros
...
BOOST_MSM_EUML_ACTION(SubState1_Entry){
    template <class Event, class FSM, class STATE> void operator()(Event const&,
                                                                   FSM&,
                                                                   STATE&){
...
;
BOOST_MSM_EUML_EXPLICIT_ENTRY_STATE(0, (SubState2_Entry, SubState2_Exit),
                                    SubState2)
...
;
BOOST_MSM_EUML_EXPLICIT_ENTRY_STATE(1, (SubState2b_Entry, SubState2b_Exit),
                                    SubState2b)
...
;
BOOST_MSM_EUML_EXPLICIT_ENTRY_STATE(2, (SubState2c_Entry, SubState2c_Exit),
                                    SubState2c)
...
;
BOOST_MSM_EUML_ENTRY_STATE(0, (PseudoEntry1_Entry, PseudoEntry1_Exit),
                           PseudoEntry1)
...
;
BOOST_MSM_EUML_EXIT_STATE((event6, PseudoExit1_Entry, PseudoExit1_Exit),
                          PseudoExit1)

// actions
BOOST_MSM_EUML_ACTION(entry_action){
    template <class FSM, class EVT, class SourceState, class TargetState> void
    operator()(FSM&, EVT const&, SourceState&,
               TargetState&){cout << "calling entry_action" << endl;
}
}
;
// SubFsm definition
...
BOOST_MSM_EUML_TRANSITION_TABLE(
    (
        ...
        SubState1 == SubState2 + event6, PseudoExit1 == SubState3 + event5
        //  +------------------------------------------------------------------------------+
        ),
    SubFsm2_transition_table)

BOOST_MSM_EUML_DECLARE_STATE_MACHINE((SubFsm2_transition_table,  // STT
                                      init_ << SubState1 << SubState1b
                                            << SubState1c,  // Init State
                                      SubFsm2_Entry,        // Entry
                                      SubFsm2_Exit),
                                     SubFsm2_def)
// inherit to add some typedef
struct SubFsm2_ : public SubFsm2_def {
  // these 2 states are not found in the transition table because they are
  // accessed only through a fork, so we need to create them explicitly
  typedef mpl::vector<BOOST_MSM_EUML_STATE_NAME(SubState2b),
                      BOOST_MSM_EUML_STATE_NAME(SubState2c)>
      explicit_creation;
};
...
BOOST_MSM_EUML_ACTION(State1_Exit){
    template <class Event, class FSM, class STATE> void operator()(
        Event const&, FSM&, STATE&){std::cout << "leaving: State1" << std::endl;
}
}
;
...
BOOST_MSM_EUML_ACTION(State2_Exit){
    template <class Event, class FSM, class STATE> void operator()(
        Event const&, FSM&, STATE&){std::cout << "leaving: State2" << std::endl;
}
}
;
...
BOOST_MSM_EUML_TRANSITION_TABLE(
    (
        ...
        (explicit_(SubFsm2, SubState2), explicit_(SubFsm2, SubState2b),
...
BOOST_MSM_EUML_ACTION(Log_No_Transition){
    template <class Event, class FSM, class STATE> void operator()(
        Event const& e, FSM&,
        STATE&){std::cout << "no transition in Fsm"
...
void test() {
  ...
  // start of the SM
  p.start();
  std::cout << "Simply move in and out of the composite, activate init states"
            << std::endl;
  p.process_event(event1);
  pstate(p);
  p.process_event(event1);
  pstate(p);
  std::cout << "Direct entry into SubFsm2::SubState2, then transition to "
               "SubState1 and back to State1"
            << std::endl;
  ...
  p.process_event(event1);
  pstate(p);
  std::cout << "processing fork to SubFsm2::SubState2, SubFsm2::SubState2b and "
               "SubFsm2::SubState2c"
            << std::endl;
