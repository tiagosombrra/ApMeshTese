...
struct Playing_impl : public msm::front::state<>,
                      public msm::front::euml::euml_state<Playing_impl> {
  template <class Event, class FSM>
  void on_entry(Event const&, FSM&) {
...
BOOST_MSM_EUML_ACTION(pause_playback2){
    ...
    operator()(EVT const& evt, FSM&, SourceState&,
               TargetState&){cout << "player::pause_playback2" << endl;
std::cout << "event type: " << typeid(EVT).name() << std::endl;
std::cout << "pause_playback2 with any event: " << evt.type().name()
          << std::endl;
}  // namespace
}
;
