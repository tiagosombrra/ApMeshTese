...
struct Insert
    : public boost::msm::front::state<BASE_TYPE, boost::msm::front::sm_ptr> {
...
struct iPodSearch_ : public msm::front::state_machine_def<iPodSearch_> {
  ...
  typedef Insert<Songset, boost::msm::front::default_base_state, iPodSearch>
      MyInsert;
  typedef StringFind<boost::msm::front::default_base_state, iPodSearch> MyFind;
  typedef Foreach<OneSong, Songset, boost::msm::front::default_base_state,
                  iPodSearch>
      MyForeach;
...
void test() {
  ...
  search.get_state<iPodSearch::MyFind*>()->set_container(
      "Sh");  // will find 2 songs
  ...
  for (iPodSearch::Songset::const_iterator it = res.begin(); it != res.end();
       ++it) {
  ...
  search.reset_search();
  search.get_state<iPodSearch::MyFind*>()->set_container(
      "She");  // will find 1 song
  search.start();
  const iPodSearch::Songset& res2 = search.get_result();
  for (iPodSearch::Songset::const_iterator it = res2.begin(); it != res2.end();
       ++it) {
    cout << "candidate song:" << *it << endl;
  }
}
