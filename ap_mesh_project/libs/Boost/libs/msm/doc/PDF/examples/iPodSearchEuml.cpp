...
struct OneSong_impl : euml_event<OneSong_impl>, OneSongDef {
  OneSong_impl() {}
  OneSong_impl(const string& asong) { get_attribute(m_song) = asong; }
  OneSong_impl(const char* asong) { get_attribute(m_song) = asong; }
...
BOOST_MSM_EUML_DECLARE_STATE_MACHINE(
    (transition_table,                // STT
     ...
     (clear_(fsm_(m_src_container)),  // clear source
      ...
      push_back_(
          fsm_(m_src_container),
          String_<mpl::string<'Yell', 'ow s', 'ubma', 'rine'> >()),  // add a
                                                                     // song
      push_back_(
          fsm_(m_src_container),
          String_<mpl::string<'Twis', 't an', 'd Sh', 'out'> >()),  // add a
                                                                    // song
      push_back_(
          fsm_(m_src_container),
...
void test() {
  ...
  search.get_state<BOOST_MSM_EUML_STATE_NAME(StringFind)&>().get_attribute(
      m_letters) = "She";  // will find 1 song
  search.start();
  // display all the songs
  for (vector<OneSong_impl>::const_iterator it =
           search.get_attribute(m_tgt_container).begin();
