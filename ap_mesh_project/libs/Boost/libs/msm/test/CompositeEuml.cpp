...
BOOST_MSM_EUML_ACTION(good_disk_format){
    ...
    operator()(EVT const& evt, FSM&, SourceState&, TargetState&){
        if (evt.get_attribute(cd_type) != DISK_CD){return false;
}
return true;
}
}
...
BOOST_AUTO_TEST_CASE(my_test) {
  ...
  BOOST_CHECK_MESSAGE(p.get_attribute(test_fct_counter) == 1,
                      "action not called correctly");
  ...
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>().current_state()[1] == 3,
                      "Region2State1 should be active");
  BOOST_CHECK_MESSAGE(
      p.get_state<Playing_type&>()
              .get_state<BOOST_MSM_EUML_STATE_NAME(Region2State1)&>()
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<Playing_type&>()
              .get_state<BOOST_MSM_EUML_STATE_NAME(Region2State2)&>()
              .get_attribute(entry_counter) == 0,
