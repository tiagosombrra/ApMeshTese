...
BOOST_MSM_EUML_STATE((++state_(entry_counter), ++state_(exit_counter),
                      attributes_ << entry_counter << exit_counter,
                      configure_ << CDLoaded << PlayingPaused),
                     Paused)

BOOST_MSM_EUML_STATE((++state_(entry_counter), ++state_(exit_counter),
                      attributes_ << entry_counter << exit_counter),
                     AllOk)
BOOST_MSM_EUML_TERMINATE_STATE((++state_(entry_counter), ++state_(exit_counter),
                                attributes_ << entry_counter << exit_counter),
                               ErrorTerminate)
BOOST_MSM_EUML_INTERRUPT_STATE((end_error, ++state_(entry_counter),
                                ++state_(exit_counter),
                                attributes_ << entry_counter << exit_counter),
                               ErrorMode)
...
BOOST_MSM_EUML_TRANSITION_TABLE(
    (
        ...
        Song1 == Song2 + previous_song[True_()] /
                             ++fsm_(start_prev_song_guard_counter),
        Song3 == Song2 + next_song / ++fsm_(start_next_song_counter),
        Song2 == Song3 + previous_song[True_()] /
                             ++fsm_(start_prev_song_guard_counter)
        //  +------------------------------------------------------------------------------+
        ),
...
BOOST_MSM_EUML_DECLARE_STATE_MACHINE(
    (playing_transition_table,  // STT
     init_ << Song1,            // Init State
     ++state_(entry_counter),   // Entry
     ++state_(exit_counter),    // Exit
     attributes_ << entry_counter << exit_counter << start_next_song_counter
...
BOOST_MSM_EUML_ACTION(No_Transition){
    template <class FSM, class Event> void operator()(Event const&, FSM&, int){
...
BOOST_MSM_EUML_TRANSITION_TABLE(
    (Playing == Stopped + play / ++fsm_(start_playback_counter),
     ...
     //  +------------------------------------------------------------------------------+
     Open == Empty + open_close, Open == Paused + open_close,
     Open == Stopped + open_close, Open == Playing + open_close,
     //  +------------------------------------------------------------------------------+
     Paused == Playing + pause,
     //  +------------------------------------------------------------------------------+
     Stopped == Playing + stop, Stopped == Paused + stop,
     Stopped == Empty + cd_detected, Stopped == Stopped + stop,
...
BOOST_MSM_EUML_DECLARE_STATE_MACHINE(
    (transition_table,         // STT
     ...
     attributes_ << start_playback_counter << can_close_drawer_counter
                 << report_error_counter
                 << report_end_error_counter,  // Attributes
...
// static char const* const state_names[] = { "Stopped", "Paused","Open",
// "Empty", "Playing" ,"AllOk","ErrorMode","ErrorTerminate" };
...
BOOST_AUTO_TEST_CASE(my_test) {
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(Open)&>().get_attribute(
          exit_counter) == 0,
      "Open exit not called correctly");
  BOOST_CHECK_MESSAGE(
      p.get_state<Playing_type&>().get_attribute(entry_counter) == 0,
      "Playing entry not called correctly");
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(Empty)&>().get_attribute(
          entry_counter) == 1,
      "Empty entry not called correctly");
  // flags
  BOOST_CHECK_MESSAGE(
      p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(CDLoaded)>() == false,
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(Empty)&>().get_attribute(
          entry_counter) == 2,
      "Empty entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_attribute(can_close_drawer_counter) == 1,
  ...
  BOOST_CHECK_MESSAGE(
      ...
      "Empty exit not called correctly");
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(Stopped)&>().get_attribute(
          entry_counter) == 1,
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<Playing_type&>().get_attribute(entry_counter) == 1,
      "Playing entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_attribute(start_playback_counter) == 1,
                      "action not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>().current_state()[0] == 0,
                      "Song1 should be active");
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>()
                              .get_state<BOOST_MSM_EUML_STATE_NAME(Song1)&>()
                              .get_attribute(entry_counter) == 1,
                      "Song1 entry not called correctly");

  // flags
  BOOST_CHECK_MESSAGE(
      p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(PlayingPaused)>() == true,
      "PlayingPaused should be active");
  BOOST_CHECK_MESSAGE(
      p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(FirstSongPlaying)>() == true,
      "FirstSongPlaying should be active");

  p.process_event(next_song);
  BOOST_CHECK_MESSAGE(p.current_state()[0] == 4,
                      "Playing should be active");  // Playing
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>().current_state()[0] == 1,
                      "Song2 should be active");
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>()
                              .get_state<BOOST_MSM_EUML_STATE_NAME(Song2)&>()
                              .get_attribute(entry_counter) == 1,
                      "Song2 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>()
                              .get_state<BOOST_MSM_EUML_STATE_NAME(Song1)&>()
                              .get_attribute(exit_counter) == 1,
  ...
  BOOST_CHECK_MESSAGE(p.current_state()[0] == 4,
                      "Playing should be active");  // Playing
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>().current_state()[0] == 2,
                      "Song3 should be active");
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>()
                              .get_state<BOOST_MSM_EUML_STATE_NAME(Song3)&>()
                              .get_attribute(entry_counter) == 1,
  ...
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>()
                              .get_state<BOOST_MSM_EUML_STATE_NAME(Song2)&>()
                              .get_attribute(entry_counter) == 2,
                      "Song2 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>()
                              .get_state<BOOST_MSM_EUML_STATE_NAME(Song3)&>()
                              .get_attribute(exit_counter) == 1,
                      "Song3 exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<Playing_type&>().get_attribute(
                          start_prev_song_guard_counter) == 1,
  ...
  BOOST_CHECK_MESSAGE(
      ...
      "PlayingPaused should be active");
  BOOST_CHECK_MESSAGE(
      p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(FirstSongPlaying)>() == false,
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<Playing_type&>().get_attribute(exit_counter) == 1,
      "Playing exit not called correctly");
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(Paused)&>().get_attribute(
          entry_counter) == 1,
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<Playing_type&>().get_attribute(entry_counter) == 2,
      "Playing entry not called correctly");
  ...
  BOOST_CHECK_MESSAGE(p.current_state()[0] == 1,
                      "Paused should be active");  // Paused
  BOOST_CHECK_MESSAGE(
      p.get_state<Playing_type&>().get_attribute(exit_counter) == 2,
      "Playing exit not called correctly");
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(Paused)&>().get_attribute(
          entry_counter) == 2,
  ...
  // flags
  BOOST_CHECK_MESSAGE(
      p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(PlayingPaused)>() == false,
      "PlayingPaused should not be active");
  BOOST_CHECK_MESSAGE(
      p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(CDLoaded)>() == true,
      "CDLoaded should be active");
  // BOOST_CHECK_MESSAGE(p.is_flag_active<BOOST_MSM_EUML_FLAG_NAME(CDLoaded),player::Flag_AND>()
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(AllOk)&>().get_attribute(
          exit_counter) == 1,
      "AllOk exit not called correctly");
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(ErrorMode)&>().get_attribute(
          entry_counter) == 1,
  ...
  BOOST_CHECK_MESSAGE(
      ...
      "Stopped exit not called correctly");
  BOOST_CHECK_MESSAGE(
      p.get_state<Playing_type&>().get_attribute(entry_counter) == 3,
      "Playing entry not called correctly");
  ...
  BOOST_CHECK_MESSAGE(
      ...
      "AllOk exit not called correctly");
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(ErrorTerminate)&>().get_attribute(
          entry_counter) == 1,
