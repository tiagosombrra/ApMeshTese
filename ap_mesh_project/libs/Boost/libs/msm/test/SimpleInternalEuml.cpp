...
BOOST_MSM_EUML_ACTION(internal_guard_fct){
    ...
    operator()(EVT const&, FSM&, SourceState& src,
               TargetState&){++src.get_attribute(empty_internal_guard_counter);
return false;
}  // namespace
}
;
BOOST_MSM_EUML_DECLARE_STATE((++state_(entry_counter), ++state_(exit_counter),
                              attributes_ << entry_counter << exit_counter
                                          << empty_internal_guard_counter
                                          << empty_internal_action_counter),
                             Empty_def)
...
struct Empty_impl : public Empty_def {
  ...
  BOOST_MSM_EUML_DECLARE_INTERNAL_TRANSITION_TABLE(
      (internal_evt[internal_guard_fct] /
       ++source_(empty_internal_action_counter)))
};
...
BOOST_MSM_EUML_ACTION(internal_guard){
    ...
    operator()(EVT const&, FSM& fsm, SourceState&,
               TargetState&){++fsm.get_attribute(internal_guard_counter);
return false;
}
}
;
...
BOOST_MSM_EUML_TRANSITION_TABLE(
    (Playing == Stopped + play / ++fsm_(start_playback_counter),
     ...
     Stopped == Empty + cd_detected[good_disk_format], Stopped == Stopped + stop
     //  +------------------------------------------------------------------------------+
     ),
    transition_table)
...
BOOST_MSM_EUML_DECLARE_STATE_MACHINE(
    (transition_table,  // STT
     ...
     attributes_ << start_playback_counter << can_close_drawer_counter
                 ...
                 << internal_guard_counter,  // Attributes
...
BOOST_AUTO_TEST_CASE(my_test) {
  ...
  p.start();
  BOOST_CHECK_MESSAGE(
      p.get_state<Empty_impl&>().get_attribute(entry_counter) == 1,
      "Empty entry not called correctly");

  // internal events
  p.process_event(to_ignore);
  p.process_event(internal_evt);
  BOOST_CHECK_MESSAGE(p.get_attribute(internal_action_counter) == 1,
                      "Internal action not called correctly");
  BOOST_CHECK_MESSAGE(p.get_attribute(internal_guard_counter) == 1,
                      "Internal guard not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<Empty_impl&>().get_attribute(
                          empty_internal_action_counter) == 0,
                      "Empty internal action not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<Empty_impl&>().get_attribute(
                          empty_internal_guard_counter) == 1,
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<Empty_impl&>().get_attribute(exit_counter) == 1,
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<Empty_impl&>().get_attribute(entry_counter) == 2,
      "Empty entry not called correctly");
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<Empty_impl&>().get_attribute(exit_counter) == 2,
      "Empty exit not called correctly");
  BOOST_CHECK_MESSAGE(
  ...
  BOOST_CHECK_MESSAGE(
      p.get_state<BOOST_MSM_EUML_STATE_NAME(Playing)&>().get_attribute(
          entry_counter) == 1,
