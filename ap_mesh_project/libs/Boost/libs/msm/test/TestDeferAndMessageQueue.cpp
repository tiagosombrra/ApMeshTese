...
struct player_ : public msm::front::state_machine_def<player_> {
  player_() : expected_action_counter(0) {}
  ...
  struct Unresolved : public msm::front::state<> {
    ...
    struct internal_transition_table
        : mpl::vector<
              ...
              Internal<eventConnect, msm::front::ActionSequence_<mpl::vector<
                                         enqueue_action1, enqueue_action2>>>
              //  +---------+-------------+---------+---------------------+----------------------+
              > {};
  };
...
BOOST_AUTO_TEST_CASE(TestDeferAndMessageQueue) {
  ...
  BOOST_CHECK_MESSAGE(p.get_state<player_::Unresolved&>().exit_counter == 1,
                      "Unresolved exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::Unresolved&>().entry_counter == 1,
                      "Unresolved entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::Resolving&>().entry_counter == 1,
                      "Resolving entry not called correctly");

  p.process_event(eventResolved());
  BOOST_CHECK_MESSAGE(p.current_state()[0] == 4, "Connecting should be active");
  BOOST_CHECK_MESSAGE(p.current_state()[1] == 3, "State22 should be active");
  BOOST_CHECK_MESSAGE(p.get_state<player_::Resolved&>().exit_counter == 1,
                      "Resolved exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::Resolved&>().entry_counter == 1,
                      "Resolved entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::Resolving&>().exit_counter == 1,
                      "Resolving exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::Connecting&>().entry_counter == 1,
                      "Connecting entry not called correctly");
