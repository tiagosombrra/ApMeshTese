...
struct player_ : public msm::front::state_machine_def<player_> {
  player_() : expected_action_counter(0), expected_action2_counter(0) {}
  ...
  struct transition_table
      : mpl::vector<
            ...
            Row<State13, event2, State11>, Row<State13, eventd, State11>
            //    +---------+-------------+---------+---------------------+----------------------+
            > {};
...
BOOST_AUTO_TEST_CASE(TestDeferAndMessageQueue2) {
  ...
  BOOST_CHECK_MESSAGE(p.get_state<player_::State12&>().exit_counter == 1,
                      "State12 exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State11&>().entry_counter == 2,
                      "State11 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State13&>().exit_counter == 1,
                      "State13 exit not called correctly");
  BOOST_CHECK_MESSAGE(p.get_state<player_::State13&>().entry_counter == 1,
                      "State13 entry not called correctly");
  BOOST_CHECK_MESSAGE(p.expected_action_counter == 1,
                      "expected_action should have been called");
  BOOST_CHECK_MESSAGE(p.expected_action2_counter == 0,
                      "expected_action2 should not have been called");
}
}  // namespace
