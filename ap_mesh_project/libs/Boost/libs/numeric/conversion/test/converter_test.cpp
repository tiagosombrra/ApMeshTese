...
template <>
inline unsigned char absG<unsigned char>(unsigned char v) {
  return v;
}
template <>
inline unsigned short absG<unsigned short>(unsigned short v) {
  return v;
}
template <>
inline unsigned int absG<unsigned int>(unsigned int v) {
  return v;
}
template <>
inline unsigned long absG<unsigned long>(unsigned long v) {
  return v;
}
...
void test_conversions() {
  ...
  volatile double
      dv;  // avoid this to be cached internally in some fpu register
  ...
  TEST_POS_OVERFLOW_CONVERSION_DEF(
      boost::int16_t, boost::int32_t,
      bounds<boost::int16_t>::highest() + boost::int32_t(1));
  TEST_NEG_OVERFLOW_CONVERSION_DEF(
      boost::int16_t, boost::int32_t,
      bounds<boost::int16_t>::lowest() - boost::int32_t(1));
  ...
  TEST_POS_OVERFLOW_CONVERSION_DEF(
      boost::uint16_t, boost::int32_t,
      bounds<boost::uint16_t>::highest() + boost::int32_t(1));
  TEST_NEG_OVERFLOW_CONVERSION_DEF(boost::uint32_t, boost::int32_t,
                                   boost::int32_t(-1));
  ...
  TEST_POS_OVERFLOW_CONVERSION_DEF(boost::int32_t, boost::uint32_t,
                                   bounds<boost::uint32_t>::highest());
  TEST_POS_OVERFLOW_CONVERSION_DEF(boost::int16_t, boost::uint32_t,
                                   bounds<boost::uint32_t>::highest());
  ...
  TEST_POS_OVERFLOW_CONVERSION_DEF(boost::uint16_t, boost::uint32_t,
                                   bounds<boost::uint32_t>::highest());
...
void test_overflow_handler(MATCH_FNTPL_ARG(T), MATCH_FNTPL_ARG(S),
                           MATCH_FNTPL_ARG(OverflowHandler), PostCondition pos,
                           PostCondition neg) {
...
void test_overflow_handlers(MATCH_FNTPL_ARG(T), MATCH_FNTPL_ARG(S)) {
  ...
  test_overflow_handler(SET_FNTPL_ARG(T), SET_FNTPL_ARG(S),
                        SET_FNTPL_ARG(boost::numeric::silent_overflow_handler),
                        c_converted, c_converted);

  cout << "Testing Default Overflow Handler policy\n";

  test_overflow_handler(SET_FNTPL_ARG(T), SET_FNTPL_ARG(S),
                        SET_FNTPL_ARG(boost::numeric::def_overflow_handler),
                        c_pos_overflow, c_neg_overflow);

  cout << "Testing Custom (User-Defined) Overflow Handler policy\n";

  test_overflow_handler(SET_FNTPL_ARG(T), SET_FNTPL_ARG(S),
                        SET_FNTPL_ARG(custom_overflow_handler), c_converted,
                        c_converted);
}

// For a given float-type number 'n' of integer value (n.0), check the
// conversions within the range [n-1,n+1] taking values at:
// (n-1,n-0.5,n,n+0.5,n+1). For each sampled value there is an expected result
// and a PostCondition according to the specified round_style.
//
template <class T, class S, class Float2IntRounder>
void test_rounding_conversion(MATCH_FNTPL_ARG(T),
                              MATCH_FNTPL_ARG(Float2IntRounder), S s,
                              PostCondition resl1, PostCondition resl0,
                              PostCondition res, PostCondition resr0,
                              PostCondition resr1) {
  typedef boost::numeric::conversion_traits<T, S> Traits;
...
void test_round_style(MATCH_FNTPL_ARG(T), MATCH_FNTPL_ARG(S)) {
  ...
  test_rounding_conversion(
      SET_FNTPL_ARG(T), SET_FNTPL_ARG(boost::numeric::Trunc<S>), min,
      c_neg_overflow, c_converted, c_converted, c_converted, c_converted);

  test_rounding_conversion(
      SET_FNTPL_ARG(T), SET_FNTPL_ARG(boost::numeric::Trunc<S>), max,
      c_converted, c_converted, c_converted, c_converted, c_pos_overflow);

  cout << "Testing 'RoundEven' Float2IntRounder policy\n";

  test_rounding_conversion(
      SET_FNTPL_ARG(T), SET_FNTPL_ARG(boost::numeric::RoundEven<S>), min,
      c_neg_overflow, c_converted, c_converted, c_converted, c_converted);

  test_rounding_conversion(
      SET_FNTPL_ARG(T), SET_FNTPL_ARG(boost::numeric::RoundEven<S>), max,
      c_converted, c_converted, c_converted, c_pos_overflow, c_pos_overflow);

  cout << "Testing 'Ceil' Float2IntRounder policy\n";

  test_rounding_conversion(
      SET_FNTPL_ARG(T), SET_FNTPL_ARG(boost::numeric::Ceil<S>), min,
      c_neg_overflow, c_converted, c_converted, c_converted, c_converted);

  test_rounding_conversion(
      SET_FNTPL_ARG(T), SET_FNTPL_ARG(boost::numeric::Ceil<S>), max,
      c_converted, c_converted, c_converted, c_pos_overflow, c_pos_overflow);

  cout << "Testing 'Floor' Float2IntRounder policy\n";

  test_rounding_conversion(
      SET_FNTPL_ARG(T), SET_FNTPL_ARG(boost::numeric::Floor<S>), min,
      c_neg_overflow, c_neg_overflow, c_converted, c_converted, c_converted);

  test_rounding_conversion(
      SET_FNTPL_ARG(T), SET_FNTPL_ARG(boost::numeric::Floor<S>), max,
      c_converted, c_converted, c_converted, c_converted, c_pos_overflow);
}
...
void test_converter_as_function_object() {
  ...
  std::vector<double> S;
  for (int i = 0; i < 10; ++i) S.push_back(i * (18.0 / 19.0));
  ...
  std::vector<int> I;
  std::transform(S.begin(), S.end(), std::back_inserter(I),
  ...
  BOOST_CHECK_MESSAGE(double_to_int_OK,
                      "converter (int,double) as function object");

  // Create a sequence of double values from s using a default
  // numeric::converter (which should be the trivial conv).
  std::vector<double> D;
  ...
  BOOST_CHECK_MESSAGE(double_to_double_OK,
...
void test_optimizations() {
  ...
  // It is intended to show the optimization of numeric::converter<> by manual
  // inspection of the generated code. Each test shows first the equivalent
  // hand-coded version. The numeric_cast<> code should be the same if full
  // compiler optimization/inlining is used.
...

  {
    ...
    range_check_result r = s < static_cast<double>(bounds<float>::lowest())
                               ...
                               : cInRange;
    if (r == cInRange) {
      r = s > static_cast<double>(bounds<float>::highest()) ? cPosOverflow
                                                            : cInRange;
    }
    if (r == cNegOverflow)
...

  {
    double const& s = dv1b;
    // range checking
    range_check_result r = s <= static_cast<double>(bounds<int>::lowest()) -
                                       static_cast<double>(1.0)
                               ? cNegOverflow
                               : cInRange;
    if (r == cInRange) {
      r = s >= static_cast<double>(bounds<int>::highest()) +
                      static_cast<double>(1.0)
              ? cPosOverflow
              : cInRange;
    }
...
int test_main(int, char* argv[]) {
  ...
  test_conversions();
  test_overflow_handlers(SET_FNTPL_ARG(boost::int16_t),
                         SET_FNTPL_ARG(boost::int32_t));
