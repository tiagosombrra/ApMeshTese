...
struct Double {
  ...
  template <typename T>
  explicit Double(T v) : v(static_cast<double>(v)) {}
  ...
  bool operator<(const Double& rhs) const { return v < rhs.v; }
...
class numeric_limits<Double> : public numeric_limits<double> {
 public:
  ...
  static Double(min)() { return Double(1.e-2); }

  static Double(max)() { return Double(1.e2); }
...
struct Ceil<Double> {
  ...
  typedef boost::mpl::integral_c<std::float_round_style,
...
struct Trunc<Double> {
  ...
  typedef boost::mpl::integral_c<std::float_round_style, std::round_toward_zero>
      round_style;
};
}  // namespace custom
...
struct numeric_cast_traits<Double, S> {
  ...
  typedef custom::range_checker<boost::numeric::conversion_traits<Double, S>,
                                overflow_policy>
...
struct numeric_cast_traits<T, Double> {
  typedef custom::overflow_handler overflow_policy;
  typedef custom::range_checker<boost::numeric::conversion_traits<T, Double>,
                                overflow_policy>
      range_checking_policy;
...
struct raw_converter<conversion_traits<T, Double> > {
  static T low_level_convert(const Double& n) { return static_cast<T>(n.v); }
};

template <typename S>
struct raw_converter<conversion_traits<Double, S> > {
  static Double low_level_convert(const S& n) { return Double(n); }
};
}  // namespace numeric
}  // namespace boost
...
void test_numeric_cast_traits() {
  typedef boost::mpl::vector<boost::int8_t, boost::uint8_t, boost::int16_t,
                             boost::uint16_t, boost::int32_t, boost::uint32_t
  ...
  Double d(56.0);
  BOOST_TEST_CATCH_CUSTOM_POSITIVE_OVERFLOW(
      d = boost::numeric_cast<Double>(101));
  BOOST_CHECK(d.v == 56.);
  BOOST_TEST_CATCH_CUSTOM_NEGATIVE_OVERFLOW(
      d = boost::numeric_cast<Double>(-101));
  BOOST_CHECK(d.v == 56.);
