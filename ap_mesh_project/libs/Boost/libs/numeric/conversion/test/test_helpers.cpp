...
//
// The Float2IntRounder policies *require* a visible 'ceil' or 'floor' math
// function with standard semantics. In a conformant compiler, ADL can pick
// these functions even if they are defined within a user namespace, as below.
//
...
std::string to_string(bool arg) { return arg ? "true" : "false"; }

std::string to_string(...) {
  throw std::runtime_error("to_string() called with wrong type!");
}
...
template <>
struct printable_number_type<signed char> {
  typedef int type;
};
template <>
struct printable_number_type<unsigned char> {
  typedef unsigned type;
};
template <>
struct printable_number_type<char> {
  typedef int type;
};

template <class T>
inline typename printable_number_type<T>::type printable(T n) {
  return n;
}
