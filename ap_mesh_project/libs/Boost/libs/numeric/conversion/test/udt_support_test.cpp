...
//     These traits instances are required in order to define the
//     specializations that follow (and which *are required* to make the library
//     work with MyInt and MyFloat)
//
namespace MyUDT {
...
//         This way, the default converter will work out of the box, which
//         means, for instance, that numeric_cast<> can be used with these UDTs.
//
//     (b) Define a user class with the appropriate interface and supply it
//     explicitely
...
//
// By default, if a UDT is involved in a conversion, internal range checking is
// disabled. This is so because a UDT type can have any sort of range, even
// unbounded, thus the library doesn't attempt to automatically figure out the
// appropriate range checking logic. (as it does when builtin types are
// involved) However, this situation is a bit unsufficient in practice,
// specially from doing narrowing (subranged) conversions from UDTs. The library
// provides a rudimentary hook to help this out: The user can plug in his own
...
template <>
struct get_builtin_type<MyInt> {
  typedef int type;
};
template <>
struct get_builtin_type<MyFloat> {
  typedef double type;
};

// U extract_builtin ( T s ) returns 's' converted to the corresponding built-in
// type U.
//
template <class N>
...
struct MyCustomRangeChecker {
  ...
  // This custom range checker uses the fact that our 'fake' UDT are merely
  // wrappers around builtin types; so it just forward the logic to the
  // correspoding range checkers for the wrapped builtin types.
  //
  ...
  // NOTE: The internal range checker used by default is *built* when you
  // instantiate a converter<> with a given Traits according to the properties
  // of the involved types. Currently, there is no way to instantiate this range
  // checker as a separate class. However, you can see it as part of the
  // interface of the converter (since the converter inherits from it)
  // Therefore, here we instantiate a converter corresponding to the builtin
  // types to access their associated builtin range checker.
  //
...
struct GenerateCustomConverter {
  ...
  typedef converter<T, S, Traits, OverflowHandler, Float2IntRounder,
                    RawConverter, RangeChecker>
      type;
};
