...
template <class T, class Policies, class CharType, class CharTraits>
inline std::basic_istream<CharType, CharTraits> &operator>>(
...
int main() {
  ...
  typedef interval<double,
                   policies<rounded_math<double>, checking_base<double> > >
      I;
  ...
  for (unsigned int i = 0; i < len; ++i) {
    {
      using namespace io_std;
      std::cout << tab[i] << '\n';
    }
    {
      using namespace io_sngl;
      std::cout << tab[i] << '\n';
    }
    {
      using namespace io_wdth;
      std::cout << tab[i] << '\n';
    }
    {
      using namespace io_prec;
      std::cout << tab[i] << '\n';
    }
    {
      using namespace io_wide;
      std::cout << tab[i] << '\n';
    }
    std::cout << '\n';
  }
