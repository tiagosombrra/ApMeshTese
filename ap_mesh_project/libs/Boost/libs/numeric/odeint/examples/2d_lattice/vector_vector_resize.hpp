...
template <>
struct same_size_impl<std::vector<std::vector<double> >,
...
template <>
struct resize_impl<std::vector<std::vector<double> >,
                   std::vector<std::vector<double> > > {
  typedef std::vector<std::vector<double> > state_type;
  ...
  static void resize(state_type &x1, const state_type &x2) {
    ...
    typename state_type::const_iterator begin2 = boost::begin(x2);
    while (begin1 != boost::end(x1)) (*begin1++).resize(boost::size(*begin2++));
  }
};
