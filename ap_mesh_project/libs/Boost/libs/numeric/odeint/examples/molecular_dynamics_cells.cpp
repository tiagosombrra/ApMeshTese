 ...
 public:
  ...
  struct cell_functor {
    ...
    void operator()(Tuple const& t) const {
      ...
      size_t i1 = size_t(point[0] / m_p.cell_size),
             i2 = size_t(point[1] / m_p.cell_size);
  ...
  struct interaction_functor {
    ...
    interaction_functor(hash_vector const& cells_begin,
                        hash_vector const& cells_end, hash_vector pos_order,
                        point_vector const& x, point_vector const& v,
                        params const& p)
        : m_cells_begin(cells_begin),
          m_cells_end(cells_end),
          m_order(pos_order),
          m_x(x),
          m_v(v),
          m_p(p) {}
    ...
    void operator()(Tuple const& t) const {
      ...
      for (int i = -1; i <= 1; ++i) {
        for (int j = -1; j <= 1; ++j) {
          ...
          for (size_t ii = m_cells_begin[cell_hash];
               ii < m_cells_end[cell_hash]; ++ii) {
  ...
  void operator()(point_vector const& x, point_vector const& v, point_vector& a,
                  double t) const {
    // init
    hash_vector pos_hash(m_p.n, 0);
    index_vector pos_index(m_p.n);
    hash_vector pos_order(m_p.n, 0);
    hash_vector cells_begin(m_p.n_cells), cells_end(m_p.n_cells),
        cell_order(m_p.n_cells);

    boost::iota(pos_order, 0);
    boost::iota(cell_order, 0);

    // calculate grid hash
    // calcHash( m_dGridParticleHash, m_dGridParticleIndex, dPos,
    // m_numParticles);
    std::for_each(boost::make_zip_iterator(boost::make_tuple(
                      x.begin(), pos_index.begin(), pos_hash.begin())),
                  boost::make_zip_iterator(boost::make_tuple(
                      x.end(), pos_index.end(), pos_hash.end())),
                  cell_functor(m_p));

    //         // sort particles based on hash
    //         // sortParticles(m_dGridParticleHash, m_dGridParticleIndex,
    //         m_numParticles);
    boost::sort(pos_order, [&](size_t i1, size_t i2) -> bool {
      return pos_hash[i1] < pos_hash[i2];
    });

    // reorder particle arrays into sorted order and find start and end of each
    // cell
    std::for_each(cell_order.begin(), cell_order.end(), [&](size_t i) {
      auto pos_begin = boost::make_transform_iterator(
          pos_order.begin(), transform_functor(pos_hash));
      auto pos_end = boost::make_transform_iterator(
          pos_order.end(), transform_functor(pos_hash));
      cells_begin[i] =
          std::distance(pos_begin, std::lower_bound(pos_begin, pos_end, i));
      cells_end[i] =
          std::distance(pos_begin, std::upper_bound(pos_begin, pos_end, i));
    });

    std::for_each(
        boost::make_zip_iterator(boost::make_tuple(
            x.begin(), v.begin(), a.begin(), pos_index.begin(),
            boost::counting_iterator<size_t>(0))),
        boost::make_zip_iterator(
            boost::make_tuple(x.end(), v.end(), a.end(), pos_index.end(),
...
md_system_bs<LocalForce, Interaction> make_md_system_bs(
    size_t n, LocalForce const& f1, Interaction const& f2, double xmax = 100.0,
    double ymax = 100.0, double cell_size = 2.0) {
  return md_system_bs<LocalForce, Interaction>(n, f1, f2, xmax, ymax,
                                               cell_size);
}
...
int main(int argc, char* argv[]) {
  ...
  auto sys = make_md_system_bs(n, local_force(),
  ...
  for (size_t oi = 0; oi < 10000; ++oi) {
    ...
    std::cout << "unset key"
              << "\n";
    std::cout << "p [0:" << sys.m_p.x_max << "][0:" << sys.m_p.y_max
              << "] '-' pt 7 ps 0.5"
              << "\n";
