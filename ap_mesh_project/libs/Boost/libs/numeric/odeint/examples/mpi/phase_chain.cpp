...
struct phase_chain_mpi_state {
  phase_chain_mpi_state(double gamma = 0.5) : m_gamma(gamma) {}
  ...
  void operator()(const state_type &x, state_type &dxdt, double /* t */) const {
    ...
    if (have_left) {
      ...
      r_left = x.world.irecv(x.world.rank() - 1, 0,
                             x_left);  // receive from x().back()
    }
    if (have_right) {
      x.world.isend(x.world.rank() + 1, 0, x().back());  // send to x_left
      r_right = x.world.irecv(x.world.rank() + 1, 0,
                              x_right);  // receive from x().front()
    }
...
int main(int argc, char **argv) {
  ...
  if (world.rank() == 0) {
    x.resize(N);
    boost::random::uniform_real_distribution<double> distribution(0.0,
                                                                  2.0 * pi);
