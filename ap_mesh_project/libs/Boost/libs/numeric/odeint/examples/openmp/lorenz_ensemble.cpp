...
#pragma omp parallel for
    for (size_t j = 0; j < x.size(); j++) {
      size_t offset = 0;
      for (size_t i = 0; i < j; i++) offset += x[i].size();
...
#pragma omp parallel for
  for (size_t i = 0; i < n; i++) R[i] = Rmin + (Rmax - Rmin) / (n - 1) * i;
  ...
  for (size_t i = 0; i != state.size(); i++) cerr << ' ' << state[i].size();
  cerr << endl;
  ...
  integrate_const(stepper(), sys_func(R), state, 0.0, t_max, dt);
