...
typedef mpl::vector<dummy_controlled_stepper, dummy_dense_output_stepper>
    dummy_steppers;

BOOST_AUTO_TEST_CASE(copy_controlled_stepper_iterator) {
  typedef adaptive_iterator<dummy_controlled_stepper, empty_system, state_type>
      iterator_type;
...
BOOST_AUTO_TEST_CASE(copy_dense_output_stepper_iterator) {
  typedef adaptive_iterator<dummy_dense_output_stepper, empty_system,
                            state_type>
      iterator_type;

  state_type x = {{1.0}};
  // fix by mario: do not dereference iterators at the end - made iter1 start
  // iterator
  iterator_type iter1(dummy_dense_output_stepper(), empty_system(), x, 0.0, 1.0,
                      0.1);
  iterator_type iter2(iter1);

  // fix by mario: iterator dereference now always gives internal state also for
  // dense output, consistent with other iterator implementations changed:
  // iterators with dense output stepper do not have an internal state now to
  // avoid a copy
...
BOOST_AUTO_TEST_CASE(
    copy_dense_output_stepper_iterator_with_reference_wrapper) {
  // bad use case, the same stepper is iterated twice
  typedef adaptive_iterator<
      boost::reference_wrapper<dummy_dense_output_stepper>, empty_system,
      state_type>
      iterator_type;
  ...
  BOOST_CHECK_EQUAL(&(*iter1), &(*iter2));
  BOOST_CHECK(!iter1.same(iter2));  // they point to the same stepper, there the
...
BOOST_AUTO_TEST_CASE(assignment_controlled_stepper_iterator) {
  ...
  iterator_type iter1 = iterator_type(dummy_controlled_stepper(),
                                      empty_system(), x1, 0.0, 1.0, 0.1);
  iterator_type iter2 = iterator_type(dummy_controlled_stepper(),
                                      empty_system(), x2, 0.0, 1.0, 0.2);
...
BOOST_AUTO_TEST_CASE(assignment_dense_output_stepper_iterator) {
  typedef adaptive_iterator<dummy_dense_output_stepper, empty_system,
      ...
      iterator_type;
  state_type x1 = {{1.0}};
  iterator_type iter1 = iterator_type(dummy_dense_output_stepper(),
                                      empty_system(), x1, 0.0, 1.0, 0.1);
  iterator_type iter2 = iterator_type(dummy_dense_output_stepper(),
                                      empty_system(), x1, 0.0, 1.0, 0.2);
...
BOOST_AUTO_TEST_CASE(
    assignment_dense_output_stepper_iterator_with_reference_wrapper) {
  typedef adaptive_iterator<
      boost::reference_wrapper<dummy_dense_output_stepper>, empty_system,
      state_type>
      iterator_type;
  ...
  dummy_dense_output_stepper stepper;
  iterator_type iter1 =
      iterator_type(boost::ref(stepper), empty_system(), x1, 0.0, 1.0, 0.1);
  iterator_type iter2 =
      iterator_type(boost::ref(stepper), empty_system(), x1, 0.0, 1.0, 0.2);
...
BOOST_AUTO_TEST_CASE(controlled_stepper_iterator_factory) {
  ...
  std::for_each(make_adaptive_iterator_begin(stepper, boost::ref(system), x,
                                             0.0, 1.0, 0.1),
...
BOOST_AUTO_TEST_CASE(controlled_stepper_range) {
  ...
  boost::for_each(
      make_adaptive_range(stepper, boost::ref(system), x, 0.0, 1.0, 0.1),
      dummy_observer());
...
BOOST_AUTO_TEST_CASE(
    controlled_stepper_iterator_with_reference_wrapper_factory) {
  dummy_controlled_stepper stepper;
  empty_system system;
  state_type x = {{1.0}};

  std::for_each(
      make_adaptive_iterator_begin(boost::ref(stepper), boost::ref(system), x,
                                   0.0, 1.0, 0.1),
...
// just test if it compiles
BOOST_AUTO_TEST_CASE(
    dense_output_stepper_iterator_with_reference_wrapper_factory) {
  dummy_dense_output_stepper stepper;
  empty_system system;
  state_type x = {{1.0}};
...
BOOST_AUTO_TEST_CASE(controlled_stepper_range_with_reference_wrapper) {
  ...
  boost::for_each(make_adaptive_range(boost::ref(stepper), boost::ref(system),
                                      x, 0.0, 1.0, 0.1),
                  dummy_observer());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_factory, Stepper,
                              dummy_steppers) {
  state_type x = {{1.0}};
  std::vector<state_type> res;
  std::copy(make_adaptive_iterator_begin(Stepper(), empty_system(), x, 0.0,
                                         0.35, 0.1),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_range_factory, Stepper,
                              dummy_steppers) {
  state_type x = {{1.0}};
  std::vector<state_type> res;
  boost::range::copy(
      make_adaptive_range(Stepper(), empty_system(), x, 0.0, 0.35, 0.1),
      std::back_insert_iterator<std::vector<state_type> >(res));
