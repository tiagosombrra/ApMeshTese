...
BOOST_AUTO_TEST_CASE(copy_stepper_iterator) {
  typedef adaptive_time_iterator<dummy_controlled_stepper, empty_system,
                                 state_type>
      iterator_type;
  state_type x = {{1.0}};
  iterator_type iter1 = iterator_type(dummy_controlled_stepper(),
                                      empty_system(), x, 0.0, 1.0, 0.1);
...
BOOST_AUTO_TEST_CASE(copy_dense_output_stepper_iterator) {
  typedef adaptive_time_iterator<dummy_dense_output_stepper, empty_system,
                                 state_type>
      iterator_type;
  state_type x = {{1.0}};
...
BOOST_AUTO_TEST_CASE(
  ...
  typedef adaptive_time_iterator<
  ...
  iterator_type iter1 =
      iterator_type(boost::ref(stepper), empty_system(), x, 0.0, 1.0, 0.1);
  iterator_type iter2 = iter1;
  BOOST_CHECK_EQUAL(&(iter1->first), &(iter2->first));
  BOOST_CHECK(iter1.same(iter2));
}
...
BOOST_AUTO_TEST_CASE(
  ...
  iterator_type iter2 =
      iterator_type(boost::ref(stepper), empty_system(), x2, 0.0, 1.0, 0.2);
...
BOOST_AUTO_TEST_CASE(stepper_iterator_factory) {
  ...
  std::for_each(make_adaptive_time_iterator_begin(stepper, boost::ref(system),
                                                  x, 0.0, 1.0, 0.1),
...
BOOST_AUTO_TEST_CASE(stepper_range) {
  ...
  boost::for_each(
      make_adaptive_time_range(stepper, boost::ref(system), x, 0.0, 1.0, 0.1),
      dummy_observer());
...
BOOST_AUTO_TEST_CASE(stepper_iterator_with_reference_wrapper_factory) {
  ...
  std::for_each(make_adaptive_time_iterator_begin(
                    boost::ref(stepper), boost::ref(system), x, 0.0, 1.0, 0.1),
                make_adaptive_time_iterator_end(boost::ref(stepper),
                                                boost::ref(system), x),
                dummy_observer());
...
BOOST_AUTO_TEST_CASE(stepper_range_with_reference_wrapper) {
  ...
  boost::for_each(
      make_adaptive_time_range(boost::ref(stepper), boost::ref(system), x, 0.0,
                               1.0, 0.1),
      dummy_observer());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(transitivity1, Stepper, dummy_steppers) {
  typedef adaptive_time_iterator<Stepper, empty_system, state_type>
      stepper_iterator;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_factory, Stepper,
  ...
  state_type x = {{1.0}};
  result_vector res;
  std::copy(make_adaptive_time_iterator_begin(Stepper(), empty_system(), x, 0.0,
                                              0.35, 0.1),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_range_factory, Stepper,
  ...
  result_vector res;
  boost::range::copy(
      make_adaptive_time_range(Stepper(), empty_system(), x, 0.0, 0.35, 0.1),
      std::back_insert_iterator<result_vector>(res));
