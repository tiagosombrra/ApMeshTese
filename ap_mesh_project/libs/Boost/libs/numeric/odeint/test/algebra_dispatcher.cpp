...
BOOST_AUTO_TEST_CASE(range_algebra_with_vector) {
  ...
  BOOST_STATIC_ASSERT(
      (boost::is_same<stepper_type::algebra_type, range_algebra>::value));
}

BOOST_AUTO_TEST_CASE(array_algebra_with_array) {
  typedef runge_kutta4<boost::array<double, 2> > stepper_type;
  BOOST_STATIC_ASSERT(
      (boost::is_same<stepper_type::algebra_type, array_algebra>::value));
}

BOOST_AUTO_TEST_CASE(range_algebra_with_array) {
  typedef runge_kutta4<boost::array<double, 2>, double, boost::array<double, 2>,
                       double, range_algebra>
      stepper_type;
  BOOST_STATIC_ASSERT(
      (boost::is_same<stepper_type::algebra_type, range_algebra>::value));
}
...
BOOST_AUTO_TEST_CASE(fusion_algebra_with_fusion_vector) {
  ...
  BOOST_STATIC_ASSERT(
      (boost::is_same<stepper_type::algebra_type, fusion_algebra>::value));
}
...
typedef boost::mpl::list<float, double, long double, std::complex<double>,
                         std::complex<float> >
    fp_types;
BOOST_AUTO_TEST_CASE_TEMPLATE(vector_space_algebra_with_floating_point, T,
                              fp_types) {
  typedef runge_kutta_fehlberg78<T> stepper_type;
  BOOST_STATIC_ASSERT((boost::is_same<typename stepper_type::algebra_type,
                                      vector_space_algebra>::value));
}
