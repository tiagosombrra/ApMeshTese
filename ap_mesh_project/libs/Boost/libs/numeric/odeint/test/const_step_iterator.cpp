...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_stepper_iterator, Stepper, dummy_steppers) {
  ...
  state_type x = {{1.0}};
  iterator_type iter1 =
      iterator_type(Stepper(), empty_system(), x, 0.0, 0.999, 0.1);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(assignment_stepper_iterator, Stepper,
  ...
  iterator_type iter1 =
      iterator_type(Stepper(), empty_system(), x1, 0.0, 1.0, 0.1);
  iterator_type iter2 =
      iterator_type(Stepper(), empty_system(), x2, 0.0, 1.0, 0.2);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_iterator_factory, Stepper,
  ...
  std::for_each(make_const_step_iterator_begin(stepper, boost::ref(system), x,
                                               0.0, 1.0, 0.1),
                make_const_step_iterator_end(stepper, boost::ref(system), x),
                dummy_observer());

  // dummy_steppers just add 0.25 at each step, the above for_each leads to 10
  // do_step calls so x should be 3.5
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_range, Stepper, dummy_steppers) {
  ...
  boost::for_each(
      make_const_step_range(stepper, boost::ref(system), x, 0.0, 1.0, 0.1),
      dummy_observer());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_iterator_with_reference_wrapper_factory,
  ...
  std::for_each(
      make_const_step_iterator_begin(boost::ref(stepper), boost::ref(system), x,
                                     0.0, 1.0, 0.1),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_range_with_reference_wrapper, Stepper,
  ...
  boost::for_each(make_const_step_range(boost::ref(stepper), boost::ref(system),
                                        x, 0.0, 1.0, 0.1),
                  dummy_observer());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(transitivity1, Stepper, dummy_steppers) {
  typedef const_step_iterator<Stepper, empty_system, state_type>
      stepper_iterator;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm, Stepper, dummy_steppers) {
  ...
  BOOST_CHECK_CLOSE(x[0], 1.75,
                    1.0e-14);  // the iterator should not iterate over the end
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_factory, Stepper,
  ...
  state_type x = {{1.0}};
  std::vector<state_type> res;
  std::copy(make_const_step_iterator_begin(Stepper(), empty_system(), x, 0.0,
                                           0.35, 0.1),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_range_factory, Stepper,
  ...
  std::vector<state_type> res;
  boost::range::copy(
      make_const_step_range(Stepper(), empty_system(), x, 0.0, 0.35, 0.1),
      std::back_insert_iterator<std::vector<state_type> >(res));
