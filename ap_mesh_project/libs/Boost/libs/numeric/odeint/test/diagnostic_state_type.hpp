...
struct same_size_impl<diagnostic_type<N>, diagnostic_type<M> > {
  static bool same_size(const diagnostic_type<N> &x1,
                        const diagnostic_type<M> &x2) {
    return false;
  }
};
...
struct state_wrapper<diagnostic_type<N> > {
  ...
  state_wrapper() : m_v() { counter<N>::construct_count()++; }
  ...
  ~state_wrapper() { counter<N>::destroy_count()++; }
};
