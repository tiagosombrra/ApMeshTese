...
BOOST_AUTO_TEST_CASE(test_integrate_times_overshoot) {
  ...
  for (int i = 0; i < 10; ++i) times[i] = 1.0 - i * 1.0 / 9.0;

  std::cout << "test rk4 stepper" << std::endl;
  // simple stepper
  std::vector<double> obs_times;
  int steps =
      integrate_times(runge_kutta4<state_type>(), lorenz, x, times.begin(),
                      times.end(), dt, push_back_time(obs_times));
// different behavior for the iterator based integrate implementaton
...
#endif
  for (int i = 0; i < 10; ++i) BOOST_CHECK_EQUAL(times[i], obs_times[i]);

  std::cout << "test rk_ck stepper" << std::endl;
  // controlled stepper
  obs_times.clear();
  integrate_times(
      controlled_runge_kutta<runge_kutta_cash_karp54<state_type> >(), lorenz, x,
      times.begin(), times.end(), dt, push_back_time(obs_times));
  for (int i = 0; i < 10; ++i) BOOST_CHECK_EQUAL(times[i], obs_times[i]);
  ...
  integrate_times(
      dense_output_runge_kutta<
          controlled_runge_kutta<runge_kutta_dopri5<state_type> > >(),
      lorenz, x, times.begin(), times.end(), dt, push_back_time(obs_times));
  for (int i = 0; i < 10; ++i) BOOST_CHECK_EQUAL(times[i], obs_times[i]);
}
