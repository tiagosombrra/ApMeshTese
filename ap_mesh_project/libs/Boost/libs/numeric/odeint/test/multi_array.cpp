...
struct vector_ode {
  ...
  void operator()(const vector_type &x, vector_type &dxdt, double t) const {
    ...
    for (size_t i = 0; i < n; ++i) dxdt[i] = x[i - 1] - 2.0 * x[i] + x[i + 1];
...
BOOST_AUTO_TEST_CASE(test_rk4_vector) {
  vector_type v1(
      boost::extents[vector_type::extent_range(-1, vector_ode::n + 1)]);
  integrate_const(runge_kutta4<vector_type>(), vector_ode(), v1, 0.0, 1.0,
                  0.01);
}
...
BOOST_AUTO_TEST_CASE(test_dopri5_vector) {
  vector_type v1(
      boost::extents[vector_type::extent_range(-1, vector_ode::n + 1)]);
  integrate_const(
      make_dense_output(1.0e-6, 1.0e-6, runge_kutta_dopri5<vector_type>()),
      vector_ode(), v1, 0.0, 1.0, 0.01);
}
