...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_stepper_iterator, Stepper, dummy_steppers) {
  ...
  state_type x = {{1.0}};
  iterator_type iter1 =
      iterator_type(Stepper(), empty_system(), x, 0.0, 0.1, 10);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(assignment_stepper_iterator, Stepper,
  ...
  iterator_type iter1 =
      iterator_type(Stepper(), empty_system(), x1, 0.0, 0.1, 10);
  iterator_type iter2 =
      iterator_type(Stepper(), empty_system(), x2, 0.0, 0.2, 10);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_range, Stepper, dummy_steppers) {
  ...
  boost::for_each(
      make_n_step_range(stepper, boost::ref(system), x, 0.0, 0.1, 10),
      dummy_observer());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_iterator_with_reference_wrapper_factory,
  ...
  std::for_each(
      make_n_step_iterator_begin(boost::ref(stepper), boost::ref(system), x,
                                 0.0, 0.1, 10),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(stepper_range_with_reference_wrapper, Stepper,
  ...
  boost::for_each(make_n_step_range(boost::ref(stepper), boost::ref(system), x,
                                    0.0, 0.1, 10),
                  dummy_observer());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_factory, Stepper,
  ...
  std::vector<state_type> res;
  std::copy(
      make_n_step_iterator_begin(Stepper(), empty_system(), x, 0.0, 0.1, 3),
...
BOOST_AUTO_TEST_CASE_TEMPLATE(copy_algorithm_with_range_factory, Stepper,
  ...
  std::vector<state_type> res;
  boost::range::copy(
      make_n_step_range(Stepper(), empty_system(), x, 0.0, 0.1, 3),
      std::back_insert_iterator<std::vector<state_type> >(res));
