...
struct perform_adams_bashforth_moulton_test {
  void operator()(void) {
    ...
    // is increased we use the rk78 as initializing stepper
    ...
    stepper.do_step(osc(), x1, t, dt);
    // only examine the error of the adams-bashforth-moulton step, not the
    // initialization
...
typedef mpl::vector<adams_bashforth_moulton<1, state_type>,
