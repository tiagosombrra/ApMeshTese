...
struct perform_adaptive_adams_bashforth_moulton_test {
  void operator()(void) {
    ...
    double t = 0.0;
    double dt = 0.25;
    // initialization, does a number of steps to self-start the stepper with a
    // small stepsize
...
typedef mpl::vector<adaptive_adams_bashforth_moulton<2, state_type>,
...
BOOST_AUTO_TEST_CASE_TEMPLATE(adaptive_adams_bashforth_moulton_test, Stepper,
                              adaptive_adams_bashforth_moulton_steppers) {
