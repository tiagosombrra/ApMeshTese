...
struct stepper_order_test {
  ...
  int estimate_order(int steps) {
    ...
    for (p = 0; true; p++) {
      ...
      double t = integrate_n_steps(Stepper(), monomial(p), x, 0.0, dt, steps);
      if (fabs(x - pow(t, (1.0 + p))) > tolerance) break;
    }
...
BOOST_AUTO_TEST_CASE_TEMPLATE(adams_bashforth_moultion_test, Stepper,
                              abm_steppers) {
  stepper_order_test<Stepper> tester;
  tester(16);
}
