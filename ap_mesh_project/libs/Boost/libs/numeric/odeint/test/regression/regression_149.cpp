...
struct perform_test {
  void operator()(void) {
    ...
    print(boost::find_if(
        make_adaptive_time_range(stepper, rhs, x, 0.0, 1.0, 0.01), pred));
  }
...
typedef mpl::vector<
    euler<state_type>, runge_kutta4<state_type>,
    runge_kutta_cash_karp54<state_type>, runge_kutta_dopri5<state_type>,
    runge_kutta_fehlberg78<state_type>, bulirsch_stoer<state_type> >
    steppers;

BOOST_AUTO_TEST_SUITE(regression_147_test)

BOOST_AUTO_TEST_CASE_TEMPLATE(regression_147_test, Stepper, steppers) {
