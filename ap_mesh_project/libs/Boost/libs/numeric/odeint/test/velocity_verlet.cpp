...
struct ode {
  ...
  void operator()(const CoorIn &q, const MomentumIn &p, AccelerationOut &a,
                  Time t) const {
    a[0] = -q[0] - p[0];
    a[1] = -q[1] - p[1];
    ++ode_call_count;
...
struct ode_units {
  void operator()(coor_vector const &q, velocity_vector const &p,
                  accelartion_vector &a, time_type t) const {
    const units::quantity<si::frequency, value_type> omega = 1.0 * si::hertz;
    const units::quantity<si::frequency, value_type> friction =
        0.001 * si::hertz;
    fusion::at_c<0>(a) =
        omega * omega * fusion::at_c<0>(q) - friction * fusion::at_c<0>(p);
    fusion::at_c<1>(a) =
        omega * omega * fusion::at_c<1>(q) - friction * fusion::at_c<0>(p);
    ++ode_call_count;
  }
};
...
struct get_resizer_test_stepper {
  typedef velocity_verlet<test_array_type, test_array_type, double,
                          test_array_type, double, double, range_algebra,
                          default_operations, Resizer>
      type;
};
...
BOOST_FIXTURE_TEST_CASE(test_with_array_ref, velocity_verlet_fixture) {
  ...
  array_type q, p;
  init_state(q, p);
  stepper.do_step(ode(), std::make_pair(boost::ref(q), boost::ref(p)), 0.0,
                  0.01);
...
BOOST_FIXTURE_TEST_CASE(test_always_resizer, velocity_verlet_fixture) {
  ...
  BOOST_CHECK_EQUAL(ode_call_count,
                    size_t(4));  // attention: one more system call, since the
                                 // size of the state has been changed
}
...
BOOST_FIXTURE_TEST_CASE(test_with_unit_pair, velocity_verlet_fixture) {
  typedef velocity_verlet<coor_vector, velocity_vector, value_type,
                          accelartion_vector, time_type, time_2_type,
                          fusion_algebra, default_operations>
      stepper_type;
...
BOOST_FIXTURE_TEST_CASE(test_with_unit_ref, velocity_verlet_fixture) {
  ...
  stepper_type stepper;
  stepper.do_step(ode_units(), std::make_pair(boost::ref(q), boost::ref(p)),
                  0.0 * si::second, 0.01 * si::second);
  BOOST_CHECK_EQUAL(ode_call_count, size_t(2));
}
