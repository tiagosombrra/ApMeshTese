...
#define TEST(z, n, text)                                              \
  BOOST_CHECK_SMALL(                                                  \
      y0(BOOST_PP_ADD(n, 1)) - T(2 BOOST_PP_REPEAT(text, SUM, text)), \
      T(1e-10));

#define TEST_CASE(z, n, text)                                              \
  BOOST_AUTO_TEST_CASE_TEMPLATE(BOOST_PP_CAT(odeint_foreach, n), T,        \
                                fp_types) {                                \
    vector_space_algebra algebra;                                          \
    BOOST_PP_REPEAT(BOOST_PP_ADD(n, 2), TABLE, tt)                         \
    BOOST_PP_CAT(algebra.for_each, BOOST_PP_ADD(n, 2))                     \
    (BOOST_PP_ENUM_PARAMS(BOOST_PP_ADD(n, 2), y),                          \
     default_operations::BOOST_PP_CAT(scale_sum, BOOST_PP_ADD(n, 1)) < T > \
