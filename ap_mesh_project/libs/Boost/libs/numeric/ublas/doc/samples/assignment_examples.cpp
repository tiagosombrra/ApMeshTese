...
int main() {
  ...
  A <<= move(0, 2), traverse_policy::by_column(), a;
  std::cout << A << std::endl;
  // [ 0 1 0 ]
  // [ 3 4 1 ]
  // [ 0 1 2 ]
  ...
  // If for performance(Benchmarks are not definite about that) or consistency
  // reasons you need to disable wraping:
  static next_row_manip endr;  // This can be defined globally
  A <<= traverse_policy::by_row_no_wrap(), 1, 2, 3, endr, 4, 5, 6, endr, 7, 8,
      9, endr;
  // [ 1 2 3 ]
  // [ 4 5 6 ]
  // [ 7 8 9 ]
  ...
  A <<= fill_policy::index_plus_assign(), 3, 2, 1;
  std::cout << A << std::endl;
  // [ 4 4 4 ]
  // [ 4 5 6 ]
  // [ 7 8 9 ]
  ...
  // Watch the difference when assigning matrices for different traverse
  // policies:
  matrix<double> BR(9, 9, 0);
  BR <<= traverse_policy::by_row(),  // This is the default, so this might as
                                     // well be omitted.
      A, A, A;
  ...
  BC <<= traverse_policy::by_column(), A, A, A;
  std::cout << BC << std::endl;
  // [ A 0 0]
  // [ A 0 0]
  // [ A 0 0]

  // The following will throw a run-time exception in debug mode (matrix
  // mid-assignment wrap is not allowed) : matrix<double> C(7,7); C <<= A, A, A;
  ...
  matrix<double> D(6, 6);
  D <<= A, zero_matrix<double>(3, 3), zero_matrix<double>(3, 3), A;
  // [ A 0 ]
  // [ 0 A ]
  ...
  E <<= 1, 2, next_row(), 3, 4, next_column(), 5;
  std::cout << E << std::endl;
  // [ 1 2 0 5 ]
  // [ 3 4 0 0 ]

  // The begin1 (moves to the begining of the column) index manipulator, begin2
  // does the same for the row:
  matrix<double> F(2, 4, 0);
  F <<= 1, 2, next_row(), 3, 4, begin1(), 5;
  std::cout << F << std::endl;
  // [ 1 2 5 0 ]
  // [ 3 4 0 0 ]

  // The move (relative) and move_to(absolute) index manipulators (probably the
  // most useful manipulators):
  matrix<double> G(2, 4, 0);
  G <<= 1, 2, move(0, 1), 3, move_to(1, 3), 4;
  std::cout << G << std::endl;
  // [ 1 2 0 3 ]
  // [ 0 0 0 4 ]
  ...
  Gs <<= 1, 2, move<0, 1>(), 3, move_to<1, 3>(), 4;
  ...
  H <<= traverse_policy::by_column(), 1, 2, 3, 4, 5, 6, 7, 8;
  std::cout << H << std::endl;
  // [ 1 3 5 7 ]
  // [ 2 4 6 8 ]
  ...
  // For compressed and coordinate matrix types a push_back or insert fill
  // policy can be chosen for faster assginment:
  compressed_matrix<double> I(2, 2);
  I <<= fill_policy::sparse_push_back(), 0, 1, 2, 3;
  std::cout << I << std::endl;
  // [ 0 1 ]
  // [ 2 3 ]

  coordinate_matrix<double> J(2, 2);
  J <<= fill_policy::sparse_insert(), 1, 2, 3, 4;
  std::cout << J << std::endl;
  // [ 1 2 ]
  // [ 3 4 ]
  ...
  compressed_matrix<double> J2(4, 4);
  J2 <<= fill_policy::sparse_push_back(), J, fill_policy::sparse_insert(), J;
  ...
  // A different traverse policy doesn't change the result, only they order it
  // is been assigned.
  coordinate_matrix<double> L(3, 3);
  L <<= fill_policy::sparse_insert(), traverse_policy::by_column(), J;
  std::cout << L << std::endl;
  // (same as previous)
  // [ 1 2 0 ]
  // [ 3 4 0 ]
  ...
  for (cmst i = 0; i != size; i += J.size1()) M <<= spb, move_to(i, i), J;
  ...
  // A "repeat" trasverser may by provided so that this becomes faster and an
  // on-liner like: M <<= spb, repeat(0, size, J.size1(), 0, size, J.size1()),
  // J; An alternate would be to create a :repeater" matrix and vector
  // expression that can be used in other places as well. The latter is probably
  // better,
  return 0;
}
