...
int main() {
  ...
  auto vf = ftype(0);
  for (auto i = 0u; i < A.size(); ++i, vf += ftype(1)) A[i] = vf;
  ...
  // using a single-index
  auto vc = ctype(0, 0);
  for (auto i = 0u; i < B.size(); ++i, vc += ctype(1, 1)) B[i] = vc;
  ...
  std::transform(B.begin(), B.end(), D.begin(),
                 [](auto const& b) { return std::conj(b); });
  std::cout << "% --------------------------- " << std::endl;
  std::cout << "% --------------------------- " << std::endl << std::endl;
