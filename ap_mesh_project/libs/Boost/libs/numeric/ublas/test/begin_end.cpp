...
BOOST_UBLAS_TEST_DEF(test_vector_iteration) {
  ...
  for (boost::numeric::ublas::iterator_type<vector_type>::type it =
           boost::numeric::ublas::begin<vector_type>(v);
       it != boost::numeric::ublas::end<vector_type>(v); ++it) {
...
BOOST_UBLAS_TEST_DEF(test_vector_const_iteration) {
  ...
  vector_type::size_type ix = 0;
  for (boost::numeric::ublas::const_iterator_type<vector_type>::type it =
           boost::numeric::ublas::begin<vector_type>(v);
       it != boost::numeric::ublas::end<vector_type>(v); ++it) {
    BOOST_UBLAS_DEBUG_TRACE("*it = " << *it << " ==> " << v(ix));
    BOOST_UBLAS_TEST_CHECK(std::abs(*it - v(ix)) <= TOL);
...
BOOST_UBLAS_TEST_DEF(test_row_major_matrix_iteration) {
  ...
  typedef boost::numeric::ublas::matrix<value_type,
      ...
      matrix_type;
  typedef boost::numeric::ublas::iterator_type<
      matrix_type, boost::numeric::ublas::tag::major>::type outer_iterator_type;
  typedef boost::numeric::ublas::iterator_type<
      matrix_type, boost::numeric::ublas::tag::minor>::type inner_iterator_type;
  ...
  for (outer_iterator_type outer_it =
           boost::numeric::ublas::begin<boost::numeric::ublas::tag::major>(A);
       outer_it !=
       boost::numeric::ublas::end<boost::numeric::ublas::tag::major>(A);
       ++outer_it) {
    matrix_type::size_type col(0);

    for (inner_iterator_type inner_it = boost::numeric::ublas::begin(outer_it);
         inner_it != boost::numeric::ublas::end(outer_it); ++inner_it) {
