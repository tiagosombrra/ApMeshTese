...
inline static std::pair<unsigned, unsigned> getResults() {
...
struct promote_distance {
  typedef typename boost::mpl::if_c<boost::is_integral<T>::value, long double,
                                    T>::type type;
};
...
struct distance {
 private:
  typedef typename boost::numeric::ublas::promote_traits<
      typename M1::value_type, typename M2::value_type>::promote_type
      value_type;
...
template <class AE>
typename distance<AE>::type mean_square(
    const boost::numeric::ublas::matrix_expression<AE>& me) {
  typename distance<AE>::type s(0);
  typename AE::size_type i, j;
  for (i = 0; i != me().size1(); i++) {
    for (j = 0; j != me().size2(); j++) {
      s += boost::numeric::ublas::scalar_traits<
          typename AE::value_type>::type_abs(me()(i, j));
    }
  }
...
template <class AE>
typename distance<AE>::type mean_square(
    const boost::numeric::ublas::vector_expression<AE>& ve) {
  // We could have use norm2 here, but ublas' ABS does not support unsigned
  // types.
  typename distance<AE>::type s(0);
  typename AE::size_type i;
  for (i = 0; i != ve().size(); i++) {
    s +=
        boost::numeric::ublas::scalar_traits<typename AE::value_type>::type_abs(
            ve()(i));
  }
  return s / ve().size();
}
