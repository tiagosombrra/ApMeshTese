...
BOOST_UBLAS_TEST_DEF(test_row_major_matrix_container) {
  ...
  BOOST_UBLAS_DEBUG_TRACE("num_rows(A) = " << boost::numeric::ublas::num_rows(A)
                                           << " ==> " << A.size1());
...
BOOST_UBLAS_TEST_DEF(test_matrix_expression) {
  ...
  BOOST_UBLAS_DEBUG_TRACE(
      "num_rows(A') = " << boost::numeric::ublas::num_rows(
                               boost::numeric::ublas::trans(A))
                        << " ==> " << boost::numeric::ublas::trans(A).size1());
  BOOST_UBLAS_TEST_CHECK(
      boost::numeric::ublas::num_rows(boost::numeric::ublas::trans(A)) ==
      boost::numeric::ublas::trans(A).size1());
}
...
BOOST_UBLAS_TEST_DEF(test_matrix_reference) {
  ...
  BOOST_UBLAS_DEBUG_TRACE(
      "num_rows(reference(A)) = "
      << boost::numeric::ublas::num_rows(matrix_reference_type(A)) << " ==> "
      << matrix_reference_type(A).size1());
  BOOST_UBLAS_TEST_CHECK(boost::numeric::ublas::num_rows(matrix_reference_type(
