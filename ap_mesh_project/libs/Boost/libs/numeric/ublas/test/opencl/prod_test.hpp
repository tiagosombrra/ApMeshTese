  ...
  void run() {
    ...
    for (int i = 0; i < number_of_tests; i++) {
      ...
      if ((!test::compare(resultUBLAS, resultOPENCL)) ||
          (!test::compare(result_vector_opencl_mv, result_vector_ublas_mv)) ||
          (!test::compare(result_vector_opencl_vm, result_vector_ublas_vm))) {
    ...
    std::cout << "All is well (matrix opencl prod) of " << typeid(T).name()
              << std::endl;
  }
};
