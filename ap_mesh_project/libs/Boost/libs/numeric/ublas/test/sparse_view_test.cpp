...
BOOST_AUTO_TEST_CASE(test_construction_and_basic_operations) {
  ...
  std::cout << (ublas::make_compressed_matrix_view<ublas::row_major, IB>(
                   3, 4, NNZ, IA, JA, VA))
            << std::endl;

  typedef ublas::compressed_matrix_view<ublas::row_major, IB, unsigned int[4],
                                        unsigned int[NNZ], double[NNZ]>
      COMPMATVIEW;
...
BOOST_AUTO_TEST_CASE(test_construction_from_pointers) {
  std::cout << (ublas::make_compressed_matrix_view<ublas::column_major, IB>(
                   4, 3, NNZ,
                   ublas::c_array_view<const unsigned int>(4, &(IA[0])),
                   ublas::c_array_view<const unsigned int>(6, &(JA[0])),
                   ublas::c_array_view<const double>(6, &(VA[0]))))
            << std::endl;
  ...
  typedef ublas::compressed_matrix_view<
      ublas::column_major, IB, ublas::c_array_view<unsigned int>,
      ublas::c_array_view<unsigned int>, ublas::c_array_view<double> >
      COMPMATVIEW;

  COMPMATVIEW viewA(4, 3, NNZ, ublas::c_array_view<unsigned int>(4, ia),
