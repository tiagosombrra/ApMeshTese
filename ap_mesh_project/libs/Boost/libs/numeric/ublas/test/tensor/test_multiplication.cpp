...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_mtv, value, test_types, fixture) {
  ...
  for (auto const& na : extents) {
    ...
    for (auto m = 0u; m < na.size(); ++m) {
      ...
      auto nc_base =
          extents_type_base(std::max(na.size() - 1, size_type{2}), 1);
      ...
      ublas::detail::recursive::mtv(size_type(m), c.data(), nc.data(),
                                    wc.data(), a.data(), na.data(), wa.data(),
                                    b.data());
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_mtm, value, test_types, fixture) {
  ...
  for (auto const& na : extents) {
    ...
    ublas::detail::recursive::mtm(c.data(), nc.data(), wc.data(), a.data(),
                                  na.data(), wa.data(), b.data(), nb.data(),
                                  wb.data());
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ttv, value, test_types, fixture) {
  ...
  for (auto const& na : extents) {
    ...
    for (auto m = 0u; m < na.size(); ++m) {
      ...
      ublas::ttv(size_type(m + 1), na.size(), c.data(), nc.data(), wc.data(),
                 a.data(), na.data(), wa.data(), b.data(), nb.data(),
                 wb.data());
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ttm, value, test_types, fixture) {
  ...
  for (auto const& na : extents) {
    ...
    for (auto m = 0u; m < na.size(); ++m) {
      ...
      ublas::ttm(size_type(m + 1), na.size(), c.data(), nc.data(), wc.data(),
                 a.data(), na.data(), wa.data(), b.data(), nb.data(),
                 wb.data());
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ttt_permutation, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  auto compute_factorial = [](auto const& p) {
    auto f = 1ul;
    for (auto i = 1u; i <= p; ++i) f *= i;
    return f;
  };

  auto compute_inverse_permutation = [](auto const& pi) {
    auto pi_inv = pi;
    for (auto j = 0u; j < pi.size(); ++j) pi_inv[pi[j] - 1] = j + 1;
    return pi_inv;
  };
  ...
  auto permute_extents = [](auto const& pi, auto const& na) {
    ...
    for (auto j = 0u; j < pi.size(); ++j) nb[j] = na[pi[j] - 1];
    return nb;
  };
  ...
  for (auto const& na : extents) {
    ...
    for (auto i = 0u; i < f; ++i) {
      ...
      for (auto q = size_type(0); q <= pa; ++q) {
        ...
        for (auto i = 0u; i < r; ++i) nc_base[i] = na[pia[i] - 1];

        for (auto i = 0u; i < s; ++i) nc_base[r + i] = nb[pib_inv[i] - 1];

        auto nc = extents_type(nc_base);
        auto wc = strides_type(nc);
        auto c = vector_type(nc.product(), value_type(0));

        ublas::ttt(pa, pb, q, pia.data(), pib_inv.data(), c.data(), nc.data(),
                   wc.data(), a.data(), na.data(), wa.data(), b.data(),
                   nb.data(), wb.data());

        auto acc = value_type(1);
        for (auto i = r; i < pa; ++i) acc *= value_type(na[pia[i] - 1]);
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ttt, value, test_types, fixture) {
  ...
  for (auto const& na : extents) {
    ...
    //		std::cout << "na = ";
    //		std::copy(na.begin(), na.end(),
    //std::ostream_iterator<size_type>(std::cout, " ")); 		std::cout << std::endl;
    ...
    for (auto q = size_type(0); q <= pa; ++q) {  // pa
      ...
      for (auto i = 0u; i < r; ++i) nc_base[i] = na[i];

      for (auto i = 0u; i < s; ++i) nc_base[r + i] = nb[i];
      ...
      //			std::cout << "nc = ";
      //			std::copy(nc.begin(), nc.end(),
      //std::ostream_iterator<size_type>(std::cout, " ")); 			std::cout <<
      //std::endl;

      ublas::ttt(pa, pb, q, c.data(), nc.data(), wc.data(), a.data(), na.data(),
                 wa.data(), b.data(), nb.data(), wb.data());

      auto acc = value_type(1);
      for (auto i = r; i < pa; ++i) acc *= value_type(na[i]);
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_inner, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  for (auto const& n : extents) {
    ...
    auto c = ublas::inner(n.size(), n.data(), a.data(), w.data(), b.data(),
                          w.data(), value_type(0));
    auto cref =
        std::inner_product(a.begin(), a.end(), b.begin(), value_type(0));

    BOOST_CHECK_EQUAL(c, cref);
  }
}

BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_outer, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  for (auto const& na : extents) {
    ...
    for (auto const& nb : extents) {
      ...
      for (auto i = 0u; i < na.size(); ++i) nc[i] = na[i];
      for (auto i = 0u; i < nb.size(); ++i) nc[i + na.size()] = nb[i];

      auto wc = strides_type(extents_type(nc));

      ublas::outer(c.data(), nc.size(), nc.data(), wc.data(), a.data(),
                   na.size(), na.data(), wa.data(), b.data(), nb.size(),
                   nb.data(), wb.data());

      for (auto const& cc : c) BOOST_CHECK_EQUAL(cc, a[0] * b[0]);
    }
  }
}
