...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ctor_extents, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_copy_ctor, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  auto check = [](auto const& e) {
    ...
    for (auto i = 0ul; i < t.size(); ++i) BOOST_CHECK_EQUAL(t[i], r[i]);
  };

  for (auto const& e : extents) check(e);
}

BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_copy_ctor_layout, value,
                                 test_types, fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  using tensor_type = ublas::tensor<value_type, layout_type>;
  using other_layout_type =
      std::conditional_t<std::is_same<ublas::first_order, layout_type>::value,
                         ublas::last_order, ublas::first_order>;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_copy_move_ctor, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ctor_extents_init, value,
                                 test_types, fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  for (auto const& e : extents) {
    ...
    for (auto i = 0ul; i < t.size(); ++i) BOOST_CHECK_EQUAL(t[i], r);
  }
}

BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_ctor_extents_array, value,
                                 test_types, fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_read_write_single_index_access,
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_read_write_multi_index_access_at,
                                 value, test_types, fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  auto check3 = [](const tensor_type& t) {
    ...
    using op_type =
        std::conditional_t<std::is_same_v<layout_type, ublas::first_order>,
                           std::minus<>, std::plus<>>;
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_reshape, value, test_types,
                                 fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
  ...
  for (auto const& efrom : extents) {
    for (auto const& eto : extents) {
      ...
      for (auto i = 0ul; i < t.size(); ++i) BOOST_CHECK_EQUAL(t[i], v);
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_swap, value, test_types, fixture) {
  ...
  for (auto const& e_t : extents) {
    for (auto const& e_r : extents) {
      ...
      for (auto i = 0ul; i < t.size(); ++i) BOOST_CHECK_EQUAL(t[i], w);

      BOOST_CHECK_EQUAL(t.size(), e_r.product());
      BOOST_CHECK_EQUAL(t.rank(), e_r.size());
      BOOST_CHECK(t.extents() == e_r);

      for (auto i = 0ul; i < r.size(); ++i) BOOST_CHECK_EQUAL(r[i], v);
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_standard_iterator, value,
                                 test_types, fixture) {
  using namespace boost::numeric;
  using value_type = typename value::first_type;
  using layout_type = typename value::second_type;
