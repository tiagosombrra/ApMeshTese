...
bool check_sortedness(
    const boost::numeric::ublas::coordinate_matrix<T>& matrix) {
  bool result = true;
  ...
  for (typename matrix_type::array_size_type i = 0; i + 1 < size && result;
       ++i) {
    result &=
        ((i1[i] < i1[i + 1]) || ((i1[i] == i1[i]) && (i2[i] < i2[i + 1])));
  }
  return result;
}
...
BOOST_UBLAS_TEST_DEF(test_coordinate_matrix_inplace_merge_random) {
  ...
  for (size_t repeats = 1; repeats < max_repeats; ++repeats) {
    for (size_t size = 1; size < max_size; size += 5) {
      ...
      boost::numeric::ublas::coordinate_matrix<double> matrix_coord(size_x,
                                                                    size_y);
...

      {
        ...
        BOOST_UBLAS_TEST_CHECK(
            compare_distance(matrix_coord, matrix_dense, TOL));
      }
