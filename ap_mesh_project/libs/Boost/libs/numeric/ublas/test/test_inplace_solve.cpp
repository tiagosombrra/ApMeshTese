...
BOOST_UBLAS_TEST_DEF(test_inplace_solve) {
  ...
  // triangular_adaptor makes A triangular for comparison.
  {
    ...
    BOOST_UBLAS_TEST_CHECK(
        diff(ublas::triangular_adaptor<mat, ublas::lower>(A), x, b) < TOL);
  }
  {
    ublas::vector<double> x(b);
    ublas::inplace_solve(A, x, ublas::upper_tag());
    BOOST_UBLAS_TEST_CHECK(
        diff(ublas::triangular_adaptor<mat, ublas::upper>(A), x, b) < TOL);
  }
  {
    ublas::vector<double> x(b);
    ublas::inplace_solve(x, A, ublas::lower_tag());
    BOOST_UBLAS_TEST_CHECK(
        diff(trans(ublas::triangular_adaptor<mat, ublas::lower>(A)), x, b) <
        TOL);
  }
  {
    ublas::vector<double> x(b);
    ...
    BOOST_UBLAS_TEST_CHECK(
        diff(trans(ublas::triangular_adaptor<mat, ublas::upper>(A)), x, b) <
        TOL);
  }
}
...
int main() {
  ...
  typedef ublas::mapped_vector_of_mapped_vector<double, ublas::row_major>
      mvmv_doub_rowmaj;
  typedef ublas::mapped_vector_of_mapped_vector<double, ublas::column_major>
      mvmv_doub_colmaj;
