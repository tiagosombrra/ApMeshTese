...
void guardSparsePreserveResize(
    Vector &vec, typename Vector::size_type new_size,
    StorageCategory)  // Because sparse matrices don't have preserve data
                      // implemented
{
  vec.resize(new_size);
}
...
void guardSparsePreserveResize(
    Vector &vec, typename Vector::size_type new_size,
    sparse_tag)  // Because sparse matrices don't have preserve data implemented
{
...
bool test_matrix_row_facade() {
  ...
  {  // Testing resize
    ...
    guardSparsePreserveResize(rows, new_num_rows,
                              typename Matrix::storage_category());
  ...
  {  // Testing iterator
    ...
    typename RowVector::size_type i = 0;
    for (typename RowVector::iterator iter = rows.begin(); iter != rows.end();
         iter++) {
...
bool test_matrix_column_facade() {
  ...
  {  // Testing resize
    ...
    guardSparsePreserveResize(columns, new_num_cols,
                              typename Matrix::storage_category());
  ...
  {  // Testing const reverse iterator
    BOOST_UBLAS_DEBUG_TRACE(
        "test_matrix_column_facade: const reverese iterator");
...
BOOST_UBLAS_TEST_DEF(test_matrix_row_facade) {
  ...
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_row_facade<bounded_matrix<double, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_row_facade<bounded_matrix<float, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_row_facade<bounded_matrix<long, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_row_facade<bounded_matrix<unsigned long, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_row_facade<bounded_matrix<int, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_row_facade<bounded_matrix<unsigned int, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_row_facade<bounded_matrix<char, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_row_facade<bounded_matrix<std::size_t, 7, 7> >()));
  ...
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_row_facade<mapped_matrix<unsigned long> >());
  ...
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_row_facade<compressed_matrix<unsigned long> >());
  BOOST_UBLAS_TEST_CHECK(test_matrix_row_facade<compressed_matrix<int> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_row_facade<compressed_matrix<unsigned int> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_row_facade<compressed_matrix<std::size_t> >());
  ...
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_row_facade<coordinate_matrix<unsigned long> >())
  BOOST_UBLAS_TEST_CHECK(test_matrix_row_facade<coordinate_matrix<int> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_row_facade<coordinate_matrix<unsigned int> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_row_facade<coordinate_matrix<std::size_t> >());
...
BOOST_UBLAS_TEST_DEF(test_matrix_column_facade) {
  ...
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_column_facade<bounded_matrix<double, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_column_facade<bounded_matrix<float, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_column_facade<bounded_matrix<long, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_column_facade<bounded_matrix<unsigned long, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_column_facade<bounded_matrix<int, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_column_facade<bounded_matrix<unsigned int, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_column_facade<bounded_matrix<char, 7, 7> >()));
  BOOST_UBLAS_TEST_CHECK(
      (test_matrix_column_facade<bounded_matrix<std::size_t, 7, 7> >()));
  ...
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<mapped_matrix<unsigned long> >());
  BOOST_UBLAS_TEST_CHECK(test_matrix_column_facade<mapped_matrix<int> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<mapped_matrix<unsigned int> >())
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<mapped_matrix<std::size_t> >())
  BOOST_UBLAS_TEST_CHECK(test_matrix_column_facade<mapped_matrix<char> >());

  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<compressed_matrix<double> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<compressed_matrix<float> >());
  BOOST_UBLAS_TEST_CHECK(test_matrix_column_facade<compressed_matrix<long> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<compressed_matrix<unsigned long> >());
  BOOST_UBLAS_TEST_CHECK(test_matrix_column_facade<compressed_matrix<int> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<compressed_matrix<unsigned int> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<compressed_matrix<std::size_t> >());
  BOOST_UBLAS_TEST_CHECK(test_matrix_column_facade<compressed_matrix<char> >());

  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<coordinate_matrix<double> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<coordinate_matrix<float> >());
  BOOST_UBLAS_TEST_CHECK(test_matrix_column_facade<coordinate_matrix<long> >())
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<coordinate_matrix<unsigned long> >())
  BOOST_UBLAS_TEST_CHECK(test_matrix_column_facade<coordinate_matrix<int> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<coordinate_matrix<unsigned int> >());
  BOOST_UBLAS_TEST_CHECK(
      test_matrix_column_facade<coordinate_matrix<std::size_t> >());
