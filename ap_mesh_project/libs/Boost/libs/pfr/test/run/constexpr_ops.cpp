...
constexpr bool operator<(test_union l, test_union r) noexcept {
  return l.i < r.i;
}
constexpr bool operator<=(test_union l, test_union r) noexcept {
  return l.i <= r.i;
...
int main() {
  ...
  // pfr/detail/functional.hpp(21): note: failure was caused by a read of a
  // variable outside its lifetime
