...
std::ostream& operator<<(std::ostream& os, my_constexpr) { return os << "{}"; }
...
int main() {
  ...
  std::stringstream ss;
  boost::pfr::for_each_field(reg{42, 'a', {}, color::green, "hello world!"},
                             [&ss](auto&& val, std::size_t i) {
                               if (i) {
                                 ss << ", ";
                               }
                               ss << val;
  ...
  std::cout << '\n';
  boost::pfr::for_each_field(simple{42, 'a', 3},
                             [&ss](auto&& val) { ss << val << ' '; });
