...
void test_empty_struct() { BOOST_TEST_EQ(empty{}, empty{}); }
...
void test_with_contatiners() {
  std::set<foo::testing, Comparator> t{{true, true, 100},
                                       {false, true, 100},
                                       {true, false, 100},
                                       {true, true, 101}};
...
void test_implicit_conversions() {
  ...
  ss.str("");
  ss << empty{};
  BOOST_TEST_EQ(ss.str(), "{}");  // Breaks implicit conversion for types marked
                                  // with BOOST_PFR_FUNCTIONS_FOR
}
...
struct hash<anonymous_comparable_struct> {
  std::size_t operator()(
      const anonymous_comparable_struct& val) const noexcept {
    return hash_value(val);
  }
};
}  // namespace std
