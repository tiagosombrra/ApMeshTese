...
// This class mimics libc++ implementation of std::chrono::duration with unfxed
// LWG3050
...
class bogus_duration {
 public:
  ...
  template <class T>
  explicit bogus_duration(
      const T& val,
      typename std::enable_if<std::is_convertible<
          T, Rep>::value  // <= libstdc++ fix for LWG3050 is 's/T/const T&/g'
                              >::type* = nullptr)
      : rep_(val) {}

  template <class Rep2, class Period2>
  bogus_duration(
      const bogus_duration<Rep2, Period2>& val,
      typename std::enable_if<std::is_convertible<Period2, Rep>::value>::type* =
          nullptr)
      : rep_(val) {}
...
int main() {
  struct_with_optional val{
      ...
      std::chrono::seconds{4}, std::chrono::seconds{5}, std::chrono::seconds{6},
  };
  ...
  struct_with_bogus_duration val2{bogus_duration<long, char>{1},
                                  bogus_duration<long, char>{2}};
