...
struct unconstrained_forwarding_ref {
  ...
  unconstrained_forwarding_ref& operator=(const unconstrained_forwarding_ref&) =
      default;
  unconstrained_forwarding_ref& operator=(unconstrained_forwarding_ref&&) =
      default;
...
int main() {
  ...
  static_assert(
      std::is_same<sanity, aggregate_unconstrained>::value,
      "Precise reflection with template constructors sanity check fails");

  boost::pfr::detail::enable_if_constructible_helper_t<aggregate_unconstrained,
                                                       2>
      foo;
  (void)foo;

  static_assert(
      std::is_same<boost::pfr::tuple_element_t<0, aggregate_unconstrained>,
                   unconstrained_forwarding_ref<int> >::value,
      "Precise reflection with template constructors fails to work");

  static_assert(
      std::is_same<boost::pfr::tuple_element_t<1, aggregate_unconstrained>,
                   unconstrained_forwarding_ref<int_element> >::value,
      "Precise reflection with template constructors fails to work");
