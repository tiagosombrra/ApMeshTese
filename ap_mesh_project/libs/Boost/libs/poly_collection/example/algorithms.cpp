...
int main() {
  ...
  using standalone_renderable =
      boost::mpl::vector<renderable, boost::type_erasure::copy_constructible<>,
  ...
  auto& bc = c;
  {
    ...
    //[algorithms_7
    const char* comma = "";
    boost::poly_collection::for_each<warrior, juggernaut,
                                     goblin>(  // restituted types
        c.begin(), c.end(),
        [&](const auto& x) {  // loop traverses *all* elements
          std::cout << comma << x;
          comma = ",";
        });
    std::cout << "\n";
