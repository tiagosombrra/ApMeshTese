...
struct std_mismatch : std_cpp11_mismatch<Ts...> {
  ...
  template <typename InputIterator1, typename InputIterator2,
            typename Predicate>
  std::pair<InputIterator1, InputIterator2> operator()(InputIterator1 first1,
...
struct std_is_permutation : std_cpp11_is_permutation<Ts...> {
  ...
  template <typename ForwardIterator1, typename ForwardIterator2,
            typename Predicate>
  bool operator()(ForwardIterator1 first1, ForwardIterator1 last1,
...
template <typename ControlAlgorithm, typename Algorithm,
...
void test_algorithm(PolyCollection& p, Args&&... args) {
  ...
  for (auto first = p.begin(), end = p.end();; ++first) {
    for (auto last = first;; ++last) {
      BOOST_TEST(alg(first, last, std::forward<Args>(args)...) ==
...
template <typename ControlAlgorithm, typename... Algorithms,
          typename PolyCollection, typename... Args>
void test_algorithms(PolyCollection& p, Args&&... args) {
  do_((test_algorithm<ControlAlgorithm, Algorithms>(
       ...
       0)...);
  do_((test_algorithm<ControlAlgorithm, Algorithms>(
           const_cast<const PolyCollection&>(p), std::forward<Args>(args)...),
       0)...);
}

template <typename ControlAlgorithm, typename... Algorithms,
          typename PolyCollection, typename... Args>
void test_algorithms_with_equality_impl(std::true_type, PolyCollection& p,
                                        Args&&... args) {
  test_algorithms<ControlAlgorithm, Algorithms...>(p,
                                                   std::forward<Args>(args)...);
}

template <typename ControlAlgorithm, typename... Algorithm,
          typename PolyCollection, typename... Args>
void test_algorithms_with_equality_impl(std::false_type, PolyCollection&,
                                        Args&&...) {}
...
void test_algorithms_with_equality(PolyCollection& p, Args&&... args) {
  test_algorithms_with_equality_impl<ControlAlgorithm, Algorithms...>(
      is_equality_comparable<typename PolyCollection::value_type>{}, p,
      std::forward<Args>(args)...);
}

template <typename ControlAlgorithm, typename Algorithm, typename ToInt,
...
void test_for_each_n_algorithm(ToInt to_int, PolyCollection& p) {
  ...
  for (auto first = p.begin(), end = p.end();; ++first) {
    for (auto n = std::distance(first, end); n >= 0; --n) {
      ...
      auto it1 = alg(first, n, acc1), it2 = control(first, n, acc2);
      BOOST_TEST(it1 == it2);
...
template <typename ControlAlgorithm, typename... Algorithms, typename ToInt,
          typename PolyCollection>
void test_for_each_n_algorithms(ToInt to_int, PolyCollection& p) {
  do_((test_for_each_n_algorithm<ControlAlgorithm, Algorithms>(to_int, p),
       0)...);
...
template <typename ControlAlgorithm, typename... Algorithms, typename ToInt,
          typename PolyCollection, typename... Args>
void test_copy_algorithms_with_equality_impl(std::true_type, ToInt to_int,
                                             PolyCollection& p,
                                             Args&&... args) {
...
template <typename ControlAlgorithm, typename... Algorithm, typename ToInt,
          typename PolyCollection, typename... Args>
void test_copy_algorithms_with_equality_impl(std::false_type, ToInt,
                                             PolyCollection&, Args&&...) {}

template <typename ControlAlgorithm, typename... Algorithms, typename ToInt,
          typename PolyCollection, typename... Args>
void test_copy_algorithms_with_equality(ToInt to_int, PolyCollection& p,
                                        Args&&... args) {
  test_copy_algorithms_with_equality_impl<ControlAlgorithm, Algorithms...>(
      is_equality_comparable<typename PolyCollection::value_type>{}, to_int, p,
      std::forward<Args>(args)...);
}
...
void test_transform2_algorithms(ToInt to_int, PolyCollection& p) {
  do_((test_transform2_algorithm<ControlAlgorithm, Algorithms>(to_int, p),
       0)...);
...
void test_rotate_copy_algorithms(ToInt to_int, PolyCollection& p) {
  do_((test_rotate_copy_algorithm<ControlAlgorithm, Algorithms>(to_int, p),
       0)...);
...
template <typename ControlAlgorithm, typename Algorithm, typename ToInt,
          typename PolyCollection, typename Predicate>
void test_partition_copy_algorithm(ToInt to_int, PolyCollection& p,
                                   Predicate pred) {
  Algorithm alg;
  ControlAlgorithm control;
  for (auto first = p.begin(), end = p.end();; ++first) {
...
template <typename ControlAlgorithm, typename... Algorithms, typename ToInt,
          typename PolyCollection, typename Predicate>
void test_partition_copy_algorithms(ToInt to_int, PolyCollection& p,
                                    Predicate pred) {
  do_((test_partition_copy_algorithm<ControlAlgorithm, Algorithms>(to_int, p,
                                                                   pred),
       0)...);
  do_((test_partition_copy_algorithm<ControlAlgorithm, Algorithms>(
           to_int, const_cast<const PolyCollection&>(p), pred),
       0)...);
}
...
struct poly_accumulator_class {
  ...
  bool operator==(const poly_accumulator_class& x) const {
    return res == x.res;
  }
...
template <typename PolyCollection, typename ValueFactory, typename ToInt,
          typename... Types>
void test_algorithm() {
  PolyCollection p;
  ValueFactory v;
...

  {
    ...
    test_algorithms<std_all_of<>, poly_all_of<>, poly_all_of<Types...>>(
        p, always_false);
    test_algorithms<std_all_of<>, poly_all_of<>, poly_all_of<Types...>>(p,
                                                                        pred);
    ...
    test_algorithms<std_any_of<>, poly_any_of<>, poly_any_of<Types...>>(
        p, always_false);
    test_algorithms<std_any_of<>, poly_any_of<>, poly_any_of<Types...>>(p,
                                                                        pred);
  ...
  }
  {
    ...
    test_for_each_n_algorithms<std_for_each_n<>, poly_for_each_n<>,
                               poly_for_each_n<Types...>>(to_int, p);
  }
  {
    for (const auto& x : p) {
      test_algorithms_with_equality<std_find<>, poly_find<>,
                                    only_eq_comparable<poly_find, Types...>>(p,
                                                                             x);
    }
  }
  {
    ...
    test_algorithms<std_find_if_not<>, poly_find_if_not<>,
                    poly_find_if_not<Types...>>(p, pred);
  }
  {
    ...
    for (; first != end; ++first) {
      test_algorithms_with_equality<
          ...
          only_eq_comparable<poly_find_end, Types...>>(p, first, end);

      test_algorithms_with_equality<std_search<>, poly_search<>,
  ...
  }
  {
    ...
    for (auto first = v.begin(), end = v.begin() + v.size() / 2; first != end;
         ++first) {
      for (int i = 1; i < 4; ++i) {
        auto pred =
            compose(to_int, [&](int x, int y) { return x % i == y % i; });

        test_algorithms<std_find_end<>, poly_find_end<>,
                        poly_find_end<Types...>>(p, first, end, pred);
    ...
    for (auto first = v.begin(), end = v.end();; ++first) {
      for (auto last = first;; ++last) {
        test_algorithms_with_equality<
            ...
            only_eq_comparable<poly_find_first_of, Types...>>(p, first, last);
        if (last == end) break;
      }
      if (first == end) break;
    }
  }
  {
    ...
    for (auto first = v.begin(), end = v.end();; ++first) {
      for (auto last = first;; ++last) {
        test_algorithms<std_find_first_of<>, poly_find_first_of<>,
                        poly_find_first_of<Types...>>(
  ...
  }
  {
    test_algorithms_with_equality<
        ...
        only_eq_comparable<poly_adjacent_find, Types...>>(p);
  }
  {
    std::vector<int> v;
    for (const auto& x : p) v.push_back(to_int(x));
    ...
    for (auto first = v.begin(), end = v.end() - 1; first != end; ++first) {
      ...
      test_algorithms<std_adjacent_find<>, poly_adjacent_find<>,
                      poly_adjacent_find<Types...>>(
          p, compose_all(to_int,
                         [=](int x, int y) { return x == n1 && y == n2; }));
    }
  }
  {
    for (const auto& x : p) {
      test_algorithms_with_equality<std_count<>, poly_count<>,
                                    only_eq_comparable<poly_count, Types...>>(
          p, x);
    }
  }
  {
  ...
  }
  {
    ...
    for (auto first = v.begin(), end = v.begin() + v.size() / 2; first != end;
      ...
      test_algorithms_with_equality<
          ...
          only_eq_comparable<poly_mismatch, Types...>>(p, first);

      test_algorithms_with_equality<
          std_mismatch<>, poly_mismatch<>,
          only_eq_comparable<poly_mismatch, Types...>>(p, first, end);

      test_algorithms_with_equality<std_equal<>, poly_equal<>,
                                    only_eq_comparable<poly_equal, Types...>>(
          p, first);
    ...
    test_algorithms_with_equality<std_mismatch<>, poly_mismatch<>,
  ...
  }
  {
    ...
    for (auto first = v.begin(), end = v.begin() + v.size() / 2; first != end;
      ...
      test_algorithms<std_equal<>, poly_equal<>, poly_equal<Types...>>(p, first,
                                                                       pred);
  ...
  }
  {
    ...
    for (auto first = unwrap_iterator(v.begin()),
              end = unwrap_iterator(v.begin() + v.size() / 2);
         first != end; ++first) {
      test_algorithms_with_equality<
          std_is_permutation<>, poly_is_permutation<>,
          only_eq_comparable<poly_is_permutation, Types...>>(p, first);

      test_algorithms<std_is_permutation<>, poly_is_permutation<>,
                      poly_is_permutation<Types...>>(p, first, pred);

      test_algorithms_with_equality<
          std_is_permutation<>, poly_is_permutation<>,
          only_eq_comparable<poly_is_permutation, Types...>>(p, first, end);

      test_algorithms<std_is_permutation<>, poly_is_permutation<>,
                      poly_is_permutation<Types...>>(p, first, end, pred);
    }
    ...
    for (const auto& x : p) {
      for (int n = 0; n < 3; ++n) {
        test_algorithms_with_equality<
            ...
            only_eq_comparable<poly_search_n, Types...>>(p, n, x);
      }
    }
  }
  {
    for (int n = 0; n < 6; ++n) {
      test_algorithms<std_search_n<>, poly_search_n<>, poly_search_n<Types...>>(
          p, n, 0, compose(to_int, [&](int x, int y) {
            return x % (6 - n) == y % (6 - n);
          }));
    }
  }
  {
    test_copy_algorithms<std_copy<>, poly_copy<>, poly_copy<Types...>>(to_int,
                                                                       p);
  }
  {
  ...
  }
  {
    test_copy_algorithms<std_move<>, poly_move<>, poly_move<Types...>>(
        to_int,
        p); /* we're not checking std::move is properly used internally */
  }
  {
    auto f = compose(to_int, [](int x) { return -x; });
    auto int_id = [](int x) { return x; };

    test_copy_algorithms<std_transform<>, poly_transform<>,
                         poly_transform<Types...>>(int_id, p, f);
  }
  {
    test_transform2_algorithms<std_transform<>, poly_transform<>,
                               poly_transform<Types...>>(to_int, p);
  }
  {
    ...
    for (const auto& x : p) {
      test_copy_algorithms_with_equality<
          ...
          only_eq_comparable<poly_replace_copy, Types...>>(to_int, p, x, y);

      test_copy_algorithms_with_equality<
          std_remove_copy<>, poly_remove_copy<>,
          only_eq_comparable<poly_remove_copy, Types...>>(to_int, p, x);
    }
  }
  {
    auto always_true = compose(to_int, [](int) { return true; });
    ...
    test_copy_algorithms<std_replace_copy_if<>, poly_replace_copy_if<>,
                         poly_replace_copy_if<Types...>>(to_int, p, always_true,
                                                         x);
    test_copy_algorithms<std_replace_copy_if<>, poly_replace_copy_if<>,
                         poly_replace_copy_if<Types...>>(to_int, p,
                                                         always_false, x);
    test_copy_algorithms<std_replace_copy_if<>, poly_replace_copy_if<>,
                         poly_replace_copy_if<Types...>>(to_int, p, pred, x);

    test_copy_algorithms<std_remove_copy_if<>, poly_remove_copy_if<>,
                         poly_remove_copy_if<Types...>>(to_int, p, always_true);
    test_copy_algorithms<std_remove_copy_if<>, poly_remove_copy_if<>,
                         poly_remove_copy_if<Types...>>(to_int, p,
                                                        always_false);
    test_copy_algorithms<std_remove_copy_if<>, poly_remove_copy_if<>,
                         poly_remove_copy_if<Types...>>(to_int, p, pred);
  }
  {
    test_copy_algorithms_with_equality<
        std_unique_copy<>, poly_unique_copy<>,
        only_eq_comparable<poly_unique_copy, Types...>>(to_int, p);
  }
  {
    for (int n = 0; n < 6; ++n) {
      test_copy_algorithms<std_unique_copy<>, poly_unique_copy<>,
                           poly_unique_copy<Types...>>(
          to_int, p, compose_all(to_int, [&](int x, int y) {
            return x % (6 - n) == y % (6 - n);
          }));
    }
  }
  {
    test_rotate_copy_algorithms<std_rotate_copy<>, poly_rotate_copy<>,
                                poly_rotate_copy<Types...>>(to_int, p);
  }
  {
    /* force_arg_copying used to avoid sharing mt19937's state */

    for (auto n = p.size() + 1; n-- > 0;) {
      test_copy_algorithms<force_arg_copying<std_sample<>>,
  ...
  }
  {
    for (int n = 0; n < 6; ++n) {
      auto pred =
          compose(to_int, [&](int x) { return x % (6 - n) <= (6 - n) / 2; });

      test_algorithms<std_is_partitioned<>, poly_is_partitioned<>,
                      poly_is_partitioned<Types...>>(p, pred);

      test_partition_copy_algorithms<std_partition_copy<>,
                                     poly_partition_copy<>,
                                     poly_partition_copy<Types...>>(to_int, p,
                                                                    pred);

      test_algorithms<std_partition_point<>, poly_partition_point<>,
                      poly_partition_point<Types...>>(p, pred);
    }
  }
}
