...
template <bool Propagate, bool AlwaysEqual, typename PolyCollection,
          typename ValueFactory, typename... Types>
void test_allocator_aware_construction() {
  ...
  fill<constraints<is_equality_comparable, is_copy_constructible>, Types...>(
      p, v, 2);
...
#endif
  {
    ...
    rooted_poly_collection p2{cp}, p3{use_same_allocator ? root1 : root2};

    auto d2 = get_layout_data<Types...>(p2), d3 = get_layout_data<Types...>(p3);

    p2.swap(p3);
    auto e2 = get_layout_data<Types...>(p2), e3 = get_layout_data<Types...>(p3);
    BOOST_TEST(d2 == e3);
    BOOST_TEST(d3 == e2);
    do_((BOOST_TEST(!p2.template is_registered<Types>()), 0)...);
    ...
    using std::swap;
    swap(p2, p3);
    auto f2 = get_layout_data<Types...>(p2), f3 = get_layout_data<Types...>(p3);
    BOOST_TEST(e2 == f3);
    BOOST_TEST(e3 == f2);
...
void test_construction() {
  {
    ...
    test_allocator_aware_construction<!propagate, !always_equal, PolyCollection,
                                      ValueFactory, Types...>();
    test_allocator_aware_construction<!propagate, always_equal, PolyCollection,
                                      ValueFactory, Types...>();
    test_allocator_aware_construction<propagate, !always_equal, PolyCollection,
...

  {
    ...
    {
      using type =
          first_of<constraints<is_equality_comparable, is_copy_constructible>,
                   Types...>;

      PolyCollection p2{cp.template begin<type>(), cp.template end<type>()};
      BOOST_TEST(p2.size() == cp.template size<type>() &&
                 std::equal(p2.template begin<type>(), p2.template end<type>(),
...

  {
    ...
    fill<constraints<is_equality_comparable, is_not_copy_constructible>,
         Types...>(p, v, 2);
    ...
    static_assert(sizeof(not_copy_constructible) > 0,
...
void test_scoped_allocator() {
  ...
  using collection_allocator =
      std::scoped_allocator_adaptor<element_allocator, vector_allocator>;
  using poly_collection = boost::any_collection<concept_, collection_allocator>;
...
void test_construction() {
  test_construction<any_types::collection, auto_increment, any_types::t1,
                    any_types::t2, any_types::t3, any_types::t4,
                    any_types::t5>();
  test_construction<base_types::collection, auto_increment, base_types::t1,
                    base_types::t2, base_types::t3, base_types::t4,
                    base_types::t5>();
  test_construction<function_types::collection, auto_increment,
