...
struct rectangle_traits<layout_pin> {
  ...
  static inline interval_type get(const layout_pin& pin,
    ...
    if (orient == HORIZONTAL) return interval_type(pin.xl, pin.xh);
    return interval_type(pin.yl, pin.yh);
  }
};

template <>
struct geometry_concept<layout_pin> {
...
inline void populate_connected_component(
  ...
  if (polygon_color[node_id] == 1) return;
  ...
  if (node_id >= polygon_id_offset)
    connectivity[net][layout_layer].insert(
        polygons[node_id - polygon_id_offset]);
  for (std::set<int>::iterator itr = graph[node_id].begin();
    ...
    populate_connected_component(connectivity, polygons, polygon_color, graph,
                                 *itr, polygon_id_offset, net, net_ids,
                                 net_prefix, layout_layer);
  }
}

inline void connect_layout_to_layer(connectivity_database& connectivity,
                                    polygon_set& layout,
                                    std::string layout_layer, std::string layer,
                                    std::string net_prefix, int& net_suffix) {
  if (layout_layer.empty()) return;
  boost::polygon::connectivity_extraction_90<int> ce;
  std::vector<std::string> net_ids;
  for (connectivity_database::iterator itr = connectivity.begin();
       itr != connectivity.end(); ++itr) {
  ...
  for (std::size_t node_id = 0; node_id < net_ids.size(); ++node_id) {
    populate_connected_component(connectivity, polygons, polygon_color, graph,
                                 node_id, polygon_id_offset, net_ids[node_id],
                                 net_ids, net_prefix, layout_layer);
  }
  // for each polygon_color that is zero populate connected compontent with
  ...
  for (std::size_t i = 0; i < polygons.size(); ++i) {
    if (polygon_color[i + polygon_id_offset] == 0) {
      ...
      ss >> internal_net;
      populate_connected_component(
          connectivity, polygons, polygon_color, graph, i + polygon_id_offset,
          polygon_id_offset, internal_net, net_ids, net_prefix, layout_layer);
    }
  }
}
...
inline void populate_connectivity_database(connectivity_database& connectivity,
  ...
  connect_layout_to_layer(connectivity, layout["METAL1"], "METAL1", "METAL1",
                          "__internal_net_", internal_net_suffix);
  // connect via0 layout to metal1
  connect_layout_to_layer(connectivity, layout["VIA0"], "VIA0", "METAL1",
                          "__internal_net_", internal_net_suffix);
  ...
  connect_layout_to_layer(connectivity, poly_not_gate, "POLY", "VIA0",
                          "__internal_net_", internal_net_suffix);
  // we don't want to short signals through transistors so we subtract the gate
  // regions from the diffusions
  polygon_set diff_not_gate =
      (layout["PDIFF"] + layout["NDIFF"]) - layout["GATE"];
  // connect diffusion minus gate to poly
  // Note that I made up the DIFF layer name for combined P and NDIFF
  connect_layout_to_layer(connectivity, diff_not_gate, "DIFF", "POLY",
                          "__internal_net_", internal_net_suffix);
  // connect gate to poly to make connections through gates on poly
  connect_layout_to_layer(connectivity, layout["GATE"], "GATE", "POLY",
                          "__internal_net_", internal_net_suffix);
