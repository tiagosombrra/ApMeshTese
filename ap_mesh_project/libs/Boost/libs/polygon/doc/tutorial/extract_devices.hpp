...
inline std::vector<std::set<int> > extract_layer(
    connectivity_extraction& ce, std::vector<std::string>& net_ids,
...
inline void extract_device_type(std::vector<device>& devices,
                                connectivity_database& connectivity,
                                polygon_set& layout, std::string type) {
  // recall that P and NDIFF were merged into one DIFF layer in the connectivity
  // database find the two nets on the DIFF layer that interact with each
  // transistor and then find the net on the poly layer that interacts with each
  // transistor
  ...
  for (std::size_t i = net_ids_poly.size(); i < graph_diff.size(); ++i) {
    ...
    tmp_devices[i - net_ids_diff.size()].terminals =
        std::vector<std::string>(3, std::string());
    std::size_t j = 0;
...
inline void extract_devices(std::vector<device>& devices,
                            connectivity_database& connectivity,
