...
typedef std::map<std::string, boost::polygon::polygon_90_set_data<int> >
    layout_database;
...
struct rectangle_traits<layout_rectangle> {
  ...
  static inline interval_type get(const layout_rectangle& rectangle,
                                  orientation_2d orient) {
    if (orient == HORIZONTAL) return interval_type(rectangle.xl, rectangle.xh);
    return interval_type(rectangle.yl, rectangle.yh);
  }
};

template <>
struct geometry_concept<layout_rectangle> {
...
inline void populate_layout_database(layout_database& layout,
