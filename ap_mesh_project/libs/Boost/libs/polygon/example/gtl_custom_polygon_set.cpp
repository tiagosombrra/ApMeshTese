...
struct polygon_mutable_traits<CPolygon> {
  ...
  static inline CPolygon& set_points(CPolygon& t, iT input_begin,
                                     iT input_end) {
    t.clear();
...
int main() {
  long long c1 = clock();
  for (int i = 0; i < 1000; ++i) test_polygon_set<CPolygonSet>();
  long long c2 = clock();
  ...
  if (diff1 > 0 && diff2)
    std::cout << "library polygon_set_data is " << float(diff1) / float(diff2)
              << "X faster than custom polygon set deque of CPolygon"
              << std::endl;
  else
