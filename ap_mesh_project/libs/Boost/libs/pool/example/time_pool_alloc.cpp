...
static void timing_test_alloc_larger() {
  ...
  for (l = 0; l < num_loops; ++l) {
    ...
    for (unsigned long i = 0; i < num_ints; ++i) a.deallocate(a.allocate(1), 1);
  }
  ...
  std::cout << "Test " << test_number++ << ": Alloc & Dealloc " << num_ints
            << " structures of size " << sizeof(larger_structure<N>) << ":"
            << std::endl;
...
static void timing_test_alloc() {
  ...
  start = std::clock();
  for (l = 0; l < num_loops; ++l) {
    for (unsigned long i = 0; i < num_ints; ++i) delete new (std::nothrow) int;
  }
  end[0][2] = (std::clock() - start) / ((double)CLOCKS_PER_SEC);
  ...
  for (l = 0; l < num_loops; ++l) {
    std::allocator<int> a;
    for (unsigned long i = 0; i < num_ints; ++i) p[i] = a.allocate(1);
    for (unsigned long i = 0; i < num_ints; ++i) a.deallocate(p[i], 1);
  }
  ...
  for (l = 0; l < num_loops; ++l) {
    ...
    for (unsigned long i = 0; i < num_ints; ++i) std::free(p[i]);
  }
  end[1][1] = (std::clock() - start) / ((double)CLOCKS_PER_SEC);

  start = std::clock();
  for (l = 0; l < num_loops; ++l) {
    for (unsigned long i = 0; i < num_ints; ++i) p[i] = new (std::nothrow) int;
    for (unsigned long i = 0; i < num_ints; ++i) delete p[i];
  }
  end[1][2] = (std::clock() - start) / ((double)CLOCKS_PER_SEC);

  start = std::clock();
  for (l = 0; l < num_loops; ++l) {
    for (unsigned long i = 0; i < num_ints; ++i) p[i] = alloc::allocate();
    for (unsigned long i = 0; i < num_ints; ++i) alloc::deallocate(p[i]);
  }
  end[1][3] = (std::clock() - start) / ((double)CLOCKS_PER_SEC);

  start = std::clock();
  for (l = 0; l < num_loops; ++l) {
    for (unsigned long i = 0; i < num_ints; ++i) p[i] = alloc_sync::allocate();
    for (unsigned long i = 0; i < num_ints; ++i) alloc_sync::deallocate(p[i]);
  }
  ...
  std::cout << "Test 4: Alloc " << num_ints << " ints & Dealloc " << num_ints
            << " ints:" << std::endl;
...
static void timing_test_containers() {
  ...
  for (l = 0; l < num_loops; ++l) {
    ...
    for (unsigned long i = 0; i < num_ints; ++i) x.push_back(0);
  }
  end[0][0] = (std::clock() - start) / ((double)CLOCKS_PER_SEC);
  ...
  for (l = 0; l < num_loops; ++l) {
    ...
    for (unsigned long i = 0; i < num_ints; ++i) x.insert(0);
  }
  end[1][0] = (std::clock() - start) / ((double)CLOCKS_PER_SEC);
  ...
  std::cout << "Test 0: Insertion & deletion of " << num_ints
  ...
  std::cout << "Test 1: Insertion & deletion of " << num_ints
  ...
  std::cout << "  Pool /w Sync:   " << end[1][4] << " seconds" << std::endl;
  std::cout << "  Pool:           not possible" << std::endl;
  std::cout << "Test 2: Insertion & deletion of " << num_ints
...
int main(int argc, char *argv[]) {
  if (argc != 1 && argc != 2) {
    std::cerr << "Usage: " << argv[0] << " [number_of_ints_to_use_each_try]"
              << std::endl;
    return 1;
  }
  ...
  try {
    ...
    std::cerr << "Timing tests ran out of memory; try again with a lower value "
                 "for number of ints"
...
MSVC 10.0  using mutli-threaded DLL
 ...
 time_pool_alloc.cpp
     Creating library J:\Cpp\pool\pool\Release\alloc_example.lib and object
J:\Cpp\pool\pool\Release\alloc_example.exp Generating code Finished generating
code alloc_example.vcxproj -> J:\Cpp\pool\pool\Release\alloc_example.exe Test 0:
Insertion & deletion of 700000 ints in a vector: std::allocator: 0.062 seconds
