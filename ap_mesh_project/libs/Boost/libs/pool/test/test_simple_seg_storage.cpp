...
int main() {
  ...
  BOOST_ASSERT(block_size >= partition_sz);
  {
    ...
    int endadd = 0;
    void* const pvret =
        test_simp_seg_store::segregate(pc, block_size, partition_sz, &endadd);
    ...
    while (cur != &endadd) {
      ...
      BOOST_TEST(std::less_equal<void*>()(
          static_cast<char*>(last) + partition_sz, cur));
      BOOST_TEST(std::less_equal<void*>()(
          static_cast<char*>(cur) + partition_sz, pc + block_size));
...

{
  ...
  tstore.add_ordered_block(pc + (4 * partition_sz), (2 * partition_sz),
                           partition_sz);
  // "Order-preserving"
  test_is_order(tstore);
}
...
{
  {
    ...
    test_simp_seg_store tstore;
    tstore.add_ordered_block(pc, 2 * partition_sz, partition_sz);
    tstore.add_ordered_block(pc + (3 * partition_sz), 3 * partition_sz,
                             partition_sz);
    tstore.add_ordered_block(pc + (7 * partition_sz), 5 * partition_sz,
                             partition_sz);
...
for (std::set<char*>::iterator itr = track_allocator::allocated_blocks.begin();
     itr != track_allocator::allocated_blocks.end(); ++itr) {
  delete[] * itr;
}
