...
class test_simp_seg_store
    : public boost::simple_segregated_storage<std::size_t> {
 private:
  ...
  bool is_inside_allocated_blocks(void* const chunk) const {
    typedef std::vector<std::pair<void*, std::size_t> >::const_iterator VPIter;
    for (VPIter iter = allocated_blocks.begin(); iter != allocated_blocks.end();
         ++iter) {
      if (std::less_equal<void*>()(iter->first, chunk) &&
          std::less_equal<void*>()(
              static_cast<char*>(chunk) + np_sz,
  ...
  void check_in(void* const chunk) {
    ...
    for (std::set<void*>::const_iterator iter = allocated_chunks.begin();
         iter != allocated_chunks.end(); ++iter) {
      BOOST_TEST(
          std::less_equal<void*>()(static_cast<char*>(chunk) + np_sz, *iter) ||
          std::less_equal<void*>()(static_cast<char*>(*iter) + np_sz, chunk));
    }
 ...
 public:
  ...
  void add_block(void* const block, const size_type nsz,
    ...
    allocated_blocks.push_back(std::pair<void*, std::size_t>(block, nsz));
    boost::simple_segregated_storage<std::size_t>::add_block(block, nsz,
                                                             npartition_sz);
    // Post: !empty()
    BOOST_TEST(!empty());
  }
  ...
  void* malloc() {
    void* const ret = boost::simple_segregated_storage<std::size_t>::malloc();
  ...
  void* malloc_n(size_type n, size_type partition_size) {
    void* const ret = boost::simple_segregated_storage<std::size_t>::malloc_n(
        n, partition_size);

    if (ret) {
      for (std::size_t i = 0; i < n; ++i) {
        void* const chunk = static_cast<char*>(ret) + (i * partition_size);
