...
class OptionsHeirarchy {
 ...
 private:
  ...
  void SetCommandLineOptions() {
    command_line_options.add_options()("help,h", "display this help message")(
        "version,v", "show program version")(
        "config,c", po::value<std::vector<std::string>>(),
        "config files to parse (always parses default.cfg)");
    hidden_command_line_options.add_options()("master-file",
                                              po::value<std::string>())(
  ...
  void SetCommonOptions() {
    common_options.add_options()("path",
                                 po::value<std::string>()->default_value(""),
                                 "the execution path to use (imports from "
        ...
        "paths to search for include files")("run-gui", po::bool_switch(),
                                             "start the GUI");
  }
  void SetConfigOnlyOptions() {
    ...
    // Run a parser here (with no command line options) to add these defaults
    // into results, this way they will be enabled even if no config files are
    // parsed.
    store(po::command_line_parser(0, 0).options(config_only_options).run(),
          results);
    notify(results);
  }
  void SetEnvMapping() {
  ...
  void ParseEnvironment() {
    store(
        po::parse_environment(
            common_options,
            // The next two lines are the crazy syntax to use EnvironmentMapper
            // as the lookup function for env->config name conversions
            boost::function1<std::string, std::string>(std::bind1st(
                std::mem_fun(&OptionsHeirarchy::EnvironmentMapper), this))),
        results);
    notify(results);
  }
...
parameter. The program will then exit.
    ...
    Usage: example [OPTION]... MASTER-FILE [FILE]...
      ...
      -c [ --config ] arg        config files to parse (always parses
default.cfg)
...
one specifies the "master" file the others are additional files.
    ...
    > example.exe --path=a-path -I an-include master.cpp additional1.cpp
additional2.cpp First 75 chars of the system path: a-path Verbosity: INFO
    Include Path:
       an-include
