...
void test_long_options() {
  ...
  test_case test_cases4[] = {
      {"--foo", s_success, "foo:"},      {"--Foo", s_success, "foo:"},
      {"--bar=Ab", s_success, "bar:Ab"}, {"--Bar=ab", s_success, "bar:ab"},
      {"--giz", s_success, "Giz:"},      {0, 0, 0}};
...
void test_short_options() {
  ...
  style = cmdline::style_t(allow_short | allow_dash_for_short |
  ...
  test_case test_cases2[] = {
      {"-f 13", s_success, "-f:13"},      {"-f -13", s_success, "-f:-13"},
      {"-f", s_missing_parameter, ""},    {"-f /foo", s_success, "-f:/foo"},
      {"-f -d", s_missing_parameter, ""}, {0, 0, 0}};
  ...
  style =
      cmdline::style_t(allow_short | short_allow_next | allow_dash_for_short |
...
void test_dos_options() {
  ...
  test_case test_cases1[] = {
      {"/d d -bar", s_success, "-d: d -bar"}, {"--foo", s_success, "--foo"},
      {"/d13", s_extra_parameter, ""},        {"/f14", s_success, "-f:14"},
      {"/f", s_missing_parameter, ""},        {0, 0, 0}};
  test_cmdline(",d ,f=", style, test_cases1);

  style =
      cmdline::style_t(allow_short | allow_slash_for_short | short_allow_next |
                       short_allow_adjacent | allow_sticky);

  test_case test_cases2[] = {
      {"/de", s_extra_parameter, ""}, {"/fe", s_success, "-f:e"}, {0, 0, 0}};
...
void test_disguised_long() {
  ...
  style = cmdline::style_t(allow_short | short_allow_adjacent |
                           allow_dash_for_short | short_allow_next |
                           allow_long_disguise | long_allow_adjacent);
...
void test_guessing() {
  ...
  style = cmdline::style_t(
      allow_short | short_allow_adjacent | allow_dash_for_short | allow_long |
      long_allow_adjacent | allow_guessing | allow_long_disguise);
...
void test_arguments() {
  ...
  test_case test_cases2[] = {{"-f - -gx - -- -e", s_success, "-f: - -g:x - -e"},
                             {0, 0, 0}};
...
void test_prefix() {
  ...
  test_case test_cases1[] = {{"--foo.bar=12", s_success, "foo.bar:12"},
                             {0, 0, 0}};
...
void test_implicit_value() {
  ...
  test_case test_cases1[] = {
      ...
      // '--bar' looks like option, and such option exists, so we don't consume
      // this token
      {"--foo --bar", s_success, "foo: bar:"},
      // '--biz' looks like option, but does not match any existing one.
      // Presently this results in parse error, since
      // (1) in cmdline.cpp:finish_option, we only consume following tokens if
      // they are
      // requires
