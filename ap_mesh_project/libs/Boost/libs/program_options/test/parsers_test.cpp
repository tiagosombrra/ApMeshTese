...
vector<string> sv(const char* array[], unsigned size) {
  vector<string> r;
  for (unsigned i = 0; i < size; ++i) r.push_back(array[i]);
  return r;
}
...
void test_many_different_options() {
  ...
  const char* cmdline3_[] = {"--foo=12", "-f4",          "--bar=11",
                             "-b4",      "--voiture=15", "--dawg=16",
                             "--dog=17", "--plug3=10"};
  ...
  vector<option> a4 =
      parse_command_line(sizeof(cmdline3_) / sizeof(const char*), cmdline3_,
                         desc, 0, additional_parser)
          .options;
  // The default style is unix-style, where the first argument on the
  // command-line is the name of a binary, not an option value, so that should
  // be ignored
...
void test_not_crashing_with_empty_string_values() {
  ...
  variables_map vm;
  po::store(po::parse_command_line(sizeof(cmdline4) / sizeof(const char*),
...
void test_multitoken() {
  ...
  desc3.add_options()(",p", po::value<string>())(
      ",o", po::value<string>()->multitoken())(",x", po::value<string>());
  vector<option> a5 = parse_command_line(sizeof(cmdline5) / sizeof(const char*),
                                         const_cast<char**>(cmdline5), desc3, 0,
                                         additional_parser)
                          .options;
...
void test_multitoken_and_multiname() {
  ...
  options_description desc;
  desc.add_options()("bar,b", po::value<string>())(
      "foo,fee,f", po::value<string>()->multitoken())("fizbaz,baz,z",
                                                      po::value<string>());

  vector<option> parsed_options =
      parse_command_line(sizeof(cmdline) / sizeof(const char*),
                         const_cast<char**>(cmdline), desc, 0,
                         additional_parser)
          .options;
  ...
  parsed_options = parse_command_line(sizeof(cmdline_2) / sizeof(const char*),
                                      const_cast<char**>(cmdline_2), desc, 0,
                                      additional_parser)
                       .options;
...
void test_multitoken_vector_option() {
  ...
  desc4.add_options()("multitoken,multi-token,m",
                      ...
                      "values")("file", po::value<std::string>(),
                                "the file to process");
...
void test_unregistered() {
  ...
  store(command_line_parser(cmdline1).options(desc).allow_unregistered().run(),
        vm);
