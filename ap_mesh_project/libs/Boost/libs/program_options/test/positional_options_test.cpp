...
void test_positional_options() {
  ...
  BOOST_CHECK_EQUAL(p.max_total_count(),
...
void test_parsing() {
  ...
  desc.add_options()("first", po::value<int>())("second", po::value<int>())(
      "input-file", po::value<vector<string> >())("some-other",
                                                  po::value<string>());
  ...
  BOOST_CHECK_THROW(command_line_parser(args).options(desc).positional(p).run(),
