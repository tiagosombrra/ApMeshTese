...
struct lambda_arity
    : proto::or_<
          ...
          proto::when<proto::nary_expr<_, proto::vararg<_> >,
                      proto::fold<_, mpl::int_<0>(),
                                  mpl::max<lambda_arity, proto::_state>()> > > {
};

// The lambda context is the same as the default context
// with the addition of special handling for lambda placeholders
template <typename Tuple>
struct lambda_context : proto::callable_context<lambda_context<Tuple> const> {
  ...
  template <typename I>
  typename fusion::result_of::at<Tuple, I>::type operator()(
      proto::tag::terminal, placeholder<I> const &) const {
...
struct lambda {
  ...
  struct result<This(A0, A1)>
      : proto::result_of::eval<T const,
                               lambda_context<fusion::tuple<A0, A1> > > {};

  // Define our operator () that evaluates the lambda expression.
  typename result<lambda()>::type operator()() const {
    fusion::tuple<> args;
  ...
  typename result<lambda(A0 const &)>::type operator()(A0 const &a0) const {
  ...
  typename result<lambda(A0 const &, A1 const &)>::type operator()(
      A0 const &a0, A1 const &a1) const {
...
#define M0(N, typename_A, A_const_ref, A_const_ref_a, ref_a)                \
  ...
  typename proto::result_of::make_expr<proto::tag::function, lambda_domain, \
                                       construct_helper<T>,                 \
                                       A_const_ref(N)>::type const          \
  construct(A_const_ref_a(N)) {                                             \
    return proto::make_expr<proto::tag::function, lambda_domain>(           \
