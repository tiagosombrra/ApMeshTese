...
struct Vec3SubscriptCtx : proto::callable_context<Vec3SubscriptCtx const> {
...
struct CountLeavesCtx
  ...
  void operator()(proto::tag::terminal, int const (&)[3]) { ++this->count; }
...
struct CountLeaves
    : proto::or_<
          ...
          ,
          proto::otherwise<
              proto::fold<_, int(), iplus(CountLeaves, proto::_state)> > > {};

// Here is the Vec3 struct, which is a vector of 3 integers.
struct Vec3 : proto::extends<proto::terminal<int[3]>::type, Vec3> {
  ...
  int &operator[](int i) { return proto::value(*this)[i]; }

  int const &operator[](int i) const { return proto::value(*this)[i]; }
