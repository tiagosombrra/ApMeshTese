...
struct VectorSubscriptCtx {
  ...
  struct eval : proto::default_eval<Expr, VectorSubscriptCtx const> {};

  // Index vector terminals with our subscript.
  template <typename Expr>
  struct eval<Expr, typename boost::enable_if<proto::matches<
                        Expr, proto::terminal<std::vector<_, _> > > >::type> {
    typedef
        typename proto::result_of::value<Expr>::type::value_type result_type;
...
struct VectorSizeCtx {
  ...
  template <typename Expr, typename EnableIf = void>
  struct eval : proto::null_eval<Expr, VectorSizeCtx const> {};
...
struct VectorGrammar
    : proto::or_<proto::terminal<_>,
                 proto::and_<proto::nary_expr<_, proto::vararg<VectorGrammar> >,
...
template <typename Expr>
struct VectorExpr : proto::extends<Expr, VectorExpr<Expr>, VectorDomain> {
...
struct IsVector<std::vector<T, A> > : mpl::true_ {};
...
std::vector<T, A> &assign(std::vector<T, A> &arr, Expr const &expr) {
  ...
  proto::eval(proto::as_expr<VectorDomain>(expr),
              size);  // will throw if the sizes don't match
