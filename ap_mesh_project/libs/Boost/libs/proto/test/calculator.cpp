...
struct calculator : proto::callable_context<calculator const> {
  ...
  calculator(int i) : i_(i) {}

  int operator()(proto::tag::terminal, placeholder) const { return this->i_; }

  int operator()(proto::tag::terminal, int j) const { return j; }
  ...
  template <typename Left, typename Right>
  int operator()(proto::tag::minus, Left const &left,
                 Right const &right) const {
    return proto::eval(left, *this) - proto::eval(right, *this);
  }

  template <typename Left, typename Right>
  int operator()(proto::tag::multiplies, Left const &left,
                 Right const &right) const {
    return proto::eval(left, *this) * proto::eval(right, *this);
  }

  template <typename Left, typename Right>
  int operator()(proto::tag::divides, Left const &left,
                 Right const &right) const {
    return proto::eval(left, *this) / proto::eval(right, *this);
  }
...
struct functional {
  ...
  functional(Expr const &expr) : expr_(expr) {}
