...
struct CalcArity : proto::or_<
                       ...
                       ,
                       proto::terminal<_>
                       /*<< a unary expression where the operand is a calculator
                          expression, or ... >>*/
                       ,
                       proto::unary_expr<_, CalcArity>
                       /*<< a binary expression where the operands are
...
struct unary_arity
  ...
  struct impl : proto::transform_impl<Expr, State, Data> {
    ...
    typedef typename boost::result_of<CalculatorArity(child_expr, State,
...
struct binary_arity
  ...
  struct impl : proto::transform_impl<Expr, State, Data> {
    ...
    /*<< Apply `CalculatorArity` to find the arity of the left and right
     * children. >>*/
    typedef
        typename boost::result_of<CalculatorArity(left_expr, State, Data)>::type
            left_arity;
    typedef typename boost::result_of<CalculatorArity(right_expr, State,
                                                      Data)>::type right_arity;
...
struct CalculatorArity
    : proto::or_<proto::when<proto::terminal<placeholder<0> >, mpl::int_<1>()>,
...
struct ArgsAsList
    : proto::when<
          proto::function<proto::terminal<_>,
                          proto::vararg<proto::terminal<_> > >
...
struct FoldTreeToList
    : proto::or_<
          ...
          proto::when<proto::comma<FoldTreeToList, FoldTreeToList>
                      /*<< Fold all terminals that are separated by commas into
                         a Fusion cons list. >>*/
                      ,
                      proto::reverse_fold_tree<
                          _, fusion::nil(),
                          fusion::cons<FoldTreeToList, proto::_state>(
                              FoldTreeToList, proto::_state)> > > {};
//]
...
struct Promote
    : proto::or_<
          ...
          proto::when<proto::terminal<float>,
                      proto::terminal<double>::type(proto::_value)>,
...
struct MakePair
    : proto::when<
          ...
          proto::function<proto::terminal<make_pair_tag>, proto::terminal<_>,
                          proto::terminal<_> >
          ...
          transform.)>>*/
          ,
          std::pair<proto::_value(proto::_child1),
...
struct make_pair : proto::callable {
  ...
  struct result<This(First, Second)> {
    typedef std::pair<BOOST_PROTO_UNCVREF(First), BOOST_PROTO_UNCVREF(Second)>
        type;
  };
...
// `make_pair_(1, 3.14)` and actually builds a `std::pair<>`
// from the arguments.
struct MakePair : proto::when<
                      /*<< Match expressions like `make_pair_(1, 3.14)` >>*/
                      proto::function<proto::terminal<make_pair_tag>,
                                      proto::terminal<_>, proto::terminal<_> >
                      /*<< Return `make_pair()(f,s)` where `f` and `s` are the
                      first and second arguments to the lazy `make_pair_()`
                      function. (This uses `proto::call<>` under the covers  to
                      evaluate the transform.)>>*/
                      ,
                      make_pair(proto::_value(proto::_child1),
                                proto::_value(proto::_child2))> {};
//]
}  // namespace lazy_make_pair2

//[ NegateInt
struct NegateInt : proto::when<proto::terminal<int>, proto::negate<_>(_)> {};
//]
...
struct at : proto::callable {
  ...
  struct result<This(Cont, Index)>
      : fusion::result_of::at<typename boost::remove_reference<Cont>::type,
                              typename boost::remove_reference<Index>::type> {};

  template <typename Cont, typename Index>
  typename fusion::result_of::at<Cont, Index>::type operator()(
      Cont &cont, Index const &) const {
    return fusion::at<Index>(cont);
  }
};

// A transform that evaluates a lambda expression.
struct LambdaEval : proto::or_<
                        /*<<When you match a placeholder ...>>*/
                        proto::when<proto::terminal<placeholder<_> >
                                    /*<<... call at() with the data parameter,
                                    which is a tuple, and the placeholder, which
...
void test_examples() {
  ...
  cons<int, cons<char, cons<std::string> > > args(
      ArgsAsList()(_1(1, 'a', std::string("b")), i, i));
  BOOST_CHECK_EQUAL(args.car, 1);
  BOOST_CHECK_EQUAL(args.cdr.car, 'a');
  BOOST_CHECK_EQUAL(args.cdr.cdr.car, std::string("b"));

  cons<int, cons<char, cons<std::string> > > lst(
      FoldTreeToList()((_1 = 1, 'a', std::string("b")), i, i));
  BOOST_CHECK_EQUAL(lst.car, 1);
  BOOST_CHECK_EQUAL(lst.cdr.car, 'a');
  BOOST_CHECK_EQUAL(lst.cdr.cdr.car, std::string("b"));

  proto::plus<proto::terminal<double>::type,
              proto::terminal<double>::type>::type p =
      Promote()(proto::lit(1.f) + 2.f, i, i);
  ...
  std::pair<int, double> p3 =
      lazy_make_pair2::MakePair()(lazy_make_pair2::make_pair_(1, 3.14), j, j);
