...
struct calc_grammar
    : proto::or_<
          ...
          proto::when<proto::terminal<proto::convertible_to<double> >,
                      proto::_value>,
          ...
          ,
          proto::when<divides_rule, proto::external_transform> > {};
...
template <typename E>
struct calc_expr : proto::extends<E, calc_expr<E>, calc_domain> {
...
struct checked_division
    : proto::external_transforms<proto::when<
          divides_rule, do_checked_divide(calc_grammar(proto::_left),
                                          calc_grammar(proto::_right))> > {};
...
void test_external_transforms() {
  ...
  int result2 = calc_grammar()(_1 / _2, fusion::make_vector(8, 2),
                               (non_checked, mydata = "foo"));
  BOOST_CHECK_EQUAL(result2, 4);

  // check that we can use the dedicated slot for this purpose
  int result3 =
      calc_grammar()(_1 / _2, fusion::make_vector(8, 2),
                     (42, proto::transforms = non_checked, mydata = "foo"));
  BOOST_CHECK_EQUAL(result2, 4);
  ...
  try {
    // This should throw
    int result4 = calc_grammar()(_1 / _2, fusion::make_vector(6, 0),
                                 (checked, mydata = test_external_transforms));
    BOOST_CHECK(!"Didn't throw an exception");  // shouldn't get here!
  } catch (division_by_zero) {
    ;  // OK
  } catch (...) {
  ...
  try {
    // This should throw
    int result5 = calc_grammar()(
        _1 / _2, fusion::make_vector(6, 0),
        (42, proto::transforms = checked, mydata = test_external_transforms));
    BOOST_CHECK(!"Didn't throw an exception");  // shouldn't get here!
  } catch (division_by_zero) {
    ;  // OK
  } catch (...) {
