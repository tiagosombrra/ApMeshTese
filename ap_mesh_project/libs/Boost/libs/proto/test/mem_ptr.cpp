...
struct evaluator : proto::when<_, proto::_default<evaluator> > {};
...
void assert_result_type(Expr &) {
  ...
  // is correct.
  BOOST_MPL_ASSERT((
      boost::is_same<Ret, typename boost::result_of<evaluator(Expr &)>::type>));

  // check that the return type as calculated by the default_context
  // is correct.
  BOOST_MPL_ASSERT(
      (boost::is_same<Ret, typename boost::result_of<proto::functional::eval(
                               Expr &, proto::default_context &)>::type>));
}
...
struct my_terminal {
  ...
  BOOST_PROTO_BASIC_EXTENDS(my_terminal_base, my_terminal,
                            proto::default_domain)

  template <typename A0>
  typename proto::result_of::make_expr<proto::tag::function,
                                       my_terminal const &, A0 &>::type const
  operator()(A0 &a0) const {
    return proto::make_expr<proto::tag::function>(boost::ref(*this),
                                                  boost::ref(a0));
  }

  template <typename A0>
  typename proto::result_of::make_expr<
      proto::tag::function, my_terminal const &, A0 const &>::type const
...
int const *get_pointer(T const &t) { return &t.x; }
...
struct dumb_ptr {
  ...
  friend T const *get_pointer(dumb_ptr const &p) { return p.p; }
...
test_suite *init_unit_test_suite(int argc, char *argv[]) {
  test_suite *test = BOOST_TEST_SUITE(
      "test handling of member pointers by the default transform and default "
      "contexts");
