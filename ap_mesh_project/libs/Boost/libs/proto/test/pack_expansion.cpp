...
#define UNARY_OP(TAG, OP)                                        \
  ...
  typename result<eval_(proto::tag::TAG, Arg)>::type operator()( \
      proto::tag::TAG, Arg arg) const {                          \
    return OP arg;                                               \
  }                                                              \
  /**/
...
#define BINARY_OP(TAG, OP)                                                 \
  ...
  struct result<This(proto::tag::TAG, Left, Right)> {                      \
    BOOST_TYPEOF_NESTED_TYPEDEF_TPL(nested,                                \
                                    (declval<Left>() OP declval<Right>())) \
    typedef typename nested::type type;                                    \
  };                                                                       \
                                                                           \
  template <typename Left, typename Right>                                 \
  typename result<eval_(proto::tag::TAG, Left, Right)>::type operator()(   \
      proto::tag::TAG, Left left, Right right) const {                     \
...
struct eval1 : proto::or_<proto::when<proto::terminal<_>, proto::_value>,
                          proto::otherwise<eval_(proto::tag_of<_>(),
                                                 eval1(proto::pack(_))...)> > {
};

struct eval2
    : proto::or_<proto::when<proto::terminal<_>, proto::_value>,
                 proto::otherwise<proto::call<eval_(
                     proto::tag_of<_>(), eval2(proto::pack(_))...)> > > {};
...
test_suite* init_unit_test_suite(int argc, char* argv[]) {
  test_suite* test =
      BOOST_TEST_SUITE("test immediate evaluation of proto parse trees");
