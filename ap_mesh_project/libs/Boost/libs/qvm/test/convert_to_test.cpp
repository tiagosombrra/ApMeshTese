...
void test_matrix() {
  ...
  test_qvm::matrix<M2, Rows, Cols> const y =
      convert_to<test_qvm::matrix<M2, Rows, Cols> >(x);
...
void test_quaternion() {
  ...
  {
    test_qvm::matrix<M1, 3, 3> const my =
        convert_to<test_qvm::matrix<M1, 3, 3> >(x);
    test_qvm::quaternion<Q1> const qy =
        convert_to<test_qvm::quaternion<Q1> >(my);
    BOOST_QVM_TEST_CLOSE(x.a, qy.a, 0.00001f);
  }
  {
    test_qvm::matrix<M1, 4, 4> const my =
        convert_to<test_qvm::matrix<M1, 4, 4> >(x);
    ...
    test_qvm::quaternion<Q1> const qy =
        convert_to<test_qvm::quaternion<Q1> >(del_row_col<3, 3>(my));
    BOOST_QVM_TEST_CLOSE(x.a, qy.a, 0.00001f);
  }
}
}  // namespace
