...
namespace qvm {
template <class T, int D>
struct vec_traits<v1<T, D> > {
  typedef T scalar_type;
  static int const dim = D;
};

template <class T, int D>
struct vec_traits<v2<T, D> > {
  typedef T scalar_type;
  static int const dim = D;
};

template <int D, class S, int V2D, class V2S>
struct deduce_vec<v2<V2S, V2D>, D, S> {
  typedef v2<S, D> type;
};

template <int D, class S, class AS, int AD, class BS, int BD>
struct deduce_vec2<v2<AS, AD>, v2<BS, BD>, D, S> {
  typedef v2<S, D> type;
};
}  // namespace qvm
}  // namespace boost
...
int main() {
  ...
  same_type<boost::qvm::deduce_vec<v1<int, 3>, 4>::type,
            boost::qvm::vec<int, 4> >();
