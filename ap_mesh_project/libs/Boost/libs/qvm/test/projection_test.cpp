...
namespace {
template <class T>
void test_perspective_lh(T fov_y, T aspect_ratio, T z_near, T z_far) {
  using namespace boost::qvm;
  test_qvm::matrix<M1, 4, 4> const m =
      perspective_lh(fov_y, aspect_ratio, z_near, z_far);
  test_qvm::matrix_perspective_lh(m.b, fov_y, aspect_ratio, z_near, z_far);
  BOOST_QVM_TEST_CLOSE(m.a, m.b, 0.000001f);
}

template <class T>
void test_perspective_rh(T fov_y, T aspect_ratio, T z_near, T z_far) {
  using namespace boost::qvm;
  test_qvm::matrix<M1, 4, 4> const m =
      perspective_rh(fov_y, aspect_ratio, z_near, z_far);
