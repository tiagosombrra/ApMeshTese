...
struct vector {
  ...
  explicit vector(T start = T(0), T step = T(0)) {
    for (int i = 0; i != Dim; ++i, start += step) a[i] = b[i] = start;
  }
};

template <int Dim, class Tag1, class T1, class Tag2, class T2>
void dump_ab(vector<Tag1, Dim, T1> const& a, vector<Tag2, Dim, T2> const& b) {
...
struct deduce_vec2<test_qvm::vector<Tag, D1, T>, test_qvm::vector<Tag, D2, T>,
                   Dim> {
