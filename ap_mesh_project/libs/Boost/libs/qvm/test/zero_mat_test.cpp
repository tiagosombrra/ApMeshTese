...
template <class T, class U>
void check_deduction(T const &, U const &) {
...
void test() {
  ...
  for (int i = 0; i != Rows; ++i)
    for (int j = 0; j != Cols; ++j) BOOST_TEST(!m1.a[i][j]);
  test_qvm::matrix<M2, Rows, Cols> m2(42, 1);
  set_zero(m2);
  for (int i = 0; i != Rows; ++i)
    for (int j = 0; j != Cols; ++j) BOOST_TEST(!m2.a[i][j]);
...
void test() {
  ...
  for (int i = 0; i != Dim; ++i)
    for (int j = 0; j != Dim; ++j) BOOST_TEST(!m1.a[i][j]);
}
}  // namespace
