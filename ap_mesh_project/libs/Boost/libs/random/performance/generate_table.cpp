...
boost::regex generator_regex(
    "(?:fixed-range )?([^:]+): (\\d+(?:\\.\\d+)?) nsec/loop = \\d+(?:\\.\\d+)? "
    "CPU cycles");
boost::regex distribution_regex(
    "([^\\s]+)( virtual function)? ([^:]+): (\\d+(?:\\.\\d+)?) nsec/loop = "
    "\\d+(?:\\.\\d+)? CPU cycles");
...
typedef boost::multi_index_container<
    std::string,
    boost::mpl::vector<boost::multi_index::sequenced<>,
...
int main(int argc, char** argv) {
  ...
  double min = std::min_element(generator_info.begin(), generator_info.end(),
                                compare_second())
                   ->second;

  std::ofstream generator_defs("performance_data.qbk");
  std::ofstream generator_performance(
      ("generator_performance_" + suffix + ".qbk").c_str());
  ...
  BOOST_FOREACH (const pair_type& pair, generator_info) {
    generator_defs << boost::format("[template %s_speed[] %d%%]\n") %
                          template_name(pair.first) %
                          static_cast<int>(100 * min / pair.second);
    generator_performance << boost::format("  [[%s][%g][%g][%d%%]]\n") %
                                 pair.first % (1000 / pair.second) %
                                 pair.second %
                                 static_cast<int>(100 * min / pair.second);
  }
  generator_performance << "]\n";
  ...
  do {
    ...
    if (boost::regex_match(line, match, distribution_regex)) {
      if (!match[2].matched && match[1] != "counting") {
        ...
        distribution_info.insert(
            std::make_pair(std::make_pair(distribution, generator), time));
      }
    } else {
      std::cerr << "oops: " << line << std::endl;
    }
  ...
  std::ofstream distribution_performance(
      ("distribution_performance_" + suffix + ".qbk").c_str());
  ...
  BOOST_FOREACH (const std::string& distribution, distribution_names) {
    ...
    BOOST_FOREACH (const std::string& generator, generator_names) {
      ...
      if (pos != distribution_info.end()) {
        distribution_performance
            << boost::format("[%g]") % (1000 / pos->second);
      } else {
        distribution_performance << "[-]";
      }
    }
