...
class equidistribution_test : test_base {
 public:
  equidistribution_test(test_environment& env, unsigned int classes,
      ...
      : test_base(env),
        classes(classes),
        test_distrib_chi_square(boost::math::chi_squared(classes - 1),
                                high_classes) {}
  ...
  void run(RNG& rng, int n1, int n2) {
    ...
    variate_generator<RNG&, uniform_smallint<> > uint_linear(
        rng, uniform_smallint<>(0, classes - 1));
    ...
    variate_generator<RNG&, uniform_smallint<> > uint_square(
        rng, uniform_smallint<>(0, root - 1));
    check_(run_experiment(test_distrib_chi_square,
                          experiment_generator(equi_2d, uint_square, n1), n2));
    check_(run_experiment(test_distrib_chi_square,
                          experiment_generator(equi_2d, uint_square, n1),
                          2 * n2));
    std::cout << std::endl;
  }
...
class ks_distribution_test : test_base {
 public:
  ks_distribution_test(test_environment& env, unsigned int classes)
      : test_base(env),
        test_distrib_chi_square(kolmogorov_smirnov_probability(5000), classes) {
  }

  template <class RNG>
  void run(RNG& rng, int n1, int n2) {
    boost::math::uniform ud(static_cast<double>((rng.min)()),
 ...
 public:
  ...
  void run(RNG& rng, int n1, int n2) {
    boost::math::uniform ud(
        ...
        static_cast<double>((rng.max)()) +
            (std::numeric_limits<typename RNG::result_type>::is_integer ? 0.0
                                                                        : 1.0));
    run(rng, ud, n1, n2);
  }
...
class poker_test : test_base {
 public:
  ...
  void run(RNG& rng, int n1, int n2) {
    ...
    variate_generator<RNG&, uniform_smallint<> > usmall(
        rng, uniform_smallint<>(0, 7));
    check_(run_experiment(test_distrib_chi_square,
 ...
 public:
  ...
  void run(RNG& rng, int n1, int n2) {
    ...
    boost::variate_generator<RNG&, boost::uniform_int<> > uni(
        rng, boost::uniform_int<>(0, (1 << 25) - 1));
...
class test_environment {
 public:
  ...
  explicit test_environment(double confid)
      : confidence(confid),
        confidence_chi_square_quantil(
            quantile(boost::math::chi_squared(classes - 1), confidence)),
    ...
    std::cout << "Confidence level: " << confid
              << "; 1-alpha = " << (1 - confid) << "; chi_square("
              << (classes - 1) << ", " << confidence_chi_square_quantil
              << ") = "
              << cdf(boost::math::chi_squared(classes - 1),
                     confidence_chi_square_quantil)
              << std::endl;
  }
  ...
  bool check_confidence(double val, double chi_square_conf) const {
    ...
    if (!result) {
      std::cout << "* [";
      double prob = (val > 10 * chi_square_conf
                         ? 1
  ...
  void run_test(const std::string& name, const Dist& dist,
                const ExpectedDist& expected_dist) {
    using namespace boost;
...
void test_base::check_(double val) const { environment.check_(val); }
...
#define TEST(name) \
  if (args.check_(#name)) env.run_test<boost::name>(#name)
  ...
  if (args.check_("normal"))
    env.run_test<boost::mt19937>("normal", boost::normal_distribution<>(),
                                 boost::math::normal());
  if (args.check_("triangle"))
    env.run_test<boost::mt19937>("triangle",
                                 boost::triangle_distribution<>(0, 1, 3),
                                 boost::math::triangular(0, 1, 3));
  if (args.check_("cauchy"))
    env.run_test<boost::mt19937>("cauchy", boost::cauchy_distribution<>(),
                                 boost::math::cauchy());
  if (args.check_("gamma"))
    env.run_test<boost::mt19937>("gamma", boost::gamma_distribution<>(1),
                                 boost::math::gamma_distribution<>(1));
  if (args.check_("exponential"))
    env.run_test<boost::mt19937>("exponential",
                                 boost::exponential_distribution<>(),
                                 boost::math::exponential());
  if (args.check_("lognormal"))
    env.run_test<boost::mt19937>(
        "lognormal", boost::lognormal_distribution<>(1, 1),
        boost::math::lognormal(std::log(1.0 / std::sqrt(2.0)),
