...
class runs_experiment : public experiment_base {
 public:
  ...
  void run(NumberGenerator& f, Counter& count, int n) const {
    ...
    result_type init = runs_direction_helper<
        up, !std::numeric_limits<result_type>::is_integer>::init(result_type());
...
class gap_experiment : public experiment_base {
 public:
  template <class Dist>
  gap_experiment(unsigned int classes, const Dist& dist, double alpha,
                 double beta)
      : experiment_base(classes),
        alpha(alpha),
        beta(beta),
        low(quantile(dist, alpha)),
        high(quantile(dist, beta)) {}
...
class poker_experiment : public experiment_base {
 public:
  ...
  void run(UniformRandomNumberGenerator& f, Counter& count, int n) const {
    ...
    for (int i = 0; i < n; ++i) {
      for (unsigned int j = 0; j < classes(); ++j) v[j] = f();
  ...
  double probability(unsigned int r) const {
    ...
    double result = range;
    for (unsigned int i = 1; i < r; ++i) result *= range - i;
    ...
    if (r == classes() - 1)
      return 1 - fac<double>(d) /
                     std::pow(static_cast<double>(d),
                              static_cast<double>(d + classes() - 2)) *
 ...
 public:
  ...
  void run(UniformRandomNumberGenerator& f, Counter& count, int n_total) const {
    ...
    for (int j = 0; j < n_total; j++) {
      ...
      for (int i = 0; i < n - 1; i++) spacing[i] = v[i + 1] - v[i];
      spacing[n - 1] = v[0] + m - v[n - 1];
      std::sort(spacing.begin(), spacing.end());
      unsigned int k = 0;
      for (int i = 0; i < n - 1; ++i) {
        if (spacing[i] == spacing[i + 1]) ++k;
      }
...
class kolmogorov_smirnov_probability : public distribution_function<double> {
 public:
  kolmogorov_smirnov_probability(int n)
    ...
    if (!approx) n_n = std::pow(static_cast<double>(n), n);
  }
...
class kolmogorov_experiment {
 public:
  ...
  double probability(double x) const { return cdf(ksp, x); }
...
class maximum_experiment {
 ...
 private:
  struct generator {
    ...
    double operator()() {
      double mx = f();
      for (int i = 1; i < t; ++i) mx = (std::max)(mx, f());
      return mx;
    }
...
typename UnaryFunction::result_type chi_square_value(
    ForwardIterator first, ForwardIterator last, UnaryFunction probability) {
...
double run_experiment(const Experiment& experiment, Generator& gen, int n) {
  ...
  return chi_square_value(v.begin(), v.end(),
                          boost::bind(&Experiment::probability, experiment,
                                      boost::placeholders::_1));
}

// chi_square test
template <class Experiment, class Generator>
double run_experiment(const Experiment& experiment, const Generator& gen,
                      int n) {
  generic_counter<std::vector<int> > v(experiment.classes());
  experiment.run(gen, v, n);
  return chi_square_value(v.begin(), v.end(),
...
template <class Experiment, class Generator>
experiment_generator_t<Experiment, Generator> experiment_generator(
    const Experiment& e, Generator& gen, int n) {
