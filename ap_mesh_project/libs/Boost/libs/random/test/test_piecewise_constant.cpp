...
class piecewise_constant {
 public:
  piecewise_constant(const std::vector<double>& intervals,
                     const std::vector<double>& weights)
      : intervals(intervals), cumulative(1, 0.0) {
  ...
  double cdf(double x) const {
    ...
    if (index == 0)
      ...
      return lower_weight +
             (x - lower) / (upper - lower) * (upper_weight - lower_weight);
    }
  }
...
double cdf(const piecewise_constant& dist, double x) { return dist.cdf(x); }

bool do_test(int n, int max) {
  std::cout << "running piecewise_constant(p0, p1, ..., p" << n - 1 << ")"
            << " " << max << " times: " << std::flush;
...
int usage() {
  std::cerr
      << "Usage: test_piecewise_constant -r <repeat> -n <max n> -t <trials>"
      << std::endl;
  return 2;
}
