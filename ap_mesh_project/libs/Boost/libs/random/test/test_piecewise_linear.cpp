...
class piecewise_linear {
 public:
  piecewise_linear(const std::vector<double>& intervals,
                   const std::vector<double>& weights)
      : intervals(intervals), weights(weights), cumulative(1, 0.0) {
    ...
    for (std::vector<double>::iterator iter = cumulative.begin(),
                                       end = cumulative.end();
         iter != end; ++iter) {
      *iter /= sum;
    }
    for (std::vector<double>::iterator iter = this->weights.begin(),
                                       end = this->weights.end();
         iter != end; ++iter) {
      *iter /= sum;
    }
  ...
  double cdf(double x) const {
    ...
    if (index == 0)
      ...
      double mid_weight =
          (lower_weight * (upper - x) + upper_weight * (x - lower)) /
          (upper - lower);
...
double cdf(const piecewise_linear& dist, double x) { return dist.cdf(x); }

bool do_test(int n, int max) {
  std::cout << "running piecewise_linear(p0, p1, ..., p" << n - 1 << ")"
            << " " << max << " times: " << std::flush;
...
int usage() {
  std::cerr << "Usage: test_piecewise_linear -r <repeat> -n <max n> -t <trials>"
            << std::endl;
  return 2;
}
