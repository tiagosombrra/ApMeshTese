...
inline void expected_values(T (&pt)[N][Dimension], std::size_t skip) {
  ...
  for (std::size_t i = 0; i != N; ++i) match_vector(eng, pt[i]);
}
...
inline void discard_function(T (&pt)[N][Dimension], std::size_t skip) {
  Engine eng(Dimension), trivial(Dimension), streamed(Dimension),
...
void test_max_seed(std::size_t dim) {
  ...
  BOOST_REQUIRE_EXCEPTION(eng.seed(maxseed), std::range_error,
                          accept_all_exceptions);
...
void test_max_discard(std::size_t dim) {
  ...
  {
    engine_type eng(dim);
    eng.discard(maxdiscard);  // must succeed, since it's maxdiscard operator()
                              // invocations
    ...
    engine_type other(dim);
    BOOST_REQUIRE_EXCEPTION(other.discard(maxdiscard + 1), std::range_error,
                            accept_all_exceptions);
  }
  ...
  // Test wraparound
  {
    ...
    BOOST_REQUIRE_EXCEPTION(eng.discard(maxdiscard * 2), std::range_error,
                            accept_all_exceptions);
  }
}
...
#define QRNG_VALIDATION_TEST_FUNCTIONS(QRNG)                                  \
  ...
  inline void test_##QRNG##_max_seed() { test::test_max_seed<engine_t>(2); }  \
  inline void test_##QRNG##_max_dimension(std::size_t dim) {                  \
    engine_t eng(dim); /*must succeed*/                                       \
    BOOST_REQUIRE_EXCEPTION(engine_t(dim + 1), std::invalid_argument,         \
                            test::accept_all_exceptions);                     \
  }                                                                           \
                                                                              \
  BOOST_AUTO_TEST_CASE(test_##QRNG##_zero_dimension_fails) {                  \
    BOOST_REQUIRE_EXCEPTION(engine_t(0), std::invalid_argument,               \
                            test::accept_all_exceptions);                     \
  }                                                                           \
  /**/
