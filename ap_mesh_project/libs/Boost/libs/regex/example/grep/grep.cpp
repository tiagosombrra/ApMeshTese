...
void process_stream(std::istream& is) {
  ...
  while (std::getline(is, line)) {
    ...
    if (result) {
      if (print_non_matching_files) return;
      if (files_only) {
  ...
  if (count_only && match_found) {
    std::cout << match_found << " matches found in file " << current_file
              << std::endl;
...
int main(int argc, char* argv[]) {
  try {
    ...
    opts.add_options()("help,h", "produce help message")
        //("after-context,A", po::value<int>(&after_context)->default_value(0),
        //"Print arg  lines  of  trailing  context  after  matching  lines.
        //Places  a  line  containing  --  between  contiguous  groups  of
        //matches.")
        //("before-context,B",
        //po::value<int>(&before_context)->default_value(0), "Print  arg  lines
        //of  leading  context  before  matching lines. Places  a  line
        //containing  --  between  contiguous  groups  of matches.")
        //("context,C", po::value<int>(), "Print  arg lines of output context.
        ...
        ("byte-offset,b",
         ...
         "output.")(
            ...
            "extended-regexp,E",
            "Interpret PATTERN as an POSIX-extended regular expression.")(
            "perl-regexp,P", "Interpret PATTERN as a Perl regular expression.")
        //("regexp,e", po::value<std::string>(&pattern), "Use PATTERN as the
        //pattern; useful to protect patterns beginning with -.")
        ("basic-regexp,G",
         "Interpret arg as a POSIX-basic regular expression (see below).  This "
         "is the default.")("ignore-case,i",
    ...
    po::store(po::command_line_parser(argc, argv)
                  .options(cmdline_options) /*.options(hidden)*/.positional(p)
                  .run(),
              vm);
    po::notify(vm);
