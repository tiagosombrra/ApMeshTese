...
bool is_valid_password(
    const U_NAMESPACE_QUALIFIER UnicodeString& password,
    const U_NAMESPACE_QUALIFIER UnicodeString& requirements) {
  return boost::u32regex_match(password, boost::make_u32regex(requirements));
}

//
// Extract filename part of a path from a UTF-8 encoded std::string and return
// the result as another std::string:
//
...
extract_greek(const U_NAMESPACE_QUALIFIER UnicodeString& text) {
...
void enumerate_currencies(const std::string& text) {
  ...
  boost::u32regex_iterator<std::string::const_iterator> i(
      boost::make_u32regex_iterator(text, r)),
      j;
  while (i != j) {
...
void enumerate_currencies2(const std::string& text) {
  ...
  const char* re =
      ...
      ")";
  boost::u32regex r = boost::make_u32regex(re);
  boost::u32regex_token_iterator<std::string::const_iterator> i(
      boost::make_u32regex_token_iterator(text, r, 1)),
      j;
  while (i != j) {
    std::cout << *i << std::endl;
    ++i;
...
U_NAMESPACE_QUALIFIER UnicodeString
human_readable_card_number(const U_NAMESPACE_QUALIFIER UnicodeString& s) {
...
int main() {
  ...
  U_NAMESPACE_QUALIFIER UnicodeString pwd_check =
      "(?=.*[[:lower:]])(?=.*[[:upper:]])(?=.*[[:punct:]]).{6,}";
  ...
  const UChar t[] = {'S',    'o',    'm',    'e',    ' ', 'w', 'h',
                     'e',    'r',    'e',    ' ',    'i', 'n', 0x0391,
                     0x039D, 0x0395, 0x0398, 0x0391, 0};
  const UChar g[] = {0x0391, 0x039D, 0x0395, 0x0398, 0x0391, 0};
  ...
  std::string text2 =
      " $100.23 or \xC2\xA3"
      "198.12 ";  // \xC2\xA3 is the pound sign encoded in UTF-8
