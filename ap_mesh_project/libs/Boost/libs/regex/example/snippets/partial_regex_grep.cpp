...
void search(std::istream& is) {
  ...
  while (have_more) {
    ...
    boost::regex_grep<bool (*)(const boost::cmatch&), const char*>(
        grep_callback, static_cast<const char*>(buf),
        static_cast<const char*>(buf + read + leftover), e,
...
int main(int argc, char* argv[]) {
  if (argc > 1) {
    ...
    std::string one(
        "<META NAME=\"keywords\" CONTENT=\"regex++, regular expressions, "
        "regular expression library, C++\">");
    std::string what;
