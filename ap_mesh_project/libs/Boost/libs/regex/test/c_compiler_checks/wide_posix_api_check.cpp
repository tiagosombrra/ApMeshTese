...
int main() {
  ...
  if (result > REG_NOERROR) {
    ...
    char nbuf[256];
    for (int i = 0; i < 256; ++i) nbuf[i] = static_cast<char>(buf[i]);
    printf("%s", nbuf);
    return result;
  }
  if (re.re_nsub != 0) {
  ...
  result =
      regexec(&re, text, 1, matches, REG_NOTBOL | REG_NOTEOL | REG_STARTEND);
  if (result > REG_NOERROR) {
    wchar_t buf[256];
    regerror(result, &re, buf, sizeof(buf));
    char nbuf[256];
...
int main() {
  std::cout << "\n<note>\n"
               "This platform does not provide the needed wide character "
