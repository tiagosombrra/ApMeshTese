...
int cpp_main(int /*argc*/, char* /*argv*/[]) {
  ...
  try {
    ...
    std::cerr << "TSS Initialisation failed with message: " << e.what()
              << std::endl;
    return -1;
  }
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      threads.push_back(
          boost::shared_ptr<boost::thread>(new boost::thread(&run_tests)));
    } catch (const std::exception& e) {
      std::cerr << "<note>Thread creation failed with message: " << e.what()
                << "</note>" << std::endl;
    }
  }
  std::list<boost::shared_ptr<boost::thread> >::const_iterator a(
      threads.begin()),
      b(threads.end());
  while (a != b) {
    (*a)->join();
...
int* get_array_data() {
  ...
  if (tp.get() == 0) tp.reset(new boost::array<int, 800>);
...
void test(const char& c, const test_regex_replace_tag& tag) { do_test(c, tag); }
void test(const char& c, const test_regex_search_tag& tag) { do_test(c, tag); }
void test(const char& c, const test_invalid_regex_tag& tag) { do_test(c, tag); }
...
int main(int argc, char* argv[]) { return cpp_main(argc, argv); }
