...
struct call_once_func {
  Regex* pregex;
  void operator()() const { return test_empty(*pregex); }
};
...
void do_test(const charT& c, const tagT& tag) {
...
#else
  test_info<charT>::set_typename(
      typeid(boost::basic_regex<charT, boost::cpp_regex_traits<charT> >)
          .name());
#endif
  ...
  call_once_func<boost::basic_regex<charT, boost::cpp_regex_traits<charT> > >
      proc = {&e1};
  boost::call_once(f, proc);
#endif
  if (test_locale::cpp_locale_state() == test_locale::test_with_locale)
    (void)e1.imbue(test_locale::cpp_locale());
  if (test_locale::cpp_locale_state() != test_locale::no_test) test(e1, tag);
#endif
...
#else
  test_info<charT>::set_typename(
      typeid(boost::basic_regex<charT, boost::c_regex_traits<charT> >).name());
#endif
  boost::basic_regex<charT, boost::c_regex_traits<charT> > e2;
  if (test_locale::c_locale_state() != test_locale::no_test) test(e2, tag);
#endif
...
#else
  test_info<charT>::set_typename(
      typeid(boost::basic_regex<charT, boost::w32_regex_traits<charT> >)
          .name());
#endif
  ...
  if (test_locale::win_locale_state() != test_locale::no_test) test(e3, tag);
#endif
...
#define TEST_REGEX_SEARCH_L(s, f, t, m, a) \
  TEST_REGEX_SEARCH_W(BOOST_JOIN(L, s), f, BOOST_JOIN(L, t), m, a)
#else
...
#define TEST_REGEX_REPLACE_W(s, f, t, m, fs, r)                             \
  do {                                                                      \
    ...
    test_info<wchar_t>::set_info(__FILE__, __LINE__, se, f, sst, m, 0, sft, \
                                 srt);                                      \
...
#define TEST_REGEX_REPLACE(s, f, t, m, fs, r)                    \
  ...
  TEST_REGEX_REPLACE_W(BOOST_JOIN(L, s), f, BOOST_JOIN(L, t), m, \
                       BOOST_JOIN(L, fs), BOOST_JOIN(L, r))
