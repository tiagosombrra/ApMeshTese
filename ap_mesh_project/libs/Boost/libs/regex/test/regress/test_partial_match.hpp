...
void test_sub_match(const boost::sub_match<BidirectionalIterator>& sub,
                    BidirectionalIterator base, const int* answer_table,
                    int i) {
#ifdef BOOST_MSVC
#pragma warning(push)
...
void test(boost::basic_regex<charT, traits>& r, const test_regex_search_tag&) {
  ...
  try {
    ...
    BOOST_REGEX_TEST_ERROR(
        "Received an unexpected std::runtime_error: " << r.what(), charT);
  } catch (const std::exception& r) {
    BOOST_REGEX_TEST_ERROR(
        "Received an unexpected std::exception: " << r.what(), charT);
  } catch (...) {
