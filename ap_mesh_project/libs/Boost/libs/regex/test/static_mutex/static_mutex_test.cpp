...
#define BOOST_CHECK(pred) \
  if (!(pred)) failed_test(__FILE__, __LINE__, BOOST_STRINGIZE(pred));
...
void failed_test(const char* file, int line, const char* pred) {
  ...
  ++total_failures;
  std::cout << "Failed test in \"" << file << "\" at line " << line << ": "
            << pred << std::endl;
}
...
int main() {
  ...
  for (int i = 0; i < 2; ++i) {
    try {
      threads.push_back(
          boost::shared_ptr<boost::thread>(new boost::thread(&thread1_proc)));
  ...
  for (int i = 0; i < 2; ++i) {
    try {
      threads.push_back(
          boost::shared_ptr<boost::thread>(new boost::thread(&thread2_proc)));
    } catch (const std::exception& e) {
      std::cerr << "<note>Thread creation failed with message: " << e.what()
                << "</note>" << std::endl;
    }
  ...
  for (int i = 0; i < 2; ++i) {
    try {
      threads.push_back(
          boost::shared_ptr<boost::thread>(new boost::thread(&thread3_proc)));
    } catch (const std::exception& e) {
      std::cerr << "<note>Thread creation failed with message: " << e.what()
                << "</note>" << std::endl;
    }
