...
inline int digits_of(const T&) {
  return std::numeric_limits<T>::is_specialized ? std::numeric_limits<T>::digits
                                                : 18;
}

inline std::ostream& report_where(const char* file, int line,
                                  const char* function) {
  if (function)
...
#define BOOST_MP_REPORT_WHERE \
  report_where(__FILE__, __LINE__, BOOST_CURRENT_FUNCTION)
...
template <class E>
void report_unexpected_exception(const E& e, int severity, const char* file,
                                 int line, const char* function) {
  report_where(file, line, function)
      << " Unexpected exception of type " << typeid(e).name() << std::endl;
  BOOST_LIGHTWEIGHT_TEST_OSTREAM << "Errot message was: " << e.what()
                                 << std::endl;
...
#define BOOST_MP_UNEXPECTED_EXCEPTION_CHECK(severity)                        \
    ...
    report_unexpected_exception(__e, severity, __FILE__, __LINE__,           \
                                BOOST_CURRENT_FUNCTION);                     \
  }                                                                          \
  catch (...) {                                                              \
    BOOST_LIGHTWEIGHT_TEST_OSTREAM << "Exception of unknown type was thrown" \
                                   << std::endl;                             \
...
#define BOOST_CHECK_IMP(x, severity)                                       \
  try {                                                                    \
    if (x) {                                                               \
      ...
      BOOST_MP_REPORT_WHERE << " Failed predicate: " << BOOST_STRINGIZE(x) \
                            << std::endl;                                  \
      BOOST_MP_REPORT_SEVERITY(severity);                                  \
    }                                                                      \
  }                                                                        \
  ...
  try {                                                                       \
    if (!((x) == (y))) {                                                      \
      BOOST_MP_REPORT_WHERE << " Failed check for equality: \n"               \
                            << std::setprecision(digits_of(x))                \
                            << std::scientific << "Value of LHS was: " << (x) \
                            << "\n"                                           \
                            << "Value of RHS was: " << (y) << "\n"            \
...
#define BOOST_MT_CHECK_THROW_IMP(x, E, severity)                             \
  try {                                                                      \
    x;                                                                       \
    BOOST_MP_REPORT_WHERE << " Expected exception not thrown in expression " \
                          << BOOST_STRINGIZE(x) << std::endl;                \
    BOOST_MP_REPORT_SEVERITY(severity);                                      \
  } catch (const E&) {                                                       \
  }                                                                          \
  BOOST_MP_UNEXPECTED_EXCEPTION_CHECK(severity)
...
bool check_equal_collections(I a, I b, J x, J y) {
  ...
  while (a != b) {
    if (x == y) {
      BOOST_LIGHTWEIGHT_TEST_OSTREAM << " Unexpected end of second sequence"
                                     << std::endl;
      return false;
    }
    if (*a != *x) {
      BOOST_LIGHTWEIGHT_TEST_OSTREAM << "Error occured in position " << i
                                     << " of the collection." << std::endl;
      BOOST_LIGHTWEIGHT_TEST_OSTREAM << "First value was "
                                     << std::setprecision(digits_of(x))
                                     << std::scientific << *a << std::endl;
      BOOST_LIGHTWEIGHT_TEST_OSTREAM << "Second value was "
                                     << std::setprecision(digits_of(x))
...
#define BOOST_CHECK_EQUAL_COLLECTIONS(a, b, x, y) \
  BOOST_MT_CHECK_EQ_COLLECTIONS(a, b, x, y, error_on_fail)
#define BOOST_WARN_EQUAL_COLLECTIONS(a, b, x, y) \
  BOOST_MT_CHECK_EQ_COLLECTIONS(a, b, x, y, warn_on_fail)
#define BOOST_REQUIRE_EQUAL_COLLECTIONS(a, b, x, y) \
  BOOST_MT_CHECK_EQ_COLLECTIONS(a, b, x, y, abort_on_fail)
