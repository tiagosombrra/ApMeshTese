...
void generate_code() {
  ...
  std::cout << "static const BOOST_REGEX_DETAIL_NS::character_pointer_range<"
            << g_char_type << "> range_data[] = {\n";
  std::size_t index = 0;
  for (i = g_table.begin(), j = g_table.end(); i != j; ++i) {
...
void generate_html() {
  ...
  std::map<std::string, std::pair<std::string, std::string> >::const_iterator i,
      j;
  ...
  std::cout << "<table width=\"100%\"><tr><td><b>Short "
               "Name</b></td><td><b>Long Name</b></td></tr>\n";
  while (h != k) {
    std::cout << "<tr><td>" << (h->first.size() ? h->first : std::string(" "))
              << "</td><td>" << h->second << "</td></tr>\n";
    ++h;
  }
  std::cout << "</table>\n\n";
}
