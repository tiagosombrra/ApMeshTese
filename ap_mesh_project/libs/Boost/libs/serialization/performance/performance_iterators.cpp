...
void test_wchar_from_mb(const wchar_t *la, const char *a,
  ...
  BOOST_CHECK((std::equal(translator(BOOST_MAKE_PFTO_WRAPPER(a)),
                          translator(BOOST_MAKE_PFTO_WRAPPER(a + size)), la)));
}
...
void test_mb_from_wchar(const char *a, const wchar_t *la,
  ...
  BOOST_CHECK(std::equal(translator(BOOST_MAKE_PFTO_WRAPPER(la)),
                         translator(BOOST_MAKE_PFTO_WRAPPER(la + size)), a));
}
...
void test_xml_escape(const CharType *xml_escaped, const CharType *xml,
  ...
  BOOST_CHECK(std::equal(translator(BOOST_MAKE_PFTO_WRAPPER(xml)),
...
void test_xml_unescape(const CharType *xml, const CharType *xml_escaped,
  ...
  BOOST_CHECK(
      std::equal(translator(BOOST_MAKE_PFTO_WRAPPER(xml_escaped)),
                 translator(BOOST_MAKE_PFTO_WRAPPER(xml_escaped + size)), xml));
}
...
void test_transform_width(unsigned int size) {
  ...
  char *rptr;
  for (rptr = rawdata + 6; rptr-- > rawdata;) *rptr = std::rand();
  ...
  std::copy(translator1(BOOST_MAKE_PFTO_WRAPPER(static_cast<char *>(rawdata))),
  ...
  BOOST_CHECK(std::equal(rawdata, rawdata + size,
