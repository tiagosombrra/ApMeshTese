...
unsigned char test_data<2>::utf8_encoding[] = {
    0x01, 0x7f, 0xc2, 0x80, 0xdf, 0xbf, 0xe0, 0xa0, 0x80, 0xe7, 0xbf, 0xbf};

template <>
wchar_t test_data<2>::wchar_encoding[] = {0x0001, 0x007f, 0x0080,
                                          0x07ff, 0x0800, 0x7fff};

template <>
unsigned char test_data<4>::utf8_encoding[] = {
    0x01, 0x7f, 0xc2, 0x80, 0xdf, 0xbf, 0xe0, 0xa0, 0x80, 0xef, 0xbf, 0xbf,
    0xf0, 0x90, 0x80, 0x80, 0xf4, 0x8f, 0xbf, 0xbf, 0xf7, 0xbf, 0xbf, 0xbf,
    0xf8, 0x88, 0x80, 0x80, 0x80, 0xfb, 0xbf, 0xbf, 0xbf, 0xbf, 0xfc, 0x84,
    0x80, 0x80, 0x80, 0x80, 0xfd, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf};

template <>
wchar_t test_data<4>::wchar_encoding[] = {
    0x00000001, 0x0000007f, 0x00000080, 0x000007ff, 0x00000800,
    0x0000ffff, 0x00010000, 0x0010ffff, 0x001fffff, 0x00200000,
    0x03ffffff, 0x04000000, 0x7fffffff};

int test_main(int /* argc */, char* /* argv */[]) {
  std::locale old_loc;
  std::locale* utf8_locale = boost::archive::add_facet(
...
// compare the data read back in with the orginal
  ...
  BOOST_CHECK(
      std::equal(from_file.begin(), from_file.end(), td::wchar_encoding));
  ...
  // Send the UCS4_data back out, converting to UTF-8
  {
    ...
    ofs.open("test2.dat");
    std::copy(from_file.begin(), from_file.end(),
  ...
  std::size_t l = std::wcslen(test3_data);
  {
    ...
    ofs.imbue(*utf8_locale);
    ofs.open("test3.dat");
    std::copy(test3_data, test3_data + l,
              boost::archive::iterators::ostream_iterator<wchar_t>(ofs));
  }
  ...
  // Make sure that both files are the same
  {
    ...
    ifs.open("test3.dat");
    BOOST_CHECK(
        std::equal(test3_data, test3_data + l,
