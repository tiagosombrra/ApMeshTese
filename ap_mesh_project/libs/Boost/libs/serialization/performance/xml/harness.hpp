...
template <>
std::string random<std::string>(void) {
  using namespace boost::uuids;
...
template <typename T>
std::string save_archive(T const& s) {
  std::string fn =
      random<std::string>() +
      "-" BOOST_PP_STRINGIZE(BSL_TYPE)
          BOOST_PP_STRINGIZE(BSL_EXP(BSL_NODE_MAX, BSL_DEPTH)) ".xml";
...
template <typename T>
std::pair<double, T> restore_archive(std::string fn) {
...
class result_set_exception : public virtual archive_exception {
 public:
  enum exception_code { invalid_archive_metadata };
...
struct entry {
  ...
  void serialize(ARC& ar, const unsigned int) {
    ar& BOOST_SERIALIZATION_NVP(type) & BOOST_SERIALIZATION_NVP(size) &
...
struct result_set {
  ...
  template <class ARC>
  void serialize(ARC& ar, const unsigned int) {
    ar& BOOST_SERIALIZATION_NVP(compiler) & BOOST_SERIALIZATION_NVP(platform) &
        BOOST_SERIALIZATION_NVP(entries);

    if ((compiler != BOOST_COMPILER) || (platform != BOOST_PLATFORM))
      throw result_set_exception();
  }

  result_set(void) : compiler(BOOST_COMPILER), platform(BOOST_PLATFORM) {}

  result_set(std::list<entry> entries)
      : compiler(BOOST_COMPILER), platform(BOOST_PLATFORM), entries(entries) {}
};
