...
inline void basic_iarchive_impl::reset_object_address(
  ...
  if (m_moveable_objects.is_pointer) return;
  ...
  for (; i < m_moveable_objects.end; ++i) {
    if (old_address == object_id_vector[i].address) break;
  }
  for (; i < m_moveable_objects.end; ++i) {
    const aobject &ao = object_id_vector[i];
    if (ao.loaded_as_pointer) continue;
...
inline const basic_pointer_iserializer *basic_iarchive_impl::load_pointer(
  ...
  if (class_id_type(cobject_info_set.size()) <= cid) {
    ...
    if (NULL == bpis_ptr
      ...
      if (0 != key[0]) eti = serialization::extended_type_info::find(key);
      if (NULL == eti)
