...
class extended_type_info_arg : public extended_type_info {
  bool is_less_than(const extended_type_info & /*rhs*/) const BOOST_OVERRIDE {
    BOOST_ASSERT(false);
    return false;
  }
  bool is_equal(const extended_type_info & /*rhs*/) const BOOST_OVERRIDE {
    BOOST_ASSERT(false);
    return false;
  }
  const char *get_debug_info() const BOOST_OVERRIDE { return get_key(); }
...
BOOST_SERIALIZATION_DECL bool extended_type_info::operator<(
  ...
  if (m_type_info_key < rhs.m_type_info_key) return true;
  return false;
}
