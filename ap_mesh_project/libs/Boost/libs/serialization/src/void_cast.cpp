...
bool void_caster::operator<(const void_caster &rhs) const {
  ...
  if (m_derived != rhs.m_derived) {
    if (*m_derived < *rhs.m_derived) return true;
    if (*rhs.m_derived < *m_derived) return false;
  }
...
class void_caster_shortcut : public void_caster {
 ...
 public:
  ...
  ~void_caster_shortcut() BOOST_OVERRIDE { recursive_unregister(); }
};
  ...
  for (it = s.begin(); it != s.end(); ++it) {
    ...
    if ((*it)->m_base == m_base) {
      ...
      if ((*it)->m_derived != m_derived) {
        ...
        if (NULL != t_new) return (*it)->upcast(t_new);
      }
    }
  }
  return NULL;
...
BOOST_SERIALIZATION_DECL void void_caster::recursive_unregister() const {
  ...
  if (void_caster_registry::is_destroyed()) return;
...
BOOST_SERIALIZATION_DECL void const *void_upcast(
  ...
  void_cast_detail::set_type::const_iterator it;
  it = s.find(&ca);
  if (s.end() != it) return (*it)->upcast(t);
...
BOOST_SERIALIZATION_DECL void const *void_downcast(
  ...
  void_cast_detail::set_type::const_iterator it;
  it = s.find(&ca);
  if (s.end() != it) return (*it)->downcast(t);
