...
A::A()
    : b(true),
#ifndef BOOST_NO_INT64_T
      f(static_cast<boost::int64_t>(std::rand()) *
        static_cast<boost::int64_t>(std::rand())),
      g(static_cast<boost::uint64_t>(std::rand()) *
        static_cast<boost::uint64_t>(std::rand())),
#endif
...
A_DLL_DECL bool A::operator==(const A &rhs) const {
  ...
  if (0 != y.compare(rhs.y)) return false;
#ifndef BOOST_NO_STD_WSTRING
  if (0 != z.compare(rhs.z)) return false;
#endif
  return true;
}

A_DLL_DECL bool A::operator!=(const A &rhs) const { return !(*this == rhs); }
