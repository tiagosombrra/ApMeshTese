...
void save_exported(const char *testfile) {
  ...
  boost::serialization::extended_type_info const *const d2_eti =
      boost::serialization::extended_type_info::find("polymorphic_derived2");
  assert(NULL != d2_eti);
  ...
  polymorphic_base const *const rb2 =
      static_cast<polymorphic_base const *>(boost::serialization::void_upcast(
          *d2_eti,
...
void load_exported(const char *testfile) {
  ...
  BOOST_CHECK_MESSAGE(
      *d2_eti == *boost::serialization::type_info_implementation<
                      polymorphic_base>::type::get_const_instance()
...
int test_main(int /* argc */, char * /* argv */[]) {
  ...
  BOOST_CHECK_MESSAGE((0 != hDLL), "Failed to find/load polymorphic_derived2");
  if (0 == hDLL) return EXIT_FAILURE;
