...
// This demonstrates a problem with boost::serialization and
// boost::enable_shared_from_this. (boost version 1.53) See boost TRAC ticket
// #9567
//
...
struct enable_shared_from_this_helper {
  ...
  void record(boost::shared_ptr<void> sp) { m_esfth.insert(sp); }
};

template <class Archive, class T>
void serialize(Archive& ar, boost::enable_shared_from_this<T>& t,
               const unsigned int file_version) {
  enable_shared_from_this_helper& h =
      ar.template get_helper<enable_shared_from_this_helper>();
...
class Base {
 ...
 protected:
  Base() {}
  virtual ~Base() {}  // "virtual" forces RTTI, to enable serialization of
                      // Derived from Base pointer
 public:
  int m_base;
};
