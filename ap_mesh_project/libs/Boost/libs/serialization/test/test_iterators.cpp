...
void test_wchar_from_mb(const wchar_t *la, const char *a,
                        const unsigned int size) {
  typedef boost::archive::iterators::wchar_from_mb<const char *> translator;
  BOOST_CHECK((std::equal(translator(a), translator(a + size), la)));
}

void test_mb_from_wchar(const char *a, const wchar_t *la,
                        const unsigned int size) {
  typedef boost::archive::iterators::mb_from_wchar<const wchar_t *> translator;
  BOOST_CHECK(std::equal(translator(la), translator(la + size), a));
}
...
void test_roundtrip(const wchar_t *la) {
  ...
  std::vector<char> a;
  {
    typedef boost::archive::iterators::mb_from_wchar<const wchar_t *>
        translator;
    std::copy(translator(la), translator(la + s), std::back_inserter(a));
  ...
  {
    typedef boost::archive::iterators::wchar_from_mb<
        std::vector<char>::const_iterator>
        translator;
    std::copy(translator(a.begin()), translator(), std::back_inserter(la2));
  }
...
void test_xml_escape(const CharType *xml_escaped, const CharType *xml,
                     unsigned int size) {
  typedef boost::archive::iterators::xml_escape<const CharType *> translator;

  BOOST_CHECK(std::equal(translator(xml), translator(xml + size), xml_escaped));
}

template <class CharType>
void test_xml_unescape(const CharType *xml, const CharType *xml_escaped,
                       unsigned int size) {
  // test xml_unescape
  typedef boost::archive::iterators::xml_unescape<const CharType *> translator;

  BOOST_CHECK(
      std::equal(translator(xml_escaped), translator(xml_escaped + size), xml));
}
...
void test_transform_width(unsigned int size) {
  ...
  typedef boost::archive::iterators::transform_width<char *, BitsOut, BitsIn>
      translator1;

  std::vector<char> vout;

  std::copy(translator1(static_cast<char *>(rawdata)),
            translator1(rawdata + size), std::back_inserter(vout));
  ...
  std::vector<char> vin;
  std::copy(translator2(vout.begin()), translator2(vout.end()),
            std::back_inserter(vin));

  // check to see we got the expected # of characters out
  BOOST_CHECK(vin.size() == size);

  BOOST_CHECK(std::equal(rawdata, rawdata + size, vin.begin()));
}

template <class CharType>
void test_stream_iterators(const CharType *test_data, unsigned int size) {
...
int test_main(int /* argc */, char * /* argv */[]) {
  ...
  test_xml_escape<const char>(xml_escaped, xml, sizeof(xml) / sizeof(char) - 1);
  test_xml_unescape<const char>(xml, xml_escaped,
                                sizeof(xml_escaped) / sizeof(char) - 1);

#ifndef BOOST_NO_CWCHAR
  const wchar_t wxml[] = L"<+>+&+\"+'";
  const wchar_t wxml_escaped[] = L"&lt;+&gt;+&amp;+&quot;+&apos;";
  test_xml_escape<const wchar_t>(wxml_escaped, wxml,
                                 sizeof(wxml) / sizeof(wchar_t) - 1);
  test_xml_unescape<const wchar_t>(wxml, wxml_escaped,
