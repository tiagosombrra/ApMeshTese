...
void test_base64(unsigned int size) {
  ...
  typedef boost::archive::iterators::insert_linebreaks<
      boost::archive::iterators::base64_from_binary<
          boost::archive::iterators::transform_width<CharType*, 6,
                                                     sizeof(CharType) * 8> >,
      76>
      translate_out;

  std::copy(translate_out(static_cast<CharType*>(rawdata)),
            translate_out(rawdata + size), std::back_inserter(text_base64));
  ...
  BOOST_CHECK(
      std::equal(rawdata, rawdata + size, translate_in(text_base64.begin())));
}
