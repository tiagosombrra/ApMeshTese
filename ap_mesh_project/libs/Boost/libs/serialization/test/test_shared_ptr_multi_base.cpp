...
struct Sub : public Base1, public Base2, public Base3 {
  static int count;
  Sub() { ++count; }
  Sub(int x) : Base1(x), Base2(x), m_x(x) { ++count; }
  Sub(const Sub& rhs) : m_x(rhs.m_x) { ++count; }
  virtual ~Sub() {
    assert(0 < count);
    --count;
  }
...
void save2(const char* testfile, const FIRST& first, const SECOND& second) {
...
template <class FIRST, class SECOND>
void load2(const char* testfile, FIRST& first, SECOND& second) {
...
void shared_weak(SP& first, WP& second) {
...
void weak_shared(WP& first, SP& second) {
  const char* testfile = boost::archive::tmpnam(NULL);
  BOOST_REQUIRE(NULL != testfile);
