...
template <class Archive>
void serialize(Archive &ar, B &b, const unsigned int file_version) {
...
template <class Archive>
void save(Archive & /* ar */, const C &c, const unsigned int /* file_version */
) {
  ++(const_cast<C &>(c).count);
}

template <class Archive>
void load(Archive & /* ar */, C &c, const unsigned int /* file_version */
) {
  --c.count;
}
