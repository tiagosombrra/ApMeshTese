...
// So, we'll just to skip it for this compiler
  ...
  boost::variant<bool, float, int> little_variant;
  {
    ...
    BOOST_TRY { ia >> BOOST_SERIALIZATION_NVP(little_variant); }
...
inline bool operator==(H const &lhs, H const &rhs) { return lhs.i == rhs.i; }

inline bool operator!=(H const &lhs, H const &rhs) { return !(lhs == rhs); }

inline bool operator<(H const &lhs, H const &rhs) { return lhs.i < rhs.i; }

inline std::size_t hash_value(H const &val) { return val.i; }
