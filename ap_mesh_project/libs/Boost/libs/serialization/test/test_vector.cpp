...
int test_main(int /* argc */, char* /* argv */[]) {
  ...
  if (res == EXIT_SUCCESS) res = test_default_constructible<int>();
  // test a bool vector
  if (res == EXIT_SUCCESS) res = test_default_constructible<bool>();
  if (res == EXIT_SUCCESS) res = test_non_default_constructible();
  return res;
}
