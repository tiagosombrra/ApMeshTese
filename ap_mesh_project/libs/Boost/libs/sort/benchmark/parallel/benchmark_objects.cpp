...
int Test(std::vector<IA> &B, rightshift shift, compare comp,
         std::vector<double> &V);
...
void Generator_sorted_end(uint64_t N, size_t n_last) {
  ...
  gen.reset();
  A.clear();
  for (uint32_t i = 0; i < (N + n_last); i++) A.emplace_back(IA::generate(gen));
...
void Generator_sorted_middle(uint64_t N, size_t n_last) {
...

  {
    ...
    for (size_t i = N; i < C.size(); ++i) B.push_back(std::move(C[i]));
    ...
    for (size_t i = 0; i < B.size(); ++i) {
      A.push_back(B[i]);
      for (size_t k = 0; k < step; ++k) A.push_back(C[pos++]);
    };
    while (pos < C.size()) A.push_back(C[pos++]);
  };
...
void Generator_reverse_sorted_end(uint64_t N, size_t n_last) {
  ...
  for (uint32_t i = 0; i < (N + n_last); i++) A.emplace_back(IA::generate(gen));
  std::sort(A.begin(), A.begin() + N, H_comp<IA>());
  for (size_t i = 0; i < (N >> 1); ++i) std::swap(A[i], A[N - 1 - i]);
...
void Generator_reverse_sorted_middle(uint64_t N, size_t n_last) {
...

  {
    ...
    for (size_t i = 0; i < (N >> 1); ++i) std::swap(C[i], C[N - 1 - i]);
...
int Test(std::vector<IA> &B, rightshift shift, compare comp,
  ...
  /*
      ...
      //    cout << "identical: " << identical << " pre-sorted: " <<
     sorted_count << std::endl; V.push_back (duration);
      //cout << duration << " secs\n";
  */
  return 0;
};
