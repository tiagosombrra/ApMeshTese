...
void Generator_sorted_middle(size_t n_last) {
  ...
  for (size_t i = NMAXSTRING; i < A.size(); ++i) B.push_back(std::move(A[i]));
  A.resize(NMAXSTRING);
  std::sort(A.begin(), A.end());
  ...
  for (size_t i = 0; i < B.size(); ++i, pos += step) {
    C.push_back(B[i]);
    for (size_t k = 0; k < step; ++k) C.push_back(A[pos + k]);
  };
...
void Generator_reverse_sorted(void) {
  ...
  A.reserve(NMAXSTRING);
  {
    ...
    A.clear();
    for (size_t i = 0; i < NMAXSTRING; ++i) A.push_back(B[NMAXSTRING - 1 - i]);
  };
  Test<std::string, std::less<std::string>>(A);
