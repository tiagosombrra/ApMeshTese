...
const unsigned bit_shift =
    detail::rough_log_2_size(max_count) - detail::int_log_mean_bin_size;
const unsigned radix_threshold = detail::rough_log_2_size(max_count) + 1;

const DATA_TYPE typed_one = 1;

void fill_vector(vector<DATA_TYPE> &input, const DATA_TYPE base_value,
...
unsigned get_index(unsigned count) {
  unsigned result =
      unsigned((rand() % (1 << 16)) * uint64_t(count) / (1 << 16));
  if (result >= count) return count - 1;
  return result;
}

// Tests std::sort vs boost::sort::spreadsort on boost::sort's worst
// distribution.
int main(int, const char **) {
  ...
  for (int repetition = 0; repetition < 10; ++repetition) {
    ...
    for (int ii = (1 << bit_length) - 1; ii >= 0; --ii)
      fill_vector(input, ii, total_length - bit_length, offsets,
                  offsets.size() - 1);
