...
int main(int argc, const char **argv) {
  ...
  if (argc > 1) high_shift = atoi(argv[1]);
  if (argc > 2) low_shift = atoi(argv[2]);
  if (argc > 3) count = atoi(argv[3]);
  if (high_shift > 16) high_shift = 16;
  if (low_shift > 16) low_shift = 16;
  std::ofstream ofile;
  ...
  if (count < uDivideFactor * 100) uDivideFactor = count;
  unsigned *pNumbers =
      static_cast<unsigned *>(malloc(uDivideFactor * sizeof(unsigned)));
