...
const unsigned absolute_min_count =
    (std::min)(1 << int_min_log_count, 1 << float_min_log_count);
...
void get_min_count_test() {
  ...
  for (int log_range = 0; log_range <= max_int_bits; ++log_range) {
    ...
    if (log_range > min_log_size) {
      ...
      covered_log_range +=
          (base_iterations * (log_min_split_count * 2 + base_iterations - 1)) /
              2 +
          log_mean_bin_size;
...
void get_log_divisor_test() {
  for (int log_range = 0; log_range <= max_int_bits; ++log_range) {
    ...
    for (int log_count = 0; log_count < max_size_bits; ++log_count) {
      ...
      if (count >= absolute_min_count) BOOST_CHECK(log_divisor <= log_range);
...
void is_sorted_or_find_extremes_test() {
  ...
  BOOST_CHECK(detail::is_sorted_or_find_extremes(
      sorted_input.begin(), sorted_input.end(), max, min, std::less<int>()));
  BOOST_CHECK(!detail::is_sorted_or_find_extremes(
      sorted_input.begin(), sorted_input.end(), max, min, std::greater<int>()));
  BOOST_CHECK(*min == 5);
  BOOST_CHECK(*max == 1);
  ...
  int div_max, div_min;
  BOOST_CHECK(detail::is_sorted_or_find_extremes(
      sorted_float_input.begin(), sorted_float_input.end(), div_max, div_min));
  // Test an unsorted input.
  BOOST_CHECK(!detail::is_sorted_or_find_extremes(
      float_input.begin(), float_input.end(), div_max, div_min));
