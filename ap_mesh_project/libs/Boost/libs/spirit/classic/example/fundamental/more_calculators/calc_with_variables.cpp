...
///////////////////////////////////////////////////////////////////////////////
struct push_num {
  push_num(stack<double>& eval_) : eval(eval_) {}
...
struct do_op {
  do_op(op const& the_op, stack<double>& eval_) : m_op(the_op), eval(eval_) {}
...
template <class op>
do_op<op> make_op(op const& the_op, stack<double>& eval) {
  return do_op<op>(the_op, eval);
}

struct do_negate {
  do_negate(stack<double>& eval_) : eval(eval_) {}
...
struct get_var {
  get_var(stack<double>& eval_) : eval(eval_) {}
...
struct set_var {
  set_var(double*& var_) : var(var_) {}

  void operator()(double& n) const { var = &n; }
...
struct do_assign {
  do_assign(double*& var_, stack<double>& eval_) : var(var_), eval(eval_) {}
...
struct calculator : public grammar<calculator> {
  calculator(stack<double>& eval_) : eval(eval_) {}

  template <typename ScannerT>
  struct definition {
    definition(calculator const& self) {
      factor = real_p[push_num(self.eval)] | vars[get_var(self.eval)] |
               '(' >> expression >> ')' | ('-' >> factor)[do_negate(self.eval)];
    ...
    symbols<double> vars;
    rule<ScannerT> statement, declaration, var_decl, assignment, expression,
        term, factor;
