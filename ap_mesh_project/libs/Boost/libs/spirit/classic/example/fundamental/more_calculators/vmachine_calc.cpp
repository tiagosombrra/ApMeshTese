...
class vmachine {
 public:
  vmachine(unsigned stackSize = 1024)
      : stack(new int[stackSize]), stackPtr(stack) {}
...
void vmachine::execute(int code[]) {
  int const* pc = code;
  bool running = true;
  stackPtr = stack;
...
///////////////////////////////////////////////////////////////////////////////
struct push_int {
  push_int(vector<int>& code_) : code(code_) {}
...
struct push_op {
  push_op(int op_, vector<int>& code_) : op(op_), code(code_) {}
...
static bool compile(GrammarT const& calc, char const* expr) {
  cout << "\n/////////////////////////////////////////////////////////\n\n";

  parse_info<char const*> result = parse(expr, calc, space_p);
  ...
  if (result.full) {
    ...
    cout << "\t\t";
    for (int i = 0; i < (result.stop - expr); i++) cout << " ";
    cout << "^--Here\n\n\n";
    return false;
  }
}
...
struct calculator : public grammar<calculator> {
  calculator(vector<int>& code_) : code(code_) {}

  template <typename ScannerT>
  struct definition {
    definition(calculator const& self) {
      integer = lexeme_d[(+digit_p)[push_int(self.code)]];

      factor = integer | '(' >> expression >> ')' |
               ('-' >> factor)[push_op(OP_NEG, self.code)] | ('+' >> factor);

      term = factor >> *(('*' >> factor)[push_op(OP_MUL, self.code)] |
                         ('/' >> factor)[push_op(OP_DIV, self.code)]);

      expression = term >> *(('+' >> term)[push_op(OP_ADD, self.code)] |
