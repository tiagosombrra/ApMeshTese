...
int main() {
  ...
  if (result.full) {
    cout << "Successfully parsed test string (should not happen)!" << endl;
  } else {
    cout << "Correctly failed parsing the test string (without refactoring)!"
         << endl;
  }
  cout << endl;
  ...
  result = parse(
      test_string2,
      refactor_action_d[(*(anychar_p - '$'))[refactor_action_actor(str)] >>
                        '$']);
  ...
  if (result.full) {
    cout << "Successfully refactored an action attached to an unary!" << endl;
    cout << "Parsed: \"" << str << "\"" << endl;
  } else {
    cout << "Failed to refactor an action!" << endl;
  }
