...
struct ipv4_prefix_data {
  ...
  ipv4_prefix_data() : prefix_len(0), n0(0), n1(0), n2(0), n3(0) {}
};
...
struct ipv4_data {
  ...
  ipv4_data() : packet_len(0), header_len(0) {}
};
...
struct ipv4 : public grammar<ipv4> {
  ...
  struct definition {
    definition(ipv4 const& self) {
      packet = '\xff' >> anychar_p[var(self.data.packet_len) = arg1] >> payload;

      payload = anychar_p[var(self.data.header_len) = arg1] >>
                for_p(var(i) = 0, var(i) < var(self.data.header_len),
                      ++var(i))[anychar_p[var(self.data.header) += arg1]] >>
                *ipv4_prefix;

      ipv4_prefix =
          anychar_p[var(temp.prefix_len) = arg1, var(temp.n0) = 0,
                    var(temp.n1) = 0, var(temp.n2) = 0, var(temp.n3) = 0]

          >>
          if_p(
              var(temp.prefix_len) >
              0x00)[anychar_p[var(temp.n0) = arg1] >>
                    if_p(
                        var(temp.prefix_len) >
                        0x08)[anychar_p[var(temp.n1) = arg1] >>
                              if_p(var(temp.prefix_len) >
                                   0x10)[anychar_p[var(temp.n2) = arg1] >>
    ...
    rule<ScannerT> const& start() const { return packet; }
  };

  ipv4(ipv4_data& data) : data(data) {}
...
void print_prefix(ipv4_prefix_data const& prefix) {
...
void parse_ipv4(char const* str, unsigned len) {
  ipv4_data data;
  ipv4 g(data);
...
char const i3[32] = {0xff, 0x20, 0x15, 'p',  'r',  'i',  'v',  'a',
                     't',  'e',  ' ',  'a',  'd',  'd',  'r',  'e',
                     's',  's',  ' ',  's',  'p',  'a',  'c',  'e',
