...
struct my_grammar : public grammar<my_grammar, parser_context<int> > {
  // grammar definition
  ...
  struct definition {
    ...
    typedef boost::spirit::classic::closure<block_closure, int,
    ...
    definition(my_grammar const& self) {
      ...
      block_item =
          // use the stored rule
          lazy_p(block.int_rule)[block.sum += arg1] | block[block.sum += arg1];

      // bind base keywords and number parsers
      base.add("bin", bin_p)("dec", uint_p);
    }

    block_rule_t const& start() const { return block; }
  };
};
