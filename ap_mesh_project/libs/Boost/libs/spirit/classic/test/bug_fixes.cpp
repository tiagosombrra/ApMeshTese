...
void bug_005() {
  BOOST_TEST(parse("   aaaaaaaaa     ", *ch_p('a') >> end_p, space_p).full);
  ...
  BOOST_TEST(parse("   aaaaaaaaa     ", *ch_p('a') >> end_p, ch_p(' ')).full);

  BOOST_TEST(
      parse("   aaaaaaaaa     ", lexeme_d[*ch_p('a')] >> end_p, ch_p(' '))
          .full);
...
void bug_006() { BOOST_TEST(parse("#some comment", comment_p('#')).full); }
...
void sf_bug_718903() {
  empty_match_parser<chset<char> > e(epsilon_p(chset_p("abc")));
}
...
//
///////////////////////////////////////////////////////////////////////////////
void bug_009() { parse("test", limit_d(1U, 10U)[uint_p] | str_p("test")); }
