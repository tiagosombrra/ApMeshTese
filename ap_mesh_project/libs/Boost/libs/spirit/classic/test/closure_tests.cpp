...
//  If debugging is switched on, all closure members should have a
//  corresponding output streaming operator
std::ostream& operator<<(std::ostream& o, Y const& /*x*/) {
  o << "Y";
  return o;
}
#endif  // defined(BOOST_SPIRIT_DEBUG)
...
struct my_closure6
    : BOOST_SPIRIT_CLASSIC_NS::closure<my_closure6, int, int, int> {
  member1 x;
  member2 y;
  member3 z;
};
...
void closure_tests() {
  ...
  num_list = (real_p[num_list.val = arg1] >>
              *(',' >> real_p[num_list.val += arg1]))[var(n) = num_list.val];
  ...
  complex_p = int_p[bind(&X::a)(complex_p.x) = arg1] >> ',' >>
