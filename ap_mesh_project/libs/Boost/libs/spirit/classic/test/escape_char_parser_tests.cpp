...
int main() {
  ...
  swprintf(octmax, octmax_size, L"\\%lo",
           (unsigned long)(std::numeric_limits<wchar_t>::max)());
  ...
  // WCHAR_MAX + 1
  swprintf(octmax, octmax_size, L"\\%lo%lo", ulwcmax / 8 + (ulwcmax % 8 == 7),
           (ulwcmax - 7) % 8);
  ...
  swprintf(hexmax, hexmax_size, L"\\x%lx",
           (unsigned long)(std::numeric_limits<wchar_t>::max)());
  BOOST_TEST(parse(hexmax, wlep[assign_a(wc)]).full);
  BOOST_TEST(wc == (std::numeric_limits<wchar_t>::max)());

  // WCHAR_MAX + 1
  swprintf(hexmax, hexmax_size, L"\\x%lx%lx",
           ulwcmax / 16 + (ulwcmax % 16 == 15), (ulwcmax - 15) % 16);
