...
static void make_definition(GrammarT &g) {
...
template <typename GrammarT>
static void make_definition3(GrammarT &g) {
  char const *text = "blah";
  BOOST_SPIRIT_CLASSIC_NS::scanner<> s(text, text + 4);
...
struct single_grammar_object_task {
  void operator()() const { make_definition3(simple1_p); };
...
////////////////////////////////////////////////////////////////////////////////
static void single_local_grammar_object_multiple_threads() {
...
static void multiple_local_grammar_objects_multiple_threads() {
...
struct single_global_grammar_object_task {
  void operator()() const { make_definition3(global_simple1_p); };
};

static void single_global_grammar_object_multiple_threads() {
...
static void multiple_global_grammar_objects_multiple_threads() {
  // check wether exactly one definition per thread
  // and per grammar object is created
  count_guard guard(simple_definition_count);
