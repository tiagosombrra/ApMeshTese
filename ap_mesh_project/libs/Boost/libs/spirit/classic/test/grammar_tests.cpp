...
struct num_list : public grammar<num_list> {
  enum { default_rule = 0, num_rule = 1 };
...
struct num_list_ex : public grammar<num_list_ex> {
  enum { default_rule = 0, num_rule = 1, integer_rule = 2 };
...
void grammar_tests() {
  ...
  pi = parse("123, 456, 789", nlistex.use_parser<num_list_ex::default_rule>(),
             space_p);
  BOOST_TEST(pi.hit);
  BOOST_TEST(pi.full);
  ...
  pi = parse("123", nlistex.use_parser<num_list_ex::integer_rule>(), space_p);
  BOOST_TEST(pi.hit);
  BOOST_TEST(pi.full);
}
