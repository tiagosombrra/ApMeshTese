...
typedef ::BOOST_SPIRIT_CLASSIC_NS::rule<
    BOOST_SPIRIT_CLASSIC_NS::no_actions_scanner<>::type>
    no_actions_rule_t;
...
void test_number(char const* s, unsigned int wanted, rule_t const& r) {
  using namespace std;
...
void test_enclosed_fail() {
  ...
  ::BOOST_SPIRIT_CLASSIC_NS::parse_info<> m =
      ::BOOST_SPIRIT_CLASSIC_NS::parse(p, r);
...
int main() {
  ...
  auto_number_rule =
      if_p(hex_prefix)[hex_rule]
          .else_p[if_p(::BOOST_SPIRIT_CLASSIC_NS::eps_p(oct_prefix))[oct_rule]
                      .else_p[dec_rule]];
