...
struct ts_real_parser_policies : public ureal_parser_policies<T> {
  ...
  static typename parser_result<uint2_t, ScannerT>::type parse_frac_n(
      ScannerT& scan) {
  ...
  //////////////////////////////////  No exponent
  template <typename ScannerT>
  static typename parser_result<chlit<>, ScannerT>::type parse_exp(
      ScannerT& scan) {
  ...
  //////////////////////////////////  No exponent
  template <typename ScannerT>
  static typename parser_result<int_parser_t, ScannerT>::type parse_exp_n(
      ScannerT& scan) {
    return scan.no_match();
  }
  ...
  template <typename ScannerT>
  static typename parser_result<uint_parser_t, ScannerT>::type parse_n(
      ScannerT& scan) {
...
real_parser<double, ts_real_parser_policies<double> > const ts_real_p =
    real_parser<double, ts_real_parser_policies<double> >();
...
real_parser<double, no_trailing_dot<double> > const notrdot_real_p =
    real_parser<double, no_trailing_dot<double> >();
...
real_parser<double, no_leading_dot<double> > const nolddot_real_p =
    real_parser<double, no_leading_dot<double> >();
...
#define LONG_LONG_MIN (-LONG_LONG_MAX - 1)
  ...
  BOOST_TEST(parse("1.2e3", ureal_p[assign_a(d)]).full &&
             d == 1.2e3);  //  Good.
  BOOST_TEST(parse("1.2e-3", ureal_p[assign_a(d)]).full &&
             d == 1.2e-3);                                            //  Good.
  BOOST_TEST(parse("1.e2", ureal_p[assign_a(d)]).full && d == 1.e2);  //  Good.
  BOOST_TEST(parse(".2e3", ureal_p[assign_a(d)]).full && d == .2e3);  //  Good.
  BOOST_TEST(parse("2e3", ureal_p[assign_a(d)]).full &&
             d == 2e3);                        //  Good. No fraction
  ...
  BOOST_TEST(parse("-1.2e3", real_p[assign_a(d)]).full &&
             d == -1.2e3);  //  Good.
  BOOST_TEST(parse("+1.2e3", real_p[assign_a(d)]).full &&
             d == 1.2e3);                                            //  Good.
  BOOST_TEST(parse("-0.1", real_p[assign_a(d)]).full && d == -0.1);  //  Good.
  BOOST_TEST(parse("-1.2e-3", real_p[assign_a(d)]).full &&
             d == -1.2e-3);                                            //  Good.
  BOOST_TEST(parse("-1.e2", real_p[assign_a(d)]).full && d == -1.e2);  //  Good.
  BOOST_TEST(parse("-.2e3", real_p[assign_a(d)]).full && d == -.2e3);  //  Good.
  BOOST_TEST(parse("-2e3", real_p[assign_a(d)]).full &&
             d == -2e3);                     //  Good. No fraction
  BOOST_TEST(!parse("-e3", real_p).full);    //  Bad! No number
  BOOST_TEST(!parse("-1.2e", real_p).full);  //  Bad! No exponent

  BOOST_TEST(
      !parse("1234", strict_ureal_p[assign_a(d)]).full);  //  Bad. Strict real
  BOOST_TEST(parse("1.2", strict_ureal_p[assign_a(d)]).full &&
             d == 1.2);  //  Good.
  BOOST_TEST(
      !parse("-1234", strict_real_p[assign_a(d)]).full);  //  Bad. Strict real
  BOOST_TEST(parse("123.", strict_real_p[assign_a(d)]).full &&
             d == 123);  //  Good.
  BOOST_TEST(parse("3.E6", strict_real_p[assign_a(d)]).full &&
             d == 3e6);  //  Good.

  BOOST_TEST(
      !parse("1234.", notrdot_real_p[assign_a(d)]).full);  //  Bad trailing dot
  BOOST_TEST(
      !parse(".1234", nolddot_real_p[assign_a(d)]).full);  //  Bad leading dot

  //  Special thousands separated numbers

  BOOST_TEST(parse("123,456,789.01", ts_real_p[assign_a(d)]).full &&
             d == 123456789.01);  //  Good.
  BOOST_TEST(parse("12,345,678.90", ts_real_p[assign_a(d)]).full &&
             d == 12345678.90);  //  Good.
  BOOST_TEST(parse("1,234,567.89", ts_real_p[assign_a(d)]).full &&
             d == 1234567.89);                           //  Good.
