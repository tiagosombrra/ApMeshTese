...
static void skipped() { std::cout << "skipped\n"; }
...
struct test_task {
  ...
  void operator()() {  // create lots of objects
    unsigned long i = 0;

    v.reserve(maximum_test_size);
    do {
      for (; i < test_size; ++i) v.push_back(new ClassT);
    } while (i < increase_test_size(i));
  }

  static unsigned long increase_test_size(unsigned long size) {
    static boost::mutex m;
    boost::unique_lock<boost::mutex> l(m);

    if (size < test_size || test_size == maximum_test_size) return test_size;
    ...
    if (seconds < 4) {
      test_size *= 2;
      if (test_size > maximum_test_size) test_size = maximum_test_size;
    }

    return test_size;
  }

  std::vector<ClassT *> const &data() const { return v; }
...
template <typename T>
class callable_reference_wrapper : public boost::reference_wrapper<T> {
 public:
  explicit callable_reference_wrapper(T &t) : boost::reference_wrapper<T>(t) {}
  inline void operator()() { this->get().operator()(); }
};

template <typename T>
callable_reference_wrapper<T> callable_ref(T &t) {
...
template <typename ClassT>
void check_ascending(test_task<ClassT> const &t) {
...
template <typename ClassT>
void check_not_contained_in(test_task<ClassT> const &candidate,
  ...
  while (p != e) {
    iter found =
        std::lower_bound(in.data().begin(), in.data().end(), *p, less1());
