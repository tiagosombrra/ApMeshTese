...
void parser_traits_tests() {
  ...
  BOOST_STATIC_ASSERT(
      is_unary_composite<action_t>::value);  // action_t _is_ an unary_t!
  ...
  BOOST_STATIC_ASSERT(
      is_composite_parser<action_t>::value);  // action_t _is_ an unary_t!
...
void parser_extraction_tests() {
  ...
  BOOST_STATIC_ASSERT(
      (::boost::is_same<chlit<char>, unary_subject<unary_t>::type>::value));
  BOOST_STATIC_ASSERT(
      (::boost::is_same<chlit<char>, action_subject<action_t>::type>::value));
  BOOST_STATIC_ASSERT(
      (::boost::is_same<assign_actor, semantic_action<action_t>::type>::value));
  BOOST_STATIC_ASSERT(
      (::boost::is_same<chlit<char>,
                        binary_left_subject<binary_t>::type>::value));
  BOOST_STATIC_ASSERT(
      (::boost::is_same<anychar_parser,
                        binary_right_subject<binary_t>::type>::value));
  ...
  BOOST_TEST(1 ==
             parse("aaaa", get_action_subject(ch_p('a')[assign(c)])).length);
  BOOST_TEST(c == 'b');

  BOOST_TEST(1 ==
             parse("aaaa", ch_p('a')[get_semantic_action(ch_p('b')[assign(c)])])
                 .length);
  BOOST_TEST(c == 'a');

  BOOST_TEST(
      1 ==
      parse("abab", get_binary_left_subject(ch_p('a') >> ch_p('b'))).length);
  BOOST_TEST(
      1 ==
      parse("baba", get_binary_right_subject(ch_p('a') >> ch_p('b'))).length);
}
