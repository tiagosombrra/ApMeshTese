...
struct InstanciateTest {
  ...
  void operator()(BaseIterT) {
    ...
    InstanciateTestOne<
        position_iterator<BaseIterT, file_position_without_column> >();
    InstanciateTestOne<
        position_iterator2<BaseIterT, file_position_without_column> >();
  }
};
...
void CheckInstantiation(void) {
  typedef mpl::list<char*, const char*, string::iterator,
...
void CheckLineExtractionOne(IterT iter) {
  ...
  BOOST_TEST(string(iter.get_currentline_begin(), iter.get_currentline_end()) ==
             string());
  ...
  AssertIterString(iter.get_currentline_begin(), iter.get_currentline_end(),
                   line1);
  ...
  AssertIterString(iter2.get_currentline_begin(), iter2.get_currentline_end(),
                   line2);
  AssertIterString(iter3.get_currentline_begin(), iter3.get_currentline_end(),
                   line2);
...
void CheckLineExtraction(void) {
  ...
  CheckLineExtractionOne(position_iterator2<iter_t, file_position>(
...
void CheckDistance(IterT begin) {
  ...
  for (IterT it = begin; it != end; ++it) ++manual_count;
...
void CheckBasicFunctionality(void) {
  ...
  test_impl::CheckIncrement(
      position_iterator<iter_t, file_position_without_column>(a, a + 10, ""));
  test_impl::CheckIncrement(
      position_iterator2<iter_t, file_position_without_column>(a, a + 10, ""));

  const char* b = "\n0123\r\n4567\n89\n\r";

  test_impl::CheckLineCounting(position_iterator<iter_t>(b, b + 16, ""));
  test_impl::CheckLineCounting(position_iterator2<iter_t>(b, b + 16, ""));
  test_impl::CheckLineCounting(
      position_iterator<iter_t, file_position_without_column>(b, b + 16, ""));
  test_impl::CheckLineCounting(
      position_iterator2<iter_t, file_position_without_column>(b, b + 16, ""));
}
...
void CheckColumnCounting(void) {
  ...
  test_impl::CheckColumnCounting_Tab4(
      position_iterator2<iter_t>(a, a + 10, ""));
  test_impl::CheckColumnCounting_Tab3(position_iterator<iter_t>(a, a + 10, ""));
  test_impl::CheckColumnCounting_Tab3(
      position_iterator2<iter_t>(a, a + 10, ""));
}

void CheckLineExtraction(void) { test_impl::CheckLineExtraction(); }
...
void CheckDistance(void) {
  ...
  test_impl::CheckDistance(
      position_iterator<iter_t, file_position_without_column>(b, b + 15, ""));
  test_impl::CheckDistance(
      position_iterator2<iter_t, file_position_without_column>(b, b + 15, ""));
}
...
void CheckSingular() {
  {
    ...
    CheckSingularImpl<position_iterator<interator_type, PositionT>,
                      interator_type>();
    CheckSingularImpl<position_iterator2<interator_type, PositionT>,
                      interator_type>();
  }
  {
