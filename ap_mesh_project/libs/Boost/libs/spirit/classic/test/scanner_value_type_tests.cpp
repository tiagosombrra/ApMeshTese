...
struct grammar : sp::grammar<grammar> {
  ...
  struct definition {
    definition(grammar const&) {
      foo = sp::alpha_p | sp::alnum_p | sp::cntrl_p | sp::print_p |
            sp::blank_p | sp::digit_p | sp::graph_p | sp::lower_p |
            sp::upper_p | sp::xdigit_p | sp::punct_p;
    }

    sp::rule<Scanner> foo;
    sp::rule<Scanner> const& start() const { return foo; }
  };
};
...
std::ostream& operator<<(std::ostream& out, non_pod const& x) {
  out << x.value;
  return out;
}
