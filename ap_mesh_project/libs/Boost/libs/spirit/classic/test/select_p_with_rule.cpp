...
struct format_grammar : public grammar<format_grammar> {
  ...
  struct definition {
    definition(format_grammar const& /*self*/) {
      descriptor_list = list_p(format_descriptor, ch_p(','));

      format_descriptor = select_p(E_descriptor, EN_descriptor);

      E_descriptor =  // E[w[.d][Ee]]
          lexeme_d[(ch_p('E') - (str_p("EN"))) >>
                   !(min_limit_d(1u)[uint_p] >> !(ch_p('.') >> uint_p) >>
    ...
    rule<ScannerT> const& start() const { return descriptor_list; }
  };
};
