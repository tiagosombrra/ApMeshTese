...
template <typename ScannerT>
switch_grammar::definition<ScannerT>::definition(
    switch_grammar const& /*self*/) {
  using boost::cref;
  ...
  expression =
      str_p("NNODES") >> uint_p[var(nnodes) = arg1] >>

      for_p(var(index) = 1, var(index) <= var(nnodes),
            var(index)++)[limit_d(cref(index), cref(index))[uint_p] >>

                          switch_p[(case_p<'s'>(uint_p), case_p<'d'>(uint_p),
                                    case_p<'n'>(uint_p))]];
}
...
int main() {
  ...
  spirit::parse_info<iterator_t> const info =
      parse(iterator_t(data.begin(), data.end(), "switch test"), iterator_t(),
            switch_grammar(), spirit::space_p);

  if (!info.full) {
    spirit::file_position const fp = info.stop.get_position();
    std::cerr << "Parsing failed at line " << fp.line << ", column "
              << fp.column << ".\n";
  }
