...
struct counter {
  ...
  counter(int max_count) : counter_(0), max_count_(max_count) {}
...
template <>
struct is_container<client::counter const> : mpl::true_ {};
...
struct begin_container<client::counter const> {
  static client::counter::iterator call(client::counter const& c) {
    return c.counter_;
  }
};
...
struct deref_iterator<client::counter::iterator> {
  ...
  static type call(client::counter::iterator const& it) { return it; }
};
}  // namespace traits
}  // namespace spirit
}  // namespace boost
...
int main() {
  ...
  std::cout << karma::format(karma::int_ % ", ", count)
            << std::endl;  // prints: '0, 1, 2, 3'
  //]
  return 0;
}
