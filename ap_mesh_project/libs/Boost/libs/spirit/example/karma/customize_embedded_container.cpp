...
template <>
struct is_container<client::embedded_container const> : mpl::true_ {};
...
struct begin_container<client::embedded_container const> {
  static client::embedded_container::iterator call(
      client::embedded_container const& d) {
    return d.data.begin();
  }
};
...
int main() {
  ...
  // STL vector
  std::cout << karma::format(karma::int_ % ", ", d1)
