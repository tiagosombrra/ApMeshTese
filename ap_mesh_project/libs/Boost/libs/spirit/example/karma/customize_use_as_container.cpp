...
template <>
struct is_container<client::use_as_container const> : mpl::true_ {};
...
struct begin_container<client::use_as_container const> {
  static client::use_as_container::iterator call(
      client::use_as_container const& c) {
...
struct compare_iterators<client::use_as_container::iterator> {
  static bool call(client::use_as_container::iterator const& it1,
    ...
    return it1.current_ == it2.current_ && it1.container_ == it2.container_;
  }
};
}  // namespace traits
}  // namespace spirit
...
int main() {
  ...
  std::cout << karma::format(karma::int_ % ", ", d2)
            << std::endl;  // prints: '1, 2, 3'
  //]
  return 0;
}
