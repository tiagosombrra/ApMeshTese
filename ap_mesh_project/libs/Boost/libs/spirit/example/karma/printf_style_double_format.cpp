...
BOOST_FUSION_ADAPT_STRUCT(client::format_data,
                          (char, flag)(char, fill)(int, width)(int,
                                                               precision)(char,
                                                                          type))
...
struct format_policies : karma::real_policies<T> {
  ...
  typedef boost::mpl::int_<karma::generator_properties::countingbuffer>
      properties;
  ...
  int floatfield(T n) const {
    ...
    if (format_.type == 'f') return base_policy_type::fmtflags::fixed;
  ...
  bool const force_sign(T) const { return format_.flag == '+'; }

  ///////////////////////////////////////////////////////////////////////////
  //  returns the number of required digits for the fractional part
  unsigned precision(T) const { return format_.precision; }
  ...
  bool exponent(OutputIterator& sink, long n) const {
    if (format_.type == 'E' || format_.type == 'G') {
      ...
      return this->base_policy_type::template exponent<char_encoding::ascii,
                                                       tag::upper>(sink, n);
    }
    return this->base_policy_type::template exponent<CharEncoding, Tag>(sink,
                                                                        n);
  }
  ...
  bool call(OutputIterator& sink, T n, Policies const& p) const {
    ...
    if (format_.flag == '-') {  // left align
      ...
      karma::detail::enable_buffering<OutputIterator> buffering(sink,
                                                                format_.width);
...
int main() {
  std::cout
      << "/////////////////////////////////////////////////////////////\n\n";
  std::cout
      << "A format driven floating point number generator for Spirit...\n\n";
  std::cout
      << "/////////////////////////////////////////////////////////////\n\n";
