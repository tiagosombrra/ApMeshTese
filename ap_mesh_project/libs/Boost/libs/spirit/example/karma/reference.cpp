...
void test_generator_attr_delim(char const* expected, G const& g,
                               Delimiter const& d, T const& attr) {
  std::string s;
  std::back_insert_iterator<std::string> out(s);
  if (boost::spirit::karma::generate_delimited(out, g, d, attr) &&
      s == expected)
    std::cout << "ok" << std::endl;
  else
    std::cout << "fail" << std::endl;
...
void test_binary_generator(char const* expected, std::size_t size, G const& g) {
  std::string s;
  std::back_insert_iterator<std::string> out(s);
  if (boost::spirit::karma::generate(out, g) &&
      !std::memcmp(s.c_str(), expected, size))
    std::cout << "ok" << std::endl;
  else
    std::cout << "fail" << std::endl;
}
...
template <typename G, typename T>
void test_binary_generator_attr(char const* expected, std::size_t size,
                                G const& g, T const& attr) {
  std::string s;
  std::back_insert_iterator<std::string> out(s);
  if (boost::spirit::karma::generate(out, g, attr) &&
      !std::memcmp(s.c_str(), expected, size))
    std::cout << "ok" << std::endl;
  else
    std::cout << "fail" << std::endl;
...
struct create_generator<complex> {
  typedef proto::result_of::deep_copy<BOOST_TYPEOF(
      '{' << karma::double_ << ',' << karma::double_ << '}')>::type type;

  static type call() {
    return proto::deep_copy('{' << karma::double_ << ',' << karma::double_
                                << '}');
  }
};
}  // namespace traits
 ...
 */
struct num_list
    : grammar<output_iterator_type, space_type, std::vector<int>()> {
...
int main() {
  ...
  ///////////////////////////////////////////////////////////////////////////
  {
    ...
    test_generator_attr("1.0,2.0", double_ << ',' << double_,
  ...
  ///////////////////////////////////////////////////////////////////////////
  {
    ...
    test_generator_attr("1.0     |2.0",
                        left_align(8)[double_] << '|' << double_, p);
    test_generator_attr("   1.0  |2.0", center(8)[double_] << '|' << double_,
                        p);
    test_generator_attr("     1.0|2.0",
                        right_align(8)[double_] << '|' << double_, p);
    //]
  }
...

  {
    ...
    test_generator_attr("[ 2.0 , 4.3 ] ",
                        delimit['[' << double_ << ',' << double_ << ']'], 2.0,
                        4.3);
    test_generator_attr("[*2.0*,*4.3*]*",
                        delimit('*')['[' << double_ << ',' << double_ << ']'],
                        2.0, 4.3);
    test_generator_attr(
        "[2.0, 4.3 ] ",
...

  {
    ...
    test_generator_attr_delim("2.0 2.0 ", duplicate[double_ << double_], space,
                              2.0);
    //]
  }
...

  {
    ...
    test_generator_attr("", char_('A', 'Z'),
                        'a');  // fails (as 'a' does not belong to 'A'...'Z')

    test_generator_attr("k", char_("a-z0-9"), 'k');
    test_generator_attr("", char_("a-z0-9"),
                        'A');  // fails (as 'A' does not belong to "a-z0-9")
                               //]
  }

  {
    ...
    test_generator_attr("", upper[alpha],
                        'a');  // fails (as isupper('a') is false)
                               //]
  }
...

  {
    ...
    //[reference_karma_eps
    test_generator("abc",
                   eps[std::cout << val("starting eps example")] << "abc");
    test_generator("abc", eps(true) << "abc");
    test_generator("", eps(false)
                           << "abc");  // fails as eps expression is 'false'
                                       //]
  }
  ...
  // binary module
  {
    ...
    test_binary_generator("\x01\x02\x03\x04\x05\x06\x07\x08", 8,
    ...
    test_binary_generator_attr("\x01\x02\x03\x04\x05\x06\x07\x08", 8, qword,
    ...
    test_binary_generator("\x08\x07\x06\x05\x04\x03\x02\x01", 8,
                          qword(0x0807060504030201LL));

    test_binary_generator_attr("\x01", 1, byte_, 0x01);
    test_binary_generator_attr("\x02\x01", 2, word, 0x0201);
    test_binary_generator_attr("\x04\x03\x02\x01", 4, dword, 0x04030201);
    test_binary_generator_attr("\x08\x07\x06\x05\x04\x03\x02\x01", 8, qword,
                               0x0807060504030201LL);
    //]
  }

  {
    ...
    test_binary_generator_attr("\x01\x02\x03\x04\x05\x06\x07\x08", 8,
                               little_qword, 0x0807060504030201LL);
    //]
  }
  ...
  // symbols
  {
    ...
    sym.add('a', "Apple")('b', "Banana")('o', "Orange");
  ...
  // as
  {
    ...
    test_generator_attr("xyz1.23", as<utf8_symbol_type>()[*char_] << double_,
                        ut);
    //]
  }
