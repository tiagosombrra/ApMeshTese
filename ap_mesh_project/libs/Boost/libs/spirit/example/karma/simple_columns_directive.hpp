...
struct columns_delimiter {
  columns_delimiter(Delimiter const& delim) : delimiter(delim), count(0) {}
  ...
  bool generate(OutputIterator& sink, Context&, Delimiter_ const&,
    ...
    if (!karma::delimit_out(sink, delimiter)) return false;
...
struct simple_columns_generator : boost::spirit::karma::unary_generator<
  ...
  simple_columns_generator(Subject const& s) : subject(s) {}
