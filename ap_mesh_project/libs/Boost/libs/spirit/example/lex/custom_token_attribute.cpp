...
struct rational {
  rational(int n = 0, int d = 0) : nominator_(n), denominator_(d) {}
...
struct assign_to_attribute_from_iterators<rational, Iterator> {
  static void call(Iterator const& first, Iterator const& last,
    ...
    int x, y;
    Iterator b = first;
    qi::parse(b, last, '{' >> qi::int_ >> ',' >> qi::int_ >> '}', x, y);
    attr = rational(x, y);
  }
};
}  // namespace traits
...
int main() {
  ...
  typedef lex::lexertl::token<std::string::iterator, mpl::vector<rational> >
      token_type;
  ...
  std::cout << "Parsing succeeded: {" << r.nominator_ << ", " << r.denominator_
            << "}" << std::endl;
  return 0;
}
