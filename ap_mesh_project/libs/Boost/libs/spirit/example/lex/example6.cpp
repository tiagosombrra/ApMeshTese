...
///////////////////////////////////////////////////////////////////////////////
enum token_ids { ID_CONSTANT = 1000, ID_IF, ID_ELSE, ID_WHILE, ID_IDENTIFIER };
...
struct example6_tokens : lex::lexer<Lexer> {
  example6_tokens() {
    ...
    this->self.add(constant, ID_CONSTANT)("if", ID_IF)("else", ID_ELSE)(
        "while", ID_WHILE)(identifier, ID_IDENTIFIER);
...
struct example6_grammar : qi::grammar<Iterator, qi::in_state_skipper<Lexer> > {
  template <typename TokenDef>
  example6_grammar(TokenDef const& tok) : example6_grammar::base_type(program) {
    ...
    if_stmt = (token(ID_IF) >> '(' >> expression >> ')' >> block >>
               -(token(ID_ELSE) >>
                 block))[std::cout << val("if expression: ") << _2 << "\n"];

    while_stmt = (token(ID_WHILE) >> '(' >> expression >> ')' >>
...
int main() {
  ...
  typedef example6_grammar<iterator_type, example6_tokens::lexer_def>
      example6_grammar;
