...
struct echo_input_functor {
  echo_input_functor(std::basic_ostream<Char, Traits>& os_) : os(os_) {}
...
template <typename Char, typename Traits>
inline echo_input_functor<Char, Traits> echo_input(
    std::basic_ostream<Char, Traits>& os) {
...
struct set_lexer_state {
  set_lexer_state(char const* state_) : state(state_) {}
...
struct strip_comments_tokens : lex::lexer<Lexer> {
  strip_comments_tokens()
    ...
    // strictly optional.
    this->self = cppcomment | ccomment[set_lexer_state("COMMENT")] |
                 eol[echo_input(std::cout)] | any[echo_input(std::cout)];
