...
struct word_count_tokens : lex::lexer<Lexer> {
  word_count_tokens() {
    ...
    // everything else
    this->self.add(word)  // no token id is needed here
...
struct word_count_grammar : qi::grammar<Iterator> {
  ...
  word_count_grammar(TokenDef const& tok)
      : word_count_grammar::base_type(start), c(0), w(0), l(0) {
    using boost::phoenix::ref;
    using boost::phoenix::size;

    start = *(tok.word[++ref(w), ref(c) += size(_1)] |
              lit('\n')[++ref(c), ++ref(l)] | qi::token(IDANY)[++ref(c)]);
  }
...
int main(int argc, char* argv[]) {
  ...
  >*/
  typedef lex::lexertl::token<char const*, boost::mpl::vector<std::string> >
      token_type;
