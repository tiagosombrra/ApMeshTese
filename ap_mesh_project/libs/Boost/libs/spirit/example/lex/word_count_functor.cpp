...
///////////////////////////////////////////////////////////////////////////////
//[wcf_token_ids
enum token_ids { ID_WORD = 1000, ID_EOL, ID_CHAR };
//]
...
/*` The template `word_count_tokens` defines three different tokens:
    ...
    `lex::lexertl::lexer<BaseIterator>`) defines which underlying lexer engine
   has to be used to provide the required state machine functionality. In this
...
struct counter {
  ...
  bool operator()(Token const& t, std::size_t& c, std::size_t& w,
                  std::size_t& l) const {
    switch (t.id()) {
...
/*` The main function simply loads the given file into memory (as a
    ...
    and finally calls `lex::tokenize`, passing an instance of the counter
   function object. The return value of `lex::tokenize()` will be `true` if the
