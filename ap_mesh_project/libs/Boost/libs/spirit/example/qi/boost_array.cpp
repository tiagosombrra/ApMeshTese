...
struct adapt_array<boost::array<T, N> > {
  ...
  adapt_array(array_type& arr) : arr_(arr), current_(0) {}
...
template <typename T, std::size_t N>
inline detail::adapt_array<boost::array<T, N> > adapt_array(
    boost::array<T, N>& arr) {
...
template <typename T, std::size_t N>
struct push_back_container<client::detail::adapt_array<boost::array<T, N> >,
                           T> {
...
int main() {
  ...
  if (result) std::cout << "Parsed: " << arr[0] << ", " << arr[1] << std::endl;
