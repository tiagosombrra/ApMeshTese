...
bool compiler::compile_expression(
    int min_precedence, std::list<ast::operation>::const_iterator& rbegin,
    std::list<ast::operation>::const_iterator rend) {
  while ((rbegin != rend) &&
         (precedence_of(rbegin->operator_) >= min_precedence)) {
    token_ids::type op = rbegin->operator_;
    if (!boost::apply_visitor(*this, rbegin->operand_)) return false;
    ++rbegin;

    while ((rbegin != rend) &&
           (precedence_of(rbegin->operator_) > precedence_of(op))) {
...
bool compiler::operator()(ast::expression const& x) {
  ...
  if (!compile_expression(0, rbegin, x.rest.end())) return false;
  return true;
}
...
void compiler::print_assembler() const {
  ...
  BOOST_FOREACH (pair const& p, functions) {
    std::cout << ";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;" << std::endl;
    std::cout << p.second->get_address() << ": function " << p.first
              << std::endl;
    p.second->print_assembler();
  }
}

boost::shared_ptr<code_gen::function> compiler::find_function(
