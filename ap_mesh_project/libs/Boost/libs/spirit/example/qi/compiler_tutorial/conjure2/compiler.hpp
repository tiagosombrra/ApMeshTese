...
struct compiler {
  ...
  boost::shared_ptr<code_gen::function> find_function(
 ...
 private:
  bool compile_expression(int min_precedence,
                          std::list<ast::operation>::const_iterator& rbegin,
                          std::list<ast::operation>::const_iterator rend);

  typedef std::map<std::string, boost::shared_ptr<code_gen::function> >
      function_table;
