...
namespace client {
namespace code_gen {
value::value() : v(0), is_lvalue_(false), builder(0) {}

value::value(llvm::Value* v, bool is_lvalue_, llvm::IRBuilder<>* builder)
    : v(v), is_lvalue_(is_lvalue_), builder(builder) {}

value::value(value const& rhs)
    : v(rhs.v), is_lvalue_(rhs.is_lvalue_), builder(rhs.builder) {}

bool value::is_lvalue() const { return is_lvalue_; }

bool value::is_valid() const { return v != 0; }

value::operator bool() const { return v != 0; }

void value::name(char const* id) { v->setName(id); }

void value::name(std::string const& id) { v->setName(id); }
...
value operator-(value a) {
  ...
  return value(a.builder->CreateNeg(a, "neg_tmp"), false, a.builder);
}

value operator!(value a) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateNot(a, "not_tmp"), false, a.builder);
}

value operator+(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateAdd(a, b, "add_tmp"), false, a.builder);
}

value operator-(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateSub(a, b, "sub_tmp"), false, a.builder);
}

value operator*(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateMul(a, b, "mul_tmp"), false, a.builder);
}

value operator/(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateSDiv(a, b, "div_tmp"), false, a.builder);
}

value operator%(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateSRem(a, b, "mod_tmp"), false, a.builder);
}

value operator&(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateAnd(a, b, "and_tmp"), false, a.builder);
}

value operator|(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateOr(a, b, "or_tmp"), false, a.builder);
}

value operator^(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateXor(a, b, "xor_tmp"), false, a.builder);
}

value operator<<(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateShl(a, b, "shl_tmp"), false, a.builder);
}

value operator>>(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateLShr(a, b, "shr_tmp"), false, a.builder);
}

value operator==(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateICmpEQ(a, b, "eq_tmp"), false, a.builder);
}

value operator!=(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateICmpNE(a, b, "ne_tmp"), false, a.builder);
}

value operator<(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateICmpSLT(a, b, "slt_tmp"), false, a.builder);
}

value operator<=(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateICmpSLE(a, b, "sle_tmp"), false, a.builder);
}

value operator>(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateICmpSGT(a, b, "sgt_tmp"), false, a.builder);
}

value operator>=(value a, value b) {
  BOOST_ASSERT(a.builder != 0);
  return value(a.builder->CreateICmpSGE(a, b, "sge_tmp"), false, a.builder);
}
...
struct function::to_value {
  ...
  value operator()(llvm::Value& v) const { return c->val(&v); }
};

bool basic_block::has_terminator() const { return b->getTerminator() != 0; }

bool basic_block::is_valid() const { return b != 0; }

function::operator llvm::Function*() const { return f; }

std::size_t function::arg_size() const { return f->arg_size(); }

void function::add(basic_block const& b) {
  f->getBasicBlockList().push_back(b);
}

void function::erase_from_parent() { f->eraseFromParent(); }

basic_block function::last_block() { return &f->getBasicBlockList().back(); }

bool function::empty() const { return f->empty(); }

std::string function::name() const { return f->getName(); }
...
bool function::is_valid() const { return f != 0; }

void function::verify() const { llvm::verifyFunction(*f); }

value llvm_compiler::val(unsigned int x) {
  return value(llvm::ConstantInt::get(context(), llvm::APInt(int_size, x)),
               false, &llvm_builder);
}
...
value llvm_compiler::val(bool x) {
  return value(llvm::ConstantInt::get(context(), llvm::APInt(1, x)), false,
               &llvm_builder);
}
...
llvm::AllocaInst* make_entry_block_alloca(llvm::Function* f, char const* name,
                                          llvm::LLVMContext& context) {
  llvm::IRBuilder<> builder(&f->getEntryBlock(), f->getEntryBlock().begin());

  return builder.CreateAlloca(llvm::Type::getIntNTy(context, int_size), 0,
                              name);
}
}  // namespace
...
value llvm_compiler::var(char const* name) {
  ...
  llvm::AllocaInst* alloca =
      builder.CreateAlloca(llvm::Type::getIntNTy(context(), int_size), 0, name);
...
struct value::to_llvm_value {
  ...
  llvm::Value* operator()(value const& x) const { return x; }
};

template <typename C>
llvm::Value* llvm_compiler::call_impl(function callee, C const& args_) {
  ...
  std::vector<llvm::Value*> args(args_.begin(), args_.end());
  ...
  return llvm_builder.CreateCall(callee, args.begin(), args.end(), "call_tmp");
}

template <typename Container>
value llvm_compiler::call(function callee, Container const& args) {
  llvm::Value* call = call_impl(
      callee, args | boost::adaptors::transformed(value::to_llvm_value()));

  if (call == 0) return val();
...
function llvm_compiler::declare_function(bool void_return,
                                         std::string const& name,
                                         std::size_t nargs) {
  llvm::Type const* int_type = llvm::Type::getIntNTy(context(), int_size);
...
basic_block llvm_compiler::make_basic_block(char const* name, function parent,
                                            basic_block before) {
  return llvm::BasicBlock::Create(context(), name, parent, before);
}

value llvm_compiler::var(std::string const& name) { return var(name.c_str()); }
...
void llvm_compiler::conditional_branch(value cond, basic_block true_br,
                                       basic_block false_br) {
  llvm_builder.CreateCondBr(cond, true_br, false_br);
}

void llvm_compiler::branch(basic_block b) { llvm_builder.CreateBr(b); }

void llvm_compiler::return_() { llvm_builder.CreateRetVoid(); }

void llvm_compiler::return_(value v) { llvm_builder.CreateRet(v); }
...
value compiler::operator()(unsigned int x) { return val(x); }

value compiler::operator()(bool x) { return val(x); }
...
value compiler::operator()(ast::unary_expr const& x) {
  ...
  if (!operand.is_valid()) return val();
...
struct compile_args {
  ...
  typedef value result_type;
  value operator()(ast::expression const& expr) const { return c(expr); }
};
}  // namespace
...
inline int precedence_of(token_ids::type op) { return precedence[op & 0xFF]; }

value compiler::compile_binary_expression(value lhs, value rhs,
                                          token_ids::type op) {
  switch (op) {
    case token_ids::plus:
      return lhs + rhs;
...
value compiler::compile_expression(
  ...
  while ((rest_begin != rest_end) &&
    ...
    while ((rest_begin != rest_end) &&
      ...
      rhs =
          compile_expression(precedence_of(next_op), rhs, rest_begin, rest_end);
    }
...
bool compiler::operator()(ast::variable_declaration const& x) {
  ...
  if (init.is_valid()) var_.assign(init);
...
bool compiler::operator()(ast::if_statement const& x) {
  ...
  if (!condition.is_valid()) return false;
  ...
  if (!then_block.has_terminator()) {
    if (!exit_block.is_valid()) exit_block = make_basic_block("if.end");
    branch(exit_block);
  }
...
bool compiler::operator()(ast::while_statement const& x) {
  ...
  if (!body_block.has_terminator()) branch(cond_block);  // loop back
...
bool compiler::operator()(ast::return_statement const& x) {
  if (void_return) {
    ...
    if (!x.expr) {
      error_handler(x.id,
                    current_function_name + " function must return a value: ");
      return false;
    }
  }
  ...
  if (x.expr) {
    ...
    if (!return_val.is_valid()) return false;
...
function compiler::function_decl(ast::function const& x) {
  ...
  function f =
      declare_function(void_return, current_function_name, x.args.size());
