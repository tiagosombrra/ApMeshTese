...
struct ts_real_policies : boost::spirit::qi::ureal_policies<T> {
  ...
  static bool parse_frac_n(Iterator& first, Iterator const& last,
                           Attribute& attr, int& frac_digits) {
    Iterator savef = first;
    bool r = boost::spirit::qi::extract_uint<T, 10, 1, 2, true>::call(
        first, last, attr);
    if (r) {
  ...
  static bool parse_n(Iterator& first, Iterator const& last,
                      Accumulator& result) {
...
struct create_parser<complex> {
  typedef proto::result_of::deep_copy<BOOST_TYPEOF(
      '{' >> qi::double_ >> ',' >> qi::double_ >> '}')>::type type;

  static type call() {
    return proto::deep_copy('{' >> qi::double_ >> ',' >> qi::double_ >> '}');
  }
};
}  // namespace traits
}  // namespace spirit
...
int main() {
...

  {
    ...
    std::string str;
    test_phrase_parser_attr("'  abc  '", '\'' >> no_skip[+~char_('\'')] >> '\'',
                            str);
...

  {
    ...
    std::vector<int> v;
    test_phrase_parser_attr("123", hold[int_ >> ':' >> int_] | int_ >> attr(0),
                            v);
...

  {
    ...
    sym.add("apple", 1)  // symbol strings are added in lowercase...
        ("banana", 2)("orange", 3);
...

  {
    ...
    std::cout << std::hex << colors[0] << ',' << colors[1] << ',' << colors[2]
              << std::endl;
    ...
    /*`This trivial example cannot be practically defined in traditional EBNF.
       ...
       other than the Kleene Star, we are still limited to parsing fixed
       strings. The nature of EBNF forces the repetition factor to be a
       constant. On the other hand, Spirit allows the repetition factor to be
       variable at run time. We could write a grammar that accepts the input
       string above. Example using phoenix:
     */
    std::string str;
    int n;
    test_parser_attr("\x0bHello World",
                     char_[phx::ref(n) = _1] >> repeat(phx::ref(n))[char_],
                     str);
  ...
  // eps
  {
    ...
    //[reference_eps_while
    /*`This example simulates the "classic" `while_p` parser. Here, the kleene
       loop will exit once the condition, `c`, becomes true. Notice that the
       condition, `c`, is turned to `false` when we get to parse `4`.
     */
    test_phrase_parser("1 2 3 4", *(eps(phx::ref(c) == true) >>
                                    int_[phx::ref(c) = (_1 == 4)]));
    //]
  }
  ...
  // sequence
  {
    ...
    //`Extracting the attributes using __qi_semantic_actions__ (using
    //__phoenix__):
    test_parser("xy",
  ...
  // alternative
  {
    ...
    /*`This should print `"Hello"`. Note: There are better ways to extract the
       value from the variant. See __boost_variant__ visitation. This code is
  ...
  // expect
  {
    ...
    //[reference_expect
    /*`The code below uses an expectation operator to throw an
       __qi_expectation_failure__ with a deliberate parsing error when `"o"` is
       expected and `"i"` is what is found in the input. The `catch` block
       prints the information related to the error. Note: This is low level code
       that demonstrates the /bare-metal/. Typically, you use an
       __qi_error_handler__ to deal with the error.
     */
    try {
      ...
      std::cout << "expected: ";
      print_info(x.what_);
      std::cout << "got: \"" << std::string(x.first, x.last) << '"'
                << std::endl;
    }
  ...
  // expectd
  {
    ...
    /*`The code below uses an expectation operator to throw an
       __qi_expectation_failure__ with a deliberate parsing error when `"o"` is
       expected and `"x"` is what is found in the input. The `catch` block
       prints the information related to the error. Note: This is low level code
       that demonstrates the /bare-metal/. Typically, you use an
       __qi_error_handler__ to deal with the error.
     */
  ...
  // kleene
  {
     ...
     */
    std::vector<int> attr;
    test_phrase_parser_attr("111, 222, 333, 444, 555", int_ >> *(',' >> int_),
                            attr);
    std::cout << attr[0] << ',' << attr[1] << ',' << attr[2] << ',' << attr[3]
              << ',' << attr[4] << std::endl;
    //]
  }
  ...
  // optional
  {
    ...
    test_phrase_parser_attr("\"James Bond\", M",
                            lexeme['"' >> +(char_ - '"') >> '"']  // name
                                >> -(',' >> int_)   // optional age
                                >> -(',' >> char_)  // optional sex
                            ,
  ...
  // list
  {
     ...
     */
    std::vector<int> attr;
    test_phrase_parser_attr("111, 222, 333, 444, 555", int_ % ',', attr);
    std::cout << attr[0] << ',' << attr[1] << ',' << attr[2] << ',' << attr[3]
              << ',' << attr[4] << std::endl;
    //]
  }
...
//<-
    ...
    test_parser_attr("\x01", byte_, uc);
    assert(uc == 0x01);
    test_parser_attr("\x01\x02", word, us);
    assert(us == 0x0201);
    test_parser_attr("\x01\x02\x03\x04", dword, ui);
    assert(ui == 0x04030201);
//<-
#ifdef BOOST_HAS_LONG_LONG
    //->
...
//<-
    ...
    test_parser_attr("\x01\x02", little_word, us);
    assert(us == 0x0201);
    test_parser_attr("\x01\x02\x03\x04", little_dword, ui);
    assert(ui == 0x04030201);
//<-
#ifdef BOOST_HAS_LONG_LONG
    //->
...
//<-
    ...
    test_parser_attr("\x01\x02", big_word, us);
    assert(us == 0x0102);
    test_parser_attr("\x01\x02\x03\x04", big_dword, ui);
    assert(ui == 0x01020304);
//<-
#ifdef BOOST_HAS_LONG_LONG
    //->
