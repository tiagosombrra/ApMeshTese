...
typedef fusion::result_of::as_nview<client::employee, 1, 2, 4>::type
    names_and_salary;
typedef fusion::result_of::as_nview<client::employee, 2, 0, 3>::type
    name_age_and_department;
...
int main() {
  ...
  // print data about employees in different formats
  {
    ...
    bool r = client::parse("John, 25", *(qi::char_ - ',') >> ", " >> qi::int_,
                           johnview);
    if (r) {
      std::cout << "Parsed: " << john.forename << ", " << john.age << std::endl;
    }
