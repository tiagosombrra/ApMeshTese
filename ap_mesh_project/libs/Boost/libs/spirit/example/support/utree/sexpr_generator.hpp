...
struct generator : karma::grammar<Iterator, utree()> {
  ...
  karma::rule<Iterator, utree()> start, ref_;

  karma::rule<Iterator, utree_list()> list;

  karma::rule<Iterator, utf8_symbol_range_type()> symbol;

  karma::rule<Iterator, utree::nil_type()> nil_;

  karma::rule<Iterator, utf8_string_range_type()> utf8;

  karma::rule<Iterator, binary_range_type()> binary;
  ...
  generator() : generator::base_type(start) {
    ...
    start =
        nil_ | double_ | int_ | boolean | utf8 | symbol | binary | list | ref_;
