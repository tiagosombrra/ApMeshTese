...
auto parse = [](std::string const& source, fs::path input_path) -> std::string {
  ...
  error_handler_type error_handler(iter, end, out,
                                   input_path.string());  // Our error handler
...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    std::cout << "usage: " << fs::path(argv[0]).filename()
              << " path/to/test/files" << std::endl;
    return -1;
  }
  ...
  if (r == 0) std::cout << num_files_tested << " files tested." << std::endl;
  std::cout << "==============================================================="
               "===================================="
            << std::endl;
  return r;
