...
typedef boost::variant<boost::recursive_wrapper<mini_xml>, std::string>
    mini_xml_node;
...
BOOST_FUSION_ADAPT_STRUCT(mini_xml,
                          (std::string, name)(std::vector<mini_xml_node>,
                                              children))
...
struct mini_xml_parser : qi::grammar<Iterator, mini_xml(), space_type> {
...
struct mini_xml_generator : karma::grammar<OutputIterator, mini_xml()> {
  mini_xml_generator() : mini_xml_generator::base_type(entry) {
    entry %= (xml = '<' << string[_1 = at_c<0>(_val)] << '>'
                        << (*node)[_1 = at_c<1>(_val)] << "</"
...
int main(int argc, char** argv) {
  ...
  if (r && iter == end) {
    ...
    if (r) std::cout << generated << std::endl;
    return 0;
  } else {
