...
std::ostream& operator<<(std::ostream& os, const person& p) {
  ...
  std::copy(p.favorite_colors.begin(), p.favorite_colors.end(),
            std::ostream_iterator<std::string>(os, "\n"));
  return os;
}

BOOST_FUSION_ADAPT_STRUCT(person,
                          (std::string, name)(int, age)(double, size)(
                              std::vector<std::string>, favorite_colors))
//]
...
int main() {
  // keyword_list
  {
    ...
    no_constraint_person_rule %= kwd("name")['=' > parse_string] /
                                 kwd("age")['=' > int_] /
                                 kwd("size")['=' > double_ > 'm'];
    //]
    ...
    */
    ...
    /*`The parser definition below uses the kwd directive occurrence constraint
    ...
    constraint_person_rule %= kwd("name", 1)['=' > parse_string] /
                              kwd("age", 1)['=' > int_] /
    ...
    // Parsing this string will fail because the age and size minimum occurrence
    ...
    the occurrence constraints aren't met. This code should print:[teletype]
