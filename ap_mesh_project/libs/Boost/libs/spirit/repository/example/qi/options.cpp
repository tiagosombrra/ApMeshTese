...
BOOST_FUSION_ADAPT_STRUCT(preprocessor_symbol,
                          (std::string, first)(boost::int32_t, second))

// A data structure to store our program options
struct program_options {
  std::vector<std::string> includes;  // include paths
  typedef std::vector<preprocessor_symbol>
      preprocessor_symbols_container;  // symbol container type definition
...
BOOST_FUSION_ADAPT_STRUCT(
    ...
    (std::vector<std::string>,
     includes)(program_options::preprocessor_symbols_container,
               preprocessor_symbols)(boost::optional<std::string>,
...
std::ostream &operator<<(std::ostream &os, const program_options &obj) {
  ...
  return os << format(lit("Includes:")
                          << (string % ',') << eol
                          << lit("Preprocessor symbols:")
                          << ((string << "=" << int_) % ',') << eol
...
int main() {
  {
    ...
    // kwd rule
    kwd_rule %= kwd("--include")[parse_string] /
    ...
    char const input[] =
        "--include path1 --source file1 --define SYMBOL1=10 --include path2 "
        "--source file2";
    char const *f(input);
