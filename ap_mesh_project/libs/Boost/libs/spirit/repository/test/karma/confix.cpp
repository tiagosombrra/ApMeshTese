...
inline typename traits::confix_spec<Prefix, Suffix>::type confix_spec(
    Prefix const& prefix, Suffix const& suffix) {
...
inline traits::confix_spec<std::string>::type tag(char const* tagname) {
...
int main() {
...

  {
    ...
    BOOST_TEST((test("<tag>a</tag>", confix("<tag>", "</tag>")[char_('a')])));
    BOOST_TEST((test("<tag>a</tag>", confix("<tag>", "</tag>")[char_], 'a')));
    BOOST_TEST((test("// some C++ comment\n",
                     confix(string("//"), eol)[" some C++ comment"])));
    BOOST_TEST((test("// some C++ comment\n", confix(string("//"), eol)[string],
                     " some C++ comment")));
...

  {
    ...
    BOOST_TEST(
        (test(L"<tag>a</tag>", confix(L"<tag>", L"</tag>")[char_(L'a')])));
...

  {
    ...
    BOOST_TEST(
        (test_delimited("// some C++ comment \n ",
                        confix(string("//"), eol)["some C++ comment"], space)));

    BOOST_TEST((
        test_delimited("<ol> some text </ol> ", html::ol["some text"], space)));
  }

  {
    ...
    BOOST_TEST((test_delimited(
        L"<tag> a </tag> ", confix(L"<tag>", L"</tag>")[char_(L'a')], space)));
    BOOST_TEST((test_delimited(L"// some C++ comment \n ",
                               confix(string(L"//"), eol)[L"some C++ comment"],
                               space)));

    BOOST_TEST((test_delimited(L"<ol> some text </ol> ", html::ol[L"some text"],
                               space)));
  }
