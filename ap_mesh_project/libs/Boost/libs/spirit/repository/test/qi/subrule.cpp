...
int main() {
  ...
  {  // basic tests
    ...
    BOOST_TEST(
        test("abcabcacb", (entry = *(a | b | c), a = 'a', b = 'b', c = 'c')));

    // check subrule group behaves as a parser
    BOOST_TEST(test("xabcabcacb",
                    'x' >> (entry = *(a | b | c), a = 'a', b = 'b', c = 'c')));
...
#pragma clang diagnostic pop
  ...
  {  // auto subrules tests: allow stl containers as attributes to
    ...
    BOOST_TEST(test("abcdef", (entry %= char_ >> char_ >> char_ >> char_ >>
                                        char_ >> char_)[phx::ref(s) = _1]));
    BOOST_TEST(s == "abcdef");
  }
  ...
  {  // context (w/args and locals) tests
    ...
    a = (sr = alpha[_a = _1 + _r1] >> char_(_a))(_r1);
  ...
  {  // std::string as container attribute with auto subrules
    ...
    std::string attr;
    BOOST_TEST(
        test_attr("x", (text %= +(!char_(')') >> !char_('>') >> char_)), attr));
    BOOST_TEST(attr == "x");
  }
