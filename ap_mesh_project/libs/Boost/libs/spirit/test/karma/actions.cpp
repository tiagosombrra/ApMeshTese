...
void read1(int& i) { i = 42; }

void read2(int& i, unused_type) { i = 42; }

void read3(int& i, unused_type, bool&) { i = 42; }
...
int main() {
...

  {
    ...
    BOOST_TEST(
        test_delimited("42 ", int_[boost::bind(&read3, _1, _2, _3)], ' '));
  }
...

  {
    BOOST_TEST(test("{42}", '{' << int_[read_action()] << '}'));
    BOOST_TEST(
        test_delimited("{ 42 } ", '{' << int_[read_action()] << '}', ' '));
  }

  {
    ...
    BOOST_TEST(test_delimited(
        "{ 42 } ", '{' << int_[boost::bind(&read1, _1)] << '}', ' '));
    BOOST_TEST(test("{42}", '{' << int_[boost::bind(&read2, _1, _2)] << '}'));
    BOOST_TEST(test_delimited(
        "{ 42 } ", '{' << int_[boost::bind(&read2, _1, _2)] << '}', ' '));
    BOOST_TEST(
        test("{42}", '{' << int_[boost::bind(&read3, _1, _2, _3)] << '}'));
    BOOST_TEST(test_delimited(
        "{ 42 } ", '{' << int_[boost::bind(&read3, _1, _2, _3)] << '}', ' '));
  }
