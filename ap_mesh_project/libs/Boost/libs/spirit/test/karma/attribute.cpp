...
BOOST_FUSION_ADAPT_STRUCT(test_data,
                          (int, i1)(std::string, s1)(std::string,
                                                     s2)(std::string,
                                                         s3)(double, d1))
...
int main() {
  ...
  {
    BOOST_TEST(
        test("s121", karma::string << karma::int_, fusion::as_nview<2, 0>(d1)));

    BOOST_TEST(test_delimited("s12 1 ", karma::string << karma::int_,
...

  {
    ...
    BOOST_TEST(
        test("1", karma::attr_cast<test_int_data1, int>(karma::int_), d));
  }

  {
    ...
    BOOST_TEST(
        test("1,2", karma::attr_cast<test_int_data1>(karma::int_) % ',', v));
    BOOST_TEST(test(
        "1,2", karma::attr_cast<test_int_data1, int>(karma::int_) % ',', v));
  }
...

  {
    ...
    //      BOOST_TEST(test("1.0,2.0", karma::attr_cast(karma::double_) % ',',
    //      v)); BOOST_TEST(test("1.0,2.0"
    //        , karma::attr_cast<test_int_data1>(karma::double_) % ',', v));

    BOOST_TEST(test("1.0,2.0",
                    karma::attr_cast<test_int_data1, int>(karma::double_) % ',',
                    v));
...

  {
    ...
    BOOST_TEST(
        test("1", karma::attr_cast<test_int_data2, int>(karma::int_), d));
  }

  {
    ...
    BOOST_TEST(test("1,2", karma::attr_cast(karma::int_) % ',', v));
    BOOST_TEST(
        test("1,2", karma::attr_cast<test_int_data2>(karma::int_) % ',', v));
    BOOST_TEST(test(
        "1,2", karma::attr_cast<test_int_data2, int>(karma::int_) % ',', v));
  }
...

  {
    ...
    BOOST_TEST(test("1.0,2.0",
                    karma::attr_cast<test_int_data2, int>(karma::double_) % ',',
                    v));
