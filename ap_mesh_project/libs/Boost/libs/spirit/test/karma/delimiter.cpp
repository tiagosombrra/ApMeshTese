...
int main() {
...

  {
    ...
    BOOST_TEST(test("ab c d", char_('a') << delimit[char_('b') << 'c'] << 'd'));
    BOOST_TEST(
        test("ab*c*d", char_('a') << delimit('*')[char_('b') << 'c'] << 'd'));

    BOOST_TEST(test_delimited("a b ", delimit[char_('a') << 'b'], char_(' ')));
    BOOST_TEST(
        test_delimited("a*b*", delimit('*')[char_('a') << 'b'], char_(' ')));

    BOOST_TEST(test_delimited("a b c d ",
                              char_('a') << delimit[char_('b') << 'c'] << 'd',
                              char_(' ')));
    BOOST_TEST(test_delimited(
        "a b*c*d ", char_('a') << delimit('*')[char_('b') << 'c'] << 'd',
        char_(' ')));
  }

  {
    BOOST_TEST(test("ab", verbatim[char_('a') << 'b']));
    BOOST_TEST(test("abcd", char_('a') << verbatim[char_('b') << 'c'] << 'd'));

    BOOST_TEST(test_delimited("ab ", verbatim[char_('a') << 'b'], char_(' ')));
...

  {
    ...
    BOOST_TEST(
        test("abcd", char_('a') << no_delimit[char_('b') << 'c'] << 'd'));

    BOOST_TEST(test_delimited("ab", no_delimit[char_('a') << 'b'], char_(' ')));
    BOOST_TEST(test_delimited(
        "a bcd ", char_('a') << no_delimit[char_('b') << 'c'] << 'd',
        char_(' ')));
  }

  {
    ...
    BOOST_TEST(test_delimited("a*b**", verbatim[delimit[char_('a') << 'b']],
                              char_('*')));
  }
