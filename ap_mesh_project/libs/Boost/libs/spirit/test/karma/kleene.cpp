...
///////////////////////////////////////////////////////////////////////////////
struct action {
  action(std::vector<char>& vec) : vec(vec), it(vec.begin()) {}

  void operator()(unsigned& value, boost::spirit::unused_type,
                  bool& pass) const {
    pass = (it != vec.end());
    if (pass) value = *it++;
  }
...
BOOST_FUSION_ADAPT_STRUCT(A, (double, d1)(double, d2))
...
int main() {
...

  {
    ...
    BOOST_TEST(
        test_delimited("a a a a a ", char_ << *(char_ << char_), s1, ' '));

    s1 = "a";
    BOOST_TEST(test("a", char_ << *(char_ << char_), s1));
    s1 = "aa";
    BOOST_TEST(!test("", char_ << *(char_ << char_), s1));
    //         BOOST_TEST(test("aa", char_ << *buffer[char_ << char_] << char_,
    //         s1));
    s1 = "aaaa";
    BOOST_TEST(!test("", char_ << *(char_ << char_), s1));
    //         BOOST_TEST(test("aaaa", char_ << *buffer[char_ << char_] <<
    //         char_, s1));
  }

  {
    ...
    std::vector<data> v1;
    v1 += std::make_pair('a', 'a'), std::make_pair('b', 'b'),
        std::make_pair('c', 'c'), std::make_pair('d', 'd'),
        std::make_pair('e', 'e'), std::make_pair('f', 'f'),
        std::make_pair('g', 'g');
  ...
  {  // actions
    ...
    BOOST_TEST(
        test_delimited("a b c d e f g h ", (*char_)[_1 = phx::ref(v)], space));
  }
  ...
  // failing sub-generators
  {
    ...
    typedef std::pair<char, char> data;
    std::vector<data> v2;
    v2 += std::make_pair('a', 'a'), std::make_pair('b', 'b'),
        std::make_pair('c', 'c'), std::make_pair('d', 'd'),
        std::make_pair('e', 'e'), std::make_pair('f', 'f'),
        std::make_pair('g', 'g');
