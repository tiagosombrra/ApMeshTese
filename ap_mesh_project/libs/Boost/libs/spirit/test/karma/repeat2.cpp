...
int main() {
...

  {
    ...
    BOOST_TEST(test_delimited("A B C D E F G H ", upper[repeat(8)[char_]], str,
                              space));
  }
  ...
  {  // more actions
    ...
    v.push_back(456);
    v.push_back(789);
    BOOST_TEST(test_delimited("123 456 789 ", repeat(3)[int_][_1 = phx::ref(v)],
                              space));
  }
  ...
  // failing sub-generators
  {
    ...
    v2 += std::make_pair('a', 'a'), std::make_pair('b', 'b'),
        ...
        std::make_pair('g', 'g');
    v3 += std::make_pair('a', 'a'), std::make_pair('b', 'b'),
        std::make_pair('c', 'c'), std::make_pair('d', 'd');
