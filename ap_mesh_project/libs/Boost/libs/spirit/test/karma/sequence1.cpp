...
int main() {
...

  {
    ...
    BOOST_TEST(!test_delimited("x i ", char_('x') << char_('o'), char(' ')));
  }

  {
    BOOST_TEST(test_delimited("Hello , World ", lit("Hello") << ',' << "World",
                              char(' ')));
  }
...

  {
    ...
    BOOST_TEST(
        test_delimited("a b cdefg ", char_ << char_ << string, p, char(' ')));
  }

  {
    fusion::vector<char, int, char> p('a', 12, 'c');
    BOOST_TEST(test("a12c", char_ << int_ << char_, p));
    BOOST_TEST(test_delimited("a 12 c ", char_ << int_ << char_, p, char(' ')));
  }

  {
    ...
    BOOST_TEST(
        test_delimited("a 12 c ", char_ << int_ << char_('c'), p1, char(' ')));

    BOOST_TEST(test("a12c", relaxed[char_ << int_ << char_('c')], p1));
    BOOST_TEST(test_delimited("a 12 c ", relaxed[char_ << int_ << char_('c')],
                              p1, char(' ')));

    BOOST_TEST(!test("", strict[char_ << int_ << char_('c')], p1));
    BOOST_TEST(!test_delimited("", strict[char_ << int_ << char_('c')], p1,
                               char(' ')));
    ...
    BOOST_TEST(
        test("\"foo ' bar\"", '"' << strict[*(~char_('*'))] << '"', value));
    BOOST_TEST(
        test("\"foo ' bar\"", strict['"' << *(~char_('*')) << '"'], value));
  }

  {
    ...
    BOOST_TEST(test("abcde", char_ << (lit('b') << 'c' << 'd') << char_, p));
...

  {
    ...
    BOOST_TEST(test_delimited("a b c ", char_ << 'b' << char_, p, char(' ')));
  }

  {
    ...
    BOOST_TEST(test_delimited("a c b ", char_ << char_ << 'b', p, char(' ')));
  }
