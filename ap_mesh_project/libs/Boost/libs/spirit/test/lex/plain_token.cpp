...
struct delimiter_tokens : lex::lexer<Lexer> {
  delimiter_tokens() {
    this->self = lex::char_('(', IDLPAREN) | lex::char_(')', IDRPAREN) |
                 lex::char_('<', IDLANGLE) | lex::char_('>', IDRANGLE) |
                 lex::char_('{', IDLBRACE) | lex::char_('}', IDRBRACE) |
                 lex::char_('[', IDLSQUARE) | lex::char_(']', IDRSQUARE) |
                 lex::string(".", IDANY);
  }
};
...
int main() {
  ...
  BOOST_TEST(lex::tokenize_and_parse(beg, angled_delimiter_str.end(), delims,
  ...
  BOOST_TEST(lex::tokenize_and_parse(beg, paren_delimiter_str.end(), delims,
                                     token(IDLPAREN, IDLSQUARE) >>
                                         +token(IDANY) >>
                                         token(IDRPAREN, IDRSQUARE)));
  ...
  BOOST_TEST(lex::tokenize_and_parse(
      beg, paren_delimiter_str.end(), delims,
