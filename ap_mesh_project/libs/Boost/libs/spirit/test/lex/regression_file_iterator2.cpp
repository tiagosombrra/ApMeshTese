...
typedef lex::lexertl::token<file_iterator,
                            boost::mpl::vector<identifier, string_literal> >
    token_type;

struct lexer : lex::lexer<lex::lexertl::actor_lexer<token_type> > {
  lexer() : id("[a-zA-Z0-9]+", 1), st("'[^'\\n]*'", 2) {
    self =
        id[lex::_state = "ST"] | lex::token_def<>(".", 3)[lex::_state = "ST"];

    self("ST") =
        st[lex::_state = "INITIAL"] |
        lex::token_def<>(".", 4)[(lex::_state = "INITIAL",
