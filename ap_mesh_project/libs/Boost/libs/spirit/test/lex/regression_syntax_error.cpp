...
struct my_grammar : qi::grammar<Iterator> {
  ...
  my_grammar(TokenDef const& tok) : my_grammar::base_type(statement) {
    statement = qi::eoi | *(delimiter | declaration);
...
typedef lex::lexertl::token<char const*, mpl::vector<std::string, double, int> >
    token_type;
