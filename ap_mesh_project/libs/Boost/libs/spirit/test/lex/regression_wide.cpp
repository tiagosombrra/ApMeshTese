...
///////////////////////////////////////////////////////////////////////////////
enum tokenids { ID_IDENT = 1, ID_CONSTANT, ID_OPERATION, ID_BRACKET };
...
// alpha+x1*(2.836-x2[i])
test_data data[] = {
    {ID_IDENT, L"alpha"}, {ID_OPERATION, L"+"}, {ID_IDENT, L"x1"},
    {ID_OPERATION, L"*"}, {ID_BRACKET, L"("},   {ID_CONSTANT, L"2.836"},
    {ID_OPERATION, L"-"}, {ID_IDENT, L"x2"},    {ID_BRACKET, L"["},
    {ID_IDENT, L"i"},     {ID_BRACKET, L"]"},   {ID_BRACKET, L")"}};
...
struct mega_tokens : lex::lexer<Lexer> {
  mega_tokens()
    ...
    this->self = operation[test(_tokenid, _val)] |
                 identifier[test(_tokenid, _val)] |
                 constant[test(_tokenid, _val)] | bracket[test(_tokenid, _val)];
  }
...
int main() {
  ...
  typedef lex::lexertl::token<base_iterator,
                              boost::mpl::vector<wchar_t, wstring_type, double>,
