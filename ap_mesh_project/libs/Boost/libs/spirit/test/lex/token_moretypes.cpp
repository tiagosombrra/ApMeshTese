...
///////////////////////////////////////////////////////////////////////////////
enum tokenids { ID_INT = 1000, ID_DOUBLE };
...
struct token_definitions : lex::lexer<Lexer> {
  token_definitions() {
    ...
    double_ =
        "{OPTSIGN}({DIGIT}*\\.{DIGIT}+|{DIGIT}+\\.){EXPONENT}?|{DIGIT}+{"
        "EXPONENT}";
    ...
    this->self =
        double_ | int_ | whitespace[lex::_pass = lex::pass_flags::pass_ignore];
  }
...
struct token_definitions_with_state : lex::lexer<Lexer> {
  token_definitions_with_state() {
    ...
    this->self("*") = double_[lex::_state = "DOUBLE"] |
                      int_[lex::_state = "INT"] |
...
inline bool test_token_ids(int const* ids, std::vector<Token> const& tokens) {
  BOOST_FOREACH (Token const& t, tokens) {
    if (*ids == -1) return false;  // reached end of expected data
...
///////////////////////////////////////////////////////////////////////////////
template <typename Token>
inline bool test_token_states(std::size_t const* states,
...
inline bool test_token_positions(Iterator begin, position_type const* positions,
                                 std::vector<Token> const& tokens) {
  BOOST_FOREACH (Token const& t, tokens) {
    ...
    // position must match
    if (start != positions->begin || end != positions->end) return false;
...
inline bool test_token_values(boost::optional<T> const* values,
                              std::vector<Token> const& tokens) {
  BOOST_FOREACH (Token const& t, tokens) {
...
int main() {
  ...
  boost::optional<int> ivalues[] = {1, none, -2, 3, none, none, 0};
  boost::optional<double> dvalues[] = {none, 1.2, none, none, 2.3e6, -3.4, 0.0};

  // token type: token id, iterator_pair as token value, no state
  {
    typedef lex::lexertl::token<base_iterator_type, mpl::vector<double, int>,
                                mpl::false_>
        token_type;
  ...
  {
    typedef lex::lexertl::position_token<base_iterator_type,
                                         mpl::vector<double, int>, mpl::false_>
        token_type;
    typedef lex::lexertl::actor_lexer<token_type> lexer_type;
  ...
  {
    typedef lex::lexertl::position_token<base_iterator_type,
                                         mpl::vector<double, int>, mpl::true_>
        token_type;
    typedef lex::lexertl::actor_lexer<token_type> lexer_type;
