...
struct test_action {
  test_action(char last) : last_(last) {}

  template <typename Context>
  void operator()(std::vector<char> const& v, Context const&, bool&) const {
    BOOST_TEST(v.size() == 4 && v[0] == 'a' && v[1] == 'b' && v[2] == '1' &&
               v[3] == last_);
  }
...
struct test_action_2 {
  ...
  void operator()(result_type const& v, Context const&, bool&) const {
    BOOST_TEST(v.size() == 5 && !v[0] && v[1] == 'a' && v[2] == 'b' &&
               v[3] == '1' && v[4] == '2');
  }
};
...
BOOST_FUSION_ADAPT_STRUCT(DIgnore, (std::string, text))

BOOST_FUSION_ADAPT_STRUCT(DInclude, (std::string, FileName))
