...
int main() {
  ...
  {
    typedef fusion::result_of::as_nview<test_data, 1, 0, 4>::type test_view;

    test_data d1 = {"", "", 0, 0.0, ""};
    test_view v1 = fusion::as_nview<1, 0, 4>(d1);
    BOOST_TEST(test_attr(
        "s1,2,1.5", *(qi::char_ - ',') >> ',' >> qi::int_ >> ',' >> qi::double_,
        v1));
...

  {
    ...
    std::string str;
    BOOST_TEST(test_attr(
        "123", qi::attr_cast<std::string, std::string>(*qi::digit), str));
    BOOST_TEST(str == "123");
  }
  ...
  // testing attribute reordering in a fusion sequence involving a rule
  {
    ...
    BOOST_TEST(v.size() == 2 && v[0].i1 == 2 && v[0].s1 == "s1" &&
               v[0].d1 == 1.5 && v[1].i1 == 4 && v[1].s1 == "s2" &&
               v[1].d1 == 3.5);
...

  {
    ...
    v.clear();
    BOOST_TEST(
        test_attr("1,2", qi::attr_cast<test_int_data1>(qi::int_) % ',', v));
    BOOST_TEST(v.size() == 2 && v[0].i == 1 && v[1].i == 2);

    v.clear();
    BOOST_TEST(test_attr(
        "1,2", qi::attr_cast<test_int_data1, int>(qi::int_) % ',', v));
    BOOST_TEST(v.size() == 2 && v[0].i == 1 && v[1].i == 2);
  }
