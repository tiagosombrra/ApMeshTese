...
bool test_rule(Char const* in, Attribute const& expected,
  ...
  Attribute attr = Attribute();
  qi::rule<Char const*, Attribute(), Skipper> r =
      qi::create_parser<Attribute>();
...
int main() {
...

  {
    ...
    // test containers
    std::vector<int> v;
    BOOST_TEST(test_create_parser("0 1 2", v) && v.size() == 3 && v[0] == 0 &&
               v[1] == 1 && v[2] == 2);
    ...
    BOOST_TEST(
        test_create_parser("some_string", vv) && vv.which() == 2 &&
        boost::equals(boost::get<std::vector<char> >(vv), "some_string"));

    // test fusion sequence
    std::pair<int, double> p;
    BOOST_TEST(test_create_parser("1 2.0", p) && p.first == 1 &&
               p.second == 2.0);
  }

  {
    ...
    BOOST_TEST(test_create_parser_auto("1", o) && !!o &&
               boost::get<int>(o) == 1);
    ...
    BOOST_TEST(test_create_parser_auto("1.0", vv) && vv.which() == 0 &&
               boost::get<double>(vv) == 1.0);
    BOOST_TEST(
        test_create_parser_auto("some_string", vv) && vv.which() == 2 &&
        boost::equals(boost::get<std::vector<char> >(vv), "some_string"));

    // test fusion sequence
    std::pair<int, double> p;
    BOOST_TEST(test_create_parser_auto("1 2.0", p) && p.first == 1 &&
               p.second == 2.0);
  }

  {
    ...
    // test fusion sequence
    std::pair<int, double> p;
    BOOST_TEST(test_attr("1 2.0", auto_, p, qi::space) && p.first == 1 &&
               p.second == 2.0);
  }
