...
int main() {
...

  {
    ...
    std::string s;
    BOOST_TEST(test_attr("a,b,c,d,e,f,g,h", char_ % ',', s) && s == "abcdefgh");
...

  {
    ...
    BOOST_TEST(test_attr("ab,cd,ef,gh", (char_ >> char_) % ',', v2) &&
               v2.size() == 4 && v2[0] == std::make_pair('a', 'b') &&
...

  {
    ...
    v1.clear();
    BOOST_TEST(
        test_attr("abc,def,gh", (char_ >> omit[*~char_(',')]) % ',', v1) &&
        compare(v1, "adg"));
    v1.clear();
    BOOST_TEST(
        test_attr("abc,def,gh", (omit[char_] >> *~char_(',')) % ',', v1) &&
        compare(v1, "bcefh"));
    ...
    s1.clear();
    BOOST_TEST(
        test_attr("abc,def,gh", (char_ >> omit[*~char_(',')]) % ',', s1) &&
        s1 == "adg");
    s1.clear();
    BOOST_TEST(
        test_attr("abc,def,gh", (omit[char_] >> *~char_(',')) % ',', s1) &&
        s1 == "bcefh");

    std::vector<std::pair<char, std::vector<char> > > v2;
    BOOST_TEST(test_attr("abc,def,gh", (char_ >> *~char_(',')) % ',', v2) &&
               v2.size() == 3 && v2[0].first == 'a' &&
               compare(v2[0].second, "bc") && v2[1].first == 'd' &&
               compare(v2[1].second, "ef") && v2[2].first == 'g' &&
               compare(v2[2].second, "h"));

    std::vector<std::vector<char> > v3;
    BOOST_TEST(
        test_attr("abc,def,gh", (omit[char_] >> *~char_(',')) % ',', v3) &&
        v3.size() == 3 && compare(v3[0], "bc") && compare(v3[1], "ef") &&
        compare(v3[2], "h"));

    std::vector<char> v4;
    BOOST_TEST(
        test_attr("abc,def,gh", (char_ >> omit[*~char_(',')]) % ',', v4) &&
        v4.size() == 3 && v4[0] == 'a' && v4[1] == 'd' && v4[2] == 'g');

    std::vector<std::string> v5;
    BOOST_TEST(
        test_attr("abc,def,gh", (omit[char_] >> *~char_(',')) % ',', v5) &&
        v5.size() == 3 && v5[0] == "bc" && v5[1] == "ef" && v5[2] == "h");

    std::string s2;
    BOOST_TEST(
        test_attr("abc,def,gh", (char_ >> omit[*~char_(',')]) % ',', s2) &&
        s2.size() == 3 && s2 == "adg");
...

  {
    ...
    v1.clear();
    BOOST_TEST(
        test_attr("ab12,cd34,ef56", (omit[*alpha] >> *digit) % ',', v1) &&
        compare(v1, "123456"));
    v1.clear();
    BOOST_TEST(
        test_attr("ab12,cd34,ef56", (*alpha >> omit[*digit]) % ',', v1) &&
        compare(v1, "abcdef"));
    ...
    s1.clear();
    BOOST_TEST(
        test_attr("ab12,cd34,ef56", (omit[*alpha] >> *digit) % ',', s1) &&
        s1 == "123456");
    s1.clear();
    BOOST_TEST(
        test_attr("ab12,cd34,ef56", (*alpha >> omit[*digit]) % ',', s1) &&
        s1 == "abcdef");

    std::vector<std::pair<std::vector<char>, std::vector<char> > > v2;
    BOOST_TEST(test_attr("ab12,cd34,ef56", (*alpha >> *digit) % ',', v2) &&
               v2.size() == 3 && compare(v2[0].first, "ab") &&
               compare(v2[0].second, "12") && compare(v2[1].first, "cd") &&
               compare(v2[1].second, "34") && compare(v2[2].first, "ef") &&
               compare(v2[2].second, "56"));

    std::vector<std::pair<std::string, std::string> > v3;
    BOOST_TEST(test_attr("ab12,cd34,ef56", (*alpha >> *digit) % ',', v3) &&
               v3.size() == 3 && v3[0].first == "ab" && v3[0].second == "12" &&
               v3[1].first == "cd" && v3[1].second == "34" &&
               v3[2].first == "ef" && v3[2].second == "56");

    std::vector<std::vector<char> > v4;
    BOOST_TEST(
        test_attr("ab12,cd34,ef56", (omit[*alpha] >> *digit) % ',', v4) &&
        v4.size() == 3 && compare(v4[0], "12") && compare(v4[1], "34") &&
        compare(v4[2], "56"));
