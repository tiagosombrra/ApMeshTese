...
int main() {
...

  {
    std::string s;
    BOOST_TEST(
        test_attr("aaaaaaaa", repeat[char_ >> char_], s));  // kleene synonym
...
#define rep01 repeat(0, 1)[alpha >> digit] >> '?'
...

  {
    ...
    BOOST_TEST(test_attr("a b c d", repeat(4)[lexeme[+alpha]], v, space) &&
               4 == v.size() && v[0] == "a" && v[1] == "b" && v[2] == "c" &&
               v[3] == "d");
  }

  {
    ...
    s.clear();
    BOOST_TEST(test_attr("bbbb", omit[repeat(4)[char_('b')]], s) &&
               s == "bbbb");

    s.clear();
    BOOST_TEST(test_attr("b b b b", omit[repeat(4)[char_('b')]], s, space) &&
               s == "bbbb");
  }
  ...
  {  // actions
    ...
    std::vector<char> v;
    BOOST_TEST(test("bbbb", repeat(4)[char_][phx::ref(v) = _1]) &&
               4 == v.size() && v[0] == 'b' && v[1] == 'b' && v[2] == 'b' &&
               v[3] == 'b');
  }
  ...
  {  // more actions
    ...
    std::vector<int> v;
    BOOST_TEST(test("123 456 789", repeat(3)[int_][phx::ref(v) = _1], space) &&
               3 == v.size() && v[0] == 123 && v[1] == 456 && v[2] == 789);
  }
