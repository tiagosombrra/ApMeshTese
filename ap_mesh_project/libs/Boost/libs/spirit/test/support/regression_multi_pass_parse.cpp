...
std::vector<double> parse(std::istream& input) {
  ...
  // parse
  bool r = qi::phrase_parse(fwd_begin, fwd_end,  // iterators over input
                            qi::double_ >> *(',' >> qi::double_) >>
                                qi::eoi,  // recognize list of doubles
                            ascii::space | '#' >> *(ascii::char_ - qi::eol) >>
                                               qi::eol,  // comment skipper
                            output);  // doubles are stored into this object

  // error detection
  if (!r || fwd_begin != fwd_end) throw std::runtime_error("parse error");
