...
inline bool check(boost::spirit::utree const& val, std::string expected) {
  std::stringstream s;
  s << val;
  if (s.str() == expected + " ") return true;

  std::cerr << "got result: " << s.str() << ", expected: " << expected
            << std::endl;
  return false;
}
...
int main() {
  ...
  {
    // test the size
    std::cout << "size of utree is: " << sizeof(utree) << " bytes" << std::endl;
...

  {
    ...
    BOOST_TEST(check(
        val,
        "( 123 \"Chuckie\" ( 123.456 \"Mah Doggie\" ) \"another string\" )"));
    val.pop_front();
    BOOST_TEST(check(
        val, "( \"Chuckie\" ( 123.456 \"Mah Doggie\" ) \"another string\" )"));
    utree::iterator i = val.begin();
    ++ ++i;
    val.insert(i, "Right in the middle");
    BOOST_TEST_EQ(val.size(), 4U);
    BOOST_TEST(check(val,
                     "( \"Chuckie\" ( 123.456 \"Mah Doggie\" ) \"Right in the "
                     "middle\" \"another string\" )"));
    val.pop_back();
    BOOST_TEST(check(
        val,
        "( \"Chuckie\" ( 123.456 \"Mah Doggie\" ) \"Right in the middle\" )"));
    BOOST_TEST_EQ(val.size(), 3U);
    utree::iterator it = val.end();
    --it;
    val.erase(it);
    ...
    BOOST_TEST(check(
        val,
        "( 123.456 \"Mah Doggie\" \"Chuckie\" ( 123.456 \"Mah Doggie\" ) )"));
    BOOST_TEST_EQ(val.size(), 4U);
...

  {
    ...
    for (utree::size_type i = 0; i < a.size(); ++i) get(a, i) = int(i + 1);
  ...
  {  // put it in an array
    ...
    utree vals[] = {utree(123), utree("Hello, World"), utree(123.456)};
