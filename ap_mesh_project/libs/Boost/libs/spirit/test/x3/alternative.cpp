...
BOOST_FUSION_ADAPT_STRUCT(di_ignore, text)

BOOST_FUSION_ADAPT_STRUCT(di_include, FileName)
...
int main() {
...

  {
    ...
    vector<char, char> v;
    BOOST_TEST(
        (test_attr("abc", char_ >> (omit[char_] | omit[char_]) >> char_, v)));
...

  {
    ...
    std::string s;
    BOOST_TEST((test_attr("abc,a,b,c", char_ >> char_ >> (char_ % ','), s)));
    BOOST_TEST(s == "abcabc");

    // test having an optional<container> inside a sequence
    s.erase();
    BOOST_TEST((test_attr("ab", char_ >> char_ >> -(char_ % ','), s)));
    BOOST_TEST(s == "ab");

    // test having a variant<container, ...> inside a sequence
    s.erase();
    BOOST_TEST((test_attr("ab", char_ >> char_ >> ((char_ % ',') | eps), s)));
    BOOST_TEST(s == "ab");
    s.erase();
    BOOST_TEST((test_attr("abc", char_ >> char_ >> ((char_ % ',') | eps), s)));
    BOOST_TEST(s == "abc");
  }
...

  {
    ...
    auto start = line = include | ignore;
  ...
  {  // attributeless parsers must not insert values
    ...
    &&BOOST_TEST_EQ(v[0], 1) && BOOST_TEST_EQ(v[1], 2) &&
        BOOST_TEST_EQ(v[2], 3) && BOOST_TEST_EQ(v[3], 5) &&
        BOOST_TEST_EQ(v[4], 7);
  }
  ...
  {  // regressing test for #603
    ...
    BOOST_TEST(
        test_attr("xx42x9y", *(int_ | +char_('x') | 'y' >> attr(X{})), v));
    BOOST_TEST_EQ(v.size(), 5);
  }
  ...
  {  // sequence parser in alternative into container
    ...
    BOOST_TEST(test_attr(
        "abcbbcd", *(char_('a') >> *(*char_('b') >> char_('c')) | char_('d')),
        s));
