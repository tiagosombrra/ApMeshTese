...
struct checked_attr_parser : x3::attr_parser<Value> {
  ...
  bool parse(Iterator& first, Iterator const& last, Context const& ctx,
             RuleContext& rctx, Attribute& attr_) const {
...
static inline checked_attr_parser<boost::decay_t<Value>, Expected> checked_attr(
    Value&& value) {
...
template <typename Expected, typename... ExpectedTail, typename Attribute,
          typename Expr, typename Value, typename... Tail>
static void gen_sequence(Attribute const& attribute, Expr&& expr,
                         Value const& v, Tail const&... tail) {
  gen_sequence<ExpectedTail...>(attribute, expr >> checked_attr<Expected>(v),
                                tail...);
  gen_sequence<ExpectedTail...>(
      attribute, expr >> x3::eps >> checked_attr<Expected>(v), tail...);
  gen_sequence<ExpectedTail...>(
      attribute, expr >> (x3::eps >> checked_attr<Expected>(v)), tail...);
}

template <typename Expected, typename... ExpectedTail, typename Attribute,
          typename Value, typename... Tail>
static void gen_sequence_tests(Attribute const& attribute, Value const& v,
                               Tail const&... tail) {
  gen_sequence<ExpectedTail...>(attribute, checked_attr<Expected>(v), tail...);
  gen_sequence<ExpectedTail...>(attribute, x3::eps >> checked_attr<Expected>(v),
                                tail...);
}
...
template <typename Expected, typename... ExpectedTail, typename Value,
          typename... Tail>
...
static void gen_tests(Values const&... values) {
  ...
  boost::fusion::vector<Expected...> attribute =
      boost::fusion::make_vector(values...);
...
void make_test(Attributes const&... attrs) {
  ...
  gen_tests<boost::optional<Attributes>...,
            boost::fusion::vector<Attributes>...>(attrs..., attrs...);
  gen_tests<boost::optional<boost::fusion::vector<Attributes>>...,
