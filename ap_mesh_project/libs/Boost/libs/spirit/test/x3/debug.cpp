...
struct my_error_handler {
  ...
  boost::spirit::x3::error_handler_result operator()(Iterator&,
                                                     Iterator const& last,
                                                     Exception const& x,
                                                     Context const&) const {
...
struct my_attribute {
  ...
  void access() const { BOOST_TEST(alive); }
  ~my_attribute() { alive = false; }
...
int main() {
  ...
  {  // basic tests w/ skipper
...

    {
      ...
      BOOST_TEST(
          test(" a a a a b a b a b a a a b b a ", start_def, space, false));
    }
  }
