...
BOOST_FUSION_ADAPT_STRUCT(adata, a, b)

struct test_attribute_type {
  template <typename Context>
  void operator()(Context& ctx) const {
    BOOST_TEST(typeid(decltype(_attr(ctx))).name() ==
               typeid(boost::optional<int>).name());
  }
};
...
int main() {
...

  {
    std::vector<adata> v;
    BOOST_TEST(
        (test_attr("a 1 2 a 2", *('a' >> int_ >> -int_), v, char_(' '))));
    BOOST_TEST(2 == v.size() && 1 == v[0].a && v[0].b && 2 == *(v[0].b) &&
               2 == v[1].a && !v[1].b);
  }
