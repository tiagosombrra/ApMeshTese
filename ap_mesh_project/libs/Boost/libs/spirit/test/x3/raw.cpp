...
int main() {
...

  {
    ...
    std::string str;
    BOOST_TEST(
        (test_attr("spirit_test_123", raw[alpha >> *(alnum | '_')], range)));
...

  {
    std::string str;
    BOOST_TEST(
        (test_attr("spirit_test_123", raw[alpha >> *(alnum | '_')], str)));
...

  {
    boost::iterator_range<char const*> range;
    BOOST_TEST(
        (test("x", raw[alpha][([&](auto& ctx) { range = _attr(ctx); })])));
...

  {
    ...
    BOOST_TEST(
        (test_attr("x123x", lit('x') >> raw[+digit] >> lit('x'), range)));
...

  {
    ...
    parse(str.begin(), str.end(), (raw[int_] >> raw[*char_]), attr);
