...
struct ts_real_policies : boost::spirit::x3::ureal_policies<T> {
  ...
  static bool parse_frac_n(Iterator& first, Iterator const& last,
    ...
    return boost::spirit::x3::extract_uint<T, 10, 1, 2, true>::call(first, last,
                                                                    attr);
  }
...
bool compare(T n, boost::type_identity_t<T> expected) {
  ...
  using std::pow;
  T const eps =
      pow(T(10), -std::numeric_limits<T>::digits10 + log10(abs(expected)));
  T delta = n - expected;
...
struct custom_real {
  ...
  friend bool operator==(custom_real a, custom_real b) { return a.n == b.n; }
