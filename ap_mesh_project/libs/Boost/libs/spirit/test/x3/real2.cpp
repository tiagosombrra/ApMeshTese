...
int main() {
...

  {
    ...
    BOOST_TEST(test_attr("-inf", double_, d) && std::isinf(d) &&
               std::signbit(d));
    BOOST_TEST(test_attr("-infinity", double_, d) && std::isinf(d) &&
               std::signbit(d));
    BOOST_TEST(test("-INF", double_));
    BOOST_TEST(test("-INFINITY", double_));
    BOOST_TEST(test_attr("-INF", double_, d) && std::isinf(d) &&
    ...
    BOOST_TEST(test_attr("-nan", double_, d) && std::isnan(d) &&
               std::signbit(d));
    BOOST_TEST(test("-NAN", double_));
    BOOST_TEST(test_attr("-NAN", double_, d) && std::isnan(d) &&
               std::signbit(d));
    ...
    BOOST_TEST(
        test_attr("1267650600228229401496703205376", double_, d) &&
        compare(
            d,
            1267650600228229401496703205376.));  // Note Qi has better precision

    BOOST_TEST(
        test_attr("12676506.00228229401496703205376", double_, d) &&
        compare(
            d,
            12676506.00228229401496703205376));  // Note Qi has better precision

    BOOST_TEST(
        test_attr("12676506.00228229401496703205376E6", double_, d) &&
        compare(d, 12676506.00228229401496703205376E6));  // Note Qi has better
                                                          // precision
  }
