...
struct my_rule_class {
  ...
  x3::error_handler_result on_error(Iterator&, Iterator const& last,
    ...
    std::cout << "Error! Expecting: " << x.which() << ", got: \""
              << std::string(x.where(), last) << "\"" << std::endl;
    return x3::error_handler_result::fail;
  }

  template <typename Iterator, typename Attribute, typename Context>
  inline void on_success(Iterator const&, Iterator const&, Attribute&,
                         Context const&) {
    ++got_it;
  }
};
...
int main() {
  ...
  {  // error handling
    ...
    auto r = rule<my_rule_class, char const*>() = '(' > int_ > ',' > int_ > ')';
  ...
  // test handling of single element fusion sequences
  {
    ...
    auto r = rule<class r, vector<int>>() = int_;
