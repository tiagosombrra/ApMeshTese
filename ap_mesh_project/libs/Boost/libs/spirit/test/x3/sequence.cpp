...
int main() {
  ...
  {
    // "hello" has an unused_type. unused attributes are not part of the
    // sequence
    vector<char, char> attr;
    BOOST_TEST(
...

  {
    ...
    auto r = rule<class r, attr_type>() = char_ >> ',' >> int_;

    BOOST_TEST(
        (test_attr("test:x,1", "test:" >> r, fv) && fv == attr_type('x', 1)));
  }

  {
    ...
    auto r = rule<class r, attr_type>() = int_;

    BOOST_TEST((test_attr("test:1", "test:" >> r, fv) && fv == attr_type(1)));
  }
  ...
  {  // alternative forms of attributes. Allow sequences to take in
    ...
    //~ v.clear();
    //~ BOOST_TEST(test_attr("abcdef", char_ >> *hold[char_ >> char_] >> char_,
    //v)); ~ BOOST_TEST(v.size() == 6); ~ BOOST_TEST(v[0] == 'a'); ~
    //BOOST_TEST(v[1] == 'b'); ~ BOOST_TEST(v[2] == 'c'); ~ BOOST_TEST(v[3] ==
    //'d'); ~ BOOST_TEST(v[4] == 'e'); ~ BOOST_TEST(v[5] == 'f');
  ...
  {  // alternative forms of attributes. Allow sequences to take in
    ...
    //~ BOOST_TEST(test_attr("ab.bc.ca", *hold[word >> string(".")] >> word,
    //s)); ~ BOOST_TEST(s == "ab.bc.ca");
  }
  ...
  // stl containers of stl containers.
  {
    ...
    BOOST_TEST(
        test_attr("abc1,abc2", *~char_(',') >> *(',' >> *~char_(',')), v));
...

  {
    ...
    auto e = rule<class e, std::string>() = *~char_(',');

    auto l = rule<class l, std::vector<std::string>>() = e >> *(',' >> e);
  ...
  // do the same with a plain string object
  {
    std::string s;
    BOOST_TEST(
        test_attr("abc1,abc2", *~char_(',') >> *(',' >> *~char_(',')), s));
...

  {
    ...
    auto l = rule<class l, std::string>() = e >> *(',' >> e);
...

  {
    ...
    auto const expr = rule<class expr, Attr>("expr") =
        term | ('(' > term > ')');
  ...
  // makes it unwrapped
  {
    BOOST_TEST((boost::is_same<
                typename attribute_of<decltype(lit("abc") >> attr(long())),
                                      unused_type>::type,
                long>()));
  }
  ...
  {  // test action
    ...
    BOOST_TEST(test("x 123 \"a string\"", (char_ >> int_ >> "\"a string\"")[f],
                    space));
    BOOST_TEST(c == 'x');
    BOOST_TEST(n == 123);
  }
  ...
  {  // test move only types
    ...
    BOOST_TEST(
        test_attr("ssszs", *synth_move_only >> 'z' >> synth_move_only, v));
