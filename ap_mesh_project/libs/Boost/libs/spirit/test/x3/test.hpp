...
bool test(Char const* in, Parser const& p, bool full_match = true) {
  Char const* last = in;
  while (*last) last++;
  return boost::spirit::x3::parse(in, last, p) && (!full_match || (in == last));
}
...
bool test(boost::basic_string_view<Char, std::char_traits<Char>> in,
  ...
  return boost::spirit::x3::parse(pos, last, p) &&
         (!full_match || (pos == last));
}

template <typename Char, typename Parser, typename Skipper>
bool test(Char const* in, Parser const& p, Skipper const& s,
...
template <typename Char, typename Parser>
bool test_failure(
    boost::basic_string_view<Char, std::char_traits<Char>> const in,
    Parser const& p) {
  auto pos = in.begin();
...
bool test_attr(Char const* in, Parser const& p, Attr& attr,
               bool full_match = true) {
  Char const* last = in;
  while (*last) last++;
...
bool test_attr(Char const* in, Parser const& p, Attr& attr, Skipper const& s,
               bool full_match = true) {
  Char const* last = in;
  while (*last) last++;
