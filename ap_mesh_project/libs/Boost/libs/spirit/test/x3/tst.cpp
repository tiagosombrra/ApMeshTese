...
void docheck(TST const& tst, CaseCompare const& comp, Char const* s,
             bool expected, int N = 0, int val = -1) {
  Char const* first = s;
  Char const* last = s;
  while (*last) last++;
  int* r = tst.find(s, last, comp);
  BOOST_TEST((r != 0) == expected);
  if (r != 0) BOOST_TEST((s - first) == N);
  if (r) BOOST_TEST(*r == val);
}
...
boost::spirit::x3::case_compare<boost::spirit::char_encoding::standard_wide>
    wcomp;
boost::spirit::x3::no_case_compare<boost::spirit::char_encoding::standard>
    nc_ncomp;
boost::spirit::x3::no_case_compare<boost::spirit::char_encoding::standard_wide>
    nc_wcomp;
