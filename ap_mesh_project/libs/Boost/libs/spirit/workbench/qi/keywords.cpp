...
#define declOptions(r, data, i, elem) \
  boost::optional<int> BOOST_PP_CAT(option, i);
#define fusionOptions(r, data, i, elem) \
  (boost::optional<int>, BOOST_PP_CAT(option, i))
...
#define generateOutput1(r, d, i, elem)                             \
  if (BOOST_PP_CAT(data.options.option, i))                        \
    os << BOOST_PP_STRINGIZE( BOOST_PP_CAT(option,i)) << " " << *BOOST_PP_CAT(data.options.option, i) \
                   << std::endl;
...
#define generateOutput2(r, d, i, elem) \
  if (BOOST_PP_CAT(data.option, i))    \
    os << BOOST_PP_STRINGIZE( BOOST_PP_CAT(option,i)) << " " << *BOOST_PP_CAT(data.option, i) << std::endl;
...
BOOST_FUSION_ADAPT_STRUCT(client::parsedDataOptions,
                          BOOST_PP_SEQ_FOR_EACH_I(fusionOptions, _, keys))

BOOST_FUSION_ADAPT_STRUCT(client::parsedData,
                          (std::string, name)(client::parsedDataOptions,
                                              options))

BOOST_FUSION_ADAPT_STRUCT(client::parsedData2,
                          (std::string,
                           name)BOOST_PP_SEQ_FOR_EACH_I(fusionOptions, _, keys))
enum variation { full, no_assign, assign };
namespace client {
...
struct permutation_parser
    : qi::grammar<Iterator, parsedData(), ascii::space_type> {
...
#define generateOptions1(r, data, i, elem) \
  BOOST_PP_IF(i, ^(lit(elem) > int_), (lit(elem) > int_))
    options = (BOOST_PP_SEQ_FOR_EACH_I(generateOptions1, _, keys));

    start %= quoted_string >> options;
    ;
...
struct alternative_parser
    : qi::grammar<Iterator, parsedData2(), ascii::space_type> {
...
#define generateOptions2(r, data, i, elem)                    \
  BOOST_PP_IF(i, | (lit(elem) > int_[at_c<i + 1>(_r1) = _1]), \
              (lit(elem) > int_[at_c<i + 1>(_r1) = _1]))
    options = (BOOST_PP_SEQ_FOR_EACH_I(generateOptions2, _, keys));

    start = quoted_string[at_c<0>(_val) = _1] >> *options(_val);
    ;
  }
...
#define generateRules(r, data, i, elem) \
  BOOST_PP_CAT(rule, i) = int_[phx::at_c<i + 1>(*phx::ref(currentObj)) = _1];
...
#define declareRules(r, data, i, elem) optionsRule BOOST_PP_CAT(rule, i);
  ...
  qi::rule<Iterator, parsedData2(), qi::locals<optionsRule *>,
           ascii::space_type>
      start;
};

template <typename Iterator, typename variation>
struct tst_map_parser
    : qi::grammar<Iterator, parsedData2(), ascii::space_type> {
  typedef variation variation_type;
...
#define generateRules3(r, data, i, elem) \
  BOOST_PP_CAT(rule, i) = int_[phx::at_c<i + 1>(*phx::ref(currentObj)) = _1];
...
#define declareRules(r, data, i, elem) optionsRule BOOST_PP_CAT(rule, i);
  ...
  qi::symbols<char, optionsRule *,
              boost::spirit::qi::tst_map<char, optionsRule *> >
      options;
  qi::rule<Iterator, parsedData2(), ascii::space_type> startalias;
  qi::rule<Iterator, parsedData2(), qi::locals<optionsRule *>,
           ascii::space_type>
...
#define generateOptions4(r, data, i, elem) \
  BOOST_PP_IF(i, / kwd(elem)[int_], kwd(elem)[int_])
  ...
  qi::rule<Iterator, parsedDataOptions(), ascii::space_type> options;
  qi::rule<Iterator,
           boost::fusion::vector<boost::optional<int>, boost::optional<int> >(),
           ascii::space_type>
      v_vals;
...
typedef client::tst_map_parser<iterator_type, boost::mpl::int_<full> >
    tst_map_parser;
...
struct tst_timer_fwd_full
    : timeParser<client::tst_parser<iterator_type, boost::mpl::int_<full> > > {
  tst_timer_fwd_full()
      : timeParser<client::tst_parser<iterator_type, boost::mpl::int_<full> > >(
            fwd) {}
};

struct tst_timer_fwd_no_assign
    : timeParser<
          client::tst_parser<iterator_type, boost::mpl::int_<no_assign> > > {
...
struct tst_timer_fwd_assign
    : timeParser<
          client::tst_parser<iterator_type, boost::mpl::int_<assign> > > {
...
int main() {
  BOOST_SPIRIT_TEST_BENCHMARK(
      ...
      (permutation_timer_fwd)(permutation_timer_back)(alternative_timer_fwd)(alternative_timer_back)(tst_timer_fwd_full)(tst_timer_fwd_no_assign)(tst_timer_fwd_assign)(tst_timer_back)(tst_map_timer_fwd)(tst_map_timer_back)(kwd_timer_fwd)(kwd_timer_back))
