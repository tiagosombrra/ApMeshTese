...
struct DeferralTest : sc::state_machine<DeferralTest, Active> {
  ...
  void IncrementProcessedCount() { ++processedCount_; }

  unsigned int ProcessedCount() const { return processedCount_; }
...
struct Active : sc::simple_state<Active, DeferralTest, Idle> {
  typedef mpl::list<sc::custom_reaction<EvLeafDeferred>,
...
struct Idle : sc::simple_state<Idle, Active> {
  typedef mpl::list<sc::transition<EvSwitch, Running>,
...
struct X1 : sc::simple_state<X1, DeferralEventBaseTest> {
  typedef mpl::list<sc::transition<EvToX2, X2>, sc::deferral<sc::event_base> >
      reactions;
};
