...
struct TerminationTest : sc::state_machine<TerminationTest, A> {
 ...
 private:
  ...
  void AddName(state_base_type::id_type stateType) const {
    const StateNamesMap::const_iterator found = stateNamesMap_.find(stateType);
...
template <class MostDerived, class Context, class InnerInitial = mpl::list<> >
