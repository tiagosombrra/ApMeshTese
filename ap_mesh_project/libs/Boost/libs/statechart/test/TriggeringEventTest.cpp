...
struct A;
struct TriggringEventTest
    : sc::state_machine<TriggringEventTest, A, std::allocator<sc::none>,
...
struct B : sc::state<B, TriggringEventTest> {
  ...
  ~B() { BOOST_REQUIRE(triggering_event() == 0); }
  ...
  void HandleException(const sc::exception_thrown &) {
    BOOST_REQUIRE(
        dynamic_cast<const sc::exception_thrown *>(triggering_event()) != 0);
  }
...
struct A : sc::state<A, TriggringEventTest> {
  typedef sc::transition<EvGoToB, B, TriggringEventTest,
                         &TriggringEventTest::Transit>
      reactions;

  A(my_context ctx) : my_base(ctx) { BOOST_REQUIRE(triggering_event() == 0); }
