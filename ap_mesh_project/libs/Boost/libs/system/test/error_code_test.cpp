...
int main(int, char**) {
  ...
  BOOST_TEST_NE(generic_category() < system_category(),
  ...
  BOOST_TEST(econd.category() == generic_category());
  BOOST_TEST(econd ==
             error_condition(errc::permission_denied, generic_category()));
  ...
  // BOOST_TEST( ec_0_system.message().substr(0,36) == "The operation completed
  // successfully" );
  ...
  BOOST_TEST(ec.default_error_condition().value() ==
             errc::operation_would_block);
  BOOST_TEST(ec.default_error_condition().category() == generic_category());
