...
BOOST_PRAGMA_MESSAGE(
    "BOOST_SYSTEM_HAS_SYSTEM_ERROR not defined, test will be skipped")

int main() {
  std::cout << "The version of the C++ standard library being used does not"
               " support header <system_error> so interoperation will not be "
               "tested.\n";
}
...
class user_category_impl : public boost::system::error_category {
 public:
  ...
  virtual boost::system::error_condition default_error_condition(int ev) const
      BOOST_NOEXCEPT {
    if (ev == 4) {
      return boost::system::error_condition(EMFILE,
                                            boost::system::generic_category());
    } else if (ev == 5) {
      return boost::system::error_condition(EACCES,
                                            boost::system::generic_category());
    } else {
  ...
  virtual bool equivalent(int code,
                          const boost::system::error_condition& condition) const
      BOOST_NOEXCEPT {
    if (code == 4 &&
        condition == make_error_condition(
...
static void test_user_category() {
...

  {
    ...
    {
      boost::system::error_condition bn = make_error_condition(
          boost::system::errc::too_many_files_open_in_system);
      BOOST_TEST(bt.equivalent(ev, bn));
    ...
    {
      boost::system::error_condition bn =
          make_error_condition(boost::system::errc::too_many_files_open);
      BOOST_TEST(bt.equivalent(ev, bn));
...
enum user2_errc { my_enoent = 1, my_einval, my_other };

class user2_category_impl : public boost::system::error_category {
 public:
  virtual const char* name() const BOOST_NOEXCEPT { return "user2"; }
