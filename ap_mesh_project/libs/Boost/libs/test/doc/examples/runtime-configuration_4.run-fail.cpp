...
class file_dataset {
 ...
 public:
  file_dataset(std::size_t line_start = 0,
               std::size_t line_end = std::size_t(-1))
      : m_line_start(line_start), m_line_end(line_end) {
    int argc = boost::unit_test::framework::master_test_suite().argc;
    char** argv = boost::unit_test::framework::master_test_suite().argv;

    if (argc != 3) throw std::logic_error("Incorrect number of arguments");
    ...
    std::size_t nb_lines = std::count_if(std::istreambuf_iterator<char>(file),
  ...
  struct iterator {
    iterator(std::string const& filename, std::size_t line_start)
      ...
      if (!file.is_open()) throw std::runtime_error("Cannot open the file");
    ...
    void operator++() { getline(file, m_current_line); }
  ...
  iterator begin() const { return iterator(m_filename, m_line_start); }
};
