...
class dangerous_call {
 public:
  ...
  int operator()() {
    // here we perform some operation under monitoring that could throw
    // my_exception
    if (m_argc < 2) throw my_exception1(23);
    if (m_argc > 3)
...
// we are currently not able to silence those errors below with UBSAN under
// clang this seems to come from the way clang handles floating point exceptions
// + UB.
