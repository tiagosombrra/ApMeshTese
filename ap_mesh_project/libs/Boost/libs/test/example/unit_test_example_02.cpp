...
void free_test_function() {
  ...
  // reports 'unknown location(0): fatal error in "free_test_function": memory
  // access violation
  //          d:\source
  //          code\boost\libs\test\example\unit_test_example_02.cpp(25): last
  //          checkpoint'
  int* p = (int*)0x01;
  BOOST_CHECK(*p == 0);
}
...
test_suite* init_unit_test_suite(int, char*[]) {
  ...
  // register the test case in test tree and specify number of expected failures
  // so this example will pass at runtime. We expect 2 errors: one from failed
  // check and one from memory acces violation
