...
struct suite_fixture2 {
  ...
  ~suite_fixture2() {
    BOOST_TEST_MESSAGE("Running some more test suite teardown");
  }
};

// automatically registered test cases could be organized in test suites
BOOST_AUTO_TEST_SUITE(my_suite1, *bt::fixture<suite_fixture>() *
                                     bt::fixture<suite_fixture2>())

BOOST_AUTO_TEST_CASE_EXPECTED_FAILURES(my_test1, 1)

void some_setup() { BOOST_TEST_MESSAGE("Running some extra setup"); }

auto& d = *bt::label("L1") * bt::label("L2") *
          bt::description("test case 1 description") * bt::fixture(&some_setup);
BOOST_AUTO_TEST_CASE(my_test1) { BOOST_TEST(2 == 1); }
...
BOOST_TEST_DECORATOR(*bt::label("L3") * bt::description("suite description"))
BOOST_AUTO_TEST_SUITE(my_suite2)

// this test case belongs to suite2 test suite
BOOST_AUTO_TEST_CASE(my_test4,
                     *bt::depends_on("my_suite2/internal_suite/my_test5")) {
  int i = 0;
...
BOOST_TEST_DECORATOR(*bt::timeout(100))
