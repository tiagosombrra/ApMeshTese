...
class hash_function {
 public:
  ...
  explicit hash_function(std::string const& alphabet) : m_alphabet(alphabet) {
    if (m_alphabet.size() != AlphabetSize)
      throw std::runtime_error("Wrong alphabet size");

    std::sort(m_alphabet.begin(), m_alphabet.end());

    if (std::adjacent_find(m_alphabet.begin(), m_alphabet.end()) !=
        m_alphabet.end())
  ...
  unsigned long operator()(std::string const& arg) {
    ...
    if (arg.length() > 8) throw std::runtime_error("Wrong argument size");
 ...
 private:
  bool helper_(char c) {
    std::string::const_iterator it =
        std::find(m_alphabet.begin(), m_alphabet.end(), c);

    if (it == m_alphabet.end()) return true;
  ...
  unsigned long power_of_10_(int i) {
    switch (i) {
      case 0:
        return power_of_10<0>::value;
      case 1:
        return power_of_10<1>::value;
      case 2:
        return power_of_10<2>::value;
      case 3:
        return power_of_10<3>::value;
      case 4:
        return power_of_10<4>::value;
      default:
        return 0;
    }
  }
 ...
 public:
  ...
  void test(hash_function_test_data const& test_data) {
    if (test_data.exp_value == (unsigned long)-1)
      BOOST_CHECK_THROW(m_function_under_test(test_data.orig_string),
                        std::runtime_error);
...
struct massive_hash_function_test : test_suite {
  massive_hash_function_test() : test_suite("massive_hash_function_test") {
    ...
    boost::shared_ptr<hash_function_tester> instance(
        new hash_function_tester(alphabet));

    std::cout << "\nEnter test data in a format [string] [value] to check "
                 "correct calculation\n";
    std::cout << "Enter test data in a format [string] -1 to check long string "
                 "validation\n";
    std::cout << "Enter test data in a format [string] -2 to check invalid "
                 "argument string validation\n";
    ...
    while (!std::cin.eof()) {
      ...
      if (!(std::cin >> test_data)) break;

      test_data_store.push_back(test_data);
    }

    add(make_test_case(&hash_function_tester::test, "hash_function_tester",
                       __FILE__, __LINE__, instance, test_data_store.begin(),
                       test_data_store.end()));
  }
};
