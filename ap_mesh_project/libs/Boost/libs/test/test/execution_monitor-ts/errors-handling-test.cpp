...
namespace {
struct this_test_log_formatter
  ...
  void test_unit_finish(std::ostream& output, test_unit const& tu,
                        unsigned long) {
...
char const* log_level_name[] = {
    "log_successful_tests", "log_test_suites",  "log_messages",
    "log_warnings",         "log_all_errors",   "log_cpp_exception_errors",
    "log_system_errors",    "log_fatal_errors", "log_nothing"};
...
char const* error_type_name[] = {"no error",         "user message",
                                 "user warning",     "user non-fatal error",
                                 "cpp exception",    " system error",
...
#define PATTERN_FILE_NAME "errors-handling-test.pattern"
  ...
  // for each log level
  for (log_level level = log_successful_tests; level <= log_nothing;
       level = static_cast<log_level>(level + 1)) {
    // for each error type
    for (error_type = et_begin; error_type != et_end;
