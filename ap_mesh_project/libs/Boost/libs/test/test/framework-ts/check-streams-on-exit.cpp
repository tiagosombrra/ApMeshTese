...
int main(int argc, char* argv[]) {
  ...
  utf::output_format outputs_to_tests[] = {
      utf::OF_CLF, utf::OF_XML, utf::OF_JUNIT, utf::OF_CUSTOM_LOGGER};
  const char* const outputs_to_test_names[] = {"CLF", "XML", "JUNIT", "CUSTOM"};

  for (int i = 0; i < sizeof(outputs_to_tests) / sizeof(outputs_to_tests[0]);
       i++) {
    std::ostream* stream = utf::unit_test_log.get_stream(outputs_to_tests[i]);
    if (stream && stream != &std::cout) {
      std::cerr << "logger stream not released properly for format '"
                << outputs_to_test_names[i] << "'" << std::endl;
      return 1;
    }
  }
  return 0;
