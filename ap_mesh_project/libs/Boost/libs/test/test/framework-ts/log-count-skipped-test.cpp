...
struct skip_with_message {
  ...
  std::string message;
  skip_with_message(std::string m) : message(m) {}
...
void test_1() { BOOST_CHECK_EQUAL(2 + 2, 4); }

void test_2() { BOOST_CHECK_EQUAL(0, 1); }

void test_3() { BOOST_CHECK_EQUAL(0, 1); }
...
void check(output_test_stream& output, output_format log_format,
...

  {
    ...
    framework::get<test_suite>(id).p_default_status.value =
...
#define PATTERN_FILE_NAME "log-count-skipped-tests.pattern"
  ...
  // add decorators to the tests, should happen only once. The status will be
  // reset in the check.
  decorator::collector_t* decorator_collector =
