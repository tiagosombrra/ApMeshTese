...
void timeout_foo() {
  ...
  throw execution_exception(execution_exception::timeout_error, "fake  timeout",
                            dummy);
}

void context_mixed_foo() {
  BOOST_TEST_CONTEXT("some context") { BOOST_TEST(true); }
...
void check(output_test_stream& output, output_format log_format,
           test_unit_id id, log_level ll = log_successful_tests,
           output_format additional_format = OF_INVALID) {
  {
    log_setup_teardown holder(output, log_format, ll, invalid_log_level,
                              additional_format);
    output << "* " << log_format
           << "-format  "
              "****************************************************************"
...
struct save_arguments {
  ...
  static const std::vector<std::string>& saved_argv() {
    if (argv.empty()) {
      for (size_t i = 0;
           i < static_cast<size_t>(framework::master_test_suite().argc); i++) {
  ...
  static std::string get_pattern_file(const std::string& to_retrieve) {
    ...
    for (size_t i = 0;
         i < static_cast<size_t>(framework::master_test_suite().argc); i++) {
      if (argv[i].find(to_retrieve) != std::string::npos) return argv[i];
    }
...
#define PATTERN_FILE_NAME "log-formatter-test.pattern"
  ...
  std::string pattern_file_name(
      // we cannot use runtime_config::save_arguments() as one of the test is
      // mutating the arguments for testing purposes we have to inspect argv as
      // b2 may run the program from an unknown location
  ...
  output_test_stream_for_loggers test_output(
      pattern_file_name, !save_arguments::save_pattern(), true, __FILE__);
...
#line 157
  ...
  ts_3->add(tc1);
  test_case* tc2 = BOOST_TEST_CASE_NAME(
      bad_foo2, "bad_foo2<int>");  // this is for skipped message GH-253
  ts_3->add(tc2);
  tc2->depends_on(tc1);
...
#line 249
  ...
  test_suite* ts_account_failures =
