...
class output_test_stream_for_loggers : public output_test_stream {
 ...
 public:
  explicit output_test_stream_for_loggers(
      boost::unit_test::const_string pattern_file_name =
          boost::unit_test::const_string(),
      bool match_or_save = true, bool text_or_binary = true,
  ...
  static std::string normalize_path(const std::string& str) {
    ...
    return utils::replace_all_occurrences_of(
        str, to_look_for,
        to_look_for + sizeof(to_look_for) / sizeof(to_look_for[0]), to_replace,
  ...
  virtual std::string get_stream_string_representation() const {
    std::string current_string =
        output_test_stream::get_stream_string_representation();

    std::string pathname_fixes;
    {
      const std::string to_look_for[] = {source_filename,
                                         normalize_path(source_filename)};
      const std::string to_replace[] = {"xxx/" + basename, "xxx/" + basename};
      pathname_fixes = utils::replace_all_occurrences_of(
          current_string, to_look_for,
    ...
    std::string other_vars_fixes;
    {
      ...
      s_version << BOOST_VERSION / 100000 << "." << BOOST_VERSION / 100 % 1000
                << "." << BOOST_VERSION % 100;
      ...
      const std::string to_look_for[] = {
          ...
          basename + "(*):",
          "unknown location(*):",
          "; testing time: *us\n",  // removing this is far more easier than
                                    // adding a testing time
          "; testing time: *ms\n",
...
struct log_setup_teardown {
  log_setup_teardown(output_test_stream& output, output_format log_format,
                     log_level ll,
