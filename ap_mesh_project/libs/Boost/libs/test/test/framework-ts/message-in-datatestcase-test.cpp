...
BOOST_DATA_TEST_CASE(test_update, boost::unit_test::data::make(filenames)) {
...
void check_pattern_loggers(output_test_stream& output, output_format log_format,
...
void check_pattern_loggers(output_test_stream& output, test_suite* ts,
  ...
  check_pattern_loggers(output, OF_JUNIT, ts->p_id, bt_module_failed,
                        log_successful_tests);
  check_pattern_loggers(
      output, OF_JUNIT, ts->p_id, bt_module_failed,
...
#define PATTERN_FILE_NAME "messages-in-datasets-test.pattern"
  ...
  test_unit_generator const& generator =
      boost::unit_test::data::ds_detail::test_case_gen<test_updatecase,
                                                       decltype(dataset)>(
          "fake_name", __FILE__, 200, std::forward<decltype(dataset)>(dataset));
