...
struct log_guard {
  ~log_guard() { unit_test_log.set_stream(std::cout); }
};
...
void check(output_test_stream& output, output_format report_format,
           test_unit_id id) {
...
struct guard {
  ~guard() {
    ...
    results_reporter::set_format(
        runtime_config::get<output_format>(runtime_config::btrt_report_format));
    results_reporter::set_level(NO_REPORT);
  }
};
...
#define PATTERN_FILE_NAME "result_report_test.pattern"
  ...
  output_test_stream test_output(pattern_file_name,
                                 !runtime_config::save_pattern());
...
void check2(output_test_stream& output, output_format report_format,
            test_unit_id id) {
  results_reporter::set_format(report_format);
...
#define PATTERN_FILE_NAME_DEFAULT_BEHAVIOUR \
  "result_report_test.pattern.default_behaviour"

  std::string pattern_file_name(
      framework::master_test_suite().argc <= 2
          ? (runtime_config::save_pattern()
                 ? PATTERN_FILE_NAME_DEFAULT_BEHAVIOUR
                 : "./baseline-outputs/" PATTERN_FILE_NAME_DEFAULT_BEHAVIOUR)
