...
void check_global_fixture(output_test_stream& output, output_format log_format,
                          test_unit_id id, bool bt_module_failed = false,
...

  {
    ...
    boost::unit_test::ut_detail::global_fixture_impl<global_fixture_t>
        fixture_stack_element;
    framework::finalize_setup_phase(id);

    try {
      framework::run(id, false);  // do not continue the test tree to have the
                                  // test_log_start/end
  ...
  BOOST_TEST(bt_module_failed ==
             ((results_collector.results(id).result_code() != 0) ||
              setup_error_caught));
  BOOST_TEST(output.match_pattern(
      true));  // flushes the stream at the end of the comparison.
}

template <class global_fixture_t>
void check_global_fixture(output_test_stream& output, test_suite* ts,
                          bool bt_module_failed = false,
                          bool has_setup_error = false) {
  ts->p_default_status.value = test_unit::RS_ENABLED;

  check_global_fixture<global_fixture_t>(output, OF_CLF, ts->p_id,
                                         bt_module_failed, has_setup_error);
  check_global_fixture<global_fixture_t>(output, OF_XML, ts->p_id,
                                         bt_module_failed, has_setup_error);
  check_global_fixture<global_fixture_t>(output, OF_JUNIT, ts->p_id,
                                         bt_module_failed, has_setup_error,
                                         log_successful_tests);
  check_global_fixture<global_fixture_t>(
      output, OF_JUNIT, ts->p_id, bt_module_failed, has_setup_error,
      log_cpp_exception_errors);  // should branch to the log log_all_errors
}

struct guard {
  ~guard() {
    boost::unit_test::unit_test_log.set_format(
        runtime_config::get<output_format>(runtime_config::btrt_log_format));
...
void very_bad_foo() {
  BOOST_TEST_CONTEXT("some context") { BOOST_FAIL("very_bad_foo is fatal"); }
}
...
#line 275
  ...
  test_suite* ts_main_no_error =
      BOOST_TEST_SUITE("Fake Test Suite Hierarchy no errors");
...
#define PATTERN_FILE_NAME "global-fixtures-test.pattern"
  ...
  output_test_stream_for_loggers test_output(
      pattern_file_name, !runtime_config::save_pattern(), true, __FILE__);
  ...
  test_output << "*********************** GlobalFixtureWithCtor<&good_foo>"
              << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithCtor<&good_foo> >(test_output, ts_main,
                                                          true);
  check_global_fixture<GlobalFixtureWithCtor<&good_foo> >(
      test_output, ts_main_no_error, false);
  ...
  test_output
      ...
      << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithCtor<&almost_good_foo> >(test_output,
                                                                 ts_main, true);
  check_global_fixture<GlobalFixtureWithCtor<&almost_good_foo> >(
      test_output, ts_main_no_error, false);

  test_output << "***********************" << std::endl;
  test_output << "*********************** GlobalFixtureWithCtor<&bad_foo>"
              << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithCtor<&bad_foo> >(
      test_output, ts_main, true);  // should fail the module
  check_global_fixture<GlobalFixtureWithCtor<&bad_foo> >(
      test_output, ts_main_no_error, true);

  test_output << "***********************" << std::endl;
  test_output << "*********************** GlobalFixtureWithCtor<&very_bad_foo>"
              << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithCtor<&very_bad_foo> >(
      test_output, ts_main, true);  // should fail the module
  check_global_fixture<GlobalFixtureWithCtor<&very_bad_foo> >(
      test_output, ts_main_no_error, true);
  ...
  test_output
      ...
      << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithCtor<&very_bad_exception> >(
      test_output, ts_main, true);  // should fail the module
  check_global_fixture<GlobalFixtureWithCtor<&very_bad_exception> >(
      test_output, ts_main_no_error, true);
  ...
  // setup does not fail
  test_output << "***********************" << std::endl;
  test_output << "*********************** GlobalFixtureWithSetup<&good_foo>"
              << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithSetup<&good_foo> >(test_output, ts_main,
                                                           true);
  check_global_fixture<GlobalFixtureWithSetup<&good_foo> >(
      test_output, ts_main_no_error, false);
  ...
  test_output
      ...
      << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithSetup<&almost_good_foo> >(
      test_output, ts_main, true);
  check_global_fixture<GlobalFixtureWithSetup<&almost_good_foo> >(
      test_output, ts_main_no_error, false);

  // setup fails
  test_output << "***********************" << std::endl;
  test_output << "*********************** GlobalFixtureWithSetup<&bad_foo>"
              << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithSetup<&bad_foo> >(test_output, ts_main,
                                                          true);
  check_global_fixture<GlobalFixtureWithSetup<&bad_foo> >(
      test_output, ts_main_no_error, true);

  // setup fails badly
  test_output << "***********************" << std::endl;
  test_output << "*********************** GlobalFixtureWithSetup<&very_bad_foo>"
              << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithSetup<&very_bad_foo> >(test_output,
                                                               ts_main, true);
  check_global_fixture<GlobalFixtureWithSetup<&very_bad_foo> >(
      test_output, ts_main_no_error, true);
  ...
  test_output
      ...
      << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithSetup<&very_bad_exception> >(
      test_output, ts_main, true);
  check_global_fixture<GlobalFixtureWithSetup<&very_bad_exception> >(
      test_output, ts_main_no_error, true);
  ...
  test_output << "***********************" << std::endl;
  test_output << "*********************** GlobalFixtureWithTeardown<&good_foo>"
              << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithTeardown<&good_foo> >(test_output,
                                                              ts_main, true);
  check_global_fixture<GlobalFixtureWithTeardown<&good_foo> >(
      test_output, ts_main_no_error, false);
  ...
  test_output
      ...
      << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithTeardown<&almost_good_foo> >(
      test_output, ts_main, true);
  check_global_fixture<GlobalFixtureWithTeardown<&almost_good_foo> >(
      test_output, ts_main_no_error, false);
  ...
  // teardown fails
  test_output << "***********************" << std::endl;
  test_output << "*********************** GlobalFixtureWithTeardown<&bad_foo>"
              << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithTeardown<&bad_foo> >(test_output,
                                                             ts_main, true);
  check_global_fixture<GlobalFixtureWithTeardown<&bad_foo> >(
      test_output, ts_main_no_error, true);
  ...
  test_output
      ...
      << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithTeardown<&very_bad_foo> >(
      test_output, ts_main, true);
  check_global_fixture<GlobalFixtureWithTeardown<&very_bad_foo> >(
      test_output, ts_main_no_error, true);
  ...
  test_output << "*********************** "
              ...
              << std::endl;
  test_output << "***********************" << std::endl;
  check_global_fixture<GlobalFixtureWithTeardown<&very_bad_exception> >(
      test_output, ts_main, true);
  check_global_fixture<GlobalFixtureWithTeardown<&very_bad_exception> >(
      test_output, ts_main_no_error, true);
}
