...
//
/// @file
/// @brief tests the access to the master test suite from the datasets, trac
/// 12953
// ***************************************************************************
...
BOOST_DATA_TEST_CASE(master_access_zips,
                     boost::unit_test::data::make_delayed<dataset_loader>() ^
...
BOOST_DATA_TEST_CASE(master_access_grid,
                     ...
                     input, expect) {
  BOOST_TEST(
      (input == expect || input == "--param1=1" || expect == "--param1=1"));
}
...
BOOST_DATA_TEST_CASE(master_access_joins,
                     boost::unit_test::data::make_delayed<dataset_loader>() +
                         dataset_loader({"--param1=1", "--param2=2"}),
                     input) {
  static const std::string values[] = {"--param1=1", "--param2=2", "--param1=1",
                                       "--param2=2"};
...
BOOST_DATA_TEST_CASE(master_access_joins_flip,
                     dataset_loader({"--param2=2", "--param1=1"}) +
                         boost::unit_test::data::make_delayed<dataset_loader>(),
                     input) {
  static const std::string values[] = {"--param2=2", "--param1=1", "--param1=1",
                                       "--param2=2"};
...
class dataset_loader_arity3 {
 ...
 public:
  ...
  struct iterator {
    iterator(data_type::const_iterator v_expected,
             data_type::const_iterator v_input, std::string additional)
        : m_input(v_input), m_expected(v_expected), m_additional(additional) {}
  ...
  boost::unit_test::data::size_t size() const {
    ...
    BOOST_TEST_I_ASSRT(m_expected.size() == m_input.size(),
                       "Lists of strings of different size");
    return m_input.size();
  }
...
BOOST_DATA_TEST_CASE(
    master_access_make_ds_with_arity,
