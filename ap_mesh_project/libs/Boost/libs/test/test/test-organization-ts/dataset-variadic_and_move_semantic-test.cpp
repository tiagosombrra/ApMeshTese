...
class non_copyable_type {
 public:
  ...
  ~non_copyable_type() { nb_destructs++; }
  ...
  explicit non_copyable_type(const int value) { value_ = value; }

  friend std::ostream& operator<<(std::ostream& ost,
                                  const non_copyable_type& rhs) {
...
BOOST_DATA_TEST_CASE(test2,
                     fibonacci_dataset<non_copyable_type>() ^
                         bdata::make({1, 2, 3, 5, 8, 13, 21, 34, 55}),
                     fib_sample, exp) {
