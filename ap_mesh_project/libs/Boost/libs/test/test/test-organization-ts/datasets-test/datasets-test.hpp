...
class copy_count {
 public:
  ...
  copy_count(copy_count&&) BOOST_NOEXCEPT_OR_NOTHROW {
  }  // noexcept is important indeed
...
struct check_arg_type {
  ...
  void operator()(S const&) const {
    BOOST_CHECK_MESSAGE((boost::is_same<S, T>::value),
                        "Sample type does not match expected");
  }
};
...
struct check_arg_type<std::tuple<T1, T2>> {
  ...
  void operator()(S1 const&, S2 const&) const {
    BOOST_CHECK_MESSAGE((boost::is_same<S1, T1>::value),
                        "S1 type does not match expected");
    BOOST_CHECK_MESSAGE((boost::is_same<S2, T2>::value),
                        "S2 type does not match expected");
  }
};
...
struct check_arg_type<std::tuple<T1, T2, T3>> {
  ...
  void operator()(S1 const&, S2 const&, S3 const&) const {
    ...
    BOOST_CHECK_MESSAGE((boost::is_same<S2, T2>::value),
                        "S2 type does not match expected");
    BOOST_CHECK_MESSAGE((boost::is_same<S3, T3>::value),
                        "S3 type does not match expected");
  }
};
...
struct check_arg_type_like {
  template <typename S>
  void operator()(S const&) const {
    BOOST_CHECK_MESSAGE((boost::is_convertible<S, T>::value),
                        "Sample type does not match expected");
  }
};
...
struct check_arg_type_like<std::tuple<T1, T2>> {
  template <typename S1, typename S2>
  void operator()(S1 const&, S2 const&) const {
    BOOST_CHECK_MESSAGE((boost::is_convertible<S1, T1>::value),
                        "S1 type does not match expected");
    BOOST_CHECK_MESSAGE((boost::is_convertible<S2, T2>::value),
                        "S2 type does not match expected");
  }
};
...
struct check_arg_type_like<std::tuple<T1, T2, T3>> {
  ...
  void operator()(S1 const&, S2 const&, S3 const&) const {
    ...
    BOOST_CHECK_MESSAGE((boost::is_convertible<S2, T2>::value),
                        "S2 type does not match expected");
    BOOST_CHECK_MESSAGE((boost::is_convertible<S3, T3>::value),
                        "S3 type does not match expected");
  }
};
...
struct print_sample {
  ...
  void operator()(T const& sample) const {
    std::cout << "S has type: " << typeid(T).name() << " and value " << sample
              << std::endl;
  }
};
...
inline std::list<copy_count> const make_copy_count_const_collection() {
