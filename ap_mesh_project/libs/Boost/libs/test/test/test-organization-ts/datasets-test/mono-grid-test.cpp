...
BOOST_AUTO_TEST_CASE(test_mono_grid_size_and_composition) {
  ...
  BOOST_TEST((data::make(std::vector<int>(2)) * data::make(std::list<float>(2)))
                 .size() == 4);
  BOOST_TEST((data::make(std::vector<int>(2)) * data::make(5.)).size() == 2);
  BOOST_TEST((data::make(std::vector<int>(3)) * data::make(std::list<int>(1)))
                 .size() == 3);

  BOOST_TEST((data::make(std::vector<int>(3)) *
              data::make(std::list<std::string>(3)) * data::make(5))
                 .size() == 9);
  BOOST_TEST((data::make(std::vector<int>(1)) * data::make(std::list<int>(3)) *
              data::make(5))
                 .size() == 3);
}
...
BOOST_AUTO_TEST_CASE(test_mono_grid_cpp11_features) {
  ...
  data::for_each_sample(samples1,
                        check_arg_type_like<std::tuple<int, char const*>>());
  ...
  data::for_each_sample(
      samples2, check_arg_type_like<std::tuple<double, char const*, int>>());

  int* exp3 = arr3;
  c = 0;

  data::for_each_sample(samples2,
                        [&c, &vec1, exp3](double a1, char const* a2, int a3) {
...
BOOST_AUTO_TEST_CASE(test_mono_grid_number_of_copies) {
  ...
  data::for_each_sample(data::make(copy_count()) * data::make(copy_count()),
                        check_arg_type<std::tuple<copy_count, copy_count>>());
  BOOST_TEST(copy_count::value() == 0);

  copy_count::value() = 0;
  data::for_each_sample(
      data::make(copy_count()) * data::make(copy_count()) *
          data::make(copy_count()),
...
BOOST_AUTO_TEST_CASE(test_mono_grid_number_of_copies_auto) {
  ...
  copy_count::value() = 0;
  data::for_each_sample(ds1 * ds1,
                        check_arg_type<std::tuple<copy_count, copy_count>>());
  BOOST_TEST(copy_count::value() == 0);

  copy_count::value() = 0;
  data::for_each_sample(ds2 * ds2,
                        check_arg_type<std::tuple<copy_count, copy_count>>());
  BOOST_TEST(copy_count::value() == 0);

  copy_count::value() = 0;
  data::for_each_sample(ds1 * ds2,
                        check_arg_type<std::tuple<copy_count, copy_count>>());
  BOOST_TEST(copy_count::value() == 0);
  ...
  data::for_each_sample(zp1,
                        check_arg_type<std::tuple<copy_count, copy_count>>());
  BOOST_TEST(copy_count::value() == 0);

  copy_count::value() = 0;
  data::for_each_sample(data::make(copy_count()) * ds1,
                        check_arg_type<std::tuple<copy_count, copy_count>>());
  BOOST_TEST(copy_count::value() == 0);
  ...
  {
    std::vector<copy_count> v(
        2);  // we cannot do better than std::vector constructor
    std_vector_constructor_count = copy_count::value() / 2;
  }

  copy_count::value() = 0;
  auto ds3 = data::make(make_copy_count_collection()) *
             data::make(make_copy_count_collection());
  BOOST_TEST(ds3.size() == 9);
  data::for_each_sample(ds3,
                        check_arg_type<std::tuple<copy_count, copy_count>>());
...
BOOST_AUTO_TEST_CASE(test_mono_grid_variadic_dimension) {
  ...
  BOOST_TEST(
      (data::make(1) * data::make(5) * data::make(1) * data::make(1)).size() ==
      1);

  BOOST_TEST(
      (data::xrange(2) * data::xrange(2) * data::xrange(2) * data::xrange(2))
          .size() == (1 << 4));
}
