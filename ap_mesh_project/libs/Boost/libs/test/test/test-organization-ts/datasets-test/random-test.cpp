...
BOOST_AUTO_TEST_CASE(test_parameterized_init) {
  ...
  BOOST_TEST((
      std::is_same<DS3::generator_type::engine_type, std::minstd_rand>::value));
  ...
  data::for_each_sample(
      ds4, [&vals](double s) { vals.push_back(s); }, 10);
  data::for_each_sample(
      ds5,
      [&vals](double s) {
  ...
  BOOST_TEST((
      std::is_same<DS6::generator_type::engine_type, std::minstd_rand>::value));
}
