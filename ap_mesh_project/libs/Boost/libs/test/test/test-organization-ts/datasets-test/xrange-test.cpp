...
BOOST_AUTO_TEST_CASE(test_single_range) {
  ...
  BOOST_TEST(data::xrange<double>((data::step = 0.5, data::end = 3)).size() ==
             6);

  int c = 0;
  data::for_each_sample(data::xrange(3), [&c](int a) { BOOST_TEST(a == c++); });
...
BOOST_AUTO_TEST_CASE(test_range_join) {
  ...
  data::for_each_sample(ds, [&c, exp](int a) { BOOST_TEST(a == exp[c++]); });
}
