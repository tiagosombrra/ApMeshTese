...
BOOST_AUTO_TEST_CASE(
    tc1) /* on the same line, otherwise some test below should be updated */
#endif
{
  BOOST_ERROR("");
}
...
BOOST_TEST_DECORATOR(*boost::unit_test::expected_failures(1))
BOOST_AUTO_TEST_CASE(
    tc1) /* on the same line, otherwise some test below should be updated (the
            test is sensitive to line numbers) */
#endif
{
  BOOST_ERROR("");
}
...
BOOST_AUTO_TEST_CASE(manual_test_case_creation_test) {
  ...
  BOOST_CHECK_THROW(framework::get(tc1->p_id, TUT_SUITE),
...
BOOST_AUTO_TEST_CASE(manual_test_suite_creation) {
  ...
  if (pos != const_string::npos) fn.trim_left(pos + 1);
  pos = fn.rfind("\\");
  if (pos != const_string::npos) fn.trim_left(pos + 1);
...
BOOST_AUTO_TEST_CASE(automated_test_units_registration) {
  ...
  BOOST_TEST(
      framework::get<test_case>(mts.get("automated_test_units_registration"))
          .p_expected_failures == 0U);
