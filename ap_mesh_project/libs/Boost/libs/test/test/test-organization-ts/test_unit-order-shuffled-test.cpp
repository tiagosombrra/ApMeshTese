...
#define TC(name)                                                         \
  boost::unit_test::make_test_case(boost::function<void()>(some_test),   \
                                   BOOST_TEST_STRINGIZE(name), __FILE__, \
                                   __LINE__)
...
static tu_order_collector run_tree(ut::test_suite* master) {
...
struct test_tree {
  test_tree() {
    ...
    for (std::size_t s = 0; s < nb_ts; s++) {
      tsuites.push_back(
          BOOST_TEST_SUITE("ts1_" + boost::unit_test::utils::string_cast(s)));
    ...
    for (std::size_t s = 0; s < nb_ts2; s++) {
      tsuites.push_back(
          BOOST_TEST_SUITE("ts2_" + boost::unit_test::utils::string_cast(s)));
      tsuites[std::rand() % nb_ts]->add(
          tsuites.back());  // picking a random one in the first level
    }
    ...
    for (std::size_t s = 0; s < 10; s++) {
      ut::test_case* tc = boost::unit_test::make_test_case(
          ...
          "tc_" + boost::unit_test::utils::string_cast(s), __FILE__, __LINE__);
...
BOOST_FIXTURE_TEST_CASE(test_no_seed, test_tree) {
  // no seed set
  ut::runtime_config::s_arguments_store.set<unsigned int>(
      ut::runtime_config::btrt_random_seed, 0);
...
BOOST_FIXTURE_TEST_CASE(test_seed_to_time, test_tree) {
  ...
  ut::runtime_config::s_arguments_store.set<unsigned int>(
      ut::runtime_config::btrt_random_seed, 1);

  tu_order_collector res1 = run_tree(master);
  tu_order_collector res2 = run_tree(master);

  BOOST_TEST(res1.m_order != res2.m_order);  // some elements might be the same,
                                             // but not the full sequences
}
...
BOOST_FIXTURE_TEST_CASE(test_seed_identical, test_tree) {
  ...
  ut::runtime_config::s_arguments_store.set<unsigned int>(
      ut::runtime_config::btrt_random_seed, seed);
  tu_order_collector res1 = run_tree(master);
  ...
  BOOST_TEST(res1.m_order != res3.m_order);  // some elements might be the same,
                                             // but not the full sequences
}
