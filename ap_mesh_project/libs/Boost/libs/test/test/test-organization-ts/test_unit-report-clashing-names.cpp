...
BOOST_AUTO_TEST_CASE(test_clashing_cases_template_test_case) {
  ...
  BOOST_CHECK_NO_THROW(
      t_suite2->add(BOOST_TEST_CASE_TEMPLATE(template_test_case, test_types1)));
  BOOST_CHECK_NO_THROW(framework::finalize_setup_phase(master_ts->p_id));

  BOOST_CHECK_NO_THROW(
      t_suite1->add(BOOST_TEST_CASE_TEMPLATE(template_test_case, test_types2)));
  BOOST_CHECK_THROW(framework::finalize_setup_phase(master_ts->p_id),
                    boost::unit_test::framework::setup_error);
}

void test2(int value) { BOOST_TEST(value >= 0); }
...
BOOST_AUTO_TEST_CASE(test_clashing_cases_with_name) {
  ...
  for (int i = 0; i < 10; i++) {
    t_suite1->add(BOOST_TEST_CASE_NAME(
        boost::bind(test2, i),
        "test-X-" + boost::unit_test::utils::string_cast(i)));
  }
}
