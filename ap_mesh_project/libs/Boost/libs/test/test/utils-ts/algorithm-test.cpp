...
bool predicate(char c1, char c2) {
...
BOOST_AUTO_TEST_CASE(test_mismatch) {
  ...
  BOOST_TEST(
      TEST_SURROUND_EXPRESSION(
          utu::mismatch(cs1.begin(), cs1.end(), cs2.begin(), cs2.end()).first -
          cs1.begin()) == 8);
  ...
  cs2 = "TeSt_liNk";
  BOOST_TEST(TEST_SURROUND_EXPRESSION(utu::mismatch(cs1.begin(), cs1.end(),
                                                    cs2.begin(), cs2.end(),
                                                    REF_FUN(predicate))
                                          .first -
                                      cs1.begin()) == 5);
...
BOOST_AUTO_TEST_CASE(test_find_first_not_of) {
  ...
  BOOST_TEST(TEST_SURROUND_EXPRESSION(
                 utu::find_first_not_of(cs.begin(), cs.end(), another.begin(),
                                        another.end()) -
                 cs.begin()) == 4);
  ...
  BOOST_TEST(TEST_SURROUND_EXPRESSION(
                 utu::find_first_not_of(cs.begin(), cs.end(), another.begin(),
                                        another.end(), REF_FUN(predicate)) -
                 cs.begin()) == 7);

  another = "tes_ring";
  BOOST_TEST(utu::find_last_not_of(cs.begin(), cs.end(), another.begin(),
                                   another.end()) == cs.end());
}
...
BOOST_AUTO_TEST_CASE(test_find_last_of) {
  ...
  BOOST_TEST(TEST_SURROUND_EXPRESSION(utu::find_last_of(cs.begin(), cs.end(),
                                                        another.begin(),
                                                        another.end()) -
                                      cs.begin()) == 6);
  ...
  another = "_Se";
  BOOST_TEST(TEST_SURROUND_EXPRESSION(
                 utu::find_last_of(cs.begin(), cs.end(), another.begin(),
                                   another.end(), REF_FUN(predicate)) -
                 cs.begin()) == 5);

  another = "qw";
  BOOST_TEST(utu::find_last_of(cs.begin(), cs.end(), another.begin(),
                               another.end()) == cs.end());
...
BOOST_AUTO_TEST_CASE(test_find_last_not_of) {
  ...
  BOOST_TEST(TEST_SURROUND_EXPRESSION(
                 utu::find_last_not_of(cs.begin(), cs.end(), another.begin(),
                                       another.end()) -
                 cs.begin()) == 4);
...
BOOST_AUTO_TEST_CASE(test_replace_all) {
  {
    std::string cs(
        "cstring: cstring is a const string that can be transformed to an "
        "std::string");
    ...
    BOOST_TEST(utu::replace_all_occurrences_of(
                   cs, to_look_for,
                   to_look_for + sizeof(to_look_for) / sizeof(to_look_for[0]),
                   to_replace,
                   to_replace + sizeof(to_replace) / sizeof(to_replace[0])) ==
               "constant_string: constant_string is a constant string that can "
               "be transformed to an std::string");
  }
...
BOOST_AUTO_TEST_CASE(test_replace_with_wildcards) {
  {
    std::string cs(
        "this string contains x='374'u elements of size y='24'u and y='75'z");

    const std::string to_look_for[] = {"x='*'u", "y='*'u"};
    const std::string to_replace[] = {"x='27'q", "k='*0'p"};

    BOOST_TEST(
        utu::replace_all_occurrences_with_wildcards(
            cs, to_look_for,
            to_look_for + sizeof(to_look_for) / sizeof(to_look_for[0]),
            to_replace,
            to_replace + sizeof(to_replace) / sizeof(to_replace[0])) ==
