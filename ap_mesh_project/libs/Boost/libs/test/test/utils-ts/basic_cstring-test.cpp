...
typedef mpl::transform<mutable_char_types, boost::add_const<mpl::_1> >::type
    const_char_types;
...
template <typename CharT>
CharT* static_literal(char const* orig, std::size_t orig_size) {
...
#define LOCAL_DEF(name, s)                                      \
  ...
  utf::basic_cstring<CharT> name(                               \
      (CharT*)(BOOST_JOIN(sl, __LINE__).begin())) /**/
...
BOOST_TEST_CASE_TEMPLATE_FUNCTION(constructors_test, CharT) {
...

  {
    ...
    BOOST_TEST(traits::compare(bcs.begin(), LITERAL("test_s"), bcs.size()) ==
               0);
  }
}
...
void array_construction_test() {
  ...
  BOOST_TEST(const_string::traits_type::compare(bcs_array[0].begin(), "str1",
                                                bcs_array[0].size()) == 0);
  BOOST_TEST(const_string::traits_type::compare(bcs_array[1].begin(), "str2",
                                                bcs_array[1].size()) == 0);

  const_string bcs("abc");
  BOOST_TEST(
      const_string::traits_type::compare(bcs.begin(), "abc", bcs.size()) == 0);
}
...
BOOST_TEST_CASE_TEMPLATE_FUNCTION(data_access_test, CharT) {
  ...
  BOOST_TEST(traits_type::compare(bcs1.begin(), bcs1.begin(), bcs1.size()) ==
             0);
...
BOOST_TEST_CASE_TEMPLATE_FUNCTION(asignment_test, CharT) {
  ...
  bcs1 = l.begin();
  BOOST_TEST(traits_type::compare(bcs1.begin(), LITERAL("test"), bcs1.size()) ==
             0);
  ...
  BOOST_TEST(traits_type::compare(bcs1.begin(), LITERAL("est"), bcs1.size()) ==
             0);

  bcs1.assign(bcs2, 4, 3);
  BOOST_TEST(traits_type::compare(bcs1.begin(), LITERAL("_st"), bcs1.size()) ==
             0);

  bcs1.swap(bcs2);
  BOOST_TEST(traits_type::compare(bcs1.begin(), TEST_STRING, bcs1.size()) == 0);
  BOOST_TEST(traits_type::compare(bcs2.begin(), LITERAL("_st"), bcs2.size()) ==
             0);
}
...
BOOST_TEST_CASE_TEMPLATE_FUNCTION(asignment_std_string_test, CharT) {
  ...
  bcs1.assign(l, 1, 2);
  BOOST_TEST(traits_type::compare(bcs1.begin(), LITERAL("es"), bcs1.size()) ==
             0);
}
...
utf::test_suite* init_unit_test_suite(int /*argc*/, char* /*argv*/[]) {
  ...
  test->add(BOOST_TEST_CASE_TEMPLATE(constructors_std_string_test,
                                     base_const_char_types));
  test->add(BOOST_TEST_CASE_TEMPLATE(asignment_std_string_test,
                                     base_const_char_types));
  test->add(BOOST_TEST_CASE_TEMPLATE(comparison_std_string_test,
                                     base_const_char_types));
