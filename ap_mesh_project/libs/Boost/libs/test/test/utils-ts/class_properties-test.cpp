...
class C {
 public:
  static void init() { p_b_ptr.value = new B; }
};
...
class D {
 public:
  static void init() { p_a_ptr.value = new A; }
};
...
BOOST_AUTO_TEST_CASE(test_readonly_property) {
  ...
  // complex expressions (on the left) not understood by boost.test v3 in C++03
  // mode
  ...
  if (p_a_ptr) delete p_a_ptr.get();

  if (p_b_ptr) delete p_b_ptr.get();
}
