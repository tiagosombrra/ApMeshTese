...
// the unit tests below assert functionality for non copyable classes, C++11
// only
...
BOOST_AUTO_TEST_CASE(test_noncopyable_arg) { dotest0((k1 = NC{9})); }
...
void dotest0(Params const&, KW const&) {
  typedef boost::is_same<typename nfp::param_type<Params, KW, D>::type, T>
      check;
