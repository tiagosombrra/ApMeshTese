...
BOOST_AUTO_TEST_CASE(test_current_token) {
  char const* argv1[] = {"test.exe",  "abc", "zxcvb", "as kl",
                         "--ooo=111", "a",   ""};
  rt::cla::argv_traverser tr(sizeof(argv1) / sizeof(char const*), argv1);
...
BOOST_AUTO_TEST_CASE(test_param_construction) {
  ...
  rt::parameter<int, rt::REQUIRED_PARAM> p2(
      "P2", (rt::description = "123", rt::env_var = "E2"));
  ...
  rt::parameter<int, rt::REPEATABLE_PARAM> p4(
      "P4", (rt::description = "123", rt::env_var = "E4"));
  ...
  rt::option p5("P5", (rt::description = "bool arg", rt::env_var = "E5"));
  p5.add_cla_id("-", "b", " ");
  ...
  rt::option p6("P6", (rt::description = "option with true default",
                       rt::env_var = "E6", rt::default_value = true));
  p6.add_cla_id("-", "b", " ");
  ...
  BOOST_CHECK_THROW(
      (rt::parameter<int, rt::REQUIRED_PARAM>("P", rt::default_value = 1)),
      rt::invalid_param_spec);
  BOOST_CHECK_THROW((rt::parameter<int, rt::REPEATABLE_PARAM>(
                        "P", rt::default_value = std::vector<int>{1})),
                    rt::invalid_param_spec);
  BOOST_CHECK_THROW((rt::parameter<int, rt::REPEATABLE_PARAM>(
                        "P", rt::optional_value = std::vector<int>{1})),
                    rt::invalid_param_spec);

  enum EnumType { V1, V2 };
  rt::enum_parameter<EnumType> p8(
      "P8", (rt::enum_values<EnumType>::value = {{"V1", V1}, {"V2", V2}},
             rt::default_value = V1));
...
BOOST_AUTO_TEST_CASE(test_basic_parsing) {
  ...
  testparser.parse(sizeof(argv1) / sizeof(char const*), (char**)argv1,
                   args_store1);
  ...
  testparser.parse(sizeof(argv2) / sizeof(char const*), (char**)argv2,
                   args_store2);
  ...
  testparser.parse(sizeof(argv3) / sizeof(char const*), (char**)argv3,
                   args_store3);
  ...
  testparser.parse(sizeof(argv4) / sizeof(char const*), (char**)argv4,
                   args_store4);
...
BOOST_AUTO_TEST_CASE(test_typed_argument_parsing) {
  ...
  char const* argv1[] = {
      "test.exe", "--another=some thing", "-p1", "1.2", "-p2", "37",
      "--third=Y"};
  rt::arguments_store args_store1;
...
BOOST_AUTO_TEST_CASE(test_repeatable_parameters) {
  ...
  testparser.parse(sizeof(argv) / sizeof(char const*), (char**)argv,
                   args_store);
...
BOOST_AUTO_TEST_CASE(test_validations) {
...

  {
    ...
    BOOST_CHECK_THROW(testparser.parse(sizeof(argv) / sizeof(char const*),
                                       (char**)argv, args_store),
                      rt::format_error);
  }
  ...
  {
    char const* argv[] = {"test.exe", "param_one=1"};
    BOOST_TEST(testparser.parse(sizeof(argv) / sizeof(char const*),
                                (char**)argv, args_store) == 2);
  }
...
BOOST_AUTO_TEST_CASE(test_unrecognized_param_suggestions) {
...

  {
    ...
    BOOST_CHECK_EXCEPTION(
        testparser.parse(sizeof(argv) / sizeof(char const*), (char**)argv,
                         args_store),
        rt::unrecognized_param, [](rt::unrecognized_param const& ex) -> bool {
...

  {
    ...
    BOOST_CHECK_EXCEPTION(
        testparser.parse(sizeof(argv) / sizeof(char const*), (char**)argv,
                         args_store),
        rt::unrecognized_param, ([](rt::unrecognized_param const& ex) -> bool {
          return ex.m_typo_candidates ==
                 std::vector<rt::cstring>{"param_three", "param_two"};
        }));
  }
...
BOOST_AUTO_TEST_CASE(test_end_of_params) {
...

  {
    ...
    int new_argc = testparser.parse(sizeof(argv) / sizeof(char const*),
                                    (char**)argv, args_store);
...
BOOST_AUTO_TEST_CASE(test_negation_prefix) {
  ...
  BOOST_CHECK_THROW(
      rt::cla::parser testparser(params_store, rt::negation_prefix = "no:"),
      rt::invalid_cla_id);
...
BOOST_AUTO_TEST_CASE(test_enum_parameter) {
  ...
  rt::enum_parameter<EnumType> p1(
      "P1", (rt::enum_values<EnumType>::value = {{"V1", V1}, {"V2", V2}},
             rt::default_value = V3));

  p1.add_cla_id("--", "param_one", "=");
  params_store.add(p1);

  rt::enum_parameter<EnumType, rt::REPEATABLE_PARAM> p2(
      "P2", (rt::enum_values<EnumType>::value = {
                 {"V1", V1}, {"V2", V2}, {"V2alt", V2}, {"V3", V3}}));
  p2.add_cla_id("--", "param_two", " ");
  params_store.add(p2);
  ...
  {
    rt::arguments_store args_store;
    char const* argv[] = {"test.exe", "--param_two", "V2alt", "--param_two",
                          "V1",       "--param_two", "V3"};
    testparser.parse(sizeof(argv) / sizeof(char const*), (char**)argv,
                     args_store);
    rt::finalize_arguments(params_store, args_store);
    BOOST_TEST(args_store.has("P2"));
    BOOST_TEST(args_store.get<std::vector<EnumType>>("P2") ==
               (std::vector<EnumType>{V2, V1, V3}));
  }
...
BOOST_AUTO_TEST_CASE(test_fetch_from_environment) {
...

  {
    ...
    auto env_read = [](rt::cstring var_name) -> std::pair<rt::cstring, bool> {
      if (var_name == "P1VAR") return std::make_pair(rt::cstring("5"), true);

      if (var_name == "P2VAR") return std::make_pair(rt::cstring("Y"), true);
...

  {
    ...
    auto env_read = [](rt::cstring var_name) -> std::pair<rt::cstring, bool> {
      if (var_name == "P2VAR") return std::make_pair(rt::cstring("No"), true);
...

  {
    ...
    auto env_read = [](rt::cstring var_name) -> std::pair<rt::cstring, bool> {
      if (var_name == "P2VAR") return std::make_pair(rt::cstring(), true);
...

  {
    ...
    auto env_read = [](rt::cstring var_name) -> std::pair<rt::cstring, bool> {
      if (var_name == "P1VAR") return std::make_pair(rt::cstring("one"), true);
...

  {
    ...
    auto env_read = [](rt::cstring var_name) -> std::pair<rt::cstring, bool> {
      if (var_name == "P1VAR") return std::make_pair(rt::cstring(), true);
...
BOOST_AUTO_TEST_CASE(test_finalize_arguments) {
...

  {
    ...
    BOOST_CHECK_THROW(rt::finalize_arguments(params_store, args_store),
                      rt::missing_req_arg);
  }
}
