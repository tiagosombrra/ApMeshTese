...
BOOST_AUTO_TEST_CASE(test_default_delim_policy) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(tit, sti_end, res,
                                res + sizeof(res) / sizeof(char const*));
}

//____________________________________________________________________________//

BOOST_AUTO_TEST_CASE(test_wide) {
  utu::wstring_token_iterator tit(
      L"\317\356\367\345\354\363 \341\373 \350 \355\345\362");
  wchar_t const* res[4] = {L"\317\356\367\345\354\363", L"\341\373", L"\350",
                           L"\355\345\362"};
...
BOOST_AUTO_TEST_CASE(test_custom_drop_delim) {
  utu::string_token_iterator tit("My:-:\t: :string, :",
...
BOOST_AUTO_TEST_CASE(test_custom_keep_delim) {
  utu::string_token_iterator tit("abc = \t\t 123, int",
                                 utu::kept_delimeters = "=,");
...
BOOST_AUTO_TEST_CASE(test_keep_empty_tokens) {
  utu::string_token_iterator tit(
      "fld,, 456,a==4=,", (utu::dropped_delimeters = " ,",
                           utu::kept_delimeters = "=", utu::keep_empty_tokens));
  char const* res[] = {"fld", "",  "",  "456", "a", "=",
                       "",    "=", "4", "=",   "",  ""};
...
BOOST_AUTO_TEST_CASE(test_range_token_iterator) {
  typedef utu::range_token_iterator<std::list<char>::iterator>
      my_token_iterator;
...
void foo(Iter b, Iter e) {
  moo(utu::make_range_token_iterator(
      b, e,
      (utu::kept_delimeters = utu::dt_isspace, utu::dropped_delimeters = "2")));
}
...
BOOST_AUTO_TEST_CASE(test_istream_token_iterator) {
  typedef utu::range_token_iterator<std::istream_iterator<char> >
      my_token_iterator;

  std::istream_iterator<char> in_it(std::cin);

  my_token_iterator tit(in_it, std::istream_iterator<char>(),
                        utu::dropped_delimeters = ":");
