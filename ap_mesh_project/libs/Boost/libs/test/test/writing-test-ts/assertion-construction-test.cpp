...
//
//  Description : unit test for new assertion construction based on input
//  expression
// ***************************************************************************
...
struct fwd_iterable_custom {
  typedef std::vector<int>::const_iterator
      custom_iterator;  // named "exotic" on purpose
...
BOOST_AUTO_TEST_CASE(test_forward_iterable_concept) {
  {
    ...
    BOOST_CHECK_MESSAGE(utf::ut_detail::has_member_size<type>::value,
                        "has_member_size failed");
    BOOST_CHECK_MESSAGE(utf::ut_detail::has_member_begin<type>::value,
                        "has_member_begin failed");
    BOOST_CHECK_MESSAGE(utf::ut_detail::has_member_end<type>::value,
                        "has_member_end failed");
    BOOST_CHECK_MESSAGE(utf::is_forward_iterable<type>::value,
                        "is_forward_iterable failed");
    BOOST_CHECK_MESSAGE(utf::is_container_forward_iterable<type>::value,
...

  {
    ...
    BOOST_CHECK_MESSAGE(utf::ut_detail::has_member_size<type2>::value,
                        "has_member_size failed");
    BOOST_CHECK_MESSAGE(utf::ut_detail::has_member_begin<type2>::value,
                        "has_member_begin failed");
    BOOST_CHECK_MESSAGE(utf::ut_detail::has_member_end<type2>::value,
                        "has_member_end failed");
    BOOST_CHECK_MESSAGE(utf::is_forward_iterable<type2>::value,
                        "is_forward_iterable failed");
    BOOST_CHECK_MESSAGE(utf::is_container_forward_iterable<type2>::value,
                        "is_container_forward_iterable failed");
  }
}
...
struct Testee {
  ...
  friend std::ostream& operator<<(std::ostream& ostr, Testee const&) {
    return ostr << "Testee";
  }
...
BOOST_AUTO_TEST_CASE(test_comparison_with_arrays) {
...

  {
    ...
    std::vector<long int> v_long_array(
        c_long_array,
        c_long_array + sizeof(c_long_array) / sizeof(c_long_array[0]));
    std::swap(v_long_array[1], v_long_array[2]);

    predicate_result const& res =
        EXPR_TYPE(c_long_array == v_long_array).evaluate();
    BOOST_TEST(!res);
...

  {
    ...
    predicate_result const& res =
        EXPR_TYPE(v_long_array == c_long_array).evaluate();
    BOOST_TEST(!res);
    BOOST_TEST(res.message() ==
