...
void validate_comparisons(Col const& c1, Col const& c2){
    VALIDATE_OP(==) VALIDATE_OP(!=) VALIDATE_OP(<) VALIDATE_OP(>)
...
// this one does not have const_iterator nor a size, but should be forward
// iterable and possible to use in the collection comparison
