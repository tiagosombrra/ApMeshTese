...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_fp_comparizon_with_percent_tolerance, FPT,
                              test_types) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_fp_comparizon_with_fraction_tolerance, FPT,
                              test_types) {
#define CHECK_CLOSE(first, second, e)             \
  fp1 = static_cast<FPT>(first);                  \
  fp2 = static_cast<FPT>(second);                 \
...
BOOST_AUTO_TEST_CASE(test_strong_weak, *expected_failures(4)) {
  ...
  BOOST_TEST(1. / 3 == 1. / 2,
             tt::tolerance(1. / 3));  // will fail; both 1/3 and 1/2 > 1/3
  BOOST_TEST(1. / 3 != 1. / 2,
             tt::tolerance(1.));  // will fail; both 1/3 and 1/2 < 1
...
BOOST_AUTO_TEST_CASE(test_close_at_tolerance) {
  ...
  ::fpc::close_at_tolerance<double> pred(::fpc::percent_tolerance(epsilon),
                                         ::fpc::FPC_WEAK);
  ...
  BOOST_CHECK_PREDICATE(::fpc::close_at_tolerance<double>(
                            ::fpc::percent_tolerance(epsilon), ::fpc::FPC_WEAK),
                        (fp1)(fp2));

  BOOST_TEST(!::fpc::close_at_tolerance<double>(
      ::fpc::percent_tolerance(epsilon))(fp1, fp2));
}

BOOST_AUTO_TEST_CASE(test_comparison_if_one_is_FPV,
