...
struct ScientificNotationExponentOutputNormalizer {
  ScientificNotationExponentOutputNormalizer()
      : m_old_format(_set_output_format(_TWO_DIGIT_EXPONENT)) {}

  ~ScientificNotationExponentOutputNormalizer() {
    _set_output_format(m_old_format);
  }
...
struct shorten_lf : public boost::unit_test::output::compiler_log_formatter {
  void print_prefix(std::ostream& output, boost::unit_test::const_string,
                    std::size_t line) {}
};
...
static tt::output_test_stream& ots() {
  static boost::shared_ptr<tt::output_test_stream> inst;

  if (!inst) {
    inst.reset(ut::framework::master_test_suite().argc <= 1
                   ? new tt::output_test_stream(
                         ut::runtime_config::save_pattern() ? save_file_name
                                                            : match_file_name,
                         !ut::runtime_config::save_pattern())
                   : new tt::output_test_stream(
...
#define TEST_CASE(name)                                              \
  ...
  BOOST_AUTO_TEST_CASE(name) {                                       \
    ut::test_case* impl =                                            \
        ut::make_test_case(&name##_impl, #name, __FILE__, __LINE__); \
    ...
    {                                                                \
      ...
      log_setup_teardown holder(                                     \
          stream, OF_CUSTOM_LOGGER, ut::log_nothing,                 \
...
TEST_CASE(test_BOOST_WARN_MESSAGE) {
  BOOST_WARN_MESSAGE(sizeof(int) == sizeof(short),
                     "memory won't be used efficiently");
  int obj_size = 33;

  BOOST_WARN_MESSAGE(obj_size <= 8,
                     "object size "
                         << obj_size
                         << " is too big to be efficiently passed by value");

  ut::unit_test_log.set_threshold_level(ut::log_successful_tests);
  BOOST_WARN_MESSAGE(obj_size > 8,
                     "object size " << obj_size << " is too small");
}

//____________________________________________________________________________//

boost::test_tools::predicate_result test_pred1() {
...
TEST_CASE(test_BOOST_ERROR) { BOOST_ERROR("Fail to miss an error"); }
...
TEST_CASE(test_BOOST_CHECK_THROW) {
  ...
  if (i / 10 > 10) {
    ...
    BOOST_CHECK_THROW(throw my_exception(),
                      my_exception);  // unreachable code warning is expected
}

//____________________________________________________________________________//

TEST_CASE(test_BOOST_CHECK_EXCEPTION) {
  BOOST_CHECK_EXCEPTION(throw my_exception(1), my_exception,
                        is_critical);  // unreachable code warning is expected
...
TEST_CASE(test_BOOST_CHECK_NO_THROW) {
  ...
  BOOST_CHECK_NO_THROW(
      throw my_exception());  // unreachable code warning is expected
}
...
std::ostream& operator<<(std::ostream& str, B const& b) {
  return str << "B(" << b.m_i << ")";
}
...
boost::test_tools::predicate_result operator==(C const& c1, C const& c2) {
  boost::test_tools::predicate_result res(c1.m_i == c2.m_i &&
                                          c1.m_id == c2.m_id);
...
std::ostream& operator<<(std::ostream& str, C const& c) {
  return str << "C(" << c.m_i << ',' << c.m_id << ")";
}
...
boost::test_tools::assertion_result compare_lists(std::list<int> const& l1,
...
TEST_CASE(test_BOOST_CHECK_PREDICATE) {
  ...
  int j = 15;
  BOOST_CHECK_PREDICATE(boost::bind(is_even, boost::bind(&foo, _1, _2)),
                        (i)(j));
...
struct A {
  friend std::ostream& operator<<(std::ostream& str, A const&) {
    str << "struct A";
    return str;
  }
};
...
TEST_CASE(test_BOOST_TEST_PASSPOINT) { BOOST_CHECK(foo()); }
...
TEST_CASE(test_context_logging) {
  ...
  BOOST_TEST_CONTEXT("outer context") {
    ...
    BOOST_TEST_CONTEXT("inner context") { BOOST_CHECK(false); }
...
TEST_CASE(test_BOOST_TEST_basic_arithmetic_op) {
  ...
  BOOST_TEST(i + j == 15,
             "This message reported instead including " << i << " and " << j);
...
std::ostream& operator<<(std::ostream& os, copy_counter const&) {
  return os << "copy_counter";
}
...
BOOST_AUTO_TEST_CASE(test_rvalue_erasure) {
  auto erase_rv = [](rv_erasure_test const& arg) -> rv_erasure_test const& {
    return arg;
  };
...
TEST_CASE(test_BOOST_TEST_fpv_comp_on_collections) {
  ...
  ut::unit_test_log.set_threshold_level(
      ut::log_successful_tests);  // we log everything
  ...
  std::vector<float> v1f(3, static_cast<float>(d1)),
      v2f(3, static_cast<float>(d2));
  ...
  BOOST_TEST(v1f == v2f,
             tt::tolerance(1e-3)
                 << tt::per_element());  // tolerance is on double, does not
                                         // apply to vectors.
  BOOST_TEST(v1 <= v2, tt::tolerance(1e-3) << tt::per_element());
  BOOST_TEST(v1 < v2, tt::tolerance(1e-3) << tt::per_element());
  BOOST_TEST(v1 <= v2, tt::tolerance(1e-2)
                           << tt::per_element());  // high tolerance
  BOOST_TEST(v1 < v2, tt::tolerance(1e-2) << tt::per_element());
  BOOST_TEST(v1f < v2f, tt::tolerance(1e-2)
                            << tt::per_element());  // not float tolerance

  // tolerance and custom message
  BOOST_TEST(
      v1 == v2,
      tt::tolerance(1e-3)
          << "'custom error message v1  == v2   tolerance<double>       ' "
          << v1[0] << " != " << v2[0]);
  BOOST_TEST(
      v1f == v2f,
      tt::tolerance(1e-3)
          << "'custom error message v1f == v2f  tolerance<double>       ' "
          << v1f[0] << " != " << v2f[0]);
  BOOST_TEST(
      v1 <= v2,
      tt::tolerance(1e-3)
  ...
  BOOST_TEST(
      v1 <= v2,
      tt::tolerance(1e-3)
          << "'custom error message v1  <= v2   high_tolerance<double>  ' "
          << v1[0] << " >  " << v2[0]);
  BOOST_TEST(
      v1 < v2,
      tt::tolerance(1e-2)
  ...
  BOOST_TEST(
      v1f < v2f,
      tt::tolerance(1e-2)
          << "'custom error message v1f <  v2f  high_tolerance<double>  ' "
          << v1f[0] << " >= " << v2f[0]);  // not float tolerance
  ...
  BOOST_TEST(
      ...
      tt::tolerance(1e-3)
          << "'custom error message v1  == v2   tolerance<double>       ' "
          << v1[0] << " != " << v2[0] << tt::per_element());
  BOOST_TEST(
      v1f == v2f,
      tt::tolerance(1e-3)
          << "'custom error message v1f == v2f  tolerance<double>       ' "
          << v1f[0] << " != " << v2f[0] << tt::per_element());
  BOOST_TEST(
      v1 <= v2,
      tt::tolerance(1e-3)
          << "'custom error message v1  <= v2   tolerance<double>       ' "
  ...
  BOOST_TEST(v1 == v2,
             "'custom error message v1  == v2   tolerance<double>       ' "
                 << v1[0] << " != " << v2[0] << tt::tolerance(1e-3)
                 << tt::per_element());
  BOOST_TEST(v1f == v2f,
             "'custom error message v1f == v2f  tolerance<double>       ' "
                 << v1f[0] << " != " << v2f[0] << tt::tolerance(1e-3)
                 << tt::per_element());
  BOOST_TEST(v1 <= v2,
             "'custom error message v1  <= v2   tolerance<double>       ' "
                 << v1[0] << " >  " << v2[0] << tt::tolerance(1e-3)
  ...
  BOOST_TEST(v1 < v2,
             "'custom error message v1  <  v2   high_tolerance<double>  ' "
                 << v1[0] << " >= " << v2[0] << tt::tolerance(1e-2)
                 << tt::per_element());
  BOOST_TEST(v1f < v2f,
             "'custom error message v1f <  v2f  high_tolerance<double>  ' "
                 << v1f[0] << " >= " << v2f[0] << tt::tolerance(1e-2)
                 << tt::per_element());  // not float tolerance

  // custom message and per_element
  BOOST_TEST(v1 == v2, "'custom error message v1  == v2  ' "
                           << v1[0] << " != " << v2[0] << tt::per_element());
  BOOST_TEST(v1f == v2f, "'custom error message v1f == v2f ' "
                             << v1f[0] << " != " << v2f[0]
                             << tt::per_element());
  BOOST_TEST(v1 <= v2, "'custom error message v1  <= v2  ' "
                           << v1[0] << " >  " << v2[0] << tt::per_element());
  ...
  // BOOST_TEST( v1f == v2f, tt::tolerance( 1e-3 ) << tt::lexicographic()); //
  // tolerance is on double, does not apply to vectors.
  v3[1] = d1;
  BOOST_TEST(v1 <= v2, tt::tolerance(1e-3) << tt::lexicographic());
  BOOST_TEST(v1 < v2, tt::tolerance(1e-3) << tt::lexicographic());
  ...
  BOOST_TEST(v3 < v2, tt::tolerance(1e-3) << tt::lexicographic());
  BOOST_TEST(v1 <= v2, tt::tolerance(1e-2)
                           << tt::lexicographic());  // high tolerance
  ...
  // custom and lexico
  BOOST_TEST(v1 <= v2, "'custom error message v1  <= v2   ' "
                           << v1[0] << " != " << v2[0] << tt::lexicographic());
  BOOST_TEST(v1 < v2, "'custom error message v1  <  v2   ' "
                          << v1f[0] << " != " << v2f[0] << tt::lexicographic());
  BOOST_TEST(v2 <= v3, "'custom error message v2  <= v3   ' "
                           << v1[0] << " >  " << v2[0] << tt::lexicographic());
  BOOST_TEST(v2 < v3, "'custom error message v2  <  v3   ' "
                          << v1[0] << " >= " << v2[0] << tt::lexicographic());
  BOOST_TEST(v3 <= v2, "'custom error message v3  <= v2   ' "
                           << v1[0] << " >  " << v2[0] << tt::lexicographic());
  BOOST_TEST(v3 < v2, "'custom error message v3  <  v2   ' "
                          << v1[0] << " >= " << v2[0] << tt::lexicographic());
  ...
  BOOST_TEST(v1 < v2,
             "'custom error message v1  <  v2   tolerance<double>       ' "
                 ...
                 << tt::lexicographic());
  BOOST_TEST(v2 <= v3,
             "'custom error message v2  <= v3   tolerance<double>       ' "
                 << v1[0] << " >  " << v2[0] << tt::tolerance(1e-3)
                 << tt::lexicographic());
  BOOST_TEST(v2 < v3,
             "'custom error message v2  <  v3   tolerance<double>       ' "
                 ...
                 << tt::lexicographic());
  BOOST_TEST(v3 <= v2,
             "'custom error message v3  <= v2   tolerance<double>       ' "
                 << v1[0] << " >  " << v2[0] << tt::tolerance(1e-3)
                 << tt::lexicographic());
  BOOST_TEST(v3 < v2,
             "'custom error message v3  <  v2   tolerance<double>       ' "
  ...
  BOOST_TEST(v1 < v2,
             "'custom error message v1  <  v2   high_tolerance<double>  ' "
                 ...
                 << tt::lexicographic());
  BOOST_TEST(v2 <= v3,
             "'custom error message v2  <= v3   high_tolerance<double>  ' "
                 << v1[0] << " >  " << v2[0] << tt::tolerance(1e-2)
                 << tt::lexicographic());
  BOOST_TEST(v2 < v3,
             "'custom error message v2  <  v3   high_tolerance<double>  ' "
                 ...
                 << tt::lexicographic());
  BOOST_TEST(v3 <= v2,
             "'custom error message v3  <= v2   high_tolerance<double>  ' "
                 << v1[0] << " >  " << v2[0] << tt::tolerance(1e-2)
                 << tt::lexicographic());
  BOOST_TEST(v3 < v2,
             "'custom error message v3  <  v2   high_tolerance<double>  ' "
