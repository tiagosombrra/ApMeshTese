    ...
    try {
      {
        ...
        submit_some(ea);
        {
          ...
          // boost::future<int> t2 = boost::async(ea3, f2, 1); // todo this
          // doesn't compiles yet on C++11 boost::future<int> t2 =
          // boost::async(ea3, boost::bind(f2, 1)); // todo this doesn't
          // compiles yet on C++98
          std::cout << BOOST_CONTEXTOF << " t1= " << t1.get() << std::endl;
          std::cout << BOOST_CONTEXTOF << " t2= " << t2.get() << std::endl;
        }
        submit_some(ea);
