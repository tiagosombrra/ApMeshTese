...
Stopwatch::rep benchmark_ping_pong(unsigned consumer_count) {
  ...
  for (int times = 10; times--;) {
    ...
    for (unsigned i = 0; i < consumer_count; ++i) consumers[i].join();
...
/*
 ...
 condition_variable::notify_all() without holding a mutex to maximize contention
 within this function. Each benchmark for a number of consumers is run three
 times and the best time is picked to get rid of outliers.

 The results are reported for each benchmark for a number of consumers. The most
 important number is (std - boost) / std * 100. Positive numbers are when
