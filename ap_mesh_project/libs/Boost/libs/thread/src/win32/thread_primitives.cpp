...
BOOST_THREAD_DECL boost::detail::win32::detail::gettickcount64_t
    gettickcount64 = &::boost::winapi::GetTickCount64;

#else  // BOOST_USE_WINAPI_VERSION >= BOOST_WINAPI_VERSION_WIN6

namespace {

enum init_state { uninitialized = 0, in_progress, initialized };
...
ticks_type WINAPI get_tick_count64() {
  ...
  uint64_t new_state = ((old_state & UINT64_C(0xffffffff00000000)) +
...
//! The function is called periodically in the system thread pool to make sure
//! g_state.ticks is timely updated
void NTAPI refresh_get_tick_count64(boost::winapi::PVOID_,
...
#pragma GCC diagnostic ignored "-Wcast-function-type"
    ...
    boost::detail::win32::detail::gettickcount64_t p =
        (boost::detail::win32::detail::gettickcount64_t)
            boost::winapi::get_proc_address(hKernel32, "GetTickCount64");
#if defined(BOOST_GCC) && BOOST_GCC >= 80000
#pragma GCC diagnostic pop
#endif
    if (p) {
      // Use native API
      boost::detail::interlocked_write_release((void**)&gettickcount64,
                                               (void*)p);
      return p();
    }
  }

  // No native API available. Use emulation with periodic refreshes to make sure
  // the GetTickCount wrap arounds are properly counted.
  ...
  if (g_state.init.compare_exchange_strong(old_init, in_progress,
    ...
    if (g_state.wait_event) {
      boost::winapi::BOOL_ res = boost::winapi::RegisterWaitForSingleObject(
          &g_state.wait_handle, g_state.wait_event, &refresh_get_tick_count64,
          NULL, 0x7fffffff, boost::winapi::WT_EXECUTEINWAITTHREAD_);
      if (res) {
...
BOOST_THREAD_DECL boost::detail::win32::detail::gettickcount64_t
    gettickcount64 = &get_tick_count_init;
