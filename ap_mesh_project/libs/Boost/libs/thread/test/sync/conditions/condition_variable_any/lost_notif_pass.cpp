...
// 2. The main thread takes the lock and then sleeps for a long time while
// holding
...
//    notifications sent after pthread_cond_timedwait() times out but before it
//    can reacquire the lock may be "lost". pthread_cond_timedwait() will report
//    that it timed out and the wait function may incorrectly assume that no
...
//    notification, it will return fairly quickly. If it missed the
//    notification, the test thread won't return until the wait function being
//    tested times out.
...
bool flagIsSet() { return flag; }

bool threadIsWaiting() { return waiting; }
...
void timed_wait_absolute_without_pred() {
  ...
  waiting = true;
  while (!flagIsSet()) {
    cv.timed_wait(lk, boost::posix_time::microsec_clock::universal_time() +
                          posix_wait_time);
  }
}
...
void timed_wait_absolute_with_pred() {
  ...
  waiting = true;
  cv.timed_wait(
      lk, boost::posix_time::microsec_clock::universal_time() + posix_wait_time,
      flagIsSet);
}
...
#else
#error \
    "Test not applicable: BOOST_THREAD_USES_DATETIME not defined for this platform as not supported"
#endif
