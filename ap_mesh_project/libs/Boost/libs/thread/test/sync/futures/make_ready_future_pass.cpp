...
struct A {
  A() : value(0) {}
  A(int i) : value(i) {}
  A(int i, int j) : value(i + j) {}
  int value;
};

A make(int i) { return A(i); }
A make(int i, int j) { return A(i, j); }
...
#endif
#if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
  // sync/futures/make_ready_future_pass.cpp:125:65: erreur: conversion from
  // Ôboost::future<boost::rv<movable2> >Õ to non-scalar type
  // Ôboost::future<movable2>Õ requested
  {
    typedef movable2 T;
    T i;
