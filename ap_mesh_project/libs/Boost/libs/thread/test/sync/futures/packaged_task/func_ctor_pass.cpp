...
class C {
 public:
  ...
  static int n_copies;
  static void reset() { n_copies = 0; }

  explicit C(long i) : data_(i) {}
  C(const C& a) : data_(a.data_) { ++n_copies; }
...
int main() {
  ...
  {
    A::reset();
    boost::packaged_task<BOOST_THREAD_DETAIL_VOID_SIGNATURE_2> p(
        BOOST_THREAD_MAKE_RV_REF(A(5)));
    BOOST_TEST(p.valid());
