...
class test_allocator : public test_alloc_base {
 ...
 public:
  ...
  typedef typename boost::add_lvalue_reference<const value_type>::type
      const_reference;
  ...
  test_allocator(const test_allocator& a) throw() : data_(a.data_) {}
  template <class U>
  test_allocator(const test_allocator<U>& a) throw() : data_(a.data_) {}
  ...
  size_type max_size() const throw() { return UINT_MAX / sizeof(T); }
...
class other_allocator {
 ...
 public:
  ...
  T* allocate(std::size_t n) { return (T*)std::malloc(n * sizeof(T)); }
