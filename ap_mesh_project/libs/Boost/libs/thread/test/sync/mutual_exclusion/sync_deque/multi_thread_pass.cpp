...
void test_concurrent_push_back_and_pull_front_on_empty_queue() {
  ...
  try {
    push_done =
        boost::async(boost::launch::async, call_push_back<int>(&q, &go));
    pull_done =
        boost::async(boost::launch::async, call_pull_front<int>(&q, &go));
...
void test_concurrent_push_back_and_wait_pull_front_on_empty_queue() {
  ...
  try {
    for (unsigned int i = 0; i < n; ++i)
      pull_done[i] =
          boost::async(boost::launch::async, call_wait_pull_front<int>(&q, &go),
                       boost::ref(results[i]));

    for (unsigned int i = 0; i < n; ++i) q.push_back(42);
...
void test_concurrent_push_back_on_empty_queue() {
  ...
  try {
    BOOST_TEST(!q.empty());
    for (unsigned int i = 0; i < n; ++i) BOOST_TEST_EQ(q.pull_front(), 42);
    BOOST_TEST(q.empty());
