...
void call_push(sync_tq* q, const steady_clock::time_point start) {
  // push elements onto the queue every 500 milliseconds but with a decreasing
  // delay each time
  for (int i = 0; i < cnt; ++i) {
    boost::this_thread::sleep_until(start + milliseconds(i * 500));
    const steady_clock::time_point expected =
        start + milliseconds(i * 500) + seconds(cnt - i);
    q->push(i, expected);
  }
}
...
void call_pull(sync_tq* q, const steady_clock::time_point start) {
  ...
  for (int i = cnt - 1; i >= 0; --i) {
    ...
    const steady_clock::time_point expected =
        start + milliseconds(i * 500) + seconds(cnt - i);
    BOOST_TEST_GE(steady_clock::now(),
                  expected - milliseconds(BOOST_THREAD_TEST_TIME_MS));
    BOOST_TEST_LE(steady_clock::now(),
                  expected + milliseconds(BOOST_THREAD_TEST_TIME_MS));
  }
}
