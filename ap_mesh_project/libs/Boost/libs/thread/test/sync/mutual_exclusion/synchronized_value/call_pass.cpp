...
void c(S const& s) { BOOST_TEST(s.f() == 1); }
...
struct cfctr {
  ...
  void operator()(S const& s) const { BOOST_TEST(s.f() == 1); }
};
struct ncfctr {
  typedef void result_type;
...
cfctr3 make_cfctr3() { return BOOST_THREAD_MAKE_RV_REF(cfctr3()); }

ncfctr3 make_ncfctr3() { return BOOST_THREAD_MAKE_RV_REF(ncfctr3()); }
