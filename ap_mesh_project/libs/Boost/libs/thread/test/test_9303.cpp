...
int int_no_params() { return 42; }

int int_with_params(int i) { return i; }

std::string string_no_params() { return std::string("forty two"); }

std::string string_with_params(std::string& ans) { return ans; }
...
int main(int /*argc*/, char** /*argv[]*/) {
  ...
  //        ../../../boost/thread/detail/thread.hpp:76:15: error: no matching
  //        function for call to 'invoke'
  //                      invoke(std::move(std::get<0>(fp)),
  ...
  //! Doesn't compile in C++03, C++11 only.
  // packagedTestTest.cpp:94:43: error: invalid use of incomplete type ‘class
  // boost::packaged_task<std::basic_string<char>(std::basic_string<char>&)>’
  // packagedTestTest.cpp:95:37: error: incomplete type ‘task_t {aka
  // boost::packaged_task<std::basic_string<char>(std::basic_string<char>&)>}’
  // used in nested name specifier boost/thread/future.hpp:1320:11: error:
  // declaration of ‘class
  // boost::packaged_task<std::basic_string<char>(std::basic_string<char>&)>’
  {
    // typedef boost::packaged_task<std::string(std::string&)> task_t;
    typedef boost::packaged_task<std::string()> task_t;
    boost::shared_ptr<task_t> example = boost::make_shared<task_t>(
        boost::bind(&string_with_params, boost::ref(ans)));
    boost::future<std::string> f = example->get_future();
