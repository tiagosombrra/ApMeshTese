...
#define BOOST_TEST_MODULE \
  Boost.Threads : condition_variable notify_one test suite
...
void do_test_condition_notify_one_wakes_from_timed_wait() {
  ...
  boost::thread thread(
      bind(&wait_for_flag::timed_wait_without_predicate, data));
...
void do_test_condition_notify_one_wakes_from_relative_timed_wait_with_predicate() {
  ...
  boost::thread thread(
      bind(&wait_for_flag::relative_timed_wait_with_predicate, data));
...
BOOST_AUTO_TEST_CASE(test_condition_notify_one) {
  timed_test(&do_test_condition_notify_one_wakes_from_wait, timeout_seconds,
             execution_monitor::use_mutex);
  timed_test(&do_test_condition_notify_one_wakes_from_wait_with_predicate,
             timeout_seconds, execution_monitor::use_mutex);
  timed_test(&do_test_condition_notify_one_wakes_from_timed_wait,
             timeout_seconds, execution_monitor::use_mutex);
  timed_test(&do_test_condition_notify_one_wakes_from_timed_wait_with_predicate,
             timeout_seconds, execution_monitor::use_mutex);
  timed_test(
      &do_test_condition_notify_one_wakes_from_relative_timed_wait_with_predicate,
      timeout_seconds, execution_monitor::use_mutex);
