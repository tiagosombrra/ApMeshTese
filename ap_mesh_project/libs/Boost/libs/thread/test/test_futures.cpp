...
#ifdef BOOST_MSVC
#pragma warning( \
    disable : 4267)  // conversion from ... to ..., possible loss of data
#endif
...
struct X {
 public:
  ...
  X() : i(42) {}
  X(BOOST_THREAD_RV_REF(X) other) : i(BOOST_THREAD_RV(other).i) {
...
void set_promise_thread(boost::promise<int>* p) { p->set_value(42); }
...
BOOST_AUTO_TEST_CASE(test_unique_future_for_string) {
  ...
  boost::unique_future<std::string> fi(
      BOOST_THREAD_MAKE_RV_REF(pt.get_future()));
...
BOOST_AUTO_TEST_CASE(test_wait_for_any_from_range) {
  ...
  for (unsigned i = 0; i < count; ++i) {
    ...
    boost::unique_future<int>* const future =
