...
void lock_mutexes_slowly(boost::mutex* m1, boost::mutex* m2, wait_data* locked,
                         wait_data* quit) {
...
BOOST_AUTO_TEST_CASE(test_lock_five_uncontended) {
  ...
  boost::unique_lock<boost::mutex> l1(m1, boost::defer_lock),
      l2(m2, boost::defer_lock), l3(m3, boost::defer_lock),
      l4(m4, boost::defer_lock), l5(m5, boost::defer_lock);
...
void lock_five_mutexes_slowly(boost::mutex* m1, boost::mutex* m2,
                              boost::mutex* m3, boost::mutex* m4,
                              boost::mutex* m5, wait_data* locked,
                              wait_data* quit) {
  boost::lock_guard<boost::mutex> l1(*m1);
  boost::this_thread::sleep(boost::posix_time::milliseconds(500));
  boost::lock_guard<boost::mutex> l2(*m2);
...
void lock_five(boost::mutex* m1, boost::mutex* m2, boost::mutex* m3,
               boost::mutex* m4, boost::mutex* m5) {
...
BOOST_AUTO_TEST_CASE(test_lock_five_other_thread_locks_in_order) {
  ...
  boost::thread t(lock_five_mutexes_slowly, &m1, &m2, &m3, &m4, &m5, &locked,
                  &release);
  boost::this_thread::sleep(boost::posix_time::milliseconds(10));
...
class dummy_iterator {
 ...
 public:
  ...
  explicit dummy_iterator(dummy_mutex* p_) : p(p_) {}

  bool operator==(dummy_iterator const& other) const { return p == other.p; }

  bool operator!=(dummy_iterator const& other) const { return p != other.p; }

  bool operator<(dummy_iterator const& other) const { return p < other.p; }

  dummy_mutex& operator*() const { return *p; }
...
class dummy_mutex2 : public dummy_mutex {};
...
BOOST_AUTO_TEST_CASE(test_try_lock_three) {
  ...
  for (int i = -1; i < num_mutexes; ++i) {
    ...
    boost::unique_lock<dummy_mutex> l1(mutexes[0], boost::defer_lock),
        l2(mutexes[1], boost::defer_lock), l3(mutexes[2], boost::defer_lock);
...
BOOST_AUTO_TEST_CASE(test_try_lock_five) {
  ...
  for (int i = -1; i < num_mutexes; ++i) {
    ...
    boost::unique_lock<dummy_mutex> l1(mutexes[0], boost::defer_lock),
        l2(mutexes[1], boost::defer_lock), l3(mutexes[2], boost::defer_lock),
        l4(mutexes[3], boost::defer_lock), l5(mutexes[4], boost::defer_lock);
