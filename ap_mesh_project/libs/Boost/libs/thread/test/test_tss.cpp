...
BOOST_AUTO_TEST_CASE(test_tss_thread_native) { test_tss_thread(); }

native_thread_t create_native_thread() {
  return std::make_shared<std::thread>(test_tss_thread_native);
}

void join_native_thread(native_thread_t thread) { thread->join(); }
...
native_thread_t create_native_thread(void) {
  native_thread_t const res =
      CreateThread(0,  // security attributes (0 = not inheritable)
...
void do_test_tss() {
  ...
  // The following is not really an error. TSS cleanup support still is
  // available for boost threads. Also this usually will be triggered only when
  // bound to the static version of thread lib. 2006-10-02 Roland Schwarz
  // BOOST_CHECK_EQUAL(tss_instances, 0);
#if !defined(__MINGW32__)
  // This fails on MinGW, when using the static lib
  BOOST_CHECK_MESSAGE(tss_instances == 0,
                      "Support of automatic tss cleanup for native threading "
                      "API not available");
#endif
  BOOST_CHECK_EQUAL(tss_total, 5);
}

BOOST_AUTO_TEST_CASE(test_tss) { timed_test(&do_test_tss, 2); }
...
void do_test_tss_void() {
  ...
  //    // Also this usually will be triggered only when bound to the static
  //    version of thread lib.
  //    // 2006-10-02 Roland Schwarz
  //    //BOOST_CHECK_EQUAL(tss_instances, 0);
  //    BOOST_CHECK_MESSAGE(tss_instances == 0, "Support of automatic tss
  //    cleanup for native threading API not available");
  //    BOOST_CHECK_EQUAL(tss_total, 5);
}
...
boost::thread_specific_ptr<dummy_class_tracks_deletions> tss_with_null_cleanup(
    NULL);

void tss_thread_with_null_cleanup(
