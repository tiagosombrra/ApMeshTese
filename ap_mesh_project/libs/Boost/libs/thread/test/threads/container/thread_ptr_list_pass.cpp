...
int main() {
  {
    ...
    // typedef boost::interprocess::shared_ptr<boost::thread,
    // std::allocator<boost::thread>, default_delete<boost::thread> >
    // thread_ptr;
    typedef boost::csbl::list<thread_ptr> thread_ptr_list;
    thread_ptr_list threads;
    for (int i = 0; i < 10; ++i) {
      // threads.push_back(BOOST_THREAD_MAKE_RV_REF(thread_ptr(new
      // boost::thread(&increment_count))));
      threads.push_back(thread_ptr(new boost::thread(&increment_count)));
    }
    BOOST_TEST(threads.size() == 10);
    // join_all(threads);
    for (thread_ptr_list::iterator it = threads.begin(); it != threads.end();
         ++it) {
      (*it)->join();
    }
  }
