...
void resource_deadlock_would_occur_tester() {
  try {
    ...
    resource_deadlock_would_occur_th->try_join_until(
        boost::chrono::steady_clock::now() + boost::chrono::milliseconds(50));
    BOOST_TEST(false);
...
int main() {
  {
    ...
    t0.try_join_until(boost::chrono::steady_clock::now() +
                      boost::chrono::milliseconds(250));
    BOOST_TEST(!t0.joinable());
  }
  {
    boost::thread t0((th_250_ms));
    BOOST_TEST(!t0.try_join_until(boost::chrono::steady_clock::now() +
