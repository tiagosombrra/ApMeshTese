...
/*`
    The following example shows that `type_index` (and `type_info`) is able to
   store the exact type, without stripping const, volatile and references.
   Example works with and without RTTI.

    In this example we'll create a class that stores a pointer to function and
   remembers the exact type of the parameter the function accepts. When the call
   to the bound function is made, he actual input parameter type is checked
   against the stored parameter type and an exception is thrown in case of
   mismatch.
*/
...
//<-
// Making `#include <cassert>` visible in docs, while actually using hand-made
// check instead of `assert`. This is required to verify correct behavior even
// if NDEBUG is defined and to avoid `unused local variable` warnings with
// defined NDEBUG.
#ifdef assert
#undef assert
#endif
#define assert(X) \
  if (!(X)) std::exit(__LINE__)
