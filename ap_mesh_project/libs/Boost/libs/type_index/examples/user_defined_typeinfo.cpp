...
/*`
    There is an easy way to force `boost::typeindex::type_id` to use your own
   type_index class.

    All we need to do is just define `BOOST_TYPE_INDEX_USER_TYPEINDEX` to the
   full path to header file of your type index class:
*/

// BOOST_TYPE_INDEX_USER_TYPEINDEX must be defined *BEFORE* first inclusion of
// <boost/type_index.hpp>
#define BOOST_TYPE_INDEX_USER_TYPEINDEX \
<boost/../libs/type_index/examples/user_defined_typeinfo.hpp>
...
int main() {
  ...
  */
  ...
  my_type_index cl1 = my_type_index::type_id<my_class>(),
  ...
  */
  ...
  assert(my_type_index::type_id<my_struct>() ==
         my_type_index::type_id_runtime(reference));
  //][/type_index_my_type_index_type_id_runtime_test]

  //[type_index_my_type_index_worldwide_usage
  /*`
      That's it! Now all TypeIndex global methods and typedefs will be using
     your class:
  */
  boost::typeindex::type_index worldwide =
      boost::typeindex::type_id<my_classes>();
