...
template <>
struct typenum<my_class> {
  enum { value = 1 };
};
template <>
struct typenum<my_struct> {
  enum { value = 2 };
};
template <>
struct typenum<my_classes> {
  enum { value = 3 };
};
template <>
struct typenum<my_string> {
  enum { value = 4 };
};
...
#pragma warning(push)
#pragma warning( \
    disable : 4510 4512 4610)  // non-copyable non-constructable type
#endif
...
/*`
    `my_type_index` is a user created type_index class. If in doubt during this
   phase, you can always take a look at the
   `<boost/type_index/ctti_type_index.hpp>` or
   `<boost/type_index/stl_type_index.hpp>` files. Documentation for
   `type_index_facade` could be also useful.
*/

/*`
    Since we are not going to override `type_index_facade::hash_code()` we must
...
class my_type_index
    : public boost::typeindex::type_index_facade<my_type_index,
                                                 detail::my_typeinfo> {
 ...
 public:
  ...
  inline my_type_index(const type_info_t& data) BOOST_NOEXCEPT : data_(&data) {}

  inline const type_info_t& type_info() const BOOST_NOEXCEPT { return *data_; }

  inline const char* raw_name() const BOOST_NOEXCEPT { return data_->type_; }

  inline std::string pretty_name() const { return data_->type_; }
...
/*`
    Note that we have used the boost::typeindex::type_index_facade class as
   base. That class took care about all the helper function and operators
   (comparison, hashing, ostreaming and others).
*/
...
/*`
    Usually to allow runtime type info we need to register class with some
   macro. Let's see how a `MY_TYPEINDEX_REGISTER_CLASS` macro could be
   implemented for our `my_type_index` class:
*/
namespace my_namespace {
namespace detail {
...
/*`
    Now when we have a MY_TYPEINDEX_REGISTER_CLASS, let's implement a
   `my_type_index::type_id_runtime` method:
*/
namespace my_namespace {
template <class T>
...
/*`
    Consider the situation, when `my_class` and `my_struct` are polymorphic
   classes:
*/
...
/*`
    You'll also need to add some typedefs and macro to your
   "user_defined_typeinfo.hpp" header file:
*/
