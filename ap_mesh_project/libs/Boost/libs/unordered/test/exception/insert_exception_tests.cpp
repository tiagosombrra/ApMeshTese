...
template <typename T>
T get_iterator(T const& x) {
  return x;
}

template <typename T>
T get_iterator(std::pair<T, bool> const& x) {
  return x.first;
}
...
template <typename T, typename Inserter>
void insert_rehash_exception_test(T*, Inserter insert,
                                  test::random_generator gen) {
  for (int i = 0; i < 5; ++i) {
    T x;
    rehash_prep(x);
...
struct inserter_base {
  template <typename T>
  void exception_check(T& x, test::strong<T>& strong) {
...
template <typename T>
struct insert_lvalue_pos_type_impl : inserter_base {
...
template <typename T>
struct emplace_lvalue_pos_type_impl : inserter_base {
  typename T::iterator pos;
...
struct emplace_lvalue_pos_type {
  template <typename T>
  friend emplace_lvalue_pos_type_impl<T> generate(emplace_lvalue_pos_type,
                                                  T& x) {
