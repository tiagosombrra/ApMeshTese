...
std::pair<object, object> generate(std::pair<object, object> const*,
                                   random_generator);
...
class object : private counted_object {
 public:
  ...
  friend bool operator<(object const& x1, object const& x2) {
    return x1.tag1_ < x2.tag1_ || (x1.tag1_ == x2.tag1_ && x1.tag2_ < x2.tag2_);
  }
...
std::pair<object, object> generate(std::pair<object, object> const*,
                                   random_generator g) {
  int* x = 0;
  return std::make_pair(object(::test::generate(x, g), ::test::generate(x, g)),
...
class equal_to {
 ...
 public:
  ...
  bool operator()(std::pair<object, object> const& x1,
                  std::pair<object, object> const& x2) const {
    UNORDERED_SCOPE(equal_to::operator()(std::pair<object, object>,
...
class allocator {
 public:
  ...
  template <class Y>
  allocator(allocator<Y> const& x) : tag_(x.tag_) {
    test::detail::tracker.allocator_ref();
  }
...
class allocator2 {
 public:
  ...
  template <class Y>
  allocator2(allocator2<Y> const& x) : tag_(x.tag_) {
    test::detail::tracker.allocator_ref();
  }
...
template <>
struct equals_to_compare<test::exception::equal_to> {
