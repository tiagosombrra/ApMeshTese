...
UNORDERED_AUTO_TEST(check_is_nothrow_move) {
  ...
  have_is_nothrow_swap = boost::is_nothrow_swappable<hash_nothrow_swap>::value;
...
UNORDERED_AUTO_TEST(test_noexcept) {
  ...
  BOOST_TEST(
      (!boost::is_nothrow_move_constructible<boost::unordered_multiset<
           int, boost::hash<int>, equal_to_possible_exception> >::value));
}
