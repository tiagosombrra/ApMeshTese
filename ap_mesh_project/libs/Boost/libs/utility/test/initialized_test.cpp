...
class key_value_pair {
 ...
 public:
  ...
  // Value-initializes the object held by m_value.
  explicit key_value_pair(const std::string& key) : m_key(key) {}
...
bool is_value_initialized(const int& arg) { return arg == 0; }

// Tells whether the argument is value-initialized.
bool is_value_initialized(const std::string& arg) { return arg.empty(); }
...
// Tells whether the argument is value-initialized.
bool is_value_initialized(const foo& arg) { return arg.data == 0; }
...
void test_key_value_pair(const T& magic_value) {
  ...
  // The value component of a key_value_pair that only has its key explicitly
  // specified must also be value-initialized.
