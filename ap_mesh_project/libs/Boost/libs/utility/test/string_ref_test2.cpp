...
void find(const char *arg) {
  ...
  while (*p) {
    ...
    BOOST_TEST(pos != string_ref::npos && pos < sr1.size() &&
               (pos >= ptr_diff(p, arg)));
    ++p;
  }
  ...
  while (*p && *(p + 1)) {
    ...
    BOOST_TEST(pos != string_ref::npos &&
               pos <= static_cast<string_ref::size_type>(p - arg));
    p++;
  }
  ...
  while (*p) {
    ...
    BOOST_TEST(pos1 != string_ref::npos && pos1 < sr1.size() &&
               pos1 <= ptr_diff(p, arg));
