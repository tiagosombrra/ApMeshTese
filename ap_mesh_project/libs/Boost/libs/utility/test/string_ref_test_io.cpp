 ...
 * \date   26.05.2013
 *
 * \brief  This header contains tests for stream operations of \c
 * basic_string_ref.
 */
...
/* Current implementations seem to be missing codecvt facets to convert chars to
 * char16_t and char32_t even though the types are available.
 */

static const char* test_strings[] = {"begin", "abcd", "end"};
...
void test_padding() {
  ...
  // Test for padding
  {
    ...
    strm_ref << ctx.begin << std::setw(8) << string_ref_type(ctx.abcd)
             << ctx.end;
  ...
  {
    ostream_type strm_ref;
    strm_ref << ctx.begin << std::setw(100) << string_ref_type(ctx.abcd)
             << ctx.end;
  ...
  {
    ostream_type strm_ref;
    strm_ref << ctx.begin << std::setw(1) << string_ref_type(ctx.abcd)
             << ctx.end;
...
void test_padding_fill() {
  ...
  strm_ref << ctx.begin << std::setfill(static_cast<char_type>('x'))
           << std::setw(8) << string_ref_type(ctx.abcd) << ctx.end;
...
void test_alignment() {
  ...
  // Left alignment
  {
    ...
    strm_ref << ctx.begin << std::left << std::setw(8)
             << string_ref_type(ctx.abcd) << ctx.end;

    ostream_type strm_correct;
    strm_correct << ctx.begin << std::left << std::setw(8) << ctx.abcd
                 << ctx.end;
  ...
  // Right alignment
  {
    ...
    ostream_type strm_correct;
    strm_correct << ctx.begin << std::right << std::setw(8) << ctx.abcd
                 << ctx.end;
