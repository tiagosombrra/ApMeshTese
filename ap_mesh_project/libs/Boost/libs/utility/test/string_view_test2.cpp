...
void ends_with(const char *arg) {
  ...
  sr2 = arg;
  if (sz > 0) BOOST_TEST(sr2.ends_with(ch));
...
void starts_with(const char *arg) {
  ...
  char ch = *arg;
  sr2 = arg;
  if (sz > 0) BOOST_TEST(sr2.starts_with(ch));
...
void find(const char *arg) {
  ...
  while (*p) {
    ...
    BOOST_TEST(pos != string_view::npos && pos < sr1.size() &&
               (pos >= ptr_diff(p, arg)));
    ++p;
  }
  ...
  while (*p && *(p + 1)) {
    ...
    BOOST_TEST(pos != string_view::npos &&
               pos <= static_cast<string_view::size_type>(p - arg));
    p++;
  }
  ...
  for (int ch = 1; ch < 256; ++ch) {
    ...
    if (strp != NULL) BOOST_TEST(ptr_diff(strp, arg) == pos);
  }
  ...
  while (*p) {
    ...
    BOOST_TEST(pos1 != string_view::npos && pos1 < sr1.size() &&
               pos1 <= ptr_diff(p, arg));
    if (pos2 != string_view::npos) {
      for (size_t i = 0; i < pos2; ++i) BOOST_TEST(sr1[i] == *p);
  ...
  while (*p) {
    ...
    if (pos2 != string_view::npos) {
      for (size_t i = sr1.size() - 1; i > pos2; --i) BOOST_TEST(sr1[i] == *p);
      BOOST_TEST(sr1[pos2] != *p);
    }
...
class custom_allocator {
 public:
  ...
  void deallocate(pointer p, size_type) const BOOST_NOEXCEPT { std::free(p); }

  pointer address(reference value) const BOOST_NOEXCEPT { return &value; }
...
BOOST_CONSTEXPR bool operator==(const custom_allocator<T> &,
                                const custom_allocator<U> &) BOOST_NOEXCEPT {
  return true;
}
template <typename T, typename U>
BOOST_CONSTEXPR bool operator!=(const custom_allocator<T> &,
...
void to_string(const char *arg) {
  ...
  std::basic_string<char, std::char_traits<char>, custom_allocator<char> >
      str3 = sr1.to_string(custom_allocator<char>());
...
const char *test_strings[] = {
    ...
    "abc",
    "AAA",  // all the same
    "adsfadadiaef;alkdg;aljt;j "
    "agl;sjrl;tjs;lga;lretj;srg[w349u5209dsfadfasdfasdfadsf",
    "abc\0asdfadsfasf",
    NULL};
