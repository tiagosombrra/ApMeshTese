...
// 15 Jan 2008 (Added tests regarding compiler issues) Fernando Cacciola, Niels
// Dekker 23 May 2008 (Added tests regarding initialized_value) Niels Dekker 21
// Ago 2008 (Added swap test) Niels Dekker
...
// Borland C++ Report #51854, "Value-initialization: POD struct should be
// zero-initialized " http://qc.codegear.com/wc/qcmain.aspx?d=51854
//
...
// Some compilers do not correctly value-initialize such a struct, for example:
// Microsoft Visual C++, Feedback ID 100744, "Value-initialization in
// new-expression"
// https://connect.microsoft.com/VisualStudio/feedback/ViewFeedback.aspx?FeedbackID=100744
//
...
bool operator==(StructWithDestructor const& lhs,
                StructWithDestructor const& rhs) {
  return lhs.i == rhs.i;
}

//
// A struct that has a virtual function.
// Some compilers do not correctly value-initialize such a struct either, for
// example: Microsoft Visual C++, Feedback ID 100744, "Value-initialization in
// new-expression"
// https://connect.microsoft.com/VisualStudio/feedback/ViewFeedback.aspx?FeedbackID=100744
//
...
bool operator==(StructWithVirtualFunction const& lhs,
                StructWithVirtualFunction const& rhs) {
  return lhs.i == rhs.i;
}
...
// Some compilers do not correctly value-initialize such a struct, for example:
// GCC Bugzilla Bug 30111,  "Value-initialization of POD base class doesn't
// initialize members", reported by Jonathan Wakely,
...
bool operator==(AggregatePODStructWrapper const& lhs,
                AggregatePODStructWrapper const& rhs) {
...
//
// A struct that allows testing whether the appropriate copy functions are
// called.
//
...
struct SwapFunctionCallTester {
  ...
  SwapFunctionCallTester() : is_custom_swap_called(false), data(0) {}
...
// The first argument (y) is assumed have the value of a value-initialized
...
int main() {
  ...
  BOOST_TEST(test(zeroInitializedAggregatePODStruct,
  ...
  static_cast<AggregatePODStruct&>(derivedFromAggregatePODStruct0) =
      zeroInitializedAggregatePODStruct;
  static_cast<AggregatePODStruct&>(derivedFromAggregatePODStruct1) =
      nonZeroInitializedAggregatePODStruct;
  BOOST_TEST(
      test(derivedFromAggregatePODStruct0, derivedFromAggregatePODStruct1));
  ...
  BOOST_TEST(std::memcmp(get(valueInitializedArrayOfBytes),
                         zeroInitializedArrayOfBytes,
                         sizeof(ArrayOfBytes)) == 0);
  ...
  boost::value_initialized<CopyFunctionCallTester> copyFunctionCallTester2 =
      boost::value_initialized<CopyFunctionCallTester>(copyFunctionCallTester1);
