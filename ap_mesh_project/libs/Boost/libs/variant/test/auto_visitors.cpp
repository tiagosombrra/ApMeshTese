...
void run_explicit() {
  ...
  BOOST_TEST(boost::apply_visitor(visitor_ref, v2).c_str() ==
             std::string("10"));
  BOOST_TEST(boost::apply_visitor(visitor_ref, v2, v1).c_str() ==
             std::string("100"));
}
...
void run() { BOOST_TEST(true); }

void run2() { BOOST_TEST(true); }

void run3() { BOOST_TEST(true); }
...
struct lex_streamer2 {
  ...
  template <class T1, class T2, class T3>
  const char* operator()(const T1& /*v1*/, const T2& /*v2*/,
                         const T3& /*v3*/) const {
    return "fail3";
  }
  ...
  std::string& operator()(const T1& v1, const T2& v2) {
    res = boost::lexical_cast<std::string>(v1) + "+" +
...
void run() {
  ...
  BOOST_TEST_IF_HAS_VARIADIC(boost::apply_visitor(lex_streamer_visitor)(v1) ==
                             "1");
  BOOST_TEST(boost::apply_visitor(lex_streamer(), v2) == "10");
  BOOST_TEST_IF_HAS_VARIADIC(boost::apply_visitor(lex_streamer_visitor)(v2) ==
                             "10");

#ifndef BOOST_NO_CXX14_GENERIC_LAMBDAS
  BOOST_TEST(boost::apply_visitor(
                 [](auto v) { return boost::lexical_cast<std::string>(v); },
                 v1) == "1");
  BOOST_TEST(boost::apply_visitor(
                 [](auto v) { return boost::lexical_cast<std::string>(v); },
                 v2) == "10");
  ...
  // BOOST_TEST(boost::apply_visitor([](auto v) { return v; }, v_diff_types) ==
  // 1);
  ...
  boost::ignore_unused(lex_streamer_visitor, visitor_ref,
...
void run2() {
  ...
  BOOST_TEST_IF_HAS_VARIADIC(
      boost::apply_visitor(lex_combine_visitor)(v2, v1) == "10+1");
  ...
  boost::apply_visitor(
      [](auto v1, auto v2) { std::cout << v1 << '+' << v2 << std::endl; }, v1,
      v2);
  boost::apply_visitor(
      [](auto v1, auto v2) { std::cout << v1 << '+' << v2 << std::endl; }, v2,
      v1);
#endif
...
void run3() {
  ...
  BOOST_TEST(boost::apply_visitor(lex_combine_visitor)(v2, v1, v3) ==
             "10+1+100");
