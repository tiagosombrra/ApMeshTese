...
class move_copy_conting_class {
 public:
  ...
  move_copy_conting_class(const move_copy_conting_class&) { ++copy_count; }
  move_copy_conting_class& operator=(
      BOOST_COPY_ASSIGN_REF(move_copy_conting_class)) {
    ++copy_count;
    return *this;
  }
};
...
void run1() { BOOST_TEST(true); }

void run_move_only() { BOOST_TEST(true); }

void run_moves_are_noexcept() { BOOST_TEST(true); }

void run_const_rvalues() { BOOST_TEST(true); }
...
void run() {
  ...
  unsigned int total_count = move_copy_conting_class::moves_count +
  ...
  v1 = var;
  // Assuring that move assignment operator moves/copyes value not more times
  // than copy assignment operator
  BOOST_TEST(total_count <= move_copy_conting_class::moves_count +
                                move_copy_conting_class::copy_count);

  move_copy_conting_class::moves_count = 0;
  move_copy_conting_class::copy_count = 0;
  v2 = boost::move(v1);
  // Assuring that `move_copy_conting_class` in v1 was moved at least once and
  // was not copied
  BOOST_TEST(move_copy_conting_class::moves_count != 0);
  BOOST_TEST(move_copy_conting_class::copy_count == 0);
  ...
  move_copy_conting_class::copy_count = 0;
  v1 = boost::move(v3);
  // Assuring that `move_copy_conting_class` in v3 was moved at least once (v1
  // and v3 have different types)
  BOOST_TEST(move_copy_conting_class::moves_count != 0);

  move_copy_conting_class::moves_count = 0;
  move_copy_conting_class::copy_count = 0;
  v2 = boost::move(v1);
  // Assuring that `move_copy_conting_class` in v1 was moved at least once (v1
  // and v3 have different types)
  BOOST_TEST(move_copy_conting_class::moves_count != 0);
  ...
  // Assuring that move constructor moves/copyes value not more times than copy
  // constructor
  BOOST_TEST(total_count <= move_copy_conting_class::moves_count +
                                move_copy_conting_class::copy_count);
}
...
struct visitor_returning_move_only_type
    : boost::static_visitor<move_only_structure> {
  template <class T>
...
void run_move_only() {
  ...
  boost::variant<int, move_only_structure> vi,
  ...
  move_only_structure from_visitor =
...
void run_moves_are_noexcept() {
  ...
  typedef boost::variant<int, short, double, move_only_structure>
      variant_except_t;
...
inline const boost::variant<int, std::string> get_variant() {
  return std::string("test");
}
inline const boost::variant<std::string, int> get_variant2() {
  return std::string("test");
}
...
struct nothrow_copyable_throw_movable {
  ...
  nothrow_copyable_throw_movable(const nothrow_copyable_throw_movable&)
      BOOST_NOEXCEPT {}
  nothrow_copyable_throw_movable& operator=(
      const nothrow_copyable_throw_movable&) BOOST_NOEXCEPT {
    return *this;
  }
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  nothrow_copyable_throw_movable(nothrow_copyable_throw_movable&&)
      BOOST_NOEXCEPT_IF(false) {}
  nothrow_copyable_throw_movable& operator=(nothrow_copyable_throw_movable&&)
      BOOST_NOEXCEPT_IF(false) {
    return *this;
  }
#endif
...
struct is_container<boost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
    : boost::mpl::bool_<is_container<T0>::value ||
                        is_container<boost::variant<
                            BOOST_VARIANT_ENUM_SHIFTED_PARAMS(T)> >::value> {};
...
#if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES) && defined(__GNUC__) && \
    (__GNUC__ == 4) && (__GNUC_MINOR__ > 6)
#ifdef BOOST_VARIANT_DO_NOT_USE_VARIADIC_TEMPLATES
  BOOST_TEST(false &&
             "Something wrong with macro definitions. GCC-4.7+ is known to "
