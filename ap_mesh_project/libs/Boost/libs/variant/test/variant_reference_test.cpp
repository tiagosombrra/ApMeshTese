...
template <typename T>
typename boost::add_reference<T>::type wknd_get(boost::variant<T&>& var, long) {
  return boost::get<T>(var);
}

template <typename T>
typename boost::add_reference<T>::type wknd_get(boost::variant<T>& var, int) {
  return boost::get<T>(var);
}
...
void base_derived_test(Derived d) {
  Base b(d);
  BOOST_TEST((check_base_derived(b, d, 1L)));

  boost::variant<Base> base_var(d);
  BOOST_TEST((check_base_derived(wknd_get(base_var, 1L), d, 1L)));

  boost::variant<Derived> derived_var(d);
  boost::variant<Base> base_from_derived_var(derived_var);
  BOOST_TEST((check_base_derived(wknd_get(base_from_derived_var, 1L),
                                 wknd_get(derived_var, 1L), 1L)));
}
