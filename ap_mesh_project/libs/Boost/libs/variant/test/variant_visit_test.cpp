...
template <typename T>
class unary_check_content_type : public boost::static_visitor<bool> {
 public:
...
class binary_check_content_type : public boost::static_visitor<bool> {
 public:
  // not recommended design, but simplifies workarounds:

  template <typename U1, typename U2>
  bool operator()(U1&, U2&) const {
    return ::boost::mpl::and_<boost::is_same<T1, U1>,
                              boost::is_same<T2, U2> >::value;
  }
};

#ifndef BOOST_NO_CXX11_REF_QUALIFIERS  // BOOST_NO_CXX11_RVALUE_REFERENCES is
                                       // not enough for disabling buggy GCCs
                                       // < 4.8
...
inline void binary_test(Variant1& var1, Variant2& var2,
                        Checker* = BOOST_VARIANT_NULL) {
  Checker checker;
  const Checker& const_checker = checker;
...
int main() {
  ...
  unary_test<check2_const_t>(cvar2);
                                       ...
                                       // not enough for disabling buggy GCCs
                                       // < 4.8
  BOOST_TEST_EQ((boost::apply_visitor(rvalue_ref_visitor(),
                                      boost::variant<udt1, udt2>(udt2()))),
                1);
#endif
#ifdef BOOST_VARIANT_HAS_DECLTYPE_APPLY_VISITOR_RETURN_TYPE
  BOOST_TEST_EQ((boost::apply_visitor(rvalue_ref_decltype_visitor(),
                                      boost::variant<udt1, udt2>(udt2()))),
                1);
#endif
