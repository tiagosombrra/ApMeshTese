...
int main() {
  ...
  {
    constexpr auto w =
        test<variant<int, float>, float>(variant<int, float>(3.0f));
    STATIC_ASSERT(w == 3.0f);
  }

  {
    constexpr auto w =
        test<variant<int, int, float>, float>(variant<int, int, float>(3.0f));
    STATIC_ASSERT(w == 3.0f);
  }
  ...
  {
    constexpr auto w = test<variant<int, int, float, float, X>, X>(
        variant<int, int, float, float, X>(X(1)));
    STATIC_ASSERT(w == 1);
  }
  ...
  {
    constexpr auto w = test<variant<int, int, float, float, Y>, Y>(
        variant<int, int, float, float, Y>(Y(1)));
    STATIC_ASSERT(w == 1);
  }
