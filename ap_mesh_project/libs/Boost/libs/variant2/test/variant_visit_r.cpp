...
struct F3 {
  ...
  auto operator()(T1 t1, T2 t2, T3 t3) const -> decltype(t1 + t2 + t3) {
    return t1 + t2 + t3;
  }
};
...
struct F4 {
  ...
  auto operator()(T1 t1, T2 t2, T3 t3, T4 t4) const
      -> decltype(t1 + t2 + t3 + t4) {
    return t1 + t2 + t3 + t4;
  }
};
...
int main() {
...

  {
    ...
    BOOST_TEST_EQ(visit<float>(F3(), v1, v2, v3),
                  static_cast<float>(1 + 3.14f + 6.28));
  }

  {
    ...
    BOOST_TEST_EQ(visit<float>(F4(), v1, v2, v3, v4),
                  static_cast<float>(1 + 3.14f + 6.28 + 'A'));
  }
