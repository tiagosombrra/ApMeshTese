...
double CurvParamHermite::CalculateCurvature(double t) {
  ...
  double v0 = 3 * pow((1 - t), 2) * (DP0.x - point0_.x) +
              6 * t * (1 - t) * (DP1.x - DP0.x) +
              3 * pow(t, 2) * (point1_.x - DP1.x);
  double v1 = 3 * pow((1 - t), 2) * (DP0.y - point0_.y) +
              6 * t * (1 - t) * (DP1.y - DP0.y) +
              3 * pow(t, 2) * (point1_.y - DP1.y);
  double v2 = 3 * pow((1 - t), 2) * (point1_.z - point0_.z) +
              6 * t * (1 - t) * (DP1.z - DP0.z) +
              3 * pow(t, 2) * (point1_.z - DP1.z);

  // B''(t)= 6(1-t)(P2-2P1+point_0_)+6t(P3-2P2+P1)
  double v00 = 6 * (1 - t) * (DP1.x - 2 * DP0.x + point0_.x) +
               6 * t * (point1_.x - 2 * DP1.x + DP0.x);
  double v10 = 6 * (1 - t) * (DP1.y - 2 * DP0.y + point0_.y) +
               6 * t * (point1_.y - 2 * DP1.y + DP0.y);
  double v20 = 6 * (1 - t) * (DP1.z - 2 * DP0.z + point0_.z) +
               6 * t * (point1_.z - 2 * DP1.z + DP0.z);
