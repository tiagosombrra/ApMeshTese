...
CurvParamHermite::CurvParamHermite() : CurveAdaptiveParametric() {
  ...
  mat_geo_gx_(2, 0) = DP0.x;
  mat_geo_gx_(3, 0) = DP1.x;
  ...
  mat_geo_gy_(2, 0) = DP0.y;
  mat_geo_gy_(3, 0) = DP1.y;
  ...
  mat_geo_gz_(2, 0) = DP0.z;
  mat_geo_gz_(3, 0) = DP1.z;
  //
...
CurvParamHermite::CurvParamHermite(Ponto p0, Ponto p1, Vetor dp0, Vetor dp1)
    : CurveAdaptiveParametric(p0, p1), DP0(dp0), DP1(dp1) {
  // 1. Preenche a matriz 'M' de 'CurvaParamétrica com a matriz de Hermite
  //
  //    M->setElement( 0, 0, 2 ); M->setElement( 0, 1,-2 ); M->setElement( 0, 2,
  //    1 ); M->setElement( 0, 3, 1 ); M->setElement( 1, 0,-3 ); M->setElement(
...
double CurvParamHermite::CalculateCurvature(double t) {
  // B'(t)= 3(1-t)²(P1-point_0_)+6t(1-t)(P2-P1)+3t²(P3-P2)
  double v0 = 3 * pow((1 - t), 2) * (DP0.x - point_0_.x) +
              6 * t * (1 - t) * (DP1.x - DP0.x) +
              3 * pow(t, 2) * (point_1_.x - DP1.x);
  double v1 = 3 * pow((1 - t), 2) * (DP0.y - point_0_.y) +
              6 * t * (1 - t) * (DP1.y - DP0.y) +
              3 * pow(t, 2) * (point_1_.y - DP1.y);
  double v2 = 3 * pow((1 - t), 2) * (point_1_.z - point_0_.z) +
              6 * t * (1 - t) * (DP1.z - DP0.z) +
              3 * pow(t, 2) * (point_1_.z - DP1.z);

  // B''(t)= 6(1-t)(P2-2P1+point_0_)+6t(P3-2P2+P1)
  double v00 = 6 * (1 - t) * (DP1.x - 2 * DP0.x + point_0_.x) +
               6 * t * (point_1_.x - 2 * DP1.x + DP0.x);
  double v10 = 6 * (1 - t) * (DP1.y - 2 * DP0.y + point_0_.y) +
               6 * t * (point_1_.y - 2 * DP1.y + DP0.y);
  double v20 = 6 * (1 - t) * (DP1.z - 2 * DP0.z + point_0_.z) +
               6 * t * (point_1_.z - 2 * DP1.z + DP0.z);
