...
std::string PMJMesh2::help(const std::string &programName)
{
...
int PMJMesh2::execute(int argc, char *argv[])
{
	PMJMesh2::MainDrive *drive = new PMJMesh2::MainDrive();
...
#endif //#if (!USE_WORKER_FILE_LOADER)
	{
		...
		static_cast<PMJMesh2::Communicator *>(drive->getComm())->setSharedParallelismEnabled(false);
		...
		drive->setPartitionerType(LOAD_BASED_BSP);

		drive->setInterfaceMeshGenerationType(A_POSTERIORI);
		...
		drive->setFinalMeshState(DISTRIBUTED);
        ...
        for (int i = 0; i < argc; i++)
        {
            ...
            else if ((std::string("-p") == argv[i]) || (std::string("--parallel") == argv[i]))
            {
                ...
                static_cast<PMJMesh2::Communicator *>(drive->getComm())->setSharedParallelismEnabled(true);
                ...
                if (i < argc - 1)
                {
                    try
                    {
                        ...
                        {
                            static_cast<PMJMesh2::Communicator *>(drive->getComm())->setSharedParallelismEnabled(true);
                            static_cast<PMJMesh2::Communicator *>(drive->getComm())->setMaxThreads(nt);
            ...
            else if ((std::string("-rlp") == argv[i]) || (std::string("--removed-layers-proportions") == argv[i]))
            {
                ...
                for (int layer = 0; (i + layer < argc - 1); layer++)
                {
                    try
                    {
            ...
            else if ((std::string("-bg") == argv[i]) || (std::string("--bsp-geometry") == argv[i]))
            {
                ...
                drive->setPartitionerType(GEOMETRY_BASED_BSP);
            }
            ...
            else if ((std::string("-apr") == argv[i]) || (std::string("--a-priori") == argv[i]))
            {
                ...
                drive->setInterfaceMeshGenerationType(A_PRIORI);
            }
            /*else if ((std::string("-api") == argv[i]) || (std::string("--a-priori-internal") == argv[i]))
            {
                used[i] = true;

                drive->setInterfaceMeshGenerationType(A_PRIORI_INTERNAL);
            }*/
            ...
            else if ((std::string("-fmj") == argv[i]) || (std::string("--final-mesh-joined") == argv[i]))
            {
                ...
                drive->setFinalMeshState(JOINED);
            }
            ...
            else if ((std::string("-fmr") == argv[i]) || (std::string("--final-mesh-redistributed") == argv[i]))
            {
                ...
                drive->setFinalMeshState(REDISTRIBUTED);
            }
#if DEBUG_CELL_COUNTER
