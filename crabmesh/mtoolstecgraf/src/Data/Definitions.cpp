...
#if USE_GUI
void Data::init(int *argc, char **argv)
#else
void Data::init(int */*argc*/, char **/*argv*/)
#endif //#if USE_GUI
{
	...
	glutInit(argc, argv);
	getState().setGlutFont(GLUT_BITMAP_9_BY_15);
#endif //#if USE_GUI
...
#if USE_POOL
#if USE_GUI
	//to avoid the deletion of the pool before the deletion of the box points
	Data::getState().setBox(Data::Box());
#endif //#if USE_GUI
...
/*#elif defined(_WINDOWS)
	//According to http://stackoverflow.com/questions/5693192/win32-backtrace-from-c-code
	//  this is how to get a backtrace in windows. Still untested.
	...
	UInt frames = CaptureStackBackTrace(0, 100, stack, NULL);
...
void Data::setState(const State &state)
{
	Data::state = state;
}
...
void Data::setTolerance(Real tolerance)
{
	Data::state.setTolerance(tolerance);
}

Real Data::getTolerance()
{
	return Data::state.getTolerance();
}
