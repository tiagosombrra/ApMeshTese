...
#include "Jmesh/JmeshPointTree.h"
...
int                jmesh_version ;
Msh3DBdryNodeList  node_list  = NULL ;
int                node_alloc = 0 ;
int                node_quantum = 1000 ;
Msh3DBdryFace      bdry_stack = 0 ;
Msh3DPolyFace      poly_stack = 0 ;
int                nintnode  = 0 ;
int                nbdrynode = 0 ;
int                GdbWait   = 0 ;
int                DebugFail = 0 ;
...
//static Msh3DBdryFace      bdry_free = 0 ;
Msh3DBdryFace      bdry_free = 0 ;
//static Msh3DBdryFace      bdry_block_ptr = 0 ;
Msh3DBdryFace      bdry_block_ptr = 0 ;
//static Msh3DBdryFace      bdry_tail = 0 ;
Msh3DBdryFace      bdry_tail = 0 ;
//static Msh3DBdryFace      bdry_last = 0 ;
Msh3DBdryFace      bdry_last = 0 ;
//static Msh3DBdryFace      bdry_cursor = 0 ;
Msh3DBdryFace      bdry_cursor = 0 ;
//static Msh3DBdryFace      bdry_try = 0 ;
Msh3DBdryFace      bdry_try = 0 ;
//static Msh3DBdryFace      bdry_use1 = 0 ;
Msh3DBdryFace      bdry_use1 = 0 ;
//static Msh3DBdryFace      bdry_use2 = 0 ;
Msh3DBdryFace      bdry_use2 = 0 ;
//static Msh3DPolyFace      poly_free = 0 ;
Msh3DPolyFace      poly_free = 0 ;
//static Msh3DPolyFace      poly_block_ptr = 0 ;
Msh3DPolyFace      poly_block_ptr = 0 ;
//static Msh3DPolyFace      poly_tail = 0 ;
Msh3DPolyFace      poly_tail = 0 ;
//static Msh3DPolyFace      poly_cursor = 0 ;
Msh3DPolyFace      poly_cursor = 0 ;
...
//markos
int num_advanced_faces = 0;
int num_total_faces = 0;
int reached_100_percent = 0;

void (*callback_advanced_faces)(int, int) = NULL;

static void call_callback_advanced_faces()
{
    if (!callback_advanced_faces)
    {
        return;
    }
    ...
    (*callback_advanced_faces)(num_advanced_faces, num_total_faces);

    if (num_advanced_faces == num_total_faces)
    {
 ...
 void (*callback_advanced_faces_ptr)(int, int)) //added by markos
{
    ...
    //markos
    num_advanced_faces = 0;
    num_total_faces = 0;
    reached_100_percent = 0;
    callback_advanced_faces = callback_advanced_faces_ptr;
    //endmarkos
...
//fprintf (stderr,"markos debug, setting metrics = %f, %f, %f, metric = %f\n", node_list[id].coord[0], node_list[id].coord[1], node_list[id].coord[2], metric);
	  ...
	  if ( node_indx == -1 ) {
           if (face->use == 2) {
             ...
             if ( Msh3DCorrePoly (&num_nodes, num_gen_elements,
                                 generated_elements, 1, 1, 0, 1) ) break ;
...
/* 2.6 smooth the mesh considering all elements done. Also correct bad
   ...
   do {
    ...
    Msh3DSmooth( num_nodes, bdr_nodes, *num_gen_elements, *generated_elements ) ;
  ...
  int                step )
{
 ...
 face = bdry_stack ;
 Msh3DFindPoly (face, face_face, face_node, &nface_face, &nface_node, 1) ;
 poly = poly_stack ;
  ...
  for (i = 0; i < 3; i++) {
   ...
   if (!found_node) {
    ...
    for (cur_face = node_list[ve].faces; cur_face; cur_face = cur_face->next)
     Msh3DFindPolyNodes (cur_face->face, face_face, face_node, nface_face, nface_node) ;
   }
  }
 }
}
 ...
 *nface_face = *nface_node = 0 ;
 Msh3DFindPolyNodes (face, face_face, face_node, nface_face, nface_node) ;

 /* find the polyhedron */

 Msh3DFindPolyBuild (face, face_node, nface_node, shp) ;
}
 ...
 if (!found_face) {
  for (i = 0; i < 3; i++) {
   ...
   if (!found_node) {
    face_node[(*nface_node)++] = ve ;
    for (cur_face = node_list[ve].faces; cur_face; cur_face = cur_face->next)
     Msh3DRecoPolyNodes (cur_face->face, face_face, face_node, nface_face, nface_node) ;
   }
  }
 }
 ...
 *nface_face = *nface_node = 0 ;
 Msh3DRecoPolyNodes (face, face_face, face_node, nface_face, nface_node) ;
...
static int Msh3DDelRepeatedFace (void)
{
  ...
  if (recon) {
   ...
   poly_stack = poly_tail = NULL ;
   Msh3DRecoPoly (new_bdry, face_face, face_node, &nface_face, &nface_node, 1) ;
  }
    ...
    int               *num_elems )
{
    ...
    //markos
    num_advanced_faces += 1;
    //endmarkos

    //markos
    call_callback_advanced_faces();
    //end markos
}
  ...
  while (bdry_stack) {
    ...
    if (!shp) {
     ...
     if ( jmesh_version == SMALL_VERSION ) /* Use improved find for SMALL */
      Msh3DFindPoly (face,face_face,face_node,&nface_face,&nface_node,shp) ;
     else                                  /* Use standard find for ADVAN */
      Msh3DRecoPoly (face,face_face,face_node,&nface_face,&nface_node,shp) ;
#else
     Msh3DFindPoly (face,face_face,face_node,&nface_face,&nface_node,shp) ;
#endif
    }
...
#if VER_BOTH
     Msh3DCrtnFDelF (visi, *elems, nelems, corre, shp) ;
     corre++ ;
     diare++ ;
     if (!shp) {
...
//fprintf (stderr,"markos debug, improvement back tracking failed, tried to create too many elements\n") ;
   ...
   Msh3DAddPolyFaces( poly, poly->face, node_list, node_indx, nelems ) ;
 ...
 int           shp )
{
 ...
 if (!shp) {
  if (Msh3DCreationFace( v, elem, nelem )) {
   ...
   while (cur_poly = Msh3DPolyNext ()) {
    ...
    if (!Msh3DCreationFace( w, elem, nelem )) {
 ...
 while (Msh3DNoElemFace (visi, v, elem, nelem))
 {
  ...
  poly_stack = poly_tail = NULL ;
  face = visi ;
  Msh3DRecoPoly (face, face_face, face_node, &nface_face, &nface_node, 1) ;
#if DIA_RECO
...
//fprintf (stderr,"markos debug, back tracking, elem = %d will be improved by back tracking\n", l) ;
    ...
    *shape = 1 ;
    ...
    star_poly = Msh3DVisiPoly (nnodes, nelems, elems, *step) ;
...
static void Msh3DFaceFreeAll(void)
{
...
static Msh3DBdryFace Msh3DBdryPop(void)
{
    ...
    popped->next = popped->prev = 0 ;
#else
    /* Update bdry_use1 and bdry_use2 if it is the case */
    if (!bdry_stack) return(0) ;
