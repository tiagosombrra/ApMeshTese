...
Real &Numerical::LMatrix::element(UInt i)
{
    ...
    UInt j = i - this->Numerical::ILowerTriangularMatrix::numElements(i2);
...
{
    UInt numElems = this->Numerical::IUpperTriangularMatrix::numElements(this->numRows());
    UInt numRows = this->Numerical::IUpperTriangularMatrix::numRows(numElems - 1 - i);

    UInt i2 = this->numRows() - numRows - 1;
    UInt j = i - i2*(numRows - 1) + this->Numerical::IUpperTriangularMatrix::numElements(i2 - 1);
