...
//#include "TecGraf/Msh3D/MainDrive.h"

std::string TecGraf::Msh3D::help(const std::string &/*programName*/)
{
	std::stringstream str;

	/*str << "Usage: " << programName << " TecGrafMsh3D <input_filename> <options>\n"
		<< "Options:\n"
		<< "-o (--output) <output_filename>: Save output file (.txt or .dat)\n"
		<< "default: Do not produce output\n"
		<< "\n";*/

    str << "Module unavailable (for now)\n";

	return str.str();
}

int TecGraf::Msh3D::execute(int /*argc*/, char */*argv*/[])
{
	/*TecGraf::Msh3D::MainDrive *drive = new TecGraf::Msh3D::MainDrive();
