...
void array(const MatrixType& m) {
  ...
  */
  ...
  VERIFY_IS_APPROX(m1.colwise().sum(),
                   m1.colwise().redux(internal::scalar_sum_op<Scalar>()));
}
...
void comparisons(const MatrixType& m) {
  ...
  Scalar mid =
      (m1.cwise().abs().minCoeff() + m1.cwise().abs().maxCoeff()) / Scalar(2);
  for (int j = 0; j < cols; ++j)
    for (int i = 0; i < rows; ++i)
      m3(i, j) = ei_abs(m1(i, j)) < mid ? 0 : m1(i, j);
  VERIFY_IS_APPROX(
      (m1.cwise().abs().cwise() < MatrixType::Constant(rows, cols, mid))
  ...
  // count
  VERIFY(((m1.cwise().abs().cwise() + 1).cwise() > RealScalar(0.1)).count() ==
         rows * cols);
  VERIFY_IS_APPROX(((m1.cwise().abs().cwise() + 1).cwise() > RealScalar(0.1))
                       .colwise()
                       .count()
                       .template cast<int>(),
                   RowVectorXi::Constant(cols, rows));
  VERIFY_IS_APPROX(((m1.cwise().abs().cwise() + 1).cwise() > RealScalar(0.1))
                       .rowwise()
                       .count()
                       .template cast<int>(),
                   VectorXi::Constant(rows, cols));
}
...
void lpNorm(const VectorType& v) {
  ...
  VERIFY_IS_APPROX(u.template lpNorm<2>(),
                   ei_sqrt(u.cwise().abs().cwise().square().sum()));
  VERIFY_IS_APPROX(
      ei_pow(u.template lpNorm<5>(), typename VectorType::RealScalar(5)),
      u.cwise().abs().cwise().pow(5).sum());
}
