...
void map_class_vector(const VectorType& m) {
  ...
  Map<VectorType>(array3unaligned, size) =
      Map<VectorType>((const Scalar*)array1,
                      size);  // test non-const-correctness support in eigen2
...
void map_class_matrix(const MatrixType& m) {
  ...
  Map<MatrixType>(array2, rows, cols) =
      Map<MatrixType>((const Scalar*)array1, rows,
                      cols);  // test non-const-correctness support in eigen2
  Map<MatrixType>(array3unaligned, rows, cols) =
      Map<MatrixType>(array1, rows, cols);
...
void test_eigen2_map() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_4(map_class_matrix(
        MatrixXcf(ei_random<int>(1, 10), ei_random<int>(1, 10))));
    CALL_SUBTEST_5(map_class_matrix(
        MatrixXi(ei_random<int>(1, 10), ei_random<int>(1, 10))));
