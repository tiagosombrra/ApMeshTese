...
void makeNoisyCohyperplanarPoints(int numPoints, VectorType **points,
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    } while (cur_point.norm() < 0.5 || cur_point.norm() > 2.0);
  }
...
void check_linearRegression(int numPoints, VectorType **points,
  ...
  typename VectorType::Scalar error =
      (result - original).norm() / original.norm();
  VERIFY(ei_abs(error) < ei_abs(tolerance));
}

template <typename VectorType, typename HyperplaneType>
void check_fitHyperplane(int numPoints, VectorType **points,
  ...
  result.coeffs() *=
      original.coeffs().coeff(size) / result.coeffs().coeff(size);
  typename VectorType::Scalar error =
      (result.coeffs() - original.coeffs()).norm() / original.coeffs().norm();
...
void test_eigen2_regression() {
  for (int i = 0; i < g_repeat; i++) {
...
#ifdef EIGEN_TEST_PART_4
    {
      ...
      Hyperplane<std::complex<double>, Dynamic> *coeffs12cd =
          new Hyperplane<std::complex<double>, Dynamic>(11);
      makeNoisyCohyperplanarPoints(1000, points11cd_ptrs, coeffs12cd, 0.01);
      CALL_SUBTEST(
          check_fitHyperplane(100, points11cd_ptrs, *coeffs12cd, 0.025));
      CALL_SUBTEST(
          check_fitHyperplane(1000, points11cd_ptrs, *coeffs12cd, 0.006));
      delete coeffs12cd;
