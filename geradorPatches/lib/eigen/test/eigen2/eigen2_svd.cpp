...
void svd(const MatrixType& m) {
  ...
  */
  ...
  if (ei_is_same_type<RealScalar, float>::ret) largerEps = 1e-3f;
  ...
  if (rows == cols) {
    ...
    VERIFY_IS_APPROX(unitary * unitary.adjoint(),
                     MatrixType::Identity(unitary.rows(), unitary.rows()));
    VERIFY_IS_APPROX(positive, positive.adjoint());
    for (int i = 0; i < rows; i++)
      VERIFY(positive.diagonal()[i] >=
             0);  // cheap necessary (not sufficient) condition for positivity
