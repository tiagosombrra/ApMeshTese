...
void sparse_basic(const SparseMatrixType& ref) {
  ...
  // test innerVector()
  {
    ...
    if (SparseMatrixType::IsRowMajor)
      VERIFY_IS_APPROX(m2.innerVector(j0) + m2.innerVector(j1),
                       refMat2.row(j0) + refMat2.row(j1));
    else
      VERIFY_IS_APPROX(m2.innerVector(j0) + m2.innerVector(j1),
                       refMat2.col(j0) + refMat2.col(j1));

    SparseMatrixType m3(rows, rows);
    m3.reserve(VectorXi::Constant(rows, rows / 2));
    for (Index j = 0; j < rows; ++j)
      for (Index k = 0; k < j; ++k) m3.insertByOuterInner(j, k) = k + 1;
    for (Index j = 0; j < rows; ++j) {
  ...
  // test generic blocks
  {
    ...
    VERIFY_IS_APPROX((2 * m2).block(j0, j1, n0, n0),
                     (2 * refMat2).block(j0, j1, n0, n0));
  }
...
void test_sparse_basic() {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_2(
        (sparse_basic(SparseMatrix<std::complex<double>, ColMajor>(s, s))));
    CALL_SUBTEST_2(
        (sparse_basic(SparseMatrix<std::complex<double>, RowMajor>(s, s))));
    CALL_SUBTEST_1((sparse_basic(SparseMatrix<double>(s, s))));
    CALL_SUBTEST_1(
        (sparse_basic(SparseMatrix<double, ColMajor, long int>(s, s))));
    CALL_SUBTEST_1(
        (sparse_basic(SparseMatrix<double, RowMajor, long int>(s, s))));

    CALL_SUBTEST_1((sparse_basic(
        SparseMatrix<double, ColMajor, short int>(short(s), short(s)))));
    CALL_SUBTEST_1((sparse_basic(
        SparseMatrix<double, RowMajor, short int>(short(s), short(s)))));
  }
}
