...
bool test_assign(const Dst&, const Src&, int traversal, int unrolling) {
  ...
  if (!res) {
    std::cerr << " Expected Traversal == " << demangle_traversal(traversal)
              << " got "
              << demangle_traversal(
                     internal::assign_traits<Dst, Src>::Traversal)
              << "\n";
    std::cerr << " Expected Unrolling == " << demangle_unrolling(unrolling)
              << " got "
              << demangle_unrolling(
                     internal::assign_traits<Dst, Src>::Unrolling)
              << "\n";
  }
  return res;
}
...
bool test_redux(const Xpr&, int traversal, int unrolling) {
  typedef internal::redux_traits<internal::scalar_sum_op<typename Xpr::Scalar>,
                                 Xpr>
      traits;
...
struct vectorization_logic {
  ...
  static void run() {
    ...
    VERIFY(test_assign(Matrix44u(), Matrix44() + Matrix44(), LinearTraversal,
                       NoUnrolling));

    VERIFY(test_assign(Matrix1u(), Matrix1() + Matrix1(), LinearTraversal,
                       CompleteUnrolling));
    ...
    VERIFY(
        (test_assign<Map<Matrix22, Aligned, OuterStride<3 * PacketSize> >,
                     Matrix22>(InnerVectorizedTraversal, CompleteUnrolling)));

    VERIFY((test_assign<Map<Matrix22, Aligned, InnerStride<3 * PacketSize> >,
                        Matrix22>(DefaultTraversal, CompleteUnrolling)));
...
void test_vectorization_logic() {
  ...
  if (internal::packet_traits<float>::Vectorizable) {
    ...
    VERIFY(
        test_redux(Matrix<float, 5, 2>(), DefaultTraversal, CompleteUnrolling));
  }
  ...
  if (internal::packet_traits<double>::Vectorizable) {
    ...
    VERIFY(test_redux(Matrix<double, 7, 3>(), DefaultTraversal,
                      CompleteUnrolling));
  }
