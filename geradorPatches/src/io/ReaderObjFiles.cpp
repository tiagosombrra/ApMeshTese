        ...
        while (fin)
        {
            ...
            while (getline(fin, line))
            {
                ...
                if(cont == 0){
                    ...
                    if (!change) {
                        iss >> v[0]; iss >> v[1]; iss >> v[2]; iss >> v[3];
                        iss >> v[4]; iss >> v[5]; iss >> v[6]; iss >> v[7];
                        iss >> v[8]; iss >> v[9]; iss >> v[10]; iss >> v[11];
                        iss >> v[12]; iss >> v[13]; iss >> v[14]; iss >> v[15];
                        vecIdsPatches.push_back(v[0]); vecIdsPatches.push_back(v[1]); vecIdsPatches.push_back(v[2]);
                        vecIdsPatches.push_back(v[3]); vecIdsPatches.push_back(v[4]); vecIdsPatches.push_back(v[5]);
                        vecIdsPatches.push_back(v[6]); vecIdsPatches.push_back(v[7]); vecIdsPatches.push_back(v[8]);
                        vecIdsPatches.push_back(v[9]); vecIdsPatches.push_back(v[10]); vecIdsPatches.push_back(v[11]);
                        vecIdsPatches.push_back(v[12]); vecIdsPatches.push_back(v[13]); vecIdsPatches.push_back(v[14]);
                        vecIdsPatches.push_back(v[15]);
                    } else if(change ){
                        iss >> vv[0]; iss >> vv[1]; iss >> vv[2];
...
//        value_x = value_x + 3;
    ...
    //                for (VectorTuplePoints::iterator it = vectorPoints.begin() ; it != vectorPoints.end(); ++it) {
...
VectorTuple48d ReaderObjFiles::fileReadingOBJ(string siqInput)
{
    ...
    sort(vectorPoints.begin(),vectorPoints.end(),[](const TuplePoint &lhs, const TuplePoint &rhs) -> bool
    {
    ...
    if (fin)
    {
        while (fin)
        {
            ...
            while (getline(fin, line))
            {
                ...
                istringstream iss(line);
                iss >> vector48d(9, 0); iss >> vector48d(10, 0); iss >> vector48d(11, 0);
                iss >> vector48d(6, 0); iss >> vector48d(7, 0); iss >> vector48d(8, 0);
                iss >> vector48d(3, 0); iss >> vector48d(4, 0); iss >> vector48d(5, 0);
                iss >> vector48d(0, 0); iss >> vector48d(1, 0); iss >> vector48d(2, 0);
                
                iss >> vector48d(21, 0); iss >> vector48d(22, 0); iss >> vector48d(23, 0);
                iss >> vector48d(18, 0); iss >> vector48d(19, 0); iss >> vector48d(20, 0);
                iss >> vector48d(15, 0); iss >> vector48d(16, 0); iss >> vector48d(17, 0);
                iss >> vector48d(12, 0); iss >> vector48d(13, 0); iss >> vector48d(14, 0);
                
                iss >> vector48d(33, 0); iss >> vector48d(34, 0); iss >> vector48d(35, 0);
                iss >> vector48d(30, 0); iss >> vector48d(31, 0); iss >> vector48d(32, 0);
                iss >> vector48d(27, 0); iss >> vector48d(28, 0); iss >> vector48d(29, 0);
                iss >> vector48d(24, 0); iss >> vector48d(25, 0); iss >> vector48d(26, 0);
                
                iss >> vector48d(45, 0); iss >> vector48d(46, 0); iss >> vector48d(47, 0);
                iss >> vector48d(42, 0); iss >> vector48d(43, 0); iss >> vector48d(44, 0);
                iss >> vector48d(39, 0); iss >> vector48d(40, 0); iss >> vector48d(41, 0);
                iss >> vector48d(36, 0); iss >> vector48d(37, 0); iss >> vector48d(38, 0);
...
bool ReaderObjFiles::checkOrientationOfPatches()
{
    ...
    for (VectorTuplePoints::iterator it = vectorPoints.begin() ; it != vectorPoints.end(); ++it) {
        ...
        if (vecIdPatchPoint.size() > 0) {
            ...
            for (vector<IdPatchPoint>::iterator it = vecIdPatchPoint.begin() ; it != vecIdPatchPoint.end(); ++it) {

                if (!verifyNeighbor(id_point, (*it).second)) {
                    cout<<"Erro de orientação no ponto: "<<point3D<<endl;
                    cout<<"patch_orig: "<<id_patch<<" point_orig: "<<id_point<<endl;
                    cout<<"patch_dest: "<<(*it).first<<" point_dest: "<<(*it).second<<endl;
...
VectorTuple48d ReaderObjFiles::convertVectorTuplePoints(VectorTuplePoints vecTuplePoints){
    ...
    for (unsigned int i = 0; i < vecTuplePoints.size(); i = i + 16) {
        ...
        vector48d << get<2>(vecTuplePoints.at(i)), get<2>(vecTuplePoints.at(i+1)), get<2>(vecTuplePoints.at(i+2)), get<2>(vecTuplePoints.at(i+3)),
                get<2>(vecTuplePoints.at(i+4)), get<2>(vecTuplePoints.at(i+5)), get<2>(vecTuplePoints.at(i+6)), get<2>(vecTuplePoints.at(i+7)),
                get<2>(vecTuplePoints.at(i+8)), get<2>(vecTuplePoints.at(i+9)), get<2>(vecTuplePoints.at(i+10)), get<2>(vecTuplePoints.at(i+11)),
                get<2>(vecTuplePoints.at(i+12)), get<2>(vecTuplePoints.at(i+13)), get<2>(vecTuplePoints.at(i+14)), get<2>(vecTuplePoints.at(i+15));
    ...
    //    for (VectorTuple16d::iterator it = vecTuple16d.begin() ; it != vecTuple16d.end(); ++it) {
    //
    //        cout<<"id do patch: "<<get<0>((*it))<< " cood: "<<get<1>((*it))<< endl;
    //    }
...
VectorTuplePoints ReaderObjFiles::convertVectorTuplePoints(VectorTuple48d vecTuplePoints48)
{
    VectorTuplePoints vecTuplePoints;

    for (VectorTuple48d::iterator it = vecTuplePoints48.begin() ; it != vecTuplePoints48.end(); ++it) {
    ...
    //                for (VectorTuplePoints::iterator it = vecTuplePoints.begin() ; it != vecTuplePoints.end(); ++it) {
...
BP ReaderObjFiles::convertBP(VectorTuple48d vectorTuple48d)
{
    ...
    for (VectorTuple48d::iterator it = vectorTuple48d.begin() ; it != vectorTuple48d.end(); ++it) {
...
int ReaderObjFiles::findControlPointMap(MapControlPoints mapControlPoints, Vector3d vec3d)
{
    for (MapControlPoints::const_iterator it = mapControlPoints.begin(); it != mapControlPoints.end(); it ++)
    {
        if (it->second == vec3d)
        {
            return it->first;
