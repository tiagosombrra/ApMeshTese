...
class IterativeSolverBase : internal::noncopyable {
 ...
 public:
   ...
   * In the future we might, for instance, implement column reodering for faster
   ...
   * the future we might, for instance, implement column reodering for faster
   * matrix vector products.
   *
   * \warning this class stores a reference to the matrix A as well as some
   * precomputed values that depend on it. Therefore, if \a A is changed
 ...
 protected:
  ...
  void grabInput(const EigenBase<InputDerived>& A) {
    // we const cast to prevent the creation of a MatrixType temporary by the
    // compiler.
