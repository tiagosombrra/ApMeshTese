...
ColPivHouseholderQR<MatrixType>& ColPivHouseholderQR<MatrixType>::compute(
  ...
  for (Index k = 0; k < size; ++k) {
    ...
    // must now recompute the actual squared norm of the selected column. Note
    // that not doing so does result in solve() sometimes returning inf/nan
    // values when running the unit test with 1000 repetitions.
    biggest_col_sq_norm =
