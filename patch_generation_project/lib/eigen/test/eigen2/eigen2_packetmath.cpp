...
template <>
const complex<float>& min(const complex<float>& a, const complex<float>& b) {
...
void packetmath() {
  ...
  for (int offset = 0; offset < PacketSize; ++offset) {
    ...
    if (offset == 0)
      ei_palign<0>(packets[0], packets[1]);
    else if (offset == 1)
      ei_palign<1>(packets[0], packets[1]);
    else if (offset == 2)
      ei_palign<2>(packets[0], packets[1]);
    else if (offset == 3)
      ei_palign<3>(packets[0], packets[1]);
  ...
  if (!ei_is_same_type<Scalar, int>::ret) CHECK_CWISE(REF_DIV, ei_pdiv);
#endif
