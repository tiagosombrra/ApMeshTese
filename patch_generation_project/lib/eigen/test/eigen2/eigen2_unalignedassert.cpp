...
struct Good8 {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  float f;  // try the f at first -- the EIGEN_ALIGN_128 attribute of m should
...
void check_unalignedassert_bad() {
  ...
  float *unaligned = buf;
  while ((reinterpret_cast<std::size_t>(unaligned) & 0xf) == 0)
    ++unaligned;  // make sure unaligned is really unaligned
...
void test_eigen2_unalignedassert() { CALL_SUBTEST(unalignedassert()); }
