...
struct GslTraits {
  ...
  static void prod(const Matrix& m, const Vector& v, Vector& x) {
    gsl_blas_dgemv(CblasNoTrans, 1, m, v, 0, x);
  }
  ...
  static void eigen_symm_gen(const Matrix& m, const Matrix& _b, Vector& eval,
                             Matrix& evec) {
...
struct GslTraits<Scalar, true> {
  ...
  static void cholesky_solve(const Matrix& m, const Vector& b, Vector& x) {
    gsl_linalg_complex_cholesky_solve(m, b, x);
  }
  static void prod(const Matrix& m, const Vector& v, Vector& x) {
    gsl_blas_zgemv(CblasNoTrans, gsl_complex_rect(1, 0), m, v,
                   gsl_complex_rect(0, 0), x);
  }
  ...
  static void eigen_symm_gen(const Matrix& m, const Matrix& _b,
                             gsl_vector*& eval, Matrix& evec) {
...
void convert(const MatrixType& m, gsl_matrix*& res) {
  ...
  for (int i = 0; i < m.rows(); ++i)
    for (int j = 0; j < m.cols(); ++j) gsl_matrix_set(res, i, j, m(i, j));
}
...
void convert(const gsl_matrix* m, MatrixType& res) {
  ...
  for (int i = 0; i < res.rows(); ++i)
    for (int j = 0; j < res.cols(); ++j) res(i, j) = gsl_matrix_get(m, i, j);
}
...
void convert(const VectorType& m, gsl_vector*& res) {
  ...
  for (int i = 0; i < m.size(); ++i) gsl_vector_set(res, i, m[i]);
}

template <typename VectorType>
void convert(const gsl_vector* m, VectorType& res) {
  res.resize(m->size);
  for (int i = 0; i < res.rows(); ++i) res[i] = gsl_vector_get(m, i);
}
...
void convert(const gsl_vector_complex* m, VectorType& res) {
  ...
  for (int i = 0; i < res.rows(); ++i)
    res[i] =
        typename VectorType::Scalar(GSL_REAL(gsl_vector_complex_get(m, i)),
