...
#define VERIFY(a)                                                    \
  do {                                                               \
    if (!(a)) {                                                      \
      std::cerr << "Test " << g_test_stack.back()                    \
                << " failed in " EI_PP_MAKE_STRING(__FILE__) << " (" \
                << EI_PP_MAKE_STRING(__LINE__) << ")" << std::endl   \
                << "    " << EI_PP_MAKE_STRING(a) << std::endl       \
                << std::endl;                                        \
      abort();                                                       \
    }                                                                \
  } while (0)
...
inline bool test_ei_isApprox(const std::complex<float>& a,
                             const std::complex<float>& b) {
  return ei_isApprox(a, b, test_precision<std::complex<float> >());
}
inline bool test_ei_isMuchSmallerThan(const std::complex<float>& a,
                                      const std::complex<float>& b) {
  return ei_isMuchSmallerThan(a, b, test_precision<std::complex<float> >());
}

inline bool test_ei_isApprox(const std::complex<double>& a,
                             const std::complex<double>& b) {
  return ei_isApprox(a, b, test_precision<std::complex<double> >());
}
inline bool test_ei_isMuchSmallerThan(const std::complex<double>& a,
                                      const std::complex<double>& b) {
...
inline bool test_ei_isMuchSmallerThan(const long double& a,
                                      const long double& b) {
  return ei_isMuchSmallerThan(a, b, test_precision<long double>());
}
inline bool test_ei_isApproxOrLessThan(const long double& a,
                                       const long double& b) {
...
inline bool test_ei_isMuchSmallerThan(
