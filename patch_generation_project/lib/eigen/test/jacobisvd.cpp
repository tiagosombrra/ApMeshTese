...
template <typename MatrixType, int QRPreconditioner>
void jacobisvd_compare_to_full(
    const MatrixType& m, unsigned int computationOptions,
...
void jacobisvd_solve(const MatrixType& m, unsigned int computationOptions) {
  ...
  // Check that there is no significantly better solution in the neighborhood of
  // x
  if (!test_isMuchSmallerThan(residual, rhs.norm())) {
    // If the residual is very small, then we have an exact solution, so we are
    // already good.
  ...
  // check minimal norm solutions
  {
    ...
    Index rank = RankAtCompileTime2 == Dynamic
                     ? internal::random<Index>(1, cols)
                     : Index(RankAtCompileTime2);
    MatrixType2 m2(rank, cols);
    int guard = 0;
    do {
      m2.setRandom();
    } while (m2.jacobiSvd().setThreshold(test_precision<Scalar>()).rank() !=
                 rank &&
             (++guard) < 10);
    VERIFY(guard < 10);
    ...
    // now check with SVD
    JacobiSVD<MatrixType2, ColPivHouseholderQRPreconditioner> svd2(
        m2, computationOptions);
    ...
    JacobiSVD<MatrixType3, ColPivHouseholderQRPreconditioner> svd3(
        m3, computationOptions);
...
void jacobisvd_test_all_computation_options(const MatrixType& m) {
  if (QRPreconditioner == NoQRPreconditioner && m.rows() != m.cols()) return;
  JacobiSVD<MatrixType, QRPreconditioner> fullSvd(m,
                                                  ComputeFullU | ComputeFullV);
  CALL_SUBTEST((jacobisvd_check_full(m, fullSvd)));
  CALL_SUBTEST((jacobisvd_solve<MatrixType, QRPreconditioner>(
...
#endif
  if (QRPreconditioner == FullPivHouseholderQRPreconditioner) return;
...
void jacobisvd(const MatrixType& a = MatrixType(), bool pickrandom = true) {
  ...
  if (pickrandom) {
    ...
    for (Index k = 0; k < diagSize; ++k)
      d(k) =
          d(k) * std::pow(RealScalar(10), internal::random<RealScalar>(-s, s));
    m = Matrix<Scalar, Dynamic, Dynamic>::Random(a.rows(), diagSize) *
        d.asDiagonal() *
        Matrix<Scalar, Dynamic, Dynamic>::Random(diagSize, a.cols());
    // cancel some coeffs
    Index n = internal::random<Index>(0, m.size() - 1);
    for (Index i = 0; i < n; ++i)
      m(internal::random<Index>(0, m.rows() - 1),
        internal::random<Index>(0, m.cols() - 1)) = Scalar(0);
  }

  CALL_SUBTEST((jacobisvd_test_all_computation_options<
                MatrixType, FullPivHouseholderQRPreconditioner>(m)));
  CALL_SUBTEST((jacobisvd_test_all_computation_options<
                MatrixType, ColPivHouseholderQRPreconditioner>(m)));
  CALL_SUBTEST(
      (jacobisvd_test_all_computation_options<MatrixType,
                                              HouseholderQRPreconditioner>(m)));
  CALL_SUBTEST(
      (jacobisvd_test_all_computation_options<MatrixType, NoQRPreconditioner>(
          m)));
}
