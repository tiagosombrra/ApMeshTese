...
bool isApproxAbs(const Scalar& a, const Scalar& b,
...
bool areApproxAbs(const Scalar* a, const Scalar* b, int size,
  ...
  for (int i = 0; i < size; ++i) {
    if (!isApproxAbs(a[i], b[i], refvalue)) {
      std::cout << "[" << Map<const Matrix<Scalar, 1, Dynamic> >(a, size) << "]"
...
#define CHECK_CWISE2(REFOP, POP)                                               \
  {                                                                            \
    ...
    internal::pstore(data2, POP(internal::pload<Packet>(data1),                \
                                internal::pload<Packet>(data1 + PacketSize))); \
...
void packetmath_real() {
  ...
  CHECK_CWISE1_IF(internal::packet_traits<Scalar>::HasSin, std::sin,
                  internal::psin);
  CHECK_CWISE1_IF(internal::packet_traits<Scalar>::HasCos, std::cos,
                  internal::pcos);
  CHECK_CWISE1_IF(internal::packet_traits<Scalar>::HasTan, std::tan,
                  internal::ptan);
  ...
  CHECK_CWISE1_IF(internal::packet_traits<Scalar>::HasASin, std::asin,
                  internal::pasin);
  CHECK_CWISE1_IF(internal::packet_traits<Scalar>::HasACos, std::acos,
                  internal::pacos);
  ...
  CHECK_CWISE1_IF(internal::packet_traits<Scalar>::HasExp, std::exp,
                  internal::pexp);
  {
  ...
  CHECK_CWISE1_IF(internal::packet_traits<Scalar>::HasSqrt, std::sqrt,
                  internal::psqrt);
  CHECK_CWISE1_IF(internal::packet_traits<Scalar>::HasLog, std::log,
                  internal::plog);
  {
    data1[0] = std::numeric_limits<Scalar>::quiet_NaN();
...
void packetmath_notcomplex() {
  ...
  Array<Scalar, Dynamic, 1>::Map(data1,
