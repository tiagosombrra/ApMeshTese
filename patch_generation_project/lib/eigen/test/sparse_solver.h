...
void check_sparse_solving(Solver& solver, const typename Solver::MatrixType& A,
  ...
  DenseRhs refX = dA.lu().solve(db);
  {
    ...
    if (solver.info() != Success) {
      std::cerr << "sparse solver testing: factorization failed "
  ...
  for (; it; ++it) {
    if (it.sym() == SPD) {
      ...
      halfA.template selfadjointView<Solver::UpLo>() =
          it.matrix().template triangularView<Eigen::Lower>().twistedBy(pnull);
...
template <typename Solver, typename DenseMat>
int generate_sparse_square_problem(Solver&, typename Solver::MatrixType& A,
                                   DenseMat& dA, int maxSize = 300) {
  typedef typename Solver::MatrixType Mat;
  typedef typename Mat::Scalar Scalar;
...
template <typename Solver>
void check_sparse_square_solving(Solver& solver, bool checkDeficient = false) {
  typedef typename Solver::MatrixType Mat;
  typedef typename Mat::Scalar Scalar;
  typedef SparseMatrix<Scalar, ColMajor> SpMat;
  typedef Matrix<Scalar, Dynamic, Dynamic> DenseMatrix;
