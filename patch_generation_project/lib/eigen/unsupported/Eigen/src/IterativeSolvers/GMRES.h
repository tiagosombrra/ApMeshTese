...
bool gmres(const MatrixType& mat, const Rhs& rhs, Dest& x,
           const Preconditioner& precond, int& iters, const int& restart,
  ...
  for (int k = 1; k <= restart; ++k) {
    ...
    if (v.tail(m - k).norm() != 0.0) {
      if (k <= restart) {
        ...
        v.tail(m - k).applyHouseholderOnTheLeft(
    ...
    if (stop || k == restart) {
      ...
      x_new.tail(m - k + 1).applyHouseholderOnTheLeft(
 ...
 public:
  ...
  solveWithGuess(const MatrixBase<Rhs>& b, const Guess& x0) const {
    ...
    eigen_assert(Base::rows() == b.rows() &&
                 "GMRES::solve(): invalid number of rows of the right hand "
                 "side matrix b");
    return internal::solve_retval_with_guess<GMRES, Rhs, Guess>(
        *this, b.derived(), x0);
  }
  ...
  void _solveWithGuess(const Rhs& b, Dest& x) const {
    ...
    for (int j = 0; j < b.cols(); ++j) {
      ...
      if (!internal::gmres(*mp_matrix, b.col(j), xj, Base::m_preconditioner,
