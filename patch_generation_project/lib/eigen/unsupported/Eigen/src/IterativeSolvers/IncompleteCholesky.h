 ...
 *                     matrix. It is advised to give  a row-oriented sparse
 * matrix \tparam _UpLo The triangular part of the matrix to reference. \tparam
 * _OrderingType
 */

template <typename Scalar, int _UpLo = Lower,
          typename _OrderingType = NaturalOrdering<int> >
class IncompleteCholesky : internal::noncopyable {
 ...
 public:
  ...
  IncompleteCholesky(const MatrixType& matrix)
      : m_shift(1), m_factorizationIsOk(false) {
    compute(matrix);
  }
  ...
  void _solve(const Rhs& b, Dest& x) const {
    ...
    if (m_perm.rows() == b.rows()) x = m_perm * x;
    x = m_scal.asDiagonal() * x;
  }
  template <typename Rhs>
  inline const internal::solve_retval<IncompleteCholesky, Rhs> solve(
      const MatrixBase<Rhs>& b) const {
    eigen_assert(m_factorizationIsOk && "IncompleteLLT did not succeed");
    eigen_assert(m_isInitialized && "IncompleteLLT is not initialized.");
    eigen_assert(cols() == b.rows() &&
                 "IncompleteLLT::solve(): invalid number of rows of the right "
                 "hand side matrix b");
 ...
 private:
  ...
  inline void updateList(const IdxType& colPtr, IdxType& rowIdx, SclType& vals,
                         const Index& col, const Index& jk, IndexType& firstElt,
                         VectorList& listCol);
};
...
void IncompleteCholesky<Scalar, _UpLo, OrderingType>::factorize(
  ...
  // Dropping strategies : Keep only the p largest elements per column, where p
  ...
  Map<IndexType> colPtr(m_L.outerIndexPtr(),
                        n + 1);  // Pointer to the beginning of each row
  IndexType firstElt(n - 1);     // for each j, points to the next entry in vals
  ...
  for (int j = 0; j < n; j++) vals[colPtr[j]] += m_shift;
  ...
  for (int j = 0; j < n; ++j) {
    ...
    if (RealScalar(diag) <= 0) {
      std::cerr << "\nNegative diagonal during Incomplete factorization... "
                << j << "\n";
...
inline void IncompleteCholesky<Scalar, _UpLo, OrderingType>::updateList(
    const IdxType& colPtr, IdxType& rowIdx, SclType& vals, const Index& col,
