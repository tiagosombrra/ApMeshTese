...
class MatrixFunction<MatrixType, AtomicType, 0> {
 ...
 private:
  typename internal::nested<MatrixType>::type
      m_A;              /**< \brief Reference to argument of matrix function. */
  AtomicType& m_atomic; /**< \brief Class for computing matrix function of
                           atomic blocks. */
...
class MatrixFunction<MatrixType, AtomicType, 1> {
 ...
 private:
  ...
  DynMatrixType solveTriangularSylvester(const DynMatrixType& A,
  ...
  ListOfClusters m_clusters; /**< \brief Partition of eigenvalues into clusters
                                of ei'vals "close" to each other */
  DynamicIntVectorType
      m_eivalToCluster; /**< \brief m_eivalToCluster[i] = j means i-th ei'val is
                           in j-th cluster */
  DynamicIntVectorType
      m_clusterSize; /**< \brief Number of eigenvalues in each clusters  */
  DynamicIntVectorType m_blockStart; /**< \brief Row index at which block
                                        corresponding to i-th cluster starts */
  IntVectorType m_permutation; /**< \brief Permutation which groups ei'vals in
 ...
 */
template <typename MatrixType, typename AtomicType>
MatrixFunction<MatrixType, AtomicType, 1>::MatrixFunction(const MatrixType& A,
                                                          AtomicType& atomic)
    : m_A(A), m_atomic(atomic) {
  /* empty body */
}
...
void MatrixFunction<MatrixType, AtomicType, 1>::partitionEigenvalues() {
  ...
  for (Index i = 0; i < rows; ++i) {
    ...
    for (Index j = i + 1; j < rows; ++j) {
      if (abs(diag(j) - diag(i)) <= separation() &&
          std::find(qi->begin(), qi->end(), diag(j)) == qi->end()) {
 ...
 */
template <typename MatrixType, typename AtomicType>
typename MatrixFunction<MatrixType, AtomicType, 1>::ListOfClusters::iterator
MatrixFunction<MatrixType, AtomicType, 1>::findCluster(Scalar key) {
  typename Cluster::iterator j;
  for (typename ListOfClusters::iterator i = m_clusters.begin();
       i != m_clusters.end(); ++i) {
...
void MatrixFunction<MatrixType, AtomicType, 1>::computeClusterSize() {
  ...
  for (typename ListOfClusters::const_iterator cluster = m_clusters.begin();
       cluster != m_clusters.end(); ++cluster) {
    for (Index i = 0; i < diag.rows(); ++i) {
      if (std::find(cluster->begin(), cluster->end(), diag(i)) !=
          cluster->end()) {
...
/** \brief Permute Schur decomposition in #m_U and #m_T according to
 * #m_permutation */
template <typename MatrixType, typename AtomicType>
...
/** \brief Swap rows \a index and \a index+1 in Schur decomposition in #m_U and
 * #m_T */
template <typename MatrixType, typename AtomicType>
void MatrixFunction<MatrixType, AtomicType, 1>::swapEntriesInSchur(
    Index index) {
  JacobiRotation<Scalar> rotation;
  rotation.makeGivens(m_T(index, index + 1),
                      m_T(index + 1, index + 1) - m_T(index, index));
 ...
 * of #m_T, with the blocking given by #m_blockStart. The matrix function of
 * each diagonal block is computed by #m_atomic. The off-diagonal parts of #m_fT
 * are set to zero.
 */
template <typename MatrixType, typename AtomicType>
 ...
 */
template <typename MatrixType, typename AtomicType>
Block<MatrixType> MatrixFunction<MatrixType, AtomicType, 1>::block(
    MatrixType& A, Index i, Index j) {
  return A.block(m_blockStart(i), m_blockStart(j), m_clusterSize(i),
                 m_clusterSize(j));
}
 ...
 * equals the matrix function applied to #m_T) has already been computed and
 * computes the part above the block diagonal. The part below the diagonal is
...
void MatrixFunction<MatrixType, AtomicType, 1>::computeOffDiagonal() {
  for (Index diagIndex = 1; diagIndex < m_clusterSize.rows(); diagIndex++) {
    for (Index blockIndex = 0; blockIndex < m_clusterSize.rows() - diagIndex;
         blockIndex++) {
      ...
      DynMatrixType C = block(m_fT, blockIndex, blockIndex) *
                        block(m_T, blockIndex, blockIndex + diagIndex);
      C -= block(m_T, blockIndex, blockIndex + diagIndex) *
      ...
      block(m_fT, blockIndex, blockIndex + diagIndex) =
          solveTriangularSylvester(A, B, C);
    }
  }
}
 ...
 */
template <typename MatrixType, typename AtomicType>
typename MatrixFunction<MatrixType, AtomicType, 1>::DynMatrixType
MatrixFunction<MatrixType, AtomicType, 1>::solveTriangularSylvester(
    const DynMatrixType& A, const DynMatrixType& B, const DynMatrixType& C) {
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::sin() const {
  ...
  return MatrixFunctionReturnValue<Derived>(
      derived(), StdStemFunctions<ComplexScalar>::sin);
}
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::cos() const {
  ...
  typedef typename internal::stem_function<Scalar>::ComplexScalar ComplexScalar;
  return MatrixFunctionReturnValue<Derived>(
      derived(), StdStemFunctions<ComplexScalar>::cos);
}
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::sinh() const {
  ...
  typedef typename internal::stem_function<Scalar>::ComplexScalar ComplexScalar;
  return MatrixFunctionReturnValue<Derived>(
      derived(), StdStemFunctions<ComplexScalar>::sinh);
}
...
const MatrixFunctionReturnValue<Derived> MatrixBase<Derived>::cosh() const {
  ...
  typedef typename internal::stem_function<Scalar>::ComplexScalar ComplexScalar;
  return MatrixFunctionReturnValue<Derived>(
      derived(), StdStemFunctions<ComplexScalar>::cosh);
}
