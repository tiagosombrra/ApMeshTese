...
class MatrixPowerAtomic {
 private:
  ...
  typedef typename MatrixType::Index Index;
  typedef Array<Scalar, RowsAtCompileTime, 1, ColMajor, MaxRowsAtCompileTime>
      ArrayType;
...
template <typename MatrixType>
void MatrixPowerAtomic<MatrixType>::compute2x2(MatrixType& res,
                                               RealScalar p) const {
  using std::abs;
  using std::pow;
...
void MatrixPowerAtomic<MatrixType>::computeBig(MatrixType& res) const {
  ...
  const RealScalar maxNormForPade =
      digits <= 24 ? 4.3386528e-1f :  // sigle precision
          digits <= 53 ? 2.789358995219730e-1
                       :  // double precision
          digits <= 64 ? 2.4471944416607995472e-1L
                       :  // extended precision
          digits <= 106 ? 1.1016843812851143391275867258512e-1L
                        :                           // double-double
...
inline int MatrixPowerAtomic<MatrixType>::getPadeDegree(
  ...
  const double maxNormForPade[] = {6.3854693117491799460e-3L /* degree = 3 */,
...
MatrixPowerAtomic<MatrixType>::computeSuperDiag(const ComplexScalar& curr,
  ...
  int unwindingNumber =
      std::ceil((numext::imag(logCurr - logPrev) - M_PI) / (2 * M_PI));
  ComplexScalar w = numext::atanh2(curr - prev, curr + prev) +
                    ComplexScalar(0, M_PI * unwindingNumber);
  return RealScalar(2) * std::exp(RealScalar(0.5) * p * (logCurr + logPrev)) *
         std::sinh(p * w) / (curr - prev);
}
...
MatrixPowerAtomic<MatrixType>::computeSuperDiag(RealScalar curr,
  ...
  return 2 * std::exp(p * (std::log(curr) + std::log(prev)) / 2) *
         std::sinh(p * w) / (curr - prev);
}
...
class MatrixPower {
 ...
 private:
  ...
  typedef Matrix<ComplexScalar, RowsAtCompileTime, ColsAtCompileTime,
                 MatrixType::Options, MaxRowsAtCompileTime,
...
MatrixPower<MatrixType>::modfAndInit(RealScalar x, RealScalar* intpart) {
  typedef Array<RealScalar, RowsAtCompileTime, 1, ColMajor,
  ...
  *intpart = std::floor(x);
  ...
  if (res > RealScalar(0.5) &&
      res > (1 - res) * std::pow(m_conditionNumber, res)) {
    --res;
    ++*intpart;
  }
  return res;
...
void MatrixPower<MatrixType>::computeIntPower(ResultType& res, RealScalar p) {
  ...
  while (pp >= 1) {
    if (std::fmod(pp, 2) >= 1) res = m_tmp * res;
    m_tmp *= m_tmp;
    pp /= 2;
  }
}
