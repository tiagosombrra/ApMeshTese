 ...
 * decomposition We plan to have a very similar interface to JacobiSVD on this
 * class. It should be used to speed up the calcul of SVD for big matrices.
 */
...
class BDCSVD : public SVDBase<_MatrixType> {
 ...
 public:
   ...
   */
  BDCSVD() : SVDBase<_MatrixType>::SVDBase(), algoswap(ALGOSWAP) {}
  ...
  BDCSVD(Index rows, Index cols, unsigned int computationOptions = 0)
      : SVDBase<_MatrixType>::SVDBase(), algoswap(ALGOSWAP) {
  ...
  inline const internal::solve_retval<BDCSVD, Rhs> solve(
      const MatrixBase<Rhs>& b) const {
    eigen_assert(this->m_isInitialized && "BDCSVD is not initialized.");
    eigen_assert(SVDBase<_MatrixType>::computeU() &&
                 SVDBase<_MatrixType>::computeV() &&
                 "BDCSVD::solve() requires both unitaries U and V to be "
 ...
 private:
  ...
  void copyUV(MatrixXr naiveU, MatrixXr naiveV, MatrixX householderU,
...
void BDCSVD<MatrixType>::allocate(Index rows, Index cols,
  ...
  if (compU)
    m_naiveU = MatrixXr::Zero(this->m_diagSize + 1, this->m_diagSize + 1);
  else
    m_naiveU = MatrixXr::Zero(2, this->m_diagSize + 1);
...
BDCSVD<Matrix<int, Dynamic, Dynamic> >::compute(
  ...
  if (this->m_computeFullU)
    this->m_matrixU = Matrix<int, Dynamic, Dynamic>::Zero(rows(), rows());
  if (this->m_computeFullV)
    this->m_matrixV = Matrix<int, Dynamic, Dynamic>::Zero(cols(), cols());
...
template <typename MatrixType>
SVDBase<MatrixType>& BDCSVD<MatrixType>::compute(
    const MatrixType& matrix, unsigned int computationOptions) {
  allocate(matrix.rows(), matrix.cols(), computationOptions);
  using std::abs;
  ...
  m_computed(this->m_diagSize - 1, this->m_diagSize - 1) =
      temp(this->m_diagSize - 1, this->m_diagSize - 1);
...
template <typename MatrixType>
void BDCSVD<MatrixType>::copyUV(MatrixXr naiveU, MatrixXr naiveV,
                                MatrixX householderU, MatrixX householderV) {
  if (this->computeU()) {
    MatrixX temp = MatrixX::Zero(naiveU.rows(), naiveU.cols());
    temp.real() = naiveU;
    if (this->m_computeThinU) {
      this->m_matrixU =
          MatrixX::Identity(householderU.cols(), this->m_nonzeroSingularValues);
      this->m_matrixU.block(0, 0, this->m_diagSize,
                            this->m_nonzeroSingularValues) =
          temp.block(0, 0, this->m_diagSize, this->m_nonzeroSingularValues);
      this->m_matrixU = householderU * this->m_matrixU;
    } else {
      this->m_matrixU =
          MatrixX::Identity(householderU.cols(), householderU.cols());
      this->m_matrixU.block(0, 0, this->m_diagSize, this->m_diagSize) =
          temp.block(0, 0, this->m_diagSize, this->m_diagSize);
      this->m_matrixU = householderU * this->m_matrixU;
    }
  }
  if (this->computeV()) {
    ...
    if (this->m_computeThinV) {
      ...
      this->m_matrixV.block(0, 0, this->m_nonzeroSingularValues,
                            this->m_nonzeroSingularValues) =
          temp.block(0, 0, this->m_nonzeroSingularValues,
                     this->m_nonzeroSingularValues);
      this->m_matrixV = householderV * this->m_matrixV;
    } else {
      this->m_matrixV =
          MatrixX::Identity(householderV.cols(), householderV.cols());
      this->m_matrixV.block(0, 0, this->m_diagSize, this->m_diagSize) =
          temp.block(0, 0, this->m_diagSize, this->m_diagSize);
      this->m_matrixV = householderV * this->m_matrixV;
    }
  }
...
void BDCSVD<MatrixType>::divide(Index firstCol, Index lastCol, Index firstRowW,
  ...
  if (n < algoswap) {
    ...
    for (int i = 0; i < n; i++) {
      m_computed(firstCol + shift + i, firstCol + shift + i) =
          b.singularValues().coeffRef(i);
    }
    return;
  }
  ...
  // the line below do the deflation of the matrix for the third part of the
  // algorithm Here the deflation is commented because the third part of the
  // algorithm is not implemented the third part of the algorithm is a fast SVD
  // on the matrix m_computed which works thanks to the deflation

  deflation(firstCol, lastCol, k, firstRowW, firstColW, shift);

  // Third part of the algorithm, since the real third part of the algorithm is
  // not implemeted we use a JacobiSVD
  JacobiSVD<MatrixXr> res = JacobiSVD<MatrixXr>(
      m_computed.block(firstCol + shift, firstCol + shift, n + 1, n),
      ComputeFullU | (ComputeFullV * compV));
  if (compU)
    m_naiveU.block(firstCol, firstCol, n + 1, n + 1) *= res.matrixU();
  else
    m_naiveU.block(0, firstCol, 2, n + 1) *= res.matrixU();

  if (compV) m_naiveV.block(firstRowW, firstColW, n, n) *= res.matrixV();
  m_computed.block(firstCol + shift, firstCol + shift, n, n)
      << MatrixXr::Zero(n, n);
  for (int i = 0; i < n; i++)
    m_computed(firstCol + shift + i, firstCol + shift + i) =
        res.singularValues().coeffRef(i);
...
void BDCSVD<MatrixType>::deflation(Index firstCol, Index lastCol, Index k,
  ...
  // condition 4.1
  RealScalar EPS =
      EPSILON * (std::max<RealScalar>(
                    m_computed(firstCol + shift + 1, firstCol + shift + 1),
                    m_computed(firstCol + k, firstCol + k)));
  ...
  for (int i = 1; i < length - 1; i++) {
    ...
    if (compV) {
      ...
      m_naiveV.col(CWI).segment(firstRowW, length)
          << m_naiveV.col(CWJ).segment(firstRowW, length);
  ...
  for (Index i = firstCol + shift + 1; i < lastCol + shift; i++) {
    if ((m_computed(i + 1, i + 1) - m_computed(i, i)) < EPS) {
      deflation44(firstCol, firstCol + shift, firstRowW, firstColW, i - Zero,
                  i + 1 - Zero, length);
    }
  }
  delete[] permutation;
...
struct solve_retval<BDCSVD<_MatrixType>, Rhs>
  ...
  void evalTo(Dest& dst) const {
    ...
    invertedSingVals.head(nonzeroSingVals) =
        dec().singularValues().head(nonzeroSingVals).array().inverse();
    invertedSingVals.tail(diagSize - nonzeroSingVals).setZero();

    dst = dec().matrixV().leftCols(diagSize) * invertedSingVals.asDiagonal() *
          dec().matrixU().leftCols(diagSize).adjoint() * rhs();
    return;
  }
};
