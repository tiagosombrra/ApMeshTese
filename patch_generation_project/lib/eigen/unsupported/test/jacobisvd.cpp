...
void jacobisvd_compare_to_full(
  ...
  svd_compare_to_full<MatrixType, JacobiSVD<MatrixType, QRPreconditioner> >(
      m, computationOptions, referenceSvd);
}

template <typename MatrixType, int QRPreconditioner>
void jacobisvd_solve(const MatrixType& m, unsigned int computationOptions) {
  svd_solve<MatrixType, JacobiSVD<MatrixType, QRPreconditioner> >(
...
void jacobisvd_test_all_computation_options(const MatrixType& m) {
  ...
  svd_test_computation_options_1<MatrixType,
                                 JacobiSVD<MatrixType, QRPreconditioner> >(
      m, fullSvd);

  if (QRPreconditioner == FullPivHouseholderQRPreconditioner) return;
  svd_test_computation_options_2<MatrixType,
                                 JacobiSVD<MatrixType, QRPreconditioner> >(
      m, fullSvd);
}
...
void jacobisvd(const MatrixType& a = MatrixType(), bool pickrandom = true) {
  ...
  jacobisvd_test_all_computation_options<MatrixType,
                                         FullPivHouseholderQRPreconditioner>(m);
  jacobisvd_test_all_computation_options<MatrixType,
                                         ColPivHouseholderQRPreconditioner>(m);
  jacobisvd_test_all_computation_options<MatrixType,
