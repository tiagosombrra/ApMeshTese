...
// Options
// FIXME HAVE_INT64_SUPPORT leads to clashes with long int and int64_t on some
// systems.
//#define MPREAL_HAVE_INT64_SUPPORT               // Enable int64_t support if
// possible. Available only for MSVC 2010 & GCC.
...
#undef MPFR_USE_INTMAX_T  // Since we cannot detect this, disable x64 by default
    ...
    defined(__x86_64) || defined(__ia64) || defined(__itanium__) ||  \
    defined(_M_IA64) || defined(__PPC64__)
#undef MPREAL_HAVE_INT64_SUPPORT  // Remove all shaman dances for x64 builds
                                  // since
#undef MPFR_USE_INTMAX_T  // GCC already supports x64 as of "long int" is 64-bit
...
#if defined(MPREAL_HAVE_INT64_SUPPORT)
  mpreal(const uint64_t u, mp_prec_t prec = mpreal::get_default_prec(),
         mp_rnd_t mode = mpreal::get_default_rnd());
  mpreal(const int64_t u, mp_prec_t prec = mpreal::get_default_prec(),
         mp_rnd_t mode = mpreal::get_default_rnd());
#endif
...
inline mpreal::mpreal(mpreal&& other) {
  mpfr_set_uninitialized(
      mpfr_ptr());  // make sure "other" holds no pinter to actual data
...
//////////////////////////////////////////////////////////////////////////
// Type Converters
inline bool mpreal::toBool(mp_rnd_t /*mode*/) const {
...
#if defined(MPREAL_HAVE_INT64_SUPPORT)
inline int64_t mpreal::toInt64(mp_rnd_t mode) const {
  return mpfr_get_sj(mpfr_srcptr(), mode);
}
inline uint64_t mpreal::toUInt64(mp_rnd_t mode) const {
...
inline const mpreal sum(const mpreal tab[], unsigned long int n,
                        mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
  mpreal x;
  mpfr_ptr* t;
  unsigned long int i;
